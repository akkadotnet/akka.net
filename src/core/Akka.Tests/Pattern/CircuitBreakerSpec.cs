...
public class ASynchronousCircuitBreakerThatIsClosed : CircuitBreakerSpecBase
{
    ...
    [Fact(DisplayName =
        "A synchronous circuit breaker that is closed must invoke onOpen if call fails and breaker transits to open state")]
    ...
    [Fact(DisplayName =
        "A synchronous circuit breaker that is closed must increment failure count on callTimeout before call finishes")]
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        await WithinAsync(TimeSpan.FromMilliseconds(900) + epsilon,
            () => AwaitConditionAsync(() => breaker.Instance.CurrentFailureCount == 1,
                Dilated(TimeSpan.FromMilliseconds(100)), TimeSpan.FromMilliseconds(100)));
    }
}
...
{
    [Fact(DisplayName =
        "A synchronous circuit breaker that is half open must pass through next call and close on success")]
...
{
    [Fact(DisplayName =
        "A synchronous circuit breaker that is open must throw exceptions when called before reset timeout")]
    ...
    [Fact(DisplayName =
        "A synchronous circuit breaker that is open must still be in open state after calling success method")]
    ...
    [Fact(DisplayName =
        "A synchronous circuit breaker that is open must still be in open state after calling fail method")]
...
public class AnAsynchronousCircuitBreakerThatIsClosed : CircuitBreakerSpecBase
{
    ...
    [Fact(DisplayName =
        "An asynchronous circuit breaker that is closed must invoke onOpen if call fails and breaker transits to open state")]
    public void Must_invoke_onOpen_if_call_fails_and_breaker_transits_to_open_state()
    {
        var breaker = ShortCallTimeoutCb();
        _ = breaker.Instance.WithCircuitBreaker(() => Task.Run(ThrowException));
...
{
    [Fact(DisplayName =
        "An asynchronous circuit breaker that is half open must pass through next call and close on success")]
...
{
    [Fact(DisplayName =
        "An asynchronous circuit breaker that is open must throw exceptions when called before reset timeout")]
    ...
    [Fact(DisplayName =
        "An asynchronous circuit breaker that is open must increase the reset timeout after it transits to open again")]
...
public class CircuitBreakerSpecBase : AkkaSpec
{
    ...
    {
        return Intercept<T>(actionThatThrows);
    }
    ...
    {
        return new TestBreaker(new CircuitBreaker(Sys.Scheduler, 1, Dilated(TimeSpan.FromMilliseconds(50)),
            Dilated(TimeSpan.FromMilliseconds(500))));
    }

    public TestBreaker ShortResetTimeoutCb()
    {
        return new TestBreaker(new CircuitBreaker(Sys.Scheduler, 1, Dilated(TimeSpan.FromMilliseconds(1000)),
            Dilated(TimeSpan.FromMilliseconds(50))));
    }

    public TestBreaker LongCallTimeoutCb()
    {
        return new TestBreaker(new CircuitBreaker(Sys.Scheduler, 1, TimeSpan.FromSeconds(5),
            Dilated(TimeSpan.FromMilliseconds(500))));
    }

    public TestBreaker LongResetTimeoutCb()
    {
        return new TestBreaker(new CircuitBreaker(Sys.Scheduler, 1, Dilated(TimeSpan.FromMilliseconds(100)),
            Dilated(LongResetTimeout)));
    }

    public TestBreaker MultiFailureCb()
    {
        return new TestBreaker(new CircuitBreaker(Sys.Scheduler, 5, Dilated(TimeSpan.FromMilliseconds(200)),
            Dilated(TimeSpan.FromMilliseconds(500))));
    }

    public TestBreaker NonOneFactorCb()
    {
        return new TestBreaker(new CircuitBreaker(Sys.Scheduler, 1, Dilated(TimeSpan.FromMilliseconds(2000)),
            Dilated(TimeSpan.FromMilliseconds(1000)), Dilated(TimeSpan.FromDays(1)), 5, 0));
    }
}
