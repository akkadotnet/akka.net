...
public class TcpInboundOnlySpec
{
    ...
    [PerfBenchmark(
        Description =
            "Measures how quickly and with how much GC overhead an actor server can receive 50B messages from client socket",
    ...
    public void TcpChannel_InboundOnly_Throughput(BenchmarkContext context)
    {
        Task.Run(async () =>
        {
            ...
            {
                await stream.WriteAsync(message, 0, message.Length, cancel.Token);
