        ...
        switch (activeStrategy)
        {
            ...
            default:
                throw new ArgumentException(
                    $"`akka.cluster.split-brain-resolver.active-strategy` setting not recognized: [{activeStrategy}]. Available options are: static-quorum, keep-majority, keep-oldest, keep-referee.");
        }
    }
}
...
/// <summary>
///     A split brain resolver strategy used to determine which nodes should be downed when a network partition has been
///     detected.
/// </summary>
...
internal sealed class StaticQuorum : ISplitBrainStrategy
{
    ...
    {
        return $"StaticQuorum(quorumSize: {QuorumSize}, role: '{Role}')";
    }
}
...
internal sealed class KeepMajority : ISplitBrainStrategy
{
    ...
    private ImmutableSortedSet<Member> MembersWithRole(ImmutableSortedSet<Member> members)
    {
...
internal sealed class KeepOldest : ISplitBrainStrategy
{
    public KeepOldest(Config config) : this(
        config.GetBoolean("down-if-alone", true),
        config.GetString("role"))
    {
    }
    ...
    public IEnumerable<Member> Apply(NetworkPartitionContext context)
    {
        ...
        if (remaining.Contains(oldest))
            return DownIfAlone && context.Remaining.Count == 1 &&
                   context.Unreachable.Count >
                   0 // oldest is current node, and it's alone, but not the only node in the cluster
                ? context.Remaining
                : context.Unreachable;
    ...
    public override string ToString()
    {
        return $"KeepOldest(downIfAlone: {DownIfAlone}, role: '{Role}')";
    }
}
...
internal sealed class KeepReferee : ISplitBrainStrategy
{
    public KeepReferee(Config config) : this(
        Address.Parse(config.GetString("address")),
        config.GetInt("down-all-if-less-than-nodes", 1))
    {
    }
    ...
    public IEnumerable<Member> Apply(NetworkPartitionContext context)
    {
        ...
        if (context.Remaining.Count < DownAllIfLessThanNodes)
            return
                context.Remaining.Union(context
                    .Unreachable); // referee is reachable but there are too few remaining nodes
        return context.Unreachable;
    }

    public override string ToString()
    {
        return $"KeepReferee(refereeAddress: {Address}, downIfLessThanNodes: {DownAllIfLessThanNodes})";
    }
}
...
internal sealed class SplitBrainDecider : UntypedActor
{
    ...
    public static Props Props(TimeSpan stableAfter, ISplitBrainStrategy strategy, Cluster cluster)
    {
        return Actor.Props.Create(() => new SplitBrainDecider(stableAfter, strategy, cluster))
    ...
    protected override void OnReceive(object message)
    {
        switch (message)
        {
            case ClusterEvent.CurrentClusterState state:
                ...
                _reachable = state.Members.Where(m => m.Status == MemberStatus.Up)
    ...
    private void HandleStabilityReached()
    {
        if (Log.IsInfoEnabled && _unreachable.Any())
            Log.Info("A network partition detected - unreachable nodes: [{0}], remaining: [{1}]",
                string.Join(", ", _unreachable.Select(m => m.Address)),
                string.Join(", ", _reachable.Select(m => m.Address)));
        ...
        if (nodesToDown.Length > 0)
        {
            if (Log.IsInfoEnabled)
                Log.Info("A network partition has been detected. {0} decided to down following nodes: [{1}]", _strategy,
                    string.Join(", ", nodesToDown));

            foreach (var member in nodesToDown) _cluster.Down(member.Address);
        }
    }
    ...
    private void ResetStabilityTimeout()
    {
        ...
        _stabilityTask = Context.System.Scheduler.ScheduleTellOnceCancelable(_stabilityTimeout, Self,
            StabilityReached.Instance, ActorRefs.NoSender);
    }
