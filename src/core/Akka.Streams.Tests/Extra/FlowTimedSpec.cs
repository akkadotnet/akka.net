...
public class FlowTimedSpec : ScriptedTest
{
    ...
    public async Task Timed_Source_must_measure_time_it_takes_between_elements_matching_a_predicate()
    {
        ...
        {
            return ScriptedTest.Script.Create(Enumerable.Range(1, n)
        ...
        var expectedNrOfOnIntervalCalls =
            testRuns.Length *
            (n / measureBetweenEvery - 1); // first time has no value to compare to, so skips calling onInterval
        foreach (var _ in Enumerable.Range(1, expectedNrOfOnIntervalCalls)) await testActor.ExpectMsgAsync<TimeSpan>();
    }
    ...
    public async Task Timed_Source_must_measure_time_it_takes_from_start_to_complete_by_wrapping_operations()
    {
        ...
        foreach (var _ in testRuns) await testActor.ExpectMsgAsync<TimeSpan>();
    ...
    public async Task Timed_Flow_must_measure_time_it_takes_from_start_to_complete_by_wrapping_operations()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            var (flowIn, flowOut) =
                flow.RunWith(Source.AsSubscriber<int>(), Sink.AsPublisher<string>(false), Materializer);
            ...
            sub.Request(200);
            foreach (var i in Enumerable.Range(0, 101)) await c1.ExpectNextAsync(i + "!");
            await c1.ExpectCompleteAsync();
