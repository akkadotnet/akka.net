...
public class ClusterDomainEventPublisherSpec : AkkaSpec
{
    ...
    private static readonly Member bExiting =
        TestMember.Create(new Address("akka.tcp", "sys", "b", 2552), MemberStatus.Exiting);

    private static readonly Member bRemoved = bExiting.Copy(MemberStatus.Removed);

    private static readonly Member cJoining = TestMember.Create(new Address("akka.tcp", "sys", "c", 2552),
        MemberStatus.Joining, ImmutableHashSet.Create("GRP"));
    ...
    private static readonly Member dUp = TestMember.Create(new Address("akka.tcp", "sys", "d", 2552), MemberStatus.Up,
        ImmutableHashSet.Create("GRP"));

    private static readonly Gossip g0 = new Gossip(ImmutableSortedSet.Create(aUp)).Seen(aUp.UniqueAddress);
    private static readonly MembershipState state0 = new(g0, aUp.UniqueAddress);

    private static readonly Gossip g1 = new Gossip(ImmutableSortedSet.Create(aUp, cJoining)).Seen(aUp.UniqueAddress)
        .Seen(cJoining.UniqueAddress);

    private static readonly MembershipState state1 = new(g1, aUp.UniqueAddress);

    private static readonly Gossip g2 =
        new Gossip(ImmutableSortedSet.Create(aUp, bExiting, cUp)).Seen(aUp.UniqueAddress);

    private static readonly MembershipState state2 = new(g2, aUp.UniqueAddress);
    private static readonly Gossip g3 = g2.Seen(bExiting.UniqueAddress).Seen(cUp.UniqueAddress);
    private static readonly MembershipState state3 = new(g3, aUp.UniqueAddress);

    private static readonly Gossip g4 =
        new Gossip(ImmutableSortedSet.Create(a51Up, aUp, bExiting, cUp)).Seen(aUp.UniqueAddress);

    private static readonly MembershipState state4 = new(g4, aUp.UniqueAddress);

    private static readonly Gossip g5 = new Gossip(ImmutableSortedSet.Create(a51Up, aUp, bExiting, cUp))
        .Seen(aUp.UniqueAddress).Seen(bExiting.UniqueAddress).Seen(cUp.UniqueAddress);

    private static readonly MembershipState state5 = new(g5, aUp.UniqueAddress);

    private static readonly Gossip g6 =
        new Gossip(ImmutableSortedSet.Create(aLeaving, bExiting, cUp)).Seen(aUp.UniqueAddress);

    private static readonly MembershipState state6 = new(g6, aUp.UniqueAddress);

    private static readonly Gossip g7 =
        new Gossip(ImmutableSortedSet.Create(aExiting, bExiting, cUp)).Seen(aUp.UniqueAddress);

    private static readonly MembershipState state7 = new(g7, aUp.UniqueAddress);

    private static readonly Gossip g8 = new Gossip(ImmutableSortedSet.Create(aUp, bExiting, cUp, dUp),
            new GossipOverview(Reachability.Empty.Unreachable(aUp.UniqueAddress, dUp.UniqueAddress)))
        .Seen(aUp.UniqueAddress);

    private static readonly MembershipState state8 = new(g8, aUp.UniqueAddress);

    private static readonly MembershipState _emptyMembershipState = new(Gossip.Empty, aUp.UniqueAddress);
    ...
    public async Task ClusterDomainEventPublisher_must_publish_role_leader_changed()
    {
        ...
        _publisher.Tell(new InternalClusterAction.Subscribe(subscriber.Ref,
            ClusterEvent.SubscriptionInitialStateMode.InitialStateAsSnapshot,
            ImmutableHashSet.Create(typeof(ClusterEvent.RoleLeaderChanged))));
        await subscriber.ExpectMsgAsync<ClusterEvent.CurrentClusterState>();
        _publisher.Tell(new InternalClusterAction.PublishChanges(
            new MembershipState(new Gossip(ImmutableSortedSet.Create(cJoining, dUp)), dUp.UniqueAddress)));
        await subscriber.ExpectMsgAsync(new ClusterEvent.RoleLeaderChanged("GRP", dUp.Address));
        _publisher.Tell(new InternalClusterAction.PublishChanges(
            new MembershipState(new Gossip(ImmutableSortedSet.Create(cUp, dUp)), dUp.UniqueAddress)));
    ...
    public async Task ClusterDomainEventPublisher_must_send_CurrentClusterState_when_subscribe()
    {
        ...
        _publisher.Tell(new InternalClusterAction.Subscribe(subscriber.Ref,
            ClusterEvent.SubscriptionInitialStateMode.InitialStateAsSnapshot,
            ImmutableHashSet.Create(typeof(ClusterEvent.IClusterDomainEvent))));
        await subscriber.ExpectMsgAsync<ClusterEvent.CurrentClusterState>();
    ...
    public async Task ClusterDomainEventPublisher_must_send_events_corresponding_to_current_state_when_subscribe()
    {
        ...
        _publisher.Tell(new InternalClusterAction.Subscribe(subscriber.Ref,
            ...
            ImmutableHashSet.Create(typeof(ClusterEvent.IMemberEvent), typeof(ClusterEvent.ReachabilityEvent))));
    ...
    public async Task ClusterDomainEventPublisher_should_support_unsubscribe()
    {
        ...
        _publisher.Tell(new InternalClusterAction.Subscribe(subscriber.Ref,
            ClusterEvent.SubscriptionInitialStateMode.InitialStateAsSnapshot,
            ImmutableHashSet.Create(typeof(ClusterEvent.IMemberEvent))));
        await subscriber.ExpectMsgAsync<ClusterEvent.CurrentClusterState>();
    ...
    public async Task ClusterDomainEventPublisher_must_publish_seen_changed()
    {
        ...
        _publisher.Tell(new InternalClusterAction.Subscribe(subscriber.Ref,
            ClusterEvent.SubscriptionInitialStateMode.InitialStateAsSnapshot,
            ImmutableHashSet.Create(typeof(ClusterEvent.SeenChanged))));
        await subscriber.ExpectMsgAsync<ClusterEvent.CurrentClusterState>();
        _publisher.Tell(new InternalClusterAction.PublishChanges(state2));
