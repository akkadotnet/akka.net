    ...
    IEnumerable<KeyValuePair<TKey, BigInteger>>, IPNCounterDictionary
{
    ...
    {
        return new PNCounterDictionary<TKey>(Underlying.Merge(other.Underlying));
    }

    public IReplicatedData Merge(IReplicatedData other)
    {
        return Merge((PNCounterDictionary<TKey>)other);
    }


    public IEnumerator<KeyValuePair<TKey, BigInteger>> GetEnumerator()
    {
        return Underlying.Select(x => new KeyValuePair<TKey, BigInteger>(x.Key, x.Value.Value)).GetEnumerator();
    }
    ...
    {
        return new PNCounterDictionary<TKey>(Underlying.Prune(removedNode, collapseInto));
    }

    public PNCounterDictionary<TKey> PruningCleanup(UniqueAddress removedNode)
    {
        return new PNCounterDictionary<TKey>(Underlying.PruningCleanup(removedNode));
    }
    ...
    {
        return Increment(node.SelfUniqueAddress, key, delta);
    }
    ...
    {
        return new PNCounterDictionary<TKey>(Underlying.AddOrUpdate(node, key, PNCounter.Empty,
            old => old.Increment(node, delta)));
    }
    ...
    {
        return Decrement(node.SelfUniqueAddress, key, delta);
    }
    ...
    {
        return new PNCounterDictionary<TKey>(Underlying.Remove(node, key));
    }


    public override bool Equals(object obj)
    {
        return obj is PNCounterDictionary<TKey> pairs && Equals(pairs);
    }
    ...
    internal sealed class PNCounterDictionaryDelta : ORDictionary<TKey, PNCounter>.IDeltaOperation,
        IReplicatedDeltaSize, IPNCounterDictionaryDeltaOperation
    {
        ...
        ORDictionary.IDeltaOperation IPNCounterDictionaryDeltaOperation.Underlying =>
        ...
        public IReplicatedData Merge(IReplicatedData other)
        {
            if (other is PNCounterDictionaryDelta d)
                return new PNCounterDictionaryDelta(
                    (ORDictionary<TKey, PNCounter>.IDeltaOperation)Underlying.Merge(d.Underlying));
        ...
        public override bool Equals(object obj)
        {
            return obj is PNCounterDictionaryDelta operation && Equals(operation.Underlying);
        }
    ...
    {
        return new PNCounterDictionary<TKey>(Underlying.ResetDelta());
    }
