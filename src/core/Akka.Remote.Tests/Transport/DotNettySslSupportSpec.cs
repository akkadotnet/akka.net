...
public class DotNettySslSupportSpec : AkkaSpec
{
    ...
    [Fact]
    public async Task
        If_EnableSsl_configuration_is_true_but_not_valid_certificate_password_is_provided_than_WindowsCryptographicException_should_be_thrown()
    {
        // skip this test due to linux/mono certificate issues
        if (IsMono) return;
    ...
    public void
        If_EnableSsl_configuration_is_false_than_no_exception_should_be_thrown_even_no_cert_detail_were_provided(
    ...
    private void RemoveCert()
    {
        using (var store = new X509Store("My", StoreLocation.CurrentUser))
        {
            ...
            if (certs.Count > 0) store.Remove(certs[0]);
        }
    }
