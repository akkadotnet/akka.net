...
internal sealed class ShardsQueryResult<T>
{
    ...
    public override string ToString()
    {
        ...
        return
            $"Queried [{Queried}] {shardsOf} [{Responses.Count}] responsive, [{Failed.Count}] failed after {Timeout}.";
    }
    ...
    /// </summary>
    /// <param name="ps">
    ///     the results of actors queried that did not reply by
    ...
    /// <returns></returns>
    public static ShardsQueryResult<T> Create(IImmutableList<(ShardId shard, Task<T> task)> ps, int total,
        TimeSpan timeout)
    {
        return new ShardsQueryResult<T>(
            ps.Where(i => i.task.IsCanceled || i.task.IsFaulted).Select(i => i.shard).ToImmutableHashSet(),
            ps.Where(i => !i.task.IsCanceled && !i.task.IsFaulted).Select(i => i.task.Result).ToImmutableList(),
            total, timeout);
    }
}