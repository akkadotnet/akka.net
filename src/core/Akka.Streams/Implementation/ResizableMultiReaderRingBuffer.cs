...
{
    public DistinctRetainingMultiReaderBuffer(long initialSize, long maxSize, ICursors cursors) : base(initialSize,
        maxSize, cursors)
    {
    }
    ...
    {
        return $"DistinctRetainingMultiReaderBuffer(size={Length}, cursors={Cursors.Cursors.Count()})";
    }
}
...
public class RetainingMultiReaderBuffer<T> : IStreamBuffer<T>
{
    ...
    public virtual bool Write(T value)
    {
        ...
        var newLength = Buffer.LongLength << 1;
        if (newLength < 0)
    ...
    public override string ToString()
    {
        return $"RetainingMultiReaderBuffer(size={Length}, cursors={Cursors.Cursors.Count()})";
    }
}
...
public class ResizableMultiReaderRingBuffer<T> : IStreamBuffer<T>
{
    ...
    ///     <see cref="Int32Extensions.NumberOfTrailingZeros" />
    ...
    public bool Write(T value)
    {
        ...
        if (LengthBit < _maxSizeBit)
        {
            ...
            var newLength = UnderlyingArray.LongLength << 1;
            if (newLength < 0)
                newLength = long.MaxValue;
            var newArray = new T[newLength];
    ...
    {
        return
            $"ResizableMultiReaderRingBuffer(size={Length}, writeIx={_writeIndex}, readIx={_readIndex}, cursors={Cursors.Cursors.Count()})";
    }
}