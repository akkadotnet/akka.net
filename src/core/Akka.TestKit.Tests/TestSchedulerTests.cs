...
{
    private readonly IActorRef _testReceiveActor;
    ...
    public async Task Delivers_message_when_scheduled_time_reached()
    {
        ...
        await _testReceiveActor.Ask<ActorIdentity>(new Identify(null),
            RemainingOrDefault); // verify that the ActorCell has started
    ...
    private class TestReceiveActor : ReceiveActor
    {
        ...
        public TestReceiveActor()
        {
            ...
            Receive<CancelableMessage>(x =>
            {
                ...
                Context.System.Scheduler.ScheduleTellRepeatedly(x.ScheduleOffset, x.ScheduleOffset, Sender, x, Self,
                    _cancelable);
            });

            Receive<CancelMessage>(_ => { _cancelable.Cancel(); });
        }
    }
