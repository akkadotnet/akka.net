...
namespace Akka.MultiNodeTestRunner.Shared.Sinks
{
    ...
    public class ConsoleMessageSinkActor : TestCoordinatorEnabledMessageSink
    {
        ...
        protected override void HandleTestRunTree(TestRunTree tree)
        {
            ...
            WriteSpecMessage(string.Format("Test run completed in [{0}] with {1}/{2} specs passed.", tree.Elapsed,
                passedSpecs, tree.Specs.Count()));
        ...
        {
            WriteSpecMessage(string.Format("Beginning spec {0}.{1} on {2} nodes", newSpec.ClassName, newSpec.MethodName,
                newSpec.Nodes.Count));
        ...
        private void WriteSpecPass(int nodeIndex, string message)
        {
            ...
            Console.WriteLine("[NODE{0}][{1}]: SPEC PASSED: {2}", nodeIndex, DateTime.UtcNow.ToShortTimeString(),
                message);
            Console.ResetColor();
        }
        ...
        private void WriteSpecFail(int nodeIndex, string message)
        {
            ...
            Console.WriteLine("[NODE{0}][{1}]: SPEC FAILED: {2}", nodeIndex, DateTime.UtcNow.ToShortTimeString(),
                message);
            Console.ResetColor();
        }
