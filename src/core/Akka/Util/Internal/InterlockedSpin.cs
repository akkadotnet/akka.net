...
public static class InterlockedSpin
{
    /// <summary>
    ///     INTERNAL!
    ///     Atomically updates the object <paramref name="reference" /> by calling <paramref name="updater" /> to get the new
    ///     value.
    ...
    /// <summary>
    ///     INTERNAL!
    ///     Atomically updates the int <paramref name="reference" /> by calling <paramref name="updateIfTrue" /> to get the new
    ///     value.
    ...
    public static TReturn ConditionallySwap<T, TReturn>(ref T reference, Func<T, (bool, T, TReturn)> updateIfTrue)
        where T : class
    {
        var spinWait = new SpinWait();
        while (true)
