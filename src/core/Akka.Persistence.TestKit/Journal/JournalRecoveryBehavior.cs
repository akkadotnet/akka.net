...
public class JournalRecoveryBehavior
{
    ...
    public Task Pass()
    {
        return SetInterceptorAsync(JournalInterceptors.Noop.Instance);
    }
    ...
    /// <remarks>
    ///     <para>
    ///         Journal will crash and <see cref="Eventsourced.OnPersistFailure">UntypedPersistentActor.OnPersistFailure</see>
    ///         will be called on persistent actor.
    ///     </para>
    ///     <para>
    ...
    public Task Fail()
    {
        return SetInterceptorAsync(JournalInterceptors.Failure.Instance);
    }
    ...
    ///         <list type="bullet">
    ///             <item>
    ///                 If <typeparamref name="TMessage" /> is interface, journal will fail when message implements that
    ///                 interface.
    ///             </item>
    ///             <item>
    ///                 If <typeparamref name="TMessage" /> is class, journal will fail when message can be assigned to
    ...
    {
        return FailOnType(typeof(TMessage));
    }
    ...
    ///         <list type="bullet">
    ///             <item>
    ///                 If <paramref name="messageType" /> is interface, journal will fail when message implements that
    ///                 interface.
    ///             </item>
    ///             <item>
    ///                 If <paramref name="messageType" /> is class, journal will fail when message can be assigned to
    ...
    public Task FailIf(Func<IPersistentRepresentation, bool> predicate)
    {
        ...
        return SetInterceptorAsync(
            new JournalInterceptors.OnCondition(predicate, JournalInterceptors.Failure.Instance));
    }
    ...
    public Task FailUnless(Func<IPersistentRepresentation, bool> predicate)
    {
        ...
        return SetInterceptorAsync(
            new JournalInterceptors.OnCondition(predicate, JournalInterceptors.Failure.Instance, true));
    }
    ...
    {
        return FailOnTypeWithDelay(delay, typeof(TMessage));
    }
