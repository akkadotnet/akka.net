...
public class DistributedPubSubMessageSerializerSpec : AkkaSpec
{
    ...
    public void Can_serialize_Status()
    {
        ...
        var message =
            new Status(
                new Dictionary<Address, long> { { address1, 3 }, { address2, 17 }, { address3, 5 } }
    ...
    public void Can_serialize_Delta()
    {
        ...
        var message = new Delta(new List<Bucket>
        {
            new(address1, 3,
                ...
                {
                    { "/user/u1", new ValueHolder(2, u1) }, { "/user/u2", new ValueHolder(3, u2) }
                }.ToImmutableDictionary()),
            new(address2, 17,
                new Dictionary<string, ValueHolder> { { "/user/u3", new ValueHolder(17, u3) } }
                    .ToImmutableDictionary()),
            new(address3, 5,
                new Dictionary<string, ValueHolder>
                {
                    { "/user/u4", new ValueHolder(4, u4) }, { "/user/u5", new ValueHolder(5, null) }
                }.ToImmutableDictionary())
        }.ToImmutableList());
