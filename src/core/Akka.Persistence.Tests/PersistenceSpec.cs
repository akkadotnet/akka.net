...
public abstract class PersistenceSpec : AkkaSpec
{
    ...
    public PersistenceExtension Extension => Persistence.Instance.Apply(Sys);
        ...
        string extraConfig = null)
    {
        ...
        return ConfigurationFactory.ParseString(extraConfig)
            .WithFallback(ConfigurationFactory.ParseString(configString));
    }
...
internal class Cleanup : IDisposable
{
    ...
    {
        StorageLocations = new[] { "akka.persistence.snapshot-store.local.dir" }
