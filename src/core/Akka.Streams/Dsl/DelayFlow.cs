...
public class LinearIncreasingDelay<T> : IDelayStrategy<T>
{
    ...
    ///     increases by <paramref name="increaseStep" /> every time when <paramref name="needsIncrease" /> returns
    ///     <code>true</code> up to <paramref name="maxDelay" />,
    ...
    /// <param name="needsIncrease">
    ///     if <code>true</code> delay increases, if <code>false</code> delay resets to
    ...
    [ApiMayChange]
    public LinearIncreasingDelay(TimeSpan increaseStep, Func<T, bool> needsIncrease, TimeSpan initialDelay,
        TimeSpan maxDelay)
    {
...
public class DelayFlow<T> : SimpleLinearGraphStage<T>
{
    ...
    ///     <code>TimeSpan DelayStrategy.NextDelay(T element)</code>.
    ///     Implementing <see cref="IDelayStrategy{T}" /> with your own gives you flexible ability to manage delay value
    ///     depending on coming elements.
