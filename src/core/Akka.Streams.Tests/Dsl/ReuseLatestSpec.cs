// -----------------------------------------------------------------------
//  <copyright file="ReuseLatestSpec.cs" company="Akka.NET Project">
...
{
    public ReuseLatestSpec(ITestOutputHelper testOutputHelper) : base(Config.Empty, testOutputHelper)
    {
    ...
    public async Task RepeatPrevious_should_fail_when_upstream_fails()
    {
        Func<Task> Exec()
        {
            return async () =>
            {
                ...
                {
                    if (i % 5 == 0) throw new ApplicationException("failed");
