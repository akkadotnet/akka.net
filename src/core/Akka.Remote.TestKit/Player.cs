    ...
    public void Enter(TimeSpan timeout, ImmutableList<string> names)
    {
        ...
        foreach (var name in names)
        {
            ...
            try
            {
                ...
                var result = _client.Ask(new ToServer<EnterBarrier>(new EnterBarrier(name, barrierTimeout)), askTimeout)
                    .Result;
            }
...
//TODO: RequireMessageQueue
{
    ...
    public void InitFSM()
    {
        ...
        When(State.Connecting, @event =>
        {
            ...
            if (@event.FsmEvent is ConnectionFailure) return GoTo(State.Failed);
            if (@event.FsmEvent is StateTimeout)
            {
                _log.Error(
                    $"Failed to connect to test conductor within {_settings.ConnectTimeout.TotalMilliseconds} ms.");
        ...
        When(State.Connected, @event =>
        {
                ...
                @event.StateData.RunningOp == null)
            {
                ...
                return Stay().Using(@event.StateData.Copy((token, Sender)));
            }
            ...
            if (@event.FsmEvent is IClientOp && @event.StateData.Channel != null)
            {
                ...
                if (barrierResult != null)
                {
                    ...
                    return Stay().Using(@event.StateData.Copy(null));
                }
        ...
        When(State.Failed, @event =>
        {
            if (@event.FsmEvent is IClientOp)
                return Stay()
                    .Replying(new Status.Failure(new Exception("cannot do " + @event.FsmEvent + " while failed")));
            if (@event.FsmEvent is INetworkOp)
            {
...
internal class PlayerHandler : ChannelHandlerAdapter
{
    ...
    private static string FormatConnectionFailure(IChannelHandlerContext context, Exception exception)
    {
        ...
        sb.AppendLine(
            $"Connection between [Local: {context.Channel.LocalAddress}] and [Remote: {context.Channel.RemoteAddress}] has failed.");
    ...
    private void Reconnect()
    {
        ...
        {
            _log.Debug("Failed to connect.... Retrying again in {0}s. {1} attempts left.", _nextAttempt.TimeLeft,
                _reconnects);
            if (_reconnects > 0)
            {
                _reconnects -= 1;
