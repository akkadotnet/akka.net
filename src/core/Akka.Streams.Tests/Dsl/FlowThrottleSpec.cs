...
public class FlowThrottleSpec : AkkaSpec
{
        ...
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var probe = Source.From(new[] { 1, 2, 3, 4, 5 })
                .Throttle(
                    ...
                    1.Seconds(),
                    10,
                    e => e == 3 ? throw new TestException("err1") : 0,
    ...
    public async Task Throttle_for_single_cost_elements_must_send_elements_downstream_as_soon_as_time_comes()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            result.Should().BeEquivalentTo(new OnNext(1), new OnNext(2));
        }, Materializer);
    }
    ...
    [Fact]
    public async Task
        Throttle_for_various_cost_elements_must_throw_exception_when_exceeding_throughtput_in_enforced_mode()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
