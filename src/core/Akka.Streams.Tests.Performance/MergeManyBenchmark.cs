...
public class MergeManyBenchmark
{
    ...
    {
        return Source.FromGraph(graph).ToMaterialized(Sink.Ignore<int>(), Keep.Right);
    }

    private static IGraph<SourceShape<int>, NotUsed> CreateSource(int count)
    {
        return Fusing.Aggressive(Source.Repeat(1).Take(count));
    }
    ...
    public void Take_100k_elements()
    {
        _takeGraph.Run(_materializer).Wait();
    }
    ...
    public void Select_many_from_1_Source_100k_elements()
    {
        _singleGraph.Run(_materializer).Wait();
    }
    ...
    public void Select_many_from_10_Sources_10k_elements_each()
    {
        _tenGraph.Run(_materializer).Wait();
    }
}