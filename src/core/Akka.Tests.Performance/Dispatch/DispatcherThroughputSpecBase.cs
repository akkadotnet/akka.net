...
public abstract class ColdDispatcherThroughputSpecBase
{
    ...
    public void Setup(BenchmarkContext context)
    {
        ...
        ScheduledWork = () =>
        {
            ...
            if (Interlocked.Increment(ref _messagesSeen) == ScheduleCount) EventBlock.Set();
        };
        Warmup(_dispatcher);
    }

    [PerfBenchmark(Description = "Tests how long it takes to schedule items onto the dispatcher",
...
public abstract class WarmDispatcherThroughputSpecBase : ColdDispatcherThroughputSpecBase
{
    protected override void Warmup(MessageDispatcher dispatcher)
    {
        ...
        var warmupWork = () =>
        {
            if (Interlocked.Increment(ref warmupsThusFar) == warmupCount) EventBlock.Set();
        };
