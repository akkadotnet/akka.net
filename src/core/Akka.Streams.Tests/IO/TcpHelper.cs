...
public abstract class TcpHelper : AkkaSpec
{
    ...
    {
        return Props.Create(() => new TestClient(connection)).WithDispatcher("akka.test.stream-dispatcher");
    }

    protected static Props TestServerProps(EndPoint address, IActorRef probe)
    {
        return Props.Create(() => new TestServer(address, probe)).WithDispatcher("akka.test.stream-dispatcher");
    }
    ...
    protected class TestClient : UntypedActor
    {
        ...
        public TestClient(IActorRef connection)
        {
            ...
            connection.Tell(new Tcp.Register(Self, true, false));
        }
    ...
    protected class Server
    {
        ...
        public void Close()
        {
            ServerRef.Tell(ServerClose.Instance);
        }
    }
    ...
    protected class ServerConnection
    {
        ...
        {
            _connectionActor.Tell(new ClientWrite(bytes));
        }

        public void Read(int count)
        {
            _connectionActor.Tell(new ClientRead(count, _connectionProbe.Ref));
        }

        public async Task<ByteString> WaitReadAsync()
        {
            return (await _connectionProbe.ExpectMsgAsync<ReadResult>()).Bytes;
        }

        public void ConfirmedClose()
        {
            _connectionActor.Tell(new ClientClose(Tcp.ConfirmedClose.Instance));
        }

        public void Close()
        {
            _connectionActor.Tell(new ClientClose(Tcp.Close.Instance));
        }

        public void Abort()
        {
            _connectionActor.Tell(new ClientClose(Tcp.Abort.Instance));
        }
    ...
    protected class TcpReadProbe
    {
        ...
        {
            (await TcpReadSubscription()).Cancel();
        }
    }
    ...
    protected class TcpWriteProbe
    {
        ...
        public async Task CloseAsync()
        {
            (await TcpWriteSubscription()).SendComplete();
        }
    }
}