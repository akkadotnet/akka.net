...
internal static class DowningProvider
{
    ...
    ///     This exception is thrown when the specified <paramref name="downingProviderType" /> does not implement
    ///     <see cref="IDowningProvider" />.
    /// </exception>
    /// <returns>The activated <see cref="IDowningProvider" /></returns>
    /// <remarks>
    ///     Required to pass in <see cref="Akka.Cluster.Cluster" /> manually here since
    ///     https://github.com/akkadotnet/akka.net/issues/5962
    ...
    public static IDowningProvider Load(Type downingProviderType, ActorSystem system, Cluster cluster)
    {
        ...
        catch (Exception e)
        {
            throw new ConfigurationException(
                $"Couldn't create downing provider of type [{downingProviderType.FullName}]", e);
        }
    }
}