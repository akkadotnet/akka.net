...
///     command, the joining node will still be marked as <see cref="MemberStatus.Up" /> as long as there are no
///     unreachable nodes
///     within the cluster.
/// </summary>
public class JoinWithOfflineSeedNodeSpec : MultiNodeClusterSpec
{
    ...
    private readonly Lazy<ActorSystem> _seedSystem;

    private volatile UniqueAddress _seedUniqueAddress;
    private readonly Lazy<ActorSystem> _unavailableSeedSystem;
    ...
    protected JoinWithOfflineSeedNodeSpec(JoinWithOfflineSeedNodeConfig config) : base(config,
        typeof(JoinWithOfflineSeedNodeSpec))
    {
        _config = config;
    ...
    {
        RunOn(() => { Shutdown(_seedSystem.Value); }, _config.Seed);
    ...
    public void JoinClusterWithOneOfflineSeedNode()
    {
        Within(TimeSpan.FromSeconds(30), () =>
        {
            ...
            RunOn(() =>
            {
                ...
                _seedSystem.Value
                    .ActorSelection(new RootActorPath(GetAddress(_config.NonSeed)) / "user" / "address-receiver")
                    .Tell(_seedUniqueAddress, probe.Ref);
    ...
    private class LeaderSynchronizer : ReceiveActor
    {
    ...
    /// </summary>
    private class DomainEventLogger : ReceiveActor
    {
