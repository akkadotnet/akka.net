...
public class RemoteMessageLocalDeliverySpec : AkkaSpec
{
    ...
    {
        var nonLocalAddress = new Address("akka.trttl.gremlin.tcp", Sys.Name, "localhost",
            RARP.For(Sys).Provider.DefaultAddress.Port);
        var nonLocalActorPath = new RootActorPath(nonLocalAddress) / "user" / "foo";
        var resolved = RARP.For(Sys).Provider
            .ResolveActorRefWithLocalAddress(nonLocalActorPath.ToSerializationFormat(), nonLocalAddress);
    ...
    public void RemoteActorRefProvider_should_correctly_resolve_valid_LocalActorRef_from_remote_address()
    {
        ...
        var resolvedActorRef = RARP.For(Sys).Provider
            .ResolveActorRefWithLocalAddress(actorPath.ToStringWithAddress(), localAddress);
