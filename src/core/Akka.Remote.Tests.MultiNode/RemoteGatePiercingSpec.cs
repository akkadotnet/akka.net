    ...
    public RemoteGatePiercingMultiNetSpec()
    {
      ...
      akka.remote.transport-failure-detector.acceptable-heartbeat-pause = 5 
        ...
        NodeConfig(new[] { First },
            new[] { ConfigurationFactory.ParseString("akka.remote.retry-gate-closed-for  = 1 d # Keep it long") });
        NodeConfig(new[] { Second },
            new[] { ConfigurationFactory.ParseString("akka.remote.retry-gate-closed-for  = 1 s # Keep it short") });
...
public class RemoteGatePiercingSpec : MultiNodeSpec
{
    ...
    public void RemoteGatePiercing_must_allow_restarted_node_to_pass_through_gate()
    {
        ...
        RunOn(() =>
        {
            ...
            EventFilter.Warning(null, "address is now gated").ExpectOne(() =>
            {
                var cmd =
                    RARP.For(Sys)
