...
public class LeaseMajority5NodeSpec : MultiNodeClusterSpec
{
    ...
    private List<RoleName> SortByAddress(RoleName[] roles)
    {
    ...
    {
        return SortByAddress(roles).First();
    }
    ...
    public void LeaseMajority_in_a_5_node_cluster_should_setup_cluster()
    {
        ...
        RunOn(() => { Cluster.Join(Node(_config.Node1).Address); }, _config.Node2, _config.Node3, _config.Node4,
            _config.Node5);
    ...
    public void LeaseMajority_in_a_5_node_cluster_should_keep_the_side_that_can_acquire_the_lease()
    {
        ...
        RunOn(() => { lease.SetNextAcquireResult(Task.FromResult(true)); }, _config.Node1, _config.Node2,
            _config.Node3);
        RunOn(() => { lease.SetNextAcquireResult(Task.FromResult(false)); }, _config.Node4, _config.Node5);
        ...
        RunOn(() =>
        {
            Within(TimeSpan.FromSeconds(20),
                () => { AwaitAssert(() => { Cluster.State.Members.Count.Should().Be(3); }); });
            RunOn(() =>
            {
        ...
        RunOn(() =>
        {
            Within(TimeSpan.FromSeconds(20), () =>
            {
                ...
                RunOn(() => { leaseProbe.ExpectMsg<TestLease.AcquireReq>(); }, Leader(_config.Node4, _config.Node5));
            });
    ...
    public void LeaseMajority_in_a_5_node_cluster_should_keep_the_side_that_can_acquire_the_lease_round_2()
    {
        ...
        RunOn(() => { lease.SetNextAcquireResult(Task.FromResult(true)); }, _config.Node1);
        RunOn(() => { lease.SetNextAcquireResult(Task.FromResult(false)); }, _config.Node2, _config.Node3);
