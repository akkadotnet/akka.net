...
/// </summary>
/// <remarks>
///     NOTE: <see cref="GetHashCode" /> and <see cref="Equals" /> are solely based on the underlying
///     <see cref="Address" />,
...
public class Member : IComparable<Member>, IComparable
{
        ...
        {
            {
                ...
                ImmutableHashSet.Create(MemberStatus.WeaklyUp, MemberStatus.Up, MemberStatus.Leaving, MemberStatus.Down,
                    MemberStatus.Removed)
            },
            {
                MemberStatus.WeaklyUp,
                ImmutableHashSet.Create(MemberStatus.Up, MemberStatus.Leaving, MemberStatus.Down, MemberStatus.Removed)
            },
            { MemberStatus.Up, ImmutableHashSet.Create(MemberStatus.Leaving, MemberStatus.Down, MemberStatus.Removed) },
            {
                MemberStatus.Leaving,
                ImmutableHashSet.Create(MemberStatus.Exiting, MemberStatus.Down, MemberStatus.Removed)
            },
    ...
    int IComparable.CompareTo(object obj)
    {
        ...
        throw new ArgumentException(
            $"Cannot compare {nameof(Member)} to an instance of type '{obj?.GetType().FullName ?? "null"}'");
    }
    ...
    {
        return new Member(node, int.MaxValue, MemberStatus.Removed, ImmutableHashSet.Create<string>(), AppVersion.Zero);
    }
    ...
    internal static Member Create(UniqueAddress uniqueAddress, int upNumber, MemberStatus status,
        ImmutableHashSet<string> roles, AppVersion appVersion)
    {
    ...
    {
        return
            $"Member(address = {Address}, Uid={UniqueAddress.Uid} status = {Status}, role=[{string.Join(",", Roles)}], upNumber={UpNumber}, version={AppVersion})";
    }
    ...
    {
        if (UpNumber.Equals(other.UpNumber)) return AddressOrdering.Compare(Address, other.Address) < 0;
    ...
    {
        return new Member(UniqueAddress, upNumber, Status, Roles, AppVersion).Copy(MemberStatus.Up);
    }
    ...
    public static ImmutableSortedSet<Member> PickNextTransition(IEnumerable<Member> a, IEnumerable<Member> b)
    {
        ...
        foreach (var g in groupedByAddress)
            if (g.Count() == 2)
            {
                acc.Add(PickNextTransition(g.First(), g.Skip(1).First()));
            }
            else
            {
                var m = g.First();
    ...
    /// <returns>
    ///     If a and b are different members, this method will return <c>null</c>.
    ///     Otherwise, will return a or b depending on which one is a valid transition of the other.
    ///     If neither are a valid transition, we return <c>null</c>
    /// </returns>
    public static Member PickNextTransition(Member a, Member b)
    {
        ...
        if (AllowedTransitions[a.Status].Contains(b.Status))
            return b;
        if (AllowedTransitions[b.Status].Contains(a.Status))
            return a;
    ...
    {
        if (m1.Status.Equals(m2.Status)) return m1.IsOlderThan(m2) ? m1 : m2;
...
public class UniqueAddress : IComparable<UniqueAddress>, IEquatable<UniqueAddress>, IComparable
{
    ...
    int IComparable.CompareTo(object obj)
    {
        ...
        throw new ArgumentException(
            $"Cannot compare {nameof(UniqueAddress)} with instance of type '{obj?.GetType().FullName ?? "null"}'.");
    }
    ...
    {
        return CompareTo(uniqueAddress, Address.Comparer);
    }
    ...
    public override bool Equals(object obj)
    {
        return obj is UniqueAddress address && Equals(address);
    }
    ...
    public override string ToString()
    {
        return $"UniqueAddress: ({Address}, {Uid})";
    }
