...
public class FlowGroupedWithinSpec : ScriptedTest
{
    ...
    public void A_GroupedWithin_must_group_early()
    {
        ...
        RandomTestRange(Sys)
            .Select(async _ =>
                await RunScriptAsync(script, Settings, flow => flow.GroupedWithin(3, TimeSpan.FromMinutes(10))));
    }
    ...
    public void A_GroupedWithin_must_group_with_rest()
    {
        ...
        var script = () =>
        {
            ...
            return Script.Create(RandomTestRange(Sys).Select(_ =>
            {
                ...
                return ((ICollection<int>)new[] { x, y, z },
                    (ICollection<IEnumerable<int>>)new[] { new[] { x, y, z } });
            }).Concat(rest).ToArray());
        };
...
public class FlowGroupedWeightedWithinSpec : ScriptedTest
{
    ...
    [Fact]
    public async Task
        A_GroupedWeightedWithin_must_append_zero_weighted_elements_to_a_full_group_before_timeout_received_if_downstream_hasnt_pulled_yet()
    {
    ...
    [Fact]
    public async Task
        A_GroupedWeightedWithin_must_handle_zero_cost_function_to_get_only_timed_based_grouping_without_limit()
    {
        var upstream = this.CreatePublisherProbe<string>();
        var downstream = this.CreateSubscriberProbe<IEnumerable<string>>();
