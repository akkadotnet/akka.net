...
public class DeadlineFailureDetector : FailureDetector
{
    ...
    private long _heartbeatTimestamp; //not used until active (first heartbeat)
    ...
    /// <param name="clock">
    ///     The clock, returning current time in milliseconds, but can be faked for testing purposes. It is
    ///     only used for measuring intervals (duration).
    /// </param>
        ...
        Clock clock = null)
    {
        if (acceptableHeartbeatPause <= TimeSpan.Zero)
            throw new ArgumentOutOfRangeException(nameof(acceptableHeartbeatPause),
        ...
        _deadlineMillis =
            Convert.ToInt64(acceptableHeartbeatPause.TotalMilliseconds + heartbeatInterval.TotalMilliseconds);
    }
