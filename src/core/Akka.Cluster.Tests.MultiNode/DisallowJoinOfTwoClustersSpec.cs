...
public class DisallowJoinOfTwoClusterSpec : MultiNodeClusterSpec
{
    ...
    protected DisallowJoinOfTwoClusterSpec(DisallowJoinOfTwoClusterSpecConfig config) : base(config,
        typeof(DisallowJoinOfTwoClusterSpec))
    {
        _config = config;
    }
    ...
    {
        RunOn(() => { StartClusterNode(); }, _config.a1, _config.b1, _config.c1);
        EnterBarrier("first-started");

        RunOn(() => { Cluster.Join(GetAddress(_config.a1)); }, _config.a1, _config.a2);

        RunOn(() => { Cluster.Join(GetAddress(_config.b1)); }, _config.b1, _config.b2);

        RunOn(() => { Cluster.Join(GetAddress(_config.c1)); }, _config.c1);

        var expectedSize = Myself == _config.c1 ? 1 : 2;
        AwaitMembersUp(expectedSize);

        EnterBarrier("two-members");

        RunOn(() => { Cluster.Join(GetAddress(_config.a1)); }, _config.b1);

        RunOn(() => { Cluster.Join(GetAddress(_config.c1)); }, _config.b2);

        RunOn(() => { Cluster.Join(GetAddress(_config.a2)); }, _config.c1);
