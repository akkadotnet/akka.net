...
internal sealed class DirectBufferPool : IBufferPool
{
    ...
    public DirectBufferPool(ExtendedActorSystem system, Config config) : this(
        config.GetInt("buffer-size", 256),
        config.GetInt("buffers-per-segment", 250),
        config.GetInt("initial-segments", 1),
        config.GetInt("buffer-pool-limit", 1000))
    {
    }
    ...
    public DirectBufferPool(int bufferSize, int buffersPerSegment, int initialSegments, int maxSegments)
    {
        ...
        if (buffersPerSegment <= 0)
            throw new ArgumentException("Number of buffers per segment must be positive", nameof(buffersPerSegment));
        if (initialSegments <= 0)
            throw new ArgumentException("Number of initial segments must be positivie", nameof(initialSegments));
        if (maxSegments < initialSegments)
            throw new ArgumentException("Maximum number of segments must not be less than the initial one",
                nameof(maxSegments));
        ...
        for (var i = 0; i < initialSegments; i++) AllocateSegment();
    }
    ...
    public BufferPoolInfo Diagnostics()
    {
        return new BufferPoolInfo(
            typeof(DirectBufferPool),
            _segments.Count * _segmentSize,
            _buffers.Count * _bufferSize,
            _segments.Count * _segmentSize - _buffers.Count * _bufferSize);
    }
    ...
    public ByteBuffer Rent()
    {
        ...
        throw new BufferPoolAllocationException(
            $"Buffer pool failed to return byte buffer after {Retries} retries made.");
    }
    ...
    public IEnumerable<ByteBuffer> Rent(int minimumSize)
    {
        ...
        try
        {
            ...
            throw new BufferPoolAllocationException(
                $"Couldn't allocate enough byte buffer to fill the total requested size of {minimumSize} bytes");
        }
        catch
        {
            for (var i = 0; i < received; i++) Release(result[i]);
            throw;
        }
    }
    ...
    {
        foreach (var buf in buffers) Release(buf);
    }
    ...
    private void AllocateSegment()
    {
        lock (_syncRoot)
        {
            if (_segments.Count >= _maxSegmentCount)
                throw new BufferPoolAllocationException(
                    $"Buffer pool cannot allocate more segments. A maximum capacity of {_maxSegmentCount * _segmentSize} bytes has been reached.");

            var segment = new byte[_segmentSize];
            _segments.Add(segment);
            for (var i = 0; i < _buffersPerSegment; i++)
            {
