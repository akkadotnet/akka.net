...
public sealed class ClusterClientDiscoverySpecConfig : MultiNodeConfig
{
    ...
    public ClusterClientDiscoverySpecConfig()
    {
        ...
        NodeConfig(new[] { First },
            new[] { ConfigurationFactory.ParseString($"akka.management.http.port = {HttpPorts[0]}") });
        NodeConfig(new[] { Second },
            new[] { ConfigurationFactory.ParseString($"akka.management.http.port = {HttpPorts[1]}") });
        NodeConfig(new[] { Third },
            new[] { ConfigurationFactory.ParseString($"akka.management.http.port = {HttpPorts[2]}") });
...
public class ClusterClientDiscoverySpec : MultiNodeClusterSpec
{
    ...
    private void ClusterClient_must_establish_connection_to_first_node()
    {
        RunOn(() =>
        {
            ...
            _clusterClient.Tell(new ClusterClient.Send("/user/testService", "hello", true));
