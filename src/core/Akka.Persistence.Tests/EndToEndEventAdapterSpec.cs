...
public class MemoryEndToEndAdapterSpec : EndToEndEventAdapterSpec
{
    private static readonly Config Config = ConfigurationFactory.ParseString(
        @"akka.persistence.journal.inmem.class = ""Akka.Persistence.Journal.SharedMemoryJournal, Akka.Persistence""");
...
public abstract class EndToEndEventAdapterSpec : PersistenceSpec
{
        ...
        : base(Configuration("EndToEndEventAdapterSpec"))
    {
        ...
        NewAdaptersConfig =
            string.Format(NewAdaptersConfigFormat, journalName, typeof(EndToEndEventAdapterSpec).FullName);
    }

    private IActorRef Persister(string name, IActorRef probe, ActorSystem system)
    {
        return (system ?? Sys).ActorOf(Props.Create(() =>
            new EndToEndAdapterActor(name, "akka.persistence.journal." + _journalName, probe)));
    }
    ...
    [Fact]
    public void
        EventAdapters_in_end_to_end_scenarios_should_use_the_same_adapter_when_reading_as_was_used_when_writing_to_journal()
    {
    ...
    [Fact]
    public void
        EventAdapters_in_end_to_end_scenarios_should_allow_using_an_adapter_when_write_was_performed_without_an_adapter()
    {
        var pid = "p2";
    ...
    [Fact]
    public void
        EventAdapters_in_end_to_end_scenarios_should_give_nice_error_message_when_unable_to_play_back_as_adapter_does_not_exist()
    {
    ...
    public class EndToEndAdapterActor : NamedPersistentActor
    {
        ...
        private readonly LinkedList<object> _state = new();
        ...
        protected bool PersistIncoming(object message)
        {
            ...
            {
                foreach (var e in _state) Sender.Tell(e);
            }
            else
            {
                var sender = Sender;
