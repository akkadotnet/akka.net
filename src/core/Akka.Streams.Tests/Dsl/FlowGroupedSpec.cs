...
public class FlowGroupedSpec : ScriptedTest
{
    ...
    private ICollection<int> RandomSeq(int n)
    {
        return Enumerable.Range(1, n).Select(_ => _random.Next()).ToList();
    }
    ...
    {
        var testLength = _random.Next(1, 16);
        var script = Script.Create(RandomTestRange(Sys).Select(_ => RandomTest(testLength)).Concat(RandomTest(1))
            .ToArray());
        foreach (var _ in RandomTestRange(Sys))
            await RunScriptAsync(script, Settings, flow => flow.Grouped(testLength));
    }
