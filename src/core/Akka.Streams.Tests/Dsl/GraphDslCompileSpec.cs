...
#pragma warning disable CS0618 // Type or member is obsolete
    private static IStage<TIn, TOut> Op<TIn, TOut>() where TIn : TOut
    {
        return new OpStage<TIn, TOut>();
    }
#pragma warning restore CS0618 // Type or member is obsolete

    private static IEnumerator<Apple> Apples()
    {
        return Enumerable.Repeat(new Apple(), int.MaxValue).GetEnumerator();
    }
