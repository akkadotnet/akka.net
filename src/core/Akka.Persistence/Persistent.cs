...
public sealed class AtomicWrite : IPersistentEnvelope, IMessage
{
    ...
    ///     or the specified <paramref name="payload" /> contains messages from different
    ///     <see cref="IPersistentRepresentation.PersistenceId" />.
    /// </exception>
    ...
    public AtomicWrite(IImmutableList<IPersistentRepresentation> payload)
    {
        ...
        for (var i = 1; i < payload.Count; i++)
            if (!payload[i].PersistenceId.Equals(firstMessage.PersistenceId))
                throw new ArgumentException(
                    $"AtomicWrite must contain messages for the same persistenceId, yet difference persistenceIds found: {payload.Select(m => m.PersistenceId).Distinct()}.",
                    nameof(payload));
        ...
        LowestSequenceNr =
            firstMessage.SequenceNr; // this assumes they're gapless; they should be (it is only our code creating AWs)
...
public class Persistent : IPersistentRepresentation, IEquatable<IPersistentRepresentation>
{
    ...
    public Persistent(object payload, long sequenceNr = 0L, string persistenceId = null, string manifest = null,
        bool isDeleted = false, IActorRef sender = null, string writerGuid = null, long timestamp = 0L)
    {
        Payload = payload;
    ...
    public IPersistentRepresentation WithTimestamp(long newTimestamp)
    {
        return Timestamp == newTimestamp
            ? this
            : new Persistent(Payload, SequenceNr, PersistenceId, Manifest, IsDeleted, Sender, WriterGuid, newTimestamp);
    }

    /// <inheritdoc />
    public IPersistentRepresentation WithPayload(object payload)
    {
        return new Persistent(payload, SequenceNr, PersistenceId, Manifest, IsDeleted, Sender, WriterGuid, Timestamp);
    }
    ...
    public IPersistentRepresentation WithManifest(string manifest)
    {
        return Manifest == manifest
            ? this
            : new Persistent(Payload, SequenceNr, PersistenceId, manifest, IsDeleted, Sender, WriterGuid, Timestamp);
    }
    ...
    public IPersistentRepresentation Update(long sequenceNr, string persistenceId, bool isDeleted, IActorRef sender,
        string writerGuid)
    {
        return new Persistent(Payload, sequenceNr, persistenceId, Manifest, isDeleted, sender, writerGuid);
    }
