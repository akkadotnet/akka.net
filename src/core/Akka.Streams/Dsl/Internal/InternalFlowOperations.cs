...
internal static class InternalFlowOperations
{
    ...
    ///     behave exactly the same as  <see cref="RecoverWith{TOut,TMat}" />.
    ///     <para>
    ///         Since the underlying failure signal onError arrives out-of-band, it might jump over existing elements.
    ///         This stage can recover the failure signal, but not the skipped elements, which will be dropped.
    ///     </para>
    ...
    ///     should be applied
    /// </param>
    public static IFlow<TOut, TMat> SelectError<TOut, TMat>(this IFlow<TOut, TMat> flow,
    ...
    public static IFlow<TOut, TMat> WireTap<TOut, TMat>(this IFlow<TOut, TMat> flow, Action<TOut> action)
    {
        ...
        return flow.WireTapMaterialized(Sink.ForEach(action), Keep.Left);
    }
    ...
    {
        return flow.Via(new Fusing.GroupedWeightedWithin<T>(long.MaxValue, n, _ => 0L, timeout)
    ...
    /// <returns>TBD</returns>
    public static IFlow<IEnumerable<TOut>, TMat> GroupedWeightedWithin<TOut, TMat>(this IFlow<TOut, TMat> flow,
        long maxWeight, TimeSpan interval, Func<TOut, long> costFn)
    {
        return flow.Via(new Fusing.GroupedWeightedWithin<TOut>(maxWeight, int.MaxValue, costFn, interval));
    }
    ...
    {
        return flow.Via(new Fusing.GroupedWeightedWithin<TOut>(maxWeight, maxNumber, costFn, interval));
    }
    ...
    /// <param name="aggregate">Takes the currently batched value and the current pending element to produce a new aggregate</param>
    /// <returns>TBD</returns>
    public static IFlow<TOut2, TMat> BatchWeighted<TOut, TOut2, TMat>(this IFlow<TOut, TMat> flow, long max,
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    ///     See also
    ///     <seealso
    ///         cref="GroupBy{T, TMat, TKey, TClosed}(IFlow{T, TMat}, int, Func{T, TKey}, Func{IFlow{Source{T, NotUsed}, TMat}, Sink{Source{T, NotUsed}, Task{Done}}, TClosed}, bool)" />
    /// </summary>
    ...
    public static SubFlow<T, TMat, TClosed> GroupBy<T, TMat, TKey, TClosed>(
        ...
        Func<IFlow<Source<T, NotUsed>, TMat>, Sink<Source<T, NotUsed>, Task<Done>>, TClosed> toFunc)
    {
        return GroupBy(flow, maxSubstreams, groupingFunc, toFunc, false);
    }
    ...
    ///         <li>
    ///             The specified <paramref name="maximumBurst" /> is less than or equal zero in
    ///             <see cref="ThrottleMode.Enforcing" /> <paramref name="mode" />.
    ///         </li>
    ///         <li>
    ///             The <see cref="TimeSpan.Ticks" /> in the specified <paramref name="per" /> is less than the specified
        ...
        int maximumBurst, ThrottleMode mode)
    {
        ...
        if (per <= TimeSpan.Zero)
            throw new ArgumentException("Throttle per timeout must not be less than or equal to zero", nameof(per));
    ...
    {
        return flow.Via(OrElseGraph(secondary));
    }
    ...
    /// <returns>TBD</returns>
    public static IFlow<T, TMat3> OrElseMaterialized<T, TMat, TMat2, TMat3>(this IFlow<T, TMat> flow,
        IGraph<SourceShape<T>, TMat2> secondary, Func<TMat, TMat2, TMat3> materializedFunction)
    {
        return flow.ViaMaterialized(OrElseGraph(secondary), materializedFunction);
    }
    ...
    ///     through will also be sent to the <seealso cref="Sink{TIn,TMat}" />.
    ///     @see
    ///     <seealso cref="InternalFlowOperations.AlsoTo{TOut,TMat}(IFlow{TOut, TMat}, IGraph{SinkShape{TOut}, TMat}, bool)" />
    ///     It is recommended to use the internally optimized <seealso cref="Keep.Left{TLeft,TRight}" /> and
    ///     <seealso cref="Keep.Right{TLeft,TRight}" /> combiners
    ///     where appropriate instead of manually writing functions that pass through one of the values.
    /// </summary>
    ...
    ///     </para>
    /// </summary>
    public static IFlow<TOut, TMat3> WireTapMaterialized<TOut, TMat, TMat2, TMat3>(this IFlow<TOut, TMat> flow,
        IGraph<SinkShape<TOut>, TMat2> that, Func<TMat, TMat2, TMat3> materializerFunction)
    {
        return flow.ViaMaterialized(WireTapGraph(that), materializerFunction);
    }
    ...
    {
        return flow.Via(WireTapGraph(that));
    }
    ...
    public static IFlow<TOut, TMat3> DivertToMaterialized<TOut, TMat, TMat2, TMat3>(
        ...
        Func<TMat, TMat2, TMat3> materializerFunction)
    {
        return flow.ViaMaterialized(DivertToGraph(that, when), materializerFunction);
    }
    ...
    /// <param name="that">TBD</param>
    /// <param name="when">TBD</param>
    public static IFlow<TOut, TMat> DivertTo<TOut, TMat>(this IFlow<TOut, TMat> flow,
        IGraph<SinkShape<TOut>, TMat> that, Func<TOut, bool> when)
    {
        return flow.Via(DivertToGraph(that, when));
    }

    private static IGraph<FlowShape<TOut, TOut>, TMat> DivertToGraph<TOut, TMat>(IGraph<SinkShape<TOut>, TMat> that,
        Func<TOut, bool> when)
    {
        return GraphDsl.Create(that, (b, r) =>
        {
    ...
    ///     '''Cancels when''' downstream cancels
    /// </summary>
    public static IFlow<T, TMat> Watch<T, TMat>(this IFlow<T, TMat> flow, IActorRef actorRef)
    {
        return flow.Via(new Fusing.Watch<T>(actorRef));
    }
    ...
    internal sealed class GroupByMergeBack<TOut, TMat, TKey> : IMergeBack<TOut, TMat>
    {
        ...
        public GroupByMergeBack(IFlow<TOut, TMat> self, int maxSubstreams, Func<TOut, TKey> groupingFunc,
            bool allowClosedSubstreamRecreation = false)
        {
            _self = self;
        ...
        {
            return _self
                .Via(new Fusing.GroupBy<TOut, TKey>(_maxSubstreams, _groupingFunc, _allowClosedSubstreamRecreation))
                .Select(f => f.Via(flow))
