...
{
    private static readonly UniqueAddress nodeA = new(new Address("akka.tcp", "sys", "a", 2552), 1);
    private static readonly UniqueAddress nodeB = new(new Address("akka.tcp", "sys", "b", 2552), 2);
    private static readonly UniqueAddress nodeC = new(new Address("akka.tcp", "sys", "c", 2552), 3);
    private static readonly UniqueAddress nodeD = new(new Address("akka.tcp", "sys", "d", 2552), 4);
    private static readonly UniqueAddress nodeE = new(new Address("akka.tcp", "sys", "e", 2552), 5);
    ...
    {
        var r = Reachability.Empty.Unreachable(nodeB, nodeA).Unreachable(nodeC, nodeA).Reachable(nodeB, nodeA)
            .Reachable(nodeC, nodeA).Unreachable(nodeB, nodeC).Unreachable(nodeC, nodeB);
        r.IsReachable(nodeA).Should().BeTrue();
    }
    ...
    {
        var r = Reachability.Empty.Unreachable(nodeC, nodeA).Reachable(nodeC, nodeA).Unreachable(nodeC, nodeB)
        ...
        r.RemoveObservers(ImmutableHashSet.Create(nodeB)).AllUnreachableOrTerminated.Should()
            .BeEquivalentTo(ImmutableHashSet.Create(nodeB));
    }

    [Fact]
    public void ReachabilityTable_must_be_pruned_when_all_records_of_an_observer_are_reachable()
    {
        var r = Reachability.Empty.Unreachable(nodeB, nodeA).Unreachable(nodeB, nodeC).Unreachable(nodeD, nodeC)
    ...
    public void ReachabilityTable_must_have_correct_aggregated_status()
    {
        ...
        {
            new KeyValuePair<UniqueAddress, long>(nodeA, 3), new KeyValuePair<UniqueAddress, long>(nodeC, 3),
    ...
    {
        var r = Reachability.Empty.Unreachable(nodeB, nodeA).Unreachable(nodeC, nodeA).Unreachable(nodeD, nodeA)
            .Unreachable(nodeC, nodeB).Reachable(nodeC, nodeB).Unreachable(nodeD, nodeB).Unreachable(nodeD, nodeC)
            .Reachable(nodeD, nodeC).Reachable(nodeE, nodeD).Unreachable(nodeA, nodeE).Terminated(nodeB, nodeE);
    ...
    public void ReachabilityTable_must_merge_versions_correctly()
    {
        ...
        var expected = new Dictionary<UniqueAddress, long> { { nodeA, 6 }, { nodeB, 5 }, { nodeC, 7 }, { nodeD, 1 } }
            .ToImmutableDictionary();
    ...
    {
        var r = Reachability.Empty.Unreachable(nodeB, nodeA).Unreachable(nodeC, nodeD).Unreachable(nodeB, nodeC)
            .Unreachable(nodeB, nodeE).Remove(ImmutableHashSet.Create(nodeA, nodeB));
    ...
    {
        var r = Reachability.Empty.Unreachable(nodeC, nodeB).Unreachable(nodeB, nodeA).Unreachable(nodeB, nodeC);
