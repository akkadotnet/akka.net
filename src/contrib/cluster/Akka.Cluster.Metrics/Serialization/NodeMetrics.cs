...
public sealed partial class NodeMetrics : IEquatable<NodeMetrics>
{
    ...
    public NodeMetrics(Address address, long timestamp, IEnumerable<Types.Metric> metrics)
    {
        Address = address;
        Timestamp = timestamp;
    ...
    public NodeMetrics Merge(NodeMetrics that)
    {
        if (!Address.Equals(that.Address))
            throw new ArgumentException($"merge only allowed for same address, {Address} != {that.Address}",
                nameof(that));
    ...
    ///     Gets metric by key
    /// </summary>
    public Option<Types.Metric> Metric(string name)
    {
        return Metrics.FirstOrDefault(m => m.Name == name) ?? Option<Types.Metric>.None;
    }
