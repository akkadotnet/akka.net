...
public class JournalInterceptorsSpecs
{
    ...
    {
        await Awaiting(async () => { await JournalInterceptors.Noop.Instance.InterceptAsync(null); }).Should()
            .NotThrowAsync();
    }
    ...
    {
        var probe = new InterceptorProbe();
        var onCondition = new JournalInterceptors.OnCondition(_ => false, probe, true);
