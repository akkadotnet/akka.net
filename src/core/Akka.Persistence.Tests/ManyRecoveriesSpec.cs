...
public class ManyRecoveriesSpec : PersistenceSpec
{
    ...
    public class TestPersistentActor : UntypedPersistentActor
    {
        ...
        {
            return Actor.Props.Create(() => new TestPersistentActor(name, latch));
        }

        protected override void OnRecover(object message)
        {
            if (message is Evt) Latch?.Ready(TimeSpan.FromSeconds(10));
        }

        protected override void OnCommand(object message)
        {
            if (message is Cmd cmd)
                Persist(new Evt(cmd.Data), _ => { Sender.Tell($"{PersistenceId}-{cmd.Data}-{LastSequenceNr}", Self); });
            else if (message is "stop") Context.Stop(Self);
        }
    }
}