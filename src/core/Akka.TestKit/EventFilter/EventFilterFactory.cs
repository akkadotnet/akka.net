...
public partial class EventFilterFactory
{
    ...
    ///     </example>
    ///     <remarks>
    ///         Please note that filtering on the <paramref name="source" /> being
    ...
    ///     Optional. When set to <c>true</c> not only the top level exception is matched, but
    ///     inner exceptions are also checked until one matches. Default: <c>false</c>
    /// </param>
    /// <returns>The new filter</returns>
    public IEventFilterApplier Exception(Type exceptionType, Regex pattern, string source = null,
    ...
    ///     <para>
    ///         <paramref name="message" /> takes priority over <paramref name="start" />.
    ...
    /// <param name="contains">
    ///     Optional. If specified (and neither <paramref name="message" /> nor <paramref name="start" />
    ///     are specified), the event must contain the string to be filtered.
    /// </param>
    /// <param name="start">
    ///     >Optional. If specified (and <paramref name="message" /> is not specified, the event must start
    ///     with the string to be filtered.
    /// </param>
    /// <param name="source">>Optional. The event source.</param>
    /// <returns>The new filter</returns>
    public IEventFilterApplier Exception<TException>(string message = null, string start = null, string contains = null,
        string source = null) where TException : Exception
    {
    ...
    /// </param>
    /// <returns>The new filter</returns>
    public IEventFilterApplier Exception(Type exceptionType, string message = null, string start = null,
        string contains = null, string source = null, bool checkInnerExceptions = false)
    {
    ...
    private IEventFilterApplier Exception(Type exceptionType, IStringMatcher messageMatcher,
        IStringMatcher sourceMatcher, bool checkInnerExceptions)
    {
    ...
    ///     This is the same as calling <see cref="Debug(string,string,string,string)" />,
    ///     <see cref="Info(string,string,string,string)" />
    ...
    public IEventFilterApplier ForLogLevel(LogLevel logLevel, string message = null, string start = null,
        string contains = null, string source = null)
    {
        switch (logLevel)
        {
            ...
            default:
                throw new ArgumentOutOfRangeException(nameof(logLevel),
                    $"Unknown {typeof(LogLevel).Name} value: {logLevel}");
        }
    }
    ...
    /// </exception>
    /// <returns>
    ///     An event filter that matches the given <paramref name="logLevel" /> with the supplied
    ...
    /// <summary>
    ///     Creates a filter that catches dead letters of the specified type and matches the predicate, and optionally from the
    ///     specified source.
    /// </summary>
    /// <typeparam name="TMessage">TBD</typeparam>
    ...
    {
        return DeadLetter(deadLetter => type.IsInstanceOfType(deadLetter.Message) && isMatch(deadLetter.Message),
            source);
    }
    ...
    protected IEventFilterApplier CreateApplier(EventFilterBase filter, ActorSystem system)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < existingFiltersCount; i++) allFilters[i] = _filters[i];
