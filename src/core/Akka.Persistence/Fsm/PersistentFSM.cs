    ...
    where TState : IFsmState
{
    ...
    /// </summary>
    private TimeSpan? CurrentStateTimeout { get; set; }
    ...
    protected override bool ReceiveRecover(object message)
    {
        ...
        if (message is SnapshotOffer snapshotOffer)
        {
            ...
            {
                StartWith(StatesMap[persistentFSMSnapshot.StateIdentifier], persistentFSMSnapshot.Data,
                    persistentFSMSnapshot.Timeout);
                return true;
            }
...
public static class PersistentFSM
{
    ...
    public class State<TS, TD, TE>
    {
        ...
        {
            return
                $"State<TS, TD, TE><StateName: {StateName}, StateData: {StateData}, Timeout: {Timeout}, StopReason: {StopReason}, Notifies: {Notifies}>";
        }
    }
}
