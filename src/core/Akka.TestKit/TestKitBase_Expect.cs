...
public abstract partial class TestKitBase
{
    ...
    public ValueTask<T> ExpectMsgAsync<T>(
        ...
        CancellationToken cancellationToken = default)
    {
        return InternalExpectMsgAsync(RemainingOrDilated(duration), (Action<T>)null, hint, cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        return InternalExpectMsgAsync<T>(
            RemainingOrDilated(timeout),
            m => _assertions.AssertEqual(message, m),
            hint,
            cancellationToken);
    }
    ...
    {
        return InternalExpectMsgAsync<T>(
            RemainingOrDilated(RemainingOrDilated(timeout)),
            (m, sender) =>
            {
        ...
        CancellationToken cancellationToken = default)
    {
        return InternalExpectMsgAsync<T>(
            RemainingOrDilated(timeout),
            actual => _assertions.AssertEqual(expected, actual, comparer, hint),
            hint,
            cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return InternalExpectMsgAsync<Terminated>(
            RemainingOrDilated(timeout),
            terminated => { _assertions.AssertEqual(target, terminated.ActorRef, msg); },
            msg,
            cancellationToken);
    }
        ...
        bool shouldLog = false)
    {
        ...
        {
            const string failMessage2 =
                "Failed: Expected a message of type {0}, but received {{{2}}} (type {1}) instead {3} from {4}";
    ...
    {
        ExpectNoMsgAsync(cancellationToken).GetAwaiter().GetResult();
    }
    ...
    {
        ExpectNoMsgAsync(milliseconds, cancellationToken).GetAwaiter().GetResult();
    }
    ...
    {
        return InternalExpectNoMsgAsync(TimeSpan.FromMilliseconds(milliseconds), cancellationToken);
    }
    ...
    {
        var enumerable =
            InternalExpectMsgAllOfAsync(RemainingOrDefault, messages, cancellationToken: cancellationToken);
    ...
    {
        if (_testState.TestKitSettings.LogTestKitCalls) _testState.Log.Debug(format, args);
    }
}