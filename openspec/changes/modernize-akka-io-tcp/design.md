...
- **Safety property that makes the disposal matrix survivable:** `RentedMemoryOwner.Dispose()` is already idempotent (nulls its array on first call). Teardown paths may therefore attempt disposal from more than one party (e.g. the coalescing stage's abort handler *and* `TcpConnection`'s drain) and only the first does anything — the failure mode degrades from leak/use-after-return to a harmless double-dispose. The hand-enforced invariant is unchanged: never dispose an owner whose bytes could still be read (dispose only at the pipe copy in steady state; on abort paths only where the write is provably dropped and no read will occur).
