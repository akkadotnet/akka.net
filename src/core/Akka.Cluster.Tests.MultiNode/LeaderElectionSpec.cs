...
public abstract class LeaderElectionSpec : MultiNodeClusterSpec
{
    ...
    public void ShutdownLeaderAndVerifyNewLeader(int alreadyShutdown)
    {
        ...
        else if (Myself == aUser)
        {
            ...
            AwaitAssert(() =>
                ClusterView.UnreachableMembers.Select(x => x.Address).Contains(leaderAddress).ShouldBeTrue());
            ...
            // removed
            AwaitAssert(() =>
                ClusterView.UnreachableMembers.Select(x => x.Address).Contains(leaderAddress).ShouldBeFalse());
