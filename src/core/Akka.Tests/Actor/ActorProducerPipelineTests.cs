...
{
    private readonly ActorProducerPipelineResolver _resolver;
    ...
    public async Task Pipeline_should_allow_to_register_multiple_generic_plugins_with_different_generic_types()
    {
        ...
        (await plugA.Ask<string[]>("plugins", TimeSpan.FromSeconds(3))).ShouldOnlyContainInOrder("working plugin",
            typeof(PlugActorA).ToString());
        (await plugB.Ask<string[]>("plugins", TimeSpan.FromSeconds(3))).ShouldOnlyContainInOrder("working plugin",
    ...
    public async Task Pipeline_application_should_apply_plugins_in_specified_order()
    {
        ...
        var actor = ActorOf<PlugActor>();
        (await actor.Ask<string[]>("plugins", TimeSpan.FromSeconds(3))).ShouldOnlyContainInOrder("plugin-1", "plugin-2",
            "plugin-3");
    }
    ...
    internal class StashingActor : ReceiveActor, IWithUnboundedStash
    {
        ...
        {
            Receive<StashStatus>(
                _ => Sender.Tell("actor stash is " + (Stash != null ? "initialized" : "uninitialized")));
