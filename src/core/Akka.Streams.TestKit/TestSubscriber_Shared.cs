...
public static partial class TestSubscriber
{
    public partial class ManualProbe<T>
    {
        ...
        internal static async Task ExpectEventTask(TestProbe probe, ISubscriberEvent e,
            ...
            CancellationToken cancellationToken)
        {
            await probe.ExpectMsgAsync<OnNext<T>>(
                x => AssertEquals(x.Element, element, "Expected '{0}', but got '{1}'", element, x.Element),
                timeout,
                cancellationToken: cancellationToken);
        }
        ...
        internal static async Task ExpectNextUnorderedTask(ManualProbe<T> probe, TimeSpan? timeout,
            CancellationToken cancellationToken, params T[] elems)
        {
            var len = elems.Length;
            var e = await probe.ExpectNextNAsync(len, timeout, cancellationToken)
            ...
            Assert(expectedSet.Count == 0, "unexpected elements [{0}] found in the result",
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var x in list)
                await probe.ExpectMsgAsync<OnNext<T>>(
                    y => AssertEquals(y.Element, x, "Expected one of ({0}), but got '{1}'", string.Join(", ", list),
                        y.Element),
                    timeout,
                    cancellationToken: cancellationToken);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            while (collection.Count > 0)
            {
                ...
                Assert(collection.Contains(next),
                    $"expected one of (${string.Join(", ", collection)}), but received {next}");
                collection.Remove(next);
            }
        }

        internal static async Task ExpectCompleteTask(TestProbe probe, TimeSpan? timeout,
            CancellationToken cancellationToken)
        {
        ...
        internal static async Task ExpectNextOrCompleteTask(TestProbe probe, T element,
            CancellationToken cancellationToken)
        {
        ...
        internal static async Task<ISubscriberEvent> ExpectEventTask(
            ...
            CancellationToken cancellationToken = default)
        {
            return await probe.ExpectMsgAsync<ISubscriberEvent>(max, cancellationToken: cancellationToken)
                .ConfigureAwait(false);
        }
            ...
            CancellationToken cancellationToken = default)
        {
            var message = await probe.FishForMessageAsync(
                m => m is OnNext<T> or TestSubscriber.OnError,
        ...
        {
            var message = await probe.FishForMessageAsync(
                m => m is OnNext<T> or TestSubscriber.OnComplete,
                ...
                switch (e)
                {
                    ...
                    default:
                        throw new InvalidOperationException(
                            $"Invalid response, expected {nameof(OnError)}, {nameof(OnComplete)}, or {nameof(OnNext<T>)}, received [{e.GetType()}]");
                }
            }
        }
