...
public class KeepGoingStageSpec : AkkaSpec
{
    ...
    [Fact]
    public async Task
        A_stage_with_keep_going_must_still_be_alive_after_all_ports_have_been_closed_until_explicity_closed()
    {
    ...
    [Fact]
    public async Task
        A_stage_with_keep_going_must_still_be_alive_after_all_ports_have_been_closed_until_explicitly_failed()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var t = Source.Maybe<int>().ToMaterialized(new PingableSink(true), Keep.Both).Run(Materializer);
    ...
    [Fact]
    public async Task
        A_stage_with_keep_going_must_still_be_alive_after_all_ports_have_been_closed_until_implicity_failed_via_exception()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var t = Source.Maybe<int>().ToMaterialized(new PingableSink(true), Keep.Both).Run(Materializer);
    ...
    private sealed class PingRef
    {
        ...
        public void Register(IActorRef probe)
        {
            _cb(new Register(probe));
        }

        public void Ping()
        {
            _cb(KeepGoingStageSpec.Ping.Instance);
        }

        public void Stop()
        {
            _cb(CompleteStage.Instance);
        }

        public void Fail()
        {
            _cb(FailStage.Instance);
        }
    ...
    private sealed class PingableSink : GraphStageWithMaterializedValue<SinkShape<int>, Task<PingRef>>
    {
        ...
        public override ILogicAndMaterializedValue<Task<PingRef>> CreateLogicAndMaterializedValue(
        ...
        private sealed class PingableLogic : GraphStageLogic
        {
            ...
            {
                _listener.Tell(KeepGoingStageSpec.PostStop.Instance);
            }
