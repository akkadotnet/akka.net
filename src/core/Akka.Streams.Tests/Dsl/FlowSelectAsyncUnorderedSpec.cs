...
public class FlowSelectAsyncUnorderedSpec : AkkaSpec
{
    ...
    public async Task A_Flow_with_SelectAsyncUnordered_must_not_run_more_futures_than_requested_elements()
    {
        ...
        {
            foreach (var i in Enumerable.Range(2, 19)) got.Add(await c.ExpectNextAsync());
    ...
    public async Task A_Flow_with_SelectAsyncUnordered_must_resume_after_multiple_failures()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var futures = new[]
            {
                Task.Run(() =>
                {
                    throw new TestException("failure1");
                    return "";
                }),
                Task.Run(() =>
                {
                    throw new TestException("failure2");
                    return "";
                }),
                Task.Run(() =>
                {
                    throw new TestException("failure3");
                    return "";
                }),
                Task.Run(() =>
                {
                    throw new TestException("failure4");
                    return "";
                }),
                Task.Run(() =>
                {
                    throw new TestException("failure5");
                    return "";
                }),
