...
public abstract partial class TestKitBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        return InternalExpectMsgAsync<T>(
            ...
            senderAssert: s => _assertions.AssertEqual(
                expected: sender,
                actual: s,
                format: FormatWrongSenderMessage(s, sender.ToString() ?? "null", hint)),
            hint: null,
            cancellationToken: cancellationToken);
    }
        ...
        object? message)
    {
        _assertions.AssertTrue(
            isSender(sender),
            FormatWrongSenderMessage(sender, hint ?? "the predicate to return true", null) + $" The message was {{{message?.ToString() ?? "null"}}}");
    }
