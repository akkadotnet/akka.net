...
public sealed class ClusterClient : ActorBase
{
    ...
    private bool Establishing(object message)
    {
        ...
        switch (message)
        {
            ...
            case ReconnectTimeout:
                _log.Warning("Receptionist reconnect not successful within {0} stopping cluster client",
                    _settings.ReconnectTimeout);
                Context.Stop(Self);
                return true;
    ...
    private void Buffer(object message)
    {
        ...
        {
            _log.Warning("Receptionist not available and buffering is disabled, dropping message [{0}]",
                message.GetType().Name);
        }
        else if (_buffer.Count == _settings.BufferSize)
        {
            var m = _buffer.Dequeue();
            _log.Warning("Receptionist not available, buffer is full, dropping first message [{0}]",
                m.Item1.GetType().Name);
    ...
    public sealed class Publish : IClusterClientProtocolMessage, IEquatable<Publish>
    {
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                return ((Topic != null ? Topic.GetHashCode() : 0) * 397) ^
    ...
    internal sealed class RefreshContactsTick : IClusterClientProtocolMessage, IEquatable<RefreshContactsTick>
    {
        ...
        public bool Equals(RefreshContactsTick other)
        {
            return other != null;
        }
    ...
    internal sealed class HeartbeatTick : IClusterClientProtocolMessage, IEquatable<HeartbeatTick>
    {
        ...
        public bool Equals(HeartbeatTick other)
        {
            return other != null;
        }
    ...
    internal sealed class ReconnectTimeout : IClusterClientProtocolMessage, IEquatable<ReconnectTimeout>
    {
        ...
        public bool Equals(ReconnectTimeout other)
        {
            return other != null;
        }
