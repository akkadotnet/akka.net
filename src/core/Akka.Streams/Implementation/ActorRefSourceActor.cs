...
internal class ActorRefSourceActor<T> : Actors.ActorPublisher<T>
{
    ...
    /// <see cref="ActorRefSourceActor{T}.Props" />
    /// as well!
    ...
    ///     This exception is thrown when the specified <paramref name="overflowStrategy" /> is
    ///     <see cref="Akka.Streams.OverflowStrategy.Backpressure" />.
    /// </exception>
    /// <returns>TBD</returns>
    ...
    {
        return DefaultReceive(message) || RequestElement(message) ||
               (message is T message1 && ReceiveElement(message1));
    }
