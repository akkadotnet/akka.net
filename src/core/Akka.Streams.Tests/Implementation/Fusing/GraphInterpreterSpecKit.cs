...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    {
        var interpreter =
            new BaseBuilderSetup<T>(Sys).Build(new BaseBuilder.Upstream(), ops, new BaseBuilder.Downstream());
        spec(interpreter);
    }
    ...
    public abstract class BaseBuilder
    {
        ...
        public void ManualInit(GraphAssembly assembly)
        {
            ...
            Interpreter = new GraphInterpreter(assembly, NoMaterializer.Instance, _logger, logics, connections,
                (_, _, _) => { }, false, null);
        }
        ...
        public class AssemblyBuilder
        {
            ...
            public AssemblyBuilder(ILoggingAdapter logger, Action<GraphInterpreter> interpreterSetter,
                IEnumerable<IGraphStageWithMaterializedValue<Shape, object>> stages)
            {
                _logger = logger;
            ...
            {
                _upstreams.Add((upstream, inlet));
                return this;
            }

            public AssemblyBuilder Connect<T>(Outlet<T> outlet,
                GraphInterpreter.DownstreamBoundaryStageLogic downstream)
            {
                _downstreams.Add((outlet, downstream));
                return this;
            }

            public AssemblyBuilder Connect<T>(Outlet<T> outlet, Inlet<T> inlet)
            {
                _connections.Add((outlet, inlet));
                return this;
            }
            ...
            public void Init()
            {
                ...
                var connections = mat.Item1;
                var logics = mat.Item2;
                var interpreter = new GraphInterpreter(assembly, NoMaterializer.Instance, _logger, logics, connections,
                    (_, _, _) => { }, false, null);
                ...
                foreach (var downstream in _downstreams)
                    interpreter.AttachDownstreamBoundary(connections[i++ + _upstreams.Count + _connections.Count],
                        downstream.Item2);
    ...
    public class BaseBuilderSetup<T> : BaseBuilder
    {
        ...
        public GraphInterpreter Build(GraphInterpreter.UpstreamBoundaryStageLogic upstream,
            GraphStage<FlowShape<T, T>>[] ops, GraphInterpreter.DownstreamBoundaryStageLogic downstream)
        {
    ...
    public class TestSetup : BaseBuilder
    {
        ...
        public class UpstreamProbe<T> : GraphInterpreter.UpstreamBoundaryStageLogic
        {
            ...
            public UpstreamProbe(TestSetup setup, string name)
            {
                ...
                var probe = this;
                SetHandler(Outlet, () => setup.LastEvent.Add(new RequestOne(probe)),
                    cause => setup.LastEvent.Add(new Cancel(probe, cause)));
            }
...
#pragma warning restore CS0162
    ...
    public class PortTestSetup : TestSetup
    {
        ...
        public PortTestSetup(ActorSystem system, bool chasing = false) : base(system)
        {
            ...
            var assembly = !chasing
                ? new GraphAssembly(Array.Empty<IGraphStageWithMaterializedValue<Shape, object>>(),
                    Array.Empty<Attributes>(),
        ...
        {
            Interpreter.Execute(!_chasing ? 1 : 2);
        }
    ...
    public abstract class OneBoundedSetup : BaseBuilder
    {
        ...
        public class Cancel : ITestEvent
        {
            ...
            protected bool Equals(Cancel other)
            {
                return Cause.GetType() == other.Cause.GetType()
                       && Cause.Message == other.Cause.Message;
            }
    ...
    {
        public OneBoundedSetup(ActorSystem system, params IGraphStageWithMaterializedValue<Shape, object>[] ops) :
            base(system)
        {
            Ops = ops;
