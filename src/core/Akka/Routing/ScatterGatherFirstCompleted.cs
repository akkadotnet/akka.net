...
/// <summary>
///     This class represents a single point <see cref="Routee" /> that sends messages to a <see cref="Routee" />
///     determined
///     using scatter-gather-first-completed. This process has the router send a message to all of its routees. The first
...
internal sealed class ScatterGatherFirstCompletedRoutees : Routee
{
    ...
    {
        if (_routees.IsNullOrEmpty()) return new Status.Failure(new AskTimeoutException("Timeout due to no routees"));
...
/// <summary>
///     This class represents a <see cref="Pool" /> router that sends messages to a <see cref="Routee" /> determined using
///     scatter-gather-first-completed.
///     This process has the router send a message to all of its routees. The first response is used and the remaining are
///     discarded. If the none of the
...
public sealed class ScatterGatherFirstCompletedPool : Pool
{
    ...
    ///     Initializes a new instance of the <see cref="ScatterGatherFirstCompletedPool" /> class.
    ///     <note>
    ///         A <see cref="ScatterGatherFirstCompletedPool" /> configured in this way uses the
    ///         <see cref="Pool.DefaultSupervisorStrategy" /> supervisor strategy.
    ///     </note>
    /// </summary>
    /// <param name="nrOfInstances">The initial number of routees in the pool.</param>
    public ScatterGatherFirstCompletedPool(int nrOfInstances) : this(nrOfInstances, null, TimeSpan.Zero,
        DefaultSupervisorStrategy, null)
    {
    }
    ...
    {
        return new ScatterGatherFirstCompletedPool(NrOfInstances, Resizer, Within, strategy, RouterDispatcher,
            UsePoolDispatcher);
    }
    ...
    {
        return new ScatterGatherFirstCompletedPool(NrOfInstances, resizer, Within, SupervisorStrategy, RouterDispatcher,
            UsePoolDispatcher);
    }
    ...
    {
        return new ScatterGatherFirstCompletedPool(NrOfInstances, Resizer, Within, SupervisorStrategy, dispatcher,
            UsePoolDispatcher);
    }
    ...
    public class ScatterGatherFirstCompletedPoolSurrogate : ISurrogate
    {
        ...
        {
            return new ScatterGatherFirstCompletedPool(NrOfInstances, Resizer, Within, SupervisorStrategy,
                RouterDispatcher, UsePoolDispatcher);
        }
    }
...
/// <summary>
///     This class represents a <see cref="Group" /> router that sends messages to a <see cref="Routee" /> determined using
///     scatter-gather-first-completed.
///     This process has the router send a message to all of its routees. The first response is used and the remaining are
///     discarded. If the none of the
///     routees respond within a specified time limit, a timeout failure occurs.
...
public sealed class ScatterGatherFirstCompletedGroup : Group
{
    ...
    /// <param name="within">The amount of time to wait for a response.</param>
    /// <param name="routerDispatcher">The dispatcher to use when passing messages to the routees.</param>
    public ScatterGatherFirstCompletedGroup(IEnumerable<string> paths, TimeSpan within, string routerDispatcher) : base(
        paths, routerDispatcher)
    {
        Within = within;
    }
    ...
    public override ISurrogate ToSurrogate(ActorSystem system)
    {
        ...
        {
            Paths = InternalPaths, Within = Within, RouterDispatcher = RouterDispatcher
        };
    }
