...
**As implemented for 1.6**, the generator natively recognizes exactly four collection shapes: `T[]`, `List<T>`, `IReadOnlyList<T>` (deserializing to a concrete `List<T>` internally), and `Dictionary<TKey,TValue>` (`AkkaSerializerGenerator.TryMapCollection`; round-trip and wire-format coverage in `CollectionFieldSpec.cs`). Collections compose (nested `List<List<int>>`, jagged arrays, dictionaries of lists, collections of nested `[AkkaSerializable]` types), and an unsupported element type collapses the whole field to `FieldKind.Unsupported` (AKKASG003) rather than partially generating. The immutable collection types (`ImmutableArray<T>`, `ImmutableList<T>`, `ImmutableHashSet<T>`, `ImmutableDictionary<TKey,TValue>`) and `IReadOnlyCollection<T>`/`IReadOnlyDictionary<TKey,TValue>` originally scoped here were not implemented in this change and remain future work (tasks.md 5.7).
...
Both features are scoped to the current compilation, the same rule every other generator lookup follows (see the cross-assembly discussion in Decision 8): a union member type or a closed-generic registration declared only in a referenced assembly is invisible to this generator.
