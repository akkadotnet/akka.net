...
public class DistributedPubSubMediator : ReceiveActor
{
    ...
    private readonly IDictionary<Address, Bucket> _registry = new Dictionary<Address, Bucket>();
    ...
    public DistributedPubSubMediator(DistributedPubSubSettings settings)
    {
        ...
        if (!string.IsNullOrEmpty(_settings.Role) && !_cluster.SelfRoles.Contains(_settings.Role))
            throw new ArgumentException(
                $"The cluster member [{_cluster.SelfAddress}] doesn't have the role [{_settings.Role}]");

        //Start periodic gossip to random nodes in cluster
        _gossipCancelable = Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(_settings.GossipInterval,
            _settings.GossipInterval, Self, GossipTick.Instance, Self);
        _pruneInterval = new TimeSpan(_settings.RemovedTimeToLive.Ticks / 2);
        _pruneCancelable =
            Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(_pruneInterval, _pruneInterval, Self,
                Prune.Instance, Self);
        ...
        Receive<SendToAll>(sendToAll => { PublishMessage(sendToAll.Path, sendToAll, sendToAll.ExcludeSelf); });
        ...
        Receive<RegisterTopic>(register => { HandleRegisterTopic(register.TopicRef); });
        ...
        Receive<GetTopics>(_ => { Sender.Tell(new CurrentTopics(GetCurrentTopics().ToImmutableHashSet())); });
        Receive<Subscribed>(subscribed => { subscribed.Subscriber.Tell(subscribed.Ack); });
        Receive<Unsubscribe>(unsubscribe =>
        {
            ...
            {
                var child = Context.Child(encodedTopic);
                if (!child.IsNobody()) child.Forward(unsubscribe);
                // no such topic here
            });
        });
        Receive<Unsubscribed>(unsubscribed => { unsubscribed.Subscriber.Tell(unsubscribed.Ack); });
        Receive<Status>(status =>
        {
            ...
            if (_nodes.Contains(Sender.Path.Address) || Sender.Path.Address.HasLocalScope)
            {
                ...
                if (!status.IsReplyToStatus && OtherHasNewerVersions(status.Versions))
                    Sender.Tell(new Status(OwnVersions, true)); // it will reply with Delta
            }
        });
        Receive<Delta>(delta =>
        {
                ...
                foreach (var bucket in delta.Buckets)
                    if (_nodes.Contains(bucket.Owner))
                    {
                        ...
                        if (bucket.Version > myBucket.Version)
                            _registry[bucket.Owner] = new Bucket(myBucket.Owner, bucket.Version,
                                myBucket.Content.SetItems(bucket.Content));
                    }
        });
        ...
        Receive<DeltaCount>(_ => { Sender.Tell(deltaCount); });
    ...
    private IEnumerable<Bucket> CollectDelta(IImmutableDictionary<Address, long> versions)
    {
        ...
        foreach (var version in versions) filledOtherVersions[version.Key] = version.Value;
        ...
        foreach (var entry in filledOtherVersions)
        {
            ...
            if (bucket.Version > v && count < _settings.MaxDeltaElements)
            {
                ...
                else
                {
                    ...
                    var chunk = sortedContent.Take(_settings.MaxDeltaElements - (count - sortedContent.Length))
                        .ToList();
        ...
        foreach (var entry in _registry)
        {
            ...
            foreach (var kv in bucket.Content)
            {
                ...
                if (key.StartsWith(topicPrefix))
                {
                    ...
                    if (!topic.Contains('/')) yield return Uri.EscapeDataString(topic);
                }
            }
        }
    }
    ...
    private void PutToRegistry(string key, IActorRef value)
    {
        ...
        if (!_registry.TryGetValue(_cluster.SelfAddress, out var bucket))
            _registry.Add(_cluster.SelfAddress,
                new Bucket(_cluster.SelfAddress, v,
                    ImmutableDictionary<string, ValueHolder>.Empty.Add(key, new ValueHolder(v, value))));
        else
            _registry[_cluster.SelfAddress] =
    ...
    private void PublishMessage(string path, IWrappedMessage publish, bool allButSelf = false)
    {
        IEnumerable<IActorRef> Refs()
        {
            foreach (var entry in _registry)
            {
                ...
                if (!(allButSelf && address == _cluster.SelfAddress) &&
                    bucket.Content.TryGetValue(path, out var valueHolder))
    ...
    private IEnumerable<KeyValuePair<string, Routee>> ExtractGroups(string prefix, string lastKey)
    {
        ...
        foreach (var keyVal in bucket.Content.Where(kv =>
                     kv.Key.CompareTo(prefix) != -1 && kv.Key.CompareTo(lastKey) != 1))
    ...
    private void HandlePrune()
    {
        ...
        foreach (var entry in _registry)
        {
            ...
            var oldRemoved = bucket.Content
                .Where(kv =>
                    kv.Value.Ref.IsNobody() && bucket.Version - kv.Value.Version >
                    _settings.RemovedTimeToLive.TotalMilliseconds)
                .Select(kv => kv.Key);

            if (oldRemoved.Any())
                modifications.Add(owner,
                    new Bucket(bucket.Owner, bucket.Version, bucket.Content.RemoveRange(oldRemoved)));
        }

        foreach (var entry in modifications) _registry[entry.Key] = entry.Value;
    }
    ...
    private void GossipTo(Address address)
    {
        ...
        sel.Tell(new Status(OwnVersions, false));
    }
    ...
    private IActorRef NewTopicActor(string encodedTopic)
    {
        var t = Context.ActorOf(
            Actor.Props.Create(() => new Topic(_settings.RemovedTimeToLive, _settings.RoutingLogic,
                    _settings.SendToDeadLettersWhenNoSubscribers))
