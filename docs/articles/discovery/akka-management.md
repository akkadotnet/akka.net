...
When developing or testing, you may need to run multiple Akka.NET nodes on the same host. Manually assigning distinct remote for each process can work for a fixed seed-node strategy, but doing so for management ports would not work with `Akka.Discovery` and `Akka.Management.Cluster.Bootstrap`.

`Cluster.Bootstrap` expects that all hosts to have the same management port as it is. When `Akka.Discovery` returns a list of open ports on a host, the bootstrap coordinator will filter out any ports that are different from the locally configured management port, ignoring the rest.
