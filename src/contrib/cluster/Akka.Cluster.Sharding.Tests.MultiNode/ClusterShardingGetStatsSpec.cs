...
public class ClusterShardingGetStatsSpecConfig : MultiNodeClusterShardingConfig
{
            ...
            ")
    {
        ...
        NodeConfig(new[] { First, Second, Third },
            new[] { ConfigurationFactory.ParseString(@"akka.cluster.roles=[""shard""]") });
    }
...
public class ClusterShardingGetStatsSpec : MultiNodeClusterShardingSpec<ClusterShardingGetStatsSpecConfig>
{
    ...
    private void Inspecting_cluster_sharding_state_must_join_cluster()
    {
        ...
        RunOn(() => { StartShard(); }, Config.First, Config.Second, Config.Third);
    ...
    {
        RunOn(() => { Cluster.Get(Sys).Leave(Node(Config.Third).Address); }, Config.Controller);

        RunOn(
            () =>
            {
                Within(TimeSpan.FromSeconds(30),
                    () => { AwaitAssert(() => { Cluster.Get(Sys).State.Members.Count.Should().Be(3); }); });
