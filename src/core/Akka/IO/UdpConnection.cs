...
internal class UdpConnection : ActorBase, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
{
    ...
    private (Send send, IActorRef sender)? _pendingSend;
    ...
    private void DoConnect(EndPoint address)
    {
        ReportConnectFailure(() =>
        {
            ...
            foreach (var option in _connect.Options) option.BeforeDatagramBind(_socket);
            ...
            foreach (var v2 in _connect.Options.OfType<Inet.SocketOptionV2>()) v2.AfterConnect(_socket);
