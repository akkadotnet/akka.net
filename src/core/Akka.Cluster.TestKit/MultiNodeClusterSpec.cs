...
public abstract class MultiNodeClusterSpec : MultiNodeSpec
{
    ...
    private readonly ConcurrentDictionary<RoleName, Address> _cachedAddresses = new();

    private readonly RoleNameComparer _roleNameComparer;
    ...
    internal ClusterReadView ClusterView => Cluster.ReadView;
    ...
    public static Config ClusterConfigWithFailureDetectorPuppet()
    {
        return ConfigurationFactory.ParseString(
                string.Format(@"akka.cluster.failure-detector.implementation-class = ""{0}""",
                    typeof(FailureDetectorPuppet).AssemblyQualifiedName))
    ...
    public virtual void MuteLog(ActorSystem sys = null)
    {
        ...
        if (!sys.Log.IsDebugEnabled)
        {
            var patterns = new[]
            {
                ".*Metrics collection has started successfully.*", ".*Cluster Node.* - is starting up.*",
                ".*Shutting down cluster Node.*", ".*Cluster node successfully shut down.*",
    ...
    public void AwaitClusterUp(params RoleName[] roles)
    {
        ...
        if (roles.Contains(Myself)) AwaitMembersUp(roles.Length);
    ...
    public void AssertLeaderIn(ImmutableList<RoleName> nodesInCluster)
    {
        ...
        _assertions.AssertTrue(isLeader == IsNode(expectedLeader), "expected leader {0}, got leader {1}, members{2}",
    ...
    {
        private readonly Address _target;
        ...
        protected override void OnReceive(object message)
        {
            ...
            if (message is EndAck) _testActor.Forward(EndAck.Instance);
        }
    ...
    {
        private readonly MultiNodeClusterSpec _spec;
