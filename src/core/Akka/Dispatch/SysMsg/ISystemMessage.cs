...
///     The type of the list also encodes that the messages contained are in reverse order, i.e. the head of the list is
///     the
...
internal struct LatestFirstSystemMessageList
{
    ...
    /// <summary>
    ///     Reverses the list. This operation mutates the underlying list. The cost of the call is O(N), where N is the number
    ///     of elements.
...
internal struct EarliestFirstSystemMessageList
{
    ...
    /// <summary>
    ///     Prepends a list in a reversed order to the head of this list. The prepended list will be reversed during the
    ///     process.
    /// </summary>
...
public abstract class SystemMessage : ISystemMessage
{
    ...
    /// </summary>
    [NonSerialized] internal SystemMessage Next;
...
public sealed class DeathWatchNotification : SystemMessage, IDeadLetterSuppression
{
    ...
    public override string ToString()
    {
        return "<DeathWatchNotification>: " + Actor + ", ExistenceConfirmed=" + ExistenceConfirmed +
               ", AddressTerminated=" + AddressTerminated;
    }
}
