...
namespace Akka.TestKit.Internal
{
    ...
    /// that uses a <see cref="ChannelQueue{T}"/> as its backing store.
    /// <remarks>Note! Part of internal API. Breaking changes may occur without notice. Use at own risk.</remarks>
    /// </summary>
    /// <typeparam name="T">The type of item to store.</typeparam>
    public class BlockingCollectionTestActorQueue<T> : ITestActorQueue<T>
    {
        private readonly ChannelQueue<T> _queue;
        ...
        public BlockingCollectionTestActorQueue(ChannelQueue<T> queue)
        {
            _queue = queue;
        }
