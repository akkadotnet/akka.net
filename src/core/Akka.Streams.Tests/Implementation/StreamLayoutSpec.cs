...
public class StreamLayoutSpec : Akka.TestKit.Xunit2.TestKit
{
    ...
    private static TestAtomicModule TestStage()
    {
        return new TestAtomicModule(1, 1);
    }

    private static TestAtomicModule TestSource()
    {
        return new TestAtomicModule(0, 1);
    }

    private static TestAtomicModule TestSink()
    {
        return new TestAtomicModule(1, 0);
    }
    ...
    public void StreamLayout_should_be_able_to_model_simple_linear_stages()
    {
        ...
        var flow12 = stage1.Compose<object, object, NotUsed>(stage2, Keep.None)
            .Wire(stage1.OutPorts.First(), stage2.InPorts.First());
        ...
        var source012 = source0.Compose<object, object, NotUsed>(flow12, Keep.None)
            .Wire(source0.OutPorts.First(), flow12.InPorts.First());
        ...
        var sink123 = flow12.Compose<object, object, NotUsed>(sink3, Keep.None)
            .Wire(flow12.OutPorts.First(), sink3.InPorts.First());
        ...
        var runnable0123A = source0.Compose<object, object, NotUsed>(sink123, Keep.None)
            .Wire(source0.OutPorts.First(), sink123.InPorts.First());
        source012.Compose<object, object, NotUsed>(sink3, Keep.None)
            .Wire(source012.OutPorts.First(), sink3.InPorts.First());
        source0
    ...
    [Fact]
    public async Task
        StreamLayout_should_not_fail_materialization_when_building_a_large_graph_with_simple_computation_when_starting_from_a_Source()
    {
        var g = Enumerable.Range(1, TooDeepForStack)
    ...
    [Fact]
    public async Task
        StreamLayout_should_not_fail_materialization_when_building_a_large_graph_with_simple_computation_when_starting_from_a_Flow()
    {
        var g = Enumerable.Range(1, TooDeepForStack)
    ...
    [Fact]
    public async Task
        StreamLayout_should_not_fail_materialization_when_building_a_large_graph_with_simple_computation_when_using_Via()
    {
        var g = Enumerable.Range(1, TooDeepForStack)
            .Aggregate(Source.Single(42).MapMaterializedValue(_ => 1), (source, _) => source.Select(x => x));
    ...
    [Fact]
    public async Task
        StreamLayout_should_not_fail_fusing_and_materialization_when_building_a_large_graph_with_simple_computation_when_starting_from_a_Source()
    {
    ...
    [Fact]
    public async Task
        StreamLayout_should_not_fail_fusing_and_materialization_when_building_a_large_graph_with_simple_computation_when_starting_from_a_Flow()
    {
    ...
    [Fact]
    public async Task
        StreamLayout_should_not_fail_fusing_and_materialization_when_building_a_large_graph_with_simple_computation_when_using_Via()
    {
        var g = Source.FromGraph(Fuse.Aggressive(Enumerable.Range(1, TooDeepForStack)
            .Aggregate(Source.Single(42).MapMaterializedValue(_ => 1), (source, _) => source.Select(x => x))));
        ...
        foreach (var atomic in allAtomic)
        {
            foreach (var inPort in atomic.InPorts)
                if (inToSubscriber.TryGetValue(inPort, out var subscriber))
                {
                    ...
                    subscriber.UpstreamModule.OutPorts.Should()
                        .Contain(x => outToPublisher[x].DownstreamPort == inPort);
                }
    ...
    private class TestAtomicModule : AtomicModule
    {
        ...
        {
            var inports = Enumerable.Range(0, inportCount).Select(i => new Inlet<object>(".in" + i))
                .ToImmutableArray<Inlet>();
            var outports = Enumerable.Range(0, outportCount).Select(i => new Outlet<object>(".out" + i))
                .ToImmutableArray<Outlet>();
