...
internal class FanoutOutputs<T, TStreamBuffer> : SubscriberManagement<T, TStreamBuffer>, IOutputs
    where TStreamBuffer : IStreamBuffer<T>
{
    ...
    /// </exception>
    public FanoutOutputs(int maxBufferSize, int initialBufferSize, IActorRef self, IPump pump,
    ...
    {
        return new ActorSubscriptionWithCursor<T>(_self, subscriber);
    }
    ...
    protected override void RequestFromUpstream(long elements)
    {
        DemandCount += elements;
    }
    ...
    ///     TBD
    /// </summary>
    protected override void CancelUpstream()
    {
        IsClosed = true;
    }
}
...
internal sealed class FanoutProcessorImpl<T, TStreamBuffer> : ActorProcessorImpl where TStreamBuffer : IStreamBuffer<T>
{
    ...
    /// <see cref="FanoutProcessorImpl{T,TStreamBuffer}.Props" />
    /// as well!
    ...
    {
        return Actor.Props.Create<FanoutProcessorImpl<T, TStreamBuffer>>(settings, onTerminated)
