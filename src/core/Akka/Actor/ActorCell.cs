...
public partial class ActorCell : IUntypedActorContext, ICell
{
    ...
    public ActorCell(ActorSystemImpl system, IInternalActorRef self, Props props, MessageDispatcher dispatcher,
    ...
    ///     TBD
    /// </summary>
    public ActorSystem System => SystemImpl;
    ...
    public void Init(bool sendSupervise, MailboxType mailboxType)
    {
        ...
        if (System.Mailboxes.ProducesMessageQueue(mailboxType.GetType()) &&
            System.Mailboxes.HasRequiredType(actorClass))
        {
            var req = System.Mailboxes.GetRequiredType(actorClass);
            if (req.IsInstanceOfType(mailbox.MessageQueue))
            {
                createMessage = new Create(); //success
            }
            else
            {
                var gotType = mailbox.MessageQueue == null ? "null" : mailbox.MessageQueue.GetType().FullName;
                createMessage = new Create(new ActorInitializationException(Self,
                    $"Actor [{Self}] requires mailbox type [{req}] got [{gotType}]"));
            }
        }
        else
        {
        ...
        if (sendSupervise) Parent.SendSystemMessage(new Supervise(self, false));
    }
    ...
    protected virtual ActorBase CreateNewActorInstance()
    {
        ...
        if (actor is IInitializableActor initializableActor) initializableActor.Init();
        return actor;
    }
    ...
    public virtual void SendMessage(Envelope message)
    {
        ...
        {
            SystemImpl.EventStream.Publish(new Error(e, _self.Parent.ToString(), ActorType,
    ...
    private Envelope SerializeAndDeserialize(Envelope envelope)
    {
        ...
        catch (Exception e)
        {
            throw new SerializationException(
                $"Failed to serialize and deserialize payload object [{unwrapped.GetType()}]. Envelope: [{envelope}], Actor type: [{Actor.GetType()}]",
                e);
        }
