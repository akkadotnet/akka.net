...
public class UseRoleIgnoredSpec : MultiNodeClusterSpec
{
    ...
    private void A_cluster_must_start_cluster()
    {
        ...
        RunOn(() => { Log.Info("third, roles: " + Cluster.SelfRoles); }, _config.Third);

        // routees for the group routers
        Sys.ActorOf(
            Props.Create(() => new UseRoleIgnoredSpecConfig.SomeActor(new UseRoleIgnoredSpecConfig.GroupRoutee())),
            "foo");
        Sys.ActorOf(
            Props.Create(() => new UseRoleIgnoredSpecConfig.SomeActor(new UseRoleIgnoredSpecConfig.GroupRoutee())),
            "bar");
        ...
        RunOn(() =>
        {
            ...
            var router = Sys.ActorOf(
                new ClusterRouterPool(
                    ...
                    new ClusterRouterPoolSettings(6, 2, false, role)).Props(
            ...
            for (var i = 0; i < iterationCount; i++) router.Tell($"hit-{i}");
        ...
        RunOn(() =>
        {
            ...
            var router = Sys.ActorOf(
                new ClusterRouterGroup(
                        ...
                        new ClusterRouterGroupSettings(6, ImmutableHashSet.Create("/user/foo", "/user/bar"), false,
                            role))
                    .Props(),
                "router-2b");
    ...
    private void A_cluster_must_pool_local_on_role_b()
    {
        RunOn(() =>
        {
            ...
            var router = Sys.ActorOf(
                new ClusterRouterPool(
                    new RoundRobinPool(6),
                    new ClusterRouterPoolSettings(6, 2, true, role)).Props(
                    Props.Create<UseRoleIgnoredSpecConfig.SomeActor>()),
                "router-3");
    ...
    private void A_cluster_must_group_local_on_role_b()
    {
        RunOn(() =>
        {
            ...
            var router = Sys.ActorOf(
                new ClusterRouterGroup(
                        new RoundRobinGroup(paths: null),
                        new ClusterRouterGroupSettings(6, ImmutableHashSet.Create("/user/foo", "/user/bar"), true,
                            role))
                    .Props(),
                "router-3b");
