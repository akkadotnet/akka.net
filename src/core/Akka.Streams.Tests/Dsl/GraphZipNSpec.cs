...
namespace Akka.Streams.Tests.Dsl
{
    public class GraphZipNSpec : TwoStreamsSetup<IImmutableList<int>>
    {
        ...
        public void ZipN_must_work_in_the_happy_case()
        {
            ...
            probe.ExpectNext().Should().AllBeEquivalentTo(new[] {1, 2});
            probe.ExpectNext().Should().AllBeEquivalentTo(new[] {2, 3});

            subscription.Request(1);
            probe.ExpectNext().Should().AllBeEquivalentTo(new[] {3, 4});

            subscription.Request(1);
            probe.ExpectNext().Should().AllBeEquivalentTo(new[] {4, 5});
        ...
        public void ZipN_must_complete_if_one_side_is_available_but_other_already_completed()
        {
            ...
            downstream.ExpectNext().Should().AllBeEquivalentTo(new[] {1, 2});
