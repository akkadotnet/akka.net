...
public class ReplicatorChaosSpec : MultiNodeClusterSpec
{
    ...
    public void Replicator_in_chaotic_cluster_should_replicate_data_in_initial_phase()
    {
        ...
        RunOn(() =>
        {
            ...
            _replicator.Tell(
                Dsl.Update(KeyB, PNCounter.Empty, new WriteTo(2, _timeout), x => x.Increment(_cluster, 20)));
            _replicator.Tell(Dsl.Update(KeyC, GCounter.Empty, new WriteAll(_timeout), x => x.Increment(_cluster, 20)));

            ReceiveN(3).ToImmutableHashSet().Should().BeEquivalentTo(new UpdateSuccess(KeyA, null),
                new UpdateSuccess(KeyB, null), new UpdateSuccess(KeyC, null));
        ...
        RunOn(() =>
        {
            _replicator.Tell(Dsl.Update(KeyX, GCounter.Empty, new WriteTo(2, _timeout),
                x => x.Increment(_cluster, 50)));
    ...
    public void Replicator_in_chaotic_cluster_should_be_available_during_network_split()
    {
        ...
        RunOn(() =>
        {
            ...
            _replicator.Tell(Dsl.Update(KeyF, ORSet<string>.Empty, new WriteTo(2, _timeout),
                x => x.Remove(_cluster, "e2")));
            ExpectMsg(new UpdateSuccess(KeyF, null));
        }, Third);
