    ...
    private void JoinAndRestart()
    {
        ...
        for (var i = 1; i <= _rounds; i++)
        {
            ...
            RunOn(() =>
            {
                restartingSystem = restartingSystem == null
                    ? ActorSystem.Create(Sys.Name, ConfigurationFactory
                        .ParseString($"akka.cluster.roles=[round-{round}]")
                ...
                Within(TimeSpan.FromSeconds(20), () =>
                {
                    AwaitAssert(() =>
                    {
                        ...
                        Cluster.Get(restartingSystem).State.Members.All(x => x.Status == MemberStatus.Up)
                            .ShouldBeTrue();
                    });
                });
            }, _config.Second);
            ...
            RunOn(
                () =>
                {
                    restartingSystem.Terminate().Wait(RemainingOrDefault)
                        .ShouldBeTrue("Timed out when terminating actorsystem");
                }, _config.Second);
