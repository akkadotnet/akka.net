...
public class CurrentAllEventsSpec : Akka.TestKit.Xunit2.TestKit
{
    ...
    private static readonly Config InternalConfig =
        DefaultTimeoutConfig.WithFallback("akka.persistence.query.journal.sql.max-buffer-size = 3");
    ...
    public virtual void ReadJournal_query_CurrentAllEvents_should_find_events_from_offset_exclusive()
    {
        ...
        var offs = probe1
            .ExpectNext<EventEnvelope>(p => p.PersistenceId == "a" && p.SequenceNr == 2L && p.Event.Equals("world"))
            .Offset;
        probe1.Cancel();
