...
internal sealed class RemoteSystemDaemon : VirtualPathContainer
{
    ...
    private readonly Switch _terminating = new();
    ...
    public RemoteSystemDaemon(ActorSystemImpl system, ActorPath path, IInternalActorRef parent, IActorRef terminator,
        ILoggingAdapter log)
    ...
    private void HandleDaemonMsgCreate(DaemonMsgCreate message)
    {
        ...
        if (ActorPath.TryParse(message.Path, out var childPath))
        {
            ...
            var isTerminating = !_terminating.WhileOff(() =>
            {
                var actor = _system.Provider.ActorOf(_system, localProps, supervisor, p, false,
