...
public sealed class RemoteRouterConfig : Pool, IEquatable<RouterConfig>
{
    ...
    public RemoteRouterConfig(Pool local, IEnumerable<Address> nodes)
        : base(local.NrOfInstances, local.Resizer, local.SupervisorStrategy, local.RouterDispatcher,
            local.UsePoolDispatcher)
    {
    ...
    /// </summary>
    public override Resizer Resizer => Local.Resizer;
    ...
    {
        return new RemoteRouterConfigSurrogate { Local = Local, Nodes = Nodes.ToArray() };
    }
