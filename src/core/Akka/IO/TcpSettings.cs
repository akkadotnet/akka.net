...
public class TcpSettings
{
    ...
    public static TcpSettings Create(Config config)
    {
        ...
        return new TcpSettings(
            config.GetString("buffer-pool", "akka.io.tcp.disabled-buffer-pool"),
            config.GetInt("nr-of-socket-async-event-args", 32),
            config.GetBoolean("trace-logging"),
            config.GetInt("batch-accept-limit", 10),
            config.GetTimeSpan("register-timeout", TimeSpan.FromSeconds(5)),
            config.GetString("max-received-message-size", "unlimited") == "unlimited"
                ? int.MaxValue
                : config.GetInt("max-received-message-size"),
            config.GetString("management-dispatcher", "akka.actor.default-dispatcher"),
            config.GetString("file-io-dispatcher", "akka.actor.default-dispatcher"),
            config.GetString("file-io-transferTo-limit") == "unlimited"
                ? int.MaxValue
                : config.GetInt("file-io-transferTo-limit", 512 * 1024),
            config.GetInt("finish-connect-retries", 5),
            config.GetBoolean("outgoing-socket-force-ipv4"),
            config.GetInt("write-commands-queue-max-size", -1));
    }
}