...
public sealed class BackoffSupervisor : BackoffSupervisorBase
{
    ...
    public BackoffSupervisor(
        ...
        TimeSpan minBackoff,
        TimeSpan maxBackoff,
        double randomFactor) : this(childProps, childName, minBackoff, maxBackoff, new AutoReset(minBackoff),
        randomFactor, Actor.SupervisorStrategy.DefaultStrategy)
    {
    }
        ...
        SupervisorStrategy strategy)
    {
        return Actor.Props.Create(
            () => new BackoffSupervisor(childProps, childName, minBackoff, maxBackoff, new AutoReset(minBackoff),
                randomFactor, strategy, null, null));
    }
    ...
    /// <summary>
    ///     Send this message to the <see cref="BackoffSupervisor" /> and it will reply with <see cref="CurrentChild" />
    ...
    /// <summary>
    ///     Send this message to the <see cref="BackoffSupervisor" /> and it will reset the back-off. This should be used in
    ///     conjunction with `withManualReset` in <see cref="BackoffOptionsImpl" />.
    /// </summary>
    [Serializable]
