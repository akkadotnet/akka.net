...
public class LastElementSpec : Akka.TestKit.Xunit2.TestKit
{
    [Fact]
    public async Task
        A_stream_via_LastElement_should_materialize_to_the_last_element_emitted_by_a_finite_nonempty_successful_source()
    {
    ...
    [Fact]
    public async Task
        A_stream_via_LastElement_should_materialize_to_materialize_to_None_for_an_empty_successful_source()
    {
    ...
    public void A_stream_via_LastElement_should_materialize_to_the_last_element_emitted_by_a_source_before_it_failed()
    {
        var t = Source.UnfoldInfinite(1, n => n >= 3 ? throw new Exception() : (n + 1, n + 1))
            .ViaMaterialized(new LastElement<int>(), Keep.Right)
            .ToMaterialized(Sink.Aggregate<int, Option<int>>(Option<int>.None, (_, o) => Option<int>.Create(o)),
                Keep.Both)
            .Run(Sys.Materializer());
