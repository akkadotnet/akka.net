...
internal sealed class EndpointManager : ReceiveActor, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
{
    ...
    public EndpointManager(Config config, ILoggingAdapter log)
    {
        ...
        _log = log;
        _eventPublisher = new EventPublisher(Context.System, log,
            Logging.LogLevelFor(_settings.RemoteLifecycleEventsLogLevel));
    ...
    private TimeSpan PruneInterval
    {
        get
        {
            ...
            if (RetryGateEnabled)
                return _settings.RetryGateClosedFor.Add(_settings.RetryGateClosedFor).Max(TimeSpan.FromSeconds(1))
    ...
    public sealed class ListensResult : INoSerializationVerificationNeeded
    {
        ...
        public ListensResult(TaskCompletionSource<IList<ProtocolTransportAddressPair>> addressesPromise,
            List<(ProtocolTransportAddressPair, TaskCompletionSource<IAssociationEventListener>)> results)
        {
            Results = results;
            AddressesPromise = addressesPromise;
        }
        ...
        ///     TBD
        /// </summary>
        public IList<(ProtocolTransportAddressPair, TaskCompletionSource<IAssociationEventListener>)> Results { get; }
    }
    ...
    public sealed class ListensFailure : INoSerializationVerificationNeeded
    {
        ...
        public ListensFailure(TaskCompletionSource<IList<ProtocolTransportAddressPair>> addressesPromise,
            Exception cause)
        {
            Cause = cause;
            AddressesPromise = addressesPromise;
                ...
                switch (e)
                {
                    case HopelessAssociation h when h.Uid != null:
                        _log.Error(e.InnerException ?? e,
                            "Association to [{0}] with UID [{1}] is irrecoverably failed. Quarantining address.",
                            h.RemoteAddress, h.Uid);
                        if (_settings.QuarantineDuration.HasValue && _settings.QuarantineDuration != TimeSpan.MaxValue)
                        {
                            ...
                            _endpoints.MarkAsQuarantined(h.RemoteAddress, h.Uid.Value,
                    ...
                    default: // no UID found
                        _log.Warning(
                            "Association to [{0}] with unknown UID is irrecoverably failed. Address cannot be quarantined without knowing the UID, gating instead for {1} ms.",
            ...
            switch (ex)
            {
                case InvalidAssociation ia:
                    KeepQuarantinedOr(ia.RemoteAddress, () =>
                    {
                        ...
                        _log.Warning(
                            "Tried to associate with unreachable remote address [{0}]. Address is now gated for {1} ms, all messages to this address will be delivered to dead letters. Reason: [{2}] {3}",
                    ...
                    if (ia.DisassociationInfo.HasValue && ia.DisassociationInfo == DisassociateInfo.Quarantined)
                        Context.System.EventStream.Publish(
                            new ThisActorSystemQuarantinedEvent(ia.LocalAddress, ia.RemoteAddress));
                    return Directive.Stop;
                case ShutDownAssociation shutdown:
                    KeepQuarantinedOr(shutdown.RemoteAddress, () =>
                    {
                        _log.Debug(
                            "Remote system with address [{0}] has shut down. Address is now gated for {1}ms, all messages to this address will be delivered to dead letters.",
                ...
                case ActorInitializationException i when i.InnerException is HopelessAssociation h2:
                    return Hopeless(h2);
                default:
                    if (ex is EndpointDisassociatedException or EndpointAssociationException)
                    {
                    } //no logging
                    else
                    {
    ...
    private void Receiving()
    {
        ...
        Receive<ManagementCommand>(_ => Sender.Tell(new ManagementCommandAck(false)));
    ...
    private void Accepting()
    {
        Receive<ManagementCommand>(mc =>
        {
            ...
            Task.WhenAll(allStatuses)
                .ContinueWith(x => { return new ManagementCommandAck(x.Result.All(y => y)); }, CancellationToken.None,
        ...
        Receive<Quarantine>(quarantine =>
        {
            ...
            else if (policy.Item1 is Pass p && policy.Item2 != null)
            {
                ...
                {
                    _endpoints.MarkAsQuarantined(quarantine.RemoteAddress, quarantineUid.Value,
                        Deadline.Now + _settings.QuarantineDuration);
                    _eventPublisher.NotifyListeners(new QuarantinedEvent(quarantine.RemoteAddress,
                        quarantineUid.Value));
            ...
            else if (policy.Item1 is Quarantined quarantined && policy.Item2 != null &&
                     quarantined.Uid == policy.Item2.Value)
            {
            ...
            else if (policy.Item2 != null)
            {
                ...
                _endpoints.MarkAsQuarantined(quarantine.RemoteAddress, policy.Item2.Value,
                    Deadline.Now + _settings.QuarantineDuration);
            ...
            else if (readPolicy.Item1?.Item1 != null && quarantine.Uid != null &&
                     readPolicy.Item1?.Item2 == quarantine.Uid)
            {
                Context.Stop(readPolicy.Item1.Value.Item1);
            } // nothing to stop
...
#pragma warning restore CS0618
        ...
        Receive<Send>(send =>
        {
            ...
            IActorRef CreateAndRegisterWritingEndpoint()
            {
                return _endpoints.RegisterWritableEndpoint(recipientAddress,
                    CreateEndpoint(recipientAddress, send.Recipient.LocalAddressToUse,
                        _transportMapping[send.Recipient.LocalAddressToUse],
                        _settings, true), null);
            }
        ...
        Receive<Terminated>(terminated =>
        {
            ...
            HandleStashedInbound(terminated.ActorRef, false);
        });
        ...
        Receive<ReliableDeliverySupervisor.GotUid>(gotuid =>
        {
            ...
            switch (policy)
            {
                case Pass pass:
                    ...
                    HandleStashedInbound(Sender, false);
                    break;
            }
        });
        Receive<ReliableDeliverySupervisor.Idle>(_ => { HandleStashedInbound(Sender, true); });
        ...
        Receive<ShutdownAndFlush>(_ =>
        {
            ...
            shutdownStatus.ContinueWith(tr => Task.WhenAll(_transportMapping.Values.Select(x => x.Shutdown()))
                    .ContinueWith(
                        result =>
                        {
                            if (result.IsFaulted || result.IsCanceled)
            ...
            foreach (var handoff in _pendingReadHandoffs.Values) handoff.Disassociate(DisassociateInfo.Shutdown);
    ...
    private void HandleInboundAssociation(InboundAssociation ia, bool writerIsIdle)
    {
        ...
        if (readonlyEndpoint != null)
        {
            ...
            if (_pendingReadHandoffs.TryGetValue(endpoint, out var protocolHandle))
                protocolHandle.Disassociate("the existing readOnly association was replaced by a new incoming one",
                    _log);
            ...
            else
            {
                ...
                switch (policy)
                {
                    ...
                    case Pass pWithUid: // pass with known UID
                        ...
                        {
                            _pendingReadHandoffs.GetOrElse(pWithUid.Endpoint, null)?.Disassociate(
                                "the existing writable association was replaced by a new incoming one", _log);
                        ...
                        else
                        {
                            ...
                            _endpoints.MarkAsQuarantined(handle.RemoteAddress, pWithUid.Uid.Value,
                                Deadline.Now + _settings.QuarantineDuration);
                            CreateAndRegisterEndpoint(handle);
                        }
        ...
        get
        {
            if (_listens == null)
            {
                ...
                foreach (var transportSettings in _settings.Transports)
                {
                    ...
                    {
                        var ei = ExceptionDispatchInfo.Capture(ex);
                        var task =
                            new Task<List<(ProtocolTransportAddressPair, TaskCompletionSource<IAssociationEventListener>
                                )>>(() =>
                            {
                                ei.Throw();
                                return null;
                    ...
                    var wrappedTransport = transportSettings.Adapters
                        .Select(x => TransportAdaptersExtension.For(Context.System).GetAdapterProvider(x)).Aggregate(
                            driver,
                    ...
                    //The chain at this point:
                    // AkkaProtocolTransport <-- Adapter <-- .. <-- Adapter <-- Driver
                    transports.Add(new AkkaProtocolTransport(wrappedTransport, Context.System,
                        new AkkaProtocolSettings(_conf), new AkkaPduProtobuffCodec(Context.System)));
                }
                ...
                var tasks = transports.Select(x => x.Listen().ContinueWith(
                    result => (new ProtocolTransportAddressPair(x, result.Result.Item1), result.Result.Item2),
                    TaskContinuationOptions.ExecuteSynchronously));
                _listens = Task.WhenAll(tasks).ContinueWith(transportResults => transportResults.Result.ToList(),
                    TaskContinuationOptions.ExecuteSynchronously);
            }
    ...
    {
        Debug.Assert(_transportMapping.ContainsKey(localAddress));
                ...
                Context.ActorOf(RARP.For(Context.System)
                        .ConfigureDispatcher(
                            EndpointWriter.EndpointWriterProps(handleOption, localAddress, remoteAddress, refuseUid,
                                    transport, endpointSettings, new AkkaPduProtobuffCodec(Context.System),
                                    _receiveBuffers)
                                .WithDeploy(Deploy.Local)),
