...
/// <summary>
///     INTERNAL API
///     Holds a `Subscriber` representing the input side of the flow. The `Subscriber` can later be connected to an
///     upstream `Publisher`.
/// </summary>
/// <typeparam name="TOut">TBD</typeparam>
[InternalApi]
...
/// <summary>
///     INTERNAL API
///     Creates and wraps an actor into <see cref="IPublisher{T}" /> from the given <see cref="Props" />, which should be
///     props for an <see cref="ActorPublisher{T}" />.
/// </summary>
/// <typeparam name="TOut">TBD</typeparam>
[InternalApi]
...
public sealed class ActorRefSource<TOut> : SourceModule<TOut, IActorRef>
{
    ...
    /// <param name="attributes">TBD</param>
    /// <param name="shape">TBD</param>
    public ActorRefSource(int bufferSize, OverflowStrategy overflowStrategy, Attributes attributes,
        SourceShape<TOut> shape) : base(shape)
    {
    ...
    public override IPublisher<TOut> Create(MaterializationContext context, out IActorRef materializer)
    {
        ...
        materializer = mat.ActorOf(context,
            ActorRefSourceActor<TOut>.Props(_bufferSize, _overflowStrategy, mat.Settings));
