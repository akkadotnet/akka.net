...
public class ShardRegionSpec : AkkaSpec
{
    ...
    private readonly ActorSystem sysA;
    private readonly ActorSystem sysB;
    ...
    public void ClusterSharding_must_initialize_cluster_and_allocate_sharded_actors()
    {
        ...
        AwaitAssert(() => { Cluster.Get(sysA).SelfMember.Status.Should().Be(MemberStatus.Up); },
    ...
    public void ClusterSharding_must_only_deliver_buffered_RestartShard_to_the_local_region()
    {
        ...
        var region1Shards = StatesFor(region1, p1, 2);
        var region2Shards = StatesFor(region2, p2, 1);
        ...
        var shardIdToMove = int.Parse(region2Shards.First());
        ...
        AwaitAssert(() => { ExpectTerminated(region2); });

        // Difficult to raise the RestartShard in conjunction with the rebalance for mode=ddata
        AwaitAssert(() => { AwaitRebalance(region1, shardIdToMove, p1).Should().BeTrue(); });

        var rebalancedOnRegion1 = StatesFor(region1, p1, numberOfShards);
        AwaitAssert(() => { rebalancedOnRegion1.Count.Should().Be(numberOfShards); }, TimeSpan.FromSeconds(5));
