...
public class Tokenizer
{
    ...
    protected string PickErrorLine(out int index)
    {
        ...
        if (from == -1) from = 0;
        else from += Environment.NewLine.Length;
    ...
    {
        while (!EoF && char.IsWhiteSpace(Peek())) Take();
    }
}
...
public class HoconTokenizer : Tokenizer
{
    ...
    public void PullWhitespaceAndComments()
    {
        ...
        {
            PullWhitespace();
            while (IsStartOfComment()) PullComment();
        } while (IsWhitespace());
    }
    ...
    public Token PullNext()
    {
        ...
        if (IsDot()) return PullDot();
        if (IsObjectStart()) return PullStartOfObject();
        if (IsEndOfObject()) return PullEndOfObject();
        if (IsAssignment()) return PullAssignment();
        if (IsInclude()) return PullInclude();
        if (IsStartOfQuotedKey()) return PullQuotedKey();
        if (IsUnquotedKeyStart()) return PullUnquotedKey();
        if (IsArrayStart()) return PullArrayStart();
        if (IsArrayEnd()) return PullArrayEnd();
    ...
    {
        var sb = new StringBuilder();
        while (!EoF && IsUnquotedKey()) sb.Append(Take());
    ...
    public Token PullValue()
    {
        ...
        if (IsStartOfTripleQuotedText()) return PullTripleQuotedText();

        if (IsStartOfQuotedText()) return PullQuotedText();

        if (IsUnquotedText()) return PullUnquotedText();
        if (IsArrayStart()) return PullArrayStart();
        if (IsArrayEnd()) return PullArrayEnd();
        if (IsSubstitutionStart()) return PullSubstitution();
    ...
    public Token PullSubstitution()
    {
        var sb = new StringBuilder();
        Take(2);
        while (!EoF && IsUnquotedText()) sb.Append(Take());
        Take();
    ...
    {
        var sb = new StringBuilder();
        while (IsSpaceOrTab()) sb.Append(Take());
        return Token.LiteralValue(sb.ToString());
    }
