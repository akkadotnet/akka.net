...
public class ReceiveTests : AkkaSpec
{
    ...
    public async Task FishForMessageAsync_should_fill_the_all_messages_param_if_not_null()
    {
        ...
        await probe.FishForMessageAsync<string>(s => s == "3", allMessages);
    ...
    public async Task FishForMessageAsync_should_clear_the_all_messages_param_if_not_null_before_filling_it()
    {
        ...
        await probe.FishForMessageAsync<string>(x => x == "3", allMessages);
        allMessages.Should().BeEquivalentTo(new ArrayList { "1", 2 });
    }
    ...
    public async Task FishUntilMessageAsync_should_succeed_with_good_input()
    {
        ...
        await Awaiting(() => probe.FishUntilMessageAsync<int>(TimeSpan.FromMilliseconds(10)))
    ...
    {
        var probe = CreateTestProbe("probe");
        var messages = await probe.WaitForRadioSilenceAsync(TimeSpan.FromMilliseconds(0));
    ...
    public async Task WaitForRadioSilenceAsync_should_reset_timer_twice_only()
    {
        ...
        var task = probe.WaitForRadioSilenceAsync(max, 2);
    ...
    public async Task WaitForRadioSilenceAsync_should_fail_immediately_with_bad_input()
    {
        var probe = CreateTestProbe("probe");
        probe.Ref.Tell(3, TestActor);
        await Awaiting(() => probe.WaitForRadioSilenceAsync(TimeSpan.FromMilliseconds(0), 0))
    ...
    [Fact]
    public async Task
        ReceiveWhileAsync_Predicate_should_break_when_predicate_returns_false_and_return_correct_messages()
    {
        TestActor.Tell("1");
        TestActor.Tell("2");
        TestActor.Tell("3");
    ...
    [Fact]
    public async Task
        ReceiveWhileAsync_Predicate_should_break_when_type_is_wrong_and_we_dont_ignore_those_and_return_correct_messages()
    {
        TestActor.Tell("1");
        TestActor.Tell("2");
        TestActor.Tell("3");
    ...
    [Fact]
    public async Task
        ReceiveWhileAsync_Predicate_should_continue_when_type_is_other_but_we_ignore_other_types_and_return_correct_messages()
    {
        TestActor.Tell("1");
        TestActor.Tell("2");
        TestActor.Tell("3");
