    ...
    BatchWriterSettings BatchWriterSettings)
{
    ...
    public static DotNettyTransportSettings Create(Config config, SslSettings? sslSettings = null)
    {
        ...
        return new DotNettyTransportSettings(
            transportMode == "tcp" ? TransportMode.Tcp : TransportMode.Udp,
            enableSsl,
            config.GetTimeSpan("connection-timeout", TimeSpan.FromSeconds(15)),
            host,
            !string.IsNullOrEmpty(publicHost) ? publicHost : host,
            config.GetInt("port", 2552),
            publicPort > 0 ? publicPort : null,
            ComputeWorkerPoolSize(config.GetConfig("server-socket-worker-pool")),
            ComputeWorkerPoolSize(config.GetConfig("client-socket-worker-pool")),
            ToNullableInt(config.GetByteSize("maximum-frame-size", null)) ?? 128000,
            enableSsl ? SslSettings.CreateOrDefault(config.GetConfig("ssl"), sslSettings) : SslSettings.Empty,
            config.GetBoolean("dns-use-ipv6"),
            ResolveTcpReuseAddrOption(config.GetString("tcp-reuse-addr", "off-for-windows")),
            config.GetBoolean("tcp-keepalive", true),
            config.GetBoolean("tcp-nodelay", true),
            config.GetInt("backlog", 4096),
            RuntimeDetector.IsMono || config.GetBoolean("enforce-ip-family"),
            ToNullableInt(config.GetByteSize("receive-buffer-size", null) ?? 256000),
            ToNullableInt(config.GetByteSize("send-buffer-size", null) ?? 256000),
            ToNullableInt(config.GetByteSize("write-buffer-high-water-mark", null)),
            ToNullableInt(config.GetByteSize("write-buffer-low-water-mark", null)),
            config.GetBoolean("enable-backwards-compatibility"),
            config.HasPath("log-transport") && config.GetBoolean("log-transport"),
            order,
    ...
    private static int ComputeWorkerPoolSize(Config config)
    {
        ...
        return ThreadPoolConfig.ScaledPoolSize(
            config.GetInt("pool-size-min", 2),
            config.GetDouble("pool-size-factor", 1.0),
            config.GetInt("pool-size-max", 2));
    }
...
internal sealed class SslSettings
{
    ...
    private SslSettings(string certificateThumbprint, string storeName, StoreLocation storeLocation,
    ...
    private SslSettings(string certificatePath, string certificatePassword, X509KeyStorageFlags flags,
        bool suppressValidation)
    {
        if (string.IsNullOrEmpty(certificatePath))
            throw new ArgumentNullException(nameof(certificatePath),
                "Path to SSL certificate was not found (by default it can be found under `akka.remote.dot-netty.tcp.ssl.certificate.path`)");
    ...
    private static SslSettings Create(Config config)
    {
        if (config.IsNullOrEmpty())
            throw new ConfigurationException(
                $"Failed to create {typeof(DotNettyTransportSettings)}: DotNetty SSL HOCON config was not found (default path: `akka.remote.dot-netty.tcp.ssl`)");
            ...
            || config.GetBoolean("certificate.use-thumbprint-over-file"))
        {
            ...
            if (string.IsNullOrWhiteSpace(thumbprint))
                throw new Exception(
                    "`akka.remote.dot-netty.tcp.ssl.certificate.use-thumbprint-over-file` is set to true but `akka.remote.dot-netty.tcp.ssl.certificate.thumbprint` is null or empty");
        ...
        var flags = flagsRaw.Aggregate(X509KeyStorageFlags.DefaultKeySet,
            (flag, str) => flag | ParseKeyStorageFlag(str));

        return new SslSettings(
            config.GetString("certificate.path"),
            config.GetString("certificate.password"),
            flags,
            config.GetBoolean("suppress-validation"));
    }
