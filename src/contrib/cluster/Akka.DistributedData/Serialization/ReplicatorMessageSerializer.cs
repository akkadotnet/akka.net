...
public sealed class ReplicatorMessageSerializer : SerializerWithStringManifest
{
    ...
    public ReplicatorMessageSerializer(ExtendedActorSystem system) : base(system)
    {
        ...
        var cacheTtl =
            system.Settings.Config.GetTimeSpan("akka.cluster.distributed-data.serializer-cache-time-to-live");
        ...
        switch (o)
        {
            ...
            default:
                throw new ArgumentException(
                    $"Can't serialize object of type [{o.GetType().FullName}] using [{GetType().FullName}]");
        }
    }
    ...
    public override byte[] ToBinary(object obj)
    {
        switch (obj)
        {
            ...
            default:
                throw new ArgumentException(
                    $"Can't serialize object of type [{obj.GetType().FullName}] using [{GetType().FullName}]");
        }
    }

    private Proto.Msg.Gossip GossipToProto(Gossip gossip)
    {
        var proto = new Proto.Msg.Gossip { SendBack = gossip.SendBack };

        foreach (var entry in gossip.UpdatedData)
            proto.Entries.Add(new Proto.Msg.Gossip.Types.Entry
            {
                Key = entry.Key, Envelope = DataEnvelopeToProto(entry.Value)
            });
    ...
    {
        var proto = new Proto.Msg.GetFailure { Key = OtherMessageToProto(msg.Key) };
    ...
    {
        var proto = new Proto.Msg.NotFound { Key = OtherMessageToProto(msg.Key) };
    ...
    private DurableDataEnvelope DurableDataEnvelopeToProto(Durable.DurableDataEnvelope msg)
    {
        var proto = new DurableDataEnvelope { Data = OtherMessageToProto(msg.Data) };
    ...
    private Proto.Msg.DataEnvelope.Types.PruningEntry PruningToProto(UniqueAddress addr, IPruningState pruning)
    {
        ...
        switch (pruning)
        {
            ...
            case PruningInitialized init:
                ...
                foreach (var address in init.Seen) proto.Seen.Add(SerializationSupport.AddressToProto(address));
                break;
        }
    ...
    {
        return new Proto.Msg.Changed { Key = OtherMessageToProto(msg.Key), Data = OtherMessageToProto(msg.Data) };
    }
    ...
    private Proto.Msg.GetSuccess GetSuccessToProto(GetSuccess msg)
    {
        ...
        {
            Key = OtherMessageToProto(msg.Key), Data = OtherMessageToProto(msg.Data)
        };
    ...
    private Proto.Msg.Get GetToProto(Get msg)
    {
        ...
        var proto = new Proto.Msg.Get { Key = OtherMessageToProto(msg.Key), Timeout = (uint)timeoutInMilis };
    ...
    {
        var proto = new Proto.Msg.Status { Chunk = (uint)status.Chunk, TotChunks = (uint)status.TotalChunks };

        foreach (var entry in status.Digests)
            proto.Entries.Add(new Proto.Msg.Status.Types.Entry
            {
                Key = entry.Key, Digest = ByteString.CopyFrom(entry.Value.ToByteArray())
            });
    ...
    private Proto.Msg.DeltaPropagation DeltaPropagationToProto(DeltaPropagation msg)
    {
        ...
        foreach (var entry in msg.Deltas)
        {
            ...
            {
                Key = entry.Key, FromSeqNr = d.FromSeqNr, Envelope = DataEnvelopeToProto(d.DataEnvelope)
            };
    ...
    {
        var proto = new Proto.Msg.DataEnvelope { Data = OtherMessageToProto(msg.Data) };
    ...
    {
        var proto = new Proto.Msg.Write { Key = write.Key, Envelope = DataEnvelopeToProto(write.Envelope) };
    ...
    {
        var proto = new Proto.Msg.Read { Key = read.Key };
    ...
    public override object FromBinary(byte[] bytes, string manifest)
    {
        switch (manifest)
        {
            ...
            default:
                throw new ArgumentException(
                    $"Unimplemented deserialization of message with manifest '{manifest}' using [{GetType().FullName}]");
        }
    }
    ...
    private Gossip GossipFromBinary(byte[] bytes)
    {
        ...
        foreach (var entry in proto.Entries) builder.Add(entry.Key, DataEnvelopeFromProto(entry.Envelope));

        return new Gossip(
            builder.ToImmutable(),
            proto.SendBack,
            proto.HasToSystemUid ? proto.ToSystemUid : null,
            proto.HasFromSystemUid ? proto.FromSystemUid : null);
    }
    ...
    private ImmutableDictionary<UniqueAddress, IPruningState> PruningFromProto(
        RepeatedField<Proto.Msg.DataEnvelope.Types.PruningEntry> pruning)
    {
        if (pruning.Count == 0) return ImmutableDictionary<UniqueAddress, IPruningState>.Empty;
        ...
        foreach (var entry in pruning)
        {
            ...
            {
                var seen = new Address[entry.Seen.Count];
                var i = 0;
    ...
    {
        var proto = DurableDataEnvelope.Parser.ParseFrom(bytes);
        var data = (IReplicatedData)_ser.OtherMessageFromProto(proto.Data);
        var pruning = PruningFromProto(proto.Pruning);
    ...
    private Get GetFromBinary(byte[] bytes)
    {
        ...
        switch (proto.Consistency)
        {
            ...
            case -1:
                consistency = new ReadAll(timeout);
                break;
            case 1:
                consistency = ReadLocal.Instance;
                break;
            default:
                consistency = new ReadFrom(proto.Consistency, timeout);
                break;
        }
    ...
    private Status StatusFromBinary(byte[] bytes)
    {
        ...
        foreach (var entry in proto.Entries) builder.Add(entry.Key, entry.Digest);
    ...
    private DeltaPropagation DeltaPropagationFromBinary(byte[] bytes)
    {
        ...
        foreach (var entry in proto.Entries)
        {
            ...
            var toSeqNr = entry.ToSeqNr == default ? fromSeqNr : entry.ToSeqNr;
        ...
        where TVal : class
    {
        ...
        public SmallCache(int capacity, TimeSpan ttl, Func<TKey, TVal> getOrAddFactory)
        {
            _mask = capacity - 1;
            if ((capacity & _mask) != 0)
                throw new ArgumentException("Capacity must be power of 2 and less than or equal 32", nameof(capacity));
        ...
        public void Evict()
        {
            if (DateTime.UtcNow - _lastUsed > _ttl) _elements.Initialize();
        }
