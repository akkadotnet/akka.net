...
public abstract class RemoteMessagingThroughputSpecBase
{
    ...
    ///     The name of the <see cref="ActorSystem" />. Needed for
    ///     <see cref="Transport.TestTransport" />.
    /// </param>
    ...
    public void Setup(BenchmarkContext context)
    {
        ...
        System1 = ActorSystem.Create("SystemA" + ActorSystemNameCounter.Next(),
            CreateActorSystemConfig("SystemA" + ActorSystemNameCounter.Current, "127.0.0.1", 0));

        System2 = ActorSystem.Create("SystemB" + ActorSystemNameCounter.Next(),
            CreateActorSystemConfig("SystemB" + ActorSystemNameCounter.Current, "127.0.0.1", 0));
    ...
    public void PingPong(BenchmarkContext context)
    {
        ...
        SpinWait.SpinUntil(() =>
            waiting.IsCompleted); // TODO: would get more accurate results if we could AWAIT and not block here.
    }
    ...
    private class BenchmarkActor : UntypedActor
    {
        ...
        public BenchmarkActor(Counter counter, long maxExpectedMessages, TaskCompletionSource<long> completion,
            IActorRef echo)
        {
            _counter = counter;
