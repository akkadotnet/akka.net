...
public class RemoteWatcherSpec : AkkaSpec
{
    ...
    private int RemoteAddressUid => AddressUidExtension.Uid(_remoteSystem);
    ...
    public async Task A_RemoteWatcher_must_generate_address_terminated_when_missing_first_heartbeat()
    {
        ...
        var monitorA = Sys.ActorOf(new Props(new Deploy(), typeof(TestRemoteWatcher), heartbeatExpectedResponseAfter),
            "monitor5");
        ...
        await WithinAsync(20.Seconds(), async () =>
        {
            await AwaitAssertAsync(async () =>
            {
                ...
                await q.ExpectMsgAsync(new TestRemoteWatcher.Quarantined(b.Path.Address, null),
                    TimeSpan.FromSeconds(1));
            });
            return true;
        });
        ...
        A_RemoteWatcher_must_generate_address_terminated_for_new_watch_after_broken_connection_was_reestablished_and_broken_again()
    {
        ...
        var monitorB = await CreateRemoteActor(new Props(new Deploy(), typeof(TestActorProxy), TestActor), "monitor6");
