...
public sealed class StreamRefSerializer : SerializerWithStringManifest
{
    ...
    private ByteString SerializeSinkRef(SinkRefImpl sinkRef)
    {
        return new SinkRef
        {
    ...
    {
        return SerializationTools.ToSourceRef(sourceRef).ToByteString();
    }

    private ByteString SerializeRemoteStreamCompleted(RemoteStreamCompleted completed)
    {
        return new Proto.Msg.RemoteStreamCompleted { SeqNr = completed.SeqNr }.ToByteString();
    }

    private ByteString SerializeRemoteStreamFailure(RemoteStreamFailure failure)
    {
        return new Proto.Msg.RemoteStreamFailure { Cause = ByteString.CopyFromUtf8(failure.Message) }.ToByteString();
    }

    private ByteString SerializeOnSubscribeHandshake(OnSubscribeHandshake handshake)
    {
        return new Proto.Msg.OnSubscribeHandshake
        {
            TargetRef = new ActorRef
            {
    ...
    {
        return new Proto.Msg.CumulativeDemand { SeqNr = demand.SeqNr }.ToByteString();
    }
    ...
    private ByteString SerializeSequencedOnNext(SequencedOnNext onNext)
    {
        ...
        return new Proto.Msg.SequencedOnNext { SeqNr = onNext.SeqNr, Payload = p }.ToByteString();
    }
}