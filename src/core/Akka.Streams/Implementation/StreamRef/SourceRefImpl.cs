...
internal sealed class
    SourceRefStageImpl<TOut> : GraphStageWithMaterializedValue<SourceShape<TOut>, Task<ISinkRef<TOut>>>
{
    ...
    public override ILogicAndMaterializedValue<Task<ISinkRef<TOut>>> CreateLogicAndMaterializedValue(
        Attributes inheritedAttributes)
    {
        var promise = new TaskCompletionSource<ISinkRef<TOut>>();
        return new LogicAndMaterializedValue<Task<ISinkRef<TOut>>>(new Logic(this, promise, inheritedAttributes),
            promise.Task);
    }
    ...
    private sealed class Logic : TimerGraphStageLogic, IOutHandler
    {
        ...
        public Logic(SourceRefStageImpl<TOut> stage, TaskCompletionSource<ISinkRef<TOut>> promise,
        ...
        public override void PreStart()
        {
            ...
            _requestStrategy = new WatermarkRequestStrategy(_receiveBuffer.Capacity);
        ...
        protected internal override void OnTimer(object timerKey)
        {
            switch (timerKey)
            {
                ...
                case DemandRedeliveryTimerKey:
                    Log.Debug("[{0}] Scheduled re-delivery of demand until [{1}]", StageActorName,
                        _localCumulativeDemand);
        ...
        private void InitialReceive((IActorRef, object) args)
        {
            ...
            switch (message)
            {
                ...
                case RemoteStreamFailure failure:
                    ...
                    Log.Warning("[{0}] The remote stream has failed, failing (reason: {1})", StageActorName,
                        failure.Message);
                    _stageActor.Unwatch(sender);
                    FailStage(new RemoteStreamRefActorTerminatedException(
                        $"Remote stream ({sender.Path}) failed, reason: {failure.Message}"));
                    break;
        ...
        private void OnReceiveElement(object payload)
        {
            ...
            if (_receiveBuffer.IsEmpty && IsAvailable(outlet))
                ...
                throw new IllegalStateException(
                    $"Attempted to overflow buffer! Capacity: {_receiveBuffer.Capacity}, incoming element: {payload}, localRemainingRequested: {_localRemainingRequested}, localCumulativeDemand: {_localCumulativeDemand}");
            else
        ...
        private void ObserveAndValidateSender(IActorRef partner, string failureMessage)
        {
            if (_partnerRef == null)
            {
                Log.Debug("Received first message from {0}, assuming it to be the remote partner for this stage",
                    partner);
                _partnerRef = partner;
        ...
        private long _expectingSeqNr;
        private long _localCumulativeDemand;
        private long _localRemainingRequested;
