...
public class TestKitSettings : IExtension
{
    ...
    public TestKitSettings(Config config)
    {
        ...
        DefaultTimeout = config.GetTimeSpan("akka.test.default-timeout", allowInfinite:false);
        SingleExpectDefault = config.GetTimeSpan("akka.test.single-expect-default", allowInfinite:false);
        TestEventFilterLeeway = config.GetTimeSpan("akka.test.filter-leeway", allowInfinite:false);
        TestTimeFactor = config.GetDouble("akka.test.timefactor");
        LogTestKitCalls = config.GetBoolean("akka.test.testkit.debug");

        if (TestTimeFactor <= 0)
            throw new ConfigurationException(
                $@"Expected a positive value for ""akka.test.timefactor"" but found {TestTimeFactor}");
    }
    ...
    ///         set configuration "akka.test.timefactor" to a positive double/integer.
    ///     </para>
    ///     <para>
