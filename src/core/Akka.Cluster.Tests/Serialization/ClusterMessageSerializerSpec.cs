...
akka.cluster.use-legacy-heartbeat-message = {(useLegacyHeartbeat ? "true" : "false")}", output)
    ...
    public void Can_serialize_GossipStatus()
    {
        ...
        var reachability3 = Reachability.Empty.Unreachable(a1.UniqueAddress, e1.UniqueAddress)
            .Unreachable(b1.UniqueAddress, e1.UniqueAddress);
        var g3 = g2.Copy(ImmutableSortedSet.Create(a1, b1, c1, d1, e1), g2.Overview.Copy(reachability: reachability3));
    ...
    public void Can_serialize_Join()
    {
        ...
        var uniqueAddress = new UniqueAddress(address, 17);
        var message =
            new InternalClusterAction.Join(uniqueAddress, ImmutableHashSet.Create("foo", "bar"), AppVersion.Zero);
        AssertEqual(message);

        message = new InternalClusterAction.Join(uniqueAddress, ImmutableHashSet.Create("foo", "bar"),
            AppVersion.Create("1.2.3"));
        AssertEqual(message);
    }
