...
internal class MsgDecoder : MessageToMessageDecoder<object>
{
    ...
    public static Address Proto2Address(AddressData addr)
    {
        return new Address(addr.Protocol, addr.System, addr.Hostname, (int)addr.Port);
    }

    public static ThrottleTransportAdapter.Direction Proto2Direction(InjectFailure.Types.Direction dir)
    {
        switch (dir)
        {
            case InjectFailure.Types.Direction.Send: return ThrottleTransportAdapter.Direction.Send;
            case InjectFailure.Types.Direction.Receive: return ThrottleTransportAdapter.Direction.Receive;
            case InjectFailure.Types.Direction.Both:
    ...
    protected object Decode(object message)
    {
        ...
        if (w != null)
        {
            ...
            if (w.Barrier != null)
            {
                switch (w.Barrier.Op)
                {
                    case Proto.Msg.EnterBarrier.Types.BarrierOp.Succeeded:
                        return new BarrierResult(w.Barrier.Name, true);
                    ...
                    case Proto.Msg.EnterBarrier.Types.BarrierOp.Enter:
                        return new EnterBarrier(w.Barrier.Name,
                            w.Barrier.Timeout > 0 ? TimeSpan.FromTicks(w.Barrier.Timeout) : null);
                }
            }
