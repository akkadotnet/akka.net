...
///             </li>
///             <li>
///                 Friedemann Mattern (1988). "Virtual Time and Global States of Distributed Systems". Workshop on
...
public sealed class VectorClock
{
    ...
    ///         <ul>
    ///             <li>
    ///                 If we're on <see cref="Ordering.After" />, then we can only go to <see cref="Ordering.After" /> or
    ///                 <see cref="Ordering.Concurrent" />.
    ///             </li>
    ///             <li>
    ///                 If we're on <see cref="Ordering.Before" />, then we can only go to <see cref="Ordering.Before" />Before
    ///                 or <see cref="Ordering.Concurrent" />.
    ///             </li>
    ///             <li>If we go to <see cref="Ordering.Concurrent" />, then we exit the loop immediately></li>
    ///             <li>
    ///                 If you send in the ordering <see cref="Ordering.FullOrder" />FullOrder, then you will get a full
    ///                 comparison.
    ///             </li>
    ///         </ul>
    ///     </para>
    ...
    internal Ordering CompareOnlyTo(VectorClock that, Ordering order)
    {
        ...
        return Compare(Versions.Select(x => (x.Key, x.Value)).GetEnumerator(),
            that.Versions.Select(y => (y.Key, y.Value)).GetEnumerator(),
    ...
    public VectorClock Merge(VectorClock that)
    {
        ...
        foreach (var pair in Versions)
        {
            ...
            if (pair.Value > mergedVersionsCurrentTime) mergedVersions = mergedVersions.SetItem(pair.Key, pair.Value);
        }
    ...
    public VectorClock Prune(Node removedNode)
    {
        ...
        if (!ReferenceEquals(newVersions, Versions)) return Create(newVersions);
        return this;
    }
