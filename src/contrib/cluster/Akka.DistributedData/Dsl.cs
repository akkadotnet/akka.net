...
public static class Dsl
{
    ...
    /// </summary>
    public static ReadLocal ReadLocal => ReadLocal.Instance;

    /// <summary>
    ///     Gets a <see cref="IWriteConsistency" /> setup, which will acknowledge success of an
    ///     <see cref="Update{T}(IKey{T}, T, IWriteConsistency)">Update</see> or <see cref="Delete" /> operation immediately as
    ///     soon, as
    ///     result will be confirmed by the local replica only.
    /// </summary>
    public static WriteLocal WriteLocal => WriteLocal.Instance;
    ...
    {
        return new Update(key, value, consistency ?? WriteLocal, old => old.Merge(value));
    }
    ...
    /// <returns>TBD</returns>
    public static Update Update<T>(IKey<T> key, IWriteConsistency consistency, Func<T, T> modify)
        where T : IReplicatedData
    {
        return new Update(key, consistency, data => modify((T)data));
    }
    ...
    /// <param name="key">Key under which a value should be updated.</param>
    /// <param name="initial">
    ///     Initial value used, when no value has been stored under the provided <paramref name="key" /> so
    ///     far.
    /// </param>
    /// <param name="consistency">Consistency determining how/when response will be emitted.</param>
    /// <param name="modify">An updating function.</param>
    /// <returns>TBD</returns>
    public static Update Update<T>(IKey<T> key, T initial, IWriteConsistency consistency, Func<T, T> modify)
        where T : IReplicatedData
    {
        return new Update(key, initial, consistency, data => modify((T)data));
    }
    ...
    /// <param name="modify">An updating function.</param>
    /// <returns>TBD</returns>
    public static Update Update<T>(IKey<T> key, T initial, IWriteConsistency consistency, object request,
        Func<T, T> modify) where T : IReplicatedData
    {
        return new Update(key, initial, consistency, data => modify((T)data), request);
    }
    ...
    /// </param>
    /// <returns>TBD</returns>
    public static Get Get<T>(IKey<T> key, IReadConsistency consistency = null, object request = null)
        where T : IReplicatedData
    {
        return new Get(key, consistency ?? ReadLocal, request);
    }
    ...
    /// </param>
    /// <returns>TBD</returns>
    public static Delete Delete<T>(IKey<T> key, IWriteConsistency consistency, object request = null)
        where T : IReplicatedData
    {
        return new Delete(key, consistency, request);
    }
    ...
    {
        return new Subscribe(key, subscriber);
    }
    ...
    {
        return new Unsubscribe(key, subscriber);
    }
}