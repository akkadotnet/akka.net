    ...
    internal static class ClusterUserAction
    {
        ...
        internal static class InternalClusterAction
        {
            ...
            public sealed class Subscribe : ISubscriptionMessage, IEquatable<Subscribe>
            {
                ...
                public Subscribe(IActorRef subscriber, ClusterEvent.SubscriptionInitialStateMode initialStateMode, ImmutableHashSet<Type> to)
                {
                ...
                public bool Equals(Subscribe other)
                {
                    if (ReferenceEquals(null, other)) return false;
                    if (ReferenceEquals(this, other)) return true;
                    return Equals(Subscriber, other.Subscriber) && InitialStateMode == other.InitialStateMode && Equals(To, other.To);
                }
                ...
                public override string ToString() => $"Subscribe(subscriber: {Subscriber}, mode: {InitialStateMode}, to: [{string.Join(", ", To)}])";
            }
            ...
            public sealed class Unsubscribe : ISubscriptionMessage, IDeadLetterSuppression, IEquatable<Unsubscribe>
            {
                ...
                public override int GetHashCode()
                {
                    unchecked
                    {
                        return ((Subscriber != null ? Subscriber.GetHashCode() : 0) * 397) ^ (To != null ? To.GetHashCode() : 0);
                    }
                }
        ...
        internal sealed class ClusterDaemon : ActorBase, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
        {
            ...
            public ClusterDaemon(ClusterSettings settings)
            {
                ...
                var self = Self;
                _coordShutdown.AddTask(CoordinatedShutdown.PhaseClusterLeave, "leave", async () =>
                {
                    ...
                    if (!(cluster.IsTerminated || cluster.SelfMember.Status == MemberStatus.Down))
                    {
                        ...
                        await self.Ask<Done>(CoordinatedShutdownLeave.LeaveReq.Instance, timeout);
                    }
                });

                _coordShutdown.AddTask(CoordinatedShutdown.PhaseClusterShutdown, "wait-shutdown", () => _clusterShutdown.Task);
            }
            ...
            {
                _coreSupervisor = Context.ActorOf(Props.Create<ClusterCoreSupervisor>().WithDispatcher(Context.Props.Dispatcher), "core");
            ...
            protected override bool Receive(object message)
            {
                switch (message)
                {
                    ...
                    case InternalClusterAction.AddOnMemberUpListener up:
                        Context.ActorOf(Props.Create(() => new OnMemberStatusChangedListener(up.Callback, MemberStatus.Up)).WithDeploy(Deploy.Local));
                        return true;
                    case InternalClusterAction.AddOnMemberRemovedListener removed:
                        Context.ActorOf(Props.Create(() => new OnMemberStatusChangedListener(removed.Callback, MemberStatus.Removed)).WithDeploy(Deploy.Local));
                        return true;
                    case CoordinatedShutdownLeave.LeaveReq leaveReq:
                        var aref = Context.ActorOf(Props.Create(() => new CoordinatedShutdownLeave()).WithDispatcher(Context.Props.Dispatcher));
        ...
        internal sealed class ClusterCoreSupervisor : ActorBase, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
        {
            ...
            private void CreateChildren()
            {
                var publisher = Context.ActorOf(Props.Create<ClusterDomainEventPublisher>().WithDispatcher(Context.Props.Dispatcher), "publisher");
                _coreDaemon = Context.ActorOf(Props.Create(() => new ClusterCoreDaemon(publisher)).WithDispatcher(Context.Props.Dispatcher), "daemon");
            }
            ...
            protected override SupervisorStrategy SupervisorStrategy() => new OneForOneStrategy(e =>
            {
                ...
                _log.Error(e, "Cluster node [{0}] crashed, [{1}] - shutting down...", Cluster.Get(Context.System).SelfAddress, e);
        ...
        internal class ClusterCoreDaemon : ActorBase, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
        {
            ...
            private readonly MembershipState _membershipState;
            ...
            private void AddCoordinatedLeave()
            {
                ...
                _coordShutdown.AddTask(CoordinatedShutdown.PhaseClusterExitingDone, "exiting-completed", async () =>
                {
                    if (!Cluster.Get(sys).IsTerminated)
                    {
                        var timeout = _coordShutdown.Timeout(CoordinatedShutdown.PhaseClusterExitingDone);
                        await self.Ask(InternalClusterAction.ExitingCompleted.Instance, timeout);
                    }
                });
            }

            private ActorSelection ClusterCore(Address address) => Context.ActorSelection(new RootActorPath(address) / "system" / "cluster" / "core" / "daemon");
            ...
            private void BecomeInitialized()
            {
                ...
                var internalHeartbeatSender = Props.Create<ClusterHeartbeatSender>().WithDispatcher(_cluster.Settings.UseDispatcher);
                Context.ActorOf(internalHeartbeatSender, "heartbeatSender");
                
                var externalHeartbeatSender = Props.Create<CrossDcHeartbeatSender>().WithDispatcher(_cluster.Settings.UseDispatcher);
            ...
            private bool Initialized(object message)
            {
                switch (message)
                {
                    case GossipEnvelope envelope: ReceiveGossip(envelope); return true;
                    case GossipStatus status: ReceiveGossipStatus(status); return true;
                    case InternalClusterAction.Tick tick:
                        switch (tick)
                        {
                            case InternalClusterAction.Tick.GossipTick: GossipTick(); break;
                            case InternalClusterAction.Tick.GossipSpeedupTick: GossipSpeedupTick(); break;
                            case InternalClusterAction.Tick.ReapUnreachableTick: ReapUnreachableMembers(); break;
                            case InternalClusterAction.Tick.LeaderActionsTick: LeaderActions(); break;
                            case InternalClusterAction.Tick.PublishStatsTick: PublishInternalStats(); break;
                        }
                        return true;
                    case InternalClusterAction.InitJoin _: 
                        ...
                        InitJoin();
                        return true;
                    case InternalClusterAction.Join join: Joining(join.Node, join.Roles); return true;
                    case Down down: Downing(down.Address); return true;
                    case Leave leave: Leaving(leave.Address); return true;
                    case InternalClusterAction.SendGossipTo sendGossipTo: SendGossipTo(sendGossipTo.Address); return true;
                    case InternalClusterAction.ISubscriptionMessage msg:
                        _publisher.Forward(msg);
                        return true;
                    case QuarantinedEvent quarantined: Quarantined(new UniqueAddress(quarantined.Address, quarantined.Uid)); return true;
                    case JoinTo joinTo: 
                        _cluster.LogInfo("Trying to join [{0}] when already part of a cluster, ignoring", SelfAddress);
                        return true;
                    case InternalClusterAction.JoinSeedNodes joinSeedNodes:
                        _cluster.LogInfo("Trying to join seed nodes [{0}] when already part of a cluster, ignoring", string.Join(", ", joinSeedNodes.SeedNodes));
                        return true;
            ...
            private void JoinSeedNodes(ImmutableList<Address> newSeedNodes)
            {
                if (!newSeedNodes.IsEmpty)
                {
                    ...
                    {
                        Self.Tell(new JoinTo(_cluster.SelfAddress));
                    ...
                    else
                    {
                        ...
                        {
                            _seedNodeProcess = Context.ActorOf(Props.Create(() => new FirstSeedNodeProcess(newSeedNodes)).WithDispatcher(_cluster.Settings.UseDispatcher),
                                    "firstSeedNodeProcess-" + _seedNodeProcessCounter);
                        }
                        else
                        {
                            _seedNodeProcess = Context.ActorOf(Props.Create(() => new JoinSeedNodeProcess(newSeedNodes)).WithDispatcher(_cluster.Settings.UseDispatcher),
            ...
            private void Join(Address address)
            {
                ...
                {
                    _log.Warning("Trying to join member with wrong protocol, but was ignored, expected [{0}] but was [{1}]", SelfAddress.Protocol, address.Protocol);
                }
                else if (address.System != _cluster.SelfAddress.System)
                {
                    _log.Warning("Trying to join member with wrong ActorSystem name, but was ignored, expected [{0}] but was [{1}]", SelfAddress.System, address.System);
                }
                else
                {
                    //TODO: Akka exception?
            ...
            private void Joining(UniqueAddress node, ImmutableHashSet<string> roles)
            {
                ...
                {
                    _log.Warning("Member with wrong protocol tried to join, but was ignored, expected [{0}] but was [{1}]", _cluster.SelfAddress.Protocol, node.Address.Protocol);
                }
                else if (node.Address.System != _cluster.SelfAddress.System)
                {
                    _log.Warning("Member with wrong ActorSystem name tried to join, but was ignored, expected [{0}] but was [{1}]", _cluster.SelfAddress.System, node.Address.System);
                }
                ...
                else
                {
                    ...
                    else if (localMember != null)
                    {
                        ...
                        if (localMember.Status != MemberStatus.Down)
                        {
                            ...
                            var newReachability = LatestGossip.Overview.Reachability.Terminated(_cluster.SelfUniqueAddress, localMember.UniqueAddress);
            ...
            private void CleanupExitingConfirmed()
            {
                ...
                {
                    _exitingConfirmed = new HashSet<UniqueAddress>(
                        _exitingConfirmed.Where(n => _latestGossip.Members.Any(m => m.UniqueAddress.Equals(n))));
                }
            }
            ...
            public ReceiveGossipType ReceiveGossip(GossipEnvelope envelope)
            {
                ...
                {
                    _log.Debug("Cluster Node [{0}] - Ignoring received gossip from unknown [{1}]", _cluster.SelfAddress,
                        from);
                    return ReceiveGossipType.Ignored;
                }
                ...
                if (!IsSingletonCluster && n > 0)
                {
                    ...
                    var possibleTargets =
                        localGossip.Members.Where(m => ValidNodeForGossip(m.UniqueAddress))
                            ...
                            .ToList();
                    var randomTargets = possibleTargets.Count <= n
                        ? possibleTargets
                        : possibleTargets.Shuffle().Slice(0, n);
            ...
            public void SendGossip()
            {
                if (!IsSingletonCluster)
                {
                    ...
                    if (ThreadLocalRandom.Current.NextDouble() < AdjustedGossipDifferentViewProbability)
                    {
                        ...
                        preferredGossipTarget = ImmutableList.CreateRange(localGossip.Members.Where(m =>
                            !localGossip.SeenByNode(m.UniqueAddress) &&
            ...
            private void MoveJoiningToWeaklyUp()
            {
                ...
                bool IsJoiningToWeaklyUp(Member m) => m.Status == MemberStatus.Joining
                                                      && enoughMembers
                                                      && _latestGossip.ReachabilityExcludingDownedObservers.Value
            ...
            public void LeaderActionsOnConvergence()
            {
                ...
                bool IsJoiningUp(Member m) => (m.Status == MemberStatus.Joining || m.Status == MemberStatus.WeaklyUp) &&
                                              enoughMembers;
                ...
                if (!removedUnreachable.IsEmpty || !removedExitingConfirmed.IsEmpty || !changedMembers.IsEmpty)
                {
                    ...
                    var newVersion = removed.Aggregate(localGossip.Version,
                        (v, node) => { return v.Prune(VectorClock.Node.Create(VclockName(node))); });
                    ...
                    if (!newlyDetectedUnreachableMembers.IsEmpty || !newlyDetectedReachableMembers.IsEmpty)
                    {
                        ...
                        if (!newReachability2.Equals(localOverview.Reachability))
                        {
                            ...
                            if (!newlyDetectedReachableMembers.IsEmpty)
                                _log.Info("Marking node(s) as REACHABLE [{0}]. Node roles [{1}]",
            ...
            public void AssertLatestGossip()
            {
                if (Cluster.IsAssertInvariantsEnabled &&
                    _latestGossip.Version.Versions.Count > _latestGossip.Members.Count)
                {
                    throw new InvalidOperationException(
                        $"Too many vector clock entries in gossip state {_latestGossip}");
                }
            }
        ...
        internal sealed class JoinSeedNodeProcess : UntypedActor
        {
            ...
            protected override void PreStart()
            {
                Self.Tell(new InternalClusterAction.JoinSeedNode());
            }
            ...
            {
                if (message is InternalClusterAction.JoinSeedNode)
                {
        ...
        internal sealed class FirstSeedNodeProcess : UntypedActor
        {
            ...
            public FirstSeedNodeProcess(ImmutableList<Address> seeds)
            {
                ...
                _retryTaskToken = Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(TimeSpan.FromSeconds(1),
                    TimeSpan.FromSeconds(1), Self, new InternalClusterAction.JoinSeedNode(), Self);
                Self.Tell(new InternalClusterAction.JoinSeedNode());
            }
