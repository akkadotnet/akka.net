    ...
    where TConfig : MultiNodeClusterShardingConfig
{
    ...
    protected bool PersistenceIsNeeded => Config.Mode == StateStoreMode.Persistence
                                          || Sys.Settings.Config
                                              .GetString("akka.cluster.sharding.remember-entities-store")
                                              .Equals(RememberEntitiesStore.Eventsourced.ToString(),
    ...
    /// <param name="onJoinedRunOnFrom">
    ///     optionally execute a function after join validation is successful, e.g. start sharding
    ///     or create coordinator
    /// </param>
    /// <param name="assertNodeUp">
    ///     if disabled - false, the joining member's `MemberStatus.Up`
        ...
        RunOn(() =>
        {
            ...
            if (assertNodeUp)
                Within(max ?? TimeSpan.FromSeconds(20),
                    () => { AwaitAssert(() => { Cluster.State.IsMemberUp(Node(from).Address).Should().BeTrue(); }); });
    ...
    protected void SetStore(ActorSystem sys, RoleName storeOn)
    {
        ...
        sys.ActorSelection(Node(storeOn) / "system" / "akka.persistence.journal.inmem")
            .Tell(new Identify(null), journalProbe.Ref);
        ...
        sys.ActorSelection(Node(storeOn) / "system" / "akka.persistence.snapshot-store.inmem")
            .Tell(new Identify(null), snapshotProbe.Ref);
    ...
    protected void StartPersistence(RoleName startOn, params RoleName[] setStoreOn)
    {
        ...
        RunOn(() => { SetStore(Sys, startOn); }, setStoreOn);
