...
/// <summary>
///     INTERNAL API: <see cref="ShardRegion" /> and <see cref="PersistentShardCoordinator" /> actors are created as
...
internal sealed class ClusterShardingGuardian : ReceiveActor
{
                ...
                if (settings.RememberEntities)
                {
                    ...
                    var rememberEntitiesProvider =
                        settings.StateStoreMode == StateStoreMode.Persistence
                            ? RememberEntitiesStore.Eventsourced
                    ...
                    switch (rememberEntitiesProvider)
                    {
                        case RememberEntitiesStore.DData:
                            rememberEntitiesStoreProvider =
                                new DDataRememberEntitiesProvider(start.TypeName, settings, _majorityMinCap,
                                    replicator);
                            break;
                        case RememberEntitiesStore.Eventsourced:
                            rememberEntitiesStoreProvider =
                                new EventSourcedRememberEntitiesProvider(start.TypeName, settings);
                            break;
                        case RememberEntitiesStore.Custom:
                            rememberEntitiesStoreProvider =
                                new CustomStateStoreModeProvider(start.TypeName, Context.System, settings);
                            break;
                    }
                }
                ...
                var shardRegion = Context.Child(encName).GetOrElse(() =>
                {
                    if (Equals(Context.Child(coordinatorSingletonManagerName), ActorRefs.Nobody))
                    {
                        ...
                        var coordinatorProps = settings.StateStoreMode == StateStoreMode.Persistence
                            ...
                            : DDataShardCoordinator.Props(start.TypeName, settings, start.AllocationStrategy,
                                replicator, _majorityMinCap, rememberEntitiesStoreProvider);
                        ...
                        var singletonSettings = settings.CoordinatorSingletonSettings.WithSingletonName("singleton")
                            .WithRole(settings.Role);
                        Context.ActorOf(
                            ClusterSingletonManager
                                .Props(singletonProps, ShardCoordinator.Terminate.Instance, singletonSettings)
                    ...
                    return Context.ActorOf(ShardRegion.Props(
                            ...
                            settings,
                            coordinatorPath,
                            start.MessageExtractor,
                            start.HandOffStopMessage,
                            rememberEntitiesStoreProvider)
    ...
    private IActorRef Replicator(ClusterShardingSettings settings)
    {
        if (settings.StateStoreMode is StateStoreMode.DData or StateStoreMode.Custom)
        {
            ...
            var name = string.IsNullOrEmpty(settings.Role)
                ? "replicator"
                : Uri.EscapeDataString(settings.Role) + "Replicator";
            var replicatorRef =
                Context.ActorOf(DistributedData.Replicator.Props(GetReplicatorSettings(settings)), name);
    ...
    {
        return (Self.Path / CoordinatorSingletonManagerName(encName) / "singleton" / "coordinator")
    ...
    public sealed class Start : INoSerializationVerificationNeeded
    {
        ...
        ///     This exception is thrown when the specified <paramref name="typeName" /> or <paramref name="entityProps" /> is
        ///     undefined.
        /// </exception>
        public Start(
            string typeName,
            ...
            object handOffStopMessage)
        {
            if (string.IsNullOrEmpty(typeName))
                throw new ArgumentNullException(nameof(typeName),
                    "ClusterSharding start requires type name to be provided");
            if (entityProps == null)
                throw new ArgumentNullException(nameof(entityProps),
                    $"ClusterSharding start requires Props for [{typeName}] to be provided");
    ...
    public sealed class StartProxy : INoSerializationVerificationNeeded
    {
            ...
            IMessageExtractor messageExtractor)
        {
            if (string.IsNullOrEmpty(typeName))
                throw new ArgumentNullException(nameof(typeName),
                    "ClusterSharding start proxy requires type name to be provided");
