...
public class FlowAskSpec : AkkaSpec
{
    ...
    [Fact]
    public async Task Flow_with_ask_must_produce_asked_elements()
    {
    ...
    [Fact]
    public async Task Flow_with_ask_must_produce_asked_elements_for_simple_ask()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
            var replyOnInts =
    ...
    [Fact]
    public async Task Flow_with_ask_must_produce_asked_elements_when_response_is_Status_Success()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
            var statusReplier =
                Sys.ActorOf(Props.Create(() => new StatusReplier()).WithDispatcher("akka.test.stream-dispatcher"),
                    "statusReplier");
            var c = this.CreateManualSubscriberProbe<Reply>();
    ...
    {
        var replyRandomDelays =
            Sys.ActorOf(Props.Create(() => new RandomDelaysReplier()).WithDispatcher("akka.test.stream-dispatcher"),
                "replyRandomDelays");
        var c = this.CreateManualSubscriberProbe<Reply>();
    ...
    [Fact]
    public async Task Flow_with_ask_must_signal_ask_timeout_failure()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
            var dontReply = Sys.ActorOf(BlackHoleActor.Props.WithDispatcher("akka.test.stream-dispatcher"),
                "dontReply");
            var c = this.CreateManualSubscriberProbe<Reply>();
    ...
    [Fact]
    public async Task Flow_with_ask_must_signal_ask_failure()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
    ...
    public async Task Flow_with_ask_signal_failure_when_target_actor_is_terminated()
    {
        ...
        {
            var r = Sys.ActorOf(Props.Create(() => new Replier()).WithDispatcher("akka.test.stream-dispatcher"),
                "replyRandomDelays");
    ...
    [Fact]
    public async Task Flow_with_ask_a_failure_mid_stream_must_skip_element_with_resume_strategy()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
            ...
            elements.Result.Should().BeEquivalentTo("a", "b", "d", "e", "f");
            return Task.CompletedTask;
        }, _materializer);
    }

    [Fact]
    public async Task Flow_with_ask_must_resume_after_ask_failure()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
            var c = this.CreateManualSubscriberProbe<Reply>();
            ...
            var sub = c.ExpectSubscription();
            sub.Request(10);
            foreach (var i in new[] { 1, 2, 4, 5 }) c.ExpectNext(new Reply(i));
    ...
    [Fact]
    public async Task Flow_with_ask_must_resume_after_multiple_failures()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
    ...
    [Fact]
    public async Task Flow_with_ask_should_handle_cancel_properly()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
            var dontReply = Sys.ActorOf(BlackHoleActor.Props.WithDispatcher("akka.test.stream-dispatcher"),
    ...
    {
        return Sys.ActorOf(Props.Create(() => new FailOn(n)).WithDispatcher("akka.test.stream-dispatcher"),
            "failureReplier-" + n);
    }

    private IActorRef ReplierFailAllExceptOn(int n)
    {
        return Sys.ActorOf(Props.Create(() => new FailOnAllExcept(n)).WithDispatcher("akka.test.stream-dispatcher"),
            "failureReplier-" + n);
    }

    #region internal classes

    private sealed class Reply : IEquatable<Reply>
    {
        ...
        {
            return obj is Reply reply && Equals(reply);
        }
    ...
    private sealed class Replier : UntypedActor
    {
        protected override void OnReceive(object message)
        {
            Sender.Tell(new Reply((int)message));
        }
    }

    private sealed class ReplyAndProxy : ReceiveActor
    {
    ...
    private sealed class RandomDelaysReplier : ReceiveActor
    {
    ...
    private sealed class StatusReplier : UntypedActor
    {
        protected override void OnReceive(object message)
        {
    ...
    private sealed class FailOn : ReceiveActor
    {
        public FailOn(int n)
        {
    ...
    private sealed class FailOnAllExcept : ReceiveActor
    {
