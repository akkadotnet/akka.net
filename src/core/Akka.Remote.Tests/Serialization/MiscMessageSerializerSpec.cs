...
public class MiscMessageSerializerSpec : AkkaSpec
{
    ...
    {
        var remoteSystem =
            ActorSystem.Create("remote", ConfigurationFactory.ParseString("akka.actor.provider = remote"));
        ...
        var serializer = remoteSystem.Serialization.FindSerializerFor(actorRef)
    ...
    {
        var message = new RoundRobinPool(25);
        AssertEqual(message);
    }
    ...
    public void Can_serialize_RoundRobinPoolWithCustomResizer()
    {
        ...
        var message = new RoundRobinPool(25, defaultResizer);
    ...
    {
        var message = new BroadcastPool(25);
        AssertEqual(message);
    }
    ...
    {
        var message = new ScatterGatherFirstCompletedPool(25, 3.Seconds());
        AssertEqual(message);
    }
    ...
    {
        var message = new TailChoppingPool(25, 3.Seconds(), 3.Seconds());
        AssertEqual(message);
    }
    ...
    {
        var message = new ConsistentHashingPool(25);
        AssertEqual(message);
    }
    ...
    public void Can_serialize_RemoteRouterConfig()
    {
        var message = new RemoteRouterConfig(
            new RandomPool(25),
            new List<Address> { new("akka.tcp", "TestSys", "localhost", 23423) });
        AssertEqual(message);
    }
    ...
    public void Can_serialize_RemoteWatcher_HearthbeatRsp()
    {
        ...
        AssertAndReturn(heartbeatRsp).AddressUid.Should()
            .Be(heartbeatRsp.AddressUid); //TODO: add Equals to RemoteWatcher.HeartbeatRsp
    }
