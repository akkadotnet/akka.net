...
public class ActorGraphInterpreterSpec : AkkaSpec
{
    ...
    public async Task ActorGraphInterpreter_should_be_able_to_interpret_and_reuse_a_simple_bidi_stage()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            var identity = identityBidiFlow.Atop(identityBidiFlow).Atop(identityBidiFlow)
                .Join(Flow.Identity<int>().Select(x => x));
    ...
    [Fact]
    public void
        ActorGraphInterpreter_should_be_able_to_report_errors_if_an_error_happens_for_an_already_completed_stage()
    {
    ...
    [Fact]
    public async Task
        ActorGraphInterpreter_should_be_able_to_properly_handle_case_where_a_stage_fails_before_subscription_happens()
    {
    ...
    [Fact]
    public async Task
        ActorGraphInterpreter_should_trigger_PostStop_in_all_stages_when_abruptly_terminated_and_no_upstream_boundaries()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
        ...
        private class Logic : GraphStageLogic
        {
            public Logic(BidiShape<int, int, int, int> shape) : base(shape)
            {
                SetHandler(shape.Inlet1,
                    () => Push(shape.Outlet1, Grab(shape.Inlet1)),
                    () => Complete(shape.Outlet1));

                SetHandler(shape.Inlet2,
                    () => Push(shape.Outlet2, Grab(shape.Inlet2)),
                    () => Complete(shape.Outlet2));

                SetHandler(shape.Outlet1,
                    () => Pull(shape.Inlet1),
                    cause => Cancel(shape.Inlet1, cause));

                SetHandler(shape.Outlet2,
                    () => Pull(shape.Inlet2),
                    cause => Cancel(shape.Inlet2, cause));
            }
        }
    }
    ...
    public class RotatedIdentityBidiGraphStage : GraphStage<BidiShape<int, int, int, int>>
    {
        ...
        private class Logic : GraphStageLogic
        {
            ...
            {
                SetHandler(shape.Inlet1,
                    () => Push(shape.Outlet2, Grab(shape.Inlet1)),
                    () => Complete(shape.Outlet2));

                SetHandler(shape.Inlet2,
                    () => Push(shape.Outlet1, Grab(shape.Inlet2)),
                    () => Complete(shape.Outlet1));
