...
public class ExpectTests : AkkaSpec
{
    ...
    public async Task ExpectMsgAllOfAsync_should_fail_when_receiving_unexpected()
    {
        ...
        TestActor.Tell("3");
        await Awaiting(async () => { await ExpectMsgAllOfAsync(new[] { "3", "1", "2" }).ToListAsync(); }).Should()
            .ThrowAsync<XunitException>().WithMessage("not found [*");
    }
    ...
    public void ExpectMsgAllOfMatchingPredicatesAsync_should_receive_correct_messages()
    {
        ...
        TestActor.Tell("3");
        TestActor.Tell("4");
        ExpectMsgAllOfMatchingPredicates(PredicateInfo.Create<string>(s => s.Equals("3")),
            PredicateInfo.Create<string>(s => s.Equals("1")), PredicateInfo.Create<string>(s => s.Equals("4")),
    ...
    public void ExpectMsgAllOfMatchingPredicatesAsync_with_mixed_message_types_should_receive_correct_messages()
    {
        ...
        TestActor.Tell(3);
        TestActor.Tell("4");
        ExpectMsgAllOfMatchingPredicates(PredicateInfo.Create<int>(s => s.Equals(3)),
            PredicateInfo.Create<int>(s => s.Equals(1)), PredicateInfo.Create<string>(s => s.Equals("4")),
            PredicateInfo.Create<string>(s => s.Equals("2")));
    }

    [Fact]
    public async Task
        ExpectMsgAllOfMatchingPredicatesAsync_with_mixed_message_types_should_receive_correct_messages_in_order()
    {
        TestActor.Tell(1);
        TestActor.Tell("2");
        TestActor.Tell(3);
    ...
    public async Task ExpectMsgAllOfMatchingPredicatesAsync_should_timeout_when_not_receiving_any_messages()
    {
        await Awaiting(async () =>
        {
            await ExpectMsgAllOfMatchingPredicatesAsync(TimeSpan.FromMilliseconds(100),
                new[]
                {
