...
internal class WriteAggregator : ReadWriteAggregator
{
    ...
    public WriteAggregator(IKey key, DataEnvelope envelope, Delta delta, IWriteConsistency consistency, object req,
    ...
    protected bool IsDone => _gotLocalStoreReply && (Remaining.Count <= DoneWhenRemainingSize ||
                                                     Remaining.Except(_gotNackFrom).Count == 0 || NotEnoughNodes);
    ...
    public static Props Props(IKey key, DataEnvelope envelope, Delta delta, IWriteConsistency consistency, object req,
        IImmutableList<Address> nodes, IImmutableSet<Address> unreachable, bool shuffle, IActorRef replyTo,
        bool durable)
    {
        return Actor.Props
            .Create(() => new WriteAggregator(key, envelope, delta, consistency, req, nodes, unreachable, shuffle,
                replyTo, durable)).WithDeploy(Deploy.Local);
    }
...
public sealed class WriteTo : IWriteConsistency, IEquatable<WriteTo>
{
    ...
    public override bool Equals(object obj)
    {
        return Equals(obj as WriteTo);
    }

    public override string ToString()
    {
        return $"WriteTo({Count}, timeout={Timeout})";
    }
...
public sealed class WriteMajority : IWriteConsistency, IEquatable<WriteMajority>
{
    ...
    public override bool Equals(object obj)
    {
        return Equals(obj as WriteMajority);
    }

    public override string ToString()
    {
        return $"WriteMajority(timeout={Timeout})";
    }
...
/// <summary>
///     <see cref="WriteMajority" /> but with the given number of <see cref="Additional" /> nodes added to the majority
///     count. At most
...
public sealed class WriteMajorityPlus : IWriteConsistency, IEquatable<WriteMajorityPlus>
{
    ...
    public override bool Equals(object obj)
    {
        return Equals(obj as WriteMajorityPlus);
    }

    public override string ToString()
    {
        return $"WriteMajorityPlus(timeout={Timeout}, additional={Additional})";
    }
...
public sealed class WriteAll : IWriteConsistency, IEquatable<WriteAll>
{
    ...
    public override bool Equals(object obj)
    {
        return Equals(obj as WriteAll);
    }

    public override string ToString()
    {
        return $"WriteAll(timeout={Timeout})";
    }
