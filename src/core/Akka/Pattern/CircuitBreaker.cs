...
public class CircuitBreaker
{
    ...
    /// <returns>TBD</returns>
    public CircuitBreaker(IScheduler scheduler, int maxFailures, TimeSpan callTimeout, TimeSpan resetTimeout,
        TimeSpan maxResetTimeout, double exponentialBackoffFactor)
    ...
    ///     factor is added, e.g. `0.2` adds up to `20%` delay. randomFactor should be in range `0.0` (inclusive) and `1.0`
    ///     (inclusive). In order to skip this additional delay pass in `0`.
    /// </param>
    /// <returns>TBD</returns>
    public CircuitBreaker(IScheduler scheduler, int maxFailures, TimeSpan callTimeout, TimeSpan resetTimeout,
        TimeSpan maxResetTimeout, double exponentialBackoffFactor, double randomFactor)
    {
        if (exponentialBackoffFactor < 1.0)
            throw new ArgumentException("factor must be >= 1.0", nameof(exponentialBackoffFactor));
        if (randomFactor is < 0.0 or > 1.0)
            throw new ArgumentException("randomFactor must be between 0.0 and 1.0", nameof(randomFactor));
    ...
    {
        return Interlocked.CompareExchange(ref _currentResetTimeout, newResetTimeout.Ticks, oldResetTimeout.Ticks) ==
               oldResetTimeout.Ticks;
    }
    ...
    /// <param name="resetTimeout"><see cref="TimeSpan" /> of time after which to attempt to close the circuit</param>
    /// <returns>TBD</returns>
    public static CircuitBreaker Create(IScheduler scheduler, int maxFailures, TimeSpan callTimeout,
    ...
    {
        return WithCircuitBreaker(body, b => Task.Run(b)).Result;
    }
    ...
    public void Fail()
    {
        _currentState.CallFails(new UserCalledFailException());
    }

    internal void OnFail(Exception cause)
    {
        LastCaughtException = cause;
    }
    ...
    /// </summary>
    /// <param name="randomFactor">
    ///     after calculation of the back-off an additional random delay based on this factor is added,
    ...
    {
        return new CircuitBreaker(Scheduler, MaxFailures, CallTimeout, ResetTimeout, MaxResetTimeout,
            ExponentialBackoffFactor,
            randomFactor);
    }

    /// <summary>
    ///     Implements consistent transition between states. Throws IllegalStateException if an invalid transition is
    ///     attempted.
    /// </summary>
    /// <param name="fromState">State being transitioning from</param>
    /// <param name="toState">State being transitioned to</param>
    /// <exception cref="IllegalStateException">
    ///     This exception is thrown if an invalid transition is attempted from <paramref name="fromState" /> to
