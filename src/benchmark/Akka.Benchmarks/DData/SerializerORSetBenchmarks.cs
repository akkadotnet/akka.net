...
public class SerializerORSetBenchmarks
{
    ...
    public void SetupSystem()
    {
        ...
        foreach (var i in Enumerable.Range(0, NumElements)) newElements.Add(new RDDBenchTypes.TestVal(i.ToString()));
        _elements = newElements.ToArray();

        _c1 = ORSet<List<RDDBenchTypes.TestVal>>.Empty;
        foreach (var node in _nodes) _c1 = _c1.Add(node, _elements.ToList());
