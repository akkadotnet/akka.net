...
public class ActorLookupSpec : AkkaSpec
{
    ...
    {
        private IActorRefProvider Provider => Context.System.AsInstanceOf<ActorSystemImpl>().Provider;

        public Node()
        {
            Receive<Create>(create => Sender.Tell(Context.ActorOf(P, create.Child)));
            Receive<LookupElems>(elems =>
                Sender.Tell(Provider.ResolveActorRef(new RootActorPath(Provider.DefaultAddress) / elems.Path)));
    ...
    private readonly IActorRef c1;
    private readonly IActorRef c2;
    private readonly IActorRef c21;
    ...
    public async Task ActorSystem_must_find_temporary_actors()
    {
        ...
        Provider.ResolveActorRef(a.Path + "/hello").AsInstanceOf<IInternalActorRef>().IsTerminated.Should().Be(true);
