...
public class ReceiveOnlyBenchmark
{
    ...
    public void Setup(BenchmarkContext context)
    {
        ...
        for (var i = 0; i < ExpectedMessages - 1; i++)
            TestActor.AsInstanceOf<RepointableActorRef>().Underlying.AsInstanceOf<ActorCell>().Mailbox.MessageQueue
                .Enqueue(TestActor, new Envelope("hit", ActorRefs.Nobody)); // queue all of the messages into the actor
    }

    [PerfBenchmark(NumberOfIterations = 10, TestMode = TestMode.Measurement, RunMode = RunMode.Iterations,
        RunTimeMilliseconds = 1000, Skip = "Causes StackoverflowExceptions when coupled with CallingThreadDispatcher")]
