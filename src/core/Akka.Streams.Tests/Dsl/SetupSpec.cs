// -----------------------------------------------------------------------
//  <copyright file="SetupSpec.cs" company="Akka.NET Project">
...
public class SetupSpec : AkkaSpec
{
    ...
    {
        var source = Source.Setup((_, _) => Source.Setup((_, attr) => Source.Single(attr.GetNameLifted)))
            .Named("my-name");
    ...
    public void SourceSetup_should_handle_factory_failure()
    {
        ...
        Assert.Throws<AggregateException>(() => materialized.Result).InnerException?.Should()
            .BeOfType<ApplicationException>();
        Assert.Throws<AggregateException>(() => completion.Result).InnerException?.Should()
            .BeOfType<ApplicationException>();
    }
    ...
    public void FlowSetup_should_expose_attributes()
    {
        ...
        Source.Empty<object>().Via(flow).RunWith(Sink.First<IEnumerable<Attributes.IAttribute>>(), Materializer).Result
            .Should().NotBeEmpty();
    }
    ...
    public void FlowSetup_should_propagate_attributes()
    {
        ...
        Source.Empty<object>().Via(flow).RunWith(Sink.First<Func<string>>(), Materializer).Result.Invoke()
            .ShouldBe("setup-my-name");
    }
