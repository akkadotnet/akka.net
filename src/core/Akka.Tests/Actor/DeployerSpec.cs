...
public class DeployerSpec : AkkaSpec
{
    ...
    private void AssertRouting(string service, RouterConfig expected, string expectPath)
    {
        ...
        if (expected is Pool pool) Assert.Equal(pool.Resizer, ((Pool)deployment.RouterConfig).Resizer);
    }

    private class RecipeActor : UntypedActor
    {
    ...
    [Fact(DisplayName =
        @"If a fallaback config is declared with a deployment an actor should be able to be created for the main configuration and the fallback configuration")]
