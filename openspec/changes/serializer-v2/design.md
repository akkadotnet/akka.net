...
`Akka.Serialization.PooledPayloadWriter` is a public, sealed `IBufferWriter<byte>` on the core V2 surface: a growable buffer rented from `ArrayPool<byte>.Shared` (double-and-copy growth) that adds three capabilities the bare `IBufferWriter<byte>` interface cannot express:
