...
public class ActorPathSpec
{
    ...
    public void ActorPath_Parse_HandlesCasing_ForLocal()
    {
        ...
        Assert.True(actorPath.Address.Protocol.Equals("akka", StringComparison.Ordinal),
...
"have correct path elements" in {
  (RootActorPath(Address("akka.tcp", "mysys")) / "user" / "foo" / "bar").elements.toSeq should be(Seq("user", "foo", "bar"))
    ...
    {
        (new RootActorPath(new Address("akka.tcp", "mysys")) / "user" / "foo" / "bar").Elements
            .ShouldOnlyContainInOrder("user", "foo", "bar");
    }
    ...
    public void ValidateElementPartsComprehensive()
    {
        ...
        foreach (var c in valid) ActorPath.IsValidPathElement(new string(new[] { c, '$' })).Should().BeTrue();
    }
    ...
    {
        var urlEncode = WebUtility.UrlEncode(element);
