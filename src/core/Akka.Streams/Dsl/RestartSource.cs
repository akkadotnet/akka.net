...
///     They are useful for graphs that need to run for longer than the <see cref="Source" /> can necessarily guarantee it
///     will, for
...
{
    /// <summary>
    ///     Wrap the given <see cref="Source" /> with a <see cref="Source" /> that will restart it when it fails or complete
    ///     using an exponential
    ///     backoff.
    ///     This <see cref="Source" /> will never emit a complete or failure, since the completion or failure of the wrapped
    ///     <see cref="Source" />
    ///     is always handled by restarting it. The wrapped <see cref="Source" /> can however be cancelled by cancelling this
    ///     <see cref="Source" />.
    ///     When that happens, the wrapped <see cref="Source" />, if currently running will be cancelled, and it will not be
    ///     restarted.
    ///     This can be triggered simply by the downstream cancelling, or externally by introducing a
    ///     <see cref="IKillSwitch" /> right
    ...
    public static Source<T, NotUsed> WithBackoff<T, TMat>(Func<Source<T, TMat>> sourceFactory, TimeSpan minBackoff,
        TimeSpan maxBackoff, double randomFactor)
    {
    ...
    [Obsolete("Use the overloaded method which accepts Akka.Stream.RestartSettings instead.")]
    public static Source<T, NotUsed> WithBackoff<T, TMat>(Func<Source<T, TMat>> sourceFactory, TimeSpan minBackoff,
        TimeSpan maxBackoff, double randomFactor, int maxRestarts)
    {
        var settings = RestartSettings.Create(minBackoff, maxBackoff, randomFactor)
    ...
    ///     backoff.
    ///     <para>
    ///         This <see cref="Source" /> will never emit a complete or failure, since the completion or failure of the
    ///         wrapped <see cref="Source" />
    ///         is always handled by restarting it. The wrapped <see cref="Source" /> can however be cancelled by cancelling
    ///         this <see cref="Source" />.
    ///         When that happens, the wrapped <see cref="Source" />, if currently running will be cancelled, and it will not
    ///         be restarted.
    ///         This can be triggered simply by the downstream cancelling, or externally by introducing a
    ///         <see cref="IKillSwitch" /> right
    ///         after this <see cref="Source" /> in the graph.
    ...
    {
        return Source.FromGraph(new RestartWithBackoffSource<T, TMat>(sourceFactory, settings, false));
    }

    /// <summary>
    ///     Wrap the given <see cref="Source" /> with a <see cref="Source" /> that will restart it when it fails using an
    ///     exponential backoff.
    ///     This <see cref="Source" /> will never emit a failure, since the failure of the wrapped <see cref="Source" /> is
    ///     always handled by
    ...
    /// </param>
    [Obsolete("Use the overloaded method which accepts Akka.Stream.RestartSettings instead.")]
    public static Source<T, NotUsed> OnFailuresWithBackoff<T, TMat>(Func<Source<T, TMat>> sourceFactory,
    ...
    {
        return Source.FromGraph(new RestartWithBackoffSource<T, TMat>(sourceFactory, settings, true));
    }
}
...
internal sealed class RestartWithBackoffSource<T, TMat> : GraphStage<SourceShape<T>>
{
    ...
    {
        return new Logic(this, inheritedAttributes, "Source");
    }
