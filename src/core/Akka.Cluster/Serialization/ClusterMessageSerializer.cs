...
public class ClusterMessageSerializer : SerializerWithStringManifest
{
    ...
    internal const string HeartBeatRspManifestPre1419 =
        "Akka.Cluster.ClusterHeartbeatSender+HeartbeatRsp, Akka.Cluster";

    internal const string HeartBeatManifest = "HB";
    internal const string HeartBeatRspManifest = "HBR";

    internal const string ExitingConfirmedManifest =
        "Akka.Cluster.InternalClusterAction+ExitingConfirmed, Akka.Cluster";
        ...
        switch (obj)
        {
            ...
            default:
                throw new ArgumentException(
                    $"Can't serialize object of type [{obj.GetType()}] in [{nameof(ClusterMessageSerializer)}]");
        }
    }
    ...
    {
        var join = Join.Parser.ParseFrom(bytes);
    ...
    {
        var welcomeProto = Welcome.Parser.ParseFrom(bytes);
    ...
    private static GossipStatus GossipStatusFrom(byte[] bytes)
    {
        ...
        return new GossipStatus(UniqueAddressFrom(gossipStatusProto.From),
            VectorClockFrom(gossipStatusProto.Version, gossipStatusProto.AllHashes));
    }
    ...
    private ClusterRouterPool ClusterRouterPoolFrom(byte[] bytes)
    {
        ...
        return new ClusterRouterPool(PoolFrom(clusterRouterPool.Pool),
            ClusterRouterPoolSettingsFrom(clusterRouterPool.Settings));
    }

    private Pool PoolToProto(Akka.Routing.Pool pool)
    {
    ...
    private Akka.Routing.Pool PoolFrom(Pool poolProto)
    {
        return (Akka.Routing.Pool)system.Serialization.Deserialize(poolProto.Data.ToByteArray(),
            (int)poolProto.SerializerId, poolProto.Manifest);
    }
    ...
    private static ClusterRouterPoolSettings ClusterRouterPoolSettingsFrom(
        Proto.Msg.ClusterRouterPoolSettings clusterRouterPoolSettingsProto)
    {
    ...
    private static Proto.Msg.Gossip GossipToProto(Gossip gossip)
    {
        ...
        int MapAppVersion(AppVersion appVersion)
        {
            return MapWithErrorMessage(appVersionMapping, appVersion.Version, "appVersion");
        }

        int MapUniqueAddress(UniqueAddress address)
        {
            return MapWithErrorMessage(addressMapping, address, "address");
        }
    ...
    private static Gossip GossipFrom(Proto.Msg.Gossip gossip)
    {
        ...
        var members = gossip.Members.Select((Func<Proto.Msg.Member, Member>)MemberFromProto)
            .ToImmutableSortedSet(Member.Ordering);
    ...
    private static IEnumerable<ObserverReachability> ReachabilityToProto(Reachability reachability,
        Dictionary<UniqueAddress, int> addressMapping)
    {
        var builderList = new List<ObserverReachability>();
        foreach (var version in reachability.Versions)
        {
            var subjectReachability = reachability.RecordsFrom(version.Key).Select(
                r =>
                {
                    var sr = new SubjectReachability();
                    sr.AddressIndex = MapWithErrorMessage(addressMapping, r.Subject, "address");
                    sr.Status = (SubjectReachability.Types.ReachabilityStatus)r.Status;
    ...
    private static Reachability ReachabilityFromProto(IEnumerable<ObserverReachability> reachabilityProto,
        List<UniqueAddress> addressMapping)
    {
    ...
    private static Proto.Msg.VectorClock VectorClockToProto(VectorClock vectorClock,
        Dictionary<string, int> hashMapping)
    {
    ...
    private static HeartBeatResponse HeartbeatRspToProto(ClusterHeartbeatSender.HeartbeatRsp heartbeatRsp)
    {
    ...
    private static ClusterHeartbeatSender.HeartbeatRsp DeserializeHeartbeatRsp(byte[] bytes)
    {
        ...
        return new ClusterHeartbeatSender.HeartbeatRsp(UniqueAddressFrom(hbsp.From), hbsp.SequenceNr,
            hbsp.CreationTime);
    }
    ...
    private static Heartbeat HeartbeatToProto(ClusterHeartbeatSender.Heartbeat heartbeat)
    {
    ...
    {
        if (serializer is SerializerWithStringManifest manifestSerializer) return manifestSerializer.Manifest(obj);
