...
internal class Program
{
    ...
    private static void Main(params string[] args)
    {
        ...
        var testAsync = args.Contains("--async");
    ...
    private static async void Start(uint timesToRun, bool testAsync)
    {
        ...
        if (testAsync) Console.Write("                       AsyncActor");
        Console.WriteLine();

        Console.Write("Throughput, Msgs/sec, Start [ms], Total [ms],  Msgs/sec, Start [ms], Total [ms]");
        if (testAsync) Console.Write(",  Msgs/sec, Start [ms], Total [ms]");
        Console.WriteLine();
        ...
        for (var i = 0; i < timesToRun; i++)
        {
            ...
            {
                var result1 = await Benchmark<ClientActorBase>(throughput, processorCount, repeat,
                    PrintStats.LineStart | PrintStats.Stats, bestThroughputActorBase, redCountActorBase);
                bestThroughputActorBase = result1.Item2;
                redCountActorBase = result1.Item3;
                Console.Write(",  ");
                var result2 = await Benchmark<ClientReceiveActor>(throughput, processorCount, repeat, PrintStats.Stats,
                    bestThroughputReceiveActor, redCountReceiveActor);
                ...
                {
                    Console.Write(",  ");
                    var result3 = await Benchmark<ClientAsyncActor>(throughput, processorCount, repeat,
                        PrintStats.Stats, bestThroughputAsyncActor, redCountAsyncActor);
    ...
    public static IEnumerable<int> GetThroughputSettings()
    {
        ...
        yield return 10;
        yield return 15;
        for (var i = 20; i < 100; i += 10) yield return i;
        for (var i = 100; i < 1000; i += 100) yield return i;
    }

    private static async Task<(bool, long, int)> Benchmark<TActor>(int factor, int numberOfClients,
        long numberOfRepeats, PrintStats printStats, long bestThroughput, int redCount) where TActor : ActorBase
    {
        ...
        for (var i = 0; i < numberOfClients; i++)
        {
            ...
            tasks.Add(ts.Task);
            var client =
                (RepointableActorRef)system.ActorOf(new Props(typeof(TActor), null, destination, repeatsPerClient, ts),
                    "client-" + i);
        ...
        var throughput = elapsedMilliseconds == 0 ? -1 : totalMessagesReceived / elapsedMilliseconds * 1000;
        ...
        else
        {
            ...
            if (printStats.HasFlag(PrintStats.Stats))
                Console.Write("{0,8}, {1,10}, {2,10}", throughput,
                    setupTime.TotalMilliseconds.ToString("F2", CultureInfo.InvariantCulture),
