    ...
    public void Durable_CRDT_should_work_in_a_multi_node_cluster()
    {
        ...
        Within(TimeSpan.FromSeconds(10), () =>
        {
            AwaitAssert(() =>
            {
                ...
                ExpectMsg<GetSuccess>().Get(keyC).Elements
                    .ShouldBe(ImmutableHashSet.CreateRange(new[] { first.Name, second.Name }));
            });
        });
    ...
    public void Durable_CRDT_should_be_durable_after_gossip_update()
    {
        ...
        RunOn(() =>
        {
            Within(TimeSpan.FromSeconds(10), () =>
            {
                AwaitAssert(() =>
                {
                    r.Tell(Dsl.Update(keyC, ORSet<string>.Empty, WriteLocal.Instance,
                        c => c.Add(cluster, Myself.Name)));
                    ExpectMsg(new UpdateSuccess(keyC, null));
                });
            });
        }, first);
                ...
                /* new TestKit(sys1) with ImplicitSender */
                {
                    ...
                    AwaitAssert(() =>
                    {
                        ...
                        ExpectMsg<GetKeysIdsResult>().Keys
                            .ShouldBe(ImmutableHashSet.CreateRange(new[] { keyA.Id, keyB.Id }));
                    });
    ...
    public void Durable_CRDT_should_stop_Replicator_if_Load_fails()
    {
        RunOn(() =>
        {
            var r = Sys.ActorOf(Replicator.Props(
                    ReplicatorSettings.Create(Sys).WithDurableStoreProps(TestDurableStoreProps(true))),
...
{
    public DurableDataSpec() : base(new DurableDataSpecConfig(false), typeof(DurableDataSpec))
    {
    }
}
...
{
    public DurableDataWriteBehindSpec() : base(new DurableDataSpecConfig(true), typeof(DurableDataWriteBehindSpec))
    {
    }
}