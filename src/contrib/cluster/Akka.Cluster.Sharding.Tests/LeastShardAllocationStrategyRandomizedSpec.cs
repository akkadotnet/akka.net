...
public class LeastShardAllocationStrategyRandomizedSpec : AkkaSpec
{
    ...
    private readonly int iterationsPerTest = 10;
    private readonly Random rnd;
    private readonly int rndSeed;
    ...
    private IImmutableDictionary<IActorRef, IImmutableList<string>> CreateAllocations(
        IImmutableDictionary<IActorRef, int> countPerRegion)
    {
        return countPerRegion.ToImmutableDictionary(i => i.Key,
            i => (IImmutableList<string>)Enumerable.Range(1, i.Value).Select(n => n.ToString("000"))
                .Select(n => $"{i.Key.Path.Name}-{n}").ToImmutableList());
    }
    ...
    private IShardAllocationStrategy StrategyWithFakeCluster(int absoluteLimit = 100000, double relativeLimit = 1.0)
    {
        ...
        return new LeastShardAllocationStrategySpec.TestLeastShardAllocationStrategy(absoluteLimit, relativeLimit,
            () => new CurrentClusterState().Copy(clusterMembers), () => clusterMembers.FirstOrDefault());
    }
        ...
        int expectedMaxSteps)
    {
        foreach (var i in Enumerable.Range(1, iterationsPerTest))
        {
            ...
            var memberArray = Enumerable.Range(1, numberOfRegions)
                .Select(n => LeastShardAllocationStrategySpec.NewUpMember("127.0.0.1", n)).ToArray();
            clusterMembers = ImmutableSortedSet.Create(memberArray); //.toIndexedSeq: _ *);
            var regions = Enumerable.Range(1, numberOfRegions).Select(n =>
                LeastShardAllocationStrategySpec.NewFakeRegion($"{iteration}-R{n}", memberArray[n - 1]));
        ...
        int maxSteps)
    {
        ...
        var newAllocations =
            LeastShardAllocationStrategySpec.AfterRebalance(allocationStrategy, allocations, rebalanceResult);

        LeastShardAllocationStrategySpec.CountShards(newAllocations).Should().Be(
            LeastShardAllocationStrategySpec.CountShards(allocations),
            $"test {allocationStrategy}[{string.Join(", ", LeastShardAllocationStrategySpec.CountShardsPerRegion(allocations))}]: ");
        ...
        if (diff <= 1)
        {
            if (round >= 3 && maxSteps <= 10)
                ...
                Sys.Log.Info(
                    $"rebalance solved in round {round}, [{string.Join(" => ", newSteps.Select(step => string.Join(", ", LeastShardAllocationStrategySpec.CountShardsPerRegion(step))))}]");
        }
        else if (round == maxSteps)
        {
            throw new AssertionFailedException(
                $"Couldn't solve rebalance in $round rounds, [{string.Join(" => ", newSteps.Select(step => string.Join(", ", LeastShardAllocationStrategySpec.CountShardsPerRegion(step))))}]");
        }
        else
        {
    ...
    {
        TestRebalance(strategyWithoutLimits, 5, 5, 2);
    }

    [Fact]
    public void LeastShardAllocationStrategy_with_random_scenario_must_rebalance_shards_with_max_5_regions_100_shards()
    {
        TestRebalance(strategyWithoutLimits, 5, 100, 2);
    }

    [Fact]
    public void LeastShardAllocationStrategy_with_random_scenario_must_rebalance_shards_with_max_20_regions_5_shards()
    {
        TestRebalance(strategyWithoutLimits, 20, 5, 2);
    }

    [Fact]
    public void LeastShardAllocationStrategy_with_random_scenario_must_rebalance_shards_with_max_20_regions__20_shards()
    {
        TestRebalance(strategyWithoutLimits, 20, 20, 2);
    }

    [Fact]
    public void LeastShardAllocationStrategy_with_random_scenario_must_rebalance_shards_with_max_20_regions_200_shards()
    {
        TestRebalance(strategyWithoutLimits, 20, 200, 5);
    }

    [Fact]
    public void
        LeastShardAllocationStrategy_with_random_scenario_must_rebalance_shards_with_max_100_regions_100_shards()
    {
        TestRebalance(strategyWithoutLimits, 100, 100, 5);
    }

    [Fact]
    public void
        LeastShardAllocationStrategy_with_random_scenario_must_rebalance_shards_with_max_100_regions_1000_shards()
    {
        TestRebalance(strategyWithoutLimits, 100, 1000, 5);
    }

    [Fact]
    public void
        LeastShardAllocationStrategy_with_random_scenario_must_rebalance_shards_with_max_20_regions_20_shards_and_limits()
    {
        ...
        TestRebalance(strategy, 20, 20, 20);
    }
}