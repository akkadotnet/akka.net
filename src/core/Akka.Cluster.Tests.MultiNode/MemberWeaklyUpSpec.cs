        ...
        Within(TimeSpan.FromSeconds(20), () =>
        {
            RunOn(() =>
            {
                ...
                foreach (var role2 in _side2)
                    TestConductor.Blackhole(role1, role2, ThrottleTransportAdapter.Direction.Both)
            ...
            RunOn(() =>
            {
                AwaitAssert(() =>
                    ClusterView.UnreachableMembers
                        ...
                        .ShouldBe(ImmutableHashSet.CreateRange(new[]
                        {
                            GetAddress(_config.Third), GetAddress(_config.Fourth)
                        })));
            }, _config.First);
    ...
    public void A_cluster_of_3_members_should_accept_joining_on_each_side_and_set_status_to_WeaklyUp()
    {
        Within(TimeSpan.FromSeconds(20), () =>
        {
            ...
            RunOn(() => AwaitAssert(() =>
            {
                ...
                ClusterView.Members
                    .Any(m => m.Address == GetAddress(_config.Second) && m.Status == MemberStatus.WeaklyUp)
                    .ShouldBe(true);
            }), _side1.ToArray());
            ...
            RunOn(() => AwaitAssert(() =>
            {
                ClusterView.Members.Count.ShouldBe(4);
                ClusterView.Members
                    .Any(m => m.Address == GetAddress(_config.Fifth) && m.Status == MemberStatus.WeaklyUp)
                    .ShouldBe(true);
            }), _side2.ToArray());
    ...
    public void A_cluster_of_3_members_should_change_status_to_Up_after_healed_network_partition()
    {
        Within(TimeSpan.FromSeconds(20), () =>
        {
            RunOn(() =>
            {
                foreach (var role1 in _side1)
                foreach (var role2 in _side2)
                    TestConductor.PassThrough(role1, role2, ThrottleTransportAdapter.Direction.Both)
                        .Wait(TimeSpan.FromSeconds(3));
            }, _config.First);
