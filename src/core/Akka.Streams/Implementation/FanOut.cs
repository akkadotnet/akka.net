...
public class OutputBunch<T>
{
    ...
    {
        Enqueue(IdToEnqueueAndYield(), element);
    }
    ...
    {
        return new LambdaTransferState(() => _pending[id],
            () => _cancelled[id] || _completed[id] || _errored[id]);
    }
    ...
    {
        return new LambdaTransferState(() => _pending[id] || _cancelled[id], () => false);
    }
    ...
    private sealed class FanoutOutputs : SimpleOutputs
    {
        ...
        public new ISubscription CreateSubscription()
        {
            return new FanOut.SubstreamSubscription(Actor, _id);
        }
    }
...
public static class FanOut
{
    ...
    public class SubstreamSubscription : ISubscription
    {
        ...
        {
            _parent.Tell(new SubstreamRequestMore(_id, elements));
        }
        ...
        public void Cancel()
        {
            _parent.Tell(new SubstreamCancel(_id));
        }
...
internal static class Unzip
{
    ...
    {
        return Actor.Props.Create<Unzip<T>>(settings, 2).WithDeploy(Deploy.Local);
    }
}
...
internal sealed class Unzip<T> : FanOut<T>
{
    ...
    /// <see cref="Akka.Streams.Implementation.FanOut.Unzip{T}" />
    /// as well!
