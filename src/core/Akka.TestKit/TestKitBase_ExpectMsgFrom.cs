...
public abstract partial class TestKitBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        return InternalExpectMsgAsync<T>(
            RemainingOrDilated(duration),
            null,
            s => _assertions.AssertEqual(
                sender,
                s,
                FormatWrongSenderMessage(s, sender.ToString(), hint)),
            null,
            cancellationToken);
    }
        ...
        object message)
    {
        _assertions.AssertTrue(
            isSender(sender),
            FormatWrongSenderMessage(sender, hint ?? "the predicate to return true", null) +
        ...
        CancellationToken cancellationToken = default)
    {
        return InternalExpectMsgAsync(
            RemainingOrDilated(timeout),
            assertMessage,
            s => _assertions.AssertEqual(sender, s, hint),
            hint,
            cancellationToken);
    }
