...
public sealed class PersistenceMessageSerializer : Serializer
{
    ...
    private PersistentPayload GetPersistentPayload(object obj)
    {
        PersistentPayload BuildPayload()
        {
            ...
            if (!string.IsNullOrEmpty(manifest)) payload.PayloadManifest = ByteString.CopyFromUtf8(manifest);
    ...
    {
        var message = new Proto.Msg.AtLeastOnceDeliverySnapshot { CurrentDeliveryId = snapshot.CurrentDeliveryId };
    ...
    {
        var message = new PersistentStateChangeEvent { StateIdentifier = changeEvent.StateIdentifier };
        if (changeEvent.Timeout.HasValue) message.TimeoutMillis = (long)changeEvent.Timeout.Value.TotalMilliseconds;
        return message;
    }
    ...
    private PersistentFSMSnapshot GetPersistentFSMSnapshot(object obj)
    {
        ...
        var timeout = (TimeSpan?)type.GetProperty("Timeout")?.GetValue(obj);
        if (timeout.HasValue) message.TimeoutMillis = (long)timeout.Value.TotalMilliseconds;
        return message;
    }
    ...
    public override object FromBinary(byte[] bytes, Type type)
    {
        ...
        if (type == typeof(IPersistentRepresentation))
    ...
    private IPersistentRepresentation GetPersistentRepresentation(PersistentMessage message)
    {
        ...
        if (message.Sender != null) sender = system.Provider.ResolveActorRef(message.Sender);
    ...
    private AtomicWrite GetAtomicWrite(byte[] bytes)
    {
        ...
        foreach (var payload in message.Payload) payloads.Add(GetPersistentRepresentation(payload));
    ...
    private AtLeastOnceDeliverySnapshot GetAtLeastOnceDeliverySnapshot(byte[] bytes)
    {
        ...
        foreach (var unconfirmed in message.UnconfirmedDeliveries)
        {
            ...
            unconfirmedDeliveries.Add(new UnconfirmedDelivery(unconfirmed.DeliveryId, actorPath,
                GetPayload(unconfirmed.Payload)));
        }
    ...
    private PersistentFSM.StateChangeEvent GetStateChangeEvent(byte[] bytes)
    {
        ...
        if (message.TimeoutMillis > 0) timeout = TimeSpan.FromMilliseconds(message.TimeoutMillis);
