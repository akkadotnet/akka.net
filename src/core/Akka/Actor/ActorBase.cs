...
public abstract class Status
{
    ...
    public sealed class Success : Status
    {
        ...
        {
            return Status is null ? "Success" : $"Success: {Status}";
        }
    }
    ...
    public sealed class Failure : Status
    {
        ...
        public override string ToString()
        {
            return State is null ? $"Failure: {Cause}" : $"Failure[{State}]: {Cause}";
        }
    }
}
...
public interface IInternalActor
{
    ...
    ///     This exception is thrown if there is no active Context. The most likely cause is due to use of async operations
    ///     from within this actor.
    /// </exception>
...
public abstract partial class ActorBase : IInternalActor
{
    ...
    ///     This exception is thrown when an actor is created using <c>new</c>. Always create actors using an
    ///     ActorContext/System.
    /// </exception>
    protected ActorBase()
    {
        ...
        if (this is IWithTimers withTimers)
            withTimers.Timers = new TimerScheduler(Context);
    ...
    /// </summary>
    protected static Receive EmptyReceive => _ => false;
    ...
    /// <value>The context.</value>
    /// <exception cref="NotSupportedException">
    ///     This exception is thrown if there is no active ActorContext. The most likely cause is due to use of async
    ...
    protected internal virtual bool AroundReceive(Receive receive, object message)
    {
        ...
        if (!wasHandled) Unhandled(message);
        return wasHandled;
    }
    ...
    {
        if (message is Terminated terminatedMessage) throw new DeathPactException(terminatedMessage.ActorRef);
    ...
    ///     </para>
    ///     <para>
    ///         Please note that the receive timeout might fire and enqueue the <see cref="ReceiveTimeout" /> message right
    ///         after
