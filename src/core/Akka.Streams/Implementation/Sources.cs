...
[InternalApi]
public sealed class
    QueueSource<TOut> : GraphStageWithMaterializedValue<SourceShape<TOut>, ISourceQueueWithComplete<TOut>>
{
    ...
    /// <returns>TBD</returns>
    public override ILogicAndMaterializedValue<ISourceQueueWithComplete<TOut>> CreateLogicAndMaterializedValue(
        Attributes inheritedAttributes)
    {
        var completion = new TaskCompletionSource<object>();
        var logic = new Logic(this, completion);
        return new LogicAndMaterializedValue<ISourceQueueWithComplete<TOut>>(logic,
            new Materialized(t => logic.Invoke(t), completion));
    }
    ...
    private sealed class Logic : GraphStageLogicWithCallbackWrapper<IInput>, IOutHandler
    {
            ...
            if (!_buffer.IsFull)
                ...
                switch (_stage._overflowStrategy)
                {
                    ...
                    case OverflowStrategy.Fail:
                        ...
                        offer.CompletionSource.NonBlockingTrySetResult(
                            new QueueOfferResult.Failure(bufferOverflowException));
                    ...
                    if (offer != null)
                    {
                        ...
                        else
                        {
                            switch (_stage._overflowStrategy)
                            {
                                case OverflowStrategy.DropHead:
                                case OverflowStrategy.DropBuffer:
                                    _pendingOffer.CompletionSource.NonBlockingTrySetResult(QueueOfferResult.Dropped
                                        .Instance);
                                    _pendingOffer = offer;
                                    break;
                                case OverflowStrategy.DropTail:
        ...
        internal void Invoke(IInput offer)
        {
            InvokeCallbacks(offer);
        }
    }
    ...
    public sealed class Materialized : ISourceQueueWithComplete<TOut>
    {
            ...
            var promise =
                TaskEx
                    .NonBlockingTaskCompletionSource<
                        IQueueOfferResult>(); // new TaskCompletionSource<IQueueOfferResult>();
        ...
        {
            _invokeLogic(Completion.Instance);
        }
...
public sealed class UnfoldResourceSourceAsync<TOut, TSource> : GraphStage<SourceShape<TOut>>
{
    ...
    /// <param name="readData">TBD</param>
    /// <param name="close">TBD</param>
    public UnfoldResourceSourceAsync(Func<Task<TSource>> create, Func<TSource, Task<Option<TOut>>> readData,
    ...
    private sealed class Logic : OutGraphStageLogic
    {
        ...
        private Action<Try<TSource>> CreatedCallback => GetAsyncCallback<Try<TSource>>(resource =>
        {
            ...
            {
                FailStage(resource.Failure.Value);
            }
        });
        ...
        private Action<Try<Option<TOut>>> ReadCallback => GetAsyncCallback<Try<Option<TOut>>>(read =>
        {
            ...
            {
                ErrorHandler(read.Failure.Value);
            }
        });
...
public sealed class LazySource<TOut, TMat> : GraphStageWithMaterializedValue<SourceShape<TOut>, Task<TMat>>
{
    ...
    private sealed class Logic : OutGraphStageLogic
    {
        ...
        public Logic(LazySource<TOut, TMat> stage, TaskCompletionSource<TMat> completion,
...
public static class LazySource
{
    ...
    {
        return new LazySource<TOut, TMat>(create);
    }
}
...
public sealed class EmptySource<TOut> : GraphStage<SourceShape<TOut>>
{
    ...
    private sealed class Logic : OutGraphStageLogic
    {
        public Logic(EmptySource<TOut> stage) : base(stage.Shape)
        {
            SetHandler(stage.Out, this);
        }
...
internal sealed class EventWrapper<TDelegate, TEventArgs> : IObservable<TEventArgs>
{
    ...
    ///     Function used to convert given event handler to delegate compatible with underlying .NET
    ///     event.
    /// </param>
    ...
    public EventWrapper(Action<TDelegate> addHandler, Action<TDelegate> removeHandler,
...
internal sealed class ObservableSourceStage<T> : GraphStage<SourceShape<T>>
{
    ...
    private sealed class Logic : GraphStageLogic, IObserver<T>
    {
        ...
        {
            _onCompleted(SubscriptionWithCancelException.StageWasCompleted.Instance);
        }

        public void OnCompleted(Exception cause)
        {
            _onCompleted(cause);
        }
