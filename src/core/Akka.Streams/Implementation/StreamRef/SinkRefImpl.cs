...
internal sealed class SinkRefImpl<T> : SinkRefImpl, ISinkRef<T>
{
    ...
    {
        return SerializationTools.ToSurrogate(this);
    }

    public Sink<T, NotUsed> Sink => Dsl.Sink.FromGraph(new SinkRefStageImpl<T>(InitialPartnerRef))
...
internal sealed class SinkRefStageImpl<TIn> : GraphStageWithMaterializedValue<SinkShape<TIn>, Task<ISourceRef<TIn>>>
{
    ...
    public override ILogicAndMaterializedValue<Task<ISourceRef<TIn>>> CreateLogicAndMaterializedValue(
        Attributes inheritedAttributes)
    {
        var promise = new TaskCompletionSource<ISourceRef<TIn>>();
        return new LogicAndMaterializedValue<Task<ISourceRef<TIn>>>(new Logic(this, promise, inheritedAttributes),
            promise.Task);
    }
    ...
    private sealed class Logic : TimerGraphStageLogic, IInHandler
    {
        ...
        private Status _completedBeforeRemoteConnected;
        ...
        private StreamRefsMaster StreamRefsMaster => _streamRefsMaster ??=
            StreamRefsMaster.Get(ActorMaterializerHelper.Downcast(Materializer).System);

        private StreamRefSettings Settings =>
            _settings ??= ActorMaterializerHelper.Downcast(Materializer).Settings.StreamRefSettings;

        private StreamRefAttributes.SubscriptionTimeout SubscriptionTimeout => _subscriptionTimeout ??=
            _inheritedAttributes.GetAttribute(
                new StreamRefAttributes.SubscriptionTimeout(Settings.SubscriptionTimeout));
        ...
        public void OnUpstreamFailure(Exception cause)
        {
            if (_partnerRef != null)
            {
                ...
                SetKeepGoing(
                    true); // we will terminate once partner ref has Terminated (to avoid racing Terminated with completion message)
            }
            else
            {
        ...
        public override void PreStart()
        {
            ...
            if (initialPartnerRef != null)
            {
                ...
                ObserveAndValidateSender(initialPartnerRef,
                    "Illegal initialPartnerRef! This would be a bug in the SinkRef usage or impl.");
                TryPull();
            }
            else
            {
            ...
            Log.Debug("Created SinkRef, pointing to remote Sink receiver: {0}, local worker: {1}", initialPartnerRef,
                Self);
            ...
            switch (message)
            {
                ...
                case CumulativeDemand demand:
                    ...
                    if (_remoteCumulativeDemandReceived < demand.SeqNr)
                    {
                        ...
                        Log.Debug("Received cumulative demand [{0}], consumable demand: [{1}]", demand.SeqNr,
                            _remoteCumulativeDemandReceived - _remoteCumulativeDemandConsumed);
                    }
        ...
        protected internal override void OnTimer(object timerKey)
        {
            if ((string)timerKey == SubscriptionTimeoutKey)
            {
                ...
                throw
                    ex; // this will also log the exception, unlike failStage; this should fail rarely, but would be good to have it "loud"
            }
        }
            ...
            if (_partnerRef == null)
            {
                ...
                switch (_completedBeforeRemoteConnected)
                {
                    case Status.Failure failure:
                        Log.Warning(
                            "Stream already terminated with exception before remote side materialized, failing now.");
        ...
        private long _remoteCumulativeDemandReceived;
        private long _remoteCumulativeDemandConsumed;
