...
public class ThrottleTransportAdapter : ActorTransportAdapter
{
    ...
    /// </summary>
    protected override string ManagerName => string.Format("throttlermanager.${0}${1}",
        WrappedTransport.SchemeIdentifier, UniqueId.GetAndIncrement());
    ...
    public override Task<bool> ManagementCommand(object message)
    {
        if (message is SetThrottle)
            return manager.Ask(message, AskTimeout).ContinueWith(r => { return r.Result is SetThrottleAck; });
    ...
    protected override void Ready(object message)
    {
        switch (message)
        {
            ...
            case SetThrottle st:
            {
                ...
                var modes = new List<Task<SetThrottleAck>> { Task.FromResult(SetThrottleAck.Instance) };
...
public sealed class TokenBucket : ThrottleMode
{
    ...
    private TokenBucket Copy(int? capacity = null, double? tokensPerSecond = null, long? nanoTimeOfLastSend = null,
        int? availableTokens = null)
    {
...
internal sealed class ThrottlerHandle : AbstractTransportAdapterHandle
{
    ...
    public ThrottlerHandle(AssociationHandle wrappedHandle, IActorRef throttlerActor) : base(wrappedHandle,
        ThrottleTransportAdapter.Scheme)
    {
    ...
    public override bool Write(ByteString payload)
    {
        ...
        bool TryConsume(ThrottleMode currentBucket)
        {
            ...
            var allow = res.Item2;
            if (allow)
                return OutboundThrottleMode.CompareAndSet(currentBucket, newBucket) ||
                       TryConsume(OutboundThrottleMode.Value);
            return false;
        }
...
internal sealed class
    ThrottledAssociation : FSM<ThrottledAssociation.ThrottlerState, ThrottledAssociation.IThrottlerData>, ILoggingFSM
{
    /// <summary>
    ...
    public ThrottledAssociation(IActorRef manager, IAssociationEventListener associationHandler,
        AssociationHandle originalHandle, bool inbound)
    {
...
#pragma warning restore CS0618
        ...
        When(ThrottlerState.Throttling, @event =>
        {
            if (@event.FsmEvent is ThrottleMode mode)
            {
                ...
                if (ThrottledMessages.Any())
                    ScheduleDequeue(InboundThrottleMode.TimeToAvailable(MonotonicClock.GetNanos(),
                        ThrottledMessages.Peek().Length));
                Sender.Tell(SetThrottleAck.Instance);
                return Stay();
            }
    ...
    private Address PeekOrigin(ByteString b)
    {
        try
        {
            ...
            if (pdu is Associate associate) return associate.Info.Origin;
            return null;
        }
        catch
        {
    ...
    private void ScheduleDequeue(TimeSpan delay)
    {
        ...
        if (delay <= TimeSpan.Zero) Self.Tell(new Dequeue());
            ...
            SetTimer(DequeueTimerName, new Dequeue(), delay, false);
    }
    ...
    public enum ThrottlerState
    {
        ...
        /// <summary>
        ///     After origin is known and a Checkin message is sent to the manager, we must wait for the
        ///     <see cref="ThrottleMode" />
        ///     for the address
        /// </summary>
        WaitMode,
