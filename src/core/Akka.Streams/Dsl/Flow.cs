...
/// <summary>
///     A <see cref="Flow{TIn,TOut,TMat}" /> is a set of stream processing steps that has one open input and one open
///     output.
/// </summary>
...
public sealed class Flow<TIn, TOut, TMat> : IFlow<TOut, TMat>, IGraph<FlowShape<TIn, TOut>, TMat>
{
    ...
    /// </summary>
    IFlow<TOut2, TMat3> IFlow<TOut, TMat>.ViaMaterialized<TOut2, TMat2, TMat3>(
        IGraph<FlowShape<TOut, TOut2>, TMat2> flow, Func<TMat, TMat2, TMat3> combine)
    {
    ...
    /// <summary>
    ///     Connect this <see cref="Flow{TIn,TOut,TMat}" /> to a <see cref="Sink{TIn,TMat}" />, concatenating the processing
    ///     steps of both.
    ...
    /// <param name="combine">TBD</param>
    /// <returns>TBD</returns>
    public Sink<TIn, TMat3> ToMaterialized<TMat2, TMat3>(IGraph<SinkShape<TOut>, TMat2> sink,
        Func<TMat, TMat2, TMat3> combine)
    {
        if (IsIdentity)
            return Sink.FromGraph(sink as IGraph<SinkShape<TIn>, TMat2>)
                .MapMaterializedValue(mat2 => combine(default, mat2));
    ...
    /// <summary>
    ///     Concatenate the given <seealso cref="Source{TOut,TMat}" /> to this <seealso cref="Flow{TIn,TOut,TMat}" />, meaning
    ///     that once this
    ...
    ///     If this <seealso cref="Flow{TIn,TOut,TMat}" /> gets upstream error - no elements from the given
    ///     <seealso cref="Source{TOut,TMat}" /> will be pulled.
    ...
    /// <summary>
    ///     Join this <see cref="Flow{TIn,TOut,TMat}" /> to another <see cref="Flow{TOut,TIn,TMat2}" />, by cross connecting
    ...
    /// <param name="combine">TBD</param>
    /// <returns>TBD</returns>
    public Flow<TIn2, TOut2, TMatRes> JoinMaterialized<TIn2, TOut2, TMat2, TMatRes>(
        IGraph<BidiShape<TOut, TOut2, TIn2, TIn>, TMat2> bidi, Func<TMat, TMat2, TMatRes> combine)
    {
    ...
    /// <summary>
    ///     Join this <see cref="Flow{TIn,TOut,TMat}" /> to another <see cref="Flow{TIn,TOut,TMat}" />, by cross connecting the
    ///     inputs and outputs, creating a <see cref="IRunnableGraph{TMat}" />
    ///     The <paramref name="combine" /> function is used to compose the materialized values of this flow and that
    ///     Flow into the materialized value of the resulting Flow.
    /// </summary>
    /// <typeparam name="TMat2">TBD</typeparam>
    ...
    /// <param name="combine">TBD</param>
    /// <returns>TBD</returns>
    public IRunnableGraph<TMat3> JoinMaterialized<TMat2, TMat3>(IGraph<FlowShape<TOut, TIn>, TMat2> flow,
        Func<TMat, TMat2, TMat3> combine)
    {
        var copy = flow.Module.CarbonCopy();
    ...
    /// <summary>
    ///     Connect the <see cref="Source{TOut,TMat1}" /> to this <see cref="Flow{TIn,TOut,TMat}" /> and then connect it to the
    ///     <see cref="Sink{TIn,TMat2}" /> and run it.
    ///     The returned tuple contains the materialized values of the <paramref name="source" /> and <paramref name="sink" />,
    ///     e.g. the <see cref="ISubscriber{T}" />
    ...
    /// <returns>TBD</returns>
    public (TMat1, TMat2) RunWith<TMat1, TMat2>(IGraph<SourceShape<TIn>, TMat1> source,
        IGraph<SinkShape<TOut>, TMat2> sink, IMaterializer materializer)
    {
        return Source.FromGraph(source).Via(this).ToMaterialized(sink, Keep.Both).Run(materializer);
    }

    /// <summary>
    ///     Converts this Flow to a <see cref="IRunnableGraph{TMat}" /> that materializes to a Reactive Streams
    ///     <see cref="IProcessor{T1,T2}" />
    ...
    /// </summary>
    /// <returns>
    ///     A <see cref="IRunnableGraph{TMat}" /> that materializes to a <see cref="IProcessor{T1,T2}" /> when Run() is
    ///     called on it.
    /// </returns>
...
public static class Flow
{
    ...
    {
        return new Flow<T, T, NotUsed>(GraphStages.Identity<T>().Module);
    }
    ...
    {
        return FromProcessorMaterialized(() => (factory(), NotUsed.Instance));
    }
    ...
    {
        return new Flow<TIn, TOut, TMat>(new ProcessorModule<TIn, TOut, TMat>(factory));
    }
    ...
    /// <summary>
    ///     Creates a <see cref="Flow{TIn,TOut,TMat}" /> which will use the given function to transform its inputs to outputs.
    ///     It is equivalent
    ...
    /// <param name="factory">TBD</param>
    /// <returns>TBD</returns>
    public static Flow<TIn, TOut, Task<TMat>> Setup<TIn, TOut, TMat>(
        Func<ActorMaterializer, Attributes, Flow<TIn, TOut, TMat>> factory)
    {
        return FromGraph(new SetupFlowStage<TIn, TOut, TMat>(factory));
    }

    /// <summary>
    ///     Creates a <see cref="Flow{TIn,TOut,TMat}" /> from a <see cref="Sink{TIn,TMat}" /> and a
    ///     <see cref="Source{TOut,TMat}" /> where the flow's input
    ...
    /// <param name="source">TBD</param>
    /// <returns>TBD</returns>
    public static Flow<TIn, TOut, NotUsed> FromSinkAndSource<TIn, TOut, TMat>(IGraph<SinkShape<TIn>, TMat> sink,
    ...
    ///     <see cref="Source{TOut,TMat}" /> where the flow's input
    ///     will be sent to the sink and the flow's output will come from the source.
    ///     The <paramref name="combine" /> function is used to compose the materialized values of the
    ///     <see cref="Sink{TIn,TMat}" /> and <see cref="Source{TOut,TMat}" />
    ...
    /// <param name="combine">TBD</param>
    /// <returns>TBD</returns>
    public static Flow<TIn, TOut, TMat> FromSinkAndSource<TIn, TOut, TMat1, TMat2, TMat>(
        IGraph<SinkShape<TIn>, TMat1> sink, IGraph<SourceShape<TOut>, TMat2> source, Func<TMat1, TMat2, TMat> combine)
    {
        return FromGraph(GraphDsl.Create(sink, source, combine,
            (_, @in, @out) => new FlowShape<TIn, TOut>(@in.Inlet, @out.Outlet)));
    }

    /// <summary>
    ///     Creates a real <see cref="Flow" /> upon receiving the first element. Internal <see cref="Flow" /> will not be
    ///     created
    ///     if there are no elements, because of completion, cancellation, or error.
    ///     <para>
    ///         The materialized value of the <see cref="Flow" /> is a <see cref="Task{TMat}" /> that is completed with `TMat`
    ///         when the internal
    ...
    /// <returns>TBD</returns>
    public static Flow<TIn, TOut, Task<Option<TMat>>> LazyInitAsync<TIn, TOut, TMat>(
        Func<Task<Flow<TIn, TOut, TMat>>> flowFactory)
    {
        return FromGraph(new LazyFlow<TIn, TOut, TMat>(_ => flowFactory()));
    }
}
...
public interface IFlow<TOut, out TMat>
{
    ...
    /// <param name="combine">TBD</param>
    /// <returns>TBD</returns>
    IFlow<T2, TMat3> ViaMaterialized<T2, TMat2, TMat3>(IGraph<FlowShape<TOut, T2>, TMat2> flow,
        Func<TMat, TMat2, TMat3> combine);
