...
internal sealed class SeqNo : IComparable<SeqNo>, IEquatable<SeqNo>
{
    ...
    /// <param name="right">The second sequence number used for comparison</param>
    /// <returns>
    ///     <c>true</c> if the first sequence number is less than or equal to the other one; otherwise <c>false</c>
    /// </returns>
    public static bool operator <=(SeqNo left, SeqNo right)
    {
        return left.CompareTo(right) <= 0;
    ...
    /// <param name="right">The second sequence number used for comparison</param>
    /// <returns>
    ///     <c>true</c> if the first sequence number is greater than or equal to the other one; otherwise <c>false</c>
    /// </returns>
    public static bool operator >=(SeqNo left, SeqNo right)
    {
        return left.CompareTo(right) >= 0;
...
/// <summary>
///     Messages that are to be buffered in an <see cref="AckedSendBuffer{T}" /> or <see cref="AckedReceiveBuffer{T}" />
///     has
...
internal sealed class Ack
{
    ...
    /// <param name="nacks">
    ///     Set of sequence numbers between the last delivered one and <paramref name="cumulativeAck" /> that
...
/// <summary>
///     This exception is thrown when the system is unable to fulfill a resend request since negatively acknowledged
///     payload is no longer in buffer.
/// </summary>
...
/// <summary>
///     Implements an immutable resend buffer that buffers messages until they have been acknowledged. Properly removes
///     messages
///     when an <see cref="Ack" /> is received. This buffer works together with <see cref="AckedReceiveBuffer{T}" /> on the
///     receiving end.
/// </summary>
...
{
    public AckedSendBuffer(int capacity, SeqNo maxSeq) : this(capacity, maxSeq, ImmutableList<T>.Empty,
    ...
    /// <exception cref="ResendUnfulfillableException">
    ///     Thrown if we couldn't fit all of the nacks stored inside
    ///     <see cref="Ack" /> onto the buffer.
    /// </exception>
    ...
    public AckedSendBuffer<T> Acknowledge(Ack ack)
    {
        if (ack.CumulativeAck > MaxSeq)
            throw new ArgumentException(nameof(ack),
                $"Highest SEQ so far was {MaxSeq} but cumulative ACK is {ack.CumulativeAck}");
        ...
        return Copy(NonAcked.Where(x => x.Seq > ack.CumulativeAck).ToImmutableList(), newNacked);
    }
    ...
    public AckedSendBuffer<T> Buffer(T msg)
    {
        if (msg.Seq <= MaxSeq)
            throw new ArgumentException(
                $"Sequence number must be monotonic. Received {msg.Seq} which is smaller than {MaxSeq}", nameof(msg));

        if (NonAcked.Count == Capacity) throw new ResendBufferCapacityReachedException(Capacity);

        return Copy(NonAcked.Add(msg), maxSeq: msg.Seq);
    }
    ...
    public AckedSendBuffer<T> Copy(IImmutableList<T> nonAcked = null, IImmutableList<T> nacked = null,
        SeqNo maxSeq = null)
    {
...
internal sealed class AckedReceiveBuffer<T> : IEquatable<AckedReceiveBuffer<T>> where T : IHasSequenceNumber
{
    ...
    public AckReceiveDeliverable<T> ExtractDeliverable()
    {
        ...
        return new AckReceiveDeliverable<T>(Copy(updatedLastDelivered, buffer: newBuf), deliver, ack);
    }
    ...
    {
        var mergedLastDelivered = SeqNo.Max(LastDelivered, other.LastDelivered);
        return Copy(mergedLastDelivered, SeqNo.Max(CumulativeAck, other.CumulativeAck),
            Buf.Union(other.Buf).Where(x => x.Seq > mergedLastDelivered).ToImmutableSortedSet(Comparer));
    }
    ...
    /// <param name="buffer">TBD</param>
    /// <returns>TBD</returns>
    public AckedReceiveBuffer<T> Copy(SeqNo lastDelivered = null, SeqNo cumulativeAck = null,
        ImmutableSortedSet<T> buffer = null)
    {
