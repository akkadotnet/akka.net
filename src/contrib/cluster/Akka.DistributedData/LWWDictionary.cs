...
public static class LWWDictionary
{
    ...
    /// <returns>TBD</returns>
    public static LWWDictionary<TKey, TValue> Create<TKey, TValue>(UniqueAddress node, TKey key, TValue value,
        Clock<TValue> clock = null)
    {
        return LWWDictionary<TKey, TValue>.Empty.SetItem(node, key, value, clock);
    }
    ...
    {
        return elements.Aggregate(LWWDictionary<TKey, TValue>.Empty,
            (dictionary, t) => dictionary.SetItem(t.Item1, t.Item2, t.Item3));
    }
    ...
    /// <param name="clock">TBD</param>
    /// <returns>TBD</returns>
    public static LWWDictionary<TKey, TValue> Create<TKey, TValue>(IEnumerable<(UniqueAddress, TKey, TValue)> elements,
        Clock<TValue> clock = null)
    {
        return elements.Aggregate(LWWDictionary<TKey, TValue>.Empty,
            (dictionary, t) => dictionary.SetItem(t.Item1, t.Item2, t.Item3, clock));
    }
}
    ...
    IEnumerable<KeyValuePair<TKey, TValue>>, ILWWDictionary
{
    ...
    {
        return new LWWDictionary<TKey, TValue>(Underlying.Merge(other.Underlying));
    }
    ...
    {
        return Merge((LWWDictionary<TKey, TValue>)other);
    }
    ...
    {
        return Underlying.Select(x => new KeyValuePair<TKey, TValue>(x.Key, x.Value.Value)).GetEnumerator();
    }
    ...
    {
        return new LWWDictionary<TKey, TValue>(Underlying.Prune(removedNode, collapseInto));
    }
    ...
    {
        return new LWWDictionary<TKey, TValue>(Underlying.PruningCleanup(removedNode));
    }
    ...
    public LWWDictionary<TKey, TValue> SetItem(Cluster.Cluster node, TKey key, TValue value,
        Clock<TValue> clock = null)
    {
        return SetItem(node.SelfUniqueAddress, key, value, clock);
    }
    ...
    /// </summary>
    public LWWDictionary<TKey, TValue> Remove(Cluster.Cluster node, TKey key)
    {
    ...
    {
        return new LWWDictionary<TKey, TValue>(Underlying.Remove(node, key));
    }
    ...
    {
        return obj is LWWDictionary<TKey, TValue> pairs && Equals(pairs);
    }
    ...
    internal sealed class LWWDictionaryDelta : ORDictionary<TKey, LWWRegister<TValue>>.IDeltaOperation,
        IReplicatedDeltaSize, ILWWDictionaryDeltaOperation
    {
        ...
        {
            Underlying = underlying ??
                         throw new ArgumentNullException(nameof(underlying), "Delta operation can not be null");
        ...
        ORDictionary.IDeltaOperation ILWWDictionaryDeltaOperation.Underlying =>
        ...
        public IReplicatedData Merge(IReplicatedData other)
        {
            if (other is LWWDictionaryDelta d)
                return new LWWDictionaryDelta(
                    (ORDictionary<TKey, LWWRegister<TValue>>.IDeltaOperation)Underlying.Merge(d.Underlying));

            return new LWWDictionaryDelta(
                (ORDictionary<TKey, LWWRegister<TValue>>.IDeltaOperation)Underlying.Merge(other));
        }
        ...
        public override bool Equals(object obj)
        {
            return obj is LWWDictionaryDelta operation &&
    ...
    {
        return MergeDelta((ORDictionary<TKey, LWWRegister<TValue>>.IDeltaOperation)delta);
    }
    ...
    {
        return new LWWDictionary<TKey, TValue>(Underlying.ResetDelta());
    }
