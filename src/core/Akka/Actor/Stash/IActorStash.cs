...
public class ActorStashPlugin : ActorProducerPluginBase
{
    ...
    {
        if (actor is IActorStash stashed && stashed.Stash == null) stashed.Stash = context.CreateStash(actor.GetType());
    }
    ...
    {
        if (actor is IActorStash actorStash) actorStash.Stash.UnstashAll();
    }
}