...
public class Tcp : ExtensionIdProvider<TcpExt>
{
    ...
    public readonly struct IncomingConnection
    {
        ...
        public IncomingConnection(EndPoint localAddress, EndPoint remoteAddress,
        ...
        {
            return Flow.JoinMaterialized(handler, Keep.Right).Run(materializer);
        }
    }
...
public class TcpExt : IExtension
{
    ...
    /// <summary>
    ///     Creates a <see cref="Tcp.ServerBinding" /> instance which represents a prospective TCP server binding on the given
    ///     <paramref name="host" /> and <paramref name="port" />/>.
    ///     <para />
    ...
    /// <param name="idleTimeout">TBD</param>
    /// <returns>TBD</returns>
    public Task<Tcp.ServerBinding> BindAndHandle(Flow<ByteString, ByteString, NotUsed> handler,
        IMaterializer materializer, string host, int port, int backlog = 100,
        IImmutableList<Inet.SocketOption> options = null, bool halfClose = false, TimeSpan? idleTimeout = null)
    {
    ...
    /// <summary>
    ///     Creates a <see cref="Tcp.OutgoingConnection" /> instance representing a prospective TCP client connection to the
    ///     given endpoint.
    ///     <para>
    ...
    ///     after both the client and server finished writing. This setting is recommended for clients and therefore it is the
    ///     default setting.
    ...
    /// <param name="idleTimeout">TBD</param>
    /// <returns>TBD</returns>
    public Flow<ByteString, ByteString, Task<Tcp.OutgoingConnection>> OutgoingConnection(EndPoint remoteAddress,
    ...
    {
        return OutgoingConnection(CreateEndpoint(host, port));
    }
