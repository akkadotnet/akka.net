...
public static class FileIO
{
    ...
    ///     It materializes a <see cref="Task{TResult}" /> of <see cref="IOResult" /> containing the number of bytes read from
    ///     the source file upon completion,
    ...
    {
        return new Source<ByteString, Task<IOResult>>(new FileSource(f, chunkSize, startPosition,
            DefaultAttributes.FileSource,
            new SourceShape<ByteString>(new Outlet<ByteString>("FileSource"))));
    }

    /// <summary>
    ///     Creates a Sink which writes incoming <see cref="ByteString" /> elements to the given file. Overwrites existing
    ///     files
    ...
    /// <param name="flushSignaler">
    ///     when passed an instance of <see cref="FlushSignaler" />, can be used to send a manual flush
    ///     signal to the file sink
    /// </param>
    /// <returns>TBD</returns>
    public static Sink<ByteString, Task<IOResult>> ToFile(FileInfo f, FileMode? fileMode = null, long startPosition = 0,
        bool autoFlush = false, FlushSignaler flushSignaler = null)
    {
        return new Sink<ByteString, Task<IOResult>>(new FileSink(f, startPosition, fileMode ?? FileMode.Create,
