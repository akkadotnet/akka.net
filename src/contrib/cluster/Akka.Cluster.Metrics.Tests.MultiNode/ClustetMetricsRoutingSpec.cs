...
public class AdaptiveLoadBalancingRouterSpec : MultiNodeClusterSpec
{
    ...
    private async Task Should_prefer_node_with_more_free_memory_capacity()
    {
        ...
        await RunOnAsync(async () =>
        {
            ...
            foreach (var i in Enumerable.Range(1, iterationCount)) router2.Tell("hit");
    ...
    private async Task Should_create_routees_from_configuration()
    {
        await RunOnAsync(async () =>
        {
            var router3 =
                Sys.ActorOf(
                    FromConfig.Instance.Props(Props.Create<AdaptiveLoadBalancingRouterConfig.MemoryAllocator>()),
                    "router3");
            ...
            routees.Select(r => FullAddress((r as ActorRefRoutee).Actor)).Distinct().Should()
                .BeEquivalentTo(Node(_config.Node1).Address);
        }, _config.Node1);
    ...
    private Task AwaitMetrics(int factor = 10)
    {
        return Task.Delay((int)(_metricsSettings.CollectorSampleInterval.TotalMilliseconds * factor));
    }
    ...
    private IImmutableDictionary<Address, int> ReceiveReplies(int expectedReplies)
    {
        ...
        return ReceiveWhile(5.Seconds(), msg => (msg as AdaptiveLoadBalancingRouterConfig.Reply)?.Address,
                expectedReplies)
