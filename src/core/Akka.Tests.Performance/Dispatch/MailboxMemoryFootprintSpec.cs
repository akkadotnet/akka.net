...
public class MailboxMemoryFootprintSpec
{
    ...
    private readonly ActorSystem _actorSystem = ActorSystem.Create("Testing");
    ...
    [PerfBenchmark(Description = "Measures the amount of memory used by 10,000 ConcurrentQueueMailboxes",
    ...
    [PerfBenchmark(Description = "Measures the amount of memory used by 10,000 BoundedDequeBasedMailboxes",
        RunMode = RunMode.Iterations, NumberOfIterations = 13, TestMode = TestMode.Measurement)]
    [MemoryMeasurement(MemoryMetric.TotalBytesAllocated)]
    [CounterMeasurement(CreateThroughputCounter)]
    ...
    public void Teardown(BenchmarkContext context)
    {
        ...
        if (!cleanShutdown) context.Trace.Warning($"Failed to shutdown ActorSystem within {shutdownTimeout}");
        _mailboxes.Clear();
        _mailboxes = null;
    }
}