...
internal class InboxQueue<T> : ICollection<T>
{
    ...
    {
        var node = _inner.First;
        while (!(node == null || predicate(node.Value))) node = node.Next;
...
public interface IInboxable : ICanWatch
{
    ...
    /// <summary>
    ///     Receive a next message from current <see cref="IInboxable" /> with default timeout. This call will return
    ///     immediately,
    ...
    /// <summary>
    ///     Receive a next message satisfying specified <paramref name="predicate" /> under provided
...
public class Inbox : IInboxable, IDisposable
{
    ...
    {
        var task = Receiver.Ask(new Select(_system.Scheduler.MonotonicClock + timeout, predicate),
    ...
    public static Inbox Create(ActorSystem system)
    {
        ...
        var receiver = ((ActorSystemImpl)system).SystemActorOf(Props.Create(() => new InboxActor(inboxSize)),
            "inbox-" + Interlocked.Increment(ref _inboxNr));
    ...
    private object AwaitResult(Task<object> task, TimeSpan timeout)
    {
        ...
        if (task.Result is Status.Failure received && received.Cause is TimeoutException)
            throw new TimeoutException(
                $"Inbox {Receiver.Path} received a status failure response message: {received.Cause.Message}",
                received.Cause);
