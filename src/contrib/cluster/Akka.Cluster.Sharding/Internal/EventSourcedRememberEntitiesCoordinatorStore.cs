...
internal class EventSourcedRememberEntitiesCoordinatorStore : PersistentActor
{
    ...
    protected override bool ReceiveRecover(object message)
    {
        switch (message)
        {
            ...
            case RecoveryCompleted _:
                Log.Debug("Recovery complete. Current shards [{0}]. Written Marker {1}", string.Join(", ", _shards),
                    _writtenMarker);
                if (!_writtenMarker)
    ...
    protected override bool ReceiveCommand(object message)
    {
        switch (message)
        {
            ...
            case DeleteMessagesSuccess e:
                ...
                var deleteFrom =
                    Math.Max(0,
                        deleteTo - Settings.TuningParameters.KeepNrOfBatches * Settings.TuningParameters.SnapshotAfter);
                Log.Debug(
    ...
    public sealed class State : IClusterShardingSerializable, IEquatable<State>
    {
        ...
        {
            return $"State(shards:{string.Join(", ", Shards)}, writtenMigrationMarker:{WrittenMigrationMarker})";
        }
