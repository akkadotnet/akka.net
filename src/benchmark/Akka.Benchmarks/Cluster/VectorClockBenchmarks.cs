...
public class VectorClockBenchmarks
{
    ...
    internal (VectorClock clock, ImmutableSortedSet<Node> nodes) CreateVectorClockOfSize(int size)
    {
        return Enumerable.Range(1, size)
            .Aggregate((Create(), ImmutableSortedSet<Node>.Empty),
                (tuple, i) =>
                {
                    var (vc, nodes) = tuple;
    ...
    internal VectorClock CopyVectorClock(VectorClock vc)
    {
        var versions = vc.Versions.Aggregate(ImmutableSortedDictionary<Node, long>.Empty,
            (newVersions, pair) => { return newVersions.SetItem(Node.FromHash(pair.Key.ToString()), pair.Value); });
