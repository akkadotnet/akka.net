...
public class ClusterMetricsEnabledSpec : MultiNodeClusterSpec
{
    ...
    private ClusterMetricsEnabledSpec(ClusterMetricsCommonConfig config) : base(config,
        typeof(ClusterMetricsEnabledSpec))
    {
        _config = config;
    ...
    {
        await AwaitAssertAsync(() => { AwaitClusterUp(Roles.ToArray()); }, TimeSpan.FromSeconds(30));
    ...
    private async Task Should_reflect_the_correct_number_or_node_metrics_in_cluster_view()
    {
        ...
        {
            RunOn(() => { Cluster.Leave(Node(_config.Node1).Address); }, _config.Node2);
...
{
    private ClusterMetricsDisabledSpec(ClusterMetricsDisabledConfig config) : base(config,
        typeof(ClusterMetricsDisabledSpec))
    {
        MetricsView = new ClusterMetricsView(Cluster.System);
    }
