...
public class ClusterDeployerSpec : AkkaSpec
{
    ...
    public void RemoteDeployer_must_be_able_to_parse_akka_actor_deployment_with_specified_cluster_pool()
    {
        ...
        deployment.RouterConfig.AsInstanceOf<ClusterRouterPool>().Local.AsInstanceOf<RoundRobinPool>().NrOfInstances
            .ShouldBe(20);
        ...
        deployment.RouterConfig.AsInstanceOf<ClusterRouterPool>().Settings.AsInstanceOf<ClusterRouterPoolSettings>()
            .MaxInstancesPerNode.ShouldBe(3);
    ...
    public void RemoteDeployer_must_be_able_to_parse_akka_actor_deployment_with_specified_cluster_group()
    {
        ...
        deployment.RouterConfig.AsInstanceOf<ClusterRouterGroup>().Local.AsInstanceOf<RoundRobinGroup>().GetPaths(Sys)
            .ShouldBe(new[] { "/user/myservice" });
        ...
        deployment.RouterConfig.AsInstanceOf<ClusterRouterGroup>().Settings.AsInstanceOf<ClusterRouterGroupSettings>()
            .RouteesPaths.ShouldBe(new[] { "/user/myservice" });
        deployment.Scope.ShouldBe(ClusterScope.Instance);
    ...
    [Fact]
    public void
        BugFix2266RemoteDeployer_must_be_able_to_parse_broadcast_group_cluster_router_with_default_nr_of_routees_routees()
    {
        ...
        deployment.RouterConfig.AsInstanceOf<ClusterRouterGroup>().Local.AsInstanceOf<BroadcastGroup>().GetPaths(Sys)
            .ShouldBe(new[] { "/user/myservice" });
