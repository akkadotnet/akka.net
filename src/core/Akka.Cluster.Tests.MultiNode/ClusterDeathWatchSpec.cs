...
public class ClusterDeathWatchSpec : MultiNodeClusterSpec
{
    ...
    {
        if (!Log.IsDebugEnabled) MuteMarkingAsUnreachable();
        base.AtStartup();
    }
        ...
        An_actor_watching_a_remote_actor_in_the_cluster_must_receive_terminated_when_watched_node_becomes_down_removed()
    {
        Within(TimeSpan.FromSeconds(30), () =>
        {
            ...
            RunOn(() =>
            {
                ...
                AwaitAssert(() =>
                    Assert.Contains(GetAddress(_config.Third), ClusterView.UnreachableMembers.Select(x => x.Address)));
                Cluster.Down(GetAddress(_config.Third));
                //removed
                AwaitAssert(() =>
                    Assert.DoesNotContain(GetAddress(_config.Third), ClusterView.Members.Select(x => x.Address)));
                AwaitAssert(() => Assert.DoesNotContain(GetAddress(_config.Third),
                    ClusterView.UnreachableMembers.Select(x => x.Address)));
                ExpectMsg(path3);
            ...
            RunOn(() =>
            {
                ...
                RunOn(() =>
                {
                    ...
                    AwaitAssert(() => Assert.Contains(GetAddress(_config.Second),
                        ClusterView.UnreachableMembers.Select(x => x.Address)));
                    Cluster.Down(GetAddress(_config.Second));
                    //removed
                    AwaitAssert(() =>
                        Assert.DoesNotContain(GetAddress(_config.Second), ClusterView.Members.Select(x => x.Address)));
                    AwaitAssert(() => Assert.DoesNotContain(GetAddress(_config.Second),
                        ClusterView.UnreachableMembers.Select(x => x.Address)));
                }, _config.Third);
                EnterBarrier("second-terminated");
                EnterBarrier("third-terminated");
        ...
        An_actor_watching_a_remote_actor_in_the_cluster_must_be_able_to_watch_actor_before_node_joins_cluster_and_cluster_remote_watcher_takes_over_from_remote_watcher()
    {
        Within(TimeSpan.FromSeconds(20), () =>
        {
            ...
            RunOn(() =>
            {
                Sys.ActorSelection(new RootActorPath(GetAddress(_config.Fifth)) / "user" / "subject5")
                    .Tell(new Identify("subject5"), TestActor);
                ...
                AwaitAssert(() =>
                {
                    ...
                    var stats = ExpectMsg<RemoteWatcher.Stats>();
            ...
            RunOn(() =>
            {
                ...
                AwaitAssert(() =>
                    ClusterView.UnreachableMembers.Select(x => x.Address).Contains(GetAddress(_config.Fifth))
                        .ShouldBeTrue());
                Cluster.Down(GetAddress(_config.Fifth));
                // removed
                AwaitAssert(() => Assert.DoesNotContain(GetAddress(_config.Fifth),
                    ClusterView.UnreachableMembers.Select(x => x.Address)));
                AwaitAssert(() =>
                    Assert.DoesNotContain(GetAddress(_config.Fifth), ClusterView.Members.Select(x => x.Address)));
            }, _config.Fourth);

            EnterBarrier("fifth-terminated");
            RunOn(() => { ExpectMsg<Terminated>().ActorRef.Path.Name.ShouldBe("subject5"); }, _config.First);
        ...
        An_actor_watching_a_remote_actor_in_the_cluster_must_be_able_to_shutdown_system_when_using_remote_deployed_actor_on_node_that_crashed()
    {
        Within(TimeSpan.FromSeconds(20), () =>
        {
            ...
            RunOn(() => { Sys.ActorOf(Props.Create(() => new EndActor(TestActor, null)), "end"); }, _config.First);
            ...
            RunOn(() =>
            {
                ...
                AwaitAssert(() =>
                    ClusterView.UnreachableMembers.Select(x => x.Address).Contains(GetAddress(_config.First))
                        .ShouldBeTrue());
                Cluster.Down(GetAddress(_config.First));
                // removed
                AwaitAssert(() => Assert.DoesNotContain(GetAddress(_config.First),
                    ClusterView.UnreachableMembers.Select(x => x.Address)));
                AwaitAssert(() =>
                    Assert.DoesNotContain(GetAddress(_config.First), ClusterView.Members.Select(x => x.Address)));
                ...
                try
                {
                    if (!Sys.WhenTerminated
                            .Wait(timeout)) // TestConductor.Shutdown called by First MUST terminate this actor system
    ...
    private class Observer : ReceiveActor
    {
        ...
        public Observer(ActorPath path2, ActorPath path3, TestLatch watchEstablished, IActorRef testActorRef)
        {
            ...
            Receive<Terminated>(terminated => { _testActorRef.Tell(terminated.ActorRef.Path); });
