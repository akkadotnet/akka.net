...
public interface ISourceQueue<in T>
{
    ...
    ///     <para>
    ///         - competes with <see cref="QueueOfferResult.Enqueued" /> if element
    ///         is consumed by a stream
    ///     </para>
    ///     <para>
    ///         - competes with <see cref="QueueOfferResult.Dropped" /> when stream
    ///         dropped offered element
    ///     </para>
    ///     <para>
    ///         - competes with <see cref="QueueOfferResult.QueueClosed" /> when
    ///         stream is completed while task is active
    ///     </para>
    ///     <para>
    ///         - competes with <see cref="QueueOfferResult.Failure" /> when failure
    ///         to enqueue element from upstream
    ///     </para>
    ///     <para>
    ///         - fails if stream is completed or you cannot call offer in this moment
    ///         because of implementation rules (like for backpressure mode and full buffer
    ///         you need to wait for last offer call task completion.
    ///     </para>
    /// </summary>
...
{
    /// <summary>
    ///     Complete the stream normally. Use <see cref="ISourceQueue{T}.WatchCompletionAsync" /> to be notified of this
    ///     operation’s success.
    /// </summary>
    void Complete();

    /// <summary>
    ///     Complete the stream with a failure. Use <see cref="ISourceQueue{T}.WatchCompletionAsync" /> to be notified of this
    ///     operation’s success.
    /// </summary>
