...
[Serializable]
public sealed class ORDictionaryKey<TKey, TValue> : Key<ORDictionary<TKey, TValue>>, IORDictionaryKey
...
{
    public static ORDictionary<TKey, TValue> Create<TKey, TValue>(UniqueAddress node, TKey key, TValue value)
        where TValue : IReplicatedData<TValue>
    {
        return ORDictionary<TKey, TValue>.Empty.SetItem(node, key, value);
    }

    public static ORDictionary<TKey, TValue> Create<TKey, TValue>(params (UniqueAddress, TKey, TValue)[] elements)
        where TValue : IReplicatedData<TValue>
    {
        return elements.Aggregate(ORDictionary<TKey, TValue>.Empty,
            (acc, t) => acc.SetItem(t.Item1, t.Item2, t.Item3));
    }

    public static ORDictionary<TKey, TValue> Create<TKey, TValue>(IEnumerable<(UniqueAddress, TKey, TValue)> elements)
        where TValue : IReplicatedData<TValue>
    {
        return elements.Aggregate(ORDictionary<TKey, TValue>.Empty,
            (acc, t) => acc.SetItem(t.Item1, t.Item2, t.Item3));
    ...
    where TValue : IReplicatedData<TValue>
{
    ...
    /// </summary>
    public static readonly ORDictionary<TKey, TValue> Empty = new(ORSet<TKey>.Empty,
    ...
    {
        return MergeDelta((IDeltaOperation)delta);
    }
    ...
    {
        return Merge((ORDictionary<TKey, TValue>)other);
    }

    public ImmutableHashSet<UniqueAddress> ModifiedByNodes =>
        KeySet.ModifiedByNodes.Union(ValueMap.Aggregate(ImmutableHashSet<UniqueAddress>.Empty,
            (acc, pair) =>
            {
    ...
    ///     type that is used.
    ///     Consider using <see cref="AddOrUpdate(Akka.Cluster.Cluster, TKey, TValue, Func{TValue, TValue})">AddOrUpdate</see>
    ///     instead of <see cref="SetItem(Akka.Cluster.Cluster,TKey,TValue)" /> if you want modify
    ///     existing entry.
    ...
    ///     undesired effects of merging will occur. Use <see cref="ORMultiValueDictionary{TKey,TValue}" /> or
    ///     <see cref="AddOrUpdate(Akka.Cluster.Cluster, TKey, TValue, Func{TValue, TValue})">AddOrUpdate</see> instead.
    /// </summary>
    public ORDictionary<TKey, TValue> SetItem(Cluster.Cluster node, TKey key, TValue value)
    {
        return SetItem(node.SelfUniqueAddress, key, value);
    }
    ...
    ///     type that is used.
    ///     Consider using <see cref="AddOrUpdate(Akka.Cluster.Cluster, TKey, TValue, Func{TValue, TValue})">AddOrUpdate</see>
    ///     instead of <see cref="SetItem(UniqueAddress,TKey,TValue)" /> if you want modify
    ///     existing entry.
    ///     <see cref="ArgumentException" /> is thrown if you try to replace an existing <see cref="ORSet{T}" />
    ///     value, because important history can be lost when replacing the `ORSet` and
    ///     undesired effects of merging will occur. Use <see cref="ORMultiValueDictionary{TKey,TValue}" /> or
    ...
    public ORDictionary<TKey, TValue> SetItem(UniqueAddress node, TKey key, TValue value)
    {
        if (value is IORSet && ValueMap.ContainsKey(key))
            throw new ArgumentException("ORDictionary.SetItems may not be used to replace an existing ORSet",
                nameof(value));
    ...
    {
        return AddOrUpdate(node.SelfUniqueAddress, key, initial, modify);
    }
        ...
        Func<TValue, TValue> modify)
    {
        ...
        if (valueDeltas && oldValue is IDeltaReplicatedData deltaOldValue)
        {
            ...
            {
                var op = new UpdateDeltaOperation(newKeys.Delta,
                    ImmutableDictionary.CreateRange(new[]
                    {
                        new KeyValuePair<TKey, IReplicatedData>(key, newValueDelta)
                    }));
    ...
    public override string ToString()
    {
        ...
        foreach (var entry in Entries) sb.Append(entry.Key).Append("->").Append(entry.Value).Append(", ");
        sb.Append(')');
        return sb.ToString();
    }
    ...
    internal abstract class AtomicDeltaOperation : IDeltaOperation, IReplicatedDeltaSize, ORDictionary.IDeltaOperation
    {
        ...
        public IDeltaReplicatedData Zero => Empty;
    ...
    internal sealed class PutDeltaOperation : AtomicDeltaOperation, ORDictionary.IPutDeltaOp
    {
        ...
        public override IReplicatedData Merge(IReplicatedData other)
        {
            ...
            if (other is PutDeltaOperation put && Equals(Key, put.Key))
                return new PutDeltaOperation((ORSet<TKey>.IDeltaOperation)Underlying.Merge(put.Underlying), put.Key,
                    put.Value);

            if ((update = other as UpdateDeltaOperation) != null && update.Values.Count == 1 &&
                update.Values.ContainsKey(Key))
            {
    ...
    {
        public UpdateDeltaOperation(ORSet<TKey>.IDeltaOperation underlying,
            ImmutableDictionary<TKey, IReplicatedData> values)
        {
        ...
        public override IReplicatedData Merge(IReplicatedData other)
        {
            ...
            if (other is UpdateDeltaOperation update)
            {
                ...
                return new UpdateDeltaOperation(
                    (ORSet<TKey>.IDeltaOperation)Underlying.Merge(update.Underlying),
                    builder.ToImmutable());
            }

            if ((put = other as PutDeltaOperation) != null && Values.Count == 1 && Values.ContainsKey(put.Key))
                return new PutDeltaOperation((ORSet<TKey>.IDeltaOperation)Underlying.Merge(put.Underlying), put.Key,
                    put.Value);
    ...
    internal sealed class RemoveDeltaOperation : AtomicDeltaOperation, ORDictionary.IRemoveDeltaOp
    {
        ...
        public override bool Equals(IDeltaOperation op)
        {
            if (ReferenceEquals(null, op)) return false;
            if (ReferenceEquals(this, op)) return true;
            if (op is RemoveDeltaOperation remove) return Underlying.Equals(remove.Underlying);
            return false;
        }
    ...
    internal sealed class DeltaGroup : IDeltaOperation, IReplicatedDeltaSize, ORDictionary.IDeltaGroupOp
    {
        ...
        public IReadOnlyList<ORDictionary.IDeltaOperation> OperationsSerialization =>
            Operations.Cast<ORDictionary.IDeltaOperation>().ToList();
        ...
        public bool Equals(IDeltaOperation op)
        {
            if (ReferenceEquals(null, op)) return false;
            if (ReferenceEquals(this, op)) return true;
            if (op is DeltaGroup group) return Operations.SequenceEqual(group.Operations);
            return false;
        }

        public override bool Equals(object obj)
        {
            return obj is IDeltaOperation operation && Equals(operation);
        }
    ...
    [NonSerialized]
    private readonly IDeltaOperation
    ...
    private bool ProcessDelta(IDeltaOperation delta, ImmutableDictionary<TKey, TValue>.Builder mergedValues,
        ImmutableDictionary<TKey, TValue>.Builder tombstonedValues, ref ORSet<TKey> mergedKeys)
    {
        switch (delta)
        {
            ...
            case RemoveDeltaOperation removeOp:
                ...
                {
                    throw new ArgumentException("ORDictionary.RemoveDeltaOp must contain ORSet.RemoveDeltaOp inside");
                }
            ...
            case UpdateDeltaOperation updateOp:
                ...
                foreach (var entry in updateOp.Values)
                {
                    ...
                    if (mergedKeys.Contains(key))
                    {
                        ...
                        {
                            if (entry.Value is IReplicatedDelta v)
                                mergedValues[key] = MergeValue((TValue)v.Zero, entry.Value);
    ...
    private bool ProcessNestedDelta(IDeltaOperation delta, ImmutableDictionary<TKey, TValue>.Builder mergedValues,
        ImmutableDictionary<TKey, TValue>.Builder tombstonedValues, ref ORSet<TKey> mergedKeys)
    {
    ...
    {
        return Delta == null ? delta : (IDeltaOperation)Delta.Merge(delta);
    }
