...
public class WriteAggregatorSpec : TestKit.Xunit2.TestKit
{
    ...
    {
        return Props.Create(() =>
            new TestWriteAggregator<GSet<string>>(KeyA, data, null, consistency, probes, nodes, unreachable,
                replyTo, durable));
    }
    ...
    private static Props TestWriteAggregatorPropsWithDelta(ORSet<string> data,
        ...
        bool durable)
    {
        return Props.Create(() => new TestWriteAggregator<ORSet<string>>(KeyB, data, delta, consistency, probes,
    ...
    public void WriteAggregator_must_send_at_least_half_N_plus_1_replicas_when_WriteMajority()
    {
        ...
        var aggregator = Sys.ActorOf(TestWriteAggregatorProps(_data, _writeMajority, Probes(probe.Ref), _nodes,
            ImmutableHashSet<Address>.Empty, TestActor, false));
    ...
    public void WriteAggregator_must_timeout_when_less_than_required_ACKs()
    {
        ...
        Within(Dilated(TimeSpan.FromSeconds(10)),
            () => // have to pad the time here, since default timeout is ~3s which is also default wait time
            {
    ...
    {
        var probe = CreateTestProbe();
        var aggregator = Sys.ActorOf(TestWriteAggregatorPropsWithDelta(_fullState2, _delta, _writeMajority,
            Probes(probe.Ref), _nodes, ImmutableHashSet<Address>.Empty, TestActor, false));
    ...
    public void WriteAggregator_with_delta_must_retry_with_full_state_when_no_immediate_reply_or_nack()
    {
        ...
        var aggregator = Sys.ActorOf(TestWriteAggregatorPropsWithDelta(_fullState2, _delta, _writeAll, testProbeRefs,
            _nodes, ImmutableHashSet<Address>.Empty, TestActor, false));
    ...
    {
        var probe = CreateTestProbe();
        var aggregator = Sys.ActorOf(TestWriteAggregatorPropsWithDelta(_fullState2, _delta, _writeAll,
            Probes(probe.Ref), _nodes, ImmutableHashSet<Address>.Empty, TestActor, false));
    ...
    {
        var probe = CreateTestProbe();
        var aggregator = Sys.ActorOf(TestWriteAggregatorProps(_data, _writeThree, Probes(probe.Ref), _nodes,
            ImmutableHashSet<Address>.Empty, TestActor, true));
        Watch(aggregator);
    ...
    internal class TestWriteAggregator<T> : WriteAggregator where T : IReplicatedData
    {
        ...
        protected override ActorSelection Replica(Address address)
        {
            return Context.ActorSelection(_probes[address].Path);
        }
    }
