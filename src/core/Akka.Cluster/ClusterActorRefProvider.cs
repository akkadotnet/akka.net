...
internal sealed class ClusterDeployer : RemoteDeployer
{
    ...
    ///     This exception is thrown when the router is not configured as either a <see cref="Pool" /> or a
    ///     <see cref="Group" />.
    /// </exception>
            ...
            if (deploy.Config.GetBoolean("cluster.enabled"))
            {
                if (deploy.Scope != Deploy.NoScopeGiven)
                    throw new ConfigurationException(
                        $"Cluster deployment can't be combined with scope [{deploy.Scope}]");
                ...
                throw new ArgumentException(
                    $"Cluster-aware router can only wrap Pool or Group, got [{deploy.RouterConfig.GetType()}]");
            }
