...
/// <summary>
///     INTERNAL API
///     Extension that allows us to look up transport adapters based upon the settings provided inside
///     <see cref="RemoteSettings" />
/// </summary>
...
public abstract class AbstractTransportAdapterHandle : AssociationHandle
{
    ...
    /// <param name="wrappedHandle">TBD</param>
    /// <param name="addedSchemeIdentifier">TBD</param>
    protected AbstractTransportAdapterHandle(Address originalLocalAddress, Address originalRemoteAddress,
        AssociationHandle wrappedHandle, string addedSchemeIdentifier) : base(originalLocalAddress,
    ...
    {
        return Equals(OriginalLocalAddress, other.OriginalLocalAddress) &&
               Equals(OriginalRemoteAddress, other.OriginalRemoteAddress) && Equals(WrappedHandle, other.WrappedHandle);
    }
...
public abstract class ActorTransportAdapter : AbstractTransportAdapter
{
    ...
    {
        return System.ActorSelection("/system/transports")
            .Ask<IActorRef>(new RegisterTransportActor(ManagerProps, ManagerName));
    }

    /// <inheritdoc />
    protected override Task<IAssociationEventListener> InterceptListen(Address listenAddress,
        Task<IAssociationEventListener> listenerTask)
    {
    ...
    /// <inheritdoc />
    protected override void InterceptAssociate(Address remoteAddress,
        TaskCompletionSource<AssociationHandle> statusPromise)
    {
    ...
    public override Task<bool> Shutdown()
    {
        ...
        return Task.WhenAll(stopTask, transportStopTask).ContinueWith(
            x => x.IsCompleted && !(x.IsFaulted || x.IsCanceled), TaskContinuationOptions.ExecuteSynchronously);
    }
}
    ...
    protected override void OnReceive(object message)
    {
        switch (message)
        {
            ...
            case ListenerRegistered listener:
                ...
                foreach (var dEvent in DelayedEvents) Self.Tell(dEvent, ActorRefs.NoSender);
