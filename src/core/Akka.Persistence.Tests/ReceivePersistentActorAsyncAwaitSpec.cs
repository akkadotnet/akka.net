...
internal class ReceiveTimeoutAsyncActor : ReceivePersistentActor
{
    ...
    public ReceiveTimeoutAsyncActor(string persistenceId)
    {
        ...
        Command<ReceiveTimeout>(_ => { _replyTo.Tell("GotIt"); });
...
internal class AsyncActor : ReceivePersistentActor
{
...
public class SuspendActor : ReceivePersistentActor
{
    public SuspendActor(string persistenceId)
    {
        ...
        var state = 0;
        Command<string>(s => s == "change", _ => { state = 1; });
...
public class ReceivePersistentActorAsyncAwaitSpec : AkkaSpec
{
    ...
    public async Task Actors_should_be_able_to_block_ask_message_loop()
    {
        var actor = Sys.ActorOf(
            Props.Create(() => new AsyncAwaitActor("pid")).WithDispatcher("akka.actor.task-dispatcher"), "Worker");
        var asker = Sys.ActorOf(
            Props.Create(() => new BlockingAsker("pid", actor)).WithDispatcher("akka.actor.task-dispatcher"), "Asker");
        var task = asker.Ask("start");
        actor.Tell(123, ActorRefs.NoSender);
        var res = await task;
        Assert.Equal("done", res);
    ...
    public async Task Actor_should_be_able_to_resume_suspend()
    {
        ...
        for (var i = 0; i < 10; i++) asker.Tell("msg #" + i);
        ...
        public AsyncExceptionCatcherActor(string persistenceId)
        {
            ...
            CommandAsync<string>(async m =>
            {
                ...
                try
                {
                    ...
                    await Task.Factory.StartNew(() => { throw new Exception("should not restart"); });
                }
                catch (Exception)
                {
                }
...
#pragma warning disable CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
                Task.Run(() =>
                {
                    ...
                    sender.Tell(msg)); // ContinueWith will schedule with the implicit ActorTaskScheduler
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
