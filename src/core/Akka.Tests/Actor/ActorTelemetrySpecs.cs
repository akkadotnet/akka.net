...
public class ActorTelemetrySpecs : AkkaSpec
{
    ...
    public async Task ActorTelemetry_must_be_accurate()
    {
        ...
        {
            var telemetry =
                await subscriber.Ask<TelemetrySubscriber.GetTelemetry>(TelemetrySubscriber.GetTelemetryRequest
                    .Instance);
    ...
    private class ParentActor : ReceiveActor
    {
        public ParentActor()
        {
            ...
            {
                for (var i = 0; i < cmd.Count; i++) Context.ActorOf(Props.Create<ChildActor>(), $"child-{i}");
            ...
            {
                foreach (var child in Context.GetChildren()) child.Forward(cmd);
            ...
            Receive<string>(cmd =>
            {
                if (cmd == "restart") throw new Exception("Restarting");
            });
        }
    }
