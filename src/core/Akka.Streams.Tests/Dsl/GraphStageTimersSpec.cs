...
public class GraphStageTimersSpec : AkkaSpec
{
    ...
    public async Task GraphStage_timer_support_must_correctly_cancel_a_named_timer()
    {
        ...
        await WithinAsync(TimeSpan.FromMilliseconds(200), TimeSpan.FromSeconds(3000),
            async () => await ExpectMsgAsync(new Tick(1)));
        await ExpectNoMsgAsync(TimeSpan.FromSeconds(1));
        driver.StopStage();
    }
    ...
    public async Task GraphStage_timer_support_must_receive_and_cancel_a_repeated_timer()
    {
        ...
        var seq = ReceiveWhile(TimeSpan.FromSeconds(30), o => (Tick)o, 5);
    ...
    private sealed class Tick
    {
        ...
        private bool Equals(Tick other)
        {
            return N == other.N;
        }
    ...
    private sealed class TestStage : SimpleLinearGraphStage<int>
    {
        ...
        private sealed class Logic : TimerGraphStageLogic
        {
            ...
            {
                return _stage._testKit.Dilated(TimeSpan.FromMilliseconds(milliseconds));
            }
    ...
    private sealed class TestStage2 : SimpleLinearGraphStage<int>
    {
        ...
        private sealed class Logic : TimerGraphStageLogic
        {
            ...
            public Logic(TestStage2 stage) : base(stage.Shape)
            {
                ...
                SetHandler(stage.Inlet, DoNothing,
                    CompleteStage,
                    FailStage);


                SetHandler(stage.Outlet, DoNothing, _ => CompleteStage());
            }

            public override void PreStart()
            {
                ScheduleRepeatedly(TimerKey, TimeSpan.FromMilliseconds(100));
            }
    ...
    private sealed class ThrowStage : SimpleLinearGraphStage<int>
    {
        ...
        private sealed class Logic : TimerGraphStageLogic
        {
            ...
            public override void PreStart()
            {
                ScheduleOnce("tick", TimeSpan.FromMilliseconds(100));
            }
