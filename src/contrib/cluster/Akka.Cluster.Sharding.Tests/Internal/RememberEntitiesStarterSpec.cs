...
public class RememberEntitiesStarterSpec : AkkaSpec
{
    ...
    public RememberEntitiesStarterSpec(ITestOutputHelper helper) : base(SpecConfig, helper)
    {
    }
    ...
    public void RememberEntitiesStarter_must_try_start_all_entities_directly_with_entity_recovery_strategy_all_default()
    {
        ...
        var rememberEntityStarter = Sys.ActorOf(
            RememberEntityStarter.Props(regionProbe.Ref, shardProbe.Ref, shardId,
                ImmutableHashSet.Create("1", "2", "3"), defaultSettings));
    ...
    [Fact]
    public void
        RememberEntitiesStarter_must_retry_start_all_entities_with_no_ack_with_entity_recovery_strategy_all_default()
    {
        var regionProbe = CreateTestProbe();
        var shardProbe = CreateTestProbe();
        var shardId = NextShardId();
        ...
        var customSettings = ClusterShardingSettings.Create(
            ...
            Sys.Settings.Config.GetConfig("akka.cluster.singleton"));

        var rememberEntityStarter = Sys.ActorOf(
            RememberEntityStarter.Props(regionProbe.Ref, shardProbe.Ref, shardId,
                ImmutableHashSet.Create("1", "2", "3"), customSettings));
    ...
    public void RememberEntitiesStarter_must_inform_the_shard_when_entities_has_been_reallocated_to_different_shard_id()
    {
        ...
        regionProbe.LastSender.Tell(new ShardRegion.StartEntityAck(start1.EntityId,
            start1.EntityId == "1" ? shardId : $"Relocated{start1.EntityId}")); // keep 1 on current shard

        var start2 = regionProbe.ExpectMsg<ShardRegion.StartEntity>();
        regionProbe.LastSender.Tell(new ShardRegion.StartEntityAck(start2.EntityId,
            start2.EntityId == "1" ? shardId : $"Relocated{start2.EntityId}"));

        var start3 = regionProbe.ExpectMsg<ShardRegion.StartEntity>();
        regionProbe.LastSender.Tell(new ShardRegion.StartEntityAck(start3.EntityId,
            start3.EntityId == "1" ? shardId : $"Relocated{start3.EntityId}"));
    ...
    public void
        RememberEntitiesStarter_must_try_start_all_entities_in_a_throttled_way_with_entity_recovery_strategy_constant()
    {
        var regionProbe = CreateTestProbe();
        var shardProbe = CreateTestProbe();
        var shardId = NextShardId();
        ...
        var rememberEntityStarter = Sys.ActorOf(
            RememberEntityStarter
                .Props(regionProbe.Ref, shardProbe.Ref, shardId, ImmutableHashSet.Create("1", "2", "3", "4", "5"),
                    customSettings));
