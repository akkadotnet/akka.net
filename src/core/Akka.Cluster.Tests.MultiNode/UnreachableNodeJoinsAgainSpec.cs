...
public class UnreachableNodeJoinsAgainSpec : MultiNodeClusterSpec
{
    ...
    private int _endBarrierNumber;

    private readonly Lazy<RoleName> _master;
    private readonly Lazy<RoleName> _victim;
    ...
    protected UnreachableNodeJoinsAgainSpec(UnreachableNodeJoinsAgainConfig config) : base(config,
        typeof(UnreachableNodeJoinsAgainSpec))
    {
        _config = config;
    ...
    {
        RunOn(() => { Cluster.Down(GetAddress(_victim.Value)); }, _master.Value);
    ...
    public void AllowFreshNodeWithSameHostAndPortToJoinAgainWhenTheNetworkIsPluggedBackIn()
    {
        ...
        RunOn(() => { Sys.ActorOf(Props.Create(() => new EndActor(TestActor, null)), "end"); }, _master.Value);
        ...
        RunOn(() =>
        {
            ...
            try
            {
                ...
                {
                    AwaitAssert(() =>
                        Assert.Contains(victimAddress, Cluster.Get(freshSystem).State.Members.Select(x => x.Address)));
                    AwaitAssert(() =>
                        Assert.Equal(expectedNumberOfMembers, Cluster.Get(freshSystem).State.Members.Count));
                    AwaitAssert(() =>
                        Assert.True(Cluster.Get(freshSystem).State.Members.All(y => y.Status == MemberStatus.Up)));
                });
        ...
        RunOn(() =>
        {
            ...
            RunOn(() => { ExpectMsg<EndActor.End>(TimeSpan.FromSeconds(20)); }, _master.Value);
            EndBarrier();
