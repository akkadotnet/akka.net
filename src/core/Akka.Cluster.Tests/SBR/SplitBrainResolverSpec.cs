...
public class SplitBrainResolverSpec : AkkaSpec
{
    ...
    private readonly LeaseSettings testLeaseSettings;

    public SplitBrainResolverSpec(ITestOutputHelper output)
        : base(Config, output)
    {
        testLeaseSettings = new LeaseSettings("akka-sbr", "test",
            new TimeoutSettings(TimeSpan.FromSeconds(1), TimeSpan.FromMinutes(2), TimeSpan.FromSeconds(3)),
    ...
    internal Reachability CreateReachability(IEnumerable<(Member, Member)> unreachability)
    {
        return new Reachability(unreachability
                .Select(i => new Reachability.Record(i.Item1.UniqueAddress, i.Item2.UniqueAddress,
                    Reachability.ReachabilityStatus.Unreachable, 1)).ToImmutableList(),
    ...
    [Fact]
    public void
        KeepMajority_must_down_indirectly_connected_on_minority_side_when_combined_with_clean_partition_A__B_C__D_E_F_G___D_E_F_G()
    {
        var setup = new KeepMajoritySetup(this);
        setup.Side1 = ImmutableHashSet.Create(MemberA, MemberB, MemberC);
    ...
    [Fact]
    public void
        KeepMajority_must_down_indirectly_connected_on_majority_side_when_combined_with_clean_partition_A_B_C__D_E__F_G___F_G()
    {
        var setup = new KeepMajoritySetup(this);
        setup.Side1 = ImmutableHashSet.Create(MemberA, MemberB, MemberC);
        setup.Side2 = ImmutableHashSet.Create(MemberD, MemberE, MemberF, MemberG);
    ...
    [Fact]
    public void
        KeepMajority_must_down_indirectly_connected_detected_via_seen_combined_with_clean_partition_A_B_C__D_E__F_G___()
    {
        var setup = new KeepMajoritySetup(this);
        setup.Side1 = ImmutableHashSet.Create(MemberA, MemberB, MemberC);
        setup.Side2 = ImmutableHashSet.Create(MemberD, MemberE, MemberF, MemberG);
    ...
    [Fact]
    public void
        KeepMajority_must_double_DownIndirectlyConnected_when_indirectly_connected_happens_before_clean_partition_A_B_C__D_E__F_G___()
    {
        var setup = new KeepMajoritySetup(this);
        setup.Side1 = ImmutableHashSet.Create(MemberA, MemberB, MemberC);
        setup.Side2 = ImmutableHashSet.Create(MemberD, MemberE, MemberF, MemberG);
    ...
    {
        var setup = new KeepOldestSetup(this, false);
        setup.Side1 = ImmutableHashSet.Create(MemberE);
        setup.Side2 = ImmutableHashSet.Create(MemberA, MemberB, MemberC, MemberD);
        setup.AssertDowning(setup.Side2);
    }
    ...
    [Fact]
    public void
        KeepOldest_must_detect_leaving_exiting_edge_case_DownReachable_on_both_sides_when_oldest_leaving_exiting_is_alone()
    {
        var setup = new KeepOldestSetup(this);
    ...
    [Fact]
    public void
        KeepOldest_must_down_indirectly_connected_on_younger_side_when_combined_with_clean_partition_A__B_C__D_E_F_G___D_E_F_G()
    {
        var setup = new KeepOldestSetup(this);
        setup.Side1 = ImmutableHashSet.Create(MemberA, MemberB, MemberC);
        setup.Side2 = ImmutableHashSet.Create(MemberD, MemberE, MemberF, MemberG);
    ...
    [Fact]
    public void
        KeepOldest_must_down_indirectly_connected_on_oldest_side_when_combined_with_clean_partition_A_B_C__D_E__F_G___F_G()
    {
        var setup = new KeepOldestSetup(this);
        setup.Side1 = ImmutableHashSet.Create(MemberA, MemberB, MemberC);
        setup.Side2 = ImmutableHashSet.Create(MemberD, MemberE, MemberF, MemberG);
    ...
    public void LeaseMajority_must_down_indirectly_connected_A_B__C___C()
    {
        ...
        strategy1.NodesToDown(decision1).Should().BeEquivalentTo(MemberA.UniqueAddress, MemberB.UniqueAddress);
        var reverseDecision1 = strategy1.ReverseDecision((IAcquireLeaseDecision)decision1);
    ...
    public void LeaseMajority_must_down_indirectly_connected_when_combined_with_clean_partition_A__B_C__D_E___A()
    {
        ...
        var decision1 = strategy1.Decide();
        decision1.Should().Be(new AcquireLeaseAndDownIndirectlyConnected(TimeSpan.Zero));
        strategy1.NodesToDown(decision1).Should()
            .BeEquivalentTo(new[] { MemberB, MemberC, MemberD, MemberE }.Select(m => m.UniqueAddress));
        var reverseDecision1 = strategy1.ReverseDecision((IAcquireLeaseDecision)decision1);
        reverseDecision1.Should().BeOfType<ReverseDownIndirectlyConnected>();
        strategy1.NodesToDown(reverseDecision1).Should().BeEquivalentTo(setup.Side1Nodes);
        ...
        strategy3.NodesToDown(reverseDecision3).Should()
            .BeEquivalentTo(new[] { MemberB, MemberC, MemberD, MemberE }.Select(m => m.UniqueAddress));
    }
    ...
    private void TestAddRemove(DowningStrategy strategy)
    {
        ...
        strategy.AllMembers.Count.Should().Be(2);
        strategy.AllMembers.Select(i => i.Status).ToImmutableHashSet().Should()
            .BeEquivalentTo(new[] { MemberStatus.Up, MemberStatus.Leaving });
        strategy.Add(Exiting(MemberB));
        strategy.AllMembers.Count.Should().Be(2);
        strategy.AllMembers.Select(i => i.Status).ToImmutableHashSet().Should()
            .BeEquivalentTo(new[] { MemberStatus.Up, MemberStatus.Exiting });
    ...
    private void TestCollectAndFilter(StrategySetup setup)
    {
        ...
        strategy1.MembersWithRole.Should().BeEquivalentTo(MemberB, MemberD, Leaving(MemberE));
        strategy1.GetMembersWithRole(true, false).Should()
            .BeEquivalentTo(MemberAWeaklyUp, MemberB, Joining(MemberC), MemberD, Leaving(MemberE));
        strategy1.GetMembersWithRole(false, true).Should().BeEquivalentTo(MemberB, MemberD);
        strategy1.GetMembersWithRole(true, true).Should()
            .BeEquivalentTo(MemberAWeaklyUp, MemberB, Joining(MemberC), MemberD);

        strategy1.ReachableMembersWithRole.Should().BeEquivalentTo(MemberB);
        strategy1.GetReachableMembersWithRole(true, false).Should()
            .BeEquivalentTo(MemberAWeaklyUp, MemberB, Joining(MemberC));
        strategy1.GetReachableMembersWithRole(false, true).Should().BeEquivalentTo(MemberB);
        strategy1.GetReachableMembersWithRole(true, true).Should()
            .BeEquivalentTo(MemberAWeaklyUp, MemberB, Joining(MemberC));

        strategy1.UnreachableMembersWithRole.Should().BeEquivalentTo(MemberD, Leaving(MemberE));
        strategy1.GetUnreachableMembers(true, false).Should().BeEquivalentTo(MemberD, Leaving(MemberE));
        strategy1.GetUnreachableMembers(false, true).Should().BeEquivalentTo(MemberD);
        strategy1.GetUnreachableMembers(true, true).Should().BeEquivalentTo(MemberD);
        ...
        strategy1.Joining.Should().BeEquivalentTo(MemberAWeaklyUp, Joining(MemberC));

        var strategy2 = setup.Strategy(setup.Side2);

        strategy2.MembersWithRole.Should().BeEquivalentTo(MemberB, MemberD, Leaving(MemberE));
        strategy2.GetMembersWithRole(true, false).Should()
            .BeEquivalentTo(MemberAWeaklyUp, MemberB, Joining(MemberC), MemberD, Leaving(MemberE));
        strategy2.GetMembersWithRole(false, true).Should().BeEquivalentTo(MemberB, MemberD);
        strategy2.GetMembersWithRole(true, true).Should()
            .BeEquivalentTo(MemberAWeaklyUp, MemberB, Joining(MemberC), MemberD);

        strategy2.UnreachableMembersWithRole.Should().BeEquivalentTo(MemberB);
        strategy2.GetUnreachableMembersWithRole(true, false).Should()
            .BeEquivalentTo(MemberAWeaklyUp, MemberB, Joining(MemberC));
        strategy2.GetUnreachableMembersWithRole(false, true).Should().BeEquivalentTo(MemberB);
        strategy2.GetUnreachableMembersWithRole(true, true).Should()
            .BeEquivalentTo(MemberAWeaklyUp, MemberB, Joining(MemberC));

        strategy2.ReachableMembersWithRole.Should().BeEquivalentTo(MemberD, Leaving(MemberE));
        strategy2.GetReachableMembers(true, false).Should().BeEquivalentTo(MemberD, Leaving(MemberE));
        strategy2.GetReachableMembers(false, true).Should().BeEquivalentTo(MemberD);
        strategy2.GetReachableMembers(true, true).Should().BeEquivalentTo(MemberD);
        ...
        strategy2.Joining.Should().BeEquivalentTo(MemberAWeaklyUp, Joining(MemberC));
    }
    ...
    [Fact]
    public async Task
        Split_Brain_Resolver_must_down_unreachable_when_becoming_leader_inbetween_detection_and_specified_duration()
    {
        var setup = new SetupKeepMajority(this, TimeSpan.FromSeconds(2), MemberA.UniqueAddress, null);
        setup.MemberUp(MemberA, MemberB, MemberC);
        setup.Leader(MemberB);
    ...
    [Fact]
    public async Task
        Split_Brain_Resolver_must_not_down_unreachable_when_loosing_leadership_inbetween_detection_and_specified_duration()
    {
    ...
    [Fact]
    public async Task
        Split_Brain_Resolver_must_not_down_when_unreachable_become_reachable_inbetween_detection_and_specified_duration()
    {
        var setup = new SetupKeepMajority(this, TimeSpan.FromSeconds(1), MemberA.UniqueAddress, null);
        setup.MemberUp(MemberA, MemberB, MemberC);
        setup.Leader(MemberA);
    ...
    [Fact]
    public async Task
        Split_Brain_Resolver_must_not_down_when_unreachable_is_removed_inbetween_detection_and_specified_duration()
    {
        var setup = new SetupKeepMajority(this, TimeSpan.FromSeconds(1), MemberA.UniqueAddress, null);
        setup.MemberUp(MemberA, MemberB, MemberC);
        setup.Leader(MemberA);
    ...
    [Fact]
    public void
        Split_Brain_Resolver_must_down_indirectly_connected_when_combined_with_partition_and_exiting_A_B_C_D__E_Fexiting___A_B_C_D()
    {
        var setup = new SetupKeepMajority(this, TimeSpan.Zero, MemberA.UniqueAddress, null);
    ...
    [Fact]
    public void
        Split_Brain_Resolver_must_down_indirectly_connected_when_combined_with_partition_and_exiting_A_B_C_D__Eexiting_F___A_B_C_D()
    {
        var setup = new SetupKeepMajority(this, TimeSpan.Zero, MemberA.UniqueAddress, null);
        setup.MemberUp(MemberA, MemberB, MemberC, MemberD, MemberE, MemberF);
    ...
    {
        var setup = new SetupKeepMajority(this, TimeSpan.FromSeconds(2), MemberA.UniqueAddress, null,
            TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(100));
        setup.MemberUp(MemberA, MemberB, MemberC, MemberD, MemberE);
        setup.Leader(MemberA);
        setup.ReachabilityChanged(
            (MemberB, MemberD),
    ...
    {
        var setup = new SetupKeepMajority(this, TimeSpan.FromSeconds(2), MemberA.UniqueAddress, null,
            TimeSpan.FromMilliseconds(500), TimeSpan.FromSeconds(100));
        setup.MemberUp(MemberA, MemberB, MemberC, MemberD, MemberE);
        setup.Leader(MemberA);
    ...
    {
        var setup = new SetupKeepMajority(this, TimeSpan.FromSeconds(2), MemberA.UniqueAddress, null,
            TimeSpan.FromMilliseconds(1000), TimeSpan.FromSeconds(100));
        setup.MemberUp(MemberA, MemberB, MemberC, MemberD, MemberE);
        setup.Leader(MemberA);
        setup.ReachabilityChanged(
            (MemberB, MemberD),
    ...
    [Fact(DisplayName =
        "Split Brain Resolver must down correct nodes when Reachability records is polluted and consensus is reached")]
    ...
    [Fact(DisplayName =
        "Split Brain Resolver must down all nodes when Reachability records is polluted and no consensus is reached")]
    ...
    {
        var setup = new SetupLeaseMajority(this, TimeSpan.Zero, MemberA.UniqueAddress, null,
    ...
    {
        private ImmutableHashSet<Address> downed = ImmutableHashSet<Address>.Empty;
    ...
    internal abstract class StrategySetup
    {
        ...
        public DowningStrategy Strategy(ImmutableHashSet<Member> side)
        {
            ...
            if (!side.IsEmpty)
            {
                ...
                var unreachability = IndirectlyConnected.Union(others.Select(o => (side.FirstOrDefault(), o)))
                    .ToImmutableHashSet().ToList();
    ...
    internal class LeaseMajoritySetup : StrategySetup
    {
        ...
        public override DowningStrategy CreateStrategy()
        {
            return new LeaseMajority(Role, TestLease, AcquireLeaseDelayForMinority, TimeSpan.FromSeconds(10));
        }
    }
    ...
    internal class SetupLeaseMajority : Setup
    {
        public SetupLeaseMajority(
            ...
            : base(
                owner,
                stableAfter,
                new LeaseMajority(role, testLease, TimeSpan.FromMilliseconds(20), TimeSpan.FromSeconds(10)),
                selfUniqueAddress,
                downAllWhenUnstable,
                tickInterval)
        {
    ...
    internal abstract class Setup
    {
            ...
            TimeSpan? tickInterval = null)
        {
            ...
            A = owner.Sys.ActorOf(
                DowningTestActor
                    .Props(stableAfter, strategy, owner.TestActor, selfUniqueAddress, DownAllWhenUnstable,
                        TickInterval));
        }
        ...
        {
            Owner.ReceiveN(members.Length).ToImmutableHashSet().Should()
                .BeEquivalentTo(members.Select(m => new DownCalled(m.Address)).ToImmutableHashSet());
        }
    ...
    public Member Joining(Member m)
    {
        return TestAddresses.Joining(m);
    }

    public Member Leaving(Member m)
    {
        return TestAddresses.Leaving(m);
    }

    public Member Exiting(Member m)
    {
        return TestAddresses.Exiting(m);
    }

    public Member Downed(Member m)
    {
        return TestAddresses.Downed(m);
    }
