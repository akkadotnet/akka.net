...
{
    public BackwardsCompatSqliteSnapshotStoreSpec(ITestOutputHelper output) : base(
        CreateSpecConfig(@"Filename=file:AssemblyVersioning/samples/memdb-snapshot-1-v123-altered.db;"),
        "BackwardsCompatSqliteSnapshotStoreSpec", output)
    {
    }
    ...
    public void Can_read_snapshot_from_v123_sqlitedb()
    {
        ...
        snapshotStoreRef.Tell(new LoadSnapshot("p-22", SnapshotSelectionCriteria.Latest, long.MaxValue),
            senderProbe.Ref);
