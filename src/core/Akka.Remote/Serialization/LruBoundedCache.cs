...
{
    public static readonly FastHashComparer Default = new();
...
internal abstract class LruBoundedCache<TKey, TValue> where TValue : class
{
    ...
    public TValue Get(TKey k)
    {
        ...
        while (true)
        {
            ...
            if (probeDistance > otherProbeDistance)
                return null;
            if (_hashes[position] == h && _keyComparer.Equals(k, _keys[position])) return _values[position];
    ...
    public bool TrySet(TKey key, TValue value)
    {
        ...
        while (true)
            ...
            else
            {
                ...
                if (_hashes[position] == h && _keyComparer.Equals(key, _keys[position]))
                {
                    // Update usage
                    _epochs[position] = _epoch;
                    _values[position] = value;
    ...
    {
        return
            $"LruBoundedCache(values = [{string.Join<TValue>(",", _values)}], hashes = [{string.Join(",", _hashes)}], " +
