...
public class EventStreamSpec : AkkaSpec
{
    ...
    {
        var bus = new EventStream(true);
        Assert.Throws<ArgumentNullException>(() => { bus.Subscribe(null, typeof(M)); });
    }
    ...
    {
        var bus = new EventStream(true);
        Assert.Throws<ArgumentNullException>(() => { bus.Unsubscribe(null, typeof(M)); });
    ...
    private void verifyLevel(LoggingBus bus, LogLevel level)
    {
        ...
        {
            new Debug("", null, "debug"), new Info("", null, "info"), new Warning("", null, "warning"),
