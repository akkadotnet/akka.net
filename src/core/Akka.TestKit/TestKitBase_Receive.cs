...
public abstract partial class TestKitBase
{
    ...
    public T FishForMessage<T>(
        ...
        CancellationToken cancellationToken = default)
    {
        return FishForMessageAsync(isMessage, max, hint, cancellationToken).AsTask()
            .GetAwaiter().GetResult();
    }
    ...
    public T FishForMessage<T>(
        ...
        CancellationToken cancellationToken = default)
    {
        return FishForMessageAsync(isMessage, allMessages, max, hint, cancellationToken).AsTask()
            .GetAwaiter().GetResult();
    }
    ...
    public async Task<ArrayList> WaitForRadioSilenceAsync(TimeSpan? max = null, uint? maxMessages = null, CancellationToken cancellationToken = default)
    {
        ...
        for (uint i = 0; ; i++)
        {
            _assertions.AssertFalse(i > maxMessages, $"{nameof(maxMessages)} violated (current iteration: {i}).");
    ...
    {
        return ReceiveOneAsync(max, cancellationToken).AsTask()
            .GetAwaiter().GetResult();
    }
    ...
    public bool TryReceiveOne(
        ...
        CancellationToken cancellationToken = default)
    {
        var (success, messageEnvelope) = TryReceiveOneAsync(max, cancellationToken).AsTask()
            .GetAwaiter().GetResult();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (maxDuration.IsZero())
        {
            ...
            take = (didTake, item ?? NullMessageEnvelope.Instance);
        }
        else if (maxDuration.IsPositiveFinite())
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    take = (false, NullMessageEnvelope.Instance);
                }
            }
            catch (OperationCanceledException) when (cancellationToken is { CanBeCanceled: true, IsCancellationRequested: true })
            {
    ...
    {
        return PeekOneAsync(max, cancellationToken).AsTask()
            .GetAwaiter().GetResult();
    } 
    ...
    {
        return PeekOneAsync(cancellationToken).AsTask()
            .GetAwaiter().GetResult();
    }
    ...
    {
        var (success, result) = InternalTryPeekOneAsync(max, true, cancellationToken).AsTask()
            .GetAwaiter().GetResult();
        envelope = result;
        return success;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (maxDuration.IsZero())
        {
            ...
            peek = (didPeek, item ?? NullMessageEnvelope.Instance);
        }
        else if (maxDuration.IsPositiveFinite())
        {
            ...
            if (completedTask == readTask && readTask.Result)
            {
                ...
                peek = (didTake, item ?? NullMessageEnvelope.Instance);
            }
            else
            {
                // Timeout occurred before data was available.
                peek = (false, NullMessageEnvelope.Instance);
            }
        }
        else if (maxDuration == Timeout.InfiniteTimeSpan)
        {
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (count < msgs)
        {
            ...
            var result = filter.Invoke(message);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < numberOfMessages; i++)
        {
            ...
            if (o == null)
            {
                ...
                _assertions.Fail(failMessage, max, numberOfMessages, i, elapsed);
