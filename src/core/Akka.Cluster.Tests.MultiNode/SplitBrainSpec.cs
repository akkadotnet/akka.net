...
public class SplitBrainWithFailureDetectorPuppetMultiNode : SplitBrainSpec
{
    public SplitBrainWithFailureDetectorPuppetMultiNode() : base(true,
        typeof(SplitBrainWithFailureDetectorPuppetMultiNode))
    {
    }
}
...
public class SplitBrainWithAccrualFailureDetectorMultiNode : SplitBrainSpec
{
    public SplitBrainWithAccrualFailureDetectorMultiNode() : base(false,
        typeof(SplitBrainWithAccrualFailureDetectorMultiNode))
    {
    }
}
...
public abstract class SplitBrainSpec : MultiNodeClusterSpec
{
    ...
    private readonly List<RoleName> side1;
    private readonly List<RoleName> side2;

    protected SplitBrainSpec(bool failureDetectorPuppet, Type type) : this(new SplitBrainConfig(failureDetectorPuppet),
        type)
    {
    }
        ...
        Cluster_of_5_members_must_detect_network_partition_and_mark_nodes_on_other_side_as_unreachable_and_form_new_cluster()
    {
        ...
        RunOn(() =>
        {
            foreach (var role in side2) MarkNodeAsUnavailable(GetAddress(role));
        ...
        RunOn(() =>
        {
            foreach (var role in side1) MarkNodeAsUnavailable(GetAddress(role));
