...
public class UnfoldResourceAsyncSourceSpec : AkkaSpec
{
    ...
    public async Task A_UnfoldResourceAsyncSource_must_fail_when_create_returns_failed_future()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            Source.UnfoldResourceAsync<Task, Task>(
                    () => Task.FromException<Task>(testException),
                    _ => default,
                    _ => default)
                .RunWith(Sink.FromSubscriber(probe), Materializer);
    ...
    [Fact]
    public async Task
        A_UnfoldResourceAsyncSource_must_close_and_open_stream_again_when_strategy_is_restart_and_read_throws()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var failed = false;
    ...
    [Fact]
    public async Task
        A_UnfoldResourceAsyncSource_must_close_and_open_stream_again_when_strategy_is_restart_and_read_returns_failed_future()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var failed = false;
    ...
    public async Task A_UnfoldResourceAsyncSource_must_close_resource_when_stream_is_abruptly_terminated()
    {
        ...
        var p = Source.UnfoldResourceAsync(
                ...
                _ => FutureTimeoutSupport.After(TimeSpan.FromMilliseconds(100), Sys.Scheduler,
                    () => Task.FromResult(Option<string>.Create("element"))),
                tcs =>
                {
    ...
    public async Task A_UnfoldResourceAsyncSource_close_the_resource_when_reading_an_element_throws()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            Task<Option<Task>> Fail(TaskCompletionSource<string> _)
            {
                throw new TestException("read failed");
            }
    ...
    private class ResourceDummy<T>
    {
        ...
        public ResourceDummy(IEnumerable<T> values, Task<Done> createFuture = default,
            Task<Done> firstReadFuture = default, Task<Done> closeFuture = default)
        {
