...
public static class RetrySupport
{
    ...
    {
        return Retry(attempt, attempts, 0);
    }
    ...
    public static Task<T> Retry<T>(Func<Task<T>> attempt, int attempts, TimeSpan minBackoff, TimeSpan maxBackoff,
        int randomFactor, IScheduler scheduler)
    {
        ...
        return Retry(attempt, attempts,
            attempted => BackoffSupervisor.CalculateDelay(attempted, minBackoff, maxBackoff, randomFactor), scheduler);
    }
    ...
    {
        return Retry(attempt, attempts, _ => delay, scheduler);
    }
    ...
    ///         this function should never return `null`, otherwise an <see cref="InvalidOperationException" /> will be
    ///         through.
    ///     </para>
    ///     If attempts are exhausted the returned Task is simply the result of invoking attempt.
    /// </summary>
    ...
    /// <param name="scheduler">The scheduler instance to use.</param>
    public static Task<T> Retry<T>(Func<Task<T>> attempt, int attempts, Func<int, Option<TimeSpan>> delayFunction,
        IScheduler scheduler)
    {
        return Retry(attempt, attempts, delayFunction, 0, scheduler);
    }

    private static Task<T> Retry<T>(Func<Task<T>> attempt, int maxAttempts, int attempted)
    {
        return Retry(attempt, maxAttempts, _ => Option<TimeSpan>.None, attempted);
    }

    private static Task<T> Retry<T>(Func<Task<T>> attempt, int maxAttempts, Func<int, Option<TimeSpan>> delayFunction,
        int attempted, IScheduler scheduler = null)
    {
        Task<T> tryAttempt()
        {
            try
