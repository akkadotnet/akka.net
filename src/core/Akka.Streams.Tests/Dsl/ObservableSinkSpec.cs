...
public class ObservableSinkSpec : AkkaSpec
{
    ...
    private sealed class TestObserver<T> : IObserver<T>
    {
        ...
        {
            _probe.Ref.Tell(new OnNext(value), ActorRefs.NoSender);
        }

        public void OnError(Exception error)
        {
            _probe.Ref.Tell(new OnError(error), ActorRefs.NoSender);
        }

        public void OnCompleted()
        {
            _probe.Ref.Tell(OnComplete.Instance, ActorRefs.NoSender);
        }

        public async Task<T> ExpectEventAsync(T expected)
        {
            return (T)(await _probe.ExpectMsgAsync<OnNext>(x => Equals(x.Element, expected))).Element;
        }

        public async Task<TError> ExpectErrorAsync<TError>(TError error) where TError : Exception
        {
            return (TError)(await _probe.ExpectMsgAsync<OnError>(x => Equals(x.Cause, error))).Cause;
        }

        public async Task ExpectCompletedAsync()
        {
            await _probe.ExpectMsgAsync<OnComplete>();
        }

        public async Task ExpectNoMsgAsync()
        {
            await _probe.ExpectNoMsgAsync();
        }
    }
