...
internal sealed class FileSink : SinkModule<ByteString, Task<IOResult>>
{
    ...
    public FileSink(FileInfo f, long startPosition, FileMode fileMode, Attributes attributes,
        SinkShape<ByteString> shape, bool autoFlush, FlushSignaler flushSignaler) : base(shape)
    {
        _f = f;
    ...
    {
        return new FileSink(_f, _startPosition, _fileMode, attributes, AmendShape(attributes), _autoFlush,
            _flushSignaler);
    }
    ...
    {
        return new FileSink(_f, _startPosition, _fileMode, Attributes, shape, _autoFlush, _flushSignaler);
    }
...
internal sealed class OutputStreamSink : SinkModule<ByteString, Task<IOResult>>
{
    ...
    public OutputStreamSink(Func<Stream> createOutput, Attributes attributes, SinkShape<ByteString> shape,
        bool autoFlush) : base(shape)
    {
    ...
    protected override SinkModule<ByteString, Task<IOResult>> NewInstance(SinkShape<ByteString> shape)
    {
        return new OutputStreamSink(_createOutput, Attributes, shape, _autoFlush);
    }
