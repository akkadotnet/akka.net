...
internal abstract class TcpConnection : ActorBase, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
{
    ...
    private bool IsWritePending
    {
        ...
        get => !HasStatus(ConnectionStatus.Sending) && !_writeCommandsQueue.IsEmpty;
    }

    private Option<PendingWrite> GetAllowedPendingWrite()
    {
        return IsWritePending ? GetNextWrite() : Option<PendingWrite>.None;
    }
        ...
        return message =>
        {
            switch (message)
            {
                ...
                case CloseCommand cmd:
                    var info = new ConnectionInfo(commander, false, false);
                ...
                case WriteCommand write:
                    ...
                    else
                    {
                        ...
                        if (_writeCommandsQueue.IsEmpty)
                        {
                            ...
                            if (!_writeCommandsQueue.EnqueueSimpleWritesExceptFirst(write, Sender,
                                    out var simpleWriteCommand))
                            {
                                DropWrite(info, write);
                                return true;
                            }

                            nextWrite = GetNextWrite(new[] { (simpleWriteCommand, Sender) });
                        }
                        else
                        {
    ...
    protected void CompleteConnect(IActorRef commander, IEnumerable<Inet.SocketOption> options)
    {
        ...
        foreach (var option in options) option.AfterConnect(Socket);
    ...
    {
        while (_pendingAcks.TryDequeue(out var ackInfo)) ackInfo.Commander.Tell(ackInfo.Ack);
    ...
    protected void AcquireSocketAsyncEventArgs()
    {
        if (ReceiveArgs != null)
            throw new InvalidOperationException(
                "Cannot acquire receive SocketAsyncEventArgs. It's already has been initialized");
        if (SendArgs != null)
            throw new InvalidOperationException(
                "Cannot acquire send SocketAsyncEventArgs. It's already has been initialized");
    ...
    protected override void PostStop()
    {
        ...
        if (_closedMessage != null)
        {
            ...
            foreach (var listener in interestedInClose) listener.Tell(_closedMessage.ClosedEvent);
        }
    }
    ...
    private Option<PendingWrite> GetNextWrite(
        IEnumerable<(SimpleWriteCommand Command, IActorRef Sender)> headCommands = null)
    {
        ...
        if (writeCommands.Count > 0) return CreatePendingBufferWrite(writeCommands);
            ...
            foreach (var writeInfo in ExtractFromCommand(command))
            {
                ...
                if (_maxQueueSizeInBytes.HasValue && _maxQueueSizeInBytes.Value < sizeAfterAppending)
                {
                    _log.Warning("Could not receive write command of size {0} bytes, " +
                                 ...
                                 "it is already {2} bytes", writeInfo.DataSize, _maxQueueSizeInBytes,
                        _totalSizeInBytes);
                    return false;
                }
            ...
            switch (command)
            {
                ...
                default:
                    throw new ArgumentException(
                        $"Trying to calculate size of unknown write type: {command.GetType().FullName}");
            }
        }
    }
}