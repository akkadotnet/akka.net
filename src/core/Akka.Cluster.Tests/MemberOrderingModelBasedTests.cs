...
public class MembershipMachine : Machine<MemberOrderingState, MembershipModel>
{
    ...
    public override Arbitrary<Setup<MemberOrderingState, MembershipModel>> Setup => Arb.From(Arb
        .Generate<MembershipSetup>()
        ...
        public override Property Check(MemberOrderingState actual, MembershipModel model)
        {
            ...
            return
                mergedMembers.SetEquals(model.AllMembers.Values)
                    ...
                    .Label(
                        $"Expected Member [{TargetedMember}] to have status [{NewStatus}] but was [{mergedMembers.Single(x => x.Address.Equals(TargetedMember)).Status}]");
        }
        ...
        public override Property Check(MemberOrderingState actual, MembershipModel model)
        {
            ...
            return
                actual.Members.SetEquals(model.AllMembers.Values)
                    .Label(
                        $"Both sets should contain same set of members. Instead found members not included in both sequences: [{string.Join(",", except)}]");
        }
