...
public abstract class RememberEntitiesShardStoreSpec : AkkaSpec
{
    ...
    private void The_store_must_store_starts_and_stops_and_list_remembered_entity_ids()
    {
        ...
        store.Tell(new RememberEntitiesShardStore.Update(ImmutableHashSet.Create("1", "2", "3"),
            ImmutableHashSet<string>.Empty));
        ExpectMsg(new RememberEntitiesShardStore.UpdateDone(ImmutableHashSet.Create("1", "2", "3"),
            ImmutableHashSet<string>.Empty));

        store.Tell(new RememberEntitiesShardStore.Update(ImmutableHashSet.Create("4", "5", "6"),
            ImmutableHashSet.Create("2", "3")));
        ExpectMsg(new RememberEntitiesShardStore.UpdateDone(ImmutableHashSet.Create("4", "5", "6"),
            ImmutableHashSet.Create("2", "3")));

        store.Tell(new RememberEntitiesShardStore.Update(ImmutableHashSet<string>.Empty, ImmutableHashSet.Create("6")));
        ExpectMsg(new RememberEntitiesShardStore.UpdateDone(ImmutableHashSet<string>.Empty,
            ImmutableHashSet.Create("6")));

        store.Tell(new RememberEntitiesShardStore.Update(ImmutableHashSet.Create("2"), ImmutableHashSet<string>.Empty));
        ExpectMsg(new RememberEntitiesShardStore.UpdateDone(ImmutableHashSet.Create("2"),
            ImmutableHashSet<string>.Empty));
    ...
    private void The_store_must_handle_a_late_request()
    {
        ...
        ExpectMsg<RememberEntitiesShardStore.RememberedEntities>().Entities.Should()
            .BeEquivalentTo("1", "2", "4", "5"); // from previous test
    }
    ...
    private void The_store_must_handle_a_large_batch()
    {
        ...
        store.Tell(new RememberEntitiesShardStore.Update(
            Enumerable.Range(1, 1000).Select(i => i.ToString()).ToImmutableHashSet(),
            Enumerable.Range(1001, 1000).Select(i => i.ToString()).ToImmutableHashSet()));
...
public class DDataRememberEntitiesShardStoreSpec : RememberEntitiesShardStoreSpec
{
    ...
    {
        return DDataRememberEntitiesShardStore.Props(shardId, typeName, settings, replicator, 1);
    }
}
