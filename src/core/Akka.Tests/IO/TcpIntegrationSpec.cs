...
public class TcpIntegrationSpec : AkkaSpec
{
    ...
    public TcpIntegrationSpec(ITestOutputHelper output)
        : base($@"akka.loglevel = DEBUG
                     ...
                     akka.io.tcp.write-commands-queue-max-size = {InternalConnectionActorMaxQueueSize}", output)
    {
    }
    ...
    public async Task
        The_TCP_transport_implementation_should_allow_connecting_to_and_disconnecting_from_the_test_server()
    {
    ...
    [Fact(Skip = "FIXME .net core / linux")]
    public async Task
        The_TCP_transport_implementation_should_properly_handle_connection_abort_from_client_side_after_chit_chat()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            var actors = await x.EstablishNewClientConnectionAsync();
    ...
    [Fact]
    public async Task
        The_TCP_transport_implementation_should_properly_handle_connection_abort_via_PoisonPill_from_client_side()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            var actors = await x.EstablishNewClientConnectionAsync();
    ...
    [Fact]
    public async Task
        The_TCP_transport_implementation_should_properly_handle_connection_abort_via_PoisonPill_from_client_side_after_chit_chat()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            var actors = await x.EstablishNewClientConnectionAsync();
    ...
    [Fact]
    public async Task
        The_TCP_transport_implementation_should_properly_handle_connection_abort_via_PoisonPill_from_server_side()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            var actors = await x.EstablishNewClientConnectionAsync();
    ...
    [Fact]
    public async Task
        The_TCP_transport_implementation_should_properly_handle_connection_abort_via_PoisonPill_from_server_side_after_chit_chat()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            var actors = await x.EstablishNewClientConnectionAsync();
        ...
        AddressFamily family)
    {
        ...
        bindCommander.Send(Sys.Tcp(), new Tcp.Bind(serverHandler.Ref, new IPEndPoint(
        ...
        var received = await serverHandler
            .ReceiveWhileAsync(o => { return o as Tcp.Received; }, RemainingOrDefault, TimeSpan.FromSeconds(0.5))
            .ToListAsync();
    ...
    public async Task BugFix_3021_Tcp_Should_not_drop_large_messages()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            ...
            var serverMsgs = await actors.ServerHandler
                .ReceiveWhileAsync(o => { return o as Tcp.Received; }, RemainingOrDefault, TimeSpan.FromSeconds(2))
                .ToListAsync();
    ...
    public async Task Write_before_Register_should_not_be_silently_dropped()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            var actors = await x.EstablishNewClientConnectionAsync(false);
    ...
    public async Task Write_before_Register_should_Be_dropped_if_buffer_is_full()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            ...
            var serverMsgs = await actors.ServerHandler
                .ReceiveWhileAsync(o => o as Tcp.Received, RemainingOrDefault, TimeSpan.FromSeconds(2)).ToListAsync();
    ...
    public async Task When_multiple_concurrent_writing_clients_Should_not_lose_messages()
    {
        ...
        await new TestSetup(this).RunAsync(async x =>
        {
            ...
            var clients = Enumerable.Range(0, clientsCount)
                .Select(i => (Index: i, Probe: CreateTestProbe($"test-client-{i}"))).ToArray();
            ...
            var received = await actors.ServerHandler
                .ReceiveWhileAsync(o => o as Tcp.Received, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(1.5))
                .ToListAsync();
    ...
    public async Task When_multiple_concurrent_writing_clients_All_acks_should_be_received()
    {
        ...
        await new TestSetup(this).RunAsync(async x =>
        {
            ...
            foreach (var client in clients)
                await client.Probe.ExpectMsgAsync<AckWithValue>(ack => ack.Value.ShouldBe(client.Index),
    ...
    public async Task Should_fail_writing_when_buffer_is_filled()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            ...
            await AwaitAssertAsync(async () =>
            {
                ...
                actors.ClientHandler.Send(actors.ClientConnection,
                    Tcp.Write.Create(overflowData)); // this is sent immidiately
                actors.ClientHandler.Send(actors.ClientConnection,
                    Tcp.Write.Create(overflowData)); // this will try to buffer
                ...
                (await actors.ServerHandler.ReceiveWhileAsync(TimeSpan.FromSeconds(1), m => m as Tcp.Received)
                        .ToListAsync())
                    .Sum(m => m.Data.Count)
                ...
                actors.ClientHandler.Send(actors.ClientConnection,
                    Tcp.ResumeWriting.Instance); // Recover after send failure
    ...
    [Fact]
    public async Task
        The_TCP_transport_implementation_should_properly_complete_one_client_server_request_response_cycle()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            var actors = await x.EstablishNewClientConnectionAsync();
            ...
            actors.ClientHandler.Send(actors.ClientConnection,
                Tcp.Write.Create(ByteString.FromString("Captain on the bridge!"), Aye.Instance));
            await actors.ClientHandler.ExpectMsgAsync(Aye.Instance);
            (await actors.ServerHandler.ExpectMsgAsync<Tcp.Received>()).Data.ToString(Encoding.ASCII)
                .ShouldBe("Captain on the bridge!");

            actors.ServerHandler.Send(actors.ServerConnection,
                Tcp.Write.Create(ByteString.FromString("For the king!"), Yes.Instance));
            await actors.ServerHandler.ExpectMsgAsync(Yes.Instance);
            (await actors.ClientHandler.ExpectMsgAsync<Tcp.Received>()).Data.ToString(Encoding.ASCII)
                .ShouldBe("For the king!");
    ...
    public async Task The_TCP_transport_implementation_should_support_waiting_for_writes_with_backpressure()
    {
        await new TestSetup(this).RunAsync(async x =>
        {
            ...
            actors.ServerHandler.Send(actors.ServerConnection,
                Tcp.Write.Create(ByteString.FromBytes(new byte[100000]), Ack.Instance));
    ...
    public async Task The_TCP_transport_implementation_dont_report_Connected_when_endpoint_isnt_responding()
    {
        ...
        var replies = await connectCommander.ReceiveWhileAsync(TimeSpan.FromSeconds(1), x => x as Tcp.Connected)
            .ToListAsync();
    ...
    {
        await new TestSetup(this, false).RunAsync(async _ =>
        {
    ...
    private async Task ChitChat(TestSetup.ConnectionDetail actors, int rounds = 100)
    {
        ...
        {
            actors.ClientHandler.Send(actors.ClientConnection, Tcp.Write.Create(testData));
            await actors.ServerHandler.ExpectMsgAsync<Tcp.Received>(x => x.Data.Count == 1 && x.Data[0] == 0,
                hint: $"server didn't received at {i} round");
            actors.ServerHandler.Send(actors.ServerConnection, Tcp.Write.Create(testData));
            await actors.ClientHandler.ExpectMsgAsync<Tcp.Received>(x => x.Data.Count == 1 && x.Data[0] == 0,
                hint: $"client didn't received at {i} round");
        }
    }

    private class Aye : Tcp.Event
    {
        public static readonly Aye Instance = new();
    }

    private class Yes : Tcp.Event
    {
        public static readonly Yes Instance = new();
    }

    private class Ack : Tcp.Event
    {
        public static readonly Ack Instance = new();
    }

    private class AckWithValue : Tcp.Event
    {
        private AckWithValue(object value)
        {
            Value = value;
        }
        ...
        public static AckWithValue Create(object value)
        {
            return new AckWithValue(value);
        }
    }
    ...
    private class TestSetup
    {
        ...
        public async Task BindServer()
        {
            ...
            bindCommander.Send(_spec.Sys.Tcp(),
                new Tcp.Bind(_bindHandler.Ref, new IPEndPoint(IPAddress.Loopback, 0), options: BindOptions));
