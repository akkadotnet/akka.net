...
internal sealed class FanoutPublisherSink<TIn, TStreamBuffer> : SinkModule<TIn, IPublisher<TIn>>
    where TStreamBuffer : IStreamBuffer<TIn>
{
    ...
    public override IModule WithAttributes(Attributes attributes)
    {
        return new FanoutPublisherSink<TIn, TStreamBuffer>(attributes, AmendShape(attributes), _onTerminated);
    }
    ...
    protected override SinkModule<TIn, IPublisher<TIn>> NewInstance(SinkShape<TIn> shape)
    {
        return new FanoutPublisherSink<TIn, TStreamBuffer>(Attributes, shape, _onTerminated);
    }
    ...
    public override object Create(MaterializationContext context, out IPublisher<TIn> materializer)
    {
        ...
        var impl = actorMaterializer.ActorOf(context,
            FanoutProcessorImpl<TIn, TStreamBuffer>.Props(settings, _onTerminated));
...
public sealed class SubscriberSink<TIn> : SinkModule<TIn, NotUsed>
{
    ...
    {
        return new SubscriberSink<TIn>(_subscriber, Attributes, shape);
    }
...
public sealed class ActorSubscriberSink<TIn> : SinkModule<TIn, IActorRef>
{
    ...
    {
        return new ActorSubscriberSink<TIn>(_props, Attributes, shape);
    }
...
public sealed class QueueSink<T> : GraphStageWithMaterializedValue<SinkShape<T>, ISinkQueue<T>>
{
    ...
    private sealed class Logic : GraphStageLogicWithCallbackWrapper<TaskCompletionSource<Option<T>>>, IInHandler
    {
        ...
        {
            EnqueueAndNotify(new Result<Option<T>>(Option<T>.None));
        }

        public void OnUpstreamFailure(Exception e)
        {
            EnqueueAndNotify(new Result<Option<T>>(e));
        }
...
internal sealed class LazySink<TIn, TMat> : GraphStageWithMaterializedValue<SinkShape<TIn>, Task<Option<TMat>>>
{
    ...
    private sealed class Logic : InGraphStageLogic
    {
        ...
        public Logic(LazySink<TIn, TMat> stage, Attributes inheritedAttributes,
            TaskCompletionSource<Option<TMat>> completion)
            : base(stage.Shape)
        {
            _stage = stage;
...
internal sealed class ObservableSinkStage<T> : GraphStageWithMaterializedValue<SinkShape<T>, IObservable<T>>
{
    ...
    public override ILogicAndMaterializedValue<IObservable<T>> CreateLogicAndMaterializedValue(
        Attributes inheritedAttributes)
    {
    ...
    {
        private readonly AtomicBoolean _disposed = new();
    ...
    private sealed class ObservableLogic : GraphStageLogic, IObservable<T>
    {
        ...
        private ImmutableDictionary<IObserver<T>, ObserverDisposable> _observers =
            ImmutableDictionary<IObserver<T>, ObserverDisposable>.Empty;
        ...
        public ObservableLogic(ObservableSinkStage<T> stage) : base(stage.Shape)
        {
            ...
            SetHandler(stage.Inlet,
                ...
                () =>
                {
                    var old = Interlocked.Exchange(ref _observers,
                        ImmutableDictionary<IObserver<T>, ObserverDisposable>.Empty);
                    foreach (var disposer in old.Values) disposer.Dispose(false);
                },
                e =>
                {
        ...
        {
            return ImmutableInterlocked.GetOrAdd(ref _observers, observer, new ObserverDisposable(this, observer));
        }
