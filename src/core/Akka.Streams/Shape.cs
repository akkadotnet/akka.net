...
public abstract class Inlet : InPort
{
    ...
    {
        Name = name ?? throw new ArgumentException("Inlet name must be defined");
    }
...
public abstract class Shape : ICloneable
{
    ...
    /// <summary>
    ///     Compare this to another shape and determine whether the arrangement of ports is the same (including their
    ///     ordering).
    /// </summary>
    /// <param name="shape">TBD</param>
    /// <returns>TBD</returns>
...
///     outputs. Only such a <see cref="IGraph{TShape,TMaterializer}" /> can be materialized by a
///     <see cref="IMaterializer" />.
/// </summary>
...
public class AmorphousShape : Shape
{
    ...
    public override Shape DeepCopy()
    {
        return new AmorphousShape(Inlets.Select(i => i.CarbonCopy()).ToImmutableArray(),
...
public sealed class SourceShape<TOut> : Shape
{
    ...
    /// <inheritdoc />
    private bool Equals(SourceShape<TOut> other)
    {
        return Outlet.Equals(other.Outlet);
    }
...
public sealed class FlowShape<TIn, TOut> : Shape, IFlowShape
{
    ...
    ///     This exception is thrown when either the specified <paramref name="inlet" /> or <paramref name="outlet" /> is
    ///     undefined.
    /// </exception>
...
public sealed class SinkShape<TIn> : Shape
{
    ...
    private bool Equals(SinkShape<TIn> other)
    {
        return Equals(Inlet, other.Inlet);
    }


    public override int GetHashCode()
    {
        return Inlet.GetHashCode();
    }
}
...
public sealed class BidiShape<TIn1, TOut1, TIn2, TOut2> : Shape
{
    ...
    {
        if (inlets.Length != 2)
            throw new ArgumentException($"Proposed inlets [{string.Join(", ", inlets)}] don't fit BidiShape");
        if (outlets.Length != 2)
            throw new ArgumentException($"Proposed outlets [{string.Join(", ", outlets)}] don't fit BidiShape");

        return new BidiShape<TIn1, TOut1, TIn2, TOut2>((Inlet<TIn1>)inlets[0], (Outlet<TOut1>)outlets[0],
            (Inlet<TIn2>)inlets[1], (Outlet<TOut2>)outlets[1]);
    }
...
public static class BidiShape
{
    ...
    {
        return new BidiShape<TIn1, TOut1, TIn2, TOut2>(top.Inlet, top.Outlet, bottom.Inlet, bottom.Outlet);
    }
}