...
internal class ClusterRemoteWatcher : RemoteWatcher
{
    ...
    public ClusterRemoteWatcher(
        ...
        TimeSpan heartbeatExpectedResponseAfter) : base(failureDetector, heartbeatInterval, unreachableReaperInterval,
    ...
    {
        return new Props(typeof(ClusterRemoteWatcher),
                new object[]
                {
                    failureDetector, heartbeatInterval, unreachableReaperInterval, heartbeatExpectedResponseAfter
                })
    ...
    private void MemberRemoved(Member member, MemberStatus previousStatus)
    {
        if (!member.Address.Equals(_cluster.SelfAddress))
        {
            ...
            if (previousStatus == MemberStatus.Down) Quarantine(member.Address, member.UniqueAddress.Uid);
