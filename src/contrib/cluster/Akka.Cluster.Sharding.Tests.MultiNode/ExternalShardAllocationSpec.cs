...
public class ExternalShardAllocationSpec : MultiNodeClusterShardingSpec<ExternalShardAllocationSpecConfig>
{
    ...
    private void External_shard_allocation_must_allocate_to_a_node_that_does_not_exist_yet()
    {
        ...
        RunOn(() =>
        {
            ...
            ExternalShardAllocation.Get(Sys).ClientFor(TypeName)
                .UpdateShardLocations(ImmutableDictionary<string, Address>.Empty.Add(onForthShardId, forthAddress));
        }, Config.Second);
        EnterBarrier("allocated-to-new-node");
        RunOn(() => { JoinWithin(Config.First, TimeSpan.FromSeconds(10)); }, Config.Forth);
    ...
    private void External_shard_allocation_must_move_allocation()
    {
        RunOn(() =>
        {
            ...
            ExternalShardAllocation.Get(Sys).ClientFor(TypeName)
                .UpdateShardLocation(InitiallyOnForth, GetAddress(Config.First));
        }, Config.Third);
