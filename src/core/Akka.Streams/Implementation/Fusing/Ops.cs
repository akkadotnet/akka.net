    ...
    private sealed class Logic : SupervisedGraphStageLogic, IInHandler, IOutHandler
    {
        ...
        public void OnPush()
        {
            ...
            if (result.HasValue)
            {
                ...
                {
                    Push(_stage.Outlet, element);
                    SetHandler(_stage.Inlet, () => Push(_stage.Outlet, Grab(_stage.Inlet)));
                }
            }
        }
...
public abstract class SupervisedGraphStageLogic : GraphStageLogic
{
    ...
    protected virtual void OnRestart(Exception ex)
    {
        OnResume(ex);
    }
    ...
    /// </summary>
    /// <param name="ex">TBD</param>
    protected virtual void OnStop(Exception ex)
    {
        FailStage(ex);
    }
}
...
public sealed class Collect<TIn, TOut> : GraphStage<FlowShape<TIn, TOut>>
{
    ...
    private readonly Func<TIn, bool> _isDefinedFunc;
...
///     While similar to <see cref="Recover{T}" /> this stage can be used to transform an error signal to a different one
///     without logging
...
public sealed class Scan<TIn, TOut> : GraphStage<FlowShape<TIn, TOut>>
{
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        {
            Push(_stage.Out, _stageAggregate);
            SetHandler(_stage.Out, () => Pull(_stage.In));
            SetHandler(_stage.In, _rest);
        }
    }
...
public sealed class ScanAsync<TIn, TOut> : GraphStage<FlowShape<TIn, TOut>>
{
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        public Logic(ScanAsync<TIn, TOut> stage, Attributes inheritedAttributes) : base(stage.Shape)
        {
            ...
            SetHandler(_stage.In, () => { }, () => SetHandler(_stage.Out, () =>
            {
                Push(_stage.Out, _current);
                CompleteStage();
            }));
        ...
        public override string ToString()
        {
            return $"ScanAsync.Logic(completed={_evemtualCurrent.IsCompleted}";
        }
    }
...
public sealed class AggregateAsync<TIn, TOut> : GraphStage<FlowShape<TIn, TOut>>
{
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        public override string ToString()
        {
            return $"AggregateAsync.Logic(completed={_aggregating.IsCompleted})";
        }
    }
...
public sealed class Buffer<T> : SimpleLinearGraphStage<T>
{
    ...
    ///     This exception is thrown when the specified <paramref name="overflowStrategy" />  has an unknown
    ///     <see cref="OverflowStrategy" />.
    /// </exception>
...
public sealed class SelectAsync<TIn, TOut> : GraphStage<FlowShape<TIn, TOut>>
{
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        public override void PreStart()
        {
            _buffer = Buffer.Create<Holder<TOut>>(_stage._parallelism, Materializer);
        }
        ...
        private void HolderCompleted(Holder<TOut> holder)
        {
            ...
            Log.Error(exception,
                "An exception occured inside SelectAsync while executing Task. Supervision strategy: {0}", strategy);
            switch (strategy)
            {
                case Directive.Stop:
                    FailStage(exception);
        ...
        public override string ToString()
        {
            return $"SelectAsync.Logic(buffer={_buffer})";
        }
...
public sealed class SelectAsyncUnordered<TIn, TOut> : GraphStage<FlowShape<TIn, TOut>>
{
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        public override void PreStart()
        {
            _buffer = Buffer.Create<TOut>(_stage._parallelism, Materializer);
        }

        public override string ToString()
        {
            return $"SelectAsyncUnordered.Logic(InFlight={_inFlight}, buffer= {_buffer}";
        }
    }
...
public sealed class Log<T> : SimpleLinearGraphStage<T>
{
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        protected override object LogSource => Event.LogSource.Create(_stage._name);
...
public sealed class Delay<T> : SimpleLinearGraphStage<T>
{
    ...
    private sealed class Logic : TimerGraphStageLogic, IInHandler, IOutHandler
    {
        ...
        private long NextElementWaitTime => (long)_stage._delay.TotalMilliseconds -
                                            (DateTime.UtcNow.Ticks - _buffer.Peek().Item1) * 1000 * 10;
        ...
        public override void PreStart()
        {
            _buffer = Buffer.Create<(long, T)>(_size, Materializer);
        }
        ...
        private Action OnPushStrategy(DelayOverflowStrategy strategy)
        {
            switch (strategy)
            {
                ...
                case DelayOverflowStrategy.Fail:
                    ...
                    {
                        FailStage(new BufferOverflowException(
                            $"Buffer overflow for Delay combinator (max capacity was: {_size})!"));
                    };
                default:
                    return () =>
                    {
                        throw new IllegalStateException($"Delay buffer must never overflow in {strategy} mode");
                    };
            }
        }
    }
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        private void SetInitialInHandler()
        {
            SetHandler(_stage.Inlet, () =>
            {
                ...
                Pull(_stage.Inlet);
                SetHandler(_stage.Inlet, this);
            }, () => FailStage(new NoSuchElementException("Sum over empty stream")));
        }
        ...
        public override string ToString()
        {
            return $"Sum.Logic(aggregator={_aggregator}";
        }
    }
...
public sealed class RecoverWith<TOut, TMat> : SimpleLinearGraphStage<TOut>
{
    ...
    /// <exception cref="ArgumentException">
    ///     This exception is thrown when the specified <paramref name="maximumRetries" /> is less than zero or not equal to
    ///     -1.
    /// </exception>
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        private void SwitchTo(IGraph<SourceShape<TOut>, TMat> source)
        {
            ...
            var outHandler = new LambdaOutHandler(() =>
            {
                if (sinkIn.IsAvailable)
                    PushOut();
            }, cause => sinkIn.Cancel(cause));
...
public sealed class AsyncEnumerable<T> : GraphStage<SourceShape<T>>
{
    ...
    protected override GraphStageLogic CreateLogic(Attributes inheritedAttributes)
    {
        return new Logic(Shape, _factory());
    }
    ...
    private sealed class Logic : OutGraphStageLogic
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void OnSuccess(T element)
        {
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        public Logic(StatefulSelectMany<TIn, TOut> stage, Attributes inheritedAttributes) : base(stage.Shape)
        {
            ...
            _decider = inheritedAttributes
                .GetAttribute(new ActorAttributes.SupervisionStrategy(Deciders.StoppingDecider)).Decider;
...
[InternalApi]
public sealed class
    LazyFlow<TIn, TOut, TMat> : GraphStageWithMaterializedValue<FlowShape<TIn, TOut>, Task<Option<TMat>>>
{
    ...
    /// <param name="inheritedAttributes">TBD</param>
    /// <returns>TBD</returns>
    public override ILogicAndMaterializedValue<Task<Option<TMat>>> CreateLogicAndMaterializedValue(
