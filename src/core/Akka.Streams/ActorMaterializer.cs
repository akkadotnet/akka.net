...
internal sealed class DefaultMaterializer : IExtension
{
    public DefaultMaterializer(ActorSystem system)
    {
        ...
        Materializer = new ActorMaterializerImpl(
            ...
            settings,
            system.Dispatchers,
            system.ActorOf(StreamSupervisor.Props(settings, haveShutDown).WithDispatcher(settings.Dispatcher),
                StreamSupervisor.NextName()),
            haveShutDown,
            EnumerableActorName.Create("Flow"));
    }
...
{
    private static readonly Config DefaultMaterializerConfig =
    ...
    {
        return DefaultMaterializerConfig;
    }
    ...
    ///     This exception is thrown when the specified <paramref name="context" /> is not of type <see cref="ActorSystem" />
    ///     or <see cref="IActorContext" />.
    /// </exception>
    /// <exception cref="ArgumentNullException">
    ///     This exception is thrown when the specified <paramref name="context" /> is undefined.
    /// </exception>
    /// <returns>TBD</returns>
    public static ActorMaterializer Create(IActorRefFactory context, ActorMaterializerSettings settings = null,
        string namePrefix = null)
    {
        ...
        return new ActorMaterializerImpl(
            ...
            settings,
            system.Dispatchers,
            context.ActorOf(StreamSupervisor.Props(settings, haveShutDown).WithDispatcher(settings.Dispatcher),
                StreamSupervisor.NextName()),
            haveShutDown,
            EnumerableActorName.Create(namePrefix ?? "Flow"));
    }
...
internal static class ActorMaterializerHelper
{
    ...
    /// <exception cref="ArgumentException">
    ///     This exception is thrown when the specified <paramref name="materializer" /> is not of type
    ///     <see cref="ActorMaterializer" />.
    /// </exception>
    /// <returns>TBD</returns>
...
/// <summary>
///     Signal that the stage was abruptly terminated, usually seen as a call to <see cref="GraphStageLogic.PostStop" />
///     without
///     any of the handler callbacks seeing completion or failure from upstream or cancellation from downstream. This can
///     happen when
...
public sealed class AbruptStageTerminationException : Exception
{
    public AbruptStageTerminationException(GraphStageLogic logic)
        : base(
            $"GraphStage {logic} terminated abruptly, caused by for example materializer or actor system termination.")
    {
    }
...
public sealed class ActorMaterializerSettings
{
        ...
        int syncProcessingLimit = DefaultMaxFixedBufferSize)
    {
        if (initialInputBufferSize <= 0)
            throw new ArgumentException($"{nameof(initialInputBufferSize)} must be > 0",
                nameof(initialInputBufferSize));
        ...
        if ((maxInputBufferSize & (maxInputBufferSize - 1)) != 0)
            throw new ArgumentException($"{nameof(maxInputBufferSize)} must be a power of two",
                nameof(maxInputBufferSize));

        if (initialInputBufferSize > maxInputBufferSize)
            throw new ArgumentException(
                $"initialInputBufferSize({initialInputBufferSize}) must be <= maxInputBufferSize({maxInputBufferSize})");
    ...
    private static ActorMaterializerSettings Create(Config config)
    {
        ...
        return new ActorMaterializerSettings(
            config.GetInt("initial-input-buffer-size", 4),
            config.GetInt("max-input-buffer-size", 16),
            config.GetString("dispatcher", string.Empty),
            Deciders.StoppingDecider,
            StreamSubscriptionTimeoutSettings.Create(config),
    ...
    public ActorMaterializerSettings WithInputBuffer(int initialSize, int maxSize)
    {
        if (initialSize == InitialInputBufferSize && maxSize == MaxInputBufferSize)
            return this;
        return Copy(initialSize, maxSize);
    }
    ...
    public ActorMaterializerSettings WithStreamRefSettings(StreamRefSettings settings)
    {
        ...
        if (ReferenceEquals(settings, StreamRefSettings)) return this;
    ...
    {
        return new Attributes(new Attributes.InputBuffer(InitialInputBufferSize, MaxInputBufferSize),
            Attributes.CancellationStrategy.Default, new ActorAttributes.Dispatcher(Dispatcher),
            new ActorAttributes.SupervisionStrategy(SupervisionDecider),
            new ActorAttributes.DebugLogging(IsDebugLogging),
            new ActorAttributes.StreamSubscriptionTimeout(SubscriptionTimeoutSettings.Timeout,
                SubscriptionTimeoutSettings.Mode), new ActorAttributes.OutputBurstLimit(OutputBurstLimit),
            new ActorAttributes.FuzzingMode(IsFuzzingMode), new ActorAttributes.MaxFixedBufferSize(MaxFixedBufferSize),
...
///     <see cref="StreamSubscriptionTimeoutSettings" />.
/// </summary>
public sealed class StreamSubscriptionTimeoutSettings : IEquatable<StreamSubscriptionTimeoutSettings>
{
    ...
    public static StreamSubscriptionTimeoutSettings Create(Config config)
    {
        ...
        switch (configMode)
        {
            ...
            case "false":
            case "noop":
                mode = StreamSubscriptionTimeoutTerminationMode.NoopTermination;
                break;
            case "warn":
                mode = StreamSubscriptionTimeoutTerminationMode.WarnTermination;
                break;
            case "cancel":
                mode = StreamSubscriptionTimeoutTerminationMode.CancelTermination;
                break;
            default:
                throw new ArgumentException(
                    "akka.stream.materializer.subscribtion-timeout.mode was not defined or has invalid value. Valid values are: no, off, false, noop, warn, cancel");
        }

        return new StreamSubscriptionTimeoutSettings(
            mode,
            c.GetTimeSpan("timeout", TimeSpan.FromSeconds(5)));
    }
    ...
    {
        return $"StreamSubscriptionTimeoutSettings<{Mode}, {Timeout}>";
    }
}
...
public static class ActorMaterializerExtensions
{
    ...
    /// <param name="namePrefix">TBD</param>
    /// <returns>TBD</returns>
    public static ActorMaterializer Materializer(this IActorRefFactory context,
        ActorMaterializerSettings settings = null, string namePrefix = null)
    {
