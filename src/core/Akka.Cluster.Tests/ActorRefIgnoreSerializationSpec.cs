...
public class ActorRefIgnoreSerializationSpec : AkkaSpec
{
    ...
    private readonly ActorSystem system1;
    ...
    [Fact]
    public void
        ActorSystem_IgnoreRef_should_return_a_serializable_ActorRef_that_can_be_sent_between_two_ActorSystems_using_remote()
    {
        var ignoreRef = system1.IgnoreRef;
        var remoteRefStr =
            ignoreRef.Path.ToSerializationFormatWithAddress(system1.AsInstanceOf<ExtendedActorSystem>().Provider
                .DefaultAddress);

        remoteRefStr.Should().Be(IgnoreActorRef.StaticPath.ToString(),
            "check ActorRef path stays untouched, ie: /local/ignore");

        var providerSys2 = system2.AsInstanceOf<ExtendedActorSystem>().Provider;
        var deserRef = providerSys2.ResolveActorRef(remoteRefStr);
        deserRef.Path.Should().Be(IgnoreActorRef.StaticPath,
            "check ActorRef path stays untouched when deserialized by another actor system");
        deserRef.Should().BeSameAs(system2.IgnoreRef,
            "check ActorRef path stays untouched when deserialized by another actor system");
