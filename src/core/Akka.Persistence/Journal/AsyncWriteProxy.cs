        ...
        public ReplayFailure(Exception cause)
        {
            if (cause == null)
                throw new ArgumentNullException(nameof(cause),
                    "AsyncWriteTarget.ReplayFailure cause exception cannot be null");
...
public abstract class AsyncWriteProxy : AsyncWriteJournal, IWithUnboundedStash
{
    ...
    public override Task<long> ReadHighestSequenceNrAsync(string persistenceId, long fromSequenceNr)
    {
        ...
        return _store
            .Ask<AsyncWriteTarget.ReplaySuccess>(new AsyncWriteTarget.ReplayMessages(persistenceId, 0, 0, 0), Timeout)
...
internal class ReplayMediator : ActorBase
{
    ...
    {
        if (message is IPersistentRepresentation representation)
        {
            _replayCallback(representation);
        }
        ...
        else if (message is ReceiveTimeout)
        {
            var timeoutException =
                new AsyncReplayTimeoutException($"Replay timed out after {_replayTimeout.TotalSeconds}s of inactivity");
