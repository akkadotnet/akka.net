...
public class RemoteNodeShutdownAndComesBackSpec : MultiNodeSpec
{
    ...
    protected RemoteNodeShutdownAndComesBackSpec(RemoteNodeShutdownAndComesBackMultiNetSpec config) : base(config,
        typeof(RemoteNodeShutdownAndComesBackSpec))
    {
        _config = config;
    ...
    [MultiNodeFact]
    public void
        RemoteNodeShutdownAndComesBack_must_properly_reset_system_message_buffer_state_when_new_system_with_same_Address_comes_up()
    {
        RunOn(() =>
        {
            ...
            Within(TimeSpan.FromSeconds(30), () =>
            {
                ...
                AwaitAssert(() =>
                {
                    ...
                    Sys.ActorSelection(new RootActorPath(secondAddress) / "user" / "subject")
                        .Tell(new Identify("subject"), p.Ref);
                    p.ExpectMsg<ActorIdentity>(i => i.Subject != null && "subject".Equals(i.MessageId),
        ...
        RunOn(() =>
        {
            ...
            var freshConfig = new StringBuilder().AppendLine("akka.remote.dot-netty.tcp {")
