...
/// <summary>
///     Builds back-off options for creating a back-off supervisor. You can pass <see cref="BackoffOptions" /> to
///     <see cref="Props" />.
/// </summary>
public static class Backoff
{
    ...
    public static BackoffOptions OnFailure(Props childProps, string childName, TimeSpan minBackoff, TimeSpan maxBackoff,
        double randomFactor)
    {
    ...
    ///     Maximum number of attempts to restart the child actor. The supervisor will terminate
    ///     itself after the maxNoOfRetries is reached. In order to restart infinitely pass in `-1`
    /// </param>
    public static BackoffOptions OnFailure(Props childProps, string childName, TimeSpan minBackoff, TimeSpan maxBackoff,
        double randomFactor, int maxNrOfRetries)
    {
        return new BackoffOptionsImpl(RestartImpliesFailure.Instance, childProps, childName, minBackoff, maxBackoff,
                randomFactor)
    ...
    /// </param>
    [Obsolete("Use the overloaded one which accepts maxNrOfRetries instead.")]
    public static BackoffOptions OnStop(Props childProps, string childName, TimeSpan minBackoff, TimeSpan maxBackoff,
        double randomFactor)
    {
    ...
    public static BackoffOptions OnStop(Props childProps, string childName, TimeSpan minBackoff, TimeSpan maxBackoff,
        double randomFactor, int maxNrOfRetries)
    {
        return new BackoffOptionsImpl(StopImpliesFailure.Instance, childProps, childName, minBackoff, maxBackoff,
                randomFactor)
            .WithMaxNrOfRetries(maxNrOfRetries);
    }
}
...
public abstract class BackoffOptions
{
    ...
    /// <summary>
    ///     Returns a new <see cref="BackoffOptions" /> with automatic back-off reset. The back-off algorithm is reset if the
    ///     child does not crash within the specified `resetBackoff`.
    /// </summary>
    ...
    /// <summary>
    ///     Returns a new <see cref="BackoffOptions" /> with manual back-off reset. The back-off is only reset if the child
    ///     sends a `BackoffSupervisor.Reset` to its parent(the backoff-supervisor actor).
    /// </summary>
    ...
    ///     The <see cref="SupervisorStrategy" /> that the back-off supervisor will use. The
    ///     default supervisor strategy is used as fallback if the specified SupervisorStrategy (its decider) does not
    ///     explicitly handle an exception
    /// </param>
    public abstract BackoffOptions WithSupervisorStrategy(OneForOneStrategy supervisorStrategy);

    /// <summary>
    ///     Returns a new <see cref="BackoffOptions" /> with a default <see cref="SupervisorStrategy.StoppingStrategy" />. The
    ///     default supervisor strategy is used as fallback for throwables not handled by
    ///     <see cref="SupervisorStrategy.StoppingStrategy" />.
    /// </summary>
    public abstract BackoffOptions WithDefaultStoppingStrategy();

    /// <summary>
    ///     Returns a new BackoffOptions with a constant reply to messages that the supervisor receives while its child is
    ///     stopped. By default, a message received while the child is stopped is forwarded to `deadLetters`. With this option,
    ///     the supervisor will reply to the sender instead.
    /// </summary>
    /// <param name="replyWhileStopped">
    ///     The message that the supervisor will send in response to all messages while its child
    ///     is stopped.
    /// </param>
    public abstract BackoffOptions WithReplyWhileStopped(object replyWhileStopped);

    /// <summary>
    ///     Returns a new BackoffOptions with a maximum number of retries to restart the child actor. By default, the
    ///     supervisor will retry infinitely. With this option, the supervisor will terminate itself after the maxNoOfRetries
    ///     is reached.
    /// </summary>
    /// <param name="maxNrOfRetries">
    ///     The number of times a child actor is allowed to be restarted, negative value means no
    ///     limit, if the limit is exceeded the child actor is stopped
    /// </param>
    /// <returns></returns>
    public abstract BackoffOptions WithMaxNrOfRetries(int maxNrOfRetries);

    /// <summary>
    ///     Predicate evaluated for each message, if it returns true and the supervised actor is stopped then the supervisor
    ///     will stop its self. If it returns true while the supervised actor is running then it will be forwarded to the
    ///     supervised actor and when the supervised actor stops itself the supervisor will stop itself.
    /// </summary>
...
internal sealed class BackoffOptionsImpl : BackoffOptions
{
    ...
    public BackoffOptionsImpl(IBackoffType backoffType, Props childProps, string childName, TimeSpan minBackoff,
        TimeSpan maxBackoff, double randomFactor, IBackoffReset reset = null)
        : this(backoffType, childProps, childName, minBackoff, maxBackoff, randomFactor, reset,
            new OneForOneStrategy(SupervisorStrategy.DefaultDecider))
    {
    }

    public BackoffOptionsImpl(IBackoffType backoffType, Props childProps, string childName, TimeSpan minBackoff,
        TimeSpan maxBackoff, double randomFactor, IBackoffReset reset, OneForOneStrategy strategy,
        object replyWhileStopped = null, Func<object, bool> finalStopMessage = null)
    {
    ...
    internal override Props Props
    {
        get
        {
            ...
            if (_randomFactor is < 0.0 or > 1.0)
            ...
            switch (_backoffType)
            {
                case RestartImpliesFailure _:
                    return Props.Create<BackoffOnRestartSupervisor>(_childProps, _childName, _minBackoff, _maxBackoff,
                        _reset, _randomFactor, _strategy, _replyWhileStopped, _finalStopMessage);
                case StopImpliesFailure _:
                    return Props.Create<BackoffSupervisor>(_childProps, _childName, _minBackoff, _maxBackoff, _reset,
                        _randomFactor, _strategy, _replyWhileStopped, _finalStopMessage);
                default:
                    return Props.Empty;
            }
        }
    ...
    {
        return new BackoffOptionsImpl(_backoffType, _childProps, _childName, _minBackoff, _maxBackoff, _randomFactor,
            new AutoReset(resetBackoff), _strategy, _replyWhileStopped, _finalStopMessage);
    }

    public override BackoffOptions WithManualReset()
    {
        return new BackoffOptionsImpl(_backoffType, _childProps, _childName, _minBackoff, _maxBackoff, _randomFactor,
            new ManualReset(), _strategy, _replyWhileStopped, _finalStopMessage);
    }

    public override BackoffOptions WithSupervisorStrategy(OneForOneStrategy supervisorStrategy)
    {
        return new BackoffOptionsImpl(_backoffType, _childProps, _childName, _minBackoff, _maxBackoff, _randomFactor,
            _reset, supervisorStrategy, _replyWhileStopped, _finalStopMessage);
    }
    ...
    public override BackoffOptions WithDefaultStoppingStrategy()
    {
        return new BackoffOptionsImpl(_backoffType, _childProps, _childName, _minBackoff, _maxBackoff, _randomFactor,
            _reset,
            new OneForOneStrategy(_strategy.MaxNumberOfRetries, null, SupervisorStrategy.StoppingStrategy.Decider),
            _replyWhileStopped, _finalStopMessage);
    }

    public override BackoffOptions WithReplyWhileStopped(object replyWhileStopped)
    {
        return new BackoffOptionsImpl(_backoffType, _childProps, _childName, _minBackoff, _maxBackoff, _randomFactor,
            _reset, _strategy, replyWhileStopped, _finalStopMessage);
    }

    public override BackoffOptions WithMaxNrOfRetries(int maxNrOfRetries)
    {
        return new BackoffOptionsImpl(_backoffType, _childProps, _childName, _minBackoff, _maxBackoff, _randomFactor,
            _reset, _strategy.WithMaxNrOfRetries(maxNrOfRetries), _replyWhileStopped, _finalStopMessage);
    }

    public override BackoffOptions WithFinalStopMessage(Func<object, bool> isFinalStopMessage)
    {
        return new BackoffOptionsImpl(_backoffType, _childProps, _childName, _minBackoff, _maxBackoff, _randomFactor,
            _reset, _strategy, _replyWhileStopped, isFinalStopMessage);
    }
}
