...
internal sealed class PersistentShardCoordinator : PersistentActor
{
    ...
    )
    {
        ...
        PersistenceId =
            $"/system/sharding/{typeName}Coordinator/singleton/coordinator"; //used for backward compatibility instead of Self.Path.ToStringWithoutAddress()
    ...
    /// <returns>TBD</returns>
    internal static Props Props(string typeName, ClusterShardingSettings settings,
        IShardAllocationStrategy allocationStrategy)
    {
                ...
                switch (evt)
                {
                    ...
                    case ShardRegionTerminated regionTerminated:
                        if (State.Regions.ContainsKey(regionTerminated.Region))
                            ...
                            Log.Debug("{0}: ShardRegionTerminated but region {1} was not registered", TypeName,
                                regionTerminated.Region);
                        return true;
    ...
    private bool ReceiveSnapshotResult(object message)
    {
        switch (message)
        {
            case SaveSnapshotSuccess m:
                ...
                InternalDeleteMessagesBeforeSnapshot(m, Settings.TuningParameters.KeepNrOfBatches,
            ...
            case DeleteMessagesFailure m:
                Log.Warning(m.Cause, "{0}: Persistent messages to [{1}] deletion failure: {2}", TypeName,
                    m.ToSequenceNr, m.Cause.Message);
                return true;

            case DeleteSnapshotsSuccess m:
                Log.Debug("{0}: Persistent snapshots to [{1}] deleted successfully", TypeName,
                    m.Criteria.MaxSequenceNr);
                return true;

            case DeleteSnapshotsFailure m:
                Log.Warning(m.Cause, "{0}: Persistent snapshots to [{1}] deletion failure: {2}", TypeName,
                    m.Criteria.MaxSequenceNr, m.Cause);
                return true;
        }
