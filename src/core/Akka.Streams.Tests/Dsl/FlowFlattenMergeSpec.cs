...
public class FlowFlattenMergeSpec : AkkaSpec
{
    ...
    private static Source<int, NotUsed> Src10(int i)
    {
        return Source.From(Enumerable.Range(i, 10));
    }
    ...
    public async Task A_FlattenMerge_must_propagate_late_failure_from_main_stream()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            var future = Source.Combine(Source.From(new[] { Blocked, Blocked }),
                    Source.Failed<Source<int, NotUsed>>(ex),
    ...
    public async Task A_FlattenMerge_must_cancel_subStreams_when_failing_from_subStream()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            // task is intentionally not awaited
            var task = Source
                .From(new[] { Source.FromPublisher(p1), Source.FromPublisher(p2), Source.FromTask(p.Task) })
                .MergeMany(5, x => x)
                .RunWith(Sink.First<int>(), Materializer);
    ...
    public async Task A_FlattenMerge_must_work_with_many_concurrently_queued_events()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            foreach (var _ in Enumerable.Range(0, noOfSources * 10)) elems.Add(await p.RequestNextAsync());
