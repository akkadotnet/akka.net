...
{
    private static readonly Member a1 = TestMember.Create(new Address("akka.tcp", "sys", "a", 2552), MemberStatus.Up);
    private static readonly Member a2 = TestMember.Create(a1.Address, MemberStatus.Joining);
    private static readonly Member b1 = TestMember.Create(new Address("akka.tcp", "sys", "b", 2552), MemberStatus.Up);
    private static readonly Member b2 = TestMember.Create(b1.Address, MemberStatus.Removed);

    private static readonly Member c1 =
        TestMember.Create(new Address("akka.tcp", "sys", "c", 2552), MemberStatus.Leaving);

    private static readonly Member c2 = TestMember.Create(c1.Address, MemberStatus.Up);
    private static readonly Member c3 = TestMember.Create(c1.Address, MemberStatus.Exiting);

    private static readonly Member d1 =
        TestMember.Create(new Address("akka.tcp", "sys", "d", 2552), MemberStatus.Leaving);

    private static readonly Member e1 =
        TestMember.Create(new Address("akka.tcp", "sys", "e", 2552), MemberStatus.Joining);

    private static readonly Member e2 = TestMember.Create(e1.Address, MemberStatus.Up);
    private static readonly Member e3 = TestMember.Create(e1.Address, MemberStatus.Down);
    ...
    public void A_gossip_must_reach_convergence_skipping_Unreachable_Leaving_with_ExitingConfirmed()
    {
        ...
        var g1 = new Gossip(ImmutableSortedSet.Create(a1, b1, c1), new GossipOverview(r1)).Seen(a1.UniqueAddress)
    ...
    public void A_gossip_must_know_who_is_youngest()
    {
        ...
        var g2 = new Gossip(ImmutableSortedSet.Create(a2, b1.CopyUp(3), e1),
            new GossipOverview(Reachability.Empty.Unreachable(a2.UniqueAddress, b1.UniqueAddress)
    ...
    {
        var a1 = TestMember.Create(new Address("akka.tcp", "sys", "a4", 2552), MemberStatus.Up,
            ImmutableHashSet<string>.Empty, upNumber: 1);
        var a2 = TestMember.Create(new Address("akka.tcp", "sys", "a3", 2552), MemberStatus.Up,
            ImmutableHashSet<string>.Empty, upNumber: 2);
        var a3 = TestMember.Create(new Address("akka.tcp", "sys", "a2", 2552), MemberStatus.Up,
            ImmutableHashSet<string>.Empty, upNumber: 3);
        var a4 = TestMember.Create(new Address("akka.tcp", "sys", "a1", 2552), MemberStatus.Up,
            ImmutableHashSet<string>.Empty, upNumber: 4);
        ...
        var r = GossipTargetsForExitingMembers(gossip, new[] { a1Exiting });
        r.Should().BeEquivalentTo(a1Exiting, a2);
    }
    ...
    public void A_gossip_must_find_two_oldest_per_role_as_targets_for_Exiting_change()
    {
        ...
        var a4 = TestMember.Create(new Address("akka.tcp", "sys", "a1", 2552), MemberStatus.Up,
            ImmutableHashSet<string>.Empty, upNumber: 4);
        var a5 = TestMember.Create(new Address("akka.tcp", "sys", "a5", 2552), MemberStatus.Exiting,
            ImmutableHashSet<string>.Empty.Add("role1").Add("role2"), upNumber: 5);
        var a6 = TestMember.Create(new Address("akka.tcp", "sys", "a6", 2552), MemberStatus.Exiting,
            ImmutableHashSet<string>.Empty.Add("role1").Add("role3"), upNumber: 6);
        var a7 = TestMember.Create(new Address("akka.tcp", "sys", "a7", 2552), MemberStatus.Exiting,
            ImmutableHashSet<string>.Empty.Add("role1"), upNumber: 7);
        var a8 = TestMember.Create(new Address("akka.tcp", "sys", "a8", 2552), MemberStatus.Exiting,
            ImmutableHashSet<string>.Empty.Add("role1"), upNumber: 8);
        var a9 = TestMember.Create(new Address("akka.tcp", "sys", "a9", 2552), MemberStatus.Exiting,
            ImmutableHashSet<string>.Empty.Add("role2"), upNumber: 9);

        var theExiting = new[] { a5, a6 };
        var gossip = new Gossip(ImmutableSortedSet.Create(a1, a2, a3, a4, a5, a6, a7, a8, a9));

        var r = GossipTargetsForExitingMembers(gossip, theExiting);
