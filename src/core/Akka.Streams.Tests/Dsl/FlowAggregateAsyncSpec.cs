...
public class FlowAggregateAsyncSpec : AkkaSpec
{
    ...
    [Fact]
    public async Task
        A_AggregateAsync_must_work_when_using_Source_AggregateAsync_and_Flow_AggregateAsync_and_Sink_AggregateAsync()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
    ...
    public async Task A_AggregateAsync_must_resume_after_multiple_failures()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var tasks = new[]
            {
                ...
                Task.FromException<string>(new Exception("failure5")), Task.FromResult("happy!")
            };
