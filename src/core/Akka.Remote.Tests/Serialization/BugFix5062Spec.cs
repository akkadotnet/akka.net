...
public class BugFix5062Spec : AkkaSpec
{
    ...
    public void Failed_serialization_should_give_proper_exception_message()
    {
        ...
        o.Invoking(_ => MessageSerializer.Deserialize((ExtendedActorSystem)Sys, serialized)).Should()
            ...
            .WithMessage(
                $"Failed to deserialize payload object when deserializing {nameof(ActorSelectionMessage)} with payload [SerializerId=13, Manifest=SM] addressed to [{childName}]. {Serializer.GetErrorForSerializerId(13)}")
