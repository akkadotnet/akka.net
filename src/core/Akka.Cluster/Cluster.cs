...
public class Cluster : IExtension
{
    ...
    ///     This exception is thrown if the <paramref name="system" /> does not have a <see cref="ClusterActorRefProvider" />
...
#pragma warning restore CS0618
        ...
        FailureDetector = new DefaultFailureDetectorRegistry<Address>(() => FailureDetectorLoader.Load(
            Settings.FailureDetectorImplementationClass, Settings.FailureDetectorConfig,
            system));
        ...
        _downingProvider = new Lazy<IDowningProvider>(
            () => Akka.Cluster.DowningProvider.Load(Settings.DowningProviderType, system, this),
            LazyThreadSafetyMode.ExecutionAndPublication);

        //create supervisor for daemons under path "/system/cluster"
        _clusterDaemons = system.SystemActorOf(Props.Create(() => new ClusterDaemon(Settings)).WithDeploy(Deploy.Local),
            "cluster");
    ...
    ///     TBD
    /// </summary>
    internal static bool IsAssertInvariantsEnabled { get; }
    ...
    internal IActorRef ClusterCore
    {
        get
        {
            if (_clusterCore == null) _clusterCore = GetClusterCoreRef().Result;
            return _clusterCore;
        }
    }
    ...
    private async Task<IActorRef> GetClusterCoreRef()
    {
        ...
        try
        {
            return await _clusterDaemons.Ask<IActorRef>(new InternalClusterAction.GetClusterCoreRef(this), timeout)
    ...
    /// <remarks>
    ///     A snapshot of <see cref="ClusterEvent.CurrentClusterState" /> will be sent to <paramref name="subscriber" />
    ...
    ///     If set to <see cref="ClusterEvent.SubscriptionInitialStateMode.InitialStateAsEvents" />, then the events
    ///     corresponding to the current state
    ///     are sent to <paramref name="subscriber" /> to mimic what it would have seen if it were listening to the events when
    ...
    ///     <see cref="ClusterEvent.CurrentClusterState" /> will be sent to <paramref name="subscriber" /> as the first
    ///     message.
    /// </param>
    ...
    /// </exception>
    public void Subscribe(IActorRef subscriber, ClusterEvent.SubscriptionInitialStateMode initialStateMode,
        params Type[] to)
    {
        if (to.Length == 0)
        ...
        if (!to.All(t => typeof(ClusterEvent.IClusterDomainEvent).IsAssignableFrom(t)))
            throw new ArgumentException(
                $"Subscribe to `IClusterDomainEvent` or subclasses, was [{string.Join(", ", to.Select(c => c.Name))}]",
                nameof(to));

        ClusterCore.Tell(
            new InternalClusterAction.Subscribe(subscriber, initialStateMode, ImmutableHashSet.Create(to)));
    }
    ...
    public Task JoinAsync(Address address, CancellationToken token = default)
    {
        ...
        RegisterOnMemberUp(() => { completion.TrySetResult(NotUsed.Instance); });
    ...
    /// <summary>
    ///     Causes the CURRENT node, i.e. the one calling this function, to leave the cluster.
    ///     Once the returned <see cref="Task" /> completes in completed or cancelled state, it means that the member has
    ...
    /// <returns>
    ///     A <see cref="Task" /> that will return upon the current node being removed from the cluster, or if await was
    ///     cancelled.
    /// </returns>
    /// <remarks>
    ///     The cancellation token doesn't cancel leave from the cluster, it only lets to give up on awaiting (by timeout for
    ///     example).
    /// </remarks>
    ...
    /// </summary>
    /// <param name="callback">
    ///     The callback that is run whenever the current member achieves a status of
    ///     <see cref="MemberStatus.Up" />
    /// </param>
    ...
    /// <summary>
    ///     Generates the remote actor path by replacing the <see cref="ActorPath.Address" /> in the RootActorPath for the
    ///     given
...
/// <summary>
///     Exception thrown, when <see cref="Cluster.JoinAsync" /> or <see cref="Cluster.JoinSeedNodesAsync" /> fails to
///     succeed.
/// </summary>
