...
public class PNCounterDictionarySpec
{
    ...
    [Fact]
    public void
        PNCounterDictionary_must_be_able_to_have_its_entries_correctly_merged_with_another_ORDictionary_with_other_entries()
    {
    ...
    public void PNCounterDictionary_must_be_able_to_work_with_deltas()
    {
        ...
        var expected = new Dictionary<string, BigInteger> { { "a", 1 }, { "b", 3 }, { "c", 7 } }
            .ToImmutableDictionary();
        PNCounterDictionary<string>.Empty.MergeDelta(m1.Delta).MergeDelta(m2.Delta).Entries.Should()
            .BeEquivalentTo(expected);
        PNCounterDictionary<string>.Empty.MergeDelta(m2.Delta).MergeDelta(m1.Delta).Entries.Should()
            .BeEquivalentTo(expected);

        var merged1 = m1.Merge(m2);

        var m3 = merged1.ResetDelta().Remove(_node1, "b");
        merged1.MergeDelta(m3.Delta).Entries.Should()
            .BeEquivalentTo(new Dictionary<string, BigInteger> { { "a", 1 }, { "c", 7 } });
