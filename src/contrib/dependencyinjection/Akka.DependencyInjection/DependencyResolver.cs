...
public sealed class DependencyResolver : IExtension
{
    ...
    /// <summary>
    ///     Uses a delegate to dynamically instantiate an actor where some of the constructor arguments are populated via
    ///     dependency injection
    ///     and others are not.
    /// </summary>
    /// <remarks>
    ///     YOU ARE RESPONSIBLE FOR MANAGING THE LIFECYCLE OF YOUR OWN DEPENDENCIES. AKKA.NET WILL NOT ATTEMPT TO DO IT FOR
    ///     YOU.
    /// </remarks>
    /// <typeparam name="T">The type of actor to instantiate.</typeparam>
    /// <param name="args">
    ///     Optional. Any constructor arguments that will be passed into the actor's constructor directly
    ///     without being resolved by DI first.
    /// </param>
    ...
    /// <summary>
    ///     Used to dynamically instantiate an actor where some of the constructor arguments are populated via dependency
    ///     injection
    ///     and others are not.
    /// </summary>
    /// <remarks>
...
public sealed class DependencyResolverExtension : ExtensionIdProvider<DependencyResolver>
{
    public override DependencyResolver CreateExtension(ExtendedActorSystem system)
    {
        ...
        var exception = new ConfigurationException(
            "Unable to find [DependencyResolverSetup] included in ActorSystem settings." +
            " Please specify one before attempting to use dependency injection inside Akka.NET.");
        system.EventStream.Publish(new Error(exception, "Akka.DependencyInjection", typeof(DependencyResolverExtension),
            exception.Message));
        throw exception;
    }
}