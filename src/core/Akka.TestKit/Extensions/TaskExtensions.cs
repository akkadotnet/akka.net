...
public static class TaskExtensions
{
    ...
    /// <param name="because">
    ///     A formatted phrase as is supported by <see cref="M:System.String.Format(System.String,System.Object[])" />
    ///     explaining why the assertion
    ...
    public static async Task ShouldThrowWithin<T>(
        ...
        where T : Exception
    {
        (await Awaiting(async () => { await task.ShouldCompleteWithin(timeout); }).Should().ThrowAsync<T>()).And
            .Should().Be(expected);
    }
    ...
    public static async Task<T> ShouldThrowWithin<T>(
        ...
        where T : Exception
    {
        var exception = await Awaiting(async () => { await task.ShouldCompleteWithin(timeout); }).Should()
            .ThrowAsync<T>();
