...
/// <summary>
///     <para>
///         This class represents a pairing of an <see cref="AkkaProtocolTransport" /> with its <see cref="Address" />
///         binding.
///     </para>
///     <para>
...
/// <summary>
///     Implementation of the Akka protocol as a (logical) <see cref="Transport" /> that wraps an underlying (physical)
///     <see cref="Transport" /> instance.
...
internal class AkkaProtocolTransport : ActorTransportAdapter
{
    ...
    protected override Props ManagerProps =>
        _managerProps ??= Props.Create(() => new AkkaProtocolManager(WrappedTransport, Settings))
    ...
    public async Task<AkkaProtocolHandle> Associate(Address remoteAddress, int? refuseUid)
    {
        ...
        manager.Tell(new AssociateUnderlyingRefuseUid(SchemeAugmenter.RemoveScheme(remoteAddress), statusPromise,
            refuseUid));
...
internal sealed class AssociateUnderlyingRefuseUid : INoSerializationVerificationNeeded
{
    ...
    /// <param name="refuseUid">TBD</param>
    public AssociateUnderlyingRefuseUid(Address remoteAddress,
        TaskCompletionSource<AssociationHandle> statusCompletionSource, int? refuseUid = null)
    {
        RefuseUid = refuseUid;
...
internal sealed class OutboundUnassociated : InitialProtocolStateData
{
    ...
    public OutboundUnassociated(Address remoteAddress, TaskCompletionSource<AssociationHandle> statusCompletionSource,
        Transport transport)
    {
        Transport = transport;
        StatusCompletionSource = statusCompletionSource;
...
internal sealed class OutboundUnderlyingAssociated : ProtocolStateData
{
    ...
    /// <param name="statusCompletionSource">TBD</param>
    /// <param name="wrappedHandle">TBD</param>
    public OutboundUnderlyingAssociated(TaskCompletionSource<AssociationHandle> statusCompletionSource,
...
internal sealed class AssociatedWaitHandler : ProtocolStateData
{
    ...
    public AssociatedWaitHandler(Task<IHandleEventListener> handlerListener, AssociationHandle wrappedHandle,
...
internal sealed class ProtocolStateActor : FSM<AssociationState, ProtocolStateData>
{
    ...
    public ProtocolStateActor(HandshakeInfo handshakeInfo, Address remoteAddress,
        ...
        : this(
            new OutboundUnassociated(remoteAddress, statusCompletionSource, transport), handshakeInfo, settings, codec,
            failureDetector,
            refuseUid)
    {
    }
    ...
    /// <param name="codec">TBD</param>
    /// <param name="failureDetector">TBD</param>
    public ProtocolStateActor(HandshakeInfo handshakeInfo, AssociationHandle wrappedHandle,
        IAssociationEventListener associationEventListener, AkkaProtocolSettings settings, AkkaPduCodec codec,
        FailureDetector failureDetector)
        : this(new InboundUnassociated(associationEventListener, wrappedHandle), handshakeInfo, settings, codec,
    ...
    ///     <dl>
    ///         <dt>
    ///             <b>when in the <see cref="AssociationState.WaitHandshake" /> state</b>
    ///         </dt>
    ///         <dd>
    ///             <dl>
    ///                 <dt>
    ///                     <b>the event is of type <see cref="HeartbeatTimer" /></b>
    ///                 </dt>
    ///                 <dd>This exception is thrown when there is no response from the remote system causing the timeout.</dd>
    ///             </dl>
    ///         </dd>
    ///         <dt>
    ///             <b>when in the <see cref="AssociationState.Open" /> state</b>
    ///         </dt>
    ///         <dd>
    ///             <dl>
    ///                 <dt>
    ///                     <b>the event is of type <see cref="InboundPayload" /></b>
    ///                 </dt>
    ...
    ///                 <dd>This exception is thrown when there is no response from the remote system causing the timeout.</dd>
    ///                 <dt>
    ///                     <b>the event is of type <see cref="DisassociateUnderlying" /></b>
    ///                 </dt>
    ///                 <dd>This exception is thrown when the message type could not be handled.</dd>
    ///             </dl>
    ///         </dd>
    ///         <dt>
    ///             <b>
    ///                 when the FSM is terminating
    ///                 <see cref="FSM{TState, TData}.OnTermination(Action{Akka.Actor.FSMBase.StopEvent{TState, TData}})" />
    ///             </b>
    ///         </dt>
    ///         <dd>
    ///             <dl>
    ///                 <dt>
    ///                     <b>the event is of type <see cref="OutboundUnassociated" /></b>
    ///                 </dt>
    ///                 <dd>This exception is thrown when the transport disassociated before the handshake finished.</dd>
    ///                 <dt>
    ///                     <b>
    ///                         the event is of type <see cref="OutboundUnderlyingAssociated" /> with
    ///                         <see cref="Akka.Actor.FSMBase.StopEvent{TState, TData}.Reason " />
    ///                         being <see cref="Akka.Actor.FSMBase.Failure" />
    ///                     </b>
    ///                 </dt>
    ///                 <dd>
    ///                     This exception is thrown when either a timeout occurs, the remote system is shutting down,
    ...
    ///     </dl>
    /// </exception>
    private ProtocolStateActor(InitialProtocolStateData initialData, HandshakeInfo localHandshakeInfo,
        AkkaProtocolSettings settings, AkkaPduCodec codec, FailureDetector failureDetector, int? refuseUid)
    {
    ...
    private void InitializeFSM()
    {
                        ...
                        case OutboundUnassociated ou:
                            ...
                            SetTimer("associate-retry", new HandleMsg(wrappedHandle),
                                RARP.For(Context.System).Provider
                                    .RemoteSettings.BackoffPeriod, false);
                            return Stay();
                    }
        ...
        When(AssociationState.WaitHandshake, @event =>
        {
            switch (@event.FsmEvent)
            {
                    ...
                    switch (pdu)
                    {
                        ...
                        default:
                            ...
                            if (_log.IsDebugEnabled)
                                _log.Debug("Expected message of type Associate; instead received {0}",
                                    @event.FsmEvent.GetType());
                ...
                case InboundPayload p when @event.StateData is InboundUnassociated iu:
                {
                    ...
                    switch (pdu)
                    {
                        ...
                        default:
                            if (_log.IsDebugEnabled)
                                _log.Debug(
                                    "Sending disassociate to [{0}] because unexpected message of type [{1}] was received unassociated.",
                                    wrappedHandle, @event.FsmEvent.GetType());
                            SendDisassociate(wrappedHandle, DisassociateInfo.Unknown);
                            return Stop();
                    }
                }
                ...
                case HandshakeTimer t when @event.StateData is OutboundUnderlyingAssociated oua:
                    if (_log.IsDebugEnabled)
                        _log.Debug(
                            "Sending disassociate to [{0}] because handshake timed out for outbound association after [{1}] ms.",
                            oua.WrappedHandle, _settings.HandshakeTimeout.TotalMilliseconds);
                ...
                case HandshakeTimer t when @event.StateData is InboundUnassociated iu:
                    if (_log.IsDebugEnabled)
                        _log.Debug(
                            "Sending disassociate to [{0}] because handshake timed out for inbound association after [{1}] ms.",
                            iu.WrappedHandle, _settings.HandshakeTimeout.TotalMilliseconds);
        ...
        When(AssociationState.Open, @event =>
        {
            switch (@event.FsmEvent)
            {
                ...
                case HandleListenerRegistered hlr when @event.StateData is AssociatedWaitHandler awh:
                    foreach (var p in awh.Queue) hlr.Listener.Notify(new InboundPayload(p));
                    ...
                    switch (@event.Reason)
                    {
                        case Failure f:
                            switch (f.Cause)
                            {
                                ...
                                default:
                                    associationFailure =
                                        new AkkaProtocolException($"Unknown Failure cause: [{f.Cause}]");
                                    break;
                            }
        ...
        switch (_initialData)
        {
            case OutboundUnassociated d:
                ...
                d.Transport.Associate(d.RemoteAddress).ContinueWith(result => new HandleMsg(result.Result),
    ...
    private static Exception DisassociateException(DisassociateInfo info)
    {
        switch (info)
        {
            case DisassociateInfo.Shutdown:
                return new AkkaProtocolException(
                    "The remote system refused the association because it is shutting down.");
            case DisassociateInfo.Quarantined:
                return new AkkaProtocolException(
                    "The remote system has quarantined this system. No further associations to the remote systems are possible until this system is restarted.");
    ...
    private State<AssociationState, ProtocolStateData> HandleTimers(AssociationHandle wrappedHandle)
    {
        ...
        if (_log.IsDebugEnabled)
            _log.Debug("Sending disassociate to [{0}] because failure detector triggered in state [{1}]", wrappedHandle,
                StateName);

        //send disassociate just to be sure
        SendDisassociate(wrappedHandle, DisassociateInfo.Unknown);
        return Stop(new Failure(new TimeoutReason(
            $"No response from remote. Handshake timed out or transport failure detector triggered. (internal state was {StateName})")));
    }
        ...
        HandshakeInfo handshakeInfo, IAssociationEventListener associationEventListener)
    {
        ...
        associationEventListener.Notify(
            new InboundAssociation(
                new AkkaProtocolHandle(_localAddress, handshakeInfo.Origin, readHandlerPromise, wrappedHandle,
                    handshakeInfo, Self, _codec)));
        return readHandlerPromise.Task;
    }
    ...
    public static Props OutboundProps(HandshakeInfo handshakeInfo, Address remoteAddress,
        ...
        Transport transport, AkkaProtocolSettings settings, AkkaPduCodec codec, FailureDetector failureDetector,
        int? refuseUid = null)
    {
        return Props.Create(() => new ProtocolStateActor(handshakeInfo, remoteAddress, statusCompletionSource,
            transport, settings, codec, failureDetector, refuseUid));
    }
    ...
    public static Props InboundProps(HandshakeInfo handshakeInfo, AssociationHandle wrappedHandle,
        ...
        FailureDetector failureDetector)
    {
        return Props.Create(() => new ProtocolStateActor(handshakeInfo, wrappedHandle, associationEventListener,
            settings, codec, failureDetector));
    }
