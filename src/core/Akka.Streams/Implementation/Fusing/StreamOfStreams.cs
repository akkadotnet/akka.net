...
internal sealed class FlattenMerge<TGraph, T, TMat> : GraphStage<FlowShape<TGraph, T>>
    where TGraph : IGraph<SourceShape<T>, TMat>
{
    ...
    /// <returns>TBD</returns>
    protected override GraphStageLogic CreateLogic(Attributes enclosingAttributes)
    {
        return new Logic(this, enclosingAttributes);
    }
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        public override string ToString()
        {
            return $"FlattenMerge({_stage._breadth})";
        }
    }
...
internal sealed class PrefixAndTail<T> : GraphStage<FlowShape<T, (IImmutableList<T>, Source<T, NotUsed>)>>
{
    ...
    private sealed class Logic : TimerGraphStageLogic, IInHandler, IOutHandler
    {
        ...
        public Logic(PrefixAndTail<T> stage) : base(stage.Shape)
        {
            ...
            _subHandler = new LambdaOutHandler(() =>
            {
                SetKeepGoing(false);
                CancelTimer(SubscriptionTimer);
                Pull(_stage._in);
                _tailSource.SetHandler(new LambdaOutHandler(() => Pull(_stage._in)));
            });
        ...
        public void OnPush()
        {
            ...
            else
            {
                ...
                if (_left == 0)
                {
                    Push(_stage._out, (_builder.ToImmutable(), OpenSubstream()));
        ...
        public void OnUpstreamFinish()
        {
            if (!IsPrefixComplete)
            {
                ...
                Emit(_stage._out, (_builder.ToImmutable(), Source.Empty<T>()), CompleteStage);
            }
            else
            {
        ...
        public void OnPull()
        {
            if (_left == 0)
            {
                Push(_stage._out, (ImmutableList<T>.Empty, OpenSubstream()));
                Complete(_stage._out);
            }
            else
            {
        ...
        {
            var timeout = ActorMaterializerHelper.Downcast(Interpreter.Materializer).Settings
...
internal sealed class GroupBy<T, TKey> : GraphStage<FlowShape<T, Source<T, NotUsed>>>
{
    ...
    private sealed class Logic : TimerGraphStageLogic, IInHandler, IOutHandler
    {
        ...
        private bool NeedToPull => !(HasBeenPulled(_stage.In) || IsClosed(_stage.In) || HasNextElement ||
                                     _substreamWaitingToBePushed.HasValue);
        ...
        protected internal override void OnTimer(object timerKey)
        {
            ...
            {
                if (!_stage._allowClosedSubstreamRecreation) _closedSubstreams.Add(key);
        ...
        private sealed class SubstreamSource : SubSourceOutlet<T>, IOutHandler
        {
            ...
            public SubstreamSource(Logic logic, string name, Option<TKey> key, Option<T> firstElement) : base(logic,
                name)
            {
                _logic = logic;
...
internal static class Split
{
    ...
    /// <returns>TBD</returns>
    public static IGraph<FlowShape<T, Source<T, NotUsed>>, NotUsed> When<T>(Func<T, bool> p,
        SubstreamCancelStrategy substreamCancelStrategy)
    {
        return new Split<T>(SplitDecision.SplitBefore, p, substreamCancelStrategy);
    }
    ...
    /// <param name="substreamCancelStrategy">TBD</param>
    /// <returns>TBD</returns>
    public static IGraph<FlowShape<T, Source<T, NotUsed>>, NotUsed> After<T>(Func<T, bool> p,
        SubstreamCancelStrategy substreamCancelStrategy)
    {
        return new Split<T>(SplitDecision.SplitAfter, p, substreamCancelStrategy);
    }
...
internal sealed class Split<T> : GraphStage<FlowShape<T, Source<T, NotUsed>>>
{
    ...
    private sealed class Logic : TimerGraphStageLogic, IInHandler, IOutHandler
    {
        ...
        protected internal override void OnTimer(object timerKey)
        {
            _substreamSource.Timeout(_timeout);
        }
        ...
        private sealed class SubstreamHandler : InAndOutHandler
        {
            ...
            public override void OnDownstreamFinish(Exception cause)
            {
                ...
                {
                    // Start draining
                    if (!_logic.HasBeenPulled(_inlet)) _logic.Pull(_inlet);
                }
            }
...
internal sealed class SubSink<T> : GraphStage<SinkShape<T>>
{
    ...
    ///     TBD
    /// </summary>
    public void CancelSubstream(Exception cause)
    {
        DispatchCommand(new SubSink.CancelScheduledBeforeMaterialization(cause));
    }
    ...
    private void DispatchCommand(SubSink.CommandScheduledBeforeMaterialization newState)
    {
        ...
        {
            case Action<SubSink.ICommand> callback:
                callback(newState.Command);
                break;
            case SubSink.Uninitialized _:
                ...
                break;
            case SubSink.RequestOneScheduledBeforeMaterialization _
                when newState is SubSink.CancelScheduledBeforeMaterialization:
            ...
            case SubSink.CommandScheduledBeforeMaterialization command:
                throw new IllegalStateException(
                    $"{newState.Command} on subsink is illegal when {command.Command} is still pending");
        }
    }
    ...
    private sealed class Logic : InGraphStageLogic
    {
        ...
        public override void OnPush()
        {
            _stage._externalCallback(new OnNext(Grab(_stage._in)));
        }

        public override void OnUpstreamFinish()
        {
            _stage._externalCallback(OnComplete.Instance);
        }

        public override void OnUpstreamFailure(Exception e)
        {
            _stage._externalCallback(new OnError(e));
        }
        ...
        private void SetCallback(Action<SubSink.ICommand> callback)
        {
            ...
            {
                case SubSink.Uninitialized _:
                    if (!status.CompareAndSet(SubSink.Uninitialized.Instance, /* Materialized */
...
internal sealed class SubSource<T> : GraphStage<SourceShape<T>>
{
    ...
    /// <returns>TBD</returns>
    public bool Timeout(TimeSpan d)
    {
        return _status.CompareAndSet(null,
            new OnError(new SubscriptionTimeoutException($"Substream Source has not been materialized in {d}")));
    }
    ...
    private sealed class Logic : OutGraphStageLogic
    {
        ...
        public override void OnPull()
        {
            _stage.ExternalCallback(SubSink.RequestOne.Instance);
        }

        public override void OnDownstreamFinish(Exception cause)
        {
            _stage.ExternalCallback(new SubSink.Cancel(cause));
        }
