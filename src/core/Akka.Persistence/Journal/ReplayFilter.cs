...
///     <see cref="IPersistentRepresentation.SequenceNr" /> in the replayed events to find events emitted by overlapping
///     writers.
/// </summary>
public class ReplayFilter : ActorBase
{
    ...
    public ReplayFilter(IActorRef persistentActor, ReplayFilterMode mode, int windowSize, int maxOldWriters,
        bool debugEnabled)
    {
    ...
    /// </exception>
    /// <returns>TBD</returns>
    public static Props Props(IActorRef persistentActor, ReplayFilterMode mode, int windowSize, int maxOldWriters,
        bool debugEnabled)
    {
        if (windowSize <= 0)
        ...
        return Actor.Props.Create(
            () => new ReplayFilter(persistentActor, mode, windowSize, maxOldWriters, debugEnabled));
    }
    ...
    {
        foreach (var message in _buffer) PersistentActor.Tell(message, ActorRefs.NoSender);
        _buffer.Clear();
    }
