...
public sealed class GraphModule : AtomicModule
{
    ...
    {
        return !newShape.Equals(Shape) ? CompositeModule.Create(this, newShape) : this;
    }
...
public sealed class GraphInterpreterShell
{
    ...
    public GraphInterpreterShell(GraphAssembly assembly, Connection[] connections, GraphStageLogic[] logics,
        Shape shape, ActorMaterializerSettings settings, ExtendedActorMaterializer materializer)
    {
        _assembly = assembly;
    ...
    /// <returns>TBD</returns>
    public int Init(IActorRef self, SubFusingActorMaterializerImpl subMat, Action<object> enqueueToShourtCircuit,
        int eventLimit)
    {
        Self = self;
        ...
        for (var i = 0; i < _outputs.Length; i++)
        {
            ...
            var output =
                (ActorGraphInterpreter.IActorOutputBoundary)typeof(ActorGraphInterpreter.ActorOutputBoundary<>)
                    .Instantiate(outputType, Self, this, i);
            _outputs[i] = output;
            ...
            switch (e)
            {
                ...
                case ActorGraphInterpreter.OnSubscribe onSubscribe:
                    ReactiveStreamsCompliance.TryCancel(onSubscribe.Subscription,
                        SubscriptionWithCancelException.StageWasCompleted.Instance);
                    _subscribersPending--;
                    if (CanShutdown)
                        _interpreterCompleted = true;
                    break;
...
#pragma warning restore CS0162
    ...
    {
        return new BusLogging(Materializer.System.EventStream, Self.ToString(), typeof(GraphInterpreterShell),
            Materializer.System.Settings.LogFormatter);
    }
...
public class ActorGraphInterpreter : ActorBase
{
    ...
    /// If this ctor gets changed you -must- change
    /// <see cref="ActorGraphInterpreter.Props" />
    /// as well!
...
#pragma warning restore CS0162
    ...
    public sealed class BoundarySubscription : ISubscriptionWithCancelException
    {
        ...
        {
            _parent.Tell(new Cancel(_shell, _id, cause));
        }
    ...
    public class BatchingActorInputBoundary : UpstreamBoundaryStageLogic
    {
        ...
        public void OnSubscribe(ISubscription subscription)
        {
            ...
            else if (_upstream != null)
            {
                ...
                ReactiveStreamsCompliance.TryCancel(subscription,
                    new IllegalStateException("Publisher can only be subscribed once."));
            }
            else
            {
        ...
        public void SubscribePending()
        {
            foreach (var subscriber in _exposedPublisher.TakePendingSubscribers())
                if (ReferenceEquals(_subscriber, null))
                {
                    ...
                    ReactiveStreamsCompliance.TryOnSubscribe(_subscriber,
                        new BoundarySubscription(_actor, _shell, _id));
                    if (IsDebug)
