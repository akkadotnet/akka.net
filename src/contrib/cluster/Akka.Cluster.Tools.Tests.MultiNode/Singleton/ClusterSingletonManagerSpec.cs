...
public class ClusterSingletonManagerSpecConfig : MultiNodeConfig
{
    ...
    public ClusterSingletonManagerSpecConfig()
    {
        ...
        NodeConfig(new[] { First, Second, Third, Fourth, Fifth, Sixth },
            new[] { ConfigurationFactory.ParseString(@"akka.cluster.roles = [worker]") });
    }
}
...
internal class Consumer : ReceiveActor
{
    ...
    {
        if (stoppedBeforeUnregistration) _log.Warning("Stopped before unregistration");
    }
...
public class ClusterSingletonManagerSpec : MultiNodeClusterSpec
{
    ...
    public void ClusterSingletonManager_should_startup_6_node_cluster()
    {
        Within(TimeSpan.FromSeconds(60), () =>
        {
            ...
            Cluster.Subscribe(memberProbe.Ref, typeof(ClusterEvent.MemberUp));
    ...
    public void ClusterSingletonManager_should_handover_when_oldest_leaves_in_6_node_cluster()
    {
        Within(TimeSpan.FromSeconds(30), () =>
        {
            ...
            RunOn(() => { Cluster.Leave(GetAddress(leaveNode)); }, leaveNode);
    ...
    public IActorRef Queue
    {
        get
        {
            ...
            Sys.ActorSelection(_controllerRootActorPath / "user" / "queue")
                .Tell(new Identify("queue"), _identifyProbe.Ref);
    ...
    protected ClusterSingletonManagerSpec(ClusterSingletonManagerSpecConfig config) : base(config,
        typeof(ClusterSingletonManagerSpec))
    {
    ...
    private void CreateSingleton()
    {
        Sys.ActorOf(ClusterSingletonManager.Props(
                Props.Create(() => new Consumer(Queue, TestActor)),
                Consumer.End.Instance,
    ...
    private void VerifyMsg(RoleName oldest, int msg)
    {
        ...
        RunOn(() => { ExpectNoMsg(TimeSpan.FromSeconds(1)); },
