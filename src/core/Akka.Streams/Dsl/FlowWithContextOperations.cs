...
public static class FlowWithContextOperations
{
    ...
    {
        var stage = new Collect<(TOut, TCtx), (TOut2, TCtx)>(x =>
        {
    ...
    /// </summary>
    public static FlowWithContext<TIn, TCtx, IReadOnlyList<TOut>, IReadOnlyList<TCtx>, TMat> Grouped<TIn, TCtx, TOut,
        TMat>(
    ...
    ///     Each output group will be associated with a `Seq` of corresponding context elements.
    /// </summary>
    public static FlowWithContext<TIn, TCtx, IReadOnlyList<TOut>, IReadOnlyList<TCtx>, TMat> Sliding<TIn, TCtx, TOut,
        TMat>(
    ...
    {
        return StatefulSelectConcat(flow, () => fn);
    }
