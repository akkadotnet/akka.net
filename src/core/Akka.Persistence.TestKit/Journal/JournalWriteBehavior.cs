...
public class JournalWriteBehavior : JournalRecoveryBehavior
{
    ...
    /// <remarks>
    ///     <para>
    ///         Journal will **NOT** crash, but
    ///         <see cref="Eventsourced.OnPersistRejected">UntypedPersistentActor.OnPersistRejected</see> will be called
    ...
    public Task Reject()
    {
        return SetInterceptorAsync(JournalInterceptors.Rejection.Instance);
    }
    ...
    ///         <list type="bullet">
    ///             <item>
    ///                 If <typeparamref name="TMessage" /> is interface, journal will reject when message implements that
    ///                 interface.
    ///             </item>
    ///             <item>
    ///                 If <typeparamref name="TMessage" /> is class, journal will reject when message can be assigned to
    ...
    ///         <list type="bullet">
    ///             <item>
    ///                 If <paramref name="messageType" /> is interface, journal will reject when message implements that
    ///                 interface.
    ///             </item>
    ///             <item>
    ///                 If <paramref name="messageType" /> is class, journal will reject when message can be assigned to
    ...
    public Task RejectIf(Func<IPersistentRepresentation, bool> predicate)
    {
        ...
        return SetInterceptorAsync(
            new JournalInterceptors.OnCondition(predicate, JournalInterceptors.Rejection.Instance));
    }
    ...
    public Task RejectUnless(Func<IPersistentRepresentation, bool> predicate)
    {
        ...
        return SetInterceptorAsync(
            new JournalInterceptors.OnCondition(predicate, JournalInterceptors.Rejection.Instance, true));
    }
