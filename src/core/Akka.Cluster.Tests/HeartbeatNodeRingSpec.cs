...
public abstract class HeartbeatNodeRingBase : ClusterSpecBase
{
    ...
    private readonly UniqueAddress ff = new(new Address("akka.tcp", "sys", "ff", 2552), 6);
    ...
    public void HeartbeatNodeRing_must_pick_specified_number_of_nodes_plus_unreachable_as_receivers()
    {
        ...
        ring.Receivers(ff).Should()
            .BeEquivalentTo(ImmutableHashSet.Create(aa, bb, cc)); // unreachable dd and ee skipped
    }
    ...
    public void HeartbeatNodeRing_must_pick_all_except_own_as_receivers_when_less_than_total_number_of_nodes()
    {
        ...
        new HeartbeatNodeRing(cc, _nodes, ImmutableHashSet<UniqueAddress>.Empty, 5).MyReceivers.Value.Should()
            .BeEquivalentTo(expected);
        new HeartbeatNodeRing(cc, _nodes, ImmutableHashSet<UniqueAddress>.Empty, 6).MyReceivers.Value.Should()
            .BeEquivalentTo(expected);
        new HeartbeatNodeRing(cc, _nodes, ImmutableHashSet<UniqueAddress>.Empty, 7).MyReceivers.Value.Should()
            .BeEquivalentTo(expected);
    }
