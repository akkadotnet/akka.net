...
public class SnapshotStoreInterceptorsSpec
{
    ...
    {
        await Awaiting(async () => { await SnapshotStoreInterceptors.Noop.Instance.InterceptAsync(null, null); })
    ...
    {
        await Awaiting(async () => { await SnapshotStoreInterceptors.Failure.Instance.InterceptAsync(null, null); })
    ...
    {
        var probe = new InterceptorProbe();
        var onCondition = new SnapshotStoreInterceptors.OnCondition((_, _) => false, probe, true);
