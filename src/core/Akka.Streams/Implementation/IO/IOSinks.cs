...
namespace Akka.Streams.Implementation.IO
{
    ...
    /// </summary>
    internal sealed class FileSink : SinkModule<ReadOnlyMemory<byte>, Task<IOResult>>
    {
        ...
        public FileSink(FileInfo f, long startPosition, FileMode fileMode, Attributes attributes, SinkShape<ReadOnlyMemory<byte>> shape, bool autoFlush, FlushSignaler flushSignaler) : base(shape)
        {
            _f = f;
        ...
        /// <param name="shape">TBD</param>
        /// <returns>TBD</returns>
        protected override SinkModule<ReadOnlyMemory<byte>, Task<IOResult>> NewInstance(SinkShape<ReadOnlyMemory<byte>> shape)
        ...
        public override object Create(MaterializationContext context, out Task<IOResult> materializer)
        {
            ...
            return new ActorSubscriberImpl<ReadOnlyMemory<byte>>(actorRef);
        }
    }
    ...
    /// (creating it before hand if necessary).
    /// </summary>
    internal sealed class OutputStreamSink : SinkModule<ReadOnlyMemory<byte>, Task<IOResult>>
    {
        ...
        public OutputStreamSink(Func<Stream> createOutput, Attributes attributes, SinkShape<ReadOnlyMemory<byte>> shape, bool autoFlush) : base(shape)
        {
