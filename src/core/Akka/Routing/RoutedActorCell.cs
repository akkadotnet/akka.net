...
internal class RoutedActorCell : ActorCell
{
    ...
    {
        foreach (var routee in routees) Watch(routee);
        var r = Router;
    ...
    private void StopIfChild(Routee routee)
    {
        if (routee is ActorRefRoutee actorRefRoutee &&
            TryGetChildStatsByName(actorRefRoutee.Actor.Path.Name, out var childActorStats))
    ...
    {
        var instance = RouterConfig.CreateRouterActor();
        return instance;
    }
}