...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
            await queue.OfferAsync(
                1); // need to wait when first offer is done as initialization can be done in this moment
    ...
    public async Task QueueSource_should_complete_watching_future_with_failure_if_materializer_shut_down()
    {
        ...
        {
            var tempMap =
                ActorMaterializer.Create(Sys,
                    ActorMaterializerSettings
                        .Create(Sys)); // need to create a new materializer to be able to shutdown it
            var s = this.CreateManualSubscriberProbe<int>();
