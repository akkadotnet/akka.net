...
public class PersistentClusterShardingGracefulShutdownSpec : ClusterShardingGracefulShutdownSpec
{
    public PersistentClusterShardingGracefulShutdownSpec()
        : base(new PersistentClusterShardingGracefulShutdownSpecConfig(),
            typeof(PersistentClusterShardingGracefulShutdownSpec))
    {
    }
}
...
public abstract class
    ClusterShardingGracefulShutdownSpec : MultiNodeClusterShardingSpec<ClusterShardingGracefulShutdownSpecConfig>
{
    [MultiNodeFact]
    ...
    private void Cluster_sharding_must_gracefully_shutdown_the_region_on_the_newest_node()
    {
        Within(TimeSpan.FromSeconds(30), () =>
        {
            RunOn(() =>
            {
                ...
                CoordinatedShutdown.Get(Sys).AddTask("cluster-sharding-shutdown-region", "postpone-actual-stop",
                    async () =>
                    {
    ...
    private void Cluster_sharding_must_gracefully_shutdown_empty_region()
    {
        Within(TimeSpan.FromSeconds(30), () =>
        {
            RunOn(() =>
            {
                var regionEmpty = StartSharding("EntityEmpty");
    ...
    {
        base.Join(from, to);
        RunOn(() => { StartSharding(typeName); }, from);
