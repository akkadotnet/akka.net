...
internal class RemoteConnection
{
    private static void ApplyChannelPipeline(IChannel channel, IChannelHandler handler)
    {
        var encoders = new IChannelHandler[]
        {
            ...
            new LengthFieldBasedFrameDecoder(ByteOrder.LittleEndian, 10000, 0, 4, 0, 4, true)
        };
        var protobuf = new IChannelHandler[] { new ProtobufEncoder(), new ProtobufDecoder(Wrapper.Parser) };
    ...
    {
        if (_clientPool == null) _clientPool = new MultithreadEventLoopGroup(poolSize);
        return _clientPool;
    }
    ...
    {
        if (_serverPool == null) _serverPool = new MultithreadEventLoopGroup(poolSize);
        return _serverPool;
    }
    ...
    {
        if (_serverWorkerPool == null) _serverWorkerPool = new MultithreadEventLoopGroup(poolSize);
        return _serverWorkerPool;
    }

    public static Task<IChannel> CreateConnection(Role role, IPEndPoint socketAddress, int poolSize,
        IChannelHandler upstreamHandler)
    {
        if (role == Role.Client)
        {
    ...
    {
        var tc = _clientPool?.ShutdownGracefullyAsync() ?? TaskEx.Completed;
        var ts = _serverPool?.ShutdownGracefullyAsync() ?? TaskEx.Completed;
