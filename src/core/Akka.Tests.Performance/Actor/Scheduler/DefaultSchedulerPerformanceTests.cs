...
public class DefaultSchedulerPerformanceTests
{
    ...
    [PerfBenchmark(
        Description = "Tests to see how many concurrent jobs we can schedule and what the effects are on throughput",
        RunMode = RunMode.Iterations, NumberOfIterations = IterationCount)]
    ...
    {
        for (var i = 0; i < DegreeOfParallelism; i++) Task.Factory.StartNew(_eventLoop);
