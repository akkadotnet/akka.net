...
public class LazySourceSpec : AkkaSpec
{
    ...
    [Fact]
    public async Task
        A_lazy_source_must_fail_the_materialized_value_when_downstream_cancels_without_ever_consuming_any_element()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
    ...
    [Fact]
    public async Task
        A_lazy_source_must_propagate_downstream_cancellation_cause_when_inner_source_has_been_materialized()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
