    ...
    : GraphDelegate<FlowShape<(TIn, TCtxIn), (TOut, TCtxOut)>, TMat>
{
    ...
    {
        return FlowWithContext.From(Flow.FromGraph(Inner).Via(viaFlow));
    }
    ...
    {
        return FlowWithContext.From(Flow.FromGraph(Inner).ViaMaterialized(viaFlow, combine));
    }
    ...
    {
        return FlowWithContext.From(Flow.FromGraph(Inner).MapMaterializedValue(combine));
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Flow<(TIn, TCtxIn), (TOut, TCtxOut), TMat> AsFlow()
    {
        return Flow.FromGraph(Inner);
    }
}
...
{
    /// <summary>
    ///     Creates an "empty" <see cref="FlowWithContext{TIn,TCtxIn,TOut,TCtxOut,TMat}" /> that passes elements through with
    ...
    {
        return new FlowWithContext<TIn, TCtxIn, TOut, TCtxOut, TMat>(flow);
    }
}