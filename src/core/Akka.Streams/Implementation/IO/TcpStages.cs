...
///     INTERNAL API
/// </summary>
internal sealed class ConnectionSourceStage : GraphStageWithMaterializedValue<SourceShape<StreamTcp.IncomingConnection>,
    Task<StreamTcp.ServerBinding>>
{
    ...
    /// <returns>TBD</returns>
    public override ILogicAndMaterializedValue<Task<StreamTcp.ServerBinding>> CreateLogicAndMaterializedValue(
    ...
    private sealed class ConnectionSourceStageLogic : TimerGraphStageLogic, IOutHandler
    {
        ...
        private bool _unbindStarted;

        public ConnectionSourceStageLogic(Shape shape, ConnectionSourceStage stage,
            TaskCompletionSource<StreamTcp.ServerBinding> bindingPromise)
            : base(shape)
        {
            _stage = stage;
        ...
        public override void PreStart()
        {
            ...
            _stage._tcpManager.Tell(
                new Tcp.Bind(StageActor.Ref, _stage._endpoint, _stage._backlog, _stage._options, true), StageActor.Ref);
        }
...
public class IncomingConnectionStage : GraphStage<FlowShape<ByteString, ByteString>>
{
    ...
    {
        return $"TCP-from({_remoteAddress})";
    }
}
...
internal static class TcpConnectionStage
{
    ...
    internal readonly struct Outbound : ITcpRole
    {
        ...
        /// <param name="halfClose">TBD</param>
        public Outbound(IActorRef manager, Tcp.Connect connectCmd, TaskCompletionSource<EndPoint> localAddressPromise,
            bool halfClose)
        {
            Manager = manager;
    ...
    internal sealed class TcpStreamLogic : GraphStageLogic
    {
        ...
        public TcpStreamLogic(FlowShape<ByteString, ByteString> shape, ITcpRole role, EndPoint remoteAddress) :
            base(shape)
        {
            _role = role;
            _remoteAddress = remoteAddress;
            ...
            _readHandler = new LambdaOutHandler(
                ...
                cause =>
                {
                    ...
                    {
                        if (Log.IsDebugEnabled)
                            Log.Debug("Closing connection from {0} because downstream cancelled stream without failure",
                    ...
                    {
                        if (Log.IsDebugEnabled)
                            Log.Debug(cause, "Aborting connection from {0} because of downstream failure",
                                (IPEndPoint)_remoteAddress);
            ...
            SetHandler(_bytesOut, DoNothing);
            SetHandler(_bytesIn,
                () =>
                {
        ...
        public override void PreStart()
        {
            ...
            if (_role is Inbound inbound)
            {
                ...
                _connection.Tell(new Tcp.Register(StageActor.Ref, true, false), StageActor.Ref);
                Pull(_bytesIn);
            }
            else
            {
        ...
        private void Connected((IActorRef, object) args)
        {
            ...
            else if (msg is Tcp.Closed)
            {
                CompleteStage();
            }
            else if (msg is Tcp.ConfirmedClosed)
            {
                CompleteStage();
            }
            else if (msg is Tcp.PeerClosed)
            {
                Complete(_bytesOut);
            }
    ...
    GraphStageWithMaterializedValue<FlowShape<ByteString, ByteString>, Task<StreamTcp.OutgoingConnection>>
{
    ...
    /// <param name="inheritedAttributes">TBD</param>
    /// <returns>TBD</returns>
    public override ILogicAndMaterializedValue<Task<StreamTcp.OutgoingConnection>> CreateLogicAndMaterializedValue(
        Attributes inheritedAttributes)
    {
        ...
        var logic = new TcpConnectionStage.TcpStreamLogic(Shape,
            new TcpConnectionStage.Outbound(_tcpManager,
                new Tcp.Connect(_remoteAddress, _localAddress, _options, _connectionTimeout, true), localAddressPromise,
                _halfClose), _remoteAddress);
...
{
    public static BidiFlow<ByteString, ByteString, ByteString, ByteString, NotUsed> Create(TimeSpan idleTimeout,
        EndPoint remoteAddress = null)
    {
        ...
        var fromNetTimeout =
            toNetTimeout
                .Reversed(); // now the bottom flow transforms the exception, the top one doesn't (since that one is "fromNet") 
