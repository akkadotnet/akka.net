...
public class OutputStreamSourceSpec : AkkaSpec
{
        ...
        helper)
    {
        ...
        _bytesArray = new[]
        {
            Convert.ToByte(new Random().Next(256)), Convert.ToByte(new Random().Next(256)),
    ...
    [Fact]
    public async Task
        OutputStreamSource_must_throw_IOException_when_writing_to_the_stream_after_the_subscriber_has_cancelled_the_reactive_stream()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var sourceProbe = CreateTestProbe();
            var (outputStream, probe) = TestSourceStage<ByteString, Stream>
                .Create(new OutputStreamSourceStage(Timeout), sourceProbe)
                .ToMaterialized(this.SinkProbe<ByteString>(), Keep.Both)
                .Run(_materializer);
    ...
    public async Task OutputStreamSource_must_not_leave_blocked_threads()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            if (field == null)
                throw new Exception(
                    $"Failed to retrieve the field `_dataQueue` from class {nameof(OutputStreamAdapter)}");
