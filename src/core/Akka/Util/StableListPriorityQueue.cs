...
public sealed class StableListPriorityQueue
{
    ...
    public void Enqueue(Envelope item)
    {
        ...
        while (ci > 0)
        {
            ...
            if (comparator.Compare(_data[ci], _data[pi]) >= 0)
    ...
    public Envelope Dequeue()
    {
        ...
        while (true)
        {
            ...
            if (rc <= li &&
                comparator.Compare(_data[rc], _data[ci]) <
                0) // if there is a rc (ci + 1), and it is smaller than left child, use the rc instead
                ci = rc;
            if (comparator.Compare(_data[pi], _data[ci]) <= 0)
    ...
    public bool IsConsistent()
    {
        ...
        for (var pi = 0; pi < _data.Count; ++pi) // each parent index
        {
            ...
            if (lci <= li && comparator.Compare(_data[pi], _data[lci]) > 0)
