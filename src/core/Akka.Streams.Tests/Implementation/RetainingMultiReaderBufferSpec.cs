...
public class RetainingMultiReaderBufferSpec
{
    ...
    public void A_RetainingMultiReaderBufferSpec_should_initially_be_empty(int iSize, int mSize, int cursorCount,
        string expected)
    {
    ...
    {
        public Test(int initialSize, int maxSize, int cursorCount) : base(initialSize, maxSize,
            new SimpleCursors(cursorCount))
        {
        }
    ...
    private class StressTestCursor : ICursor
    {
        ...
        {
            return $"cursorNr {_cursorNr}, ix {Cursor}, counter {_counter}";
        }
    }
}