...
public class LWWDictionarySpec
{
    ...
    public void LWWDictionary_must_be_able_to_remove_entry()
    {
        ...
        Assert.Equal(merged1.Merge(m3).Entries,
            ImmutableDictionary.CreateRange(new[]
            {
        ...
        Assert.Equal(m3.Merge(m4).Entries,
            ImmutableDictionary.CreateRange(new Dictionary<string, int> { { "a", 1 }, { "b", 22 }, { "c", 3 } }));
    }
    ...
    public void LWWDictionary_must_be_able_to_work_with_deltas()
    {
        ...
        LWWDictionary<string, int>.Empty.MergeDelta(m1.Delta).MergeDelta(m2.Delta).Entries.Should()
            .BeEquivalentTo(expected);
        LWWDictionary<string, int>.Empty.MergeDelta(m2.Delta).MergeDelta(m1.Delta).Entries.Should()
            .BeEquivalentTo(expected);
        ...
        merged1.MergeDelta(m3.Delta).Entries.Should()
            .BeEquivalentTo(new Dictionary<string, int> { { "a", 1 }, { "c", 3 } });
