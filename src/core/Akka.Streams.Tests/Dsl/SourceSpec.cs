...
public class SourceSpec : AkkaSpec
{
    ...
    {
        9227465, 5702887, 3524578, 2178309, 1346269, 832040, 514229, 317811, 196418, 121393, 75025, 46368, 28657,
        17711, 10946, 6765, 4181, 2584, 1597, 987, 610, 377, 233, 144, 89, 55, 34, 21, 13, 8, 5, 3, 2, 1, 1, 0
    };
    ...
    [Fact]
    public async Task
        Composite_Source_must_combine_from_two_inputs_with_CombineMaterialized_and_take_a_materialized_value()
    {
        ...
        queue2.Complete();
        sinkProbe2.ExpectNextN(new[]
        {
            1, 2, 3
        }); //as intSequenceSource is the first in combined source, elements from intSequenceSource come first
        sinkProbe2.ExpectNextN(new[]
        {
            10, 20, 30
        }); // after intSequenceSource run out elements, queueSource elements come
    }
    ...
    {
        EventFilter.Exception<Exception>("expected").ExpectOne(() =>
        {
    ...
    public async Task A_ZipN_Source_must_properly_ZipN()
    {
        var sources = new[]
        {
            Source.From(new[] { 1, 2, 3 }), Source.From(new[] { 10, 20, 30 }), Source.From(new[] { 100, 200, 300 })
        };
        ...
        complete.Should().BeEquivalentTo(new[] { 1, 10, 100 }, new[] { 2, 20, 200 }, new[] { 3, 30, 300 });
    }
