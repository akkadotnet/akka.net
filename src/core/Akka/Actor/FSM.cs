...
public abstract class FSMBase : ActorBase
{
    ...
    public sealed class CurrentState<TS>
    {
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                return ((FsmRef != null ? FsmRef.GetHashCode() : 0) * 397) ^
                       EqualityComparer<TS>.Default.GetHashCode(State);
            }
        }
    ...
    public sealed class Transition<TS>
    {
        ...
        {
            return Equals(FsmRef, other.FsmRef) && EqualityComparer<TS>.Default.Equals(From, other.From) &&
                   EqualityComparer<TS>.Default.Equals(To, other.To);
        }
    ...
    /// <summary>
    ///     Send this to an <see cref="SubscribeTransitionCallBack" /> to request first the
    ...
    public sealed class State<TS, TD> : IEquatable<State<TS, TD>>
    {
        ...
        public State(TS stateName, TD stateData, TimeSpan? timeout = null, Reason stopReason = null,
            IReadOnlyList<object> replies = null, bool notifies = true)
        {
        ...
        public bool Equals(State<TS, TD> other)
        {
            ...
            return EqualityComparer<TS>.Default.Equals(StateName, other.StateName) &&
                   EqualityComparer<TD>.Default.Equals(StateData, other.StateData) && Timeout.Equals(other.Timeout) &&
                   Equals(StopReason, other.StopReason) && Equals(Replies, other.Replies);
        }
...
public abstract class FSM<TState, TData> : FSMBase, IListeners, IInternalSupportsTestFSMRef<TState, TData>
{
    ...
    public sealed class TransformHelper
    {
        ...
        {
            State<TState, TData> ContinuedDelegate(Event<TData> @event)
            {
                return andThen.Invoke(Func.Invoke(@event));
            }
    ...
    {
        get => _handleEvent ??= HandleEventDefault;
    ...
    {
        foreach (var tran in _transitionEvent) tran.Invoke(previous, next);
    }
    ...
    protected override bool Receive(object message)
    {
        switch (message)
        {
            ...
            {
                if (_generation == timeoutMarker.Generation) ProcessMsg(StateTimeout.Instance, "state timeout");
                return true;
            }
            case Timer timer:
            {
                ...
                _generation++;
                if (!timer.Repeat) _timers.Remove(timer.Name);
    ...
    private void ProcessEvent(Event<TData> fsmEvent, object source)
    {
        ...
        if (stateFunc != null) nextState = stateFunc(fsmEvent);

        if (nextState == null) nextState = HandleEvent(fsmEvent);

        ApplyState(nextState);

        if (DebugEvent && !Equals(oldState, nextState)) _log.Debug("transition {0} -> {1}", oldState, nextState);
    }
    ...
    private void ApplyState(State<TState, TData> nextState)
    {
        ...
        {
            for (var i = nextState.Replies.Count - 1; i >= 0; i--) Sender.Tell(nextState.Replies[i]);
            Terminate(nextState);
            Context.Stop(Self);
        }
    }
    ...
    private void MakeTransition(State<TState, TData> nextState)
    {
        ...
        else
        {
            ...
            // avoid boxing
            if (!EqualityComparer<TState>.Default.Equals(_currentState.StateName, nextState.StateName) ||
                nextState.Notifies)
            {
