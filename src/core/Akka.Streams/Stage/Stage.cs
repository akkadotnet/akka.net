...
///     <see cref="FlowOperations.Transform{TIn,TOut1,TOut2,TMat}" /> to extend the <see cref="FlowOperations" /> API when
///     there
...
/// <summary>
///     <para>
///         <see cref="PushPullStage{TIn,TOut}" /> implementations participate in 1-bounded regions. For every external
///         non-completion signal these
///         stages produce *exactly one* push or pull signal.
///     </para>
///     <para>
///         <see cref="AbstractStage{TIn,TOut}.OnPush" /> is called when an element from upstream is available and there is
///         demand from downstream, i.e.
///         in <see cref="AbstractStage{TIn,TOut}.OnPush" /> you are allowed to call <see cref="IContext.Push" /> to emit
///         one element downstream, or you can absorb the
///         element by calling <see cref="IContext.Pull" />. Note that you can only emit zero or one element downstream
///         from <see cref="AbstractStage{TIn,TOut}.OnPull" />.
///         To emit more than one element you have to push the remaining elements from
///         <see cref="AbstractStage{TIn,TOut}.OnPush" />, one-by-one.
///         <see cref="AbstractStage{TIn,TOut}.OnPush" /> is not called again until
///         <see cref="AbstractStage{TIn,TOut}.OnPull" /> has requested more elements with <see cref="IContext.Pull" />.
///     </para>
///     <para>
///         <see cref="StatefulStage{TIn,TOut}" /> has support for making it easy to emit more than one element from
///         <see cref="AbstractStage{TIn,TOut}.OnPush" />.
///     </para>
///     <para>
///         <see cref="AbstractStage{TIn,TOut}.OnPull" />> is called when there is demand from downstream, i.e. you are
///         allowed to push one element
///         downstream with <see cref="IContext.Push" />, or request elements from upstreams with
///         <see cref="IContext.Pull" />. If you
///         always perform transitive pull by calling <see cref="IContext.Pull" /> from
///         <see cref="AbstractStage{TIn,TOut}.OnPull" /> you can use
///         <see cref="PushStage{TIn,TOut}" /> instead of <see cref="PushPullStage{TIn,TOut}" />.
///     </para>
///     <para>
///         Stages are allowed to do early completion of downstream and cancel of upstream. This is done with
///         <see cref="StatefulStage.Finish" />,
...
///         immediately finish. This combination is exposed as <see cref="IContext.PushAndFinish" /> which enables stages
///         to
...
///         <see cref="IContext.Push" /> as a response to <see cref="AbstractStage{TIn,TOut}.OnUpstreamFinish(IContext)" />
///         or <see cref="AbstractStage{TIn,TOut}.OnUpstreamFailure(Exception,IContext)" /> will very likely break
///         boundedness
///         and result in a buffer overflow somewhere. Therefore the only allowed command in this case is
///         <see cref="IContext.AbsorbTermination" /> which stops the propagation of the termination signal, and puts the
///         stage in a
///         <see cref="IContext.IsFinishing" /> state. Depending on whether the stage has a pending pull signal it
///         has not yet "consumed" by a push its <see cref="AbstractStage{TIn,TOut}.OnPull" /> handler might be called
///         immediately or later. From
///         <see cref="AbstractStage{TIn,TOut}.OnPull" /> final elements can be pushed before completing downstream with
///         <see cref="StatefulStage.Finish" /> or
...
///     </para>
///     <para>
///         All these rules are enforced by types and runtime checks where needed. Always return the
///         <see cref="Directive" />
///         from the call to the <see cref="IContext" /> method, and do only call <see cref="IContext" /> commands once per
///         callback.
///     </para>
/// </summary>
...
public abstract class
    PushPullStage<TIn, TOut> : AbstractStage<TIn, TOut, ISyncDirective, ISyncDirective, IContext<TOut>>
{
}

/// <summary>
///     <see cref="PushStage{TIn,TOut}" /> is a <see cref="PushPullStage{TIn,TOut}" /> that always perform transitive pull
///     by calling <see cref="IContext.Pull" /> from <see cref="OnPull" />.
/// </summary>
/// <typeparam name="TIn">TBD</typeparam>
/// <typeparam name="TOut">TBD</typeparam>
[Obsolete("Please use GraphStage instead. [1.1.0]")]
...
///     <see cref="FlowOperations.Expand{TIn,TOut1,TOut2,TMat}" /> and
///     <see cref="FlowOperations.Conflate{TIn,TOut,TMat}" />.
...
///     answer an <see cref="AbstractStage{TIn,TOut}.OnPull" /> with a <see cref="IContext.Pull" /> or answer an
///     <see cref="AbstractStage{TIn,TOut}.OnPush" /> with a <see cref="IContext.Push" /> since such an action
...
///     However, DetachedStages have the ability to call <see cref="IDetachedContext.HoldUpstream" /> and
///     <see cref="IDetachedContext.HoldDownstream" /> as a response to
///     <see cref="AbstractStage{TIn,TOut}.OnPush" /> and <see cref="AbstractStage{TIn,TOut}.OnPull" /> which temporarily
///     takes the signal off and
...
[Obsolete("Please use GraphStage instead. [1.1.0]")]
public abstract class
    DetachedStage<TIn, TOut> : AbstractStage<TIn, TOut, IUpstreamDirective, IDownstreamDirective,
...
/// <summary>
///     <see cref="StatefulStage{TIn,TOut}" /> is a <see cref="PushPullStage{TIn,TOut}" /> that provides convenience to
///     make some things easier.
///     The behavior is defined in <see cref="StageState{TIn,TOut}" /> instances. The initial behavior is specified
///     by subclass implementing the <see cref="Initial" /> method. The behavior can be changed by using
///     <see cref="Become" />.
///     Use <see cref="Emit(IEnumerator{TOut},IContext{TOut},StageState{TIn,TOut})" /> or <see cref="EmitAndFinish" /> to
///     push more than one element from <see cref="StageState{TIn,TOut}.OnPush" /> or
///     <see cref="StageState{TIn,TOut}.OnPull" />.
///     Use <see cref="TerminationEmit" /> to push final elements from <see cref="OnUpstreamFinish" /> or
///     <see cref="AbstractStage{TIn,TOut}.OnUpstreamFailure" />.
/// </summary>
/// <typeparam name="TIn">TBD</typeparam>
/// <typeparam name="TOut">TBD</typeparam>
[Obsolete("Please use GraphStage instead. [1.1.0]")]
public abstract class StatefulStage<TIn, TOut> : PushPullStage<TIn, TOut>
{
    ...
    {
        return Current.OnPush(element, context);
    }
    ...
    /// <summary>
    ///     Can be used from <see cref="StageState{TIn,TOut}.OnPush" /> or <see cref="StageState{TIn,TOut}.OnPull" /> to push
    ///     more than one
    ///     element downstream.
    /// </summary>
    ...
    {
        return Emit(enumerator, context, Current);
    }
    ...
    ///     This exception is thrown when either currently in the emitting state or the specified
    ///     <paramref name="enumerator" /> is empty.
    /// </exception>
    /// <returns>TBD</returns>
    ...
    ///     <see cref="AbstractStage{TIn,TOut}.OnUpstreamFailure" /> the failure will be absorbed and the stream will be
    ///     completed
    ///     successfully.
    /// </summary>
    /// <param name="enumerator">TBD</param>
...
internal sealed class EmittingState<TIn, TOut> : StageState<TIn, TOut>
{
    ...
    public EmittingState(IEnumerator<TOut> enumerator, StatefulStage.IAndThen andThen,
        Func<IContext<TOut>, ISyncDirective> onPull)
    {
