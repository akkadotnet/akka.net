...
internal abstract class DeltaPropagationSelector
{
    ...
    private ImmutableDictionary<string, ImmutableSortedDictionary<long, IReplicatedData>> _deltaEntries =
        ImmutableDictionary<string, ImmutableSortedDictionary<long, IReplicatedData>>.Empty;

    private long _deltaNodeRoundRobinCounter;

    private ImmutableDictionary<string, ImmutableDictionary<Address, long>> _deltaSentToNode =
        ImmutableDictionary<string, ImmutableDictionary<Address, long>>.Empty;
    ...
    public void Update(string key, IReplicatedData delta)
    {
        ...
        var deltaEntriesForKey =
            _deltaEntries.GetValueOrDefault(key, ImmutableSortedDictionary<long, IReplicatedData>.Empty);
    ...
    {
        return Math.Min(Math.Max(allNodesSize / GossipInternalDivisor + 1, 2), Math.Min(allNodesSize, 10));
    }
    ...
    public ImmutableDictionary<Address, DeltaPropagation> CollectPropagations()
    {
        ...
        foreach (var node in slice)
        {
            ...
            foreach (var entry in _deltaEntries)
            {
                ...
                var deltaSentToNodeForKey =
                    _deltaSentToNode.GetValueOrDefault(key, ImmutableDictionary<Address, long>.Empty);
    ...
    {
        if (_deltaEntries.TryGetValue(key, out var entries)) return !entries.IsEmpty;
            ...
            _deltaEntries = _deltaEntries.Select(entry =>
                {
                    ...
                    return new KeyValuePair<string, ImmutableSortedDictionary<long, IReplicatedData>>(entry.Key,
                        deltasAfterMin);
                })
    ...
    public void CleanupRemovedNode(Address address)
    {
        _deltaSentToNode = _deltaSentToNode
            .Select(entry =>
                new KeyValuePair<string, ImmutableDictionary<Address, long>>(entry.Key, entry.Value.Remove(address)))
            .ToImmutableDictionary();
    }
    ...
    {
        return entries.Where(e => e.Key > version).ToImmutableSortedDictionary();
    }
