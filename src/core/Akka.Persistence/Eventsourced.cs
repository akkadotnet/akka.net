...
public abstract partial class Eventsourced : ActorBase, IPersistentIdentity, IPersistenceStash, IPersistenceRecovery
{
    ...
    private readonly LinkedList<IPersistentEnvelope> _eventBatch = new();
    ...
    private long _pendingStashingPersistInvocations;
    ...
    public virtual IStashOverflowStrategy InternalStashOverflowStrategy =>
        Extension.DefaultInternalStashOverflowStrategy;
    ...
    {
        SnapshotStore.Tell(new SaveSnapshot(
            new SnapshotMetadata(SnapshotterId, SnapshotSequenceNr, Context.System.Scheduler.Now.Date), snapshot));
    }
    ...
    ///     this handler will receive a <see cref="SnapshotOffer" /> message followed by events that are younger than offer
    ///     itself.
    ...
    ///     is called with the persisted event. This method guarantees that no new commands will be received by a persistent
    ///     actor
    ///     between a call to <see cref="Persist{TEvent}(TEvent,System.Action{TEvent})" /> and execution of its handler. It
    ///     also
    ///     holds multiple persist calls per received command. Internally this is done by stashing. The stash used
    ///     for that is an internal stash which doesn't interfere with the inherited user stash.
    ///     An event <paramref name="handler" /> may close over eventsourced actor state and modify it. Sender of the
    ///     persistent event
    ///     is considered a sender of the corresponding command. That means one can respond to sender from within an event
    ///     handler.
    ...
    public void PersistAllAsync<TEvent>(IEnumerable<TEvent> events, Action<TEvent> handler)
    {
        ...
        foreach (var @event in enumerable) _pendingInvocations.AddLast(new AsyncHandlerInvocation(@event, Inv));
    ...
    ///     The given <paramref name="toSequenceNr" /> must be less than or equal to <see cref="Eventsourced.LastSequenceNr" />
    ///     , otherwise
    ...
    public void DeleteMessages(long toSequenceNr)
    {
        if (toSequenceNr == long.MaxValue || toSequenceNr <= LastSequenceNr)
            Journal.Tell(new DeleteMessagesTo(PersistenceId,
                toSequenceNr == long.MaxValue ? LastSequenceNr : toSequenceNr, Self));
        else
            Self.Tell(new DeleteMessagesFailure(
                new InvalidOperationException(
                    $"toSequenceNr [{toSequenceNr}] must be less than or equal to LastSequenceNr [{LastSequenceNr}]"),
                toSequenceNr));
    }

    /// <summary>
    ///     An <see cref="Eventsourced" /> actor can request cleanup by deleting either a range of, or all persistent events.
    ///     For example, on successful snapshot completion, delete messages within a configurable
    ///     <paramref name="snapshotAfter" />
    ...
    protected virtual void OnRecoveryFailure(Exception reason, object message = null)
    {
        if (message != null)
            Log.Error(reason,
                "Exception in ReceiveRecover when replaying event type [{0}] with sequence number [{1}] for persistenceId [{2}]",
                message.GetType(), LastSequenceNr, PersistenceId);
        else
            Log.Error(reason,
                "Persistence failure when replaying events for persistenceId [{0}]. Last known sequence number [{1}]",
    ...
    {
        Log.Error(cause,
            "Rejected to persist event type [{0}] with sequence number [{1}] for persistenceId [{2}] due to [{3}].",
    ...
    ///     <remarks>
    ///         The actor will be suspended until the task returned by <paramref name="action" /> completes, including the
    ///         <see cref="Eventsourced.Persist{TEvent}(TEvent, Action{TEvent})" />
    ///         and <see cref="Eventsourced.PersistAll{TEvent}(IEnumerable{TEvent}, Action{TEvent})" /> calls.
    ///     </remarks>
    /// </summary>
