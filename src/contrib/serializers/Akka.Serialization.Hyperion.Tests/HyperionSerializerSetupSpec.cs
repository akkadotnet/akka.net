...
");
    ...
    public void Setup_surrogate_should_work()
    {
        ...
        var setup = HyperionSerializerSetup.Empty
            .WithSurrogates(new[]
            {
                Surrogate.Create<Foo, FooSurrogate>(
                    foo =>
                    {
                        surrogated.Add(foo);
    ...
    {
        _output.WriteLine($"Dangerous type: [{type}]");
        var deserializer = new HyperionSerializer((ExtendedActorSystem)Sys,
            HyperionSerializerSettings.Default.WithDisallowUnsafeType(false));
    ...
    public void Setup_TypeFilter_should_filter_types_properly(object sampleObject, bool shouldSucceed)
    {
        ...
        var serializer =
            new HyperionSerializer((ExtendedActorSystem)Sys, deserializer.Settings.WithDisallowUnsafeType(false));
    ...
    public static IEnumerable<object[]> DangerousObjectFactory()
    {
        ...
        if (isWindow)
        {
            ...
            {
                Serialize(new WindowsPrincipal(WindowsIdentity.GetAnonymous())), typeof(WindowsPrincipal)
            };
        }
#if NET48
    ...
    private sealed class CustomTypeProvider : IKnownTypesProvider
    {
        ...
        {
            return new[] { typeof(ClassA), typeof(ClassB) };
        }
    }
