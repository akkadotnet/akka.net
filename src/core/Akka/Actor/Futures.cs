...
public static class Futures
{
    ...
    /// <param name="cancellationToken">TBD</param>
    /// <returns>TBD</returns>
    public static Task<object> Ask(this ICanTell self, object message, TimeSpan? timeout,
    ...
    /// </exception>
    /// <returns>TBD</returns>
    public static Task<T> Ask<T>(this ICanTell self, object message, TimeSpan? timeout,
        CancellationToken cancellationToken)
    {
    ...
    /// </exception>
    /// <returns>TBD</returns>
    public static Task<T> Ask<T>(this ICanTell self, Func<IActorRef, object> messageFactory, TimeSpan? timeout,
        CancellationToken cancellationToken)
    {
        ...
        if (cancellationToken.CanBeCanceled) ctr2 = cancellationToken.Register(() => result.TrySetCanceled());
...
internal sealed class PromiseActorRef : MinimalActorRef
{
    ...
    private readonly AtomicReference<ImmutableHashSet<IActorRef>> _watchedByDoNotCallMeDirectly =
        new(ImmutableHashSet<IActorRef>.Empty);
    ...
    private ImmutableHashSet<IActorRef> WatchedBy => _watchedByDoNotCallMeDirectly.Value;

    public override IActorRefProvider Provider { get; }

    private object State
    {
        get => _stateDoNotCallMeDirectly.Value;
        set => _stateDoNotCallMeDirectly.Value = value;
    }

    /// <inheritdoc cref="InternalActorRefBase.Parent" />
    public override IInternalActorRef Parent => Provider.TempContainer;


    /// <inheritdoc cref="InternalActorRefBase" />
    public override ActorPath Path => GetPath();
    ...
    {
        if (WatchedBy.Contains(watcher)) return false;
    ...
    private void RemoveWatcher(IActorRef watcher)
    {
        while (true)
        {
            if (!WatchedBy.Contains(watcher)) return;
    ...
    {
        if (State is Stopped or StoppedWithPath)
        {
            ...
            case Watch watch:
            {
                if (Equals(watch.Watchee, this))
                {
                    if (!AddWatcher(watch.Watcher))
                        ...
                        watch.Watcher.SendSystemMessage(new DeathWatchNotification(watch.Watchee, true,
                            false));
                    else
        ...
        if (watchers.Any())
            foreach (var watcher in watchers)
                watcher.AsInstanceOf<IInternalActorRef>()
                    .SendSystemMessage(new DeathWatchNotification(watcher, true,
                        false));
    }
     ...
     */
    private readonly AtomicReference<object> _stateDoNotCallMeDirectly = new(null);
        ...
        string messageClassName, CancellationToken cancellationToken = default)
    {
        ...
        if (cancellationToken != default) cancellationToken.Register(CancelAction, result);
