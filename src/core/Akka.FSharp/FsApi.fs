...
module Logging = 
    ...
    let inline private doLog level (mailbox: Actor<'Message>) msg = 
        mailbox.Log.Value.Log(level, msg) |> ignore

    /// Logs a message using configured Akka logger.
    let inline log (level : LogLevel) (mailbox : Actor<'Message>) = 
        kprintf (doLog level mailbox)
     
    /// Logs a message at Debug level using configured Akka logger.
    let inline logDebug mailbox = kprintf (doLog LogLevel.DebugLevel mailbox)
    
    /// Logs a message at Info level using configured Akka logger.
    let inline logInfo mailbox = kprintf (doLog LogLevel.InfoLevel mailbox)
    
    /// Logs a message at Warning level using configured Akka logger.
    let inline logWarning mailbox = kprintf (doLog LogLevel.WarningLevel mailbox)
    
    /// Logs a message at Error level using configured Akka logger. 
    let inline logError mailbox = kprintf (doLog LogLevel.ErrorLevel mailbox)
    
    /// Logs an exception message at Error level using configured Akka logger.
    let inline logException mailbox (e : exn) = doLog LogLevel.ErrorLevel mailbox e.Message
