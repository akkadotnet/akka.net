...
public class RecoveryPermitterSpec : PersistenceSpec
{
    ...
    public void RecoveryPermitter_must_return_permit_when_actor_throws_from_RecoveryCompleted()
    {
        ...
        var persistentActor = Sys.ActorOf(TestPersistentActor.Props("p3", p3.Ref, true));
    ...
    public class TestPersistentActor : UntypedPersistentActor
    {
        ...
        public static Props Props(string name, IActorRef probe, bool throwFromRecoveryCompleted = false)
        {
            return Actor.Props.Create(() => new TestPersistentActor(name, probe, throwFromRecoveryCompleted));
        }
