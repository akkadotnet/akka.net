...
public class WrappedMessage : RouterEnvelope, IEquatable<WrappedMessage>
{
    ...
    public bool Equals(WrappedMessage other)
    {
        return other is not null && other.Message.Equals(Message);
    }
...
public abstract class DistributedPubSubMediatorRouterSpec : AkkaSpec
{
    ...
    private void Keep_the_RouterEnvelope_when_sending_to_local_logical_path<T>(T msg)
    {
        ...
        mediator.Tell(new Send(path, msg, true));
        ExpectMsg(msg);
    ...
    {
        mediator.Tell(new Put(TestActor));
        mediator.Tell(new Send(path, msg, false));
        ExpectMsg(msg);
        mediator.Tell(new Remove(path));
    }
    ...
    private void Keep_the_RouterEnvelope_when_sending_to_topic_for_group<T>(T msg)
    {
        ...
        mediator.Tell(new Publish("topic", msg, true));
        ExpectMsg(msg);
    ...
    private void Send_message_to_dead_letters_if_no_recipients_available<T>(T msg)
    {
        ...
        mediator.Tell(new Publish("nowhere", msg, true));
...
{
    public DistributedPubSubMediatorWithRandomRouterSpec(ITestOutputHelper output) : base(
        DistributedPubSubMediatorRouterConfig.GetConfig("random"), output)
    {
    }
...
{
    public DistributedPubSubMediatorWithRoundRobinRouterSpec(ITestOutputHelper output) : base(
        DistributedPubSubMediatorRouterConfig.GetConfig("round-robin"), output)
    {
    }
...
{
    public DistributedPubSubMediatorWithBroadcastRouterSpec(ITestOutputHelper output) : base(
        DistributedPubSubMediatorRouterConfig.GetConfig("broadcast"), output)
    {
    }
...
{
    public DistributedPubSubMediatorWithHashRouterSpec() : base(
        DistributedPubSubMediatorRouterConfig.GetConfig("consistent-hashing"))
    {
    }
