...
public sealed class Source<TOut, TMat> : IFlow<TOut, TMat>, IGraph<SourceShape<TOut>, TMat>
{
    ...
    /// </summary>
    IFlow<T, TMat3> IFlow<TOut, TMat>.ViaMaterialized<T, TMat2, TMat3>(IGraph<FlowShape<TOut, T>, TMat2> flow,
    ...
    {
        return MapMaterializedValue(mapFunc);
    }
    ...
    /// <returns>TBD</returns>
    public IRunnableGraph<TMat3> ToMaterialized<TMat2, TMat3>(IGraph<SinkShape<TOut>, TMat2> sink,
        Func<TMat, TMat2, TMat3> combine)
    {
    ...
    public Source<TOut2, TMat> Ask<TOut2>(IActorRef actorRef, TimeSpan timeout, int parallelism = 2)
    {
        ...
        var askFlow = Flow.Create<TOut>()
            ...
            {
                var reply = await actorRef.Ask(e, timeout);
                switch (reply)
                {
        ...
        Func<TMat, TMat2, TMat3> combine)
    {
        if (flow.Module == GraphStages.Identity<TOut2>().Module)
        {
            ...
            return MapMaterializedValue(value => combine(value, (TMat2)(object)NotUsed.Instance)) as
                Source<TOut2, TMat3>;
        }
    ...
    /// <returns>
    ///     A tuple containing the (1) materialized value and (2) a new <see cref="Source" />
    ///     that can be used to consume elements from the newly materialized <see cref="Source" />.
    /// </returns>
    public (TMat, Source<TOut, NotUsed>) PreMaterialize(IMaterializer materializer)
    {
        var tup = ToMaterialized(Sink.AsPublisher<TOut>(true), Keep.Both).Run(materializer);
    ...
    {
        var tup = ToMaterialized(Sink.AsPublisher<TOut>(true), Keep.Both).Run(actorSystem);
        return (tup.Item1, Source.FromPublisher(tup.Item2));
    }

    /// <summary>
    ///     Connect this <see cref="Source{TOut,TMat}" /> to a <see cref="Sink{TIn,TMat}" /> and run it. The returned value is
    ///     the materialized value
    ///     of the <see cref="Sink{TIn,TMat}" /> , e.g. the <see cref="IPublisher{TIn}" /> of a
    ///     <see cref="Sink.Publisher{TIn}" />.
    /// </summary>
    /// <typeparam name="TMat2">TBD</typeparam>
    /// <param name="sink">TBD</param>
    ...
    /// <param name="materializer">TBD</param>
    /// <returns>TBD</returns>
    public Task<TOut2> RunAggregateAsync<TOut2>(TOut2 zero, Func<TOut2, TOut, Task<TOut2>> aggregate,
        IMaterializer materializer)
    {
        return RunWith(Sink.AggregateAsync(zero, aggregate), materializer);
    }
    ...
    public IAsyncEnumerable<TOut> RunAsAsyncEnumerable(
        IMaterializer materializer)
    {
        return new StreamsAsyncEnumerableRerunnable<TOut, TMat>(this, materializer);
    }
    ...
    public IAsyncEnumerable<TOut> RunAsAsyncEnumerable(
        ActorSystem materializer)
    {
        return new StreamsAsyncEnumerableRerunnable<TOut, TMat>(this, materializer.Materializer());
    }
    ...
    public IAsyncEnumerable<TOut> RunAsAsyncEnumerableBuffer(
        ...
        int maxBuffer = 16)
    {
        return new StreamsAsyncEnumerableRerunnable<TOut, TMat>(
    ...
    /// <summary>
    ///     Combines several sources with fun-in strategy like <see cref="Merge{TIn,TOut}" /> or
    ///     <see cref="Concat{TIn,TOut}" /> and returns <see cref="Source{TOut,TMat}" />.
    /// </summary>
    ...
    /// <returns>TBD</returns>
    public Source<TOut2, NotUsed> Combine<T, TOut2>(Source<T, NotUsed> first, Source<T, NotUsed> second,
        Func<int, IGraph<UniformFanInShape<T, TOut2>, NotUsed>> strategy, params Source<T, NotUsed>[] rest)
    {
...
public static class Source
{
    ...
    {
        return new Source<T, NotUsed>(new PublisherSource<T>(publisher, DefaultAttributes.PublisherSource,
            Shape<T>("PublisherSource")));
    }
    ...
    ///     <see cref="System.Collections.IEnumerator.MoveNext">IEnumerator&lt;T&gt;.MoveNext</see> method.
    ...
    ///     Starts a new <see cref="Source{TOut,TMat}" /> from the given <see cref="IEnumerable{T}" />. This is like starting
    ///     from an
    ...
    ///     Example usage: Source.From(Enumerable.Range(1, 10))
    ///     Starts a new <see cref="Source{TOut,TMat}" /> from the given <see cref="IAsyncEnumerable{T}" />. This is like
    ///     starting from an
    ///     Enumerator, but every Subscriber directly attached to the Publisher of this
    ///     stream will see an individual flow of elements (always starting from the
    ///     beginning) regardless of when they subscribed.
    ...
    {
        return FromGraph(new AsyncEnumerable<T>(asyncEnumerable)).WithAttributes(DefaultAttributes.EnumerableSource);
    }

    /// <summary>
    ///     Create a <see cref="Source{TOut,TMat}" /> with one element.
    ///     Every connected <see cref="Sink{TIn,TMat}" /> of this stream will see an individual stream consisting of one
    ///     element.
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    ...
    {
        return FromGraph(new SetupSourceStage<T, TMat>(factory));
    }
    ...
    {
        return FromTask(new TaskCompletionSource<T>().Task).WithAttributes(DefaultAttributes.NeverSource);
    }

    /// <summary>
    ///     Streams the elements of the given future source once it successfully completes.
    ///     If the <see cref="Task{T}" /> fails the stream is failed with the exception from the future. If downstream cancels
    ///     before the
    ///     stream completes the materialized <see cref="Task{M}" /> will be failed with a
    ...
    {
        return FromGraph(new TaskFlattenSource<T, M>(task));
    }
    ...
    /// <returns>TBD</returns>
    public static Source<TElem, NotUsed> Unfold<TState, TElem>(TState state,
        Func<TState, Option<(TState, TElem)>> unfold)
    {
    ...
    /// <returns>TBD</returns>
    public static Source<TElem, NotUsed> UnfoldAsync<TState, TElem>(TState state,
        Func<TState, Task<Option<(TState, TElem)>>> unfoldAsync)
    {
        return FromGraph(new UnfoldAsync<TState, TElem>(state, unfoldAsync))
            .WithAttributes(DefaultAttributes.UnfoldAsync);
    }
    ...
    /// <param name="unfold">TBD</param>
    /// <returns>TBD</returns>
    public static Source<TElem, NotUsed> UnfoldInfinite<TState, TElem>(TState state,
        Func<TState, (TState, TElem)> unfold)
    {
        return FromGraph(new UnfoldInfinite<TState, TElem>(state, unfold)).WithAttributes(DefaultAttributes.UnfoldInf);
    }

    /// <summary>
    ///     A <see cref="Source{TOut,TMat}" /> with no elements, i.e. an empty stream that is completed immediately for every
    ///     connected <see cref="Sink{TIn,TMat}" />.
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    /// <returns>TBD</returns>
    public static Source<T, NotUsed> Empty<T>()
    {
        return FromGraph(new EmptySource<T>());
    }

    /// <summary>
    ///     Create a <see cref="Source{TOut,TMat}" /> which materializes a <see cref="TaskCompletionSource{TResult}" /> which
    ...
    /// <summary>
    ///     Create a <see cref="Source{TOut,TMat}" /> that immediately ends the stream with the <paramref name="cause" /> error
    ///     to every connected <see cref="Sink{TIn,TMat}" />.
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    ...
    /// <summary>
    ///     Creates a <see cref="Source{TOut,TMat}" /> that is not materialized until there is downstream demand, when the
    ///     source gets materialized
    ...
    {
        return FromGraph(LazySource.Create(create));
    }
    ...
    /// <summary>
    ///     Creates a <see cref="Source{TOut,TMat}" /> that is materialized to an <see cref="IActorRef" /> which points to an
    ///     Actor
    ...
    ///     This exception is thrown when the specified actor <paramref name="props" /> is not of type
    ///     <see cref="Actors.ActorPublisher{T}" />.
    /// </exception>
    /// <returns>TBD</returns>
    public static Source<T, IActorRef> ActorPublisher<T>(Props props)
    {
        ...
        return new Source<T, IActorRef>(new ActorPublisherSource<T>(props, DefaultAttributes.ActorPublisherSource,
            Shape<T>("ActorPublisherSource")));
    }
    ...
    ///         if there is no demand from downstream. When <paramref name="bufferSize" /> is 0 the
    ///         <paramref name="overflowStrategy" /> does
    ///         not matter. An async boundary is added after this Source; as such, it is never safe to assume the downstream
    ///         will always generate demand.
    ///     </para>
    ///     <para>
    ///         The stream can be completed successfully by sending the actor reference a <see cref="Status.Success" />
    ///         message (whose content will be ignored) in which case already buffered elements will be signaled before
    ...
    ///     This exception is thrown when the specified <paramref name="overflowStrategy" /> is of type
    ///     <see cref="OverflowStrategy.Backpressure" />.
    /// </exception>
    /// <returns>TBD</returns>
    public static Source<T, IActorRef> ActorRef<T>(int bufferSize, OverflowStrategy overflowStrategy)
    {
        if (bufferSize < 0)
            throw new ArgumentException("Buffer size must be greater than or equal 0", nameof(bufferSize));
        ...
        return new Source<T, IActorRef>(new ActorRefSource<T>(bufferSize, overflowStrategy,
            DefaultAttributes.ActorRefSource, Shape<T>("ActorRefSource")));
    }
    ...
    /// <param name="rest">TBD</param>
    /// <returns>TBD</returns>
    public static Source<TOut2, NotUsed> Combine<T, TOut2>(Source<T, NotUsed> first, Source<T, NotUsed> second,
        Func<int, IGraph<UniformFanInShape<T, TOut2>, NotUsed>> strategy, params Source<T, NotUsed>[] rest)
    {
    ...
    /// <summary>
    ///     Combines two sources with fan-in strategy like <see cref="Merge{TIn,TOut}" /> or <see cref="Concat{TIn,TOut}" />
    ///     and returns <see cref="Source{TOut,TMat}" /> with a materialized value.
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    /// <typeparam name="TOut2">TBD</typeparam>
    ...
    /// <returns>TBD</returns>
    public static Source<TOut2, TMatOut> CombineMaterialized<T, TOut2, TMat1, TMat2, TMatOut>(Source<T, TMat1> first,
        Source<T, TMat2> second, Func<int, IGraph<UniformFanInShape<T, TOut2>, NotUsed>> strategy,
    ...
    ///     <see cref="ISourceQueue{T}.OfferAsync">ISourceQueueWithComplete&lt;T&gt;.OfferAsync</see> returns
    ///     <see cref="Task" />
    ...
    ///     The strategy <see cref="OverflowStrategy.Backpressure" /> will not complete
    ///     <see cref="ISourceQueue{T}.OfferAsync">ISourceQueueWithComplete&lt;T&gt;.OfferAsync</see> when buffer is full.
    ...
    public static Source<T, ISourceQueueWithComplete<T>> Queue<T>(int bufferSize, OverflowStrategy overflowStrategy)
    {
        ...
        return FromGraph(
            new QueueSource<T>(bufferSize, overflowStrategy).WithAttributes(DefaultAttributes.QueueSource));
    }
    ...
    ///     </para>
    ///     <para>
    ///         You can use the supervision strategy to handle exceptions for <paramref name="read" /> function. All exceptions
    ///         thrown by <paramref name="create" />
    ///         or <paramref name="close" /> will fail the stream.
    ///     </para>
    ///     <para>
    ///         <see cref="Supervision.Directive.Restart" /> supervision strategy will close and create blocking IO again.
    ///         Default strategy is <see cref="Supervision.Directive.Stop" /> which means
    ...
    ///     is received. Stream calls close and completes when <paramref name="read" /> returns <see cref="Option{T}.None" />.
    /// </param>
    ...
    ///     <para>
    ///         Start a new <see cref="Source{TOut,TMat}" /> from some resource which can be opened, read and closed.
    ///         It's similar to <see cref="UnfoldResource{T,TSource}" /> but takes functions that return <see cref="Task" />s
    ///         instead of plain values.
    ///     </para>
    ///     <para>
    ///         You can use the supervision strategy to handle exceptions for <paramref name="read" /> function or failures of
    ///         produced <see cref="Task" />s.
    ///         All exceptions thrown by <paramref name="create" /> or <paramref name="close" /> as well as fails of returned
    ///         futures will fail the stream.
    ///     </para>
    ///     <para>
    ///         <see cref="Supervision.Directive.Restart" /> supervision strategy will close and create resource .Default
    ///         strategy is <see cref="Supervision.Directive.Stop" /> which means
    ...
    /// <param name="read">
    ///     function that reads data from opened resource. It is called each time backpressure signal
    ///     is received. Stream calls close and completes when <see cref="Task" /> from read function returns None.
    /// </param>
    ...
    /// <summary>
    ///     Start a new <see cref="Source{TOut,TMat}" /> attached to a .NET event. In case when event will be triggered faster,
    ///     than a downstream is able
    ///     to consume incoming events, a buffering will occur. It can be configured via optional
    ///     <paramref name="maxBufferCapacity" /> and
    ...
    ///     A function used to convert provided event handler into a delegate compatible with an
    ///     underlying .NET event type.
    /// </param>
    ...
    /// <param name="maxBufferCapacity">
    ///     Maximum size of the buffer, used in situation when amount of emitted events is higher
    ///     than current processing capabilities of the downstream.
    /// </param>
    /// <param name="overflowStrategy">
    ///     Overflow strategy used, when buffer (size specified by
    ///     <paramref name="maxBufferCapacity" />) has been overflown.
    /// </param>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Start a new <see cref="Source{TOut,TMat}" /> attached to an existing <see cref="IObservable{T}" />. In case when
    ///     upstream (an <paramref name="observable" />)
    ///     is producing events in a faster pace, than downstream is able to consume them, a buffering will occur. It can be
    ///     configured via optional
