...
/// <summary>
///     This spec measures execution times of the basic operations that an <see cref="Akka.Persistence.PersistentActor" />
///     provides,
...
///     if their plugin's performance is roughly as expected. It also validates the plugin still works under "more
///     messages" scenarios.
///     In case your journal plugin needs some kind of teardown, override the `AfterAll` method (don't forget to call
///     `base` in your overridden methods).
/// </summary>
public abstract class JournalPerfSpec : Akka.TestKit.Xunit2.TestKit
{
        ...
        IReadOnlyList<int> commands, int numExpect)
    {
        ...
        for (var i = 0; i < numExpect; i++) autSet.probe.ExpectMsg(commands.Last(), ExpectDuration);
    }
    ...
    {
        var numGroup = 10;
        var numCommands = Math.Max(Math.Min(EventsCount / 10, 1000), 100);
    ...
    {
        var numGroup = 25;
        var numCommands = Math.Max(Math.Min(EventsCount / 25, 1000), 50);
        RunPersistGroupBenchmark(numGroup, numCommands);
    }
    ...
    {
        var numGroup = 50;
        var numCommands = Math.Max(Math.Min(EventsCount / 50, 1000), 20);
        RunPersistGroupBenchmark(numGroup, numCommands);
    }
    ...
    {
        var numGroup = 100;
        var numCommands = Math.Max(Math.Min(EventsCount / 100, 1000), 10);
        RunPersistGroupBenchmark(numGroup, numCommands);
    }
    ...
    {
        var numGroup = 200;
        var numCommands = Math.Max(Math.Min(EventsCount / 200, 500), 10);
        RunPersistGroupBenchmark(numGroup, numCommands);
    }
