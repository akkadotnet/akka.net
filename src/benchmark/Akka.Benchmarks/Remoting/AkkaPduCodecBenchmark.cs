...
using Payload = Akka.Remote.Serialization.Proto.Msg.Payload;
...
public class AkkaPduCodecBenchmark
{
    ...
    private Payload _payloadDecoded;
    ...
    public async Task Setup()
    {
        ...
        _localReceiveRef = _sys1.ActorOf(
            act => { act.ReceiveAny((_, context) => context.Sender.Tell(context.Sender)); },
            "recv1");

        // create an association
        _remoteReceiveRef = (RemoteActorRef)await _sys2.ActorSelection(
            new RootActorPath(RARP.For(_sys1).Provider.DefaultAddress) / "user" /
            _localReceiveRef.Path.Name).ResolveOne(TimeSpan.FromSeconds(3));

        _remoteSenderRef = (RemoteActorRef)await _sys1.ActorSelection(
            new RootActorPath(RARP.For(_sys2).Provider.DefaultAddress) / "user" /
        ...
        _pduDecoded = ((Remote.Transport.Payload)_recvCodec.DecodePdu(_fullDecode)).Bytes;
    ...
    public async Task Cleanup()
    {
        ...
        ConsoleLogger.Default.WriteLine(LogKind.Result,
            $"[Addr1] Used ResolveCache for recipient? {resolveCache.Cache.TryGet(_remoteReceiveRef.Path.ToSerializationFormat(), out _)}");
        ConsoleLogger.Default.WriteLine(LogKind.Result,
            $"[Addr1] Used PathCache for recipient? {pathCache.Cache.TryGet(_remoteReceiveRef.Path.ToSerializationFormat(), out _)}");
        ConsoleLogger.Default.WriteLine(LogKind.Result,
            $"[Addr1] Used ResolveCache for sender? {resolveCache.Cache.TryGet(_remoteSenderRef.Path.ToSerializationFormat(), out _)}");
        ConsoleLogger.Default.WriteLine(LogKind.Result,
            $"[Addr1] Used PathCache for sender? {pathCache.Cache.TryGet(_remoteSenderRef.Path.ToSerializationFormat(), out _)}");

        ConsoleLogger.Default.WriteLine(LogKind.Result,
            $"[Addr2] Used ResolveCache for recipient? {senderResolveCache.Cache.TryGet(_remoteReceiveRef.Path.ToSerializationFormat(), out _)}");
        ConsoleLogger.Default.WriteLine(LogKind.Result,
            $"[Addr2] Used PathCache for recipient? {senderPathCache.Cache.TryGet(_remoteReceiveRef.Path.ToSerializationFormat(), out _)}");
        ConsoleLogger.Default.WriteLine(LogKind.Result,
            $"[Addr2] Used ResolveCache for sender? {senderResolveCache.Cache.TryGet(_senderActorRef.Path.ToSerializationFormat(), out _)}");
        ConsoleLogger.Default.WriteLine(LogKind.Result,
            $"[Addr2] Used PathCache for sender? {senderPathCache.Cache.TryGet(_senderActorRef.Path.ToSerializationFormat(), out _)}");

        ConsoleLogger.Default.WriteLine(LogKind.Result,
            $"[Addr1] Used AddressCache for sys1? {addressCache.Cache.TryGet(_addr2.ToString(), out _)}");
        ConsoleLogger.Default.WriteLine(LogKind.Result,
            $"[Addr2] Used AddressCache for sys2? {senderAddressCache.Cache.TryGet(_addr1.ToString(), out _)}");
    ...
    {
        for (var i = 0; i < Operations; i++) CreatePayloadPdu();
    }
    ...
    public void DecodePayloadPdu()
    {
        for (var i = 0; i < Operations; i++)
        {
            ...
            if (pdu is Remote.Transport.Payload p)
            {
    ...
    {
        return _sendCodec.ConstructPayload(_sendCodec.ConstructMessage(_remoteReceiveRef.LocalAddressToUse,
            _remoteReceiveRef,
