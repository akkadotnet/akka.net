...
public class SplitBrainStrategySpec
{
    ...
    private static Member Member(Address address, int upNumber = 1, MemberStatus status = MemberStatus.Up,
        string role = null)
    {
        return new Member(new UniqueAddress(address, ThreadLocalRandom.Current.Next()), upNumber, status,
            role == null ? ImmutableHashSet<string>.Empty : ImmutableHashSet.Create(role), AppVersion.Zero);
    }

    private static ImmutableSortedSet<Member> Members(params Member[] members)
    {
        return ImmutableSortedSet.CreateRange(Akka.Cluster.Member.AgeOrdering, members);
    }
    ...
    public void StaticQuorum_must_down_unreachable_nodes_if_remaining_size_is_equal_quorum_size()
    {
        ...
        var strategy = new StaticQuorum(4, null);
    ...
    public void StaticQuorum_must_down_unreachable_nodes_if_remaining_size_is_greater_quorum_size()
    {
        ...
        var strategy = new StaticQuorum(3, null);
        strategy.Apply(new NetworkPartitionContext(unreachable, remaining)).Should().Equal(unreachable);
    }
    ...
    public void StaticQuorum_must_limit_node_counts_to_role_if_provided()
    {
        ...
        var strategy = new StaticQuorum(3, role);
    ...
    {
        var unreachable = Members(Member(e, 1), Member(d, 5));
        var remaining = Members(Member(a, 2), Member(b, 3), Member(c, 4));

        var strategy = new KeepOldest(false);
        strategy.Apply(new NetworkPartitionContext(unreachable, remaining)).Should().Equal(remaining);
    }
    ...
    {
        var unreachable = Members(Member(e, 2), Member(d, 5));
        var remaining = Members(Member(a, 1), Member(b, 3), Member(c, 4));
    ...
    {
        const string role = "test";
        var unreachable = Members(Member(e, 2, role: role), Member(d, 5));
        var remaining = Members(Member(a, 1), Member(b, 3, role: role), Member(c, 4, role: role));

        var strategy = new KeepOldest(false, role);
        strategy.Apply(new NetworkPartitionContext(unreachable, remaining)).Should().Equal(remaining);
    }
    ...
    {
        const string role = "test";
        var unreachable = Members(Member(e, 1), Member(d, 3, role: role));
        var remaining = Members(Member(a, 2, role: role), Member(b, 5, role: role), Member(c, 4));
    ...
    {
        var unreachable = Members(Member(e, 1));
        var remaining = Members(Member(a, 2), Member(b, 3), Member(c, 4));

        var strategy = new KeepOldest(true);
        strategy.Apply(new NetworkPartitionContext(unreachable, remaining)).Should().Equal(unreachable);
    }
    ...
    {
        var unreachable = Members(Member(e, 2), Member(b, 3), Member(c, 4));
        var remaining = Members(Member(a, 1));
    ...
    public void KeepReferee_must_down_remaining_if_referee_node_was_unreachable()
    {
        ...
        var strategy = new KeepReferee(referee, 1);
        strategy.Apply(new NetworkPartitionContext(unreachable, remaining)).Should().Equal(remaining);
    }
    ...
    [Fact]
    public void
        KeepReferee_must_down_all_nodes_if_referee_node_was_in_remaining_but_DownAllIfLessThanNodes_was_not_reached()
    {
        var referee = a;
        var unreachable = Members(Member(d), Member(e), Member(c));
        var remaining = Members(Member(referee), Member(b));

        var strategy = new KeepReferee(referee, 3);
        strategy.Apply(new NetworkPartitionContext(unreachable, remaining)).Should()
            .Equal(unreachable.Union(remaining));
    }
}