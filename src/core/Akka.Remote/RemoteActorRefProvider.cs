...
public class RemoteActorRefProvider : IRemoteActorRefProvider
{
    ...
    public RemoteActorRefProvider(string systemName, Settings settings, EventStream eventStream)
    {
        ...
        IInternalActorRef DeadLettersFactory(ActorPath path)
        {
            return new RemoteDeadLetterActorRef(this, path, eventStream);
        }
    ...
    private Internals RemoteInternals { get; set; }
    ...
    ///     The remote death watcher.
    /// </summary>
    public IActorRef RemoteWatcher { get; private set; }
    ...
    public virtual void Init(ActorSystemImpl system)
    {
        ...
        RemoteInternals = CreateInternals();
        ...
        bool systemService, Deploy deploy, bool lookupDeploy, bool async)
    {
        ...
        var deployment = new List<Deploy> { deploy, configDeploy }.Where(x => x != null)
        ...
        if (propsDeploy.Scope is RemoteScope remoteScope)
        {
            ...
            if (HasAddress(addr)) return LocalActorOf(system, props, supervisor, path, false, deployment, false, async);

            //check for correct scope configuration
            if (props.Deploy.Scope is LocalScope)
                throw new ConfigurationException(
                    $"configuration requested remote deployment for local-only Props at {path}");
            ...
            try
            {
                ...
                var rpath = (new RootActorPath(addr) / "remote" / localAddress.Protocol / localAddress.HostPort() /
                             path.Elements.ToArray()).WithUid(path.Uid);
    ...
    {
        if (HasAddress(actorPath.Address)) return _local.ResolveActorRef(RootGuardian, actorPath.ElementsWithUid);
        try
        {
    ...
    /// <returns>An <see cref="IInternalActorRef" /> instance.</returns>
    protected virtual IInternalActorRef CreateRemoteRef(Props props, IInternalActorRef supervisor, Address localAddress,
        ActorPath rpath, Deploy deployment)
    {
    ...
    private class Internals : INoSerializationVerificationNeeded
    {
        ...
        public Internals(RemoteTransport transport, Akka.Serialization.Serialization serialization,
            IInternalActorRef remoteDaemon)
        {
            Transport = transport;
    ...
    private class RemoteDeadLetterActorRef : DeadLetterActorRef
    {
        ...
        protected override void TellInternal(object message, IActorRef sender)
        {
            ...
            {
                if (send.Seq == null) base.TellInternal(send.Message, send.SenderOption ?? ActorRefs.NoSender);
            }
