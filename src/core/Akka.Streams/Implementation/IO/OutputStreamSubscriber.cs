...
internal sealed class OutputStreamSubscriber : ActorSubscriber
{
    ...
    /// <see cref="OutputStreamSubscriber.Props" />
    /// as well!
    public OutputStreamSubscriber(Stream outputStream, TaskCompletionSource<IOResult> completionPromise, int bufferSize,
        bool autoFlush)
    {
        _outputStream = outputStream;
        _completionPromise = completionPromise;
        _autoFlush = autoFlush;
        RequestStrategy = new WatermarkRequestStrategy(bufferSize);
    ...
    /// <returns>TBD</returns>
    public static Props Props(Stream os, TaskCompletionSource<IOResult> completionPromise, int bufferSize,
        bool autoFlush)
    {
        if (bufferSize <= 0)
    ...
    protected override bool Receive(object message)
    {
        switch (message)
        {
            case OnNext next:
                ...
                return true;
            case OnError error:
                _log.Error(error.Cause, "Tearing down OutputStreamSink due to upstream error, wrote bytes: {0}",
                    _bytesWritten);
                _completionPromise.TrySetException(new AbruptIOTerminationException(IOResult.Success(_bytesWritten),
                    error.Cause));
                Context.Stop(Self);
                return true;
            case OnComplete _:
