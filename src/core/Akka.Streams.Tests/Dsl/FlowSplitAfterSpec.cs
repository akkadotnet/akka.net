...
public class FlowSplitAfterSpec : AkkaSpec
{
    ...
    [Fact]
    public async Task SplitAfter_should_work_when_last_element_is_split_by()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
            WithSubstreamsSupport(3, 3,
                run: (masterSubscriber, masterSubscription, expectSubFlow) =>
                {
                    var s1 = new StreamPuppet(expectSubFlow()
                        .RunWith(Sink.AsPublisher<int>(false), Materializer), this);
    ...
    [Fact]
    public async Task SplitAfter_should_fail_stream_if_substream_not_materialized_in_time()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
            var timeout = new StreamSubscriptionTimeoutSettings(
                StreamSubscriptionTimeoutTerminationMode.CancelTermination, TimeSpan.FromMilliseconds(500));
    ...
    public async Task SplitAfter_should_not_create_a_subflow_when_no_element_is_left()
    {
        ...
        Source.From(new[] { (true, 1), (true, 2), (false, 0), (true, 3), (true, 4), (false, 0), (true, 5), (false, 0) })
    ...
    private sealed class StreamPuppet
    {
        ...
        public void ExpectNext(int element)
        {
            _probe.ExpectNext(element);
        }

        public void ExpectNoMsg(TimeSpan max)
        {
            _probe.ExpectNoMsg(max);
        }

        public void ExpectComplete()
        {
            _probe.ExpectComplete();
        }

        public void ExpectError(Exception ex)
        {
            _probe.ExpectError().Should().Be(ex);
        }
