...
public class DistributedPubSubPublishWithAckSpec : AkkaSpec
{
    ...
    public async Task PublishWithAckMissingRemoteSubscriber()
    {
        ...
        var probeActor = sys2.ActorOf(Props.Create(() => new ProbeActor(probe)));
        ...
        mediator.Tell(new PublishWithAck("topic", "message", 5.Seconds()), TestActor);
        await ExpectMsgAsync<PublishSucceeded>();
