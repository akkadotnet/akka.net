...
public class HotSwapSpec : AkkaSpec
{
    ...
    private class ConstructorBecomer : ReceiveActor
    {
    ...
    private class MultipleConstructorBecomer : ReceiveActor
    {
    ...
    private class StackingConstructorBecomer : ReceiveActor
    {
    ...
    private class MultipleStackingConstructorBecomer : ReceiveActor
    {
    ...
    private class HotSwapWithBecome : ReceiveActor
    {
        public HotSwapWithBecome()
        {
            Receive<string>(m => "init".Equals(m), _ => Sender.Tell("init"));
            Receive<string>(m => "swap".Equals(m), _ => { Become(() => Receive<string>(x => Sender.Tell(x))); });
        }
    }

    private class HotSwapRevertUnBecome : ReceiveActor
    {
    ...
    private class RevertToInitialState : ReceiveActor
    {
        public RevertToInitialState()
        {
            ...
            Receive<string>(m => "swap".Equals(m), _ =>
            {
                BecomeStacked(() =>
                {
                    ...
                    Receive<string>(x => "crash".Equals(x), _ => { Crash(); });
                });
                Sender.Tell("swapped");
            });
        }
