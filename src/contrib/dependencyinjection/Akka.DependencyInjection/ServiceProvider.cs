...
public sealed class ServiceProvider : IExtension
{
    ...
    {
        return Actor.Props.CreateBy(new ServiceProviderActorProducer<T>(Provider, args));
    }
}
    ...
    public override ServiceProvider CreateExtension(ExtendedActorSystem system)
    {
        ...
        if (!setup.HasValue)
        {
            var exception = new ConfigurationException(
                "Unable to find [ServiceProviderSetup] included in ActorSystem settings." +
                " Please specify one before attempting to use dependency injection inside Akka.NET.");
            system.EventStream.Publish(new Error(exception, "Akka.DependencyInjection",
                typeof(ServiceProviderExtension), exception.Message));
            throw exception;
        }
