...
lease-a {
  ...
  lease-fallback-to-defaults {
    ...
    {
        Assert.Throws<ArgumentException>(() => { LeaseProvider.Get(Sys).GetLease("x", "lease-missing", "owner1"); })
    ...
    {
        Assert.Throws<TypeLoadException>(() => { LeaseProvider.Get(Sys).GetLease("x", "lease-unknown", "owner1"); });
    }
