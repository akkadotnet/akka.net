...
public class RemoteScatterGatherSpec : MultiNodeSpec
{
    ...
    protected RemoteScatterGatherSpec(RemoteScatterGatherMultiNetSpec config) : base(config,
        typeof(RemoteScatterGatherSpec))
    {
        _config = config;
    }
        ...
        A_remote_ScatterGatherFirstCompleted_pool_must_be_locally_instantiated_on_a_remote_node_and_be_able_to_communicate_through_its_RemoteActorRef()
    {
        ...
        RunOn(() => { EnterBarrier("start", "broadcast-end", "end", "done"); }, _config.First, _config.Second,
            _config.Third);

        RunOn(() =>
        {
            EnterBarrier("start");
            var actor = Sys.ActorOf(new ScatterGatherFirstCompletedPool(1, TimeSpan.FromSeconds(10))
