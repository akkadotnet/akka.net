...
public sealed class ReplicatedDataSerializer : SerializerWithStringManifest
{
        ...
        switch (obj)
        {
            ...
            default:
                throw new ArgumentException(
                    $"Can't serialize object of type [{obj.GetType().FullName}] in [{GetType().FullName}]");
        }
    }
    ...
    public override string Manifest(object o)
    {
        switch (o)
        {
            ...
            default:
                throw new ArgumentException(
                    $"Can't serialize object of type [{o.GetType().FullName}] in [{GetType().FullName}]");
        }
    }
    ...
    {
        var b = new Proto.Msg.ORSet { TypeInfo = new TypeDescriptor() };
    ...
    private static readonly MethodInfo ORSetMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(ToGenericORSet),
    ...
    private static readonly MethodInfo ORSetUnknownMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(ORSetUnknownToProto),
    ...
    private ORSetDeltaGroup ToProto(ORSet.IDeltaGroupOperation orset)
    {
        var deltaGroup = new ORSetDeltaGroup();
        ...
        foreach (var op in orset.OperationsSerialization)
            switch (op)
            {
                case ORSet.IAddDeltaOperation add:
                    deltaGroup.Entries.Add(new ORSetDeltaGroup.Types.Entry
                    {
                        Operation = ORSetDeltaOp.Add, Underlying = ToProto(add.UnderlyingSerialization)
                    });
                ...
                case ORSet.IRemoveDeltaOperation remove:
                    deltaGroup.Entries.Add(new ORSetDeltaGroup.Types.Entry
                    {
                        Operation = ORSetDeltaOp.Remove, Underlying = ToProto(remove.UnderlyingSerialization)
                    });
                ...
                case ORSet.IFullStateDeltaOperation full:
                    deltaGroup.Entries.Add(new ORSetDeltaGroup.Types.Entry
                    {
                        Operation = ORSetDeltaOp.Full, Underlying = ToProto(full.UnderlyingSerialization)
                    });
    ...
    {
        var deltaGroup = ORSetDeltaGroup.Parser.ParseFrom(bytes);
    ...
    private Proto.Msg.GSet GSetToProtoUnknown<T>(IGSet g)
    {
        ...
        var p = new Proto.Msg.GSet { TypeInfo = GetTypeDescriptor(typeof(T)) };
        p.OtherElements.Add(otherElements);
        return p;
    }

    private static readonly MethodInfo GSetUnknownToProtoMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(GSetToProtoUnknown),
            BindingFlags.Instance | BindingFlags.NonPublic);
        ...
        switch (gset.TypeInfo.Type)
        {
            ...
            default:
                throw new SerializationException(
                    $"Unknown ValType of [{gset.TypeInfo.Type}] detected while deserializing GSet");
        }
    }
    ...
    private Proto.Msg.GCounter ToProto(GCounter counter)
    {
        ...
        gProto.Entries.AddRange(counter.State.Select(x => new Proto.Msg.GCounter.Types.Entry
        {
            Node = SerializationSupport.UniqueAddressToProto(x.Key),
            Value = ByteString.CopyFrom(BitConverter.GetBytes(x.Value))
        }));
    ...
    private LWWRegister ToProto(ILWWRegister register)
    {
        var protoMaker = LWWProtoMaker.MakeGenericMethod(register.RegisterType);
        return (LWWRegister)protoMaker.Invoke(this, new object[] { register });
    }
    ...
    private LWWRegister LWWToProto<T>(ILWWRegister r)
    {
        var register = (LWWRegister<T>)r;
        var pLww = new LWWRegister();
        ...
        if (_backwardCompatWireFormat && pLww.TypeInfo.TypeName ==
            "Akka.Cluster.Sharding.ShardCoordinator+CoordinatorState, Akka.Cluster.Sharding")
    ...
    private ILWWRegister LWWRegisterFromProto(LWWRegister proto)
    {
        switch (proto.TypeInfo.Type)
        {
            ...
            default:
                throw new SerializationException(
                    $"Unknown ValType of [{proto.TypeInfo.Type}] detected while deserializing LWWRegister");
        }
    }

    private static readonly MethodInfo LWWRegisterMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(GenericLWWRegisterFromProto),
            BindingFlags.Instance | BindingFlags.NonPublic);

    private LWWRegister<T> GenericLWWRegisterFromProto<T>(LWWRegister proto)
    {
    ...
    private ORMap ToProto(IORDictionary ormap)
    {
        var protoMaker = ORDictProtoMaker.MakeGenericMethod(ormap.KeyType, ormap.ValueType);
        return (ORMap)protoMaker.Invoke(this, new object[] { ormap });
    }

    private static readonly MethodInfo ORDictProtoMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(ORDictToProto),
            BindingFlags.Instance | BindingFlags.NonPublic);

    private ORMap ORDictToProto<TKey, TValue>(IORDictionary o) where TValue : IReplicatedData<TValue>
    {
    ...
    private void ToORMapEntries<TKey, TValue>(IImmutableDictionary<TKey, TValue> ormapEntries, ORMap proto)
    ...
    {
        var proto = ORMap.Parser.ParseFrom(bytes);
        return ORDictionaryFromProto(proto);
    }

    private IORDictionary ORDictionaryFromProto(ORMap proto)
    {
    ...
    private IORDictionary GenericORDictionaryFromProto<TKey, TValue>(ORMap proto) where TValue : IReplicatedData<TValue>
    {
    ...
    private ORMapDeltaGroup ORDictionaryDeltasToProto(
        List<ORDictionary.IDeltaOperation> deltaGroupOps)
    {
        ...
        return (ORMapDeltaGroup)protoMaker.Invoke(this, new object[] { deltaGroupOps });
    }

    private static readonly MethodInfo ORDeltaGroupProtoMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(ORDictionaryDeltaGroupToProto),
            BindingFlags.Instance | BindingFlags.NonPublic);

    private ORMapDeltaGroup ORDictionaryDeltaGroupToProto<TKey, TValue>(
        List<ORDictionary.IDeltaOperation> deltaGroupOps) where TValue : IReplicatedData<TValue>
    {
        ...
        group.Entries.Add(deltaGroupOps.Cast<ORDictionary<TKey, TValue>.IDeltaOperation>().Select(x => CreateEntry(x))
            .ToList());
        return group;
    }

    private ORMapDeltaGroup ToProto(ORDictionary.IDeltaOperation op)
    {
        switch (op)
        {
            case null:
                throw new ArgumentNullException(nameof(op),
                    $"Failed to serialize {nameof(ORDictionary.IDeltaOperation)} to protobuf");
    ...
    private ORMapDeltaGroup ORDictionaryPutToProto(ORDictionary.IPutDeltaOp op)
    {
        return ORDictionaryDeltasToProto(new List<ORDictionary.IDeltaOperation> { op });
    }

    private ORMapDeltaGroup ORDictionaryRemoveToProto(ORDictionary.IRemoveDeltaOp op)
    {
        return ORDictionaryDeltasToProto(new List<ORDictionary.IDeltaOperation> { op });
    }

    private ORMapDeltaGroup ORDictionaryRemoveKeyToProto(ORDictionary.IRemoveKeyDeltaOp op)
    {
        return ORDictionaryDeltasToProto(new List<ORDictionary.IDeltaOperation> { op });
    }

    private ORMapDeltaGroup ORDictionaryUpdateToProto(ORDictionary.IUpdateDeltaOp op)
    {
        return ORDictionaryDeltasToProto(new List<ORDictionary.IDeltaOperation> { op });
    }

    private ORDictionary.IDeltaGroupOp ORDictionaryDeltaGroupFromProto(ORMapDeltaGroup deltaGroup)
    {
    ...
    private static readonly MethodInfo ORDeltaGroupMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(GenericORDictionaryDeltaGroupFromProto),
            BindingFlags.Instance | BindingFlags.NonPublic);

    private ORDictionary.IDeltaGroupOp GenericORDictionaryDeltaGroupFromProto<TKey, TValue>(ORMapDeltaGroup deltaGroup)
        where TValue : IReplicatedData<TValue>
    {
        ...
        foreach (var entry in deltaGroup.Entries)
        {
            ...
            switch (entry.Operation)
            {
                case ORMapDeltaOp.OrmapPut:
                {
                    ...
                    deltaOps.Add(new ORDictionary<TKey, TValue>.PutDeltaOperation(
                        new ORSet<TKey>.AddDeltaOperation((ORSet<TKey>)underlying), (TKey)key, (TValue)value));
                }
                    break;
                case ORMapDeltaOp.OrmapRemove:
                {
                    deltaOps.Add(
                        new ORDictionary<TKey, TValue>.RemoveDeltaOperation(
                            new ORSet<TKey>.RemoveDeltaOperation((ORSet<TKey>)underlying)));
                }
                    break;
                case ORMapDeltaOp.OrmapRemoveKey:
                {
                    if (entry.EntryData.Count > 1)
                        throw new ArgumentOutOfRangeException(
                            "Can't deserialize key/value pair in ORDictionary delta - too many pairs on the wire");
                    var (key, value) = MapEntryFromProto(entry.EntryData[0]);
                    deltaOps.Add(new ORDictionary<TKey, TValue>.RemoveKeyDeltaOperation(
                        new ORSet<TKey>.RemoveDeltaOperation((ORSet<TKey>)underlying), (TKey)key));
                }
                    break;
                case ORMapDeltaOp.OrmapUpdate:
                {
                    ...
                    deltaOps.Add(new ORDictionary<TKey, TValue>.UpdateDeltaOperation(
                        new ORSet<TKey>.AddDeltaOperation((ORSet<TKey>)underlying), entries));
                }
                    break;
                default:
    ...
    {
        var group = ORMapDeltaGroup.Parser.ParseFrom(bytes);
    ...
    private LWWMap ToProto(ILWWDictionary lwwDictionary)
    {
        var protoMaker = LWWDictProtoMaker.MakeGenericMethod(lwwDictionary.KeyType, lwwDictionary.ValueType);
        return (LWWMap)protoMaker.Invoke(this, new object[] { lwwDictionary });
    }

    private static readonly MethodInfo LWWDictProtoMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(LWWDictToProto),
            BindingFlags.Instance | BindingFlags.NonPublic);

    private LWWMap LWWDictToProto<TKey, TValue>(ILWWDictionary o)
    {
        var lwwmap = (LWWDictionary<TKey, TValue>)o;
        var proto = new LWWMap();
    ...
    private void ToLWWMapEntries<TKey, TValue>(IImmutableDictionary<TKey, LWWRegister<TValue>> underlyingEntries,
        LWWMap proto)
    {
    ...
    private static readonly MethodInfo LWWDictMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(GenericLWWDictFromProto),
            BindingFlags.Instance | BindingFlags.NonPublic);

    private ILWWDictionary LWWDictFromProto(LWWMap proto)
    {
        var keyType = GetTypeFromDescriptor(proto.Keys.TypeInfo);
        var valueType = GetTypeFromDescriptor(proto.ValueTypeInfo);
    ...
    private ILWWDictionary GenericLWWDictFromProto<TKey, TValue>(LWWMap proto)
    {
        var keys = FromProto(proto.Keys);
        switch (proto.Keys.TypeInfo.Type)
        {
    ...
    {
        var proto = LWWMap.Parser.ParseFrom(bytes);
    ...
    {
        var proto = ORMapDeltaGroup.Parser.ParseFrom(bytes);
    ...
    private static readonly MethodInfo LWWDictionaryDeltaMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(LWWDictionaryDeltaFromProto),
            BindingFlags.Instance | BindingFlags.NonPublic);
    ...
    private ILWWDictionaryDeltaOperation LWWDictionaryDeltaFromProto<TKey, TValue>(ORDictionary.IDeltaOperation op)
    {
        return op switch
        {
            null => throw new ArgumentNullException(nameof(op),
                $"Failed to deserialize {nameof(ILWWDictionaryDeltaOperation)}"),
            ORDictionary<TKey, LWWRegister<TValue>>.IDeltaOperation casted =>
                new LWWDictionary<TKey, TValue>.LWWDictionaryDelta(casted),
            _ => throw new ArgumentException(
                $"Failed to cast cast {op.GetType().FullName} to {typeof(ORDictionary<TKey, LWWRegister<TValue>>.IDeltaOperation).FullName}")
        };
    }
    ...
    private PNCounterMap ToProto(IPNCounterDictionary pnCounterDictionary)
    {
        var protoMaker = PNCounterDictProtoMaker.MakeGenericMethod(pnCounterDictionary.KeyType);
        return (PNCounterMap)protoMaker.Invoke(this, new object[] { pnCounterDictionary });
    }

    private static readonly MethodInfo PNCounterDictProtoMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(GenericPNCounterDictionaryToProto),
            BindingFlags.Instance | BindingFlags.NonPublic);


    private PNCounterMap GenericPNCounterDictionaryToProto<TKey>(IPNCounterDictionary pnCounterDictionary)
    {
        var pnDict = (PNCounterDictionary<TKey>)pnCounterDictionary;
        var proto = new PNCounterMap();
    ...
    {
        var proto = PNCounterMap.Parser.ParseFrom(bytes);
        return PNCounterDictionaryFromProto(proto);
    }

    private IPNCounterDictionary PNCounterDictionaryFromProto(PNCounterMap proto)
    {
        var keyType = GetTypeFromDescriptor(proto.Keys.TypeInfo);
    ...
    private IPNCounterDictionary GenericPNCounterDictionaryFromProto<TKey>(PNCounterMap proto)
    {
        var keys = FromProto(proto.Keys);
        switch (proto.Keys.TypeInfo.Type)
        {
    ...
    private static readonly MethodInfo PNCounterDeltaMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(PNCounterDeltaFromProto),
            BindingFlags.Instance | BindingFlags.NonPublic);
    ...
    private ORMultiMap ToProto(IORMultiValueDictionary multi)
    {
        var protoMaker = MultiMapProtoMaker.MakeGenericMethod(multi.KeyType, multi.ValueType);
        return (ORMultiMap)protoMaker.Invoke(this, new object[] { multi });
    }

    private static readonly MethodInfo MultiMapProtoMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(MultiMapToProto),
            BindingFlags.Instance | BindingFlags.NonPublic);

    private ORMultiMapDelta ToProto(IORMultiValueDictionaryDeltaOperation op)
    {
    ...
    private ORMultiMap MultiMapToProto<TKey, TValue>(IORMultiValueDictionary multi)
    {
        var ormm = (ORMultiValueDictionary<TKey, TValue>)multi;
        var proto = new ORMultiMap();
        proto.ValueTypeInfo = GetTypeDescriptor(typeof(TValue));
        if (ormm.DeltaValues) proto.WithValueDeltas = true;
    ...
    private void ToORMultiMapEntries<TKey, TValue>(IImmutableDictionary<TKey, ORSet<TValue>> underlyingEntries,
        ORMultiMap proto)
    {
    ...
    {
        var ormm = ORMultiMap.Parser.ParseFrom(bytes);
    ...
    private static readonly MethodInfo MultiDictMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(GenericORMultiDictionaryFromProto),
            BindingFlags.Instance | BindingFlags.NonPublic);
    ...
    {
        var proto = ORMultiMapDelta.Parser.ParseFrom(bytes);
        ...
        return (IORMultiValueDictionaryDeltaOperation)maker.Invoke(this,
            new object[] { orDictOp, proto.WithValueDeltas });
    }

    private static readonly MethodInfo ORMultiDictionaryDeltaMaker =
        typeof(ReplicatedDataSerializer).GetMethod(nameof(ORMultiDictionaryDeltaFromProto),
            BindingFlags.Instance | BindingFlags.NonPublic);

    private IORMultiValueDictionaryDeltaOperation ORMultiDictionaryDeltaFromProto<TKey, TValue>(
        ORDictionary.IDeltaOperation op, bool withValueDeltas)
    {
    ...
    private Key KeyFromBinary(byte[] bytes)
    {
        return Key.Parser.ParseFrom(bytes);
    }
