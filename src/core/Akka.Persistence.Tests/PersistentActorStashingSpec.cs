...
public class PersistentActorStashingSpec : PersistenceSpec
{
    ...
    public void PersistentActor_should_support_user_stash_operations_with_several_stashed_messages()
    {
        ...
        var n = 10;
        var commands = Enumerable.Range(1, n)
            .SelectMany(_ => new[] { new Cmd("a"), new Cmd("b-1"), new Cmd("b-2"), new Cmd("c") });
        ...
        protected override bool ReceiveCommand(object message)
        {
            if (!UnstashBehavior(message))
            {
                if (message is Cmd cmd)
                {
                    ...
                    else if (cmd.Data.ToString() == "b")
                    {
                        Persist(new Evt("b"), evt => Sender.Tell(evt.Data));
                    }
    ...
    internal class UserStashManyActor : StashExamplePersistentActor
    {
        ...
        protected override bool ReceiveCommand(object message)
        {
            if (!CommonBehavior(message))
            {
                ...
                if (cmd != null)
                {
                    ...
                    else if (data is "b-1" or "b-2") Persist(new Evt(cmd.Data.ToString()), UpdateStateHandler);
        ...
        {
            if (!UnstashBehavior(message)) Stash.Stash();
            return true;
        }
...
public class SteppingMemoryPersistentActorStashingSpec : PersistenceSpec
{
    public SteppingMemoryPersistentActorStashingSpec()
        : base(SteppingMemoryJournal.Config("persistence-stash")
            .WithFallback(Configuration("SteppingMemoryPersistentActorStashingSpec")))
    {
    }
