...
public class MemoryJournal : AsyncWriteJournal
{
    ...
    ///     TBD
    /// </summary>
    protected virtual ConcurrentDictionary<string, LinkedList<IPersistentRepresentation>> Messages => _messages;
    ...
    {
        return Task.FromResult(Math.Max(HighestSequenceNr(persistenceId),
            _meta.TryGetValue(persistenceId, out var metaSeqNr) ? metaSeqNr : 0L));
    }
            ...
            case SelectCurrentPersistenceIds request:
                SelectAllPersistenceIdsAsync(request.Offset)
                    .PipeTo(request.ReplyTo,
                        success: result => new CurrentPersistenceIds(result.Item1, result.LastOrdering));
                return true;

            case ReplayTaggedMessages replay:
                ReplayTaggedMessagesAsync(replay)
                    .PipeTo(replay.ReplyTo, success: h => new ReplayTaggedMessagesSuccess(h),
    ...
    {
        return Task.FromResult<(IEnumerable<string> Ids, int LastOrdering)>((
            new HashSet<string>(_allMessages.Skip(offset).Select(p => p.PersistenceId)), _allMessages.Count));
    }
    ...
    private Task<int> ReplayTaggedMessagesAsync(ReplayTaggedMessages replay)
    {
                     ...
                     .Take(replay.ToOffset))
        {
            ...
            replay.ReplyTo.Tell(
                new ReplayedTaggedMessage(persistence.WithPayload(payload.Payload), replay.Tag,
                    replay.FromOffset + index), ActorRefs.NoSender);
            index++;
        }
...
public class SharedMemoryJournal : MemoryJournal
{
    ...
    ///     TBD
    /// </summary>
    protected override ConcurrentDictionary<string, LinkedList<IPersistentRepresentation>> Messages => SharedMessages;
}