...
public class ConsistentHash<T>
{
    ...
    public ConsistentHash(SortedDictionary<int, T> nodes, int virtualNodesFactor)
    {
        ...
        if (virtualNodesFactor < 1)
            throw new ArgumentException("virtualNodesFactor must be >= 1", nameof(virtualNodesFactor));

        _virtualNodesFactor = virtualNodesFactor;
    }

    private (int[], T[])? RingTuple => _ring ??= (_nodes.Keys.ToArray(), _nodes.Values.ToArray());

    private int[] NodeHashRing => RingTuple.Value.Item1;

    private T[] NodeRing => RingTuple.Value.Item2;
    ...
    public static ConsistentHash<T> operator +(ConsistentHash<T> hash, T node)
    {
        ...
        return new ConsistentHash<T>(
            hash._nodes.CopyAndAdd(Enumerable.Range(1, hash._virtualNodesFactor).Select(r =>
                new KeyValuePair<int, T>(ConsistentHash.ConcatenateNodeHash(nodeHash, r), node))),
    ...
    public static ConsistentHash<T> operator -(ConsistentHash<T> hash, T node)
    {
        var nodeHash = ConsistentHash.HashFor(node.ToString());
        return new ConsistentHash<T>(
            hash._nodes.CopyAndRemove(Enumerable.Range(1, hash._virtualNodesFactor).Select(r =>
                new KeyValuePair<int, T>(ConsistentHash.ConcatenateNodeHash(nodeHash, r), node))),
            hash._virtualNodesFactor);
    }
