...
public class ClusterSingletonLeaseSpec : AkkaSpec
{
    ...
    {
        return $"important-{_counter.GetAndIncrement()}";
    }

    private ClusterSingletonManagerSettings NextSettings()
    {
        return ClusterSingletonManagerSettings.Create(Sys).WithSingletonName(NextName());
    }

    private string LeaseNameFor(ClusterSingletonManagerSettings settings)
    {
        return $"{Sys.Name}-singleton-akka://{Sys.Name}/user/{settings.SingletonName}";
    }
    ...
    public void ClusterSingleton_with_lease_should_not_start_until_lease_is_available()
    {
        ...
        Sys.ActorOf(
            ClusterSingletonManager.Props(Props.Create(() => new ImportantSingleton(probe.Ref)), PoisonPill.Instance,
                settings),
    ...
    public void ClusterSingleton_with_lease_should_retry_trying_to_get_lease_if_acquire_returns_false()
    {
        ...
        Sys.ActorOf(
            ClusterSingletonManager.Props(Props.Create(() => new ImportantSingleton(singletonProbe.Ref)),
                PoisonPill.Instance, settings),
            settings.SingletonName);
    ...
    public void ClusterSingleton_with_lease_should_stop_singleton_if_the_lease_fails_periodic_check()
    {
        ...
        Sys.ActorOf(
            ClusterSingletonManager.Props(Props.Create(() => new ImportantSingleton(lifecycleProbe.Ref)),
                PoisonPill.Instance, settings),
            settings.SingletonName);
