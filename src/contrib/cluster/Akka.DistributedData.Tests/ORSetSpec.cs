...
public class ORSetSpec
{
    ...
    [Fact]
    public void
        ORSet_must_be_able_to_have_its_element_set_correctly_merged_with_another_ORSet_with_unique_element_sets()
    {
        // set 1
    ...
    {
        // set 1
        var c1 = ORSet<string>.Empty.Add(_node1, _user1).Add(_node1, _user2).Add(_node1, _user3).Remove(_node1, _user1)
            .Remove(_node1, _user3);
        Assert.DoesNotContain(_user1, c1.Elements);
        Assert.Contains(_user2, c1.Elements);
        Assert.DoesNotContain(_user3, c1.Elements);

        // set 2
        var c2 = ORSet<string>.Empty.Add(_node2, _user1).Add(_node2, _user2).Add(_node2, _user3).Add(_node2, _user4)
            .Remove(_node2, _user3);
    ...
    private ORSet<T>.AddDeltaOperation AddDelta<T>(ORSet<T> set)
    {
        return (ORSet<T>.AddDeltaOperation)set.Delta;
    }
    ...
    public void ORSet_must_verify_MergeCommonKeys()
    {
        ...
        var thisDot1 = VersionVector.Create(new Dictionary<UniqueAddress, long> { { _nodeA, 3L }, { _nodeD, 7L } }
            .ToImmutableDictionary());
        var thisDot2 = VersionVector.Create(new Dictionary<UniqueAddress, long> { { _nodeB, 5L }, { _nodeC, 2L } }
            .ToImmutableDictionary());
        var thisVVector =
            VersionVector.Create(
                new Dictionary<UniqueAddress, long> { { _nodeA, 3L }, { _nodeB, 5L }, { _nodeC, 2L }, { _nodeD, 7L } }
                    .ToImmutableDictionary());
        ...
        var thatVVector =
            VersionVector.Create(
                new Dictionary<UniqueAddress, long> { { _nodeA, 3L }, { _nodeB, 6L }, { _nodeC, 1L }, { _nodeD, 8L } }
                    .ToImmutableDictionary());
        var thatSet = new ORSet<string>(
            new Dictionary<string, VersionVector> { { "K1", thatDot1 }, { "K2", thatDot2 } }.ToImmutableDictionary(),
            thatVVector);
        ...
        var expectedDots = new Dictionary<string, VersionVector>
        {
            ...
            {
                "K2",
                VersionVector.Create(new Dictionary<UniqueAddress, long> { { _nodeB, 6L }, { _nodeC, 2L } }
                    .ToImmutableDictionary())
            }
        };
    ...
    public void ORSet_must_verify_MergeDisjointKeys()
    {
        ...
        var elements = new Dictionary<string, VersionVector>
        {
            ...
            {
                "K4",
                VersionVector.Create(new Dictionary<UniqueAddress, long> { { _nodeA, 3L }, { _nodeD, 8L } }
                    .ToImmutableDictionary())
            },
            { "K5", VersionVector.Create(_nodeA, 2L) }
        }.ToImmutableDictionary();
        var vvector = VersionVector.Create(new Dictionary<UniqueAddress, long> { { _nodeA, 3L }, { _nodeD, 7L } }
            .ToImmutableDictionary());
        var acc = new Dictionary<string, VersionVector> { { "K1", VersionVector.Create(_nodeA, 3L) } }
            .ToImmutableDictionary();
        var expectedDots = new Dictionary<string, VersionVector>
        {
            { "K1", VersionVector.Create(_nodeA, 3L) },
