...
public abstract class PersistentFSMBase<TState, TData, TEvent> : PersistentActor, IListeners
{
    ...
    /// <returns>TBD</returns>
    public delegate State<TState, TData, TEvent> StateFunction(FSMBase.Event<TData> fsmEvent,
        State<TState, TData, TEvent> state = null);
    ...
    {
        if (_debugEvent) _log.Debug($"setting {(repeat ? "repeating" : "")} timer {name}/{timeout}: {msg}");
        if (_timers.TryGetValue(name, out var oldTimer)) oldTimer.Cancel();
    ...
    {
        if (_debugEvent) _log.Debug($"Cancelling timer {name}");
    ...
    {
        if (_stateFunctions.TryGetValue(name, out var oldFunction))
        {
    ...
    protected override bool ReceiveCommand(object message)
    {
        ...
        {
            if (_generation == timeoutMarker.Generation) ProcessMsg(FSMBase.StateTimeout.Instance, "state timeout");
            return true;
        }

        if (message is FSMBase.Timer t)
        {
            if (ReferenceEquals(t.Owner, this) && _timers.TryGetValue(t.Name, out var timer) &&
                timer.Generation == t.Generation)
            {
    ...
    private void ProcessEvent(FSMBase.Event<TData> fsmEvent, object source)
    {
        ...
        if (stateFunc != null) upcomingState = stateFunc(fsmEvent);

        if (upcomingState == null) upcomingState = HandleEvent(fsmEvent);
            ...
            if (timeout.HasValue)
            {
                ...
                if (t < TimeSpan.MaxValue)
                    _timeoutFuture = Context.System.Scheduler.ScheduleTellOnceCancelable(t, Context.Self,
                        new FSMBase.TimeoutMarker(_generation), Context.Self);
            }
        }
    }
    ...
    private void Terminate(State<TState, TData, TEvent> upcomingState)
    {
        if (_currentState.StopReason == null)
        {
            ...
            var stopEvent =
                new FSMBase.StopEvent<TState, TData>(reason, _currentState.StateName, _currentState.StateData);
