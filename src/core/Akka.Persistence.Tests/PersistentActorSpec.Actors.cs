...
public partial class PersistentActorSpec
{
    ...
    internal abstract class ExamplePersistentActor : NamedPersistentActor
    {
        ...
        protected bool CommonBehavior(object message)
        {
            ...
            {
                Sender.Tell(Events.Reverse().ToArray());
            }
            else if (message.ToString() == "boom")
            {
        ...
        protected override bool ReceiveCommand(object message)
        {
            if (!CommonBehavior(message))
            {
                ...
                if (cmd != null)
                {
                    ...
                    PersistAsync(new Evt(cmd.Data + "-" + ++_counter), evt => { Sender.Tell(evt.Data); });
        ...
        protected override bool ReceiveCommand(object message)
        {
            if (!CommonBehavior(message))
            {
                ...
                if (cmd != null)
                {
                    Sender.Tell(cmd.Data);
                    for (var i = 1; i <= 3; i++)
                        PersistAsync(new Evt(cmd.Data + "-" + ++_counter),
    ...
    {
        private readonly AtomicCounter _sendMessageCounter = new(0);
        ...
        protected override bool ReceiveCommand(object message)
        {
            if (!CommonBehavior(message))
            {
                ...
                if (cmd != null)
                {
                    ...
                    {
                        Thread.Sleep(300);
                        Sender.Tell(evt.Data + "-a-" + _sendMessageCounter.IncrementAndGet());
                    });

                    PersistAsync(@event, evt => Sender.Tell(evt.Data + "-b-" + _sendMessageCounter.IncrementAndGet()));
    ...
    internal class DeeplyNestedPersists : ExamplePersistentActor
    {
        ...
        private void WeMustGoDeeper(string dWithDepth)
        {
            ...
            if (!_currentDepths.TryGetValue(d, out var currentDepth))
                currentDepth = 1;
