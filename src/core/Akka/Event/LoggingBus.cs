...
public class LoggingBus : ActorEventBus<object, Type>
{
    ...
    /// <summary>
    ///     The minimum log level that this bus will subscribe to, any <see cref="LogEvent">LogEvents</see> with a log level
    ///     below will not be subscribed to.
    /// </summary>
    public LogLevel LogLevel { get; private set; }

    /// <summary>
    ///     Determines whether a specified classifier, <paramref name="child" />, is a subclass of another classifier,
    ...
    /// <returns>
    ///     <c>true</c> if the <paramref name="child" /> classifier is a subclass of <paramref name="parent" />; otherwise
    ///     <c>false</c>.
    /// </returns>
    ...
    ///     This exception is thrown if the logger specified in the <paramref name="system" /> configuration could not be found
    ///     or loaded.
    /// </exception>
    /// <exception cref="LoggerInitializationException">
    ///     This exception is thrown if the logger doesn't respond with a <see cref="LoggerInitialized" /> message when
    ///     initialized.
    /// </exception>
    internal void StartDefaultLoggers(ActorSystemImpl system)
    {
        ...
        {
            var forwarder = system.SystemActorOf(Props.Create(typeof(UnhandledMessageForwarder)),
    ...
    internal void StopDefaultLoggers(ActorSystem system)
    {
        ...
        if (!_loggers.Any(c => c is MinimalLogger))
        {
            ...
            Publish(new Debug(SimpleName(this), GetType(),
                $"Shutting down: {Logging.SimpleName(system.Settings.StdoutLogger)} started"));
        }
        ...
        foreach (var logger in _loggers) RemoveLogger(logger);
    ...
    private void RemoveLogger(IActorRef logger)
    {
        ...
        {
            Unsubscribe(logger);
            if (logger is IInternalActorRef internalActorRef) internalActorRef.Stop();
        }
    }
    ...
    private (Task task, string name) AddLogger(ActorSystemImpl system, Type loggerType, string loggingBusName)
    {
        ...
        var logger = system.SystemActorOf(Props.Create(loggerType).WithDispatcher(system.Settings.LoggersDispatcher),
            loggerName);
    ...
    public void SetLogLevel(LogLevel logLevel)
    {
        ...
        foreach (var logger in _loggers)
        {
            ...
            foreach (var level in AllLogLevels.Where(l => l < logLevel)) Unsubscribe(logger, level.ClassFor());
        }
    }
    ...
    {
        //subscribe to given log level and above
        foreach (var level in AllLogLevels.Where(l => l >= logLevel)) Subscribe(logger, level.ClassFor());
    }
