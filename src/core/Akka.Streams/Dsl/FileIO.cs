...
namespace Akka.Streams.Dsl
{
    ...
    public static class FileIO
    {
        ...
        /// <returns>TBD</returns>
        public static Source<ReadOnlyMemory<byte>, Task<IOResult>> FromFile(FileInfo f, int chunkSize = 8192, long startPosition = 0) =>
            new(new FileSource(f, chunkSize, startPosition, DefaultAttributes.FileSource,
                new SourceShape<ReadOnlyMemory<byte>>(new Outlet<ReadOnlyMemory<byte>>("FileSource"))));

        /// <summary>
        /// Creates a Sink which writes incoming <c>ReadOnlyMemory&lt;byte&gt;</c> elements to the given file. Overwrites existing files
        ...
        /// <returns>TBD</returns>
        public static Sink<ReadOnlyMemory<byte>, Task<IOResult>> ToFile(FileInfo f, FileMode? fileMode = null, long startPosition = 0, bool autoFlush = false, FlushSignaler flushSignaler = null) =>
            new(new FileSink(f, startPosition, fileMode ?? FileMode.Create, DefaultAttributes.FileSink,
                new SinkShape<ReadOnlyMemory<byte>>(new Inlet<ReadOnlyMemory<byte>>("FileSink")), autoFlush, flushSignaler));
    }
}
