...
namespace Akka.Persistence.Sql.Common.Journal
{
    public abstract class SqlJournal : AsyncWriteJournal, IWithUnboundedStash
    {
        ...
        protected override async Task<IImmutableList<Exception>> WriteMessagesAsync(IEnumerable<AtomicWrite> messages)
        {
            ...
            var writeTasks = messages.Select(async message =>
            {
                using (var connection = CreateDbConnection())
                {
                    ...
                    var eventToTags = new Dictionary<IPersistentRepresentation, IEnumerable<WriteTag>>();
                    ...
                    for (int i = 0; i < persistentMessages.Length; i++)
                    {
                        ...
                        if (p.Payload is Tagged)
                        {
                            ...
                            {
                                var tags = new WriteTag[tagged.Tags.Count];
                                allTags.UnionWith(tagged.Tags);
                                var j = 0;
                                foreach (var tag in tagged.Tags)
                                {
                                    tags[j] = new WriteTag(tag, await NextTagSequenceNr(tag));
                                    j++;
                                }

                                eventToTags.Add(p, tags);
                            }
                            else eventToTags.Add(p, Enumerable.Empty<WriteTag>());
                        }
                        else eventToTags.Add(p, Enumerable.Empty<WriteTag>());

                        if (IsTagId(p.PersistenceId))
                            throw new InvalidOperationException($"Persistence Id {p.PersistenceId} must not start with {QueryExecutor.Configuration.TagPersistenceIdPrefix}");
        ...
        protected virtual async Task<long> ReplayTaggedMessagesAsync(ReplayTaggedMessages replay)
        {
            using (var connection = CreateDbConnection())
            {
                ...
                return await QueryExecutor
                    .SelectByTagAsync(connection, _pendingRequestsCancellation.Token, replay.Tag, replay.FromSequenceNr, replay.ToSequenceNr, replay.Max, replayedTagged => {
        ...
        private string TagId(string tag) => QueryExecutor.Configuration.TagPersistenceIdPrefix + tag;
        ...
        {
            return persistenceId.StartsWith(QueryExecutor.Configuration.TagPersistenceIdPrefix);
        }
