...
public class ClusterSpec : AkkaSpec
{
    ...
    public IActorRef Self => TestActor;
    ...
    public async Task A_cluster_must_publish_member_removed_when_shutdown()
    {
        ...
        _cluster.RegisterOnMemberRemoved(() => { callbackProbe.Tell("OnMemberRemoved"); });

        _cluster.RegisterOnMemberUp(() => { callbackProbe.Tell("OnMemberUp"); });
    ...
    public async Task BugFix_2442_RegisterOnMemberUp_should_fire_if_node_already_up()
    {
        ...
        var callbackProbe = CreateTestProbe();
        _cluster.RegisterOnMemberUp(() => { callbackProbe.Tell("RegisterOnMemberUp"); });
    ...
    public async Task A_cluster_must_cancel_LeaveAsync_task_if_CancellationToken_fired_before_node_left()
    {
        ...
        await WithinAsync(TimeSpan.FromSeconds(10), async () =>
        {
            ...
            await AwaitConditionAsync(() => Task.FromResult(task2.IsCompleted && !task2.IsCanceled), null,
                "Task should be completed, but not cancelled.");
        }, cancellationToken: cts.Token);

        // Subsequent LeaveAsync() tasks expected to complete immediately (not cancelled)
        var task3 = _cluster.LeaveAsync();
        await AwaitConditionAsync(() => Task.FromResult(task3.IsCompleted && !task3.IsCanceled), null,
            "Task should be completed, but not cancelled.");
    }
    ...
    public async Task A_cluster_must_be_allowed_to_join_and_leave_with_local_address()
    {
        ...
        try
        {
            ...
            await WithinAsync(5.Seconds(),
                async () => { await AwaitAssertAsync(() => { Cluster.Get(sys2).IsTerminated.Should().BeTrue(); }); });
        }
        finally
        {
            Shutdown(sys2);
    ...
    public async Task A_cluster_must_leave_via_CoordinatedShutdownRun()
    {
        ...
        try
        {
            ...
            // MemberExited might not be published before MemberRemoved
            var removed =
                (ClusterEvent.MemberRemoved)await probe.FishForMessageAsync(m => m is ClusterEvent.MemberRemoved);
