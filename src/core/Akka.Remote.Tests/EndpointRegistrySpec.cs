...
{
    private readonly IActorRef actorA;
    private readonly IActorRef actorB;

    private readonly Address address1 = new("test", "testsys1", "testhost1", 1234);
    private readonly Address address2 = new("test", "testsy2", "testhost2", 1234);
    ...
    public void EndpointRegistry_must_be_able_to_register_Gated_policy_for_an_address()
    {
        ...
        Assert.Equal(deadline,
            reg.WritableEndpointWithPolicyFor(address1).AsInstanceOf<EndpointManager.Gated>().TimeOfRelease);
        Assert.False(reg.IsReadOnly(actorA));
        Assert.False(reg.IsWritable(actorA));
    }
    ...
    public void EndpointRegistry_must_keep_tombstones_when_removing_an_endpoint()
    {
        ...
        Assert.Equal(deadline,
            reg.WritableEndpointWithPolicyFor(address1).AsInstanceOf<EndpointManager.Gated>().TimeOfRelease);
        Assert.Equal(deadline,
            reg.WritableEndpointWithPolicyFor(address2).AsInstanceOf<EndpointManager.Quarantined>().Deadline);
    ...
    public void EndpointRegistry_should_prune_outdated_Gated_directives_properly()
    {
        ...
        Assert.Equal(farIntheFuture,
            reg.WritableEndpointWithPolicyFor(address2).AsInstanceOf<EndpointManager.Gated>().TimeOfRelease);
    }
    ...
    public void EndpointRegistry_should_be_able_to_register_Quarantined_policy_for_an_address()
    {
        ...
        Assert.Equal(deadline,
            reg.WritableEndpointWithPolicyFor(address1).AsInstanceOf<EndpointManager.Quarantined>().Deadline);
    }
