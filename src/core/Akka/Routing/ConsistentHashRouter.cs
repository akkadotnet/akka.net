...
public sealed class ConsistentHashingRoutingLogic : RoutingLogic
{
    ...
    public override Routee Select(object message, Routee[] routees)
    {
        ...
        Routee Target(object hashData)
        {
            try
            {
                ...
                switch (hashData)
                {
                    ...
                    default:
                        return currentConsistentHash
                            .NodeFor(_system.Serialization.FindSerializerFor(hashData).ToBinary(hashData)).Routee;
                }
            }
            catch (Exception ex)
            {
        ...
        if (hashMapping != null) return Target(ConsistentHash.ToBytesOrObject(hashMapping));

        if (message is IConsistentHashable hashable)
        {
    ...
    /// <summary>
    ///     Creates a new <see cref="ConsistentHashingRoutingLogic" /> router logic with a given
    ///     <see cref="ConsistentHashMapping" />.
    ///     <note>
...
public sealed class ConsistentHashingPool : Pool
{
    ...
    ///     <note>
    ///         A <see cref="ConsistentHashingPool" /> configured in this way uses the
    ...
    public ConsistentHashingPool(
        int nrOfInstances,
        ConsistentHashMapping hashMapping)
        : this(nrOfInstances, null, DefaultSupervisorStrategy, Dispatchers.DefaultDispatcherId, false, 0, hashMapping)
    {
    }
    ...
    ///     This exception is thrown if the given <paramref name="routerConfig" /> is not a
    ///     <see cref="ConsistentHashingPool" />.
    /// </exception>
    ...
    public override RouterConfig WithFallback(RouterConfig routerConfig)
    {
        ...
        if (routerConfig is ConsistentHashingPool other)
...
public sealed class ConsistentHashingGroup : Group
{
    ...
    /// <exception cref="ArgumentException">
    ///     This exception is thrown if the given <paramref name="routerConfig" /> is not a
    ///     <see cref="ConsistentHashingGroup" />.
    /// </exception>
    /// <returns>The router configured with the auxiliary information.</returns>
    public override RouterConfig WithFallback(RouterConfig routerConfig)
    {
        ...
        if (routerConfig is ConsistentHashingGroup other)
    ...
    public override ISurrogate ToSurrogate(ActorSystem system)
    {
        return new ConsistentHashingGroupSurrogate { Paths = InternalPaths, RouterDispatcher = RouterDispatcher };
    }
    ...
    public class ConsistentHashingGroupSurrogate : ISurrogate
    {
        ...
        public ISurrogated FromSurrogate(ActorSystem system)
        {
            return new ConsistentHashingGroup(Paths, 0, ConsistentHashingRouter.EmptyConsistentHashMapping,
                RouterDispatcher);
        }
    }
}