...
public class ActorSelection : ICanTell
{
    ...
    public void Tell(object message, IActorRef sender = null)
    {
        ...
        DeliverSelection(Anchor as IInternalActorRef, sender,
            new ActorSelectionMessage(message, Path, false));
    }
    ...
    ///     This exception is thrown if no such actor exists or the identification didn't complete within the supplied
    ///     <paramref name="timeout" />.
    /// </exception>
    /// <returns>
    ///     A Task that will be completed with the <see cref="IActorRef" />, if the actor was found. Otherwise it will be
    ///     failed with an <see cref="ActorNotFoundException" />.
    /// </returns>
    public Task<IActorRef> ResolveOne(TimeSpan timeout)
    {
        return InnerResolveOne(timeout, CancellationToken.None);
    }
    ...
    {
        return InnerResolveOne(timeout, ct);
    }
                ...
                if (actorRef is ActorRefWithCell refWithCell)
                {
                    EmptyLocalActorRef EmptyRef()
                    {
                        return new EmptyLocalActorRef(
                            ...
                            anchor.Path / sel.Elements.Select(el => el.ToString()),
                    ...
                    switch (iter.Next())
                    {
                        ...
                        case SelectChildRecursive _:
                            ...
                            var msg = new ActorSelectionMessage(sel.Message,
                                new SelectionPathElement[] { SelectChildRecursive.Instance }, true);
...
public class ActorSelectionMessage : IAutoReceivedMessage, IPossiblyHarmful, IWrappedMessage
{
    ...
    /// <param name="wildCardFanOut">
    ///     Optional. Indicates whether or not we're delivering a wildcard
    ///     <see cref="ActorSelection" />.
    /// </param>
...
public class SelectChildPattern : SelectionPathElement
{
    ...
    public override int GetHashCode()
    {
        return PatternStr?.GetHashCode() ?? 0;
    }
...
public class SelectParent : SelectionPathElement
{
    ...
    public override bool Equals(object obj)
    {
        return !ReferenceEquals(obj, null) && obj is SelectParent;
    }


    public override int GetHashCode()
    {
        return nameof(SelectParent).GetHashCode();
    }
