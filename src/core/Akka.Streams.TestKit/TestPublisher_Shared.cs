...
public static partial class TestPublisher
{
    public partial class ManualProbe<T>
    {
            ...
            CancellationToken cancellationToken)
        {
            await probe.ExpectMsgAsync<RequestMore>(
                x => x.NrOfElements == nrOfElements && x.Subscription == subscription,
                cancellationToken: cancellationToken);
        }
        ...
        {
            return probe.ReceiveWhileAsync(max, idle, filter, msgCount, cancellationToken);
        }
        ...
        internal static ValueTask<IPublisherEvent> ExpectEventTask(
            ...
            CancellationToken cancellationToken)
        {
            return probe.ExpectMsgAsync<IPublisherEvent>(cancellationToken: cancellationToken);
        }
    ...
    public partial class Probe<T>
    {
        ...
        internal static async Task SendNextTask(Probe<T> probe, IEnumerable<T> elements,
            CancellationToken cancellationToken)
        {
            await EnsureSubscriptionTask(probe, cancellationToken);
            var sub = probe.Subscription;
