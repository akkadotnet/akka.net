...
public class TestProbe : TestKitBase, INoImplicitSender, IInternalActorRef
{
    ...
    public IActorRef Ref => TestActor;

    /// <summary>Gets the sender of the last message</summary>
    public IActorRef Sender => LastSender;
    ...
    ActorPath IActorRef.Path => TestActor.Path;
    ...
    bool IActorRefScope.IsLocal => ((IInternalActorRef)TestActor).IsLocal;

    IInternalActorRef IInternalActorRef.Parent => ((IInternalActorRef)TestActor).Parent;

    IActorRefProvider IInternalActorRef.Provider => ((IInternalActorRef)TestActor).Provider;

#pragma warning disable CS0618 // Type or member is obsolete
    bool IInternalActorRef.IsTerminated => ((IInternalActorRef)TestActor).IsTerminated;
...
#pragma warning restore CS0809
    ...
    public IActorRef ChildActorOf<T>(
        ...
        where T : ActorBase
    {
        return ChildActorOfAsync(Props.Create<T>(), name, supervisorStrategy, cancellationToken)
    ...
    public async Task<IActorRef> ChildActorOfAsync<T>(
        ...
        where T : ActorBase
    {
        return await ChildActorOfAsync(Props.Create<T>(), name, supervisorStrategy, cancellationToken);
    }
    ...
    public IActorRef ChildActorOf<T>(
        ...
        where T : ActorBase
    {
        return ChildActorOfAsync(Props.Create<T>(), supervisorStrategy, cancellationToken)
            .GetAwaiter().GetResult();
    }
    ...
    public async Task<IActorRef> ChildActorOfAsync<T>(
        ...
        where T : ActorBase
    {
        return await ChildActorOfAsync(Props.Create<T>(), supervisorStrategy, cancellationToken);
    }
    ...
    public IActorRef ChildActorOf<T>(string name, CancellationToken cancellationToken = default)
        where T : ActorBase
    {
        return ChildActorOfAsync(Props.Create<T>(), name, cancellationToken)
            .GetAwaiter().GetResult();
    }
    ...
    public async Task<IActorRef> ChildActorOfAsync<T>(string name, CancellationToken cancellationToken = default)
        where T : ActorBase
    {
        return await ChildActorOfAsync(Props.Create<T>(), name, cancellationToken);
    }
    ...
    public IActorRef ChildActorOf<T>(CancellationToken cancellationToken = default)
        where T : ActorBase
    {
        return ChildActorOfAsync(Props.Create<T>(), cancellationToken)
            .GetAwaiter().GetResult();
    }
    ...
    public async Task<IActorRef> ChildActorOfAsync<T>(CancellationToken cancellationToken = default)
        where T : ActorBase
    {
        return await ChildActorOfAsync(Props.Create<T>(), cancellationToken);
    }
