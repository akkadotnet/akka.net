...
{
    /// <summary>
    ///     Specialized type of rate limiter which emits batches of elements (with size limited by the
    ///     <paramref name="maxBatchSize" /> parameter)
    ...
    public static IGraph<FlowShape<T, IEnumerable<T>>, NotUsed> Create<T>(TimeSpan minInterval, int maxBatchSize)
    {
        ...
        return Flow.Create<T>().GroupedWithin(maxBatchSize, minGroupingInterval)
            .Via(new DelayFlow<IEnumerable<T>>(minInterval));
    }
}