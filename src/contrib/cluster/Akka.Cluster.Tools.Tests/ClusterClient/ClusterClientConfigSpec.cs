...
public class ClusterClientConfigSpec : AkkaSpec
{
    ...
    public void ClusterClientSettings_must_throw_exception_on_wrong_buffer(int bufferSize)
    {
        var sys = ActorSystem.Create("test",
            ConfigurationFactory.ParseString("akka.cluster.client.buffer-size = " + bufferSize));
    ...
    public void ClusterClientSettings_must_throw_exception_on_empty_initial_contacts()
    {
        ...
        var exception = Assert.Throws<ArgumentException>(() =>
            clusterClientSettings.WithInitialContacts(ImmutableHashSet<ActorPath>.Empty));
    ...
    {
        var initialContacts =
            ImmutableHashSet<ActorPath>.Empty.Add(new RootActorPath(Address.AllSystems) / "user" / "foo");
        var clusterClientSettings =
            ClusterClientSettings.Create(Sys).WithInitialContacts(initialContacts).WithBufferSize(2000);
