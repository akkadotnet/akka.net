...
///     <see cref="ShardingConsumerController" /> is the entity actor that is initialized via
///     <see cref="ClusterSharding" />.
///     It will manage the lifecycle and message delivery to the destination consumer actor (your actor type specified via
///     <see cref="Props" />
///     in the
///     <see cref="ShardingConsumerController.Create{T}(System.Func{Akka.Actor.IActorRef,Akka.Actor.Props},Settings)" />
///     method.)
...
///     Received messages from the producer are wrapped in a <see cref="ConsumerController.Delivery{T}" /> message and sent
///     to the destination consumer actor,
///     which is supposed to reply with <see cref="ConsumerController.Confirmed" /> when it has successfully processed the
///     message.
///     Next message from the producer will not be delivered until the destination consumer actor has confirmed the
///     previous message.
///     However, since there can be several producers, e.g. one per node, sending messages to the same destination entity
///     actor there can
///     be multiple <see cref="ConsumerController.Delivery{T}" /> messages in flight at the same time.
///     More messages from a specific producer that arrive while waiting for the confirmation are stashed by the
///     <see cref="ConsumerController" />
...
public static class ShardingConsumerController
{
    ...
    /// </summary>
    /// <param name="consumerProps">
    ///     A function that passes in the <see cref="ShardingConsumerController" /> actor reference
    ///     in exchange for the consumer's <see cref="Props" />.
    /// </param>
    ...
    public static Props Create<T>(Func<IActorRef, Props> consumerProps, Settings settings)
    {
        return Props.Create(() => new ShardingConsumerController<T>(consumerProps, settings))
            .WithStashCapacity(settings.BufferSize);
    }
    ...
    public sealed record Settings
    {
        ...
        internal static Settings
            Create(Config config, Config consumerControllerConfig) // made internal so users can't foot-gun themselves
        {
            return new Settings(config,
                ConsumerController.Settings.Create(config.WithFallback(consumerControllerConfig)));
        }

        public override string ToString()
        {
            return
                $"ShardingConsumerController.Settings(BufferSize={BufferSize}, ConsumerControllerSettings={ConsumerControllerSettings})";
        }
    }
}