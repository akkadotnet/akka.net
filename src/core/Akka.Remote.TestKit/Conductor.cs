...
partial class TestConductor //Conductor trait in JVM version
{
    ...
    public async Task<IPEndPoint> StartController(int participants, RoleName name, IPEndPoint controllerPort)
    {
        ...
        var node = await _controller.Ask<IPEndPoint>(TestKit.Controller.GetSockAddr.Instance, Settings.QueryTimeout)
    ...
    public Task<Done> Exit(RoleName node, int exitValue)
    {
        ...
        return Controller.Ask(new Terminate(node, new Right<bool, int>(exitValue)), Settings.QueryTimeout).ContinueWith(
            t =>
            {
...
internal class ConductorHandler : ChannelHandlerAdapter
{
    ...
    public override void ChannelActive(IChannelHandlerContext context)
    {
        ...
        _controller.Ask<IActorRef>(new Controller.CreateServerFSM(channel),
            TimeSpan.FromMilliseconds(int.MaxValue)).ContinueWith(tr =>
        {
            var fsm = tr.Result;
    ...
    {
        var channel = context.Channel;
        _log.Warning("handled network error from {0}: {1} {2}", channel.RemoteAddress, exception.Message,
...
internal class ServerFSM : FSM<ServerFSM.State, IActorRef>, ILoggingFSM
{
    ...
    protected void InitFSM()
    {
        ...
        WhenUnhandled(@event =>
        {
            ...
            if (clientDisconnected != null)
            {
                if (@event.StateData != null)
                    @event.StateData.Tell(new Failure(
                        new Controller.ClientDisconnectedException("client disconnected in state " + StateName + ": " +
                                                                   _channel)));
                return Stop();
            }
        ...
        When(State.Initial, @event =>
        {
            ...
            {
                _log.Warning("client {0}, sent not Hello in first message (instead {1}), disconnecting",
                    _channel.RemoteAddress, @event.FsmEvent);
                _channel.CloseAsync();
                return Stop();
            }
