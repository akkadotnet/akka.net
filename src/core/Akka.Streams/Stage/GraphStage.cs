...
/// <summary>
///     A GraphStage represents a reusable graph stream processing stage. A GraphStage consists of a <see cref="Shape" />
///     which describes
///     its input and output ports and a factory function that creates a <see cref="GraphStageLogic" /> which implements
///     the processing
...
public abstract class GraphStage<TShape> : GraphStageWithMaterializedValue<TShape, NotUsed> where TShape : Shape
{
    ...
    public sealed override ILogicAndMaterializedValue<NotUsed> CreateLogicAndMaterializedValue(
        Attributes inheritedAttributes)
    {
        return new LogicAndMaterializedValue<NotUsed>(CreateLogic(inheritedAttributes), NotUsed.Instance);
    }
...
public abstract class TimerGraphStageLogic : GraphStageLogic
{
    ...
    protected internal void ScheduleRepeatedly(object timerKey, TimeSpan initialDelay, TimeSpan interval)
    {
        ...
        var task = Interpreter.Materializer.ScheduleRepeatedly(initialDelay, interval,
            () => { TimerAsyncCallback(new TimerMessages.Scheduled(timerKey, id, true)); });
    ...
    protected internal void ScheduleOnce(object timerKey, TimeSpan delay)
    {
        ...
        var task = Interpreter.Materializer.ScheduleOnce(delay,
            () => { TimerAsyncCallback(new TimerMessages.Scheduled(timerKey, id, false)); });
        _keyToTimers[timerKey] = new TimerMessages.Timer(id, task);
    }
...
/// <summary>
///     Represents the processing logic behind a <see cref="GraphStage{TShape}" />. Roughly speaking, a subclass of
///     <see cref="GraphStageLogic" /> is a
///     collection of the following parts:
///     <para>
///         * A set of <see cref="InHandler" /> and <see cref="OutHandler" /> instances and their assignments to the
///         <see cref="Inlet" />s and <see cref="Outlet" />s
///         of the enclosing <see cref="GraphStage{TShape}" />
///     </para>
///     <para>
///         * Possible mutable state, accessible from the <see cref="InHandler" /> and <see cref="OutHandler" /> callbacks,
///         but not from anywhere
...
///     <para />
///     The <see cref="PostStop" /> lifecycle hook on the logic itself is called once all ports are closed. This is the
///     only tear down
///     callback that is guaranteed to happen, if the actor system or the materializer is terminated the handlers may never
///     see any callbacks to <see cref="InHandler.OnUpstreamFailure" />, <see cref="InHandler.OnUpstreamFinish" /> or
///     <see cref="OutHandler.OnDownstreamFinish" />.
...
public abstract class GraphStageLogic : IStageLogging
{
    ...
    /// </summary>
    [InternalApi] private Exception _lastCancellationCause;
    ...
    {
        LogSource = Event.LogSource.Create(shape.ToString());
    }
    ...
    /// <param name="outlet">TBD</param>
    /// <returns>TBD</returns>
    private IOutHandler GetHandler(Outlet outlet)
    {
    ...
    {
        return GetHandler((Outlet)outlet);
    }

    private Connection GetConnection(Inlet inlet)
    {
        return PortToConn[inlet.Id];
    }

    private Connection GetConnection(Outlet outlet)
    {
    ...
    {
        Interpreter.Cancel(GetConnection(inlet), cause);
    }

    protected void Cancel<T>(Inlet<T> inlet)
    {
        Interpreter.Cancel(GetConnection(inlet), SubscriptionWithCancelException.NoMoreElementsNeeded.Instance);
    }

    /// <summary>
    ///     Once the callback <see cref="InHandler.OnPush" /> for an input port has been invoked, the element that has been
    ///     pushed
    ///     can be retrieved via this method. After <see cref="Grab{T}(Inlet)" /> has been called the port is considered to be
    ///     empty, and further
    ///     calls to <see cref="Grab{T}(Inlet)" /> will fail until the port is pulled again and a new element is pushed as a
    ///     response.
    ///     The method <see cref="IsAvailable(Inlet)" /> can be used to query if the port has an element that can be grabbed or
    ///     not.
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    /// <param name="inlet">TBD</param>
    ...
    ///     Once the callback <see cref="InHandler.OnPush" /> for an input port has been invoked, the element that has been
    ///     pushed
    ///     can be retrieved via this method. After <see cref="Grab{T}(Inlet{T})" /> has been called the port is considered to
    ///     be empty, and further
    ///     calls to <see cref="Grab{T}(Inlet{T})" /> will fail until the port is pulled again and a new element is pushed as a
    ///     response.
    ///     The method <see cref="IsAvailable(Inlet)" /> can be used to query if the port has an element that can be grabbed or
    ///     not.
    /// </summary>
    ...
    {
        return (GetConnection(inlet).PortState & (InReady | InClosed)) == 0;
    }
    ...
    {
        return HasBeenPulled((Inlet)inlet);
    }

    /// <summary>
    ///     Indicates whether there is an element waiting at the given input port. <see cref="Grab{T}(Inlet{T})" /> can be used
    ///     to retrieve the
    ...
    private bool IsAvailable(Inlet inlet)
    {
        ...
        if ((connection.PortState & (InReady | InFailed)) == (InReady | InFailed))
            return connection.Slot switch
            {
                Failed failed => !ReferenceEquals(failed.PreviousElement,
    ...
    /// <param name="inlet">TBD</param>
    /// <returns>TBD</returns>
    protected internal bool IsAvailable<T>(Inlet<T> inlet)
    {
        return IsAvailable((Inlet)inlet);
    }
    ...
    /// <param name="inlet">TBD</param>
    /// <returns>TBD</returns>
    private bool IsClosed(Inlet inlet)
    {
        return (GetConnection(inlet).PortState & InClosed) != 0;
    }
    ...
    {
        return IsClosed((Inlet)inlet);
    }

    /// <summary>
    ///     Emits an element through the given output port. Calling this method twice before a <see cref="Pull{T}(Inlet{T})" />
    ///     has been arrived
    ///     will fail. There can be only one outstanding push request at any given time. The method
    ///     <see cref="IsAvailable(Inlet)" /> can be
    ...
    /// <summary>
    ///     Automatically invokes <see cref="Cancel{T}(Inlet{T}, Exception)" /> or <see cref="Complete" /> on all the input or
    ///     output ports that have been called,
    ...
    /// <summary>
    ///     Automatically invokes <see cref="Cancel{T}(Inlet{T}, Exception)" /> or <see cref="Fail{T}" /> on all the input or
    ///     output ports that have been called,
    ///     then marks the stage as stopped.
    /// </summary>
    /// <param name="reason">TBD</param>
    ...
    {
        return (GetConnection(outlet).PortState & (OutReady | OutClosed)) == OutReady;
    }
    ...
    {
        return (GetConnection(outlet).PortState & OutClosed) != 0;
    }
    ...
    ///     suspending execution if necessary. This action replaces the <see cref="InHandler" />
    ///     for the given inlet if suspension is needed and reinstalls the current
    ///     handler upon receiving the <see cref="InHandler.OnPush" /> signal (before invoking the <paramref name="andThen" />
    ///     function).
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    /// <param name="inlet">TBD</param>
    ...
    /// <param name="outlet">TBD</param>
    /// <param name="element">TBD</param>
    protected internal void Emit<T>(Outlet<T> outlet, T element)
    {
    ...
    ///     given outlet before pulling for more data. <paramref name="doFinish" /> and <paramref name="doFail" /> control
    ///     whether
    ...
    {
        return @event => Interpreter.OnAsyncInput(this, @event, x => handler((T)x));
    }
    ...
    {
        return () => Interpreter.OnAsyncInput(this, NotUsed.Instance, _ => handler());
    }

    /// <summary>
    ///     Initialize a <see cref="StageActorRef" /> which can be used to interact with from the outside world "as-if" an
    ///     actor.
    ///     The messages are looped through the <see cref="GetAsyncCallback{T}" /> mechanism of
    ///     <see cref="GraphStage{TShape}" /> so they are safe to modify
    ///     internal state of this stage.
    ///     This method must (the earliest) be called after the <see cref="GraphStageLogic" /> constructor has finished
    ///     running,
    ...
    protected class SubSinkInlet<T>
    {
        ...
        /// </summary>
        /// <param name="handler">TBD</param>
        public void SetHandler(IInHandler handler)
        {
            _handler = handler;
        }
        ...
        public void Cancel()
        {
            Cancel(SubscriptionWithCancelException.NoMoreElementsNeeded.Instance);
        }
    ...
    protected class SubSourceOutlet<T>
    {
        ...
        /// </summary>
        /// <param name="handler">TBD</param>
        public void SetHandler(IOutHandler handler)
        {
            _handler = handler;
        }
    ...
    private abstract class Emitting : OutHandler
    {
        ...
        {
            Previous.OnDownstreamFinish(cause);
        }
    }
    ...
    private sealed class EmittingSingle<T> : Emitting
    {
        ...
        public EmittingSingle(Outlet<T> @out, T element, IOutHandler previous, Action andThen, GraphStageLogic logic) :
            base(@out, previous, andThen, logic)
        {
            _element = element;
        }
...
public interface IInHandler
{
    ...
    ///     <see cref="GraphStageLogic.Grab{T}(Inlet{T})" /> method.
    /// </summary>
    void OnPush();
...
public abstract class InHandler : IInHandler
{
    ...
    {
        Current.ActiveStage.FailStage(e);
    }
}
...
public interface IOutHandler
{
    ...
    /// <summary>
    ///     Called when the output port will no longer accept any new elements. After this callback no other callbacks will be
...
/// <summary>
///     Input handler that does not terminate the stage upon receiving completion. The stage fails upon receiving a
///     failure.
/// </summary>
...
public sealed class StageActor
{
    ...
    {
        Volatile.Write(ref _behavior, receive);
    }
    ...
    public void Stop()
    {
        _cell.RemoveFunctionRef(_functionRef);
    }
    ...
    {
        _functionRef.Watch(actorRef);
    }

    /// <summary>
    ///     Makes current <see cref="StageActor" /> stop watching previously <see cref="Watch" />ed
    ///     <paramref name="actorRef" />.
    ///     If <paramref name="actorRef" /> was not watched over, this method has no result.
    /// </summary>
    /// <param name="actorRef"></param>
    public void Unwatch(IActorRef actorRef)
    {
        _functionRef.Unwatch(actorRef);
    }
...
internal class GraphStageLogicWithCallbackWrapper<T> : GraphStageLogic
{
    ...
    {
        Locked(() => _callbackState.Value = new Stopped(callback));
    }
