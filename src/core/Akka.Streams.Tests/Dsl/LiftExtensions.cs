...
internal static class LiftExtensions
{
    ...
    public static Source<(int, Source<int, NotUsed>), TMat> Lift<TMat>(
        this SubFlow<int, TMat, IRunnableGraph<TMat>> source, Func<int, int> key)
    {
        return
            (Source<(int, Source<int, NotUsed>), TMat>)
            source.PrefixAndTail(1).Select(p => (key(p.Item1.First()), Source.Single(p.Item1.First()).Concat(p.Item2)))
                .ConcatSubstream();
    }
}