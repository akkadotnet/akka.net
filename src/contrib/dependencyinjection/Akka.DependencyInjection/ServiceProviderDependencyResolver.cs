...
public class ServiceProviderDependencyResolver : IDependencyResolver
{
    ...
    public Props Props(Type type, params object[] args)
    {
        if (typeof(ActorBase).IsAssignableFrom(type))
            return Actor.Props.CreateBy(new ServiceProviderActorProducer(ServiceProvider, type, args));
    ...
    {
        return Actor.Props.CreateBy(new ServiceProviderActorProducer<T>(ServiceProvider, args));
    }
}