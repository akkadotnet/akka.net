...
public class ClusterShardingSpecConfig : MultiNodeClusterShardingConfig
{
    public ClusterShardingSpecConfig(
        ...
        : base(mode, rememberEntitiesStore: rememberEntitiesStore, loglevel: "DEBUG")
    {
...
public class PersistentClusterShardingWithEntityRecoverySpec : ClusterShardingSpec
{
    public PersistentClusterShardingWithEntityRecoverySpec()
        : base(new PersistentClusterShardingWithEntityRecoverySpecConfig(),
            typeof(PersistentClusterShardingWithEntityRecoverySpec))
    {
    }
}
...
public class DDataClusterShardingWithEntityRecoverySpec : ClusterShardingSpec
{
    public DDataClusterShardingWithEntityRecoverySpec()
        : base(new DDataClusterShardingWithEntityRecoverySpecConfig(),
            typeof(DDataClusterShardingWithEntityRecoverySpec))
    {
    }
}
...
public abstract class ClusterShardingSpec : MultiNodeClusterShardingSpec<ClusterShardingSpecConfig>
{
    ...
    internal class Counter : PersistentActor
    {
        ...
        {
            return Actor.Props.Create(() => new Counter());
        }
    ...
    private sealed class MessageExtractor : IMessageExtractor
    {
        ...
        {
            return (long.Parse(entityId) % NumberOfShards).ToString();
        }
    }
    ...
    private EventSourcedRememberEntitiesProvider EventSourcedRememberEntitiesProvider(string typeName,
    ...
    private void CreateCoordinator()
    {
        ...
        var typeNames = new[]
        {
            "counter", "rebalancingCounter", "RememberCounterEntities", "AnotherRememberCounter", "RememberCounter",
            "RebalancingRememberCounter", "AutoMigrateRememberRegionTest"
        };
    ...
    private IActorRef CreateRegion(string typeName, bool rememberEntities)
    {
        ...
        return Sys.ActorOf(
            ShardRegion.Props(
                typeName,
                _ => QualifiedCounter.Props(typeName),
                settings,
    ...
    private void ClusterSharding_should_use_second_node()
    {
        Within(TimeSpan.FromSeconds(20), () =>
        {
            ...
            RunOn(() =>
            {
                ...
                ((path11.Address.HasLocalScope && path12.Address.HasGlobalScope) ||
                 (path11.Address.HasGlobalScope && path12.Address.HasLocalScope)).Should().BeTrue();
            }, Config.First);
    ...
    private void ClusterSharding_should_support_proxy_only_mode()
    {
        Within(TimeSpan.FromSeconds(10), () =>
        {
            RunOn(() =>
            {
                ...
                var settings =
                    ClusterShardingSettings.Create(cfg, Sys.Settings.Config.GetConfig("akka.cluster.singleton"));
    ...
    private void ClusterSharding_should_failover_shards_on_crashed_node()
    {
        Within(TimeSpan.FromSeconds(30), () =>
        {
            ...
            if (!Log.IsDebugEnabled)
                Sys.EventStream.Publish(new Mute(new DeadLettersFilter(new PredicateMatcher(_ => true),
                    new PredicateMatcher(_ => true))));
            EnterBarrier("logs-muted");

            RunOn(() => { TestConductor.Exit(Config.Second, 0).Wait(); }, Config.Controller);
    ...
    private void ClusterSharding_should_recover_coordinator_state_after_coordinator_crash()
    {
        Within(TimeSpan.FromSeconds(60), () =>
        {
            ...
            RunOn(() => { TestConductor.Exit(Config.First, 0).Wait(); }, Config.Controller);
                ...
                AwaitAssert(() =>
                {
                    ...
                    Within(TimeSpan.FromSeconds(3), () =>
                    {
                        ...
                        for (var i = 1; i <= 10; i++)
                        {
                            ...
                            probe.ExpectMsg<int>();
                            if (probe.LastSender.Path.Equals(
                                    rebalancingRegion.Path / (i % 12).ToString() / i.ToString()))
                                count++;
                        }
    ...
    private void PersistentClusterSharding_should_permanently_stop_entities_which_passivate()
    {
        Within(TimeSpan.FromSeconds(15), () =>
        {
            RunOn(() => { _ = _persistentRegion.Value; }, Config.Third, Config.Fourth, Config.Fifth);
    ...
    private void PersistentClusterSharding_should_restart_entities_which_stop_without_passivation()
    {
        Within(TimeSpan.FromSeconds(50), () =>
        {
            RunOn(() => { _ = _persistentRegion.Value; }, Config.Third, Config.Fourth);
            EnterBarrier("cluster-started-12");
        ...
        Within(TimeSpan.FromSeconds(15), () =>
        {
            ...
            RunOn(() =>
            {
                ...
                LastSender.Path.Should()
                    .Be(Node(Config.Third) / "user" / "AutoMigrateRememberRegionTestRegion" / "1" / "1");
    ...
    private void PersistentClusterSharding_should_ensure_rebalance_restarts_shards()
    {
        Within(TimeSpan.FromSeconds(50), () =>
        {
            ...
            RunOn(() => { _ = _rebalancingPersistentRegion.Value; }, Config.Fifth);
            ...
            RunOn(() =>
            {
                AwaitAssert(() =>
                {
                    ...
                    for (var i = 2; i <= 12; i++)
                    {
                        var entity = Sys.ActorSelection(_rebalancingPersistentRegion.Value.Path / (i % 12).ToString() /
                                                        i.ToString());
