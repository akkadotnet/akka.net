    ...
    public class StageActorRefSpec : AkkaSpec
    {
        ...
        public async Task A_Graph_stage_ActorRef_must_ignore_and_log_warnings_for_PoisonPill_and_Kill_messages()
        {
            ...
            warn.Message.ToString()
                .Should()
                .MatchRegex(
                    "<PoisonPill> message sent to StageActor\\(akka\\://StageActorRefSpec-1/user/StreamSupervisor-[0-9]+/\\$\\$[a-z]+\\) will be ignored, since it is not a real Actor. Use a custom message type to communicate with it instead.");
            ...
            warn.Message.ToString()
                           .Should()
                           .MatchRegex(
                               "<Kill> message sent to StageActor\\(akka\\://StageActorRefSpec-1/user/StreamSupervisor-[0-9]+/\\$\\$[a-z]+\\) will be ignored, since it is not a real Actor. Use a custom message type to communicate with it instead.");
