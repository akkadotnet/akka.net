...
public class Mailbox : IRunnable
{
     ...
     * stay as it is.
     */
    private volatile ActorCell _actor;
    private volatile SystemMessage _systemQueueDoNotCallMeDirectly; // null by default
    ...
    internal LatestFirstSystemMessageList SystemQueue =>
        ...
        new(_systemQueueDoNotCallMeDirectly);
    ...
    /// <summary>
    ///     Returns <c>true</c> if the mailbox is scheduled for execution on a <see cref="Dispatcher" />. <c>false</c>
    ///     otherwise.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    internal virtual EarliestFirstSystemMessageList SystemDrain(LatestFirstSystemMessageList newContents)
    {
        ...
        if (SystemQueuePut(currentList, newContents)) return currentList.Reverse;
...
///     Lower values will be delivered first. Messages ordered by the same number will remain in delivered in undefined
///     order.
/// </summary>
...
/// <summary>
///     UnboundedDequeBasedMailbox is an unbounded <see cref="MailboxType" /> backed by a double-ended queue. Used for
///     stashing.
/// </summary>
...
/// <summary>
///     BoundedDequeBasedMailbox is an bounded <see cref="MailboxType" /> backed by a double-ended queue. Used for
///     stashing.
/// </summary>
