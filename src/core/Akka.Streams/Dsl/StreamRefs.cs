...
{
    /// <summary>
    ///     A local <see cref="Sink{TIn,TMat}" /> which materializes a <see cref="SourceRef{T}" /> which can be used by other
    ///     streams (including remote ones),
    ...
    {
        return Sink.FromGraph(new SinkRefStageImpl<T>(null));
    }

    /// <summary>
    ///     A local <see cref="Sink{TIn,TMat}" /> which materializes a <see cref="SinkRef{T}" /> which can be used by other
    ///     streams (including remote ones),
    ///     to consume data from this local stream, as if they were attached in the spot of the local Sink directly.
    ///     Adheres to <see cref="StreamRefAttributes" />.
    ...
    {
        return Source.FromGraph(new SourceRefStageImpl<T>(null));
    }
}
...
public sealed class StreamRefSettings
{
    public StreamRefSettings(int bufferCapacity, TimeSpan demandRedeliveryInterval, TimeSpan subscriptionTimeout,
        TimeSpan finalTerminationSignalDeadline)
    {
    ...
    public static StreamRefSettings Create(Config config)
    {
        ...
        return new StreamRefSettings(
            config.GetInt("buffer-capacity", 32),
            config.GetTimeSpan("demand-redelivery-interval", TimeSpan.FromSeconds(1)),
            config.GetTimeSpan("subscription-timeout", TimeSpan.FromSeconds(30)),
            config.GetTimeSpan("final-termination-signal-deadline", TimeSpan.FromSeconds(2)));
    }

    public StreamRefSettings WithBufferCapacity(int value)
    {
        return Copy(value);
    }

    public StreamRefSettings WithDemandRedeliveryInterval(TimeSpan value)
    {
        return Copy(demandRedeliveryInterval: value);
    }

    public StreamRefSettings WithSubscriptionTimeout(TimeSpan value)
    {
        return Copy(subscriptionTimeout: value);
    }
    ...
    public StreamRefSettings Copy(int? bufferCapacity = null,
        ...
        TimeSpan? finalTerminationSignalDeadline = null)
    {
        return new StreamRefSettings(
            bufferCapacity ?? BufferCapacity,
            demandRedeliveryInterval ?? DemandRedeliveryInterval,
