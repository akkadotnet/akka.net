...
public class Discovery : IExtension
{
    ...
    {
        return _implementations.GetOrAdd(method, new Lazy<ServiceDiscovery>(() => CreateServiceDiscovery(method)))
            .Value;
    }
    ...
    private ServiceDiscovery CreateServiceDiscovery(string method)
    {
        ...
        if (!config.HasPath("class"))
            throw new ArgumentException(
                $"akka.discovery.{method} must contain field `class` that is a FQN of an `Akka.Discovery.ServiceDiscovery` implementation");
        ...
        catch (Exception ex)
        {
            if (ex is TypeLoadException or MissingMethodException)
                throw new ArgumentException(
                    $"Illegal akka.discovery.{method}.class value or incompatible class!\n" +
        ...
        ServiceDiscovery Create(string typeName)
        {
            ...
            var ctor = type.GetConstructor(
                bindFlags,
                null,
                new[] { typeof(ExtendedActorSystem), typeof(Configuration.Config) },
                null);
            if (ctor is not null)
            ...
            ctor = type.GetConstructor(
                bindFlags,
                null,
                new[] { typeof(ExtendedActorSystem) },
                null);
            if (ctor is not null)
    ...
    {
        return system.WithExtension<Discovery, DiscoveryProvider>();
    }
}

public class DiscoveryProvider : ExtensionIdProvider<Discovery>
{
    public override Discovery CreateExtension(ExtendedActorSystem system)
    {
        return new Discovery(system);
    }
    ...
    {
        return ConfigurationFactory.FromResource<Discovery>("Akka.Discovery.Resources.reference.conf");
    }
}