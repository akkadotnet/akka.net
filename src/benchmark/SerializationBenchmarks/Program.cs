...
public class JsonSerializerTests
{
    ...
    private static readonly TestSer testObj = new()
    {
        Id = 124,
        someStr =
    ...
    private readonly Serializer _noPoolSer;
    private readonly Serializer _poolSer;

    private readonly ActorSystem _sys_noPool;
    private readonly ActorSystem _sys_pool;
...
serialization-settings {{
  json {{
    ...
    public void Pooling()
    {
        for (var i = 0; i < _numIters; i++) _poolSer.ToBinary(testObj);
    }

    [Benchmark]
    public void NoPooling()
    {
        for (var i = 0; i < _numIters; i++) _noPoolSer.ToBinary(testObj);
    }
...
akka.actor {{
    ...
    {
        var res = Serialization.WithTransport(
