...
///     The array <see cref="OriginalAttributes" /> may contain the attribute information of the original atomic module,
///     otherwise
///     it must contain a none (otherwise the enclosing module could not overwrite attributes defined in this array).
///     The arrays <see cref="Inlets" /> and <see cref="Outlets" /> correspond to the notion of a *connection* in the
///     <see cref="GraphInterpreter" />. Each slot
...
///     The arrays <see cref="InletOwners" /> and <see cref="OutletOwners" /> are lookup tables from a connection id(the
///     index of the slot)
///     to a slot in the <see cref="Stages" /> array, indicating which stage is the owner of the given input or output
///     port.
...
public sealed class GraphAssembly
{
    ...
    /// <returns>TBD</returns>
    public GraphAssembly(IGraphStageWithMaterializedValue<Shape, object>[] stages, Attributes[] originalAttributes,
        Inlet[] inlets, int[] inletOwners, Outlet[] outlets, int[] outletOwners)
    {
        ...
        if (outlets.Length != outletOwners.Length)
            throw new ArgumentException("'outlets' and 'outletOwners' must have the same length.",
                nameof(outletOwners));
    ...
    /// <exception cref="ArgumentException">TBD</exception>
    /// <returns>TBD</returns>
    public static GraphAssembly Create(IList<Inlet> inlets, IList<Outlet> outlets,
        IList<IGraphStageWithMaterializedValue<Shape, object>> stages)
    {
        ...
        if (connectionsCount <= 0)
            throw new ArgumentException($"Sum of inlets ({inletsCount}) and outlets ({outletsCount}) must be > 0");

        return new GraphAssembly(
            stages.ToArray(),
            SingleNoAttribute,
            Add(inlets, new Inlet[connectionsCount], 0),
            MarkBoundary(new int[connectionsCount], inletsCount, connectionsCount),
            Add(outlets, new Outlet[connectionsCount], inletsCount),
            MarkBoundary(new int[connectionsCount], 0, inletsCount));
    }
        ...
        Action<IMaterializedValueSource> register)
    {
        ...
        for (var i = 0; i < Stages.Length; i++)
        {
            ...
            var logicAndMaterialized =
                stage.CreateLogicAndMaterializedValue(inheritedAttributes.And(OriginalAttributes[i]));
