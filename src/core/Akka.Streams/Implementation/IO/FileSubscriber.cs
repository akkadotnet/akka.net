...
internal sealed class FileSubscriber : ActorSubscriber
{
    ...
    /// If this changes you must change
    /// <see cref="FileSubscriber.Props" />
    /// as well!
        ...
        FlushSignaler flushSignaler)
    {
        ...
        _requestStrategy = new WatermarkRequestStrategy(bufferSize);
        ...
        FlushSignaler flushCommand = null)
    {
        ...
        return Actor.Props.Create<FileSubscriber>(f, completionPromise, bufferSize, startPosition, fileMode, autoFlush,
                flushCommand)
    ...
    protected override bool Receive(object message)
    {
        switch (message)
        {
            ...
            case OnError error:
                ...
                CloseAndComplete(
                    new Try<IOResult>(new AbruptIOTerminationException(IOResult.Success(_bytesWritten), error.Cause)));
                Context.Stop(Self);
                return true;
    ...
    public void Flush()
    {
        if (FileSubscriber == null)
            throw new InvalidOperationException(
                "Instance has not been initialized by passing it into a file sink factory");
