...
public class ActorLifeCycleFlowSpec : AkkaSpec
{
    ...
    [Fact(DisplayName =
        "ActorBase with manual timer trigger, Lifecycle flow during actor restart, Timers must be reset")]
    ...
    [Fact(DisplayName =
        "ActorBase with timers declared in [AroundPreRestart, PreRestart], Lifecycle flow during actor restart, Timers must be reset")]
    ...
    [Fact(DisplayName =
        "ActorBase with timers declared in [AroundPostReStart, PostRestart], Lifecycle flow during actor restart, Timers must survive")]
    ...
    private async Task AssertActorStartFlow(int id, ImmutableArray<Msg.StartTimer> timerSequence)
    {
        ...
        var startedTimers = await AssertTimerStarted(Msg.Methods.AroundPreStart, Msg.CallTime.PreBase, _emptyTimers,
            timerSequence);

        await ExpectMsgAsync(new Msg.Invocation(Msg.Methods.PreStart, id, Msg.CallTime.Single, startedTimers));
        startedTimers =
            await AssertTimerStarted(Msg.Methods.PreStart, Msg.CallTime.Single, startedTimers, timerSequence);
     ...
     *               L-> PreStart()
     */
    private async Task AssertActorRestartFlow(int id, ImmutableArray<Msg.StartTimer> startedTimers,
        ImmutableArray<Msg.StartTimer> timerSequence)
    {
        ...
        await AssertTimerStarted(Msg.Methods.AroundPreRestart, Msg.CallTime.PreBase, startedTimers,
            timerSequence); // started timers doesn't matter because Timers will be cleared

        await ExpectMsgAsync(new Msg.Invocation(Msg.Methods.PreRestart, id, Msg.CallTime.PreBase,
            _emptyTimers)); // Timers cleared
        startedTimers =
            await AssertTimerStarted(Msg.Methods.PreRestart, Msg.CallTime.PreBase, _emptyTimers, timerSequence);

        await ExpectMsgAsync(new Msg.Invocation(Msg.Methods.PostStop, id, Msg.CallTime.Single, startedTimers));
        startedTimers =
            await AssertTimerStarted(Msg.Methods.PostStop, Msg.CallTime.Single, startedTimers, timerSequence);

        await ExpectMsgAsync(new Msg.Invocation(Msg.Methods.PreRestart, id, Msg.CallTime.PostBase, startedTimers));
        startedTimers =
            await AssertTimerStarted(Msg.Methods.PreRestart, Msg.CallTime.PostBase, startedTimers, timerSequence);

        await ExpectMsgAsync(new Msg.Invocation(Msg.Methods.AroundPreRestart, id, Msg.CallTime.PostBase,
            startedTimers));
        await AssertTimerStarted(Msg.Methods.AroundPreRestart, Msg.CallTime.PostBase, startedTimers,
            timerSequence); // started timers doesn't matter because Timers will be cleared

        // ActorCell.FinishRecreate()
        await ExpectMsgAsync(new Msg.Invocation(Msg.Methods.AroundPostRestart, id, Msg.CallTime.PreBase,
            _emptyTimers)); // Timers cleared
        startedTimers = await AssertTimerStarted(Msg.Methods.AroundPostRestart, Msg.CallTime.PreBase, _emptyTimers,
            timerSequence);

        await ExpectMsgAsync(new Msg.Invocation(Msg.Methods.PostRestart, id, Msg.CallTime.PreBase, startedTimers));
        startedTimers =
            await AssertTimerStarted(Msg.Methods.PostRestart, Msg.CallTime.PreBase, startedTimers, timerSequence);
        ...
        startedTimers =
            await AssertTimerStarted(Msg.Methods.PostRestart, Msg.CallTime.PostBase, startedTimers, timerSequence);

        await ExpectMsgAsync(
            new Msg.Invocation(Msg.Methods.AroundPostRestart, id, Msg.CallTime.PostBase, startedTimers));
     ...
     *       L-> PostStop()
     */
    private async Task AssertActorStopFlow(int id, ImmutableArray<Msg.StartTimer> startedTimers,
        ImmutableArray<Msg.StartTimer> timerSequence)
    {
        // ActorCell.FinishTerminate()
        await ExpectMsgAsync(new Msg.Invocation(Msg.Methods.AroundPostStop, id, Msg.CallTime.PreBase, startedTimers));
        await AssertTimerStarted(Msg.Methods.AroundPostStop, Msg.CallTime.PreBase, startedTimers,
            timerSequence); // started timers doesn't matter because Timers will be cleared

        await ExpectMsgAsync(new Msg.Invocation(Msg.Methods.PostStop, id, Msg.CallTime.Single,
            _emptyTimers)); // Timers cleared
        startedTimers =
            await AssertTimerStarted(Msg.Methods.PostStop, Msg.CallTime.Single, _emptyTimers, timerSequence);
