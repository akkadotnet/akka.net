...
public static class ORSet
{
    ...
    {
        return ORSet<T>.Empty.Add(node, element);
    }

    public static ORSet<T> Create<T>(params KeyValuePair<UniqueAddress, T>[] elements)
    {
        return elements.Aggregate(ORSet<T>.Empty, (set, kv) => set.Add(kv.Key, kv.Value));
    }
    ...
    ///     <paramref name="dot" /> that is &lt;= an entry in <paramref name="vvector" /> is removed from
    ///     <paramref name="dot" />.
...
///     <a href="https://hal.inria.fr/file/index/docid/738680/filename/RR-8083.pdf">
///         An optimized conflict-free replicated
///         set
///     </a>
    ...
    IDeltaReplicatedData<ORSet<T>, ORSet<T>.IDeltaOperation>
{
    ...
    internal ORSet(ImmutableDictionary<T, VersionVector> elementsMap, VersionVector versionVector,
        IDeltaOperation delta)
    {
    ...
    {
        return ElementsMap.Keys.GetEnumerator();
    }
    ...
    public override ORSet<T> Merge(ORSet<T> other)
    {
        ...
        if (IsAncestorOf(other)) return other.ClearAncestor();
        return DryMerge(other, false);
    }
    ...
    {
        var pruned = ElementsMap.Aggregate(ImmutableDictionary<T, VersionVector>.Empty, (acc, kv) =>
            kv.Value.NeedPruningFrom(removedNode)
    ...
    internal static ImmutableDictionary<T, VersionVector> MergeCommonKeys(IEnumerable<T> commonKeys, ORSet<T> lhs,
        ORSet<T> rhs)
    {
        return commonKeys.Aggregate(ImmutableDictionary<T, VersionVector>.Empty, (acc, k) =>
        {
            ...
            else
            {
                ...
                else
                {
                    var rhsDots = (MultiVersionVector)r;
                    var commonDots = rhsDots.Versions
                        .Where(kv => { return rhsDots.Versions.TryGetValue(kv.Key, out var v) && v == kv.Value; })
                        .ToImmutableDictionary();
                    var commonDotKeys = commonDots.Keys.ToImmutableArray();
                    var lhsUniqueDots = lhsDots.Versions.RemoveRange(commonDotKeys);
                    var rhsUniqueDots = rhsDots.Versions.RemoveRange(commonDotKeys);
    ...
    {
        return ElementsMap.ContainsKey(elem);
    }

    /// <summary>
    ///     Adds an element to the set
    /// </summary>
    public ORSet<T> Add(Cluster.Cluster cluster, T element)
    {
        return Add(cluster.SelfUniqueAddress, element);
    }
    ...
    {
        return Remove(node.SelfUniqueAddress, element);
    }
    ...
    /// </summary>
    public ORSet<T> Clear(Cluster.Cluster node)
    {
        return Clear(node.SelfUniqueAddress);
    }
    ...
    private ORSet<T> DryMerge(ORSet<T> other, bool addDeltaOp)
    {
        ...
        var entries0 = addDeltaOp
            ...
            : MergeDisjointKeys(ElementsMap.Keys.Where(key => !other.ElementsMap.ContainsKey(key)), ElementsMap,
                other.VersionVector, entries00);
    ...
    {
        return obj is ORSet<T> set && Equals(set);
    }
    ...
    internal abstract class AtomicDeltaOperation : IDeltaOperation, IReplicatedDeltaSize, ORSet.IDeltaOperation
    {
        ...
        public bool Equals(IDeltaOperation other)
        {
            ...
            if (ReferenceEquals(this, other)) return true;
            if (other is AtomicDeltaOperation op) return Underlying.Equals(op.Underlying);
            return false;
        }
        ...
        public override int GetHashCode()
        {
            return GetType().GetHashCode() ^ Underlying.GetHashCode();
        }
    }
        ...
        public RemoveDeltaOperation(ORSet<T> underlying)
        {
            if (underlying.Count != 1)
                throw new ArgumentException(
                    $"RemoveDeltaOperation should contain one removed element, but was {underlying}");
    ...
    internal sealed class DeltaGroup : IDeltaOperation, IReplicatedDeltaSize, ORSet.IDeltaGroupOperation
    {
        ...
        public bool Equals(IDeltaOperation other)
        {
            if (ReferenceEquals(null, other)) return false;
            if (ReferenceEquals(this, other)) return true;
            if (other is DeltaGroup group) return Operations.SequenceEqual(group.Operations);
            return false;
        }
        ...
        public override int GetHashCode()
        {
            ...
            unchecked
            {
                foreach (var op in Operations) hash = (hash * 297) ^ op.GetHashCode();
                return hash;
            }
        }
    }
    ...
    public ORSet<T> MergeDelta(IDeltaOperation delta)
    {
        ...
        switch (delta)
        {
            case AddDeltaOperation op: return DryMerge(op.Underlying, true);
            case RemoveDeltaOperation op: return MergeRemoveDelta(op);
            case FullStateDeltaOperation op: return DryMerge(op.Underlying, false);
                ...
                foreach (var operation in group.Operations)
                    switch (operation)
                    {
                        case AddDeltaOperation op:
                            acc = acc.DryMerge(op.Underlying, true);
                            break;
                        case RemoveDeltaOperation op:
                            acc = acc.MergeRemoveDelta(op);
                            break;
                        case FullStateDeltaOperation op:
                            acc = acc.DryMerge(op.Underlying, false);
                            break;
