...
///     the <see cref="ShardRegion" /> and then the <see cref="ShardRegion" /> actor will be stopped. You can
///     <see cref="ICanWatch.Watch" />
...
internal sealed class ShardInitialized : IEquatable<ShardInitialized>
{
    ...
    {
        return $"ShardInitialized({ShardId})";
    }
...
///     In order for this query to work, the <see cref="MessageExtractor" /> must support
///     <see cref="ShardRegion.StartEntity" />,
...
public sealed class CurrentRegions : IClusterShardingSerializable, IEquatable<CurrentRegions>
{
    ...
    public override string ToString()
    {
        return $"CurrentRegions({string.Join(", ", Regions.Select(r => $"[{r}]"))})";
    }
...
public sealed class GetClusterShardingStats : IShardRegionQuery, IClusterShardingSerializable,
    IEquatable<GetClusterShardingStats>
{
    /// <summary>
    ...
    public override string ToString()
    {
        return $"GetClusterShardingStats({Timeout})";
    }
...
public sealed class ClusterShardingStats : IClusterShardingSerializable, IEquatable<ClusterShardingStats>
{
    ...
    public override string ToString()
    {
        return $"ClusterShardingStats({string.Join(", ", Regions.Select(r => $"[{r.Key}]:[{r.Value}]"))})";
    }
...
public sealed class ShardRegionStats : IClusterShardingSerializable, IEquatable<ShardRegionStats>
{
    ...
    {
        return
            $"ShardRegionStats[stats={string.Join(", ", Stats.Select(i => $"({i.Key}:{i.Value})"))}, failed={string.Join(", ", Failed)}]";
    }
