...
public class PersistentShardingMigrationSpec : AkkaSpec
{
    ...
    private void WithSystem(Config config, string typeName, string systemName,
        Action<ActorSystem, IActorRef, TestProbe> f)
    {
        ...
        AwaitAssert(() => { cluster.SelfMember.Status.Should().Be(MemberStatus.Up); });
