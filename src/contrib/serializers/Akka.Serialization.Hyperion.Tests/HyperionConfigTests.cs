    ...
    public void Hyperion_serializer_should_allow_to_setup_custom_flags()
    {
        ...
        using (var system = ActorSystem.Create(nameof(HyperionConfigTests), config))
        {
            ...
            Assert.Equal(
                ...
                ((TypeFilter)serializer.Settings.TypeFilter).FilteredTypes.First());
        }
    }
...
internal class DummyTypesProvider : IKnownTypesProvider
{
...
internal class DummyTypesProviderWithDefaultCtor : IKnownTypesProvider
{
    public IEnumerable<Type> GetKnownTypes()
    {
        return Enumerable.Empty<Type>();
