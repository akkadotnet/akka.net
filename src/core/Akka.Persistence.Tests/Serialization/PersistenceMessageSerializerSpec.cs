...
public class PersistenceMessageSerializerSpec : AkkaSpec
{
    ...
    public void MessageSerializer_should_serialize_AtLeastOnceDeliverySnapshot()
    {
        var unconfirmed = new[]
        {
            new UnconfirmedDelivery(1, TestActor.Path, "a"), new UnconfirmedDelivery(2, TestActor.Path, "b"),
    ...
    {
        var snapshot =
            new PersistentFSM.PersistentFSMSnapshot<MyPayload>("a", new MyPayload("b"), TimeSpan.FromSeconds(10));
    ...
    public void MessageSerializer_ToBinary_should_throw_an_exception_on_wrong_type()
    {
        ...
        serializeAction.Should().Throw<ArgumentException>()
            .WithMessage(
                $"Can't serialize object of type [{typeof(string)}] in [{typeof(PersistenceMessageSerializer)}]");

        var deserializeAction = () => { _serializer.FromBinary<string>(new byte[] { 4, 5, 6 }); };

        deserializeAction.Should().Throw<SerializationException>()
            .WithMessage(
                $"Unimplemented deserialization of message with type [{typeof(string)}] in [{typeof(PersistenceMessageSerializer)}]");
    }
}