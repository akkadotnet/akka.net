...
internal abstract class CommonHandlers : ChannelHandlerAdapter
{
    ...
    public override void ExceptionCaught(IChannelHandlerContext context, Exception exception)
    {
        ...
        Log.Error(exception, "Error caught channel [{0}->{1}](Id={2})", context.Channel.LocalAddress,
            context.Channel.RemoteAddress, context.Channel.Id);
    }
    ...
    {
        var localAddress = DotNettyTransport.MapSocketToAddress((IPEndPoint)channel.LocalAddress,
            Transport.SchemeIdentifier, Transport.System.Name, Transport.Settings.Hostname);
...
internal abstract class DotNettyTransport : Transport
{
    ...
    protected DotNettyTransport(ActorSystem system, Config config)
    {
        ...
        SchemeIdentifier = (Settings.EnableSsl ? "ssl." : string.Empty) +
                           Settings.TransportMode.ToString().ToLowerInvariant();
    }
    ...
    public sealed override bool IsResponsibleFor(Address remote)
    {
        return true;
    }
    ...
    protected async Task<IChannel> NewServer(EndPoint listenAddress)
    {
        ...
        try
        {
            if (listenAddress is DnsEndPoint dns) listenAddress = await DnsToIPEndpoint(dns).ConfigureAwait(false);

            return await ServerFactory().BindAsync(listenAddress).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            throw new RemoteTransportException($"Failed to bind to [{listenAddress}]. See InnerException for details.",
                ex);
        }
    }
    ...
    public override async Task<(Address, TaskCompletionSource<IAssociationEventListener>)> Listen()
    {
        ...
        try
        {
            ...
            var addr = MapSocketToAddress(
                (IPEndPoint)newServerChannel.LocalAddress,
                SchemeIdentifier,
                System.Name,
...
#pragma warning restore 4014
    ...
    public override async Task<bool> Shutdown()
    {
        try
        {
            ...
            foreach (var channel in ConnectionGroup) tasks.Add(channel.CloseAsync());
...
#pragma warning restore 4014
    ...
    protected Bootstrap ClientFactory(Address remoteAddress)
    {
        ...
        var client = new Bootstrap()
            ...
            .Handler(new ActionChannelInitializer<TcpSocketChannel>(
                channel => SetClientPipeline(channel, remoteAddress)));

        if (Settings.ReceiveBufferSize.HasValue)
            client.Option(ChannelOption.SoRcvbuf, Settings.ReceiveBufferSize.Value);
        ...
        if (Settings.WriteBufferHighWaterMark.HasValue)
            client.Option(ChannelOption.WriteBufferHighWaterMark, Settings.WriteBufferHighWaterMark.Value);
        if (Settings.WriteBufferLowWaterMark.HasValue)
            client.Option(ChannelOption.WriteBufferLowWaterMark, Settings.WriteBufferLowWaterMark.Value);
    ...
    private void SetInitialChannelPipeline(IChannel channel)
    {
        ...
        if (Settings.LogTransport) pipeline.AddLast("Logger", new AkkaLoggingHandler(Log));

        if (InternalTransport == TransportMode.Tcp)
        {
            pipeline.AddLast("FrameDecoder",
                new LengthFieldBasedFrameDecoder(Settings.ByteOrder, (int)MaximumPayloadBytes, 0, 4, 0, 4, true));
        ...
        if (Settings.BatchWriterSettings.EnableBatching)
            pipeline.AddLast("BatchWriter",
                new FlushConsolidationHandler(Settings.BatchWriterSettings.MaxExplicitFlushes));
    }
    ...
    private void SetClientPipeline(IChannel channel, Address remoteAddress)
    {
        if (Settings.EnableSsl)
        {
            ...
            var tlsHandler = Settings.Ssl.SuppressValidation
                ? new TlsHandler(stream => new SslStream(stream, true, (_, _, _, _) => true),
    ...
    {
        if (Settings.EnableSsl) channel.Pipeline.AddFirst("TlsHandler", TlsHandler.Server(Settings.Ssl.Certificate));
    ...
    public static Address MapSocketToAddress(IPEndPoint socketAddress, string schemeIdentifier, string systemName,
        string hostName = null, int? publicPort = null)
    {
        return socketAddress == null
            ? null
            : new Address(schemeIdentifier, systemName, SafeMapHostName(hostName) ?? SafeMapIPv6(socketAddress.Address),
    ...
    {
        return ip.AddressFamily == AddressFamily.InterNetworkV6 ? "[" + ip + "]" : ip.ToString();
    }

    public static EndPoint ToEndpoint(Address address)
    {
        if (!address.Port.HasValue)
            throw new ArgumentNullException(nameof(address), $"Address port must not be null: {address}");

        return IPAddress.TryParse(address.Host, out var ip)
            ? new IPEndPoint(ip, address.Port.Value)
...
internal sealed class HeliosBackwardsCompatabilityLengthFramePrepender : LengthFieldPrepender
{
    ...
    public HeliosBackwardsCompatabilityLengthFramePrepender(int lengthFieldLength,
        bool lengthFieldIncludesLengthFieldLength) : base(ByteOrder.LittleEndian, lengthFieldLength, 0,
        lengthFieldIncludesLengthFieldLength)
    {
    }
    ...
    protected override void Encode(IChannelHandlerContext context, IByteBuffer message, List<object> output)
    {
        ...
        message.SafeRelease(1); // ready to release it - bytes have been copied
