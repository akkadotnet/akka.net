...
{
    public static readonly Config Config = ConfigurationFactory.ParseString("akka.scheduler.implementation = \"" +
                                                                            typeof(ShutdownScheduler)
    ...
    public async Task ActorSystem_must_terminate_scheduler_with_queued_work_on_shutdown()
    {
        ...
        try
        {
            ...
            sys = ActorSystem.Create("SchedulerShutdownSys1", Config);
            var scheduler = (ShutdownScheduler)sys.Scheduler;
            sys.Scheduler.Advanced.ScheduleRepeatedly(TimeSpan.FromMilliseconds(0), TimeSpan.FromMilliseconds(10),
                () => i++);
    ...
    {
        var sys = ActorSystem.Create("SchedulerShutdownSys2");
    ...
    {
        var sys = ActorSystem.Create("SchedulerShutdownSys3");
    ...
    private class MyScheduledActor : ReceiveActor
    {
        ...
        {
            Receive<string>(str => str.Equals("get"), _ => { Sender.Tell(_received); });

            Receive<string>(str => str.Equals("set"), _ => { _received = true; });
        }
    }
}