...
akka.remote.default-remote-dispatcher {
    ...
    public class Settings
    {
        ...
        public Settings(Config config, int totalNumberOfNodes)
        {
            ...
            JoinRemoveDuration =
                TimeSpan.FromMilliseconds(_testConfig.GetTimeSpan("join-remove-duration").TotalMilliseconds * DFactor);
            IdleGossipDuration =
                TimeSpan.FromMilliseconds(_testConfig.GetTimeSpan("idle-gossip-duration").TotalMilliseconds * DFactor);
            ExpectedTestDuration =
                TimeSpan.FromMilliseconds(_testConfig.GetTimeSpan("expected-test-duration").TotalMilliseconds *
                                          DFactor);
...
internal class ClusterResultAggregator : ReceiveActor
{
    ...
    private ImmutableSortedDictionary<Address, ClusterEvent.CurrentInternalStats> _clusterStatsObservedByNode =
        ImmutableSortedDictionary<Address, ClusterEvent.CurrentInternalStats>.Empty.WithComparers(
            Member.AddressOrdering);
    ...
    public ClusterResultAggregator(string title, int expectedResults, StressSpecConfig.Settings settings)
    {
        ...
        Receive<ReportTick>(_ =>
        {
            if (_settings.Infolog)
                _log.Info(
                    "BEGIN CLUSTER OPERATION: [{0}] in progress" + Environment.NewLine + "{1}" + Environment.NewLine +
                    "{2}", _title,
            ...
            if (_results.Count == _expectedResults)
            {
                ...
                if (_settings.Infolog)
                    _log.Info("END CLUSTER OPERATION: [{0}] completed in [{1}] ms" + Environment.NewLine + "{2}" +
                              Environment.NewLine + "{3}" + Environment.NewLine + "{4}", _title,
                        aggregated.Duration.TotalMilliseconds,
        ...
        Receive<ReportTo>(re => { _reportTo = re.Ref; });
    }
    ...
    {
        if (_phiValuesObservedByNode.IsEmpty)
        {
            return string.Empty;
        }

        var lines = from mon in _phiValuesObservedByNode
            from phi in mon.Value
            select FormatPhiLine(mon.Key, phi.Address, phi);
...
{
    public static readonly string FormatHistoryHeader =
    ...
    {
        Receive<AggregatedClusterResult>(result => { _history = _history.Add(result); });
    }
...
internal class PhiObserver : ReceiveActor
{
    ...
    private readonly ICancelable _checkPhiTask = Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(
    ...
    public PhiObserver()
    {
            ...
            foreach (var node in _nodes)
            {
                ...
                if (p > 0 || _cluster.FailureDetector.IsMonitoring(node))
                {
                    if (double.IsInfinity(p))
                    {
                        ...
                        _log.Warning(
                            "PhiValues: (Timestamp={0}, Mean={1}, Variance={2}, StdDeviation={3}, Intervals=[{4}])",
                            time,
        ...
        Receive<ClusterEvent.IMemberEvent>(m => { _nodes.Add(m.Member.Address); });
...
internal class StatsObserver : ReceiveActor
{
    ...
    public StatsObserver()
    {
        ...
        Receive<Reset>(_ => { _startStats = Option<GossipStats>.None; });
...
internal sealed class StatsResult
{
    ...
    public ClusterEvent.CurrentInternalStats Stats { get; }
}
    ...
    public MeasureDurationUntilDown()
    {
        ...
        Receive<ClusterEvent.MemberDowned>(d =>
        {
            ...
            if (m.UniqueAddress == _cluster.SelfUniqueAddress)
                _log.Info("Downed [{0}] after [{1} ms]", _cluster.SelfAddress,
                    TimeSpan.FromTicks(MonotonicClock.GetTicks() - _startTime).TotalMilliseconds);
        });
...
public class StressSpec : MultiNodeClusterSpec
{
    ...
    public override void MuteLog(ActorSystem sys = null)
    {
        sys ??= Sys;
        base.MuteLog(sys);
        Sys.EventStream.Publish(new Mute(new ErrorFilter(typeof(ApplicationException),
            new ContainsString("Simulated exception"))));
        MuteDeadLetters(sys, typeof(AggregatedClusterResult), typeof(StatsResult), typeof(PhiResult),
            typeof(RetryTick));
    }
    ...
    public void JoinOne()
    {
        Within(TimeSpan.FromSeconds(5) + ConvergenceWithin(TimeSpan.FromSeconds(2), NbrUsedRoles + 1), () =>
        {
            ...
            CreateResultAggregator(title, currentRoles.Length, true);
            RunOn(() =>
            {
                ReportResult(() =>
                {
                    RunOn(() => { Cluster.Join(GetAddress(Roles.First())); }, currentRoles.Last());
    ...
    public void RemoveOne(bool shutdown)
    {
            ...
            =>
        {
            ...
            Console.WriteLine(
                $"Preparing to {FormatNodeLeave()}[{removeAddress}] role [{removeRole.Name}] out of [{Roles.Count}]");
            RunOn(() =>
            {
            ...
            RunOn(() =>
            {
                AwaitAssert(() =>
                {
                    Sys.ActorSelection(new RootActorPath(removeAddress) / "user" / "watchee")
                        .Tell(new Identify("watchee"), IdentifyProbe.Ref);
            ...
            RunOn(() =>
            {
                ReportResult(() =>
                {
                    RunOn(() =>
                    {
                        if (shutdown)
                        {
                            if (Settings.Infolog) Log.Info("Shutting down [{0}]", removeAddress);
    ...
    public void RemoveSeveral(int numberOfNodes, bool shutdown)
    {
        ...
        Within(TimeSpan.FromSeconds(25) + ConvergenceWithin(TimeSpan.FromSeconds(5), NbrUsedRoles - numberOfNodes),
            () =>
            {
                ...
                RunOn(() =>
                {
                    if (!shutdown) Cluster.Leave(GetAddress(Myself));
                }, removeRoles);
    ...
    public void PartitionSeveral(int numberOfNodes)
    {
        Within(TimeSpan.FromSeconds(25) + ConvergenceWithin(TimeSpan.FromSeconds(5), NbrUsedRoles - numberOfNodes),
            () =>
            {
                ...
                Console.WriteLine("[{0}] are blackholing [{1}]",
                    string.Join(",", currentRoles.Select(x => x.ToString())),
                    string.Join(",", removeRoles.Select(x => x.ToString())));
                CreateResultAggregator(title, currentRoles.Length, true);
                ...
                RunOn(() =>
                {
                    ReportResult(() =>
                    {
                        ...
                        Sys.Log.Info("Removed [{0}] members after [{0} ms]",
                            removeRoles.Length,
                            TimeSpan.FromTicks(MonotonicClock.GetTicks() - startTime).TotalMilliseconds);
                        AwaitAllReachable();
                        return true;
                    });
                }, currentRoles);
    ...
    public T ReportResult<T>(Func<T> thunk)
    {
        ...
        ClusterResultAggregator().OnSuccess(r =>
        {
            r.Tell(new ClusterResult(Cluster.SelfAddress, TimeSpan.FromTicks(MonotonicClock.GetTicks() - startTime),
    ...
    public void ExerciseJoinRemove(string title, TimeSpan duration)
    {
            ...
            ImmutableHashSet<Address> allPreviousAddresses)
        {
            ...
            CreateResultAggregator(t, NbrUsedRoles, true);
            ...
            Within(loopDuration, () =>
            {
                var (nextAsy, nextAddr) = ReportResult(() =>
                {
                    ...
                    {
                        previousAs.OnSuccess(s => { Shutdown(s); });

                        var sys = ActorSystem.Create(Sys.Name, Sys.Settings.Config);
                        MuteLog(sys);
                        Cluster.Get(sys).JoinSeedNodes(SeedNodes.Select(x => GetAddress(x)));
                    ...
                    RunOn(() => { nextAddresses.Count.Should().Be(Settings.NumberOfNodesJoinRemove); }, usedRoles);
        ...
        Loop(1, Option<ActorSystem>.None, ImmutableHashSet<Address>.Empty).OnSuccess(aSys => { Shutdown(aSys); });
    ...
    {
        CreateResultAggregator(title, NbrUsedRoles, true);
        ReportResult(() =>
        {
    ...
    public void MustLogSettings()
    {
        if (Settings.Infolog)
        {
            ...
            RunOn(() => { Log.Info("StressSpec settings:" + Environment.NewLine + Settings); });
        }
    ...
    public void MustJoinSeedNodes()
    {
        Within(TimeSpan.FromSeconds(30), () =>
        {
            ...
            CreateResultAggregator("join seed nodes", size, true);
    ...
    {
        RemoveOneByOne(Settings.NumberOfNodesLeavingOneByOneLarge, false);
        EnterBarrier("after-" + Step);
    }

    public void MustShutdownNodesOneByOneFromLargeCluster()
    {
        RemoveOneByOne(Settings.NumberOfNodesShutdownOneByOneLarge, true);
        EnterBarrier("after-" + Step);
    }

    public void MustLeaveSeveralNodes()
    {
        RemoveSeveral(Settings.NumberOfNodesLeaving, false);
    ...
    {
        RemoveSeveral(Settings.NumberOfNodesShutdown, true);
    ...
    {
        if (Settings.Infolog) Log.Info("StressSpec CLR: " + Environment.NewLine + "{0}", ClrInfo());
        EnterBarrier("after-" + Step);
    }
}