...
namespace Akka.Remote
{
    ...
    public class AkkaProtocolSettings
    {
        ...
        public AkkaProtocolSettings(Config config)
        {
            ...
            TransportFailureDetectorConfig = config.GetConfig("akka.remote.classic.transport-failure-detector");
            ...
            var enabledTransports = config.GetStringList("akka.remote.classic.enabled-transports", new string[] { });
            if (enabledTransports.Contains("akka.remote.classic.dot-netty.tcp"))
                HandshakeTimeout = config.GetTimeSpan("akka.remote.classic.dot-netty.tcp.connection-timeout", null);
            else if (enabledTransports.Contains("akka.remote.classic.dot-netty.ssl"))
                HandshakeTimeout = config.GetTimeSpan("akka.remote.classic.dot-netty.ssl.connection-timeout", null);
            else
                HandshakeTimeout = config.GetTimeSpan("akka.remote.classic.handshake-timeout", TimeSpan.FromSeconds(20), allowInfinite:false);
        }
    }
}
