...
internal sealed class ChannelSinkLogic<T> : InGraphStageLogic
{
    ...
    private void OnWriteFailed(Exception cause)
    {
        TryFail(cause);
    }
...
internal sealed class ChannelReaderSink<T> : GraphStageWithMaterializedValue<SinkShape<T>, ChannelReader<T>>
{
    ...
    public ChannelReaderSink(int bufferSize, bool singleReader = false,
    ...
    public override ILogicAndMaterializedValue<ChannelReader<T>> CreateLogicAndMaterializedValue(
        Attributes inheritedAttributes)
    {
        ...
        return new LogicAndMaterializedValue<ChannelReader<T>>(
            new ChannelSinkLogic<T>(Shape, Inlet, channel.Writer, true), channel);
    }
}
...
internal sealed class ChannelWriterSink<T> : GraphStage<SinkShape<T>>
{
    ...
    {
        return new ChannelSinkLogic<T>(Shape, Inlet, _writer, _isOwner);
    }
}