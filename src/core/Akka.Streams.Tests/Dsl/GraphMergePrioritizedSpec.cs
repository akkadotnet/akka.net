...
public class GraphMergePrioritizedSpec : TwoStreamsSetup<int>
{
    ...
    {
        return new MergePrioritizedFixture(builder);
    }
    ...
    public void MergePrioritized_must_stream_data_when_only_one_source_produces()
    {
        ...
        var ones = collected.Count(x => x == 1);
        var twos = collected.Count(x => x == 2);
        var threes = collected.Count(x => x == 3);
    ...
    private IRunnableGraph<(NotUsed, NotUsed, NotUsed)> ThreeSourceMerge<T>(Source<T, NotUsed> source1,
        Source<T, NotUsed> source2,
        Source<T, NotUsed> source3, List<int> priorities, TestSubscriber.ManualProbe<T> probe)
    {
        return RunnableGraph.FromGraph(GraphDsl.Create(source1, source2, source3, ValueTuple.Create,
            (builder, s1, s2, s3) =>
            {
