...
public class FileSinkSpec : AkkaSpec
{
    ...
    public async Task SynchronousFileSink_should_write_into_existing_file_without_wiping_existing_data()
    {
        await WithinAsync(_expectTimeout, async () =>
        {
            await TargetFileAsync(async f =>
            {
                Task<IOResult> Write(IEnumerable<string> lines)
                {
                    return Source.From(lines)
    ...
    public async Task SynchronousFileSink_should_by_default_replace_the_existing_file()
    {
        await WithinAsync(_expectTimeout, async () =>
        {
            await TargetFileAsync(async f =>
            {
                ...
                {
                    return Source.From(lines).Select(ByteString.FromString)
                        .RunWith(FileIO.ToFile(f), _materializer);
                }
        ...
        await WithinAsync(_expectTimeout, async () =>
        {
            await TargetFileAsync(async f =>
            {
                Task<IOResult> Write(List<string> lines)
                {
                    return Source.From(lines)
                        .Select(ByteString.FromString)
                        .RunWith(FileIO.ToFile(f, FileMode.Append), _materializer);
                }
    ...
    public async Task SynchronousFileSink_should_allow_writing_from_specific_position_to_the_file()
    {
        await WithinAsync(_expectTimeout, async () =>
        {
            await TargetFileAsync(async f =>
            {
                ...
                var testLinesPart2 = new List<string> { new string('x', 1000) + "\n", new string('x', 1000) + "\n" };

                Task<IOResult> Write(List<string> lines, long pos)
                {
                    return Source.From(lines)
                        .Select(ByteString.FromString)
                        .RunWith(
                            FileIO.ToFile(f, FileMode.OpenOrCreate, pos),
                            _materializer);
                }
            ...
            await TargetFileAsync(f =>
            {
                ...
                try
                {
                    ...
                    Source.FromEnumerator(
                            () => Enumerable.Repeat(_testByteStrings.Head(), int.MaxValue).GetEnumerator())
                        .To(FileIO.ToFile(f))
                    ...
                    ((ActorMaterializerImpl)materializer).Supervisor.Tell(StreamSupervisor.GetChildren.Instance,
                        TestActor);
                    var actorRef = ExpectMsg<StreamSupervisor.Children>().Refs
                        .First(@ref => @ref.Path.ToString().Contains("File"));
    ...
    public async Task SynchronousFileSink_should_complete_materialized_task_with_an_exception_when_upstream_fails()
    {
        await TargetFileAsync(async f =>
        {
            ...
            var ex = await InterceptAsync<AbruptIOTerminationException>(() => completion)
    ...
    public async Task SynchronousFileSink_should_complete_with_failure_when_file_cannot_be_open()
    {
        await TargetFileAsync(async _ =>
        {
            var completion = Source.Single(ByteString.FromString("42"))
                .RunWith(FileIO.ToFile(new FileInfo("I-hope-this-file-doesnt-exist.txt"), FileMode.Open),
                    _materializer);
        ...
        await WithinAsync(TimeSpan.FromSeconds(10), async () =>
        {
            await TargetFileAsync(async f =>
            {
                var (actor, task) = Source.ActorRef<string>(64, OverflowStrategy.DropNew)
                    .Select(ByteString.FromString)
                    .ToMaterialized(
                        FileIO.ToFile(f, FileMode.OpenOrCreate, 0, true),
                        Keep.Both)
                    .Run(_materializer);
                Watch(actor);
        ...
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            await TargetFileAsync(async f =>
            {
                ...
                var (actor, task) = Source.ActorRef<string>(64, OverflowStrategy.DropNew)
                    .Select(ByteString.FromString)
                    .ToMaterialized(
                        FileIO.ToFile(f, FileMode.OpenOrCreate, 0, flushSignaler: flusher),
                        (a, t) => (a, t))
                    .Run(_materializer);
