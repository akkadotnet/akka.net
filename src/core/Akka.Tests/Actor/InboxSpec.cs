...
public class InboxSpec : AkkaSpec
{
    ...
    public void Inbox_support_queueing_multiple_queries()
    {
        var tasks = new[]
        {
            Task.Factory.StartNew(() => _inbox.Receive()), Task.Factory.StartNew(() =>
            {
                Thread.Sleep(100);
    ...
    public async Task Inbox_have_maximum_queue_size()
    {
        try
        {
            ...
            await EventFilter.Warning(start: "Dropping message").ExpectOneAsync(() =>
            {
                _inbox.Receiver.Tell(42);
            ...
            foreach (var o in gotit) o.ShouldBe(0);
    ...
    public void Select_WithClient_should_update_Client_and_copy_the_rest_of_the_properties_BUG_427()
    {
        ...
        var actorRef = new EmptyLocalActorRef(((ActorSystemImpl)Sys).Provider,
            new RootActorPath(new Address("akka", "test")), Sys.EventStream);
        var select = new Select(deadline, predicate, actorRef);

        var updatedActorRef = new EmptyLocalActorRef(((ActorSystemImpl)Sys).Provider,
            new RootActorPath(new Address("akka2", "test2")), Sys.EventStream);
