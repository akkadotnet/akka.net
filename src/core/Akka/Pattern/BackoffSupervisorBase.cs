...
public abstract class BackoffSupervisorBase : ActorBase
{
    ...
    {
        if (Child == null) Child = Context.Watch(Context.ActorOf(ChildProps, ChildName));
    }
    ...
    protected bool HandleBackoff(object message)
    {
        switch (message)
        {
            ...
            {
                if (count.Current == RestartCountN) RestartCountN = 0;
                break;
            }
            ...
            default:
            {
                ...
                else
                {
                    ...
                    if (FinalStopMessage != null && FinalStopMessage(message)) Context.Stop(Self);
                }
