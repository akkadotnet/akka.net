...
public class CoordinatedShutdownSpec : AkkaSpec
{
    ...
    private static readonly Reason customReason = new CustomReason();
    public ExtendedActorSystem ExtSys => Sys.AsInstanceOf<ExtendedActorSystem>();

    private Phase Phase(params string[] dependsOn)
    {
        return new Phase(dependsOn?.ToImmutableHashSet() ?? ImmutableHashSet<string>.Empty, TimeSpan.FromSeconds(10),
            true);
    }

    private List<string> CheckTopologicalSort(Dictionary<string, Phase> phases)
    {
        var result = TopologicalSort(phases);
    ...
    public void CoordinatedShutdown_must_sort_phases_in_topological_order()
    {
        ...
        {
            { "b", Phase("a") }, { "c", Phase("b") }, { "d", Phase("b", "c") }, { "e", Phase("d") }
        ...
        {
            { "a2", Phase("a1") }, { "a3", Phase("a2") }, { "b2", Phase("b1") }, { "b3", Phase("b2") }
        });
    ...
    public void CoordinatedShutdown_must_detect_cycles_in_phases_non_DAG()
    {
        ...
        {
            TopologicalSort(new Dictionary<string, Phase> { { "a", Phase("a") } });
        });

        Assert.Throws<ArgumentException>(() =>
        {
            TopologicalSort(new Dictionary<string, Phase> { { "b", Phase("a") }, { "a", Phase("b") } });
        });

        Assert.Throws<ArgumentException>(() =>
        {
            TopologicalSort(new Dictionary<string, Phase>
            {
    ...
    {
        Get(Sys).OrderedPhases.Should().Equal(PhaseBeforeServiceUnbind, PhaseServiceUnbind, PhaseServiceRequestsDone,
            PhaseServiceStop, PhaseBeforeClusterShutdown, PhaseClusterShardingShutdownRegion, PhaseClusterLeave,
            PhaseClusterExiting, PhaseClusterExitingDone, PhaseClusterShutdown, PhaseBeforeActorSystemTerminate,
    ...
    public async Task CoordinatedShutdown_must_run_ordered_phases()
    {
        ...
        {
            { "a", EmptyPhase }, { "b", Phase("a") }, { "c", Phase("b", "a") }
        };
        ...
        await co.Run(UnknownReason.Instance).AwaitWithTimeout(RemainingOrDefault);
        (await ReceiveNAsync(4).ToListAsync()).Should().Equal("A", "B", "B", "C");
    }
    ...
    public async Task CoordinatedShutdown_must_run_from_given_phase()
    {
        ...
        (await ReceiveNAsync(2).ToListAsync()).Should().Equal("B", "C");
    ...
    public async Task CoordinatedShutdown_must_abort_if_recover_is_off()
    {
        ...
        var task = co.Run(UnknownReason.Instance);
        await ExpectMsgAsync("B");
    ...
    {
        var phases = new Dictionary<string, Phase> { { "a", EmptyPhase }, { "b", Phase("a") } };
    ...
    {
        PhasesFromConfig(ConfigurationFactory.ParseString(@"
            ...
            .Equal(new Dictionary<string, Phase>
            {
                ...
                {
                    "c",
                    new Phase(ImmutableHashSet<string>.Empty.Add("a").Add("b"), TimeSpan.FromSeconds(10), false)
                }
            });
    }
    ...
    public async Task CoordinatedShutdown_must_be_run_by_ActorSystem_Terminate()
    {
        await Sys.Terminate();
        Sys.WhenTerminated.IsCompleted.Should().BeTrue();
        Get(Sys).ShutdownReason.Should().BeEquivalentTo(ActorSystemTerminateReason.Instance);
    }

    [Fact]
    public async Task
        CoordinatedShutdown_must_not_be_run_by_ActorSystem_Terminate_when_run_by_actor_system_terminate_is_off()
    {
    ...
    [Fact]
    public void
        CoordinatedShutdown_must_not_allow_terminate_actor_system_set_to_off_and_run_by_actor_system_terminate_set_to_on()
    {
        var act = () =>
        {
    ...
    private class CustomReason : Reason
    {
    }
}