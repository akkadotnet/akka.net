...
namespace Akka.Persistence.Fsm
{
    ...
    public abstract class PersistentFSM<TState, TData, TEvent> : PersistentFSMBase, IListeners
    {
        ...
        private void BaseApplyState(State<TState, TData, TEvent> upcomingState)
        {
        ...
        private void MakeTransition(State<TState, TData, TEvent> upcomingState)
        {
            ...
            else
            {
                ...
                if (timeout.HasValue)
                {
                    ...
                    {
                        _timeoutFuture = Context.System.Scheduler.ScheduleTellOnceCancelable(t, Context.Self, new PersistentFSMBase.TimeoutMarker(_generation), Context.Self);
                    }
                }
            }
        }
        ...
        {
            var match = PatternMatch.Match(message)
                .With<PersistentFSMBase.TimeoutMarker>(marker =>
                {
