...
public abstract class BatchingInputBuffer : IInputs
{
    ...
    ///     This exception is thrown when the specified <paramref name="count" /> is either less than or equal to zero or is
    ///     not a power of two.
    /// </exception>
    ...
    {
        return
            $"BatchingInputBuffer(Count={Count}, elems={_inputBufferElements}, completed={IsClosed}, remaining={_batchRemaining})";
    }
    ...
    protected virtual void OnSubscribe(ISubscription subscription)
    {
        if (subscription == null)
            throw new ArgumentNullException(nameof(subscription), "OnSubscribe require subscription not to be null");
...
internal abstract class ActorProcessorImpl : ActorBase, IPump
{
    ...
    private sealed class InternalExposedPublisherReceive : ExposedPublisherReceive
    {
        ...
        public InternalExposedPublisherReceive(Receive activeReceive, Action<object> unhandled, ActorProcessorImpl self)
            : base(activeReceive, unhandled)
        {
            _self = self;
        }
