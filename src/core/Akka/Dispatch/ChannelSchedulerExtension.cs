...
{
    [ThreadStatic] private static TaskSchedulerPriority _threadPriority = TaskSchedulerPriority.None;
    ...
    public ChannelTaskScheduler(ExtendedActorSystem system)
    {
        ...
        {
            AllowSynchronousContinuations = true, SingleReader = _maximumConcurrencyLevel == 1, SingleWriter = false
        };

        _highScheduler = new PriorityTaskScheduler(Channel.CreateUnbounded<Task>(channelOptions),
            TaskSchedulerPriority.AboveNormal);
        _normalScheduler =
            new PriorityTaskScheduler(Channel.CreateUnbounded<Task>(channelOptions), TaskSchedulerPriority.Normal);
        _lowScheduler =
            new PriorityTaskScheduler(Channel.CreateUnbounded<Task>(channelOptions), TaskSchedulerPriority.Low);
        _idleScheduler =
            new PriorityTaskScheduler(Channel.CreateUnbounded<Task>(channelOptions), TaskSchedulerPriority.Idle);
    ...
    private async Task ControlAsync()
    {
        ...
        var readTasks = new[]
        {
            highReader.WaitToReadAsync().AsTask(), normalReader.WaitToReadAsync().AsTask(),
            lowReader.WaitToReadAsync().AsTask(), idleReader.WaitToReadAsync().AsTask()
        };
    ...
    private void ScheduleCoWorkers(object state)
    {
        ...
        if (newWorkerToStart == 0 && reqWorkerCount > controlWorkerCount &&
            controlWorkerCount + coworkerCount < _maximumConcurrencyLevel)
            newWorkerToStart = 1;
    ...
    private int DoWork(int workerId)
    {
        ...
        int c;
        var rounds = 0; //loop count
        int roundWork; //work count in the current loop
        var roundClean = 0; //clean loop count
