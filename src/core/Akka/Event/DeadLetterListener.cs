...
/// <summary>
///     This class represents an actor responsible for listening to <see cref="DeadLetter" /> messages and logging them
///     using the <see cref="EventStream" />.
/// </summary>
public class DeadLetterListener : ActorBase
{
    ...
    private void IncrementCount()
    {
        if (_count == int.MaxValue)
        {
            Logging.GetLogger(Context.System, this)
                .Info("Resetting DeadLetterListener counter after reaching Int.MaxValue.");
            _count = 1;
        }
        else
        {
    ...
    {
        if (_isAlwaysLoggingDeadLetters) return ReceiveWithAlwaysLogging(message);
    ...
    private void LogDeadLetter(AllDeadLetters d, string doneMsg)
    {
        ...
        if (!content.Equals(messageStr)) logMessage += $" Message content: {content}";
    ...
    {
        return !ReferenceEquals(snd, ActorRefs.NoSender) && !ReferenceEquals(snd, Context.System.DeadLetters) &&
               snd is not DeadLetterActorRef;
    }
    ...
    private readonly struct Deadline : IEquatable<Deadline>
    {
        ...
        private Deadline(DateTime when)
        {
            When = when;
        }
        ...
        {
            return obj is Deadline deadline && Equals(deadline);
        }
