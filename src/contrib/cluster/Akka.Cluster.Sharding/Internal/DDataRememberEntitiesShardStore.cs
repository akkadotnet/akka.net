...
internal sealed class DDataRememberEntitiesShardStore : ActorBase, IWithUnboundedStash
{
        ...
        int majorityMinCap)
    {
        ...
        _writeMajority = new WriteMajority(new TimeSpan(settings.TuningParameters.UpdatingStateTimeout.Ticks / 4),
            majorityMinCap);
        _keys = StateKeys(typeName, shardId);

        if (_log.IsDebugEnabled)
            _log.Debug(
                "Starting up DDataRememberEntitiesStore, read timeout: [{0}], write timeout: [{1}], majority min cap: [{2}]",
        ...
        int majorityMinCap)
    {
        return Actor.Props.Create(() =>
            new DDataRememberEntitiesShardStore(shardId, typeName, settings, replicator, majorityMinCap));
    }

    private static ImmutableArray<ORSetKey<EntityId>> StateKeys(string typeName, ShardId shardId)
    {
        return Enumerable.Range(0, numberOfKeys).Select(i => new ORSetKey<EntityId>($"shard-{typeName}-{shardId}-{i}"))
            .ToImmutableArray();
    }
    ...
    private Receive WaitingForAllEntityIds(IImmutableSet<int> gotKeys, IImmutableSet<EntityId> ids,
        IActorRef shardWaiting)
    {
        ...
        bool Receive(object message)
        {
            switch (message)
            {
                ...
                case DataDeleted m:
                    ...
                    return true;
                case RememberEntitiesShardStore.Update update:
                    _log.Warning("Got an update before load of initial entities completed, dropping update: [{0}]",
                        update);
                    return true;
                case RememberEntitiesShardStore.GetEntities _:
    ...
    {
        var allEvts = update.Started.Select(i => (IEvt)new Started(i)).Union(update.Stopped.Select(i => new Stopped(i)))
            .ToImmutableHashSet();
        ...
        var ddataUpdates = allEvts.GroupBy(evt => Key(evt.Id)).Select(i =>
        {
            ...
            return new KeyValuePair<IImmutableSet<IEvt>, (Update Update, int MaxUpdateAttempts)>(evts,
                (Update: Dsl.Update(i.Key, ORSet<EntityId>.Empty, _writeMajority, evts, existing =>
                {
        ...
        foreach (var u in ddataUpdates) _replicator.Tell(u.Value.Update);
            ...
            bool Receive(object message)
            {
                switch (message)
                {
                    ...
                    case UpdateTimeout m when m.Request is IImmutableSet<IEvt> evts:
                        ...
                        {
                            _log.Error(
                                "Unable to update state, within 'updating-state-timeout'= [{0}], gave up after [{1}] retries",
