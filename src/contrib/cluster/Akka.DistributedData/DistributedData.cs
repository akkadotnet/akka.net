...
public class DistributedData : IExtension
{
    ...
    /// </summary>
    public bool IsTerminated => Cluster.Cluster.Get(_system).IsTerminated || (!string.IsNullOrEmpty(_settings.Role) &&
                                                                              !Cluster.Cluster.Get(_system).SelfRoles
                                                                                  .Contains(_settings.Role));
    ...
    {
        return system.WithExtension<DistributedData, DistributedDataProvider>();
    }

    private static Props GetSupervisedReplicator(ReplicatorSettings settings, string name)
    {
    ...
    {
        return ConfigurationFactory.FromResource<DistributedData>("Akka.DistributedData.reference.conf");
    }
    ...
    /// </summary>
    /// <exception cref="DataDeletedException">
    ///     Thrown if value under provided <paramref name="key" /> was permamently deleted.
    ///     That key can't be used anymore.
    /// </exception>
    /// <exception cref="TimeoutException">
    ///     Thrown if get request consistency was not achieved within possible time limit
    ///     attached to a provided read <paramref name="consistency" />.
    /// </exception>
    ...
    public async Task<T> GetAsync<T>(IKey<T> key, IReadConsistency consistency = null,
        ...
        switch (response)
        {
            case GetSuccess success:
                ...
                throw new NotSupportedException(
                    $"Received response id [{success.Request}] and request correlation id [{id}] are different.");
            case NotFound notFound: return null;
            case DataDeleted deleted:
                throw new DataDeletedException(
                    $"Cannot retrieve data under key [{key}]. It has been permanently deleted and the key cannot be reused.");
            case GetFailure failure:
                throw new TimeoutException(
                    $"Couldn't retrieve the data under key [{key}] within consistency constraints {consistency} and under provided timeout.");
            case Status.Failure failure:
                ExceptionDispatchInfo.Capture(failure.Cause).Throw();
                return default;
            default: throw new NotSupportedException("Unknown response type: " + response);
    ...
    /// </exception>
    /// <exception cref="TimeoutException">
    ///     Thrown if update request consistency was not achieved within possible time limit
    ///     attached to a provided read <paramref name="consistency" />.
    /// </exception>
    ...
    /// <param name="cancellation">Cancellation token used to cancel request prematurelly if needed.</param>
    /// <returns>
    ///     A task which may complete successfully if update was confirmed within provided consistency or throw an
    ///     exception.
    /// </returns>
    public async Task UpdateAsync<T>(IKey<T> key, T replica, IWriteConsistency consistency = null,
        CancellationToken cancellation = default)
        where T : IReplicatedData<T>
    {
        var id = Guid.NewGuid();
        var response = await Replicator.Ask(Dsl.Update(key, replica, consistency, id, old => old.Merge(replica)),
            cancellation);
        switch (response)
        {
            ...
            case DataDeleted deleted:
                throw new DataDeletedException(
                    $"Cannot store data under key [{key}]. It has been permanently deleted and the key cannot be reused.");
            case ModifyFailure failure:
                ExceptionDispatchInfo.Capture(failure.Cause).Throw();
                return;
            case StoreFailure failure:
                ExceptionDispatchInfo.Capture(failure.Cause).Throw();
                return;
            case UpdateTimeout timeout:
                throw new TimeoutException(
                    $"Couldn't confirm update of the data under key [{key}] within consistency constraints {consistency} and under provided timeout.");
            case Status.Failure failure:
                ExceptionDispatchInfo.Capture(failure.Cause).Throw();
                return;
            default: throw new NotSupportedException("Unknown response type: " + response);
    ...
    /// <param name="cancellation">Cancellation token used to cancel request prematurelly if needed.</param>
    /// <returns></returns>
    public async Task DeleteAsync<T>(IKey<T> key, IWriteConsistency consistency = null,
        CancellationToken cancellation = default) where T : IReplicatedData<T>
    {
        var id = Guid.NewGuid();
        ...
        switch (response)
        {
            case DeleteSuccess success:
                ...
                throw new NotSupportedException(
                    $"Received response id [{success.Request}] and request correlation id [{id}] are different.");
            case ReplicationDeleteFailure failure:
                throw new TimeoutException(
                    $"Couldn't confirm deletion of the data under key [{key}] within consistency constraints {consistency} and under provided timeout.");
            case StoreFailure failure:
                ExceptionDispatchInfo.Capture(failure.Cause).Throw();
                return;
            case DataDeleted deleted:
