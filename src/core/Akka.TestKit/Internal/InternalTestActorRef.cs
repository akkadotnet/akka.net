...
public class InternalTestActorRef : LocalActorRef
{
    ...
    private InternalTestActorRef(ActorSystemImpl system, Props props, MessageDispatcher dispatcher,
    ...
    {
        return new TestActorCell(system, self, props, dispatcher, supervisor);
    }
    ...
    /// <returns>TBD</returns>
    public static InternalTestActorRef Create(ActorSystem system, Props props, IActorRef supervisor = null,
        string name = null)
    {
        if (name == null)
        ...
        else
        {
            if (supervisor is RepointableActorRef supervisorRep)
            {
                ...
                if (repUnderlying is ActorCell cellUnderlying)
                    ...
                    system.Log.Error(
                        "Trying to attach child {0} to unknown type of supervisor cell {1}, this is not going to end well",
                        name, repUnderlying.GetType());
            }
        }

        var mailbox = system.Mailboxes.GetMailboxType(props, dispatcher.Configurator.Config);
        var testActorRef = new InternalTestActorRef((ActorSystemImpl)system, props, dispatcher, mailbox,
            (IInternalActorRef)supervisor, supervisor.Path / name);
    ...
    protected class TestActorCell : ActorCell
    {
        ...
        public TestActorCell(ActorSystemImpl system, IInternalActorRef self, Props props, MessageDispatcher dispatcher,
    ...
    internal class TestActorTaskScheduler : ActorTaskScheduler, IAsyncResultInterceptor
    {
        ...
        /// <inheritdoc />
        internal TestActorTaskScheduler(ActorCell testActorCell, Action<object, Exception> taskCallback) : base(
            testActorCell)
        {
