...
{
    private readonly Member _a1 = TestMember.Create(new Address("akka", "sys", "a", 2552), MemberStatus.Joining,
        ImmutableHashSet<string>.Empty);

    private readonly Member _b1 = TestMember.Create(new Address("akka", "sys", "b", 2552), MemberStatus.Up,
        ImmutableHashSet<string>.Empty.Add("r1"));

    private readonly ClusterMetricsMessageSerializer _serializer;

    private Member _c1 = TestMember.Create(new Address("akka", "sys", "c", 2552), MemberStatus.Leaving,
        ImmutableHashSet<string>.Empty.Add("r2"));

    private Member _d1 = TestMember.Create(new Address("akka", "sys", "d", 2552), MemberStatus.Exiting,
        ImmutableHashSet<string>.Empty.Add("r1").Add("r2"));

    private Member _e1 = TestMember.Create(new Address("akka", "sys", "e", 2552), MemberStatus.Down,
        ImmutableHashSet<string>.Empty.Add("r3"));

    private Member _f1 = TestMember.Create(new Address("akka", "sys", "f", 2552), MemberStatus.Removed,
        ImmutableHashSet<string>.Empty.Add("r2").Add("r3"));
    ...
    public void Cluster_messages_should_be_serializable()
    {
        ...
        metrics = metrics.Add(new NodeMetrics(_a1.Address, 4711,
            new[] { new NodeMetrics.Types.Metric("foo", 1.2, Option<NodeMetrics.Types.EWMA>.None) }));
        metrics = metrics.Add(new NodeMetrics(_b1.Address, 4712,
            new[]
            {
                new NodeMetrics.Types.Metric("foo", 2.1, new NodeMetrics.Types.EWMA(100, 0.18)),
    ...
    public void AdaptiveLoadBalancingPool_Should_be_serializable()
    {
        ...
        var complicatedPool = new AdaptiveLoadBalancingPool(
            new MixMetricsSelector(new CapacityMetricsSelector[]
            {
                CpuMetricsSelector.Instance, MemoryMetricsSelector.Instance
            }.ToImmutableArray()),
            7,
