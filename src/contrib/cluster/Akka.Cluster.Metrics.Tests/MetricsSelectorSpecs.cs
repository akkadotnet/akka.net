...
public class MetricsSelectorSpecs
{
    ...
    private readonly Address _a1 = new("akka", "sys", "a1", 2551);

    private readonly CapacityMetricsSelector _abstractSelector = new AbstractSelector();
    private readonly Address _b1 = new("akka", "sys", "b1", 2551);
    private readonly Address _c1 = new("akka", "sys", "c1", 2551);
    private readonly Address _d1 = new("akka", "sys", "d1", 2551);
    private readonly IImmutableSet<NodeMetrics> _nodeMetrics;

    public MetricsSelectorSpecs()
    {
        var nodeMetricsA = new NodeMetrics(_a1, DateTime.UtcNow.ToTimestamp(),
            new[]
            {
        ...
        var nodeMetricsB = new NodeMetrics(_b1, DateTime.UtcNow.ToTimestamp(),
            new[]
            {
        ...
        var nodeMetricsC = new NodeMetrics(_c1, DateTime.UtcNow.ToTimestamp(),
            new[]
            {
        ...
        var nodeMetricsD = new NodeMetrics(_d1, DateTime.UtcNow.ToTimestamp(),
            new[]
            {
    ...
    {
        var capacity = new Dictionary<Address, double> { [_a1] = 0.6, [_b1] = 0.3, [_c1] = 0.1 }
            .ToImmutableDictionary();
        var weights = _abstractSelector.Weights(capacity);
        weights.Should().BeEquivalentTo(new Dictionary<Address, int> { [_c1] = 1, [_b1] = 3, [_a1] = 6 });
    }

    [Fact]
    public void CapacityMetricsSelector_Should_handle_low_and_zero_capacity()
    {
        var capacity = new Dictionary<Address, double> { [_a1] = 0.0, [_b1] = 1.0, [_c1] = 0.005, [_d1] = 0.004 }
            .ToImmutableDictionary();
        var weights = _abstractSelector.Weights(capacity);
        weights.Should().BeEquivalentTo(new Dictionary<Address, int> { [_a1] = 0, [_b1] = 100, [_c1] = 1, [_d1] = 0 });
    }
