...
public class PreRestartException : ActorInitializationException
{
    ...
    /// <param name="restartException">
    ///     The exception thrown by the <paramref name="actor" /> within
    ///     <see cref="ActorBase.PreRestart" />.
    /// </param>
    ...
    public PreRestartException(IActorRef actor, Exception restartException, Exception cause, object optionalMessage)
        : base(actor,
            "Exception pre restart (" + (restartException == null ? "null" : restartException.GetType().ToString()) +
            ")", cause)
    {
        Actor = actor;
...
public class PostRestartException : ActorInitializationException
{
    ...
    public PostRestartException(IActorRef actor, Exception cause, Exception originalCause)
        : base(actor,
            "Exception post restart (" + (originalCause == null ? "null" : originalCause.GetType().ToString()) + ")",
            cause)
    {
...
public class ActorNotFoundException : AkkaException
{
    ...
    /// <summary>
    ///     <see cref="ActorNotFoundException" /> that takes a descriptive <paramref name="message" /> and optional
