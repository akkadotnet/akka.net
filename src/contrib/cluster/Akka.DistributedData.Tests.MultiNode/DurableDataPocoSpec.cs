...
public class DurableDataPocoSpecConfig : MultiNodeConfig
{
    public DurableDataPocoSpecConfig(bool writeBehind)
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), "target", "DurableDataPocoSpec",
            $"Spec-{DateTime.UtcNow.Ticks}");
...
internal sealed class PocoObject : IReplicatedData<PocoObject>
{
    ...
    {
        return other != null && other.Created > Created ? other : this;
    }

    public IReplicatedData Merge(IReplicatedData other)
    {
        return other is PocoObject state ? Merge(state) : this;
    }
}
...
public abstract class DurableDataPocoSpecBase : MultiNodeClusterSpec
{
    ...
    private void TellUpdate(IActorRef replicator, ORDictionaryKey<string, PocoObject> key,
        IWriteConsistency consistency, PocoObject message)
    {
    ...
    public void Durable_CRDT_should_work_in_a_multi_node_cluster()
    {
        ...
        ExpectMsg<GetSuccess>().Get(_keyC).Elements
            .ShouldBe(ImmutableHashSet.CreateRange(new[] { _first.Name, _second.Name, _third.Name }));
        ...
        RunOn(() =>
        {
            ...
            try
            {
                ...
                /* new TestKit(sys1) with ImplicitSender */
                {
                    ...
                    {
                        r2.Tell(Dsl.GetKeyIds);
                        ExpectMsg<GetKeysIdsResult>().Keys
                            .ShouldBe(ImmutableHashSet.CreateRange(new[] { _keyA.Id, _keyB.Id }));
                    });
...
{
    public DurableDataPocoSpec() : base(new DurableDataPocoSpecConfig(false), typeof(DurableDataPocoSpec))
    {
    }
}
...
{
    public DurableDataWriteBehindPocoSpec() : base(new DurableDataPocoSpecConfig(true),
        typeof(DurableDataWriteBehindPocoSpec))
    {
    }
}