...
///     using tail-chopping.
///     This process has the router select a random routee, then waits an interval before sending to a different randomly
///     chosen routee.
///     The first response is used and the remaining are discarded. If the none of the routees respond within a specified
///     time limit,
...
public sealed class TailChopping : RoutingLogic
{
    ...
    {
        if (routees.IsNullOrEmpty()) return Routee.NoRoutee;
...
///     determined using tail-chopping.
///     This process has the routee select a random routee, then waits an interval before sending to a different randomly
///     chosen routee.
///     The first response is used and the remaining are discarded. If the none of the routees respond within a specified
///     time limit,
///     a timeout failure occurs.
...
public sealed class TailChoppingPool : Pool
{
    ...
    public TailChoppingPool(Config config)
        : this(
            ...
            config.GetTimeSpan("within"), config.GetTimeSpan("tail-chopping-router.interval"),
    ...
    public TailChoppingPool(int nrOfInstances, TimeSpan within, TimeSpan interval)
        : this(nrOfInstances, null, DefaultSupervisorStrategy, Dispatchers.DefaultDispatcherId, within, interval)
    {
    }
    ...
    {
        return new TailChoppingPool(NrOfInstances, Resizer, strategy, RouterDispatcher, Within, Interval,
            UsePoolDispatcher);
    }
    ...
    {
        return new TailChoppingPool(NrOfInstances, resizer, SupervisorStrategy, RouterDispatcher, Within, Interval,
            UsePoolDispatcher);
    }
    ...
    {
        return new TailChoppingPool(NrOfInstances, Resizer, SupervisorStrategy, dispatcher, Within, Interval,
            UsePoolDispatcher);
    }
...
public sealed class TailChoppingGroup : Group
{
    ...
    ///         If 'tail-chopping-router.interval' is defined in the provided configuration then that will be used as the
    ///         interval.
    ///     </note>
    /// </param>
    ...
    public override ISurrogate ToSurrogate(ActorSystem system)
    {
        ...
        {
            Paths = InternalPaths, Within = Within, Interval = Interval, RouterDispatcher = RouterDispatcher
        };
    }
