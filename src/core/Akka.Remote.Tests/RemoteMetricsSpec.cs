...
public class RemoteMetricsSpec : AkkaSpec
{
    ...
    private class InfoEventListener : ActorBase
    {
        ...
        protected override bool Receive(object message)
        {
            ...
            {
                if (info.Message.ToString().Contains("New maximum payload size for")) _testActor.Tell(new NewMaximum());
                if (info.Message.ToString().Contains("Payload size for")) _testActor.Tell(new PayloadSize());
            }
