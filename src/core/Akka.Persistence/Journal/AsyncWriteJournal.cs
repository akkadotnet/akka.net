...
public abstract class AsyncWriteJournal : WriteJournalBase, IAsyncRecovery
{
    ...
    ///     This exception is thrown when the Persistence extension related to this journal has not been used in the current
    ///     <see cref="ActorSystem" /> context.
    /// </exception>
    ...
    private void HandleDeleteMessagesTo(DeleteMessagesTo message)
    {
        ...
        async Task ProcessDelete()
        {
            try
            {
                ...
                if (CanPublish) eventStream.Publish(message);
            }
            catch (Exception ex)
            {
...
#pragma warning restore CS4014
    ...
    private void HandleReplayMessages(ReplayMessages message)
    {
        ...
        async Task ExecuteHighestSequenceNr()
        {
            ...
            try
            {
                var highSequenceNr = await _breaker.WithCircuitBreaker((message, readHighestSequenceNrFrom, awj: this),
                    state =>
                        state.awj.ReadHighestSequenceNrAsync(state.message.PersistenceId,
                            state.readHighestSequenceNrFrom));
            ...
            catch (OperationCanceledException cx)
            {
                ...
                var newEx = new OperationCanceledException("ReplayMessagesAsync canceled, possibly due to timing out.",
                    cx);
...
#pragma warning restore CS4014
    ...
    private async Task ExecuteBatch(WriteMessages message, int atomicWriteCount, IActorRef self,
        long resequencerCounter)
    {
        try
        {
            ...
            try
            {
                var writeResult =
                    await _breaker
                        .WithCircuitBreaker((prepared, awj: this),
                            state => state.awj.WriteMessagesAsync(state.prepared)).ConfigureAwait(false);
            ...
            {
                _resequencer.Tell(
                    new Desequenced(new WriteMessagesFailed(e, atomicWriteCount), resequencerCounter,
                        message.PersistentActor, self), self);
                Resequence((x, _) => new WriteMessageFailure(x, e, message.ActorInstanceId), null, resequencerCounter,
                    message, _resequencer, self);
            }
        }
        catch (Exception ex)
        {
            // exception from PreparePersistentBatch => rejected
            ProcessResults(Enumerable.Repeat(ex, atomicWriteCount).ToImmutableList(), atomicWriteCount, message,
                _resequencer, resequencerCounter, self);
        }
    }

    private void ProcessResults(IImmutableList<Exception> results, int atomicWriteCount, WriteMessages writeMessage,
        ...
        long resequencerCounter, IActorRef writeJournal)
    {
        ...
        resequencer.Tell(
            new Desequenced(WriteMessagesSuccessful.Instance, resequencerCounter, writeMessage.PersistentActor,
                writeJournal), writeJournal);
        Resequence((x, exception) => exception == null
                ? new WriteMessageSuccess(x, writeMessage.ActorInstanceId)
                : new WriteMessageRejected(x, exception, writeMessage.ActorInstanceId), results, resequencerCounter,
            writeMessage, resequencer, writeJournal);
    }

    private void Resequence(Func<IPersistentRepresentation, Exception, object> mapper,
        IImmutableList<Exception> results, long resequencerCounter, WriteMessages msg, IActorRef resequencer,
        IActorRef writeJournal)
    {
        ...
        foreach (var resequencable in msg.Messages)
            ...
            else
            {
                ...
                resequencer.Tell(
                    new Desequenced(loopMsg, resequencerCounter + i + 1, msg.PersistentActor, resequencable.Sender),
                    writeJournal);
                i++;
            }
    }
