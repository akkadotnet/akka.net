...
public class RemoteConfigSpec : AkkaSpec
{
    ...
    public void Remoting_should_contain_correct_configuration_values_in_ReferenceConf()
    {
        ...
        Assert.Equal(typeof(PhiAccrualFailureDetector),
            Type.GetType(remoteSettings.WatchFailureDetectorImplementationClass));
        ...
        Assert.Equal(TimeSpan.FromSeconds(10),
            remoteSettings.WatchFailureDetectorConfig.GetTimeSpan("acceptable-heartbeat-pause"));
        Assert.Equal(TimeSpan.FromMilliseconds(100),
            remoteSettings.WatchFailureDetectorConfig.GetTimeSpan("min-std-deviation"));

        var remoteSettingsAdaptersStandart = new List<KeyValuePair<string, Type>>
        {
            new("gremlin", typeof(FailureInjectorProvider)), new("trttl", typeof(ThrottlerProvider))
        };
    ...
    public void Remoting_should_be_able_to_parse_AkkaProtocol_related_config_elements()
    {
        ...
        Assert.Equal(typeof(DeadlineFailureDetector),
            Type.GetType(settings.TransportFailureDetectorImplementationClass));
        Assert.Equal(TimeSpan.FromSeconds(4), settings.TransportHeartBeatInterval);
        Assert.Equal(TimeSpan.FromSeconds(120),
            settings.TransportFailureDetectorConfig.GetTimeSpan("acceptable-heartbeat-pause"));
    }
    ...
    public void Remoting_should_contain_correct_heliosTCP_values_in_ReferenceConf()
    {
        ...
        s.ConnectTimeout.Should()
            .Be(new AkkaProtocolSettings(RARP.For(Sys).Provider.RemoteSettings.Config).HandshakeTimeout);
