...
public class InputStreamSinkSpec : AkkaSpec
{
    ...
    [Fact]
    public async Task
        InputStreamSink_should_return_less_than_was_expected_when_data_source_has_provided_some_but_not_enough_data()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
            var inputStream = Source.Single(_byteString).RunWith(StreamConverters.AsInputStream(), _materializer);
            ...
            inputStream.Dispose();
            ByteString.FromBytes(arr).Should()
                .BeEquivalentTo(Enumerable.Concat(_byteString, ByteString.FromBytes(new byte[] { 0 })));
            return Task.CompletedTask;
        }, _materializer);
    }
    ...
    private Action Action(Action a)
    {
        return a;
    }
    ...
    public async Task InputStreamSink_should_use_dedicated_default_blocking_io_dispatcher_by_default()
    {
        await this.AssertAllStagesStoppedAsync(() =>
        {
            ...
            try
            {
                ...
                (materializer as ActorMaterializerImpl).Supervisor.Tell(StreamSupervisor.GetChildren.Instance,
                    TestActor);
    ...
    {
        Action a = () =>
            Source.Single(_byteString)
                .RunWith(StreamConverters.AsInputStream(Timeout).WithAttributes(Attributes.CreateInputBuffer(0, 0)),
                    _materializer);
    ...
    {
        return TestSinkStage<ByteString, Stream>.Create(new InputStreamSinkStage(Timeout), probe);
    }
}