...
public class DeadLetterSuspensionSpec : AkkaSpec
{
    ...
    public DeadLetterSuspensionSpec()
        : base(Config)
    {
        _deadActor = Sys.ActorOf(Props.Create<EchoActor>());
        Watch(_deadActor);
    ...
    private class Dropping : ActorBase
    {
        ...
        {
            return Akka.Actor.Props.Create(() => new Dropping());
        }
    ...
    private class Unandled : ActorBase
    {
        public static Props Props()
        {
            return Akka.Actor.Props.Create(() => new Unandled());
        }
