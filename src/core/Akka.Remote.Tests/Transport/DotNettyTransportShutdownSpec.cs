...
public class DotNettyTransportShutdownSpec : AkkaSpec
{
    ...
    public async Task DotNettyTcpTransport_should_cleanly_terminate_active_endpoints_upon_outbound_disassociate()
    {
        ...
        try
        {
            ...
            var inboundHandle =
                (await p2.ExpectMsgAsync<InboundAssociation>())
                .Association; // wait for the inbound association handle to show up
    ...
    public async Task DotNettyTcpTransport_should_cleanly_terminate_active_endpoints_upon_outbound_shutdown()
    {
        ...
        try
        {
            ...
            // verify that the connections are terminated
            await AwaitConditionAsync(() => Task.FromResult(t1.ConnectionGroup.Count == 0), null,
                $"Expected 0 open connection but found {t1.ConnectionGroup.Count}");
            await AwaitConditionAsync(() => Task.FromResult(t2.ConnectionGroup.Count == 1), null,
                $"Expected 1 open connection but found {t2.ConnectionGroup.Count}");
    ...
    public async Task DotNettyTcpTransport_should_cleanly_terminate_active_endpoints_upon_inbound_disassociate()
    {
        ...
        try
        {
            ...
            // verify that the connections are terminated
            await AwaitConditionAsync(() => Task.FromResult(t1.ConnectionGroup.Count == 1), null,
                $"Expected 1 open connection but found {t1.ConnectionGroup.Count}");
            await AwaitConditionAsync(() => Task.FromResult(t2.ConnectionGroup.Count == 1), null,
                $"Expected 1 open connection but found {t2.ConnectionGroup.Count}");
    ...
    public async Task DotNettyTcpTransport_should_cleanly_terminate_active_endpoints_upon_inbound_shutdown()
    {
        ...
        try
        {
            ...
            await AwaitConditionAsync(() => Task.FromResult(t1.ConnectionGroup.Count == 1), null,
                $"Expected 1 open connection but found {t1.ConnectionGroup.Count}");
            await AwaitConditionAsync(() => Task.FromResult(t2.ConnectionGroup.Count == 0), null,
                $"Expected 0 open connection but found {t2.ConnectionGroup.Count}");
