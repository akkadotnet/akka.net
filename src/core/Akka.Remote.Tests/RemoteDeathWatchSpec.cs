...
public class RemoteDeathWatchSpec : AkkaSpec
{
    ...
    {
        var mute = EventFilter.Warning(new Regex("received dead letter.*Disassociate")).Mute();
    ...
    public async Task Must_receive_Terminated_when_system_of_deserialized_ActorRef_is_not_running()
    {
        ...
        {
            dsl.OnPreStart = context => { context.Watch(@ref); };
            dsl.Receive<Terminated>((t, _) => { TestActor.Tell(t.ActorRef); });
        };
    ...
    public async Task Must_receive_terminated_when_watched_node_is_unknown_host()
    {
        ...
        Action<IActorDsl> act = dsl =>
        {
            dsl.OnPreStart = context => { context.Watch(rarp.ResolveActorRef(path)); };
            dsl.Receive<Terminated>((t, _) => { TestActor.Tell(t.ActorRef.Path); });
        };
    ...
    [Fact]
    public async Task
        Must_quarantine_systems_after_unsuccessful_system_message_delivery_if_have_not_communicated_before()
    {
        ...
        var extinctPath = new RootActorPath(new Address("akka.tcp", "extinct-system", "localhost", port)) / "user" /
                          "noone";
