...
public abstract class ClusterHeartBeatSenderStateBase : ClusterSpecBase
{
    ...
    private readonly UniqueAddress aa = new(new Address("akka.tcp", "sys", "aa", 2552), 1);
    private readonly UniqueAddress bb = new(new Address("akka.tcp", "sys", "bb", 2552), 2);
    private readonly UniqueAddress cc = new(new Address("akka.tcp", "sys", "cc", 2552), 3);
    private readonly UniqueAddress dd = new(new Address("akka.tcp", "sys", "dd", 2552), 4);
    private readonly UniqueAddress ee = new(new Address("akka.tcp", "sys", "ee", 2552), 5);
    ...
    {
        return new ClusterHeartbeatSenderState(
            new HeartbeatNodeRing(selfUniqueAddress, ImmutableHashSet.Create(selfUniqueAddress),
                ImmutableHashSet<UniqueAddress>.Empty, 3),
    ...
    public void ClusterHeartbeatSenderState_must_update_FailureDetector_in_active_set()
    {
        ...
        s2.FailureDetector.IsMonitoring(ee.Address).Should().BeFalse(
            "Never added (ee) to active set, so we should not be monitoring it even if we did receive HeartbeatRsp from it");
    }
    ...
    public void ClusterHeartbeatSenderState_must_behave_correctly_for_random_operations()
    {
        ...
        var nodes = Enumerable.Range(1, rnd.Next(10, 200))
            ...
            .ToList();
        var rndNode = () => nodes[rnd.Next(0, nodes.Count)];
        ...
        for (var i = 1; i <= 100000; i++)
        {
                ...
                switch (operation)
                {
                    case Add:
                        if (node != selfUniqueAddress && !state.Ring.Nodes.Contains(node))
                        {
                            ...
                            //keep unreachable
                            oldUnreachable.Except(state.ActiveReceivers).Should()
                                .BeEquivalentTo(ImmutableHashSet<UniqueAddress>.Empty);
                    ...
                    case Remove:
                        if (node != selfUniqueAddress && state.Ring.Nodes.Contains(node))
                        {
                            ...
                            if (oldUnreachable.Contains(node))
                                oldUnreachable.Except(state.ActiveReceivers).Should()
                                    .BeEquivalentTo(ImmutableHashSet.Create(node));
                            else
                                oldUnreachable.Except(state.ActiveReceivers).Should()
                                    .BeEquivalentTo(ImmutableHashSet<UniqueAddress>.Empty);
            ...
            {
                Debug.WriteLine(
                    "Failure context: i = {0}, node = {1}, op={2}, unreachable={3}, ringReceivers={4}, ringNodes={5}",
                    i, node, operation,
    ...
    /// </summary>
    private class FailureDetectorStub : FailureDetector
    {
        private Status _status = Status.Unknown;

        public override bool IsAvailable => _status is Status.Up or Status.Unknown;

        public override bool IsMonitoring => _status != Status.Unknown;
