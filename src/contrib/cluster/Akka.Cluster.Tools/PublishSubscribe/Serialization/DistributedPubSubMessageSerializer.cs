...
public class DistributedPubSubMessageSerializer : SerializerWithStringManifest
{
    ...
    ///     <see cref="Akka.Cluster.Tools.PublishSubscribe.Internal.Status" /> |
    ///     <see cref="Akka.Cluster.Tools.PublishSubscribe.Internal.Delta" /> | <see cref="Send" /> | <see cref="SendToAll" />
    ///     | <see cref="Publish" />
    /// </exception>
        ...
        switch (obj)
        {
            ...
            default:
                throw new ArgumentException(
                    $"Can't serialize object of type {obj.GetType()} with {nameof(DistributedPubSubMessageSerializer)}");
        }
    }
    ...
    public override object FromBinary(byte[] bytes, string manifest)
    {
        ...
        throw new SerializationException(
            $"Unimplemented deserialization of message with manifest [{manifest}] in serializer {nameof(DistributedPubSubMessageSerializer)}");
    }

    /// <summary>
    ///     Returns the manifest (type hint) that will be provided in the
    ///     <see cref="FromBinary(System.Byte[],System.String)" /> method.
    ///     <note>
    ...
    public override string Manifest(object o)
    {
        switch (o)
        {
            ...
            default:
                throw new ArgumentException(
                    $"Serializer {nameof(DistributedPubSubMessageSerializer)} cannot serialize message of type {o.GetType()}");
        }
    }

    private byte[] StatusToProto(Status status)
    {
    ...
    private Status StatusFrom(byte[] bytes)
    {
        ...
        return new Status(versions.ToImmutable(), statusProto.ReplyToStatus);
    }
            ...
            foreach (var bucketContent in bucket.Content)
            {
                ...
                valueHolder.Ref =
                    Akka.Serialization.Serialization
                        .SerializedActorPath(bucketContent.Value
                            .Ref); // TODO: reuse the method from the core serializer
    ...
    private Delta DeltaFrom(byte[] bytes)
    {
        ...
        foreach (var protoBuckets in deltaProto.Buckets)
        {
            ...
            {
                var valueHolder = new ValueHolder(protoBucketContent.Value.Version,
                    ResolveActorRef(protoBucketContent.Value.Ref));
                content.Add(protoBucketContent.Key, valueHolder);
            }

            var bucket = new Bucket(AddressFrom(protoBuckets.Owner), protoBuckets.Version,
    ...
    private SendToAll SendToAllFrom(byte[] bytes)
    {
        ...
        return new SendToAll(sendToAllProto.Path, _payloadSupport.PayloadFrom(sendToAllProto.Payload),
            sendToAllProto.AllButSelf);
    }
    ...
    private Publish PublishFrom(byte[] bytes)
    {
        ...
        return new Publish(publishProto.Topic, _payloadSupport.PayloadFrom(publishProto.Payload),
            publishProto.SendOneMessageToEachGroup);
    }
