...
public class FlowGroupBySpec : AkkaSpec
{
    ...
    public async Task GroupBy_must_work_in_the_happy_case()
    {
        ...
        {
            await WithSubStreamsSupport(run: async (masterSubscriber, masterSubscription, getSubFlow) =>
            {
    ...
    public async Task GroupBy_must_accept_cancelling_subStreams()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            await WithSubStreamsSupport(maxSubStream: 3,
                run: async (masterSubscriber, masterSubscription, getSubFlow) =>
                {
                    var (s1, _) =
                        await StreamPuppet((await getSubFlow(1)).RunWith(Sink.AsPublisher<int>(false), Materializer),
                            this);
                    s1.Cancel();

                    var (subscription, probe) =
                        await StreamPuppet((await getSubFlow(0)).RunWith(Sink.AsPublisher<int>(false), Materializer),
                            this);
    ...
    public async Task GroupBy_must_resume_stream_when_GroupBy_function_throws()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            var (_, subStream) = await subscriber.ExpectNextAsync();
            var (sub1, probe1) =
                await StreamPuppet(subStream.RunWith(Sink.AsPublisher<int>(false), Materializer), this);
            ...
            var (sub2, probe2) =
                await StreamPuppet(subStream2.RunWith(Sink.AsPublisher<int>(false), Materializer), this);
    ...
    public async Task GroupBy_must_fail_when_exceeding_maxSubStreams()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            var (up, down) = ((Flow<int, (IImmutableList<int>, Source<int, NotUsed>), NotUsed>)f)
                .RunWith(this.SourceProbe<int>(), this.SinkProbe<(IImmutableList<int>, Source<int, NotUsed>)>(),
                    Materializer);
            ...
            var (sub, probe) =
                await StreamPuppet(first.Item2.RunWith(Sink.AsPublisher<int>(false), Materializer), this);
    ...
    [Fact]
    public async Task
        GroupBy_must_work_if_pull_is_exercised_from_multiple_subStreams_while_downstream_is_back_pressuring()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var upstream = this.CreatePublisherProbe<int>();
    ...
    public async Task GroupBy_must_work_with_random_demand()
    {
        ...
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var props = new RandomDemandProperties { Kit = this };
    ...
    private static async Task<(ISubscription subscription, TestSubscriber.ManualProbe<int> probe)> StreamPuppet(
        IPublisher<int> p, TestKitBase kit)
    {
    ...
    private async Task WithSubStreamsSupport(
        ...
        int elementCount = 6,
        int maxSubStream = -1,
        Func<TestSubscriber.ManualProbe<(int, Source<int, NotUsed>)>, ISubscription,
            Func<int, Task<Source<int, NotUsed>>>, Task> run = null)
    {
    ...
    private sealed class ProbeSink : SinkModule<ByteString, TestSubscriber.Probe<ByteString>>
    {
        ...
        public ProbeSink(SinkShape<ByteString> shape, RandomDemandProperties properties, Attributes attributes) :
            base(shape)
        {
        ...
        protected override SinkModule<ByteString, TestSubscriber.Probe<ByteString>> NewInstance(
