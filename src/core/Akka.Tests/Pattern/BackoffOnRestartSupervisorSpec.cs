...
public class BackoffOnRestartSupervisorSpec : AkkaSpec
{
    ...
    public async Task BackoffOnRestartSupervisor_must_accept_commands_while_child_is_terminating()
    {
        ...
        var options = Backoff.OnFailure(SlowlyFailingActor.Props(postStopLatch), "someChildName", 1.Ticks(), 1.Ticks(),
                0.0, -1)
        ...
        await ExpectNoMsgAsync(100
            .Milliseconds()); // Child is in limbo due to latch in postStop. There is no Terminated message yet
    ...
    public async Task BackoffOnRestartSupervisor_must_respect_withinTimeRange_property_of_OneForOneStrategy()
    {
        ...
        // that is acceptable.
        var options = Backoff.OnFailure(TestActor.Props(probe.Ref), "someChildName", 100.Milliseconds(), 10.Seconds(),
                0.0, -1)
        ...
        stopwatch.Stop();
        Skip.If(stopwatch.ElapsedMilliseconds > 1500,
            "Could not satisfy test condition. Execution time exceeds the prescribed 2 seconds limit.");
...
#pragma warning disable CS0162 // Disabled because without the return, the compiler complains about ambigious reference between Receive<T>(Action<T>,Predicate<T>) and Receive<T>(Predicate<T>,Action<T>)
        public TestActor(IActorRef probe)
        {
            ...
            Receive<(string, string)>(str => str.Item1.Equals("TO_PARENT"), msg => { Context.Parent.Tell(msg.Item2); });
...
#pragma warning disable CS0162 // Disabled because without the return, the compiler complains about ambigious reference between Receive<T>(Action<T>,Predicate<T>) and Receive<T>(Predicate<T>,Action<T>)
        public SlowlyFailingActor(TestLatch latch)
        {
            ...
            Receive<string>(str => str.Equals("PING"), _ => { Sender.Tell("PONG"); });
        }
#pragma warning restore CS0162
    ...
    {
        var options = Backoff.OnFailure(TestActor.Props(probeRef), "someChildName", 200.Milliseconds(), 10.Seconds(),
                0.0, -1)
            .WithManualReset()
