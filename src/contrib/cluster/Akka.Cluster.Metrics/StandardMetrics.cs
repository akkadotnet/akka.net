...
public static class StandardMetrics
{
    ...
    public static Option<Memory> ExtractMemory(NodeMetrics nodeMetrics)
    {
        ...
        {
            return new Memory(data.Address, data.Timestamp, data.UsedSmoothValue, data.AvailableSmoothValue,
                data.MaxRecommendedSmoothValue);
        });
    }
    ...
    public sealed class Memory
    {
        ...
        public static Option<(Address Address, long Timestamp, double UsedSmoothValue, double AvailableSmoothValue,
                Option<double> MaxRecommendedSmoothValue)>
            Decompose(NodeMetrics nodeMetrics)
        {
            ...
            if (!used.HasValue || !available.HasValue)
                return Option<(Address, long, double, double, Option<double>)>.None;
    ...
    public sealed class Cpu
    {
        ...
        public Cpu(Address address, long timestamp, double cpuProcessUsage, double cpuTotalUsage, int processorsNumber)
        {
            if (cpuProcessUsage is < 0 or > 1)
                throw new ArgumentException(nameof(cpuProcessUsage),
                    $"{nameof(cpuProcessUsage)} must be between [0.0 - 1.0], was {cpuProcessUsage}");
            if (cpuTotalUsage is < 0 or > 1)
                throw new ArgumentException(nameof(cpuTotalUsage),
                    $"{nameof(cpuTotalUsage)} must be between [0.0 - 1.0], was {cpuTotalUsage}");
        ...
        public static
            Option<(Address Address, long Timestamp, double CpuProcessUsage, double CpuTotalUsage, int Processors)>
            Decompose(NodeMetrics nodeMetrics)
        {
            ...
            if (!processors.HasValue || !cpuProcessUsage.HasValue || !cpuTotalUsage.HasValue)
                return Option<(Address, long, double, double, int)>.None;
