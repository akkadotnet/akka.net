...
public class FailureInjectorTransportAdapter : AbstractTransportAdapter, IAssociationEventListener
{
    ...
    private volatile IAssociationEventListener _upstreamListener;
    ...
    public FailureInjectorTransportAdapter(Transport wrappedTransport, ExtendedActorSystem extendedActorSystem) : base(
        wrappedTransport)
    {
    ...
    public void Notify(IAssociationEvent ev)
    {
        if (ev is InboundAssociation inboundAssociation &&
            ShouldDropInbound(inboundAssociation.Association.RemoteAddress, ev, "notify"))
        {
            //ignore
        }
        else
    ...
    public bool ShouldDropInbound(Address remoteAddress, object instance, string debugMessage)
    {
        ...
        if (mode is Drop drop)
            ...
            {
                if (_shouldDebugLog)
                    _log.Debug("Dropping inbound [{0}] for [{1}] {2}", instance.GetType(),
    ...
    public bool ShouldDropOutbound(Address remoteAddress, object instance, string debugMessage)
    {
        ...
        if (mode is Drop drop)
            ...
            {
                if (_shouldDebugLog)
                    _log.Debug("Dropping outbound [{0}] for [{1}] {2}", instance.GetType(), remoteAddress,
                        debugMessage);
                return true;
            }
...
internal sealed class FailureInjectorHandle : AbstractTransportAdapterHandle, IHandleEventListener
{
    ...
    private volatile IHandleEventListener _upstreamListener;
