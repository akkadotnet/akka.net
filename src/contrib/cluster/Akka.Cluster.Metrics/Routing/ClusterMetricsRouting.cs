...
///     It uses random selection of routees based on probabilities derived from the remaining capacity of corresponding
///     node.
/// </summary>
public sealed class AdaptiveLoadBalancingRoutingLogic : RoutingLogic, IClusterMetricsRoutingLogic
{
    ...
    private readonly AtomicReference<Tuple<ImmutableArray<Routee>, IImmutableSet<NodeMetrics>, Option<WeightedRoutees>>>
        _weightedRouteesRef;
    ...
    ///     Decides what probability to use for selecting a routee, based on remaining capacity as indicated by the node
    ///     metrics
    /// </param>
    public AdaptiveLoadBalancingRoutingLogic(ActorSystem system, IMetricsSelector metricsSelector = null)
    {
        ...
        _weightedRouteesRef =
            new AtomicReference<Tuple<ImmutableArray<Routee>, IImmutableSet<NodeMetrics>, Option<WeightedRoutees>>>(
    ...
    public void MetricsChanged(ClusterMetricsChanged @event)
    {
        ...
        if (!_weightedRouteesRef.CompareAndSet(oldValue,
                Tuple.Create(routees, @event.NodeMetrics, weightedRoutees.AsOption())))
    ...
    public override Routee Select(object message, Routee[] routees)
    {
        ...
        Option<WeightedRoutees> UpdateWeightedRoutees()
        {
            ...
            _weightedRouteesRef.CompareAndSet(oldValue,
                Tuple.Create(routees.ToImmutableArray(), oldMetrics, weightedRoutees.AsOption()));
...
public sealed class AdaptiveLoadBalancingPool : Pool
{
    ...
    ///     Dispatcher to use for the router head actor, which handles supervision, death watch and
    ///     router management messages
    /// </param>
    /// <param name="usePoolDispatcher"></param>
    public AdaptiveLoadBalancingPool(IMetricsSelector metricsSelector = null, int nrOfInstances = 0,
        SupervisorStrategy supervisorStrategy = null,
        string routerDispatcher = null, bool usePoolDispatcher = false)
        : base(nrOfInstances, null, supervisorStrategy ?? DefaultSupervisorStrategy,
            routerDispatcher ?? Dispatchers.DefaultDispatcherId, usePoolDispatcher)
    {
    ...
    public override Props RoutingLogicController(RoutingLogic routingLogic)
    {
        return Actor.Props.Create(() =>
            new AdaptiveLoadBalancingMetricsListener((IClusterMetricsRoutingLogic)routingLogic));
    }
    ...
    {
        return new AdaptiveLoadBalancingPool(MetricsSelector, NrOfInstances, strategy, RouterDispatcher,
            UsePoolDispatcher);
    }
    ...
    {
        return new AdaptiveLoadBalancingPool(MetricsSelector, NrOfInstances, SupervisorStrategy, dispatcherId,
            UsePoolDispatcher);
    }
    ...
    public class AdaptiveLoadBalancingPoolSurrogate : ISurrogate
    {
        ...
        {
            return new AdaptiveLoadBalancingPool(MetricsSelector, NrOfInstances, SupervisorStrategy, RouterDispatcher,
                UsePoolDispatcher);
        }
    }
}
...
public sealed class AdaptiveLoadBalancingGroup : Group
{
    ...
    /// </param>
    public AdaptiveLoadBalancingGroup(IMetricsSelector metricsSelector = null, IEnumerable<string> paths = null,
        string routerDispatcher = null)
    ...
    public AdaptiveLoadBalancingGroup(Config config)
        : this(MetricsSelectorBuilder.BuildFromConfig(config), config.GetStringList("routees.paths", new string[] { }))
    {
    }
    ...
    public override ISurrogate ToSurrogate(ActorSystem system)
    {
        ...
        {
            Paths = InternalPaths, MetricsSelector = _metricsSelector, RouterDispatcher = RouterDispatcher
        };
    }
