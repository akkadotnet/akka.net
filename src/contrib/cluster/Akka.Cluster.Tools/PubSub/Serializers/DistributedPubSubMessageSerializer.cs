...
using Delta = Akka.Cluster.Tools.PubSub.Internal.Delta;
...
namespace Akka.Cluster.Tools.PubSub.Serializers
{
    ...
    public class DistributedPubSubMessageSerializer : SerializerWithStringManifest
    {
        ...
        public override byte[] ToBinary(object obj)
        {
            ...
            else if (obj is DistributedPubSubMediator.Send) return SendToProto(obj as DistributedPubSubMediator.Send).ToByteArray();
            else if (obj is DistributedPubSubMediator.SendToAll) return SendToAllToProto(obj as DistributedPubSubMediator.SendToAll).ToByteArray();
            else if (obj is DistributedPubSubMediator.Publish) return PublishToProto(obj as DistributedPubSubMediator.Publish).ToByteArray();
        ...
        private Akka.Cluster.PubSub.Serializers.Proto.Delta DeltaToProto(Delta delta)
        {
            var buckets = delta.Buckets.Select(b =>
            {
                var entries = b.Content.Select(c =>
               {
                   ...
                   if (c.Value.Ref != null)
                   {
                       bb.SetRef(Serialization.Serialization.SerializedActorPath(c.Value.Ref));
                   }
                   return bb.Build();
               });
        ...
        private Delta DeltaFromProto(Akka.Cluster.PubSub.Serializers.Proto.Delta delta)
        {
            ...
            {
                var content = b.ContentList.Aggregate(ImmutableTreeMap<string, ValueHolder>.Empty, (map, entry) =>
        ...
        private Send SendToProto(DistributedPubSubMediator.Send send)
        {
        ...
        private DistributedPubSubMediator.Send SendFromBinary(byte[] binary)
        {
            return SendFromProto(Send.ParseFrom(binary));
        }

        private DistributedPubSubMediator.Send SendFromProto(Send send)
        {
            return new DistributedPubSubMediator.Send(send.Path, PayloadFromProto(send.Payload), send.LocalAffinity);
        }

        private SendToAll SendToAllToProto(DistributedPubSubMediator.SendToAll sendToAll)
        {
        ...
        private DistributedPubSubMediator.SendToAll SendToAllFromBinary(byte[] binary)
        {
            return SendToAllFromProto(SendToAll.ParseFrom(binary));
        }

        private DistributedPubSubMediator.SendToAll SendToAllFromProto(SendToAll send)
        {
            return new DistributedPubSubMediator.SendToAll(send.Path, PayloadFromProto(send.Payload), send.AllButSelf);
        }

        private Publish PublishToProto(DistributedPubSubMediator.Publish publish)
        {
        ...
        private DistributedPubSubMediator.Publish PublishFromBinary(byte[] binary)
        {
            return PublishFromProto(Publish.ParseFrom(binary));
        }

        private DistributedPubSubMediator.Publish PublishFromProto(Publish publish)
        {
            return new DistributedPubSubMediator.Publish(publish.Topic, PayloadFromProto(publish.Payload));
        }
