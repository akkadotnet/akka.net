...
public class ClusterRoundRobinSpecConfig : MultiNodeConfig
{
    public ClusterRoundRobinSpecConfig()
    {
        ...
        NodeConfig(new List<RoleName> { Third },
...
public class ClusterRoundRobinSpec : MultiNodeClusterSpec
{
        ...
        : base(config, typeof(ClusterRoundRobinSpec))
    {
        ...
        router1 = new Lazy<IActorRef>(() =>
            Sys.ActorOf(FromConfig.Instance.Props(Props.Create<ClusterRoundRobinSpecConfig.SomeActor>()), "router1"));
        router2 = new Lazy<IActorRef>(() => Sys.ActorOf(
            ...
            "router2"));
        router3 = new Lazy<IActorRef>(() =>
            Sys.ActorOf(FromConfig.Instance.Props(Props.Create<ClusterRoundRobinSpecConfig.SomeActor>()), "router3"));
        router4 = new Lazy<IActorRef>(() => Sys.ActorOf(FromConfig.Instance.Props(), "router4"));
        router5 = new Lazy<IActorRef>(() =>
            Sys.ActorOf(new RoundRobinPool(0).Props(Props.Create<ClusterRoundRobinSpecConfig.SomeActor>()), "router5"));
    }
    ...
    private Dictionary<Address, int> ReceiveReplays(ClusterRoundRobinSpecConfig.IRouteeType routeeType,
        int expectedReplies)
    {
    ...
    private void A_cluster_router_with_a_RoundRobin_router_must_deploy_routees_to_the_member_nodes_in_the_cluster()
    {
        RunOn(() =>
        {
            ...
            for (var i = 0; i < iterationCount; i++) router1.Value.Tell("hit");
    ...
    private void A_cluster_router_with_a_RoundRobin_router_must_lookup_routees_on_the_member_nodes_in_the_cluster()
    {
        ...
        Sys.ActorOf(
            Props.Create(() =>
                new ClusterRoundRobinSpecConfig.SomeActor(new ClusterRoundRobinSpecConfig.GroupRoutee())),
            "myserviceA");
        Sys.ActorOf(
            Props.Create(() =>
                new ClusterRoundRobinSpecConfig.SomeActor(new ClusterRoundRobinSpecConfig.GroupRoutee())),
        ...
        RunOn(() =>
        {
            ...
            Within(10.Seconds(), () => { AwaitAssert(() => CurrentRoutees(router4.Value).Count().Should().Be(4)); });

            var iterationCount = 10;
            for (var i = 0; i < iterationCount; i++) router4.Value.Tell("hit");
        ...
        A_cluster_router_with_a_RoundRobin_router_must_deploy_routees_to_only_remote_nodes_when_allowlocalrouteesoff()
    {
        RunOn(() =>
        {
            ...
            var iterationCount = 10;
            for (var i = 0; i < iterationCount; i++) router3.Value.Tell("hit");
    ...
    private void A_cluster_router_with_a_RoundRobin_router_must_deploy_routees_to_specified_node_role()
    {
        RunOn(() =>
        {
            ...
            var iterationCount = 10;
            for (var i = 0; i < iterationCount; i++) router5.Value.Tell("hit");
        ...
        A_cluster_router_with_a_RoundRobin_router_must_deploy_programatically_defined_routees_to_the_member_nodes_in_the_cluster()
    {
        RunOn(() =>
        {
            ...
            var iterationCount = 10;
            for (var i = 0; i < iterationCount; i++) router2.Value.Tell("hit");

            var replays = ReceiveReplays(new ClusterRoundRobinSpecConfig.PoolRoutee(), iterationCount);

            // note that router2 has totalInstances = 3, maxInstancesPerNode = 1
            var routees = CurrentRoutees(router2.Value);
            var routeeAddresses = routees.Where(c => c is ActorRefRoutee)
                .Select(c => FullAddress(((ActorRefRoutee)c).Actor));
        ...
        A_cluster_router_with_a_RoundRobin_router_must_deploy_programatically_defined_routees_to_other_node_when_a_node_becomes_down()
    {
        ...
        RunOn(() =>
        {
            ...
            {
                if (actorRef is ActorRefRoutee actorRefRoutee) Watch(actorRefRoutee.Actor);
            });
