...
public interface IActorRefProvider
{
    ...
    ///     described by <paramref name="props" /> with the given <paramref name="supervisor" /> and <paramref name="path" />
    ///     (may be different
    ...
    /// <returns>TBD</returns>
    IInternalActorRef ActorOf(ActorSystemImpl system, Props props, IInternalActorRef supervisor, ActorPath path,
        bool systemService, Deploy deploy, bool lookupDeploy, bool async);

    /// <summary>
    ///     Get the actor reference for a specified path. If no such actor exists, it will be (equivalent to) a dead
    ///     letter reference.
    /// </summary>
    /// <param name="path">TBD</param>
    /// <returns>TBD</returns>
...
public sealed class LocalActorRefProvider : IActorRefProvider
{
    ...
    public LocalActorRefProvider(string systemName, Settings settings, EventStream eventStream, Deployer deployer,
        Func<ActorPath, IInternalActorRef> deadLettersFactory)
    {
        ...
        _userGuardianStrategyConfigurator =
            SupervisorStrategyConfigurator.CreateConfigurator(Settings.SupervisorStrategyClass);
    }
    ...
    private SupervisorStrategy UserGuardianSupervisorStrategy => _userGuardianStrategyConfigurator.Create();
    ...
    ///     TBD
    /// </summary>
    public IActorRef DeadLetters => _deadLetters;
    ...
    ///     TBD
    /// </summary>
    public IInternalActorRef RootGuardian => _rootGuardian;

    /// <summary>
    ///     TBD
    /// </summary>
    public ActorPath RootPath => _rootPath;
    ...
    ///     TBD
    /// </summary>
    public IInternalActorRef TempContainer => _tempContainer;

    /// <summary>
    ///     TBD
    /// </summary>
    public Task TerminationTask => _terminationPromise.Task;
    ...
    ///     This exception is thrown if the given <paramref name="path" /> is not on
    ///     the temp path.
    /// </exception>
    ...
    ///         </dt>
    ///         <dd>
    ///             The dispatcher in the given <paramref name="props" /> is not configured for the given
    ///             <paramref name="path" />.
    ///         </dd>
    ///         <dd>or</dd>
    ///         <dd>
    ///             There was a configuration problem while creating the given <paramref name="path" /> with the dispatcher and
    ///             mailbox from the given <paramref name="props" />
    ///         </dd>
    ///         <dt>
    ///             <b>routers</b>
    ///         </dt>
    ///         <dd>
    ///             The dispatcher in the given <paramref name="props" /> is not configured for routees of the given
    ///             <paramref name="path" />
    ///         </dd>
    ///         <dd>or</dd>
    ///         <dd>
    ///             The dispatcher in the given <paramref name="props" /> is not configured for router of the given
    ///             <paramref name="path" />
    ///         </dd>
    ///         <dd>or</dd>
    ///         <dd>
    ///             $There was a configuration problem while creating the given <paramref name="path" /> with router dispatcher
    ///             and mailbox and routee dispatcher and mailbox.
    ///         </dd>
    ///     </dl>
    /// </exception>
    /// <returns>TBD</returns>
        ...
        bool systemService, Deploy deploy, bool lookupDeploy, bool async)
    {
        ...
        //routers!!!
        {
            ...
            if (!system.Dispatchers.HasDispatcher(p.Dispatcher))
                throw new ConfigurationException(
                    $"Dispatcher [{p.Dispatcher}] not configured for routees of path [{path}]");
            if (!system.Dispatchers.HasDispatcher(d.RouterConfig.RouterDispatcher))
                throw new ConfigurationException(
                    $"Dispatcher [{p.RouterConfig.RouterDispatcher}] not configured for router of path [{path}]");

            var routerProps = Props.Empty.WithRouter(p.Deploy.RouterConfig)
                .WithDispatcher(p.RouterConfig.RouterDispatcher);
            ...
            try
            {
                ...
                var routedActorRef = new RoutedActorRef(system, routerProps, routerDispatcher, routerMailbox,
                    routeeProps,
                    supervisor, path);
            ...
            {
                throw new ConfigurationException(
                    $"Configuration problem while creating [{path}] with router dispatcher [{routerProps.Dispatcher}] and mailbox [{routerProps.Mailbox}] and routee dispatcher [{routeeProps.Dispatcher}] and mailbox [{routeeProps.Mailbox}].",
                    ex);
            }
        }
    }
    ...
    /// </summary>
    public Address DefaultAddress => _rootPath.Address;
    ...
    private RootGuardianActorRef CreateRootGuardian(ActorSystemImpl system)
    {
        ...
        var rootGuardian = new RootGuardianActorRef(system, props, InternalDispatcher, _defaultMailbox, supervisor,
            _rootPath, _deadLetters, _extraNames);
        return rootGuardian;
    }

    private LocalActorRef
        CreateUserGuardian(LocalActorRef rootGuardian,
            string name) //Corresponds to Akka's: override lazy val guardian: LocalActorRef
    {
    ...
    private LocalActorRef
        CreateSystemGuardian(LocalActorRef rootGuardian, string name,
            LocalActorRef userGuardian) //Corresponds to Akka's: override lazy val guardian: systemGuardian
    {
        var cell = rootGuardian.Cell;
        cell.ReserveChild(name);
        var props = Props.Create(() => new SystemGuardianActor(userGuardian), _systemGuardianStrategy);

        var systemGuardian = new LocalActorRef(_system, props, InternalDispatcher, _defaultMailbox, rootGuardian,
            RootPath / name);
