...
internal class ValveSwitch : IValveSwitch
{
    ...
    public ValveSwitch(Action<(SwitchMode, TaskCompletionSource<bool>)> flipCallback,
        Action<TaskCompletionSource<SwitchMode>> getModeCallback)
    {
...
/// <summary>
///     Materializes into a task of <see cref="IValveSwitch" /> which provides a method that will stop or restart the flow
///     of elements passing through the stage.
///     As long as the valve is closed it will backpressure.
///     Note that closing the valve could result in one element being buffered inside the stage, and if the stream
///     completes or fails while being closed, that element may be lost.
/// </summary>
...
public class Valve<T> : GraphStageWithMaterializedValue<FlowShape<T, T>, Task<IValveSwitch>>
{
    ...
    public override ILogicAndMaterializedValue<Task<IValveSwitch>> CreateLogicAndMaterializedValue(
    ...
    private sealed class ValveGraphStageLogic : GraphStageLogic, IInHandler, IOutHandler
    {
        ...
        private void FlipHandler((SwitchMode, TaskCompletionSource<bool>) t)
        {
