...
internal class BarrierCoordinator : FSM<BarrierCoordinator.State, BarrierCoordinator.Data>, ILoggingFSM
{
    ...
    protected void InitFSM()
    {
        StartWith(State.Idle,
            new Data(ImmutableHashSet.Create<Controller.NodeInfo>(), "", ImmutableHashSet.Create<IActorRef>(), null));
            ...
            switch (@event.FsmEvent)
            {
                ...
                case Controller.ClientDisconnected disconnected:
                    if (arrived == null || arrived.Count == 0)
                        return Stay()
                            .Using(@event.StateData.Copy(clients.Where(x => x.Name != disconnected.Name)
                                .ToImmutableHashSet()));
                    ...
                    throw new ClientLostException(
                        @event.StateData.Copy(clients.Remove(client),
                            arrived: arrived.Where(x => x != client.FSM).ToImmutableHashSet()), disconnected.Name);
                ...
                case RemoveClient client:
                    if (clients.Count == 0)
                        throw new BarrierEmptyException(@event.StateData,
                            $"cannot remove {client.Name}: no client to remove");

                    return Stay()
                        .Using(@event.StateData.Copy(clients.Where(x => x.Name != client.Name).ToImmutableHashSet()));
        ...
        {
            if (state == State.Idle && nextState == State.Waiting)
                SetTimer("Timeout", StateTimeout.Instance, NextStateData.Deadline.TimeLeft);
    ...
    public State<State, Data> HandleBarrier(Data data)
    {
        ...
        if (data.Arrived.Count == 0) return GoTo(State.Idle).Using(data.Copy(barrier: string.Empty));

        if (data.Clients.Select(x => x.FSM).ToImmutableHashSet().Except(data.Arrived).Count == 0)
        {
    ...
    {
        public Data(IEnumerable<Controller.NodeInfo> clients, string barrier, IEnumerable<IActorRef> arrived,
            Deadline deadline) :
            this(
                clients == null
                    ? ImmutableHashSet.Create<Controller.NodeInfo>()
                    : ImmutableHashSet.Create(clients.ToArray()),
                barrier,
        ...
        public Data(ImmutableHashSet<Controller.NodeInfo> clients, string barrier, ImmutableHashSet<IActorRef> arrived,
            Deadline deadline)
        {
            Deadline = deadline;
            Arrived = arrived;
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                var hashCode = Clients != null ? Clients.GetHashCode() : 0;
    ...
    public sealed class WrongBarrierException : Exception
    {
        ...
        {
            return string.Equals(Barrier, other.Barrier) && Equals(Client, other.Client) &&
                   Equals(BarrierData, other.BarrierData);
        }
