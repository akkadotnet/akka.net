...
public class PhiAccrualFailureDetector : FailureDetector
{
    ...
    ///     of a real crash. Conversely, a high threshold generates fewer mistakes but needs more time to detect actual crashes
    /// </param>
    ...
    /// </param>
    public PhiAccrualFailureDetector(double threshold, int maxSampleSize, TimeSpan minStdDeviation,
        TimeSpan acceptableHeartbeatPause, TimeSpan firstHeartbeatEstimate, Clock clock = null)
        : this(clock)
    {
    ...
    private double MinStdDeviationMillis => _minStdDeviation.TotalMilliseconds;

    private double AcceptableHeartbeatPauseMillis => _acceptableHeartbeatPause.TotalMilliseconds;
...
internal readonly struct HeartbeatHistory
{
    ...
    /// </summary>
    /// <param name="maxSampleSize">
    ///     The maximum number of samples to retain. Older ones are dropped once intervals exceeds this
    ///     value.
    /// </param>
    ...
    public HeartbeatHistory(int maxSampleSize, ImmutableList<long> intervals, long intervalSum, long squaredIntervalSum)
    {
        ...
        if (maxSampleSize < 1)
            throw new ArgumentOutOfRangeException(nameof(maxSampleSize),
                $"maxSampleSize must be >= 1, got {maxSampleSize}");
        if (intervalSum < 0L)
            throw new ArgumentOutOfRangeException(nameof(intervalSum), $"intervalSum must be >= 0, got {intervalSum}");
        if (squaredIntervalSum < 0L)
            throw new ArgumentOutOfRangeException(nameof(squaredIntervalSum),
                $"squaredIntervalSum must be >= 0, got {squaredIntervalSum}");
    }
    ...
    {
        return new HeartbeatHistory(history._maxSampleSize, history.Intervals.RemoveAt(0),
            history._intervalSum - history.Intervals.First(),
