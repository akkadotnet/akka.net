...
internal sealed class FileSource : SourceModule<ByteString, Task<IOResult>>
{
    ...
    /// <returns>TBD</returns>
    public FileSource(FileInfo f, int chunkSize, long startPosition, Attributes attributes,
        SourceShape<ByteString> shape) : base(shape)
    {
        if (chunkSize <= 0)
    ...
    {
        return new FileSource(_f, _chunkSize, _startPosition, Attributes, shape);
    }
    ...
    public override IPublisher<ByteString> Create(MaterializationContext context, out Task<IOResult> task)
    {
        ...
        var props = FilePublisher.Props(_f, ioResultPromise, _chunkSize, _startPosition,
            settings.InitialInputBufferSize, settings.MaxInputBufferSize);
        var dispatcher =
            context.EffectiveAttributes.GetAttribute(DefaultAttributes.IODispatcher
                .GetAttribute<ActorAttributes.Dispatcher>());
...
internal sealed class InputStreamSource : SourceModule<ByteString, Task<IOResult>>
{
    ...
    /// <param name="attributes">TBD</param>
    /// <param name="shape">TBD</param>
    public InputStreamSource(Func<Stream> createInputStream, int chunkSize, Attributes attributes,
        SourceShape<ByteString> shape) : base(shape)
    {
    ...
    protected override SourceModule<ByteString, Task<IOResult>> NewInstance(SourceShape<ByteString> shape)
    {
        return new InputStreamSource(_createInputStream, _chunkSize, Attributes, shape);
    }
