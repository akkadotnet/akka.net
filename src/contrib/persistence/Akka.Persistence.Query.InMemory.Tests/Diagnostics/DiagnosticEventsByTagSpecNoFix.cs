// -----------------------------------------------------------------------
//  <copyright file="DiagnosticEventsByTagSpecNoFix.cs" company="Akka.NET Project">
...
/// <summary>
/// Diagnostic version WITHOUT the fix - use this to reproduce and capture the race condition.
/// WARNING: This test is expected to be flaky/timeout!
/// </summary>
public class DiagnosticEventsByTagSpecNoFix : Akka.TestKit.Xunit2.TestKit, IDisposable
{
    ...
    public DiagnosticEventsByTagSpecNoFix(ITestOutputHelper output)
        : base(Config(), nameof(DiagnosticEventsByTagSpecNoFix), output)
    {
        _output = output;
        ...
        // *** NO FIX - Persistence.Instance.Apply(Sys) is NOT called ***
        // This means auto-start-journals config won't be processed until
        // a persistent actor triggers lazy initialization
        DiagnosticTimeline.Record("Test", "SKIPPING Persistence.Instance.Apply(Sys) - NO FIX");

        // Record before loading ReadJournal
        DiagnosticTimeline.Record("Test", "About to load ReadJournal (only loads PersistenceQuery, not Persistence)");

        _materializer = Sys.Materializer();
        _readJournal = Sys.ReadJournalFor<InMemoryReadJournal>(InMemoryReadJournal.Identifier);

        DiagnosticTimeline.Record("Test", "Constructor completed (NO FIX VERSION)");
    }
    ...
    /// <summary>
    /// This test is expected to be flaky/timeout without the fix.
    /// Run it multiple times to capture diagnostic data about the race condition.
    /// </summary>
    [Fact]
    public void NoFix_ReadJournal_live_query_EventsByTag_should_find_events_from_offset_exclusive()
    {
        DiagnosticTimeline.Record("Test", "Test method started (NO FIX - may timeout!)");
        ...
        // Create actors - this is where the race condition occurs!
        // Without the fix, RecoveryPermitter is created lazily during AroundPreStart
        DiagnosticTimeline.Record("Test", "Creating actor a (this triggers lazy RecoveryPermitter init!)");
        ...
        // This is where the timeout typically occurs!
        DiagnosticTimeline.Record("Test", "Sending 'hello' to actor a - CRITICAL: waiting for recovery to complete");
        a.Tell("hello");

        DiagnosticTimeline.Record("Test", "Waiting for 'hello-done' from actor a (3 second timeout)");
        try
        {
            var helloResult = ExpectMsg<string>(TimeSpan.FromSeconds(3));
            DiagnosticTimeline.Record("Test", $"SUCCESS: Received: {helloResult}");
            helloResult.Should().Be("hello-done");
        }
        catch (Exception ex)
        {
            DiagnosticTimeline.Record("Test", $"TIMEOUT/ERROR: {ex.GetType().Name}: {ex.Message}");
            throw; // Re-throw to fail the test but still dump diagnostics in Dispose
        }

        // Continue with rest of test...
        DiagnosticTimeline.Record("Test", "Sending 'a green apple' to actor a");
        a.Tell("a green apple");
        ExpectMsg("a green apple-done", TimeSpan.FromSeconds(3));
        DiagnosticTimeline.Record("Test", "Received 'a green apple-done'");

        DiagnosticTimeline.Record("Test", "Sending 'a black car' to actor b");
        b.Tell("a black car");
        ExpectMsg("a black car-done", TimeSpan.FromSeconds(3));
        ...
        ExpectMsg("something else-done", TimeSpan.FromSeconds(3));
        ...
        ExpectMsg("a green banana-done", TimeSpan.FromSeconds(3));
        ...
        b.Tell("a green leaf");
        ExpectMsg("a green leaf-done", TimeSpan.FromSeconds(3));
        ...
        ExpectMsg("a green cucumber-done", TimeSpan.FromSeconds(3));
        ...
        DiagnosticTimeline.Record("Test", "Test completed successfully (NO FIX VERSION - got lucky this time!)");
    }
}
