...
public class ClusterConsistentHashingRouterSpec : MultiNodeClusterSpec
{
    ...
    protected ClusterConsistentHashingRouterSpec(ConsistentHashingRouterMultiNodeConfig config) : base(config,
        typeof(ClusterConsistentHashingRouterSpec))
    {
        _config = config;
        ...
        {
            return Sys.ActorOf(
                FromConfig.Instance.Props(Props.Create<ConsistentHashingRouterMultiNodeConfig.Echo>()), "router1");
        });
    }
    ...
    private void AssertHashMapping(IActorRef router)
    {
        ...
        routerMembers.SetEquals(expected).Should()
            .BeTrue($"Expected [{string.Join(",", expected)}] but was [{string.Join(",", routerMembers)}]");
    ...
    protected void A_cluster_router_with_consistent_hashing_pool_must_create_routees_from_configuration()
    {
        ...
        {
            // it may take some time until router receives cluster member events
            AwaitAssert(() => { CurrentRoutees(router1.Value).Should().HaveCount(4); });
        ...
        A_cluster_router_with_consistent_hashing_pool_must_deploy_routees_to_new_member_nodes_in_the_cluster()
    {
        ...
        {
            //it may take some time until router receives cluster member events
            AwaitAssert(() => { CurrentRoutees(router1.Value).Should().HaveCount(6); });
            var routees = CurrentRoutees(router1.Value);
                ...
                Sys.ActorOf(
                    new ClusterRouterPool(
                        ...
                        new ClusterRouterPoolSettings(
                            ...
                            2,
                            true,
                            null)).Props(Props.Create<ConsistentHashingRouterMultiNodeConfig.Echo>()), "router2");

            //it may take some time until router receives cluster member events
            AwaitAssert(() => { CurrentRoutees(router2).Should().HaveCount(6); });
        ...
        A_cluster_router_with_consistent_hashing_pool_must_handle_combination_of_configured_router_and_programatically_defined_hash_mapping()
    {
        RunOn(() =>
        {
            ...
            var router3 = Sys.ActorOf(new ConsistentHashingPool(0).WithHashMapping(hashMapping)
        ...
        A_cluster_router_with_consistent_hashing_pool_must_handle_combination_of_configured_router_and_programatically_defined_hash_mapping_and_cluster_config()
    {
        RunOn(() =>
        {
            ...
            var router4 =
                Sys.ActorOf(
                    new ClusterRouterPool(
                        new ConsistentHashingPool(0).WithHashMapping(hashMapping),
                        new ClusterRouterPoolSettings(
                            10,
                            1,
                            true,
                            null)).Props(Props.Create<ConsistentHashingRouterMultiNodeConfig.Echo>()), "router4");
    ...
    protected void A_cluster_router_with_consistent_hashing_pool_must_remove_routees_from_downed_node()
    {
        RunOn(() =>
        {
            ...
            //removed
            AwaitAssert(() =>
                Assert.DoesNotContain(GetAddress(_config.Third),
            ...
            // it may take some time until router receives cluster member events
            AwaitAssert(() => { CurrentRoutees(router1.Value).Count().ShouldBe(4); });
            var routees = CurrentRoutees(router1.Value);
            var routerMembers = routees.Select(x => FullAddress(((ActorRefRoutee)x).Actor)).Distinct().ToList();
