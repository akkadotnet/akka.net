...
public sealed class SplitBrainResolverSettings
{
    ...
    public SplitBrainResolverSettings(Config config)
    {
        ...
        _lazyLeaseMajoritySettings = new Lazy<LeaseMajoritySettings>(() =>
        {
            ...
            return new LeaseMajoritySettings(leaseImplementation, acquireLeaseDelayForMinority, releaseAfter, Role(c),
                leaseName);
        });
    }
...
{
    public LeaseMajoritySettings(string leaseImplementation, TimeSpan acquireLeaseDelayForMinority,
        TimeSpan releaseAfter, string role, string leaseName)
    {
