...
public sealed class MetricsGossip
{
    ...
    {
        return new MetricsGossip(Nodes.Where(n => !n.Address.Equals(node))
            .ToImmutableHashSet(NodeMetricsComparer.Instance));
    }
    ...
    {
        return new MetricsGossip(Nodes.Where(n => includeNodes.Contains(n.Address))
            .ToImmutableHashSet(NodeMetricsComparer.Instance));
    }
    ...
    public static MetricsGossip operator +(MetricsGossip gossip, NodeMetrics newNodeMetrics)
    {
        ...
        if (existingMetrics.HasValue)
            return new MetricsGossip(gossip.Nodes.Remove(existingMetrics.Value)
                .Add(existingMetrics.Value.Update(newNodeMetrics)));
