...
internal class RecoveryPermitter : UntypedActor
{
    ...
    {
        return Actor.Props.Create(() => new RecoveryPermitter(maxPermits));
    }
    ...
    private void ReturnRecoveryPermit(IActorRef actorRef)
    {
        ...
        Log.Debug("Drained pending recovery permit requests, max in progress was [{0}], still [{1}] in progress",
