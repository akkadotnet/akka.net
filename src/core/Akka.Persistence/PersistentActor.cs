...
public sealed class RecoveryCompleted
{
    ...
    {
        return nameof(RecoveryCompleted).GetHashCode();
    }
}
...
///     <see cref="SnapshotOffer" /> message, followed by replayed messages, if any, that are younger than the snapshot, up
///     to the
...
public sealed class Recovery
{
    ...
    ///     Criteria for selecting a saved snapshot from which recovery should start. Default is
    ///     latest(= youngest) snapshot.
    /// </param>
    ...
    public Recovery(SnapshotSelectionCriteria fromSnapshot, long toSequenceNr) : this(fromSnapshot, toSequenceNr,
        long.MaxValue)
    {
    }
    ...
    public Recovery(SnapshotSelectionCriteria fromSnapshot = null, long toSequenceNr = long.MaxValue,
...
public abstract class UntypedPersistentActor : Eventsourced
{
    ...
    /// <summary>
    ///     Changes the actor's behavior and replaces the current receive handler with the specified handler.
    ///     The current handler is stored on a stack, and you can revert to it by calling
    ///     <see cref="IActorContext.UnbecomeStacked" />
    ///     <remarks>
    ///         Please note, that in order to not leak memory, make sure every call to <see cref="BecomeStacked" />
    ///         is matched with a call to <see cref="IActorContext.UnbecomeStacked" />.
    ///     </remarks>
    /// </summary>
    /// <param name="receive">The new message handler.</param>
...
public abstract class ReceivePersistentActor : UntypedPersistentActor, IInitializableActor
{
    ...
    ///     Changes the actor's command behavior and replaces the current receive command handler with the specified handler.
    ///     The current handler is stored on a stack, and you can revert to it by calling
    ///     <see cref="ActorBase.UnbecomeStacked" />
    ///     <remarks>
    ///         Please note, that in order to not leak memory, make sure every call to <see cref="BecomeStacked" />
    ///         is matched with a call to <see cref="ActorBase.UnbecomeStacked" />.
    ///     </remarks>
    /// </summary>
    ...
    private void EnsureMayConfigureRecoverHandlers()
    {
        if (_matchRecoverBuilders.Count <= 0)
            throw new InvalidOperationException(
                "You may only call Recover-methods when constructing the actor and inside Become().");
    }
    ...
    private void EnsureMayConfigureCommandHandlers()
    {
        if (_matchCommandBuilders.Count <= 0)
            throw new InvalidOperationException(
                "You may only call Command-methods when constructing the actor and inside Become().");
    }
    ...
    ///     <remarks>
    ///         The actor will be suspended until the task returned by <paramref name="handler" /> completes, including the
    ///         <see cref="Eventsourced.Persist{TEvent}(TEvent, Action{TEvent})" /> and
    ///         <see cref="Eventsourced.PersistAll{TEvent}(IEnumerable{TEvent}, Action{TEvent})" /> calls.
    ///     </remarks>
    ///     <remarks>
    ///         This method may only be called when constructing the actor or from <see cref="Become(System.Action)" /> or
    ///         <see cref="BecomeStacked" />.
    ///     </remarks>
    ///     <remarks>
