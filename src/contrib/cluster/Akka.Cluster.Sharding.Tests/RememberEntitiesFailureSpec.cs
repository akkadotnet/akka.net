...
public class RememberEntitiesFailureSpec : AkkaSpec
{
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_when_initial_remember_entities_load_fails_NoResponse()
    {
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_when_initial_remember_entities_load_fails_CrashStore()
    {
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_when_initial_remember_entities_load_fails_StopStore()
    {
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_when_initial_remember_entities_load_fails_Delay_500()
    {
        Remember_entities_handling_in_sharding_must_recover_when_initial_remember_entities_load_fails(
            new Delay(TimeSpan.FromMilliseconds(500)));
    }

    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_when_initial_remember_entities_load_fails_Delay_1000()
    {
        Remember_entities_handling_in_sharding_must_recover_when_initial_remember_entities_load_fails(
            new Delay(TimeSpan.FromSeconds(1)));
    }

    private void Remember_entities_handling_in_sharding_must_recover_when_initial_remember_entities_load_fails(
        IFail wayToFail)
    {
    ...
    {
        Remember_entities_handling_in_sharding_must_recover_when_shard_storing_a_start_event_fails(
            new Delay(TimeSpan.FromMilliseconds(500)));
    }
    ...
    private void Remember_entities_handling_in_sharding_must_recover_when_shard_storing_a_start_event_fails(
        IFail wayToFail)
    {
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_on_abrupt_entity_stop_when_storing_a_stop_event_fails_NoResponse()
    {
        Remember_entities_handling_in_sharding_must_recover_on_abrupt_entity_stop_when_storing_a_stop_event_fails(
            new NoResponse());
    }

    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_on_abrupt_entity_stop_when_storing_a_stop_event_fails_CrashStore()
    {
        Remember_entities_handling_in_sharding_must_recover_on_abrupt_entity_stop_when_storing_a_stop_event_fails(
            new CrashStore());
    }
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_on_abrupt_entity_stop_when_storing_a_stop_event_fails_StopStore()
    {
        Remember_entities_handling_in_sharding_must_recover_on_abrupt_entity_stop_when_storing_a_stop_event_fails(
            new StopStore());
    }
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_on_abrupt_entity_stop_when_storing_a_stop_event_fails_Delay_500()
    {
        Remember_entities_handling_in_sharding_must_recover_on_abrupt_entity_stop_when_storing_a_stop_event_fails(
            new Delay(TimeSpan.FromMilliseconds(500)));
    }
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_on_abrupt_entity_stop_when_storing_a_stop_event_fails_Delay_1000()
    {
        Remember_entities_handling_in_sharding_must_recover_on_abrupt_entity_stop_when_storing_a_stop_event_fails(
            new Delay(TimeSpan.FromSeconds(1)));
    }
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_on_graceful_entity_stop_when_storing_a_stop_event_fails_NoResponse()
    {
        Remember_entities_handling_in_sharding_must_recover_on_graceful_entity_stop_when_storing_a_stop_event_fails(
            new NoResponse());
    }

    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_on_graceful_entity_stop_when_storing_a_stop_event_fails_CrashStore()
    {
        Remember_entities_handling_in_sharding_must_recover_on_graceful_entity_stop_when_storing_a_stop_event_fails(
            new CrashStore());
    }
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_on_graceful_entity_stop_when_storing_a_stop_event_fails_StopStore()
    {
        Remember_entities_handling_in_sharding_must_recover_on_graceful_entity_stop_when_storing_a_stop_event_fails(
            new StopStore());
    }
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_on_graceful_entity_stop_when_storing_a_stop_event_fails_Delay_500()
    {
        Remember_entities_handling_in_sharding_must_recover_on_graceful_entity_stop_when_storing_a_stop_event_fails(
            new Delay(TimeSpan.FromMilliseconds(500)));
    }
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_on_graceful_entity_stop_when_storing_a_stop_event_fails_Delay_1000()
    {
        Remember_entities_handling_in_sharding_must_recover_on_graceful_entity_stop_when_storing_a_stop_event_fails(
            new Delay(TimeSpan.FromSeconds(1)));
    }
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_when_coordinator_storing_shard_start_fails_NoResponse()
    {
        Remember_entities_handling_in_sharding_must_recover_when_coordinator_storing_shard_start_fails(
            new NoResponse());
    }

    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_when_coordinator_storing_shard_start_fails_CrashStore()
    {
        Remember_entities_handling_in_sharding_must_recover_when_coordinator_storing_shard_start_fails(
            new CrashStore());
    }
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_when_coordinator_storing_shard_start_fails_StopStore()
    {
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_when_coordinator_storing_shard_start_fails_Delay_500()
    {
        Remember_entities_handling_in_sharding_must_recover_when_coordinator_storing_shard_start_fails(
            new Delay(TimeSpan.FromMilliseconds(500)));
    }
    ...
    [Fact]
    public void
        Remember_entities_handling_in_sharding_must_recover_when_coordinator_storing_shard_start_fails_Delay_1000()
    {
        Remember_entities_handling_in_sharding_must_recover_when_coordinator_storing_shard_start_fails(
            new Delay(TimeSpan.FromSeconds(1)));
    }

    private void Remember_entities_handling_in_sharding_must_recover_when_coordinator_storing_shard_start_fails(
        IFail wayToFail)
    {
        var storeProbe = CreateTestProbe();
    ...
    private class Delay : IFail
    {
        ...
        public override string ToString()
        {
            return $"Delay{HowLong.TotalMilliseconds}";
        }
    }
    ...
    private class FakeShardStoreActor : ActorBase, IWithTimers
    {
        ...
        {
            return Actor.Props.Create(() => new FakeShardStoreActor(shardId));
        }
            ...
            switch (message)
            {
                case RememberEntitiesShardStore.GetEntities _:
                    switch (failShardGetEntities.GetValueOrDefault(shardId))
                    {
                        case null:
                            Sender.Tell(
                                new RememberEntitiesShardStore.RememberedEntities(ImmutableHashSet<string>.Empty));
                            return true;
                        case NoResponse _:
                        ...
                        case Delay f:
                            ...
                            Timers.StartSingleTimer("get-entities-delay",
                                new Delayed(Sender, ImmutableHashSet<string>.Empty), f.HowLong);
                            return true;
                    }
                ...
                case RememberEntitiesShardStore.Update m:
                    switch (failUpdate)
                    {
                        ...
                        case Delay f:
                            ...
                            Timers.StartSingleTimer("add-entity-delay",
                                new Delayed(Sender, ImmutableHashSet<string>.Empty), f.HowLong);
                            return true;
                    }
    ...
    private class FakeCoordinatorStoreActor : ActorBase, IWithTimers
    {
        ...
        {
            return Actor.Props.Create(() => new FakeCoordinatorStoreActor());
        }
            ...
            switch (message)
            {
                case RememberEntitiesCoordinatorStore.GetShards _:
                    switch (failCoordinatorGetShards)
                    {
                        case null:
                            Sender.Tell(
                                new RememberEntitiesCoordinatorStore.RememberedShards(ImmutableHashSet<string>.Empty));
                            return true;
                        case NoResponse _:
                            return true;
                        case CrashStore _:
                        ...
                        case Delay f:
                            ...
                            Timers.StartSingleTimer("list-shards-delay",
                                new FakeShardStoreActor.Delayed(Sender, ImmutableHashSet<string>.Empty), f.HowLong);
                            return true;
                    }
                ...
                case RememberEntitiesCoordinatorStore.AddShard m:
                    switch (failAddShard.GetValueOrDefault(m.ShardId))
                    {
                        ...
                        case Delay f:
                            ...
                            Timers.StartSingleTimer("add-shard-delay",
                                new FakeShardStoreActor.Delayed(Sender, ImmutableHashSet<string>.Empty), f.HowLong);
                            return true;
                    }
