...
public class PublisherFluentBuilder<T>
{
    ...
    public async Task<long> ExpectRequestAsync(CancellationToken cancellationToken = default)
    {
        if (!(Probe is Probe<T> probe))
            throw new InvalidOperationException(
                $"{nameof(ExpectCancellation)} can only be used on a {nameof(Probe<T>)} instance");
    ...
    /// <summary>
    ///     Execute the async chain and then receive messages for a given duration or until one does not match a given partial
    ///     function.
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default) where TOther : class
    {
        await ExecuteAsync(cancellationToken: cancellationToken)
            .ConfigureAwait(false);
        await foreach (var item in ManualProbe<T>.ReceiveWhileTask(Probe.Probe, max, idle, filter, msgCount,
                           cancellationToken)) yield return item;
    }
    ...
    /// <summary>
    ///     Execute the async chain and then execute the code block while bounding its execution time between
    ///     <paramref name="min" /> and
    ///     <paramref name="max" />. <see cref="WithinAsync{TOther}(TimeSpan,TimeSpan,Func{Task{TOther}},CancellationToken)" />
    ...
    ///     <see cref="WithinAsync{TOther}(TimeSpan,TimeSpan,Func{Task{TOther}},CancellationToken)" /> block.
    ///     <para />
        ...
        CancellationToken cancellationToken = default)
    {
        await ExecuteAsync(cancellationToken: cancellationToken)
            .ConfigureAwait(false);
        return await Probe.WithinAsync(min, max, function, cancellationToken)
            .ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        await ExecuteAsync(cancellationToken: cancellationToken)
            .ConfigureAwait(false);
        return await Probe.WithinAsync(max, execute, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    public PublisherFluentBuilder<T> EnsureSubscription()
    {
        if (!(Probe is Probe<T> probe))
            throw new InvalidOperationException(
                $"{nameof(EnsureSubscription)} can only be used on a {nameof(Probe<T>)} instance");
    ...
    public PublisherFluentBuilder<T> SendNext(T element)
    {
        if (!(Probe is Probe<T> probe))
            throw new InvalidOperationException(
                $"{nameof(SendNext)} can only be used on a {nameof(Probe<T>)} instance");
    ...
    public PublisherFluentBuilder<T> UnsafeSendNext(T element)
    {
        if (!(Probe is Probe<T> probe))
            throw new InvalidOperationException(
                $"{nameof(UnsafeSendNext)} can only be used on a {nameof(Probe<T>)} instance");
    ...
    public PublisherFluentBuilder<T> SendComplete()
    {
        if (!(Probe is Probe<T> probe))
            throw new InvalidOperationException(
                $"{nameof(SendComplete)} can only be used on a {nameof(Probe<T>)} instance");
    ...
    public PublisherFluentBuilder<T> SendError(Exception e)
    {
        if (!(Probe is Probe<T> probe))
            throw new InvalidOperationException(
                $"{nameof(SendError)} can only be used on a {nameof(Probe<T>)} instance");
