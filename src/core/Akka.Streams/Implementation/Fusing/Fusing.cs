...
#pragma warning restore CS0162 // Unreachable code detected
    ...
    /// <returns></returns>
    public static StructuralInfoModule StructuralInfo<TShape, TMat>(IGraph<TShape, TMat> graph, Attributes attributes)
        where TShape : Shape
    {
        ...
        try
        {
            ...
            return structuralInfo.ToInfo(shape, materializedValue.Select(pair => (pair.Key, pair.Value)).ToList(),
                attributes);
        }
        catch (Exception)
        {
    ...
    /// <summary>
    ///     Take the fusable islands identified by <see cref="Descend{T}" /> in the <see cref="BuildStructuralInfo.Groups" />
    ///     list
    ...
    private static GraphModule FuseGroup(BuildStructuralInfo info, ISet<IModule> group)
    {
        ...
        while (enumerator.MoveNext())
        {
            ...
            if ((copy = enumerator.Current as CopiedModule) != null &&
                (graphStageModule = copy.CopyOf as GraphStageModule) != null)
            {
        ...
        while (enumerator.MoveNext())
        {
                ...
                (graphStageModule = copy.CopyOf as GraphStageModule) != null)
            {
                ...
                while (copyOutlets.MoveNext() && originalOutlets.MoveNext())
                {
                    ...
                    if (outConns.TryGetValue(copyOutlet, out var idx))
                    {
        ...
        for (var i = inOwnersB1.Count + inOwnersB2.Count; i < inOwners.Length; i++) inOwners[i] = -1;
        ...
        for (var i = 0; i < connStart; i++) outOwners[i] = -1;
        ...
        return new GraphModule(new GraphAssembly(stages, attributes, ins, inOwners, outs, outOwners), shape, attr,
        ...
        int indent)
    {
        ...
        else
        {
            ...
            foreach (var entry in down) structInfo.Wire(entry.Key, entry.Value, indent);
            ...
            foreach (var c in materializedSources)
            {
                ...
                var newSrc =
                    (IMaterializedValueSource)Activator.CreateInstance(materializedValueSourceType, mapped, ms.Outlet);
    ...
    ///     descend().
    /// </summary>
    private static IMaterializedValueNode RewriteMaterializer(IDictionary<IModule, IMaterializedValueNode> subMat,
        IMaterializedValueNode mat, Dictionary<IMaterializedValueNode, IMaterializedValueNode> mapping)
    {
        ...
        {
            var result = new Combine(combine.Combinator, RewriteMaterializer(subMat, combine.Left, mapping),
                RewriteMaterializer(subMat, combine.Right, mapping));
...
internal sealed class BuildStructuralInfo
{
    ...
    /// </summary>
    public readonly IDictionary<OutPort, LinkedList<OutPort>> NewOutputs =
        new Dictionary<OutPort, LinkedList<OutPort>>();

    /// <summary>
    ///     A mapping from OutPort to its containing group, needed when determining whether an upstream connection is internal
    ///     or not.
    /// </summary>
    ...
    /// </summary>
    /// <returns>TBD</returns>
    public StructuralInfoModule ToInfo<TShape>(TShape shape,
        IList<(IModule, IMaterializedValueNode)> materializedValues, Attributes attributes = null) where TShape : Shape
    {
    ...
    /// <returns>TBD</returns>
    public Atomic AddModule(IModule module, ISet<IModule> group, Attributes inheritedAttributes, int indent,
        Shape oldShape = null)
    {
    ...
    internal void Dump()
    {
        ...
        NewInputs.ForEach(kvp =>
            Console.WriteLine($"    {kvp.Key} ({Hash(kvp.Key)}) -> {string.Join(",", kvp.Value.Select(Hash))}"));
