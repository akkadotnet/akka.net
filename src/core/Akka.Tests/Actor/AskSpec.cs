...
public class AskSpec : AkkaSpec
{
    ...
    public async Task Can_get_timeout_when_asking_actor()
    {
        ...
        await Assert.ThrowsAsync<AskTimeoutException>(async () =>
            await actor.Ask<string>("timeout", TimeSpan.FromSeconds(3)));
    }
    ...
    public async Task Ask_should_put_timeout_answer_into_deadletter()
    {
        ...
        await EventFilter.DeadLetter<object>().ExpectOneAsync(TimeSpan.FromSeconds(5),
            async () =>
            {
                await Assert.ThrowsAsync<AskTimeoutException>(async () =>
                    await actor.Ask<string>("delay", TimeSpan.FromSeconds(1)));
            });
    }
        ...
        await EventFilter.DeadLetter<object>().ExpectAsync(0, async () =>
        {
            using (var cts = new CancellationTokenSource(TimeSpan.FromSeconds(1)))
            {
                await Assert.ThrowsAsync<TaskCanceledException>(async () =>
                    await actor.Ask<string>("delay", Timeout.InfiniteTimeSpan, cts.Token));
            }
        });
    }
    ...
    public async Task Ask_should_put_invalid_answer_into_deadletter()
    {
        ...
        await EventFilter.DeadLetter<object>().ExpectOne(async () =>
        {
            await Assert.ThrowsAsync<ArgumentException>(async () =>
                await actor.Ask<string>("invalid", TimeSpan.FromSeconds(1)));
        });
    }
    ...
    public async Task Ask_should_fail_on_system_message()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(async () =>
            await actor.Ask<ISystemMessage>("system", TimeSpan.FromSeconds(1)));
    }
    ...
    public async Task Can_cancel_when_asking_actor()
    {
        ...
        {
            await Assert.ThrowsAsync<TaskCanceledException>(async () =>
                await actor.Ask<string>("timeout", Timeout.InfiniteTimeSpan, cts.Token));
        }
    }
    ...
    public async Task Cancelled_ask_with_timeout_should_remove_temp_actor()
    {
        ...
        {
            await Assert.ThrowsAsync<TaskCanceledException>(async () =>
                await actor.Ask<string>("cancel", TimeSpan.FromSeconds(30), cts.Token));
        }
    ...
    {
        var actor = Sys.ActorOf(act => act.ReceiveAny((_, context) => { context.Sender.Tell(null); }));
    ...
    public class ReplyActor : UntypedActor
    {
        ...
        {
            if (message.Equals("foo")) Sender.Tell("bar");
        }
    }
