...
{
    private readonly RestartNodeSpecConfig _config;
    private readonly Lazy<ActorSystem> _secondRestartedSystem;

    // use a separate ActorSystem, to be able to simulate restart
    private readonly Lazy<ActorSystem> _secondSystem;

    private volatile UniqueAddress _secondUniqueAddress;
    ...
    protected RestartNodeSpec(RestartNodeSpecConfig config) : base(config, typeof(RestartNodeSpec))
    {
        ...
        _secondRestartedSystem = new Lazy<ActorSystem>(() => ActorSystem.Create(Sys.Name,
            ConfigurationFactory.ParseString("akka.remote.dot-netty.tcp.port=" + _secondUniqueAddress.Address.Port)
                .WithFallback(Sys.Settings.Config)));
    }

    public UniqueAddress SecondUniqueAddress => _secondUniqueAddress;

    private ImmutableList<Address> SeedNodes => ImmutableList.Create(GetAddress(_config.First),
        SecondUniqueAddress.Address,
    ...
    public void ClusterNodesMustBeAbleToRestartAndJoinAgain()
    {
        Within(TimeSpan.FromSeconds(60), () =>
        {
            ...
            RunOn(() =>
            {
                ...
                AwaitAssert(() => Assert.True(Cluster.Get(_secondSystem.Value).State.Members.Select(x => x.Status)
                    .All(s => s == MemberStatus.Up)));
            }, _config.Second);
            ...
            RunOn(() =>
            {
                ...
                AwaitAssert(() => Assert.True(Cluster.Get(_secondRestartedSystem.Value).State.Members
                    .Select(x => x.Status).All(s => s == MemberStatus.Up)));
            }, _config.Second);
            ...
            RunOn(() =>
            {
                AwaitAssert(() =>
                {
                    ...
                    Assert.Contains(
                        ...
                        m => m.Address.Equals(SecondUniqueAddress.Address) &&
