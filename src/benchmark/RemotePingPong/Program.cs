...
namespace PingPong
{
    ...
    internal class Program
    {
        ...
        {
            const int repeatFactor = 2;
            const long repeat = 2000 * repeatFactor;
        ...
        private static async Task<Tuple<bool, long, int>> Benchmark<TActor>(int factor, int numberOfClients, long numberOfRepeats, PrintStats printStats, long bestThroughput, int redCount) where TActor : ActorBase
        {
            ...
            var pingConfig = ConfigurationFactory.ParseString(@"
akka {  
...
");
            ...
            var pongConfig = ConfigurationFactory.ParseString(@"
akka {  
    log-config-on-start = off
    stdout-loglevel = OFF
    loglevel = OFF
...
");
            ...
            var system = ActorSystem.Create("ping", pingConfig);
            var remoteSystem = ActorSystem.Create("pong", pongConfig);
            ...
            system.Dispatchers.Lookup("akka.actor.default-fork-join-dispatcher").Throughput = factor;
            remoteSystem.Dispatchers.Lookup("akka.actor.default-fork-join-dispatcher").Throughput = factor;
            for(int i = 0; i < numberOfClients; i++)
            {
                var destination = system.ActorOf(Props.Create<Destination>().WithDeploy(
                        new Deploy(new RemoteScope(new Address("akka.tcp", "pong", "localhost", 8080)))),"destination-" + i);

                var ts = new TaskCompletionSource<bool>();
                tasks.Add(ts.Task);
                var props =
                    new Props(typeof (TActor), null, destination, repeatsPerClient, ts);

                var client = system.ActorOf(props, "client-" + i);
            ...
            if(!countdown.Wait(TimeSpan.FromSeconds(60)))
            {
                Console.WriteLine("The system did not start in 60 seconds. Aborting.");
            ...
            long throughput = elapsedMilliseconds == 0 ? -1 : (long)((totalMessagesReceived / (double)elapsedMilliseconds) * 1000.0);
