...
public abstract partial class TestKitBase
{
    ...
    ///     </para>
    ///     <remarks>
    ///         Note that the max duration is scaled using <see cref="Dilated(TimeSpan)" /> which uses the config value
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _assertions.AssertTrue(rem.IsInfiniteTimeout() || rem >= min,
            "Required min time {0} not possible, only {1} left. {2}", min, rem, hint ?? "");
