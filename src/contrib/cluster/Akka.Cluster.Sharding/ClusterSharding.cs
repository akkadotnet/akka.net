...
public abstract class HashCodeMessageExtractor : IMessageExtractor
{
    ...
    protected HashCodeMessageExtractor(int maxNumberOfShards)
    {
        ...
        foreach (var c in Enumerable.Range(0, maxNumberOfShards)) _cachedIds[c] = c.ToString();
    }
    ...
    /// <returns></returns>
    public static HashCodeMessageExtractor Create(int maxNumberOfShards, Func<Msg, string?> entityIdExtractor,
        Func<Msg, Msg>? messageExtractor = null)
    {
        return new Implementation(maxNumberOfShards, entityIdExtractor, messageExtractor);
    }
    ...
    private sealed class Implementation : HashCodeMessageExtractor
    {
        ...
        public Implementation(int maxNumberOfShards, Func<Msg, string?> entityIdExtractor,
            Func<Msg, Msg>? messageExtractor = null) : base(maxNumberOfShards)
        {
        ...
        {
            return _entityIdExtractor.Invoke(message);
        }

        public override Msg? EntityMessage(Msg message)
        {
            return _messageExtractor?.Invoke(message) ?? base.EntityMessage(message);
        }
    }
}
...
///         sending the messages via a <see cref="Sharding.ShardRegion" /> actor, provided by this extension. The
///         <see cref="Sharding.ShardRegion" />
...
///         1. Retrieve the <see cref="Sharding.ShardRegion" /> actor for a named entity type with
///         <see cref="ClusterSharding.ShardRegion" />
...
///         <see cref="ShardCoordinator" />. The <see cref="ShardCoordinator" /> decides which
///         <see cref="Sharding.ShardRegion" />
...
///         <see cref="Sharding.ShardRegion" /> actors will start buffering incoming messages for that shard, as they do
///         when
///         shard location is unknown. During the rebalance process the coordinator will not answer any
///         requests for the location of shards that are being rebalanced, i.e. local buffering will
///         continue until the handoff is complete. The <see cref="Sharding.ShardRegion" /> responsible for the rebalanced
///         shard
///         will stop all entities in that shard by sending them a <see cref="PoisonPill" />. When all entities have
///         been terminated the <see cref="Sharding.ShardRegion" /> owning the entities will acknowledge to the coordinator
///         that
...
///         <see cref="Sharding.ShardRegion" /> actors are delivered to the new location. This means that the state of the
///         entities
...
///         picks shards for handoff from the <see cref="Sharding.ShardRegion" /> with highest number of previously
///         allocated shards.
///         They will then be allocated to the <see cref="Sharding.ShardRegion" /> with lowest number of previously
///         allocated shards,
...
#pragma warning restore CS0419 // Ambiguous reference in cref attribute
{
    ...
    ///     Function that, given an entity id, returns the <see cref="Actor.Props" /> of the entity actors that will be created
    ///     by the <see cref="Sharding.ShardRegion" />
    /// </param>
    /// <param name="settings">Configuration settings, see <see cref="ClusterShardingSettings" /></param>
    /// <param name="extractEntityId">
    ///     Partial function to extract the entity id and the message to send to the
        ...
        Msg handOffStopMessage)
    {
        ...
        _cluster.System.Log.Debug("Starting Shard Region Proxy [{0}] (no actors will be hosted on this node)...",
            typeName);
        return StartProxy(
            typeName,
            settings.Role,
    ...
    /// <returns>The actor ref of the <see cref="Sharding.ShardRegion" /> that is to be responsible for the shard.</returns>
    [Obsolete("Use one of the overloads that accepts an IMessageExtractor instead")]
    public Task<IActorRef> StartProxyAsync(string typeName, string role, ExtractEntityId extractEntityId,
        ExtractShardId extractShardId)
    {
    ...
    public IActorRef StartProxy(string typeName, string role, IMessageExtractor messageExtractor)
    {
        ...
        var startMsg = new ClusterShardingGuardian.StartProxy(typeName, settings, messageExtractor);
        var reply = _guardian.Value.Ask(startMsg, timeout).Result;
        switch (reply)
        {
            case ClusterShardingGuardian.Started started:
...
#pragma warning disable CS0419 // Ambiguous reference in cref attribute
    /// <summary>
    ///     Retrieve the actor reference of the <see cref="Sharding.ShardRegion" /> actor responsible for the named entity
    ///     type.
    ///     The entity type must be registered with the <see cref="ClusterSharding.Start" /> or
...
#pragma warning disable CS0419 // Ambiguous reference in cref attribute
    ...
    ///     <see cref="ClusterShardingGuardian.StartProxy" /> method before it can be used here. Messages to the entity is
    ///     always sent
...
#pragma warning disable CS0618 // Type or member is obsolete
    ...
    public ShardId ShardId(string entityId, Msg? messageHint = null)
    {
        if (messageHint is null)
            throw new ArgumentNullException(nameof(messageHint),
                "DeprecatedHandlerExtractorAdapter: Message hint must be provided when using the ShardId(EntityId, object) overload.");
