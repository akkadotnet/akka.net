...
public interface IActorDsl
{
    ...
    /// <summary>
    ///     Changes the actor's behavior and replaces the current handler with the specified handler without discarding the
    ///     current.
    ...
    ///         is matched with a call to <see cref="UnbecomeStacked" />.
    ///     </remarks>
    /// </summary>
    /// <param name="handler">TBD</param>
    ...
    ///     <remarks>
    ///         In order to store an actor on the behavior stack, a call to <see cref="BecomeStacked" /> must have been made
    ///         prior to this call
    ///     </remarks>
    /// </summary>
...
public static class ActExtensions
{
    ...
    /// <param name="name">TBD</param>
    /// <returns>TBD</returns>
    public static IActorRef ActorOf(this IActorRefFactory factory, Action<IActorDsl, IActorContext> config,
        string name = null)
    {
        return factory.ActorOf(Props.Create(() => new Act(config)), name);
    }
