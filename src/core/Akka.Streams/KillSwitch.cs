...
///     - The factory <see cref="Single{T}" /> returns a <see cref="IGraph{TShape}" /> of
///     <see cref="FlowShape{TIn,TOut}" />
...
{
    /// <summary>
    ///     Creates a new <see cref="SharedKillSwitch" /> with the given name that can be used to control the completion of
    ///     multiple
    ...
    {
        return new SharedKillSwitch(name);
    }

    /// <summary>
    ///     Creates a new <see cref="IGraph{TShape}" /> of <see cref="FlowShape{TIn,TOut}" /> that materializes to an external
    ///     switch that allows external completion
    ...
    {
        return UniqueKillSwitchStage<T>.Instance;
    }

    /// <summary>
    ///     Creates a new <see cref="IGraph{TShape}" /> of <see cref="BidiShape{TIn1,TIn1,TOut1,TOut1}" /> that materializes to
    ///     an external switch that allows external completion
    ///     of that unique materialization. Different materializations result in different, independent switches.
    ...
    public static IGraph<BidiShape<T1, T1, T2, T2>, UniqueKillSwitch> SingleBidi<T1, T2>
        ()
    {
        return UniqueBidiKillSwitchStage<T1, T2>.Instance;
    }
    ...
    /// </param>
    /// <returns></returns>
    public static IGraph<FlowShape<T, T>, NotUsed> AsFlow<T>(this CancellationToken cancellationToken,
        bool cancelGracefully = false)
    {
    ...
    internal sealed class CancellableKillSwitchStage<T> : GraphStage<FlowShape<T, T>>
    {
        ...
        private sealed class Logic : InAndOutGraphStageLogic
        {
            ...
            {
                FailStage(new OperationCanceledException("Stage cancelled due to cancellation token request.",
                    _stage._cancellationToken));
            }
        }
        ...
        GraphStageWithMaterializedValue<BidiShape<T1, T1, T2, T2>, UniqueKillSwitch>
    {
        ...
        {
            Shape = new BidiShape<T1, T1, T2, T2>(In1, Out1, In2, Out2);
        }
        ...
        private sealed class Logic : KillableGraphStageLogic
        {
                ...
                : base(terminationSignal, killSwitch.Shape)
            {
                ...
                SetHandler(killSwitch.In2,
                    ...
                    () => Complete(killSwitch.Out2),
                    cause => Fail(killSwitch.Out2, cause));

                SetHandler(killSwitch.Out2,
                    () => Pull(killSwitch.In2),
                    cause => Cancel(killSwitch.In2, cause));
            }

            public override void OnPush()
            {
                Push(_killSwitch.Out1, Grab(_killSwitch.In1));
            }

            public override void OnUpstreamFinish()
            {
                Complete(_killSwitch.Out1);
            }

            public override void OnUpstreamFailure(Exception e)
            {
                Fail(_killSwitch.Out1, e);
            }

            public override void OnPull()
            {
                Pull(_killSwitch.In1);
            }

            public override void OnDownstreamFinish(Exception cause)
            {
                Cancel(_killSwitch.In1, cause);
            }
        }
...
///     Depending on whether the <see cref="IKillSwitch" /> is a <see cref="UniqueKillSwitch" /> or a
///     <see cref="SharedKillSwitch" /> one or
...
{
    /// <summary>
    ///     After calling <see cref="Shutdown" /> the linked <see cref="IGraph{TShape}" />s of
    ///     <see cref="FlowShape{TIn,TOut}" /> are completed normally.
    /// </summary>
    void Shutdown();

    /// <summary>
    ///     After calling <see cref="Abort" /> the linked <see cref="IGraph{TShape}" />s of <see cref="FlowShape{TIn,TOut}" />
    ///     are failed.
    /// </summary>
...
/// <summary>
///     A <see cref="UniqueKillSwitch" /> is always a result of a materialization (unlike <see cref="SharedKillSwitch" />
///     which is constructed
///     before any materialization) and it always controls that graph and stage which yielded the materialized value.
///     After calling <see cref="Shutdown" /> the running instance of the <see cref="IGraph{TShape}" /> of
///     <see cref="FlowShape{TIn,TOut}" /> that materialized to the
///     <see cref="UniqueKillSwitch" /> will complete its downstream and cancel its upstream (unless if finished or failed
///     already in which
///     case the command is ignored). Subsequent invocations of completion commands will be ignored.
///     After calling <see cref="Abort" /> the running instance of the <see cref="IGraph{TShape}" /> of
///     <see cref="FlowShape{TIn,TOut}" /> that materialized to the
///     <see cref="UniqueKillSwitch" /> will fail its downstream with the provided exception and cancel its upstream
///     (unless if finished or failed already in which case the command is ignored). Subsequent invocations of completion
///     commands will be ignored.
///     It is also possible to individually cancel, complete or fail upstream and downstream parts by calling the
...
public sealed class UniqueKillSwitch : IKillSwitch
{
    ...
    {
        _promise.TrySetResult(NotUsed.Instance);
    }
    ...
    public override string ToString()
    {
        return $"SingleKillSwitch({GetHashCode()})";
    }
}

/// <summary>
///     A <see cref="SharedKillSwitch" /> is a provider for <see cref="IGraph{TShape}" />s of
///     <see cref="FlowShape{TIn,TOut}" /> that can be completed or failed from the outside.
///     A <see cref="IGraph{TShape}" /> returned by the switch can be materialized arbitrary amount of times: every newly
///     materialized<see cref="IGraph{TShape}" />
///     belongs to the switch from which it was acquired. Multiple <see cref="SharedKillSwitch" /> instances are isolated
///     from each other,
///     shutting down or aborting on instance does not affect the <see cref="IGraph{TShape}" />s provided by another
///     instance.
///     After calling <see cref="Shutdown" /> all materialized, running instances of all <see cref="IGraph{TShape}" />s
///     provided by the
///     <see cref="SharedKillSwitch" /> will complete their downstreams and cancel their upstreams(unless if finished or
///     failed already in which
///     case the command is ignored). Subsequent invocations of <see cref="Shutdown" /> and <see cref="Abort" /> will be
///     ignored.
///     After calling <see cref="Abort" /> all materialized, running instances of all <see cref="IGraph{TShape}" />s
///     provided by the
...
///     The <see cref="IGraph{TShape}" />s provided by the <see cref="SharedKillSwitch" /> do not modify the passed through
///     elements in any way or affect
///     backpressure in the stream. All provided <see cref="IGraph{TShape}" />s provide the parent
///     <see cref="SharedKillSwitch" /> as materialized value.
///     This class is thread-safe, the instance can be passed safely among threads and its methods may be invoked
///     concurrently.
/// </summary>
public sealed class SharedKillSwitch : IKillSwitch
{
    ...
    public void Shutdown()
    {
        _shutdownPromise.TrySetResult(NotUsed.Instance);
    }
    ...
    ///     (unless it finished or failed already in which case the command is ignored). Subsequent invocations of
    ///     <see cref="Shutdown" /> and <see cref="Abort" /> will be ignored.
    ///     These provided <see cref="IGraph{TShape}" />s materialize to their owning switch. This might make certain
    ///     integrations simpler than
    ...
    public void Abort(Exception cause)
    {
        _shutdownPromise.TrySetException(cause);
    }

    /// <summary>
    ///     Returns a typed Flow of a requested type that will be linked to this <see cref="SharedKillSwitch" /> instance. By
    ///     invoking
    ///     <see cref="Shutdown" /> or <see cref="Abort" /> all running instances of all provided <see cref="IGraph{TShape}" />
    ///     s by this
    ///     switch will be stopped normally or failed.
    /// </summary>
    /// <returns>
    ///     A reusable <see cref="IGraph{TShape}" /> that is linked with the switch. The materialized value provided is
    ///     this switch itself.
    /// </returns>
    ...
    {
        return new SharedKillSwitchFlow<T>(this);
    }
        ...
        GraphStageWithMaterializedValue<FlowShape<T, T>, SharedKillSwitch>
    {
        ...
        public override ILogicAndMaterializedValue<SharedKillSwitch> CreateLogicAndMaterializedValue(
            Attributes inheritedAttributes)
        {
            return new LogicAndMaterializedValue<SharedKillSwitch>(new Logic(this), _killSwitch);
        }

        public override string ToString()
        {
            return $"Flow({_killSwitch})";
        }
        ...
        private sealed class Logic : KillSwitches.KillableGraphStageLogic
        {
            ...
            public override void OnPush()
            {
                Push(_killSwitchFlow.Out, Grab(_killSwitchFlow.In));
            }

            public override void OnPull()
            {
                Pull(_killSwitchFlow.In);
            }
        }
