...
public class TerminatingChildrenContainer : ChildrenContainerBase
{
    ...
    public TerminatingChildrenContainer(IImmutableDictionary<string, IChildStats> children, IActorRef toDie,
    ...
    /// <param name="toDie">TBD</param>
    /// <param name="reason">TBD</param>
    public TerminatingChildrenContainer(IImmutableDictionary<string, IChildStats> children,
        ImmutableHashSet<IActorRef> toDie, SuspendReason reason)
        : base(children)
    {
        ToDie = toDie;
        Reason = reason;
    ...
    ///     TBD
    /// </summary>
    public override bool IsTerminating => Reason is SuspendReason.Termination;

    /// <summary>
    ///     TBD
    /// </summary>
    public override bool IsNormal => Reason is SuspendReason.UserRequest;
    ...
    ///     This exception is thrown if the given <paramref name="name" /> belongs to
    ///     an actor that is terminating.
    /// </exception>
    /// <exception cref="InvalidActorNameException">
    ///     This exception is thrown if the given <paramref name="name" /> is not
    ///     unique in the container.
    /// </exception>
    /// <returns>TBD</returns>
    public override IChildrenContainer Reserve(string name)
    {
        if (Reason is SuspendReason.Termination)
        ...
        return new TerminatingChildrenContainer(InternalChildren.SetItem(name, ChildNameReserved.Instance), ToDie,
            Reason);
    }
