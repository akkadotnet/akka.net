...
{
    public static readonly ORMultiValueDictionary<TKey, TValue> Empty = new(ORDictionary<TKey, ORSet<TValue>>.Empty,
        false);

    public static readonly ORMultiValueDictionary<TKey, TValue> EmptyWithValueDeltas =
        new(ORDictionary<TKey, ORSet<TValue>>.Empty, true);
    ...
    public ORMultiValueDictionary<TKey, TValue> Merge(ORMultiValueDictionary<TKey, TValue> other)
    {
        if (DeltaValues == other.DeltaValues)
            return DeltaValues
                ? new ORMultiValueDictionary<TKey, TValue>(Underlying.MergeRetainingDeletedValues(other.Underlying),
                    DeltaValues)
    ...
    {
        return Merge((ORMultiValueDictionary<TKey, TValue>)other);
    }

    public IEnumerator<KeyValuePair<TKey, IImmutableSet<TValue>>> GetEnumerator()
    {
        return Underlying.Select(x => new KeyValuePair<TKey, IImmutableSet<TValue>>(x.Key, x.Value.Elements))
            .GetEnumerator();
    }
    ...
    {
        return Underlying.NeedPruningFrom(removedNode);
    }
    ...
    {
        return new ORMultiValueDictionary<TKey, TValue>(Underlying.Prune(removedNode, collapseInto), DeltaValues);
    }

    public ORMultiValueDictionary<TKey, TValue> PruningCleanup(UniqueAddress removedNode)
    {
        return new ORMultiValueDictionary<TKey, TValue>(Underlying.PruningCleanup(removedNode), DeltaValues);
    }
    ...
    {
        return Underlying.KeySet.Contains(key);
    }
    ...
    {
        return SetItems(node.SelfUniqueAddress, key, bucket);
    }
    ...
    {
        return AddItem(node.SelfUniqueAddress, key, element);
    }
    ...
    {
        var newUnderlying =
            Underlying.AddOrUpdate(node, key, ORSet<TValue>.Empty, DeltaValues, x => x.Add(node, element));
        return new ORMultiValueDictionary<TKey, TValue>(newUnderlying, DeltaValues);
    }
    ...
    {
        return RemoveItem(node.SelfUniqueAddress, key, element);
    }
    ...
    {
        var newUnderlying =
            Underlying.AddOrUpdate(node, key, ORSet<TValue>.Empty, DeltaValues, set => set.Remove(node, element));
    ...
    /// <summary>
    ///     Replace an element of a set associated with a key with a new one if it is different. This is useful when an element
    ///     is removed
    ...
    public ORMultiValueDictionary<TKey, TValue> ReplaceItem(Cluster.Cluster node, TKey key, TValue oldElement,
        TValue newElement)
    {
        return ReplaceItem(node.SelfUniqueAddress, key, oldElement, newElement);
    }
    ...
    ///     to retain history for replicated data.
    /// </summary>
    public ORMultiValueDictionary<TKey, TValue> ReplaceItem(UniqueAddress node, TKey key, TValue oldElement,
        TValue newElement)
    {
        return !Equals(newElement, oldElement)
    ...
    {
        return obj is ORMultiValueDictionary<TKey, TValue> pairs && Equals(pairs);
    }
    ...
    public override string ToString()
    {
        ...
        foreach (var entry in Entries)
        {
            ...
            foreach (var value in entry.Value) sb.Append(value).Append(", ");
            sb.Append("], ");
        }
    ...
    internal sealed class ORMultiValueDictionaryDelta : ORDictionary<TKey, ORSet<TValue>>.IDeltaOperation,
        IReplicatedDeltaSize, IORMultiValueDictionaryDeltaOperation
    {
        ...
        ORDictionary.IDeltaOperation IORMultiValueDictionaryDeltaOperation.Underlying =>
            (ORDictionary.IDeltaOperation)Underlying;
        ...
        public IReplicatedData Merge(IReplicatedData other)
        {
            if (other is ORMultiValueDictionaryDelta d)
                return new ORMultiValueDictionaryDelta(
                    (ORDictionary<TKey, ORSet<TValue>>.IDeltaOperation)Underlying.Merge(d.Underlying),
                    WithValueDeltas || d.WithValueDeltas);

            return new ORMultiValueDictionaryDelta(
                (ORDictionary<TKey, ORSet<TValue>>.IDeltaOperation)Underlying.Merge(other), WithValueDeltas);
        }

        public IDeltaReplicatedData Zero => WithValueDeltas ? EmptyWithValueDeltas : Empty;
        ...
        public bool Equals(ORDictionary<TKey, ORSet<TValue>>.IDeltaOperation other)
        {
            if (other is ORDictionary<TKey, ORSet<TValue>>.DeltaGroup group)
            {
                ...
                if (group.Operations.Length == 1) return Underlying.Equals(group.Operations.First());
        ...
        public override bool Equals(object obj)
        {
            return obj is ORMultiValueDictionaryDelta operation &&
    ...
    public ORDictionary<TKey, ORSet<TValue>>.IDeltaOperation Delta =>
        new ORMultiValueDictionaryDelta(Underlying.Delta, DeltaValues);
    ...
    public ORMultiValueDictionary<TKey, TValue> MergeDelta(ORDictionary<TKey, ORSet<TValue>>.IDeltaOperation delta)
    {
        ...
        if (DeltaValues)
            return new ORMultiValueDictionary<TKey, TValue>(Underlying.MergeDeltaRetainingDeletedValues(delta),
                DeltaValues);
    ...
    {
        return new ORMultiValueDictionary<TKey, TValue>(Underlying.ResetDelta(), DeltaValues);
    }
