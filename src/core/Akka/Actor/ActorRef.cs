    ...
    protected override void TellInternal(object message, IActorRef sender)
    {
        ...
        switch (message)
        {
            case ISystemMessage msg:
                handled = _result.TrySetException(new InvalidOperationException(
                    $"system message of type '{msg.GetType().Name}' is invalid for {nameof(FutureActorRef<T>)}"));
                break;
            case T t:
            ...
            case Status.Failure f:
                handled = _result.TrySetException(f.Cause
                                                  ?? new TaskCanceledException(
                                                      "Task cancelled by actor via Failure message."));
                break;
...
/// <summary>
///     An actor reference. Acts as a handle to an actor. Used to send messages to an actor, whether an actor is local or
///     remote.
...
///     If you want to be notified about an actor terminating, call
///     <see cref="ICanWatch.Watch(IActorRef)">IActorContext.Watch</see>
...
public abstract class ActorRefBase : IActorRef
{
    ...
    {
        if (sender == null) sender = ActorRefs.NoSender;
    ...
    public int CompareTo(object obj)
    {
        ...
        if (!(obj is IActorRef other))
            throw new ArgumentException($"Object must be of type IActorRef, found {obj.GetType()} instead.",
                nameof(obj));
...
public interface IInternalActorRef : IActorRef, IActorRefScope
{
    ...
    /// <summary>
    ///     Obsolete. Use <see cref="Watch" /> or
    ///     <see cref="ReceiveActor.Receive{T}(Action{T}, Predicate{T})">Receive&lt;<see cref="Akka.Actor.Terminated" />&gt;</see>
    /// </summary>
...
public abstract class InternalActorRefBase : ActorRefBase, IInternalActorRef
{
    ...
    /// <inheritdoc cref="IInternalActorRef" />
    public abstract IActorRef
        GetChild(IReadOnlyList<string> name); //TODO: Refactor this to use an IEnumerator instead as this will be faster instead of enumerating multiple times over name, as the implementations currently do.
...
public abstract class MinimalActorRef : InternalActorRefBase, ILocalRef
{
    ...
    public override IInternalActorRef Parent => ActorRefs.Nobody;
...
internal sealed class IgnoreActorRef : MinimalActorRef
{
    ...
    {
        return StaticPath.Equals(otherPath);
    }
...
public sealed class Nobody : MinimalActorRef
{
    ...
    /// </exception>
    public override IActorRefProvider Provider => throw new NotSupportedException("Nobody does not provide");
...
internal class VirtualPathContainer : MinimalActorRef
{
    ...
    public VirtualPathContainer(IActorRefProvider provider, ActorPath path, IInternalActorRef parent,
        ILoggingAdapter log)
    {
        _parent = parent;
        Log = log;
    ...
    ///     <c>false</c> otherwise.
    /// </summary>
    public bool HasChildren => !_children.IsEmpty;
    ...
    public void RemoveChild(string name)
    {
        ...
        if (!_children.TryRemove(name, out tmp)) Log.Warning("{0} trying to remove non-child {1}", Path, name);
    }
...
*/
    ...
    {
        foreach (var child in _children.Values) action(child);
    }
...
///     by calling <see cref="ActorCell.AddFunctionRef(Action{IActorRef, object}, string)" /> and must be deregistered when
///     no longer
...
internal sealed class FunctionRef : MinimalActorRef
{
    ...
    public FunctionRef(ActorPath path, IActorRefProvider provider, EventStream eventStream,
        Action<IActorRef, object> tell)
    {
    ...
    {
        return _watching.Contains(actorRef);
    }
    ...
    public override void SendSystemMessage(ISystemMessage message)
    {
        switch (message)
        {
            ...
            case DeathWatchNotification deathWatch:
                Tell(new Terminated(deathWatch.Actor, true, false), deathWatch.Actor);
                break;
        }
    }
    ...
    private void SendTerminated(IActorRef watcher)
    {
        if (watcher is IInternalActorRef scope)
            scope.SendSystemMessage(new DeathWatchNotification(this, true, false));
    }
    ...
    private void AddWatcher(IInternalActorRef watchee, IInternalActorRef watcher)
    {
        while (true)
        {
            ...
            else
            {
                ...
                {
                    if (!watchedBy.Contains(watcher) && !ReferenceEquals(watchedBy,
                            Interlocked.CompareExchange(ref _watchedBy, watchedBy.Add(watcher), watchedBy))) continue;
                }
                else if (!watcheeSelf && watcherSelf)
                {
                    Publish(new Warning(Path.ToString(), typeof(FunctionRef),
                        $"Externally triggered watch from {watcher} to {watchee} is illegal on FunctionRef"));
                }
                else
                {
                    Publish(new Warning(Path.ToString(), typeof(FunctionRef),
                        $"BUG: illegal Watch({watchee},{watcher}) for {this}"));
                }
            }
    ...
    private void RemoveWatcher(IInternalActorRef watchee, IInternalActorRef watcher)
    {
        while (true)
        {
            ...
            else
            {
                ...
                {
                    if (!watchedBy.Contains(watcher) && !ReferenceEquals(watchedBy,
                            Interlocked.CompareExchange(ref _watchedBy, watchedBy.Remove(watcher), watchedBy)))
                        continue;
                }
                else if (!watcheeSelf && watcherSelf)
                {
