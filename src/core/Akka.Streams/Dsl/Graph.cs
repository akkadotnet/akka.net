...
///     A <see cref="MergePreferred{T}" /> has one <see cref="Out" /> port, one <see cref="Preferred" /> input port and 0
///     or more secondary <see cref="In" /> ports.
///     <para>
...
public sealed class MergePreferred<T> : GraphStage<MergePreferred<T>.MergePreferredShape>
{
    ...
    {
        if (secondaryPorts < 1)
            throw new ArgumentException("A MergePreferred must have at least one secondary port",
                nameof(secondaryPorts));
    ...
    private sealed class Logic : InGraphStageLogic
    {
        ...
        {
            for (var i = 0; i < _stage._secondaryPorts; i++)
            {
...
///     A <see cref="MergePrioritized{T}" /> has one <see cref="MergePrioritized{T}.Out" /> port, one or more input port
...
public sealed class MergePrioritized<T> : GraphStage<UniformFanInShape<T, T>>
{
    ...
    protected override GraphStageLogic CreateLogic(Attributes inheritedAttributes)
    {
        return new MergePrioritizedLogic(this);
    }
...
public sealed class Interleave<TIn, TOut> : GraphStage<UniformFanInShape<TIn, TOut>> where TIn : TOut
{
    ...
    {
        if (inputPorts <= 1)
            throw new ArgumentException("Interleave input ports count must be greater than 1", nameof(inputPorts));
        if (segmentSize <= 0)
            throw new ArgumentException("Interleave segment size must be greater than 0", nameof(segmentSize));
...
public sealed class MergeSorted<T> : GraphStage<FanInShape<T, T, T>>
{
    ...
    private sealed class Logic : GraphStageLogic
    {
        ...
        public Logic(MergeSorted<T> stage) : base(stage.Shape)
        {
            ...
            void DispatchRight(T right)
            {
                Dispatch(_other, right);
            }

            void DispatchLeft(T left)
            {
                Dispatch(left, _other);
            }

            void PassRight()
            {
                Emit(_stage.Out, _other, () =>
                {
                    NullOut();
...
public sealed class Broadcast<T> : GraphStage<UniformFanOutShape<T, T>>
{
    ...
    {
        if (outputPorts < 1)
            throw new ArgumentException("A Broadcast must have one or more output ports", nameof(outputPorts));
...
public sealed class Balance<T> : GraphStage<UniformFanOutShape<T, T>>
{
    ...
    {
        if (outputPorts < 1)
            throw new ArgumentException("A Balance must have one or more output ports", nameof(outputPorts));
        _outputPorts = outputPorts;
...
public static class ZipWithN
{
    ...
    public static IGraph<UniformFanInShape<TIn, TOut>> Create<TIn, TOut>(Func<IImmutableList<TIn>, TOut> zipper,
        int n)
    {
        return new ZipWithN<TIn, TOut>(zipper, n);
    }
}
...
public class Concat<TIn, TOut> : GraphStage<UniformFanInShape<TIn, TOut>> where TIn : TOut
{
    ...
    {
        if (inputPorts <= 1)
            throw new ArgumentException("A Concat must have more than 1 input port", nameof(inputPorts));
        _inputPorts = inputPorts;
                ...
                SetHandler(i,
                    ...
                    () =>
                    {
                        if (idx == _activeStream)
                        {
                            _activeStream++;
                            // skip closed inputs
                            while (_activeStream < stage._inputPorts && IsClosed(stage.In(_activeStream)))
                                _activeStream++;
...
///     '''Emits when''' element is available from primary stream or the primary stream closed without emitting any
    ...
    private sealed class Logic : InAndOutGraphStageLogic
    {
        ...
        public Logic(OrElse<T> stage) : base(stage.Shape)
        {
            ...
            SetHandler(stage.Secondary,
                () => Push(stage.Out, Grab(stage.Secondary)),
                () =>
                {
...
///     </para>
///     <para>
///         Emits when element is available and demand exists from the 'main' output; the element will also be sent to
///         the 'tap' output if there is demand.
///     </para>
...
public sealed class WireTap<T> : GraphStage<FanOutShape<T, T, T>>
{
    ...
    public WireTap()
    {
        Shape = new FanOutShape<T, T, T>(In, OutMain, OutTap);
    }
