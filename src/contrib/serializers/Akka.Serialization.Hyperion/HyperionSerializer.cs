        ...
        : base(system)
    {
        ...
        _serializer =
            new HySerializer(new SerializerOptions(
                ...
                surrogates,
                null,
                provider.GetKnownTypes(),
                true,
                settings.PackageNameOverrides,
                settings.DisallowUnsafeType,
                settings.TypeFilter));
    }
...
public sealed class HyperionSerializerSettings
{
    ...
    /// <param name="knownTypesProvider">
    ///     Type implementing <see cref="IKnownTypesProvider" /> to be used to determine a list of
    ///     types implicitly known by all cooperating serializer.
    /// </param>
    ...
    public HyperionSerializerSettings(bool preserveObjectReferences, bool versionTolerance, Type knownTypesProvider)
        : this(preserveObjectReferences, versionTolerance, knownTypesProvider, new List<Func<string, string>>(),
            Array.Empty<Surrogate>(), true, DisabledTypeFilter.Instance)
    {
    }
    ...
    public HyperionSerializerSettings(
        ...
        : this(preserveObjectReferences, versionTolerance, knownTypesProvider, packageNameOverrides,
            Array.Empty<Surrogate>(), true, DisabledTypeFilter.Instance)
    {
    }
    ...
    ///     A list of Surrogate instances that are used to de/serialize complex objects into a much
    ///     simpler serialized objects.
    /// </param>
    /// <exception>
    ///     ArgumentException: Raised when `known-types-provider` type doesn't implement
    ///     <see cref="IKnownTypesProvider" /> interface.
    /// </exception>
    public HyperionSerializerSettings(
        bool preserveObjectReferences,
        bool versionTolerance,
    ...
    public HyperionSerializerSettings(
        ...
        : this(preserveObjectReferences, versionTolerance, knownTypesProvider, packageNameOverrides, surrogates,
            disallowUnsafeType, DisabledTypeFilter.Instance)
    {
    }
        ...
        ITypeFilter typeFilter)
    {
        ...
        if (!typeof(IKnownTypesProvider).IsAssignableFrom(knownTypesProvider))
            throw new ArgumentException(
                $"Known types provider must implement an interface {typeof(IKnownTypesProvider).FullName}");
    ...
    public static HyperionSerializerSettings Create(Config config)
    {
        if (config.IsNullOrEmpty())
            throw ConfigurationException.NullOrEmptyConfig<HyperionSerializerSettings>(
        ...
        foreach (var surrogateClass in surrogateList)
        {
            ...
            if (surrogateType == null)
                throw new ConfigurationException(
                    $"The surrogate type name did not resolve to an actual Type: '{surrogateClass}'");
        ...
        return new HyperionSerializerSettings(
            config.GetBoolean("preserve-object-references", true),
            config.GetBoolean("version-tolerance", true),
            type,
            packageNameOverrides,
            surrogates,
            config.GetBoolean("disallow-unsafe-type", true),
            typeFilter);
    }
