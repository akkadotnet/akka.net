...
namespace Akka.Remote.Tests
{
    ...
    public class FailureDetectorRegistrySpec : AkkaSpec
    {
        ...
        public void FailureDetectorRegistry_must_mark_node_as_available_after_a_series_of_successful_heartbeats()
        {
            ...
            fd.Heartbeat("resource1");
            fd.Heartbeat("resource1");
            Assert.IsTrue(fd.IsAvailable("resource1"));
        }
        ...
        public void FailureDetectorRegistry_must_mark_node_as_dead_if_heartbeats_are_missed()
        {
            ...
            Assert.IsTrue(fd.IsAvailable("resource1")); //1200
            fd.Heartbeat("resource2"); //5200, but unrelated resouce
            Assert.IsFalse(fd.IsAvailable("resource1"));
        }
            ...
            ()
        {
            ...
            Assert.IsFalse(fd.IsMonitoring("resource1"));
            fd.Heartbeat("resource1"); //0
            fd.Heartbeat("resource1"); //1000
            fd.Heartbeat("resource1"); //1100
            Assert.IsTrue(fd.IsAvailable("resource1")); //2200
            Assert.IsTrue(fd.IsMonitoring("resource1"));

            fd.Remove("resource1");

            Assert.IsFalse(fd.IsMonitoring("resource1"));
            Assert.IsTrue(fd.IsAvailable("resource1")); //3300
            ...
            Assert.IsTrue(fd.IsAvailable("resource1")); //6700
            Assert.IsTrue(fd.IsMonitoring("resource1"));
        }
    }
}
