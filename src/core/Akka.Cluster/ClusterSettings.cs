    ...
    public ClusterSettings(Config config, string systemName)
    {
        ...
        if (clusterConfig?.GetConfig("failure-detector") == null)
            throw new ConfigurationException(
                $"Failed to instantiate {nameof(ClusterSettings)}: Configuration does not contain `akka.cluster` node. Did you forgot to set the 'akka.cluster.provider' HOCON property to 'cluster'?");

        LogInfoVerbose = clusterConfig.GetBoolean("log-info-verbose");
        LogInfo = LogInfoVerbose || clusterConfig.GetBoolean("log-info");
        FailureDetectorConfig = clusterConfig.GetConfig("failure-detector");
        FailureDetectorImplementationClass = FailureDetectorConfig.GetString("implementation-class");
        HeartbeatInterval = FailureDetectorConfig.GetTimeSpan("heartbeat-interval");
        HeartbeatExpectedResponseAfter = FailureDetectorConfig.GetTimeSpan("expected-response-after");
        MonitoredByNrOfMembers = FailureDetectorConfig.GetInt("monitored-by-nr-of-members");
        ...
        ShutdownAfterUnsuccessfulJoinSeedNodes =
            clusterConfig.GetTimeSpanWithOffSwitch("shutdown-after-unsuccessful-join-seed-nodes");
...
#pragma warning restore CS0618
        ...
        AppVersion = AppVersion.Create(clusterConfig.GetString("app-version"));

        MinNrOfMembers = clusterConfig.GetInt("min-nr-of-members");

        UseDispatcher = clusterConfig.GetString("use-dispatcher");
...
#pragma warning restore CS0618
                        ...
                        .ToLowerInvariant())
            {
                ...
                default:
                    ...
                    if (!(val > TimeSpan.Zero))
                        throw new ConfigurationException(
                            $"Valid settings for [akka.cluster.{cKey}] are 'off', 'on', or a timespan greater than 0s. Received [{val}]");
                    return val;
            }
        }
        ...
        UseLegacyHeartbeatMessage = clusterConfig.GetBoolean("use-legacy-heartbeat-message");
    }

    /// <summary>
    ///     Determine whether to log verbose <see cref="Akka.Event.LogLevel.InfoLevel" /> messages for temporary
    ///     troubleshooting.
    /// </summary>
    ...
    /// <summary>
    ///     If this is set to "off", the leader will not move <see cref="MemberStatus.Joining" /> members to
    ///     <see cref="MemberStatus.Up" /> during a network
    ///     split. This feature allows the leader to accept <see cref="MemberStatus.Joining" /> members to be
    ///     <see cref="MemberStatus.WeaklyUp" />
    ///     so they become part of the cluster even during a network split.
    ///     The leader will move <see cref="MemberStatus.WeaklyUp" /> members to <see cref="MemberStatus.Up" /> status once
