...
{
    /// <summary>
    ///     Creates a <see cref="SourceWithContext" /> from a regular source that operates on a tuple of `(data, context)`
    ///     elements.
    /// </summary>
    public static SourceWithContext<TOut, TCtxOut, TMat> FromTuples<TOut, TCtxOut, TMat>(
        Source<(TOut, TCtxOut), TMat> source)
    {
        return new SourceWithContext<TOut, TCtxOut, TMat>(source);
    }
}
...
public sealed class SourceWithContext<TOut, TCtx, TMat> : GraphDelegate<SourceShape<(TOut, TCtx)>, TMat>
{
    ...
    /// </summary>
    public SourceWithContext<TOut2, TCtx2, TMat> Via<TOut2, TCtx2, TMat2>(
        IGraph<FlowShape<(TOut, TCtx), (TOut2, TCtx2)>, TMat2> viaFlow)
    {
        return new SourceWithContext<TOut2, TCtx2, TMat>(Source.FromGraph(Inner).Via(viaFlow));
    }
    ...
    {
        return new SourceWithContext<TOut2, TCtx2, TMat3>(Source.FromGraph(Inner).ViaMaterialized(viaFlow, combine));
    }
    ...
    {
        return Source.FromGraph(Inner).ToMaterialized(sink, Keep.Left);
    }
    ...
    ///     concatenating the processing steps of both.
    /// </summary>
    public IRunnableGraph<TMat3> ToMaterialized<TMat2, TMat3>(IGraph<SinkShape<(TOut, TCtx)>, TMat2> sink,
        Func<TMat, TMat2, TMat3> combine)
    {
        return Source.FromGraph(Inner).ToMaterialized(sink, combine);
    }
    ...
    {
        return new SourceWithContext<TOut, TCtx, TMat2>(Source.FromGraph(Inner).MapMaterializedValue(combine));
    }

    /// <summary>
    ///     Connect this <see cref="SourceWithContext{TOut,TCtx,TMat}" /> to a Sink and run it. The returned value is the
    ///     materialized value of the Sink.
    ///     Note that the ActorSystem can be used as the implicit materializer parameter to use the SystemMaterializer for
    ///     running the stream.
    /// </summary>
    ...
    {
        return Source.FromGraph(Inner).RunWith(sink, materializer);
    }
    ...
    {
        return Inner is Source<(TOut, TCtx), TMat> source ? source : Source.FromGraph(Inner);
    }
}