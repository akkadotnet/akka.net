...
{
    /// <summary>
    ///     By default derivatives of this plugin will be applied to all actors inheriting from
    ///     <typeparamref name="TActor">actor generic type</typeparamref>.
    /// </summary>
    /// <param name="actorType">TBD</param>
    /// <returns>TBD</returns>
    public virtual bool CanBeAppliedTo(Type actorType)
...
public class ActorProducerPipelineResolver
{
    ...
    /// </summary>
    public int TotalPluginCount => _plugins.Count;
...
public class ActorProducerPipeline : IEnumerable<IActorProducerPlugin>
{
    ...
    public void AfterActorIncarnated(ActorBase actor, IActorContext context)
    {
        foreach (var plugin in _plugins)
            ...
            {
                const string fmt =
                    "An exception occurred while trying to apply plugin of type {0} to the newly created actor (Type = {1}, Path = {2})";
    ...
    public void BeforeActorIncarnated(ActorBase actor, IActorContext context)
    {
        foreach (var plugin in _plugins)
            ...
            {
                const string fmt =
                    "An exception occurred while trying to apply plugin of type {0} to the actor before it's destruction (Type = {1}, Path = {2})";
                LogException(actor, cause, fmt, plugin);
            }
    }
