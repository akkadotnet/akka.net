...
namespace Akka.Pattern
{
    ...
    public abstract class BackoffOptions
    {
        ...
        /// <summary>
        /// Returns a new <see cref="BackoffOptions"/> with a constant reply to messages that the supervisor receives while its child is stopped. By default, a message received while the child is stopped is forwarded to `deadLetters`. With this option, the supervisor will reply to the sender instead.
        /// </summary>
    ...
    internal sealed class BackoffOptionsImpl : BackoffOptions
    {
        ...
        public BackoffOptionsImpl(IBackoffType backoffType, Props childProps, string childName, TimeSpan minBackoff, TimeSpan maxBackoff, double randomFactor, IBackoffReset reset = null) 
            : this(backoffType, childProps, childName, minBackoff, maxBackoff, randomFactor, reset, new OneForOneStrategy(SupervisorStrategy.DefaultDecider), null)
        {
        }

        public BackoffOptionsImpl(IBackoffType backoffType, Props childProps, string childName, TimeSpan minBackoff, TimeSpan maxBackoff, double randomFactor, IBackoffReset reset, OneForOneStrategy strategy, object replyWhileStopped)
        {
        ...
        {
            return new BackoffOptionsImpl(_backoffType, _childProps, _childName, _minBackoff, _maxBackoff, _randomFactor, _reset, SupervisorStrategy.StoppingStrategy, _replyWhileStopped);
        }
        ...
        internal override Props Props
        {
            get
            {
                ...
                {
                    return Props.Create(
                         () => new BackoffOnRestartSupervisor(_childProps, _childName, _minBackoff, _maxBackoff, _reset, _randomFactor, _strategy, _replyWhileStopped));
                }
                else if (_backoffType is StopImpliesFailure)
                {
                    return Props.Create(
                        () => new BackoffSupervisor(_childProps, _childName, _minBackoff, _maxBackoff, _reset, _randomFactor, _strategy, _replyWhileStopped));
                }
                else
                {
                    return Props.Empty;
