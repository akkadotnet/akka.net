        ...
        switch (message)
        {
            ...
            case SaveSnapshotFailure failure:
            {
                if (Log.IsWarningEnabled)
                    Log.Warning("Failed to SaveSnapshot given metadata [{0}] due to: [{1}: {2}]", failure.Metadata,
                        failure.Cause, failure.Cause.Message);
                break;
            }
            case DeleteSnapshotFailure failure:
            {
                if (Log.IsWarningEnabled)
                    Log.Warning("Failed to DeleteSnapshot given metadata [{0}] due to: [{1}: {2}]", failure.Metadata,
                        failure.Cause, failure.Cause.Message);
                break;
            }
            ...
            {
                if (Log.IsWarningEnabled)
                    Log.Warning("Failed to DeleteSnapshots given criteria [{0}] due to: [{1}: {2}]", failure.Criteria,
                        failure.Cause, failure.Cause.Message);
                break;
            }
            ...
            {
                if (Log.IsWarningEnabled)
                    Log.Warning(failure.Cause,
                        "Failed to DeleteMessages ToSequenceNr [{0}] for PersistenceId [{1}] due to: {2}",
                        failure.ToSequenceNr, PersistenceId, failure.Cause.Message);
                break;
            }
        }
