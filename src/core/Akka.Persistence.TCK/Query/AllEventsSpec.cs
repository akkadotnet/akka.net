...
public class AllEventsSpec : Akka.TestKit.Xunit2.TestKit
{
    ...
    public virtual void ReadJournal_query_AllEvents_should_find_events_from_offset_exclusive()
    {
        ...
        var offs = probe1
            .ExpectNext<EventEnvelope>(p => p.PersistenceId == "a" && p.SequenceNr == 3L && p.Event.Equals("keep"))
            .Offset;
        probe1.Cancel();
        ...
        probe2.ExpectNext<EventEnvelope>(p =>
            p.PersistenceId == "a" && p.SequenceNr == 4L && p.Event.Equals("streaming"));
