...
namespace Akka.Remote.Artery
{
    ...
    // Compression advertisement fields the V2 generator has no native kind for (design.md
    // "artery-ref-manifest-compression" Decision 5): a byte table-version and an ordered
    // string-list table, each carried through the AddressFormatter-style escape hatch. Both
    // targets are compression-specific and only occur on the four advertisement/ack messages, so
    // they never touch the wire form of the other nine control messages.
    [AkkaSerializerFormatter(typeof(byte), typeof(CompressionTableVersionFormatter))]
    [AkkaSerializerFormatter(typeof(CompressionAdvertisementTable), typeof(CompressionAdvertisementTableFormatter))]
    internal sealed partial class ArteryControlMessageSerializer : MessagePackSerializer<IArteryControlMessage>
    {
        ...
        /// <summary>
        /// The manifest for <see cref="ActorRefCompressionAdvertisement"/> (design.md
        /// "artery-ref-manifest-compression", actor-ref table advertisement).
        /// </summary>
        public const string ActorRefCompressionAdvertisementManifest = "CRA";

        /// <summary>
        /// The manifest for <see cref="ActorRefCompressionAdvertisementAck"/>.
        /// </summary>
        public const string ActorRefCompressionAdvertisementAckManifest = "CRK";

        /// <summary>
        /// The manifest for <see cref="ClassManifestCompressionAdvertisement"/> (class-manifest table advertisement).
        /// </summary>
        public const string ClassManifestCompressionAdvertisementManifest = "CMA";

        /// <summary>
        /// The manifest for <see cref="ClassManifestCompressionAdvertisementAck"/>.
        /// </summary>
        public const string ClassManifestCompressionAdvertisementAckManifest = "CMK";
