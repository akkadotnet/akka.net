...
public class ActorsLeakSpec : AkkaSpec
{
    ...
    [Fact(Skip =
        "EventFilter can receive 1-2 notifications about nodes shutting down depending on timing, which makes this spec racy")]
    public async Task Remoting_must_not_leak_actors()
    {
        ...
        Sys.EventStream.Publish(new Mute(new WarningFilter(
            new ContainsString("Association with remote system akka.trttl.tcp://remote@127.0.0.1:2553 has failed"))));
        ...
        for (var i = 1; i <= 3; i++)
        {
            ...
            try
            {
                ...
                Assert.True(await RARP.For(Sys).Provider.Transport
                    .ManagementCommand(new ForceDisassociate(remoteAddress))
        ...
        try
        {
            ...
            // This will make sure that no SHUTDOWN message gets through
            Assert.True(await RARP.For(Sys).Provider.Transport
                .ManagementCommand(new ForceDisassociate(idleRemoteAddress))
                .AwaitWithTimeout(TimeSpan.FromSeconds(3)));
        }
        finally
        {
    ...
    private class StoppableActor : ReceiveActor
    {
        ...
        {
            Receive<string>(str => str.Equals("stop"), _ => { Context.Stop(Self); });
        }
    }
}