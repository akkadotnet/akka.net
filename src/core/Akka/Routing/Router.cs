...
internal class NoRoutee : Routee
{
    ...
    {
        if (sender is LocalActorRef localActorRef) localActorRef.Provider.DeadLetters.Tell(message);
    }
}
...
public class SeveralRoutees : Routee
{
    ...
    public override void Send(object message, IActorRef sender)
    {
        foreach (var routee in _routees) routee.Send(message, sender);
    }
}
...
public class Router
{
    ...
    /// </summary>
    public IEnumerable<Routee> Routees => _routees;
    ...
    {
        if (message is RouterEnvelope routerEnvelope) return routerEnvelope.Message;
    ...
    public virtual void Route(object message, IActorRef sender)
    {
        if (message is Broadcast)
        {
            ...
            foreach (var r in _routees) r.Send(unwrapped, sender);
        }
        else
        {
