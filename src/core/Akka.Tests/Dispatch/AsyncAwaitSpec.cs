...
internal class ReceiveTimeoutAsyncActor : ReceiveActor
{
    private IActorRef _replyTo;

    public ReceiveTimeoutAsyncActor()
    {
        Receive<ReceiveTimeout>(_ => { _replyTo.Tell("GotIt"); });
...
internal class AsyncActor : ReceiveActor
{
    public AsyncActor()
    {
...
public class ActorAsyncAwaitSpec : AkkaSpec
{
    ...
    public async Task Actors_should_be_able_to_block_ask_message_loop()
    {
        ...
        var asker = Sys.ActorOf(
            Props.Create(() => new BlockingAsker(actor)).WithDispatcher("akka.actor.task-dispatcher"), "Asker");
        var task = asker.Ask("start", TimeSpan.FromSeconds(5));
        actor.Tell(123, ActorRefs.NoSender);
        var res = await task;
        Assert.Equal("done", res);
...
#pragma warning disable AK1003
            ReceiveAsync<string>(async _ => { ThrowException(); });
