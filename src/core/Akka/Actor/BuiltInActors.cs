    ...
    protected override bool Receive(object message)
    {
        ...
        if (terminated != null)
        {
            ...
            if (_userGuardian.Equals(terminatedActor))
            {
                ...
                foreach (var terminationHook in _terminationHooks) terminationHook.Tell(TerminationHook.Instance);
...
public static class WrappedMessage
{
    ...
    {
        while (message is IWrappedMessage wm) message = wm.Message;
        return message;
    }
}
    ...
    protected override bool SpecialHandle(object message, IActorRef sender)
    {
        if (message is Watch w)
        {
            if (!w.Watchee.Equals(this) && !w.Watcher.Equals(this))
                w.Watcher.SendSystemMessage(new DeathWatchNotification(w.Watchee, false, false));
            return true;
        }
