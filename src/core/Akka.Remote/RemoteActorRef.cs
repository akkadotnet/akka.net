...
#pragma warning restore CS0809
    ...
    public override IActorRef GetChild(IReadOnlyList<string> name)
    {
        switch (name.FirstOrDefault())
        {
            ...
            default:
                return new RemoteActorRef(Remote, LocalAddressToUse, Path / name, ActorRefs.Nobody, Props.None,
                    Deploy.None);
        }
    }
    ...
    {
        Remote.System.EventStream.Publish(new Error(ex, Path.ToString(), GetType(),
    ...
    /// <summary>
    ///     Determine if a <see cref="Watch" />/<see cref="Unwatch" /> message must be handled by the
    ///     <see cref="RemoteWatcher" />
