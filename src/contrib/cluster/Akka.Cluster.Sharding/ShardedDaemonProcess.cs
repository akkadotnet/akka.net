...
internal sealed class KeepAlivePinger : UntypedActor, IWithTimers
{
    ...
    public static Props Props(ShardedDaemonProcessSettings settings, string name, string[] identities,
        IActorRef shardingRef)
    {
        return Actor.Props.Create(() => new KeepAlivePinger(settings, name, identities, shardingRef));
    }
    ...
    private void TriggerStartAll()
    {
        Identities.ForEach(id => ShardingRef.Tell(new ShardRegion.StartEntity(id)));
    }
...
internal sealed class MessageExtractor : HashCodeMessageExtractor
{
    ...
    {
        return (message as ShardingEnvelope)?.EntityId;
    }

    public override object? EntityMessage(object message)
    {
        return (message as ShardingEnvelope)?.Message;
    }
...
public class ShardedDaemonProcess : IExtension
{
    ...
    {
        return system.WithExtension<ShardedDaemonProcess, ShardedDaemonProcessExtensionProvider>();
    }
    ...
    /// <returns>
    ///     A reference to a router actor that will distribute all messages evenly across the workers
    ///     using round-robin message routing. <c>null</c> if the ShardedDaemonProcess is misconfigured.
    /// </returns>
    public IActorRef? Init(string name, int numberOfInstances, Func<int, Props> propsFactory)
    {
        return Init(name, numberOfInstances, propsFactory, ShardedDaemonProcessSettings.Create(_system), null);
    }
    ...
    /// <param name="propsFactory">Given a unique id of `0` until `numberOfInstance` create an entity actor.</param>
    /// <param name="stopMessage">
    ///     Sent to the actors when they need to stop because of a rebalance across the nodes of the
    ///     cluster or cluster shutdown.
    /// </param>
    /// <returns>
    ///     A reference to a router actor that will distribute all messages evenly across the workers
    ///     using round-robin message routing. <c>null</c> if the ShardedDaemonProcess is misconfigured.
    ...
    {
        return Init(name, numberOfInstances, propsFactory, ShardedDaemonProcessSettings.Create(_system),
            stopMessage);
    }

    /// <summary>
    ///     Start a specific number of actors, each with a unique numeric id in the set, that is then kept alive in the
    ///     cluster.
    /// </summary>
    /// <param name="name">The name of this sharded daemon set</param>
    /// <param name="numberOfInstances">The number of instances to run</param>
    ...
    /// <param name="stopMessage">
    ///     If defined sent to the actors when they need to stop because of a rebalance across the nodes
    ///     of the cluster or cluster shutdown.
    /// </param>
    /// <returns>
    ///     A reference to a router actor that will distribute all messages evenly across the workers
    ///     using round-robin message routing. <c>null</c> if the ShardedDaemonProcess is misconfigured.
        ...
        ShardedDaemonProcessSettings settings, object? stopMessage)
    {
            ...
            Cluster.Get(_system).SelfRoles.Contains(shardingSettings.Role))
        {
            ...
            var shardingRef = sharding.Start(
                FormatWorkerProcessName(name),
                entityId => propsFactory(int.Parse(entityId)),
                shardingSettings,
                new MessageExtractor(numberOfShards),
                sharding.DefaultShardAllocationStrategy(shardingSettings),
    ...
    private static string FormatWorkerProcessName(string name)
    {
        return $"sharded-daemon-process-{name}";
    }
    ...
    public IActorRef InitProxy(string name, int numberOfInstances, string role)
    {
        ...
        var shardingRef = sharding.StartProxy(
            FormatWorkerProcessName(name),
            role,
            new MessageExtractor(numberOfInstances));
...
{
    public override ShardedDaemonProcess CreateExtension(ExtendedActorSystem system)
    {
        return new ShardedDaemonProcess(system);
    }
}