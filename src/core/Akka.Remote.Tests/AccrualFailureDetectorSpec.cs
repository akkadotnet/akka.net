...
public class AccrualFailureDetectorSpec
{
    ...
    public void AccrualFailureDetector_must_return_realistic_phi_values()
    {
        ...
        foreach (var kv in test) ShouldBe(fd.Phi(kv.Key, 1000.0, 100.0), kv.Value, 0.1);
    ...
    public void AccrualFailureDetector_must_return_phi_based_on_guess_when_only_one_heartbeat()
    {
        ...
        var fd = FailureDetectorSpecHelpers.CreateFailureDetector(8.0d, 1000, TimeSpan.FromMilliseconds(100),
            TimeSpan.Zero,
    ...
    public void AccrualFailureDetector_must_mark_node_as_dead_if_heartbeats_are_missed()
    {
        ...
        var fd = FailureDetectorSpecHelpers.CreateFailureDetector(3.0d, 1000, TimeSpan.FromMilliseconds(100),
            TimeSpan.Zero,
            TimeSpan.FromSeconds(1), FailureDetectorSpecHelpers.FakeTimeGenerator(timeInterval));
    ...
    [Fact]
    public void
        AccrualFailureDetector_must_mark_node_as_available_if_it_starts_heartbeats_again_after_being_marked_dead_due_to_detection_of_failure
        ()
    {
        ...
        var timeIntervals = regularIntervals
            .Concat(new[] { 5 * 60 * 1000L, 100L, 900L, 100L, 7000L, 100L, 900L, 100L, 900L }).ToList();
        var fd = FailureDetectorSpecHelpers.CreateFailureDetector(8.0d, 1000, TimeSpan.FromMilliseconds(100),
    ...
    public void AccrualFailureDetector_must_accept_some_configured_missing_heartbeats()
    {
        ...
        var fd = FailureDetectorSpecHelpers.CreateFailureDetector(8.0d, 1000, TimeSpan.FromMilliseconds(100),
            TimeSpan.FromSeconds(3), //3 seconds acceptableLostDuration
            TimeSpan.FromSeconds(1), FailureDetectorSpecHelpers.FakeTimeGenerator(timeIntervals));
    ...
    public void AccrualFailureDetector_must_use_maxSampleSize_heartbeats()
    {
        ...
        var fd = FailureDetectorSpecHelpers.CreateFailureDetector(8.0d, 3, TimeSpan.FromMilliseconds(100),
            TimeSpan.FromSeconds(3),
            TimeSpan.FromSeconds(1), FailureDetectorSpecHelpers.FakeTimeGenerator(timeIntervals));
    ...
    protected void ShouldBe(double actual, double expected, double epsilon = 0.001d)
    {
        Assert.True(Math.Abs(actual - expected) <= epsilon,
            string.Format("Expected {0} but received {1}", expected, actual));
    }
}
...
public static class FailureDetectorSpecHelpers
{
    public static Clock FakeTimeGenerator(IList<long> timeIntervals)
    {
        var times = timeIntervals.Skip(1)
            .Aggregate(new List<long> { timeIntervals.Head() },
                (list, l) => list.Concat(new List<long> { list.Last() + l }).ToList());
    ...
    {
        return new PhiAccrualFailureDetector(threshold, maxSampleSize, minStdDeviation, acceptableLostDuration,
            firstHeartbeatEstimate, clock);
    }

    public static DefaultFailureDetectorRegistry<string> CreateFailureDetectorRegistry(double threshold,
        int maxSampleSize,
        TimeSpan minStdDeviation, TimeSpan acceptableLostDuration, TimeSpan firstHeartbeatEstimate, Clock clock)
    {
        return new DefaultFailureDetectorRegistry<string>(() => CreateFailureDetector(threshold, maxSampleSize,
            minStdDeviation, acceptableLostDuration, firstHeartbeatEstimate, clock));
    }
