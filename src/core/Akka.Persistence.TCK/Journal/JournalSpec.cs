...
public abstract class JournalSpec : PluginSpec
{
    ...
    /// </summary>
    protected virtual bool SupportsAtomicPersistAllOfSeveralEvents => true;

    /// <summary>
    ///     When true enables tests which check if the Journal properly rejects
    ///     writes of objects which are not serializable.
    /// </summary>
    protected virtual bool SupportsRejectingNonSerializableObjects => true;
    ...
    {
        Func<long, Persistent> persistent = i =>
            new Persistent("a-" + i, i, pid, string.Empty, false, sender, writerGuid);
                ...
                ? Enumerable.Range(from, to - 1)
                    .Select(
                        i =>
                            i == to - 1
                                ? new AtomicWrite(
                                    new[] { persistent(i), persistent(i + 1) }
    ...
    public virtual void Journal_should_serialize_events()
    {
        ...
        probe.ExpectMsg<WriteMessageSuccess>(o =>
        {
            ...
            Assertions.AssertTrue(
                o.Persistent.Sender == ActorRefs.NoSender || o.Persistent.Sender.Equals(Sys.DeadLetters),
                $"Expected WriteMessagesSuccess.Persistent.Sender to be null or {Sys.DeadLetters}, but found {o.Persistent.Sender}");
