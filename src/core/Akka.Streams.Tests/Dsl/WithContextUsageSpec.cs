...
{
    private readonly Sink<Offset, TestSubscriber.Probe<Offset>> _commitOffsets;
    ...
    [Fact]
    public void
        Context_propagation_used_for_committing_offsets_must_commit_offsets_after_SelectConcat_plus_Grouped_on_offset_change()
    {
        const int groupSize = 2;
        var input = GenInput(0, 10);
        ...
        var expectedOffsets = Enumerable.Range(0, 10).Select(x => new Offset(x)).ToArray();
        expectedOffsets = expectedOffsets.Take(expectedOffsets.Length - 1).ToArray();
        var expectedRecords = ToRecords(input).SelectMany(f).Grouped(groupSize)
            .Select(r => new MultiRecord(r.ToArray())).ToArray();
    ...
    {
        return messages.Select(m => m.Record);
    }
    ...
    {
        return CommittableConsumer.CommittableSource(messages)
    ...
    private sealed class Offset : IEquatable<Offset>
    {
    ...
    private sealed class Record : IEquatable<Record>
    {
    ...
    private sealed class Committed<T>
    {
        ...
        public override string ToString()
        {
            return $"Committed(record:{Record}, offset:{Offset})";
        }
    }

    private sealed class MultiRecord : IEquatable<MultiRecord>
    {
        ...
        public override string ToString()
        {
            return $"MultiRecord({string.Join(", ", Records)})";
        }
    ...
    private interface ICommittableOffset : ICommittable
    {
        int Offset { get; }
    }

    private sealed class CommittableOffsetImpl : ICommittableOffset
    {
        ...
        public override string ToString()
        {
            return $"CommittableOffsetImpl({Offset})";
        }
    }

    private sealed class CommittableMessage<T>
    {
        ...
        public override string ToString()
        {
            return $"CommittableMessage(record:{Record}, offset:{Offset})";
        }
    }

    private static class CommittableConsumer
    {
        ...
        {
            return Source.From(committableMessages);
        }
    }
