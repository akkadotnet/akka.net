...
public class FramingSpec : AkkaSpec
{
    ...
    public void Delimiter_bytes_based_framing_must_respect_maximum_line_settings()
    {
        ...
        task1.Result.Should().BeEquivalentTo("a", "b", "c", "d");
    ...
    {
        return EncodeComplexFrame(payload, fieldLength, byteOrder, ByteString.FromBytes(new byte[fieldOffset]),
            ByteString.Empty);
    }
    ...
    public async Task Length_field_based_framing_must_ignore_length_field_value_when_provided_computeFrameSize()
    {
        ...
        var bs = ByteString.FromBytes(tempArray.PutInt(0x04050607, order: ByteOrder.LittleEndian));
