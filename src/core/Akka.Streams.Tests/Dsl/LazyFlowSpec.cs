...
public class LazyFlowSpec : AkkaSpec
{
    ...
    public async Task A_LazyFlow_must_work_in_happy_case()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            probe.Request(100);
            foreach (var i in Enumerable.Range(2, 10).Select(i => (i * 2).ToString())) await probe.ExpectNextAsync(i);
        }, Materializer);
    }
    ...
    [Fact]
    public async Task
        A_LazyFlow_must_complete_normally_when_upstream_completes_BEFORE_the_stage_has_switched_to_the_inner_flow()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
    ...
    [Fact]
    public async Task
        A_LazyFlow_must_complete_normally_when_upstream_completes_AFTER_the_stage_has_switched_to_the_inner_flow()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var (pub, sub) = this.SourceProbe<int>()
