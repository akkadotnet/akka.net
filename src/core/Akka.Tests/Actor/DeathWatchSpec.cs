...
public class DeathWatchSpec : AkkaSpec
{
    ...
    private readonly IActorRef _terminal;
    ...
    {
        var monitor1 = await StartWatching(_terminal);
        var monitor2 = Sys.ActorOf(Props.Create(() => new WatchAndUnwatchMonitor(_terminal, TestActor))
    ...
    [Fact]
    public async Task
        DeathWatch_must_notify_with_a_Terminated_message_once_when_Actor_is_stopped_but_not_when_restarted()
    {
    ...
    {
        await EventFilter.Exception<ActorKilledException>().And.Exception<DeathPactException>().ExpectAsync(2,
            async () =>
            {
    ...
    {
        var task = _supervisor.Ask(CreateWatchWithAndForwarderProps(target, TestActor, message),
