...
public static class SubFlowOperations
{
    ...
    ///     <para>
    ///         Emits when element is available from the upstream or upstream is failed and <paramref name="partialFunc" />
    ///         returns an element
    ///     </para>
    ///     <para>
    ...
    /// <returns>TBD</returns>
    public static SubFlow<Option<TOut>, TMat, TClosed> Recover<TOut, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, Func<Exception, Option<TOut>> partialFunc)
    {
    ...
    ///     <paramref name="partialFunc" /> and a new
    ...
    ///     <para>
    ///         Emits when element is available from the upstream or upstream is failed and element is available from
    ///         alternative Source
    ///     </para>
    ///     <para>
    ///         Backpressures when downstream backpressures
    ...
    ///     a failure has been recovered up to <paramref name="attempts" /> number of times so that each time there is a
    ///     failure it is fed into the <paramref name="partialFunc" /> and a new
    ///     Source may be materialized. Note that if you pass in 0, this won't attempt to recover at all. Passing in -1 will
    ///     behave exactly the same as  <see cref="RecoverWithRetries{TOut,TMat,TClosed}" />.
    ///     <para>
    ///         Since the underlying failure signal onError arrives out-of-band, it might jump over existing elements.
    ///         This stage can recover the failure signal, but not the skipped elements, which will be dropped.
    ///     </para>
    ...
    /// <summary>
    ///     While similar to <see cref="Recover{TOut,TMat,TClosed}" /> this stage can be used to transform an error signal to a
    ///     different one without logging
    ///     it as an error in the process. So in that sense it is NOT exactly equivalent to Recover(e => throw e2) since
    ///     Recover
    ...
    ///     <para>
    ///         Emits when element is available from the upstream or upstream is failed and <paramref name="selector" />
    ///         returns an element
    ///     </para>
    ///     <para>
    ///         Backpressures when downstream backpressures
    ///     </para>
    ///     <para>
    ///         Completes when upstream completes or upstream failed with exception returned by the
    ...
    /// <param name="flow">TBD</param>
    /// <param name="selector">
    ///     Receives the failure cause and returns the new cause, return the original exception if no other
    ///     should be applied
    /// </param>
    public static SubFlow<TOut, TMat, TClosed> SelectError<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Select<TIn, TOut, TMat, TClosed>(this SubFlow<TIn, TMat, TClosed> flow,
    ...
    ///     <para>
    ///         If the wire-tap operation is slow (it backpressures), elements that would've been sent to it will be dropped
    ///         instead.
    ///     </para>
    ///     <para>
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> WireTap<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        Action<TOut> action)
    {
        return (SubFlow<TOut, TMat, TClosed>)InternalFlowOperations.WireTap(flow, action);
    }
    ...
    ///     <para>
    ///         Emits when the mapping function <paramref name="mapConcater" /> returns an element or there are still remaining
    ///         elements
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> SelectMany<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow, Func<TOut1, IEnumerable<TOut2>> mapConcater)
    {
    ...
    ///     which is enabled by creating the transformation function <paramref name="mapConcaterFactory" /> a new for every
    ///     materialization —
    ...
    ///     If the group by function <paramref name="asyncMapper" /> throws an exception or if the <see cref="Task" /> is
    ///     completed
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> SelectAsync<TIn, TOut, TMat, TClosed>(
        this SubFlow<TIn, TMat, TClosed> flow, int parallelism, Func<TIn, Task<TOut>> asyncMapper)
    {
    ...
    ///     <see cref="SelectAsyncUnordered{TIn,TOut,TMat,TClosed}" />.
    ...
    /// <param name="asyncMapper">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> SelectAsyncUnordered<TIn, TOut, TMat, TClosed>(
        this SubFlow<TIn, TMat, TClosed> flow, int parallelism, Func<TIn, Task<TOut>> asyncMapper)
    {
        return (SubFlow<TOut, TMat, TClosed>)InternalFlowOperations.SelectAsyncUnordered(flow, parallelism,
            asyncMapper);
    }
    ...
    ///     </para>
    ///     <para>
    ///         Backpressures when the given <paramref name="predicate" /> returns true for the element and downstream
    ///         backpressures
    ///     </para>
    ///     <para>
    ///         Completes when upstream completes
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Where<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
    ...
    /// <param name="predicate">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> WhereNot<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        Predicate<TOut> predicate)
    {
    ...
    ///     </para>
    ///     <para>
    ///         Completes when <paramref name="predicate" /> returned false (or 1 after predicate returns false if
    ///         <paramref name="inclusive" />) or upstream completes
    ///     </para>
    ///     <para>
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> TakeWhile<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        Predicate<TOut> predicate, bool inclusive = false)
    {
    ...
    /// <param name="predicate">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> SkipWhile<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        Predicate<TOut> predicate)
    {
    ...
    ///     </para>
    ///     Backpressures when the function <paramref name="collector" /> is defined for the element and downstream
    ///     backpressures
    ///     <para>
    ///         Completes when upstream completes
    ///     </para>
    ...
    public static SubFlow<TOut2, TMat, TClosed> Collect<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow, Func<TOut1, TOut2> collector)
    {
    ...
    /// <returns>TBD</returns>
    public static SubFlow<IEnumerable<TOut>, TMat, TClosed> Grouped<TOut, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, int n)
    {
    ...
    /// <returns>TBD</returns>
    public static SubFlow<T, TMat, TClosed> LimitWeighted<T, TMat, TClosed>(this SubFlow<T, TMat, TClosed> flow,
        long max, Func<T, long> costFunc)
    {
    ...
    ///     Thrown when <paramref name="n" /> or <paramref name="step" /> is less than or equal
    ///     zero.
    /// </exception>
    /// <returns>TBD</returns>
    public static SubFlow<IEnumerable<TOut>, TMat, TClosed> Sliding<TOut, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, int n, int step = 1)
    {
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> Scan<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow, TOut2 zero, Func<TOut2, TOut1, TOut2> scan)
    {
    ...
    /// <param name="scan">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> ScanAsync<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow, TOut2 zero, Func<TOut2, TOut1, Task<TOut2>> scan)
    {
    ...
    ///     after which it also completes. Applies the given function <paramref name="fold" /> towards its current and next
    ///     value,
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> Aggregate<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow, TOut2 zero, Func<TOut2, TOut1, TOut2> fold)
    {
    ...
    /// <param name="fold">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> AggregateAsync<TIn, TOut, TMat, TClosed>(
        this SubFlow<TIn, TMat, TClosed> flow, TOut zero,
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Sum<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
    ...
    ///     Thrown when any of the <paramref name="start" />, <paramref name="inject" /> or
    ///     <paramref name="end" /> is undefined.
    /// </exception>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Intersperse<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        TOut start, TOut inject, TOut end)
    {
    ...
    /// <exception cref="ArgumentException">
    ///     Thrown if <paramref name="n" /> is less than or equal zero or
    ///     <paramref name="timeout" /> is <see cref="TimeSpan.Zero" />.
    /// </exception>
    /// <returns>TBD</returns>
    public static SubFlow<IEnumerable<TOut>, TMat, TClosed> GroupedWithin<TOut, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, int n, TimeSpan timeout)
    {
    ...
    ///     since the previously emitted group.
    ///     <para>
    ///         <paramref name="maxWeight" /> must be positive, and <paramref name="interval" /> must be greater than 0
    ///         seconds,
    ...
    /// <returns>TBD</returns>
    public static SubFlow<IEnumerable<TOut>, TMat, TClosed> GroupedWeightedWithin<TOut, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, long maxWeight, int maxNumber, TimeSpan interval,
        Func<TOut, long> costFn)
    {
        return (SubFlow<IEnumerable<TOut>, TMat, TClosed>)InternalFlowOperations.GroupedWeightedWithin(flow, maxWeight,
            maxNumber, interval, costFn);
    }
    ...
    ///     <see cref="Attributes.CreateInputBuffer" />
    ///     <para>
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Delay<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        TimeSpan of, DelayOverflowStrategy? strategy = null)
    {
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> SkipWithin<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        TimeSpan duration)
    {
    ...
    /// <param name="duration">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> TakeWithin<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        TimeSpan duration)
    {
    ...
    ///     be different than the input type. See <see cref="Conflate{TOut,TMat,TClosed}" /> for a simpler version that does
    ///     not change types.
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TSeed, TMat, TClosed> ConflateWithSeed<TOut, TMat, TSeed, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, Func<TOut, TSeed> seed, Func<TSeed, TOut, TSeed> aggregate)
    {
    ...
    ///     until the subscriber is ready to accept them. For example a conflate step might average incoming numbers if the
    ///     upstream publisher is faster.
    ///     This version of conflate does not change the output type of the stream. See
    ///     <see cref="ConflateWithSeed{TOut,TMat,TSeed,TClosed}" />
    ...
    /// <param name="aggregate">Takes the currently aggregated value and the current pending element to produce a new aggregate</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Conflate<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
    ...
    ///     Backpressures when there are <paramref name="max" /> batched elements and 1 pending element and downstream
    ///     backpressures
    ///     Completes when upstream completes and there is no batched/pending element waiting
    ///     Cancels when downstream cancels
    ///     See also <seealso cref="ConflateWithSeed{TOut,TMat,TSeed,TClosed}" />,
    ///     <seealso cref="BatchWeighted{TOut,TOut2,TMat,TClosed}" />
    /// </summary>
    /// <typeparam name="TOut">TBD</typeparam>
    /// <typeparam name="TOut2">TBD</typeparam>
    /// <typeparam name="TMat">TBD</typeparam>
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> Batch<TOut, TOut2, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, long max,
    ...
    ///     being applied with the <paramref name="seed" /> function) without batching further elements with it, and then the
    ///     rest of the
    ///     incoming elements are batched.
    ///     Emits when downstream stops backpressuring and there is a batched element available
    ///     Backpressures when there are <paramref name="max" /> weighted batched elements + 1 pending element and downstream
    ///     backpressures
    ///     Completes when upstream completes and there is no batched/pending element waiting
    ///     Cancels when downstream cancels
    ///     See also <seealso cref="ConflateWithSeed{TOut,TMat,TSeed,TClosed}" />,
    ///     <seealso cref="Batch{TOut,TOut2,TMat,TClosed}" />
    /// </summary>
    /// <typeparam name="TOut">TBD</typeparam>
    /// <typeparam name="TOut2">TBD</typeparam>
    /// <typeparam name="TMat">TBD</typeparam>
    ...
    /// <param name="aggregate">Takes the currently batched value and the current pending element to produce a new aggregate</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> BatchWeighted<TOut, TOut2, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, long max, Func<TOut, long> costFunction,
        Func<TOut, TOut2> seed, Func<TOut2, TOut, TOut2> aggregate)
    {
        return (SubFlow<TOut2, TMat, TClosed>)InternalFlowOperations.BatchWeighted(flow, max, costFunction, seed,
            aggregate);
    }
    ...
    /// </param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> Expand<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow, Func<TOut1, IEnumerator<TOut2>> extrapolate)
    {
    ...
    /// <param name="strategy">Strategy that is used when incoming elements cannot fit inside the buffer</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Buffer<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        int size, OverflowStrategy strategy)
    {
    ...
    public static SubFlow<TOut2, TMat, TClosed> Transform<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow, Func<IStage<TOut1, TOut2>> stageFactory)
    {
        return (SubFlow<TOut2, TMat, TClosed>)InternalFlowOperations.Transform(flow, stageFactory);
    }

    /// <summary>
    ///     Takes up to <paramref name="n" /> elements from the stream and returns a pair containing a strict sequence of the
    ///     taken element
    ///     and a stream representing the remaining elements. If <paramref name="n" /> is zero or negative, then this will
    ///     return a pair
    ...
    /// <param name="n">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<(IImmutableList<TOut>, Source<TOut, NotUsed>), TMat, TClosed>
        PrefixAndTail<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow, int n)
    {
        return (SubFlow<(IImmutableList<TOut>, Source<TOut, NotUsed>), TMat, TClosed>)InternalFlowOperations
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> ConcatMany<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow, Func<TOut1, IGraph<SourceShape<TOut2>, TMat>> flatten)
    {
    ...
    /// <param name="flatten">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> MergeMany<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow, int breadth, Func<TOut1, IGraph<SourceShape<TOut2>, TMat>> flatten)
    {
    ...
    ///     Cancels when downstream cancels
    /// </summary>
    public static SubFlow<(TOut1, long), TMat, TClosed> ZipWithIndex<TOut1, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow)
    {
    ...
    /// <param name="timeout">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> InitialTimeout<TOut, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, TimeSpan timeout)
    {
    ...
    /// <param name="timeout">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> CompletionTimeout<TOut, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, TimeSpan timeout)
    {
    ...
    /// <param name="timeout">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> IdleTimeout<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        TimeSpan timeout)
    {
    ...
    /// <param name="timeout">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> BackpressureTimeout<TOut, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, TimeSpan timeout)
    {
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TInjected, TMat, TClosed> KeepAlive<TOut, TInjected, TMat, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, TimeSpan timeout, Func<TInjected> injectElement) where TOut : TInjected
    {
        return (SubFlow<TInjected, TMat, TClosed>)InternalFlowOperations.KeepAlive(flow, timeout, injectElement);
    }

    /// <summary>
    ///     Sends elements downstream with speed limited to <paramref name="elements" />/<paramref name="per" />. In other
    ///     words, this stage set the maximum rate
    ///     for emitting messages. This combinator works for streams where all elements have the same cost or length.
    ///     Throttle implements the token bucket model. There is a bucket with a given token capacity (burst size or
    ///     maximumBurst).
    ...
    /// <exception cref="ArgumentException">
    ///     Thrown when <paramref name="elements" /> is less than or equal zero,
    ///     or <paramref name="per" /> timeout is equal <see cref="TimeSpan.Zero" />
    ///     or <paramref name="maximumBurst" /> is less than or equal zero in in <see cref="ThrottleMode.Enforcing" />
    ///     <paramref name="mode" />.
    /// </exception>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Throttle<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        int elements, TimeSpan per, int maximumBurst, ThrottleMode mode)
    {
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Throttle<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        int cost, TimeSpan per, int maximumBurst, Func<TOut, int> calculateCost, ThrottleMode mode)
    {
        return (SubFlow<TOut, TMat, TClosed>)InternalFlowOperations.Throttle(flow, cost, per, maximumBurst,
            calculateCost, mode);
    }
    ...
    ///     It is recommended to use the internally optimized <seealso cref="Keep.Left{TLeft,TRight}" /> and
    ///     <seealso cref="Keep.Right{TLeft,TRight}" /> combiners
    ...
    {
        return (SubFlow<TOut, TMat3, TClosed>)InternalFlowOperations.AlsoToMaterialized(flow, that,
    ...
    /// <param name="that">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> AlsoTo<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
    ...
    /// <summary>
    ///     <para>
    ///         Attaches the given <seealso cref="Sink{TIn,TMat}" /> to this <see cref="IFlow{TOut,TMat}" />, as a wire tap,
    ///         meaning that elements that pass
    ///         through will also be sent to the wire-tap Sink, without the latter affecting the mainline flow. If the wire-tap
    ///         Sink backpressures,
    ///         elements that would've been sent to it will be dropped instead.
    ///     </para>
    ///     <para>
    ///         It is similar to <seealso cref="AlsoTo{TOut,TMat,TClosed}" /> which does backpressure instead of dropping
    ///         elements.
    ///     </para>
    ///     <para>
    ///         Emits when element is available and demand exists from the downstream; the element will also be sent to the
    ///         wire-tap Sink if there is demand.
    ///     </para>
    ///     <para>Backpressures when downstream backpressures</para>
    ///     <para>Completes when upstream completes</para>
    ///     <para>Cancels when downstream cancels</para>
    ...
    public static SubFlow<TOut, TMat, TClosed> WireTap<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        IGraph<SinkShape<TOut>, TMat> that)
    {
        return (SubFlow<TOut, TMat, TClosed>)InternalFlowOperations.WireTap(flow, that);
    }

    /// <summary>
    ///     Attaches the given <seealso cref="Sink{TIn,TMat}" /> to this <see cref="IFlow{TOut,TMat}" />, meaning that elements
    ///     will be sent to the <seealso cref="Sink{TIn,TMat}" /> instead of being passed through if the predicate `when`
    ///     returns `true`.
    ...
    public static SubFlow<TOut, TMat3, TClosed> DivertToMaterialized<TOut, TMat, TMat2, TMat3, TClosed>(
        ...
        Func<TMat, TMat2, TMat3> materializerFunction)
    {
        return (SubFlow<TOut, TMat3, TClosed>)InternalFlowOperations.DivertToMaterialized(flow, that, when,
            materializerFunction);
    }
    ...
    /// <param name="that">TBD</param>
    /// <param name="when">TBD</param>
    public static SubFlow<TOut, TMat, TClosed> DivertTo<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        IGraph<SinkShape<TOut>, TMat> that, Func<TOut, bool> when)
    {
        return (SubFlow<TOut, TMat, TClosed>)InternalFlowOperations.DivertTo(flow, that, when);
    }
    ...
    ///     downstream.
    ///     It is recommended to use the internally optimized <see cref="Keep.Left{TLeft,TRight}" /> and
    ///     <see cref="Keep.Right{TLeft,TRight}" /> combiners
    ///     where appropriate instead of manually writing functions that pass through one of the values.
    /// </summary>
    /// <typeparam name="TOut">TBD</typeparam>
    /// <typeparam name="TMat">TBD</typeparam>
    ...
    /// <param name="materializerFunction">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat2, TClosed> WatchTermination<TOut, TMat, TMat2, TClosed>(
        this SubFlow<TOut, TMat, TClosed> flow, Func<TMat, Task<Done>, TMat2> materializerFunction)
    {
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> InitialDelay<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        TimeSpan delay)
    {
    ...
    ///     This can be adjusted according to your needs by providing a custom <see cref="Attributes.LogLevels" /> attribute on
    ///     the given Flow.
    ///     <para>
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Log<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        string name, Func<TOut, object> extract = null, ILoggingAdapter log = null)
    {
    ...
    /// <returns>TBD</returns>
    public static SubFlow<(T1, T2), TMat, TClosed> Zip<T1, T2, TMat, TClosed>(this SubFlow<T1, TMat, TClosed> flow,
    ...
    /// <returns>TBD</returns>
    public static SubFlow<T3, TMat, TClosed> ZipWith<T1, T2, T3, TMat, TClosed>(this SubFlow<T1, TMat, TClosed> flow,
    ...
    /// <summary>
    ///     Interleave is a deterministic merge of the given <see cref="Source{TOut,TMat}" /> with elements of this
    ///     <see cref="IFlow{TOut,TMat}" />.
    ///     It first emits <paramref name="segmentSize" /> number of elements from this flow to downstream, then - same amount
    ///     for <paramref name="other" />
    ...
    /// <returns>TBD</returns>
    public static SubFlow<T2, TMat, TClosed> Interleave<T1, T2, TMat, TClosed>(this SubFlow<T1, TMat, TClosed> flow,
        IGraph<SourceShape<T2>, TMat> other, int segmentSize) where T1 : T2
    {
    ...
    /// <summary>
    ///     Interleave is a deterministic merge of the given <see cref="Source{TOut,TMat}" /> with elements of this
    ///     <see cref="IFlow{T,TMat}" />.
    ///     It first emits <paramref name="segmentSize" /> number of elements from this flow to downstream, then - same amount
    ///     for <paramref name="graph" /> source,
    ///     then repeat process.
    ///     After one of upstreams is complete than all the rest elements will be emitted from the second one
    ///     If it gets error from one of upstreams - stream completes with failure.
    ...
    /// <param name="combine">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<T2, TMat3, TClosed> InterleaveMaterialized<T1, T2, TMat, TMat2, TMat3, TClosed>(
    ...
    {
        return (SubFlow<T2, TMat3, TClosed>)InternalFlowOperations.InterleaveMaterialized(flow, graph, segmentSize,
            combine);
    }

    /// <summary>
    ///     Merge the given <see cref="Source{TOut,TMat}" /> to this <see cref="IFlow{TOut,TMat}" />, taking elements as they
    ///     arrive from input streams,
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> Merge<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow, IGraph<SourceShape<TOut2>, TMat> other, bool eagerComplete = false)
        where TOut1 : TOut2
    {
    ...
    /// <param name="combine">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat3, TClosed> MergeMaterialized<TOut1, TOut2, TMat, TMat2, TMat3, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow,
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> MergeSorted<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        IGraph<SourceShape<TOut>, TMat> other, Func<TOut, TOut, int> orderFunc)
    {
    ...
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> MergeSorted<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        IGraph<SourceShape<TOut>, TMat> other, IComparer<TOut> comparer)
    {
        return (SubFlow<TOut, TMat, TClosed>)InternalFlowOperations.MergeSorted(flow, other, comparer);
    }

    /// <summary>
    ///     Concatenate the given <see cref="Source{TOut,TMat}" /> to this <see cref="IFlow{TOut,TMat}" />, meaning that once
    ///     this
    ///     Flow’s input is exhausted and all result elements have been generated,
    ///     the Source’s elements will be produced.
    ///     Note that the <see cref="Source{TOut,TMat}" /> is materialized together with this <see cref="IFlow{TOut,TMat}" />
    ///     and just kept
    ///     from producing elements by asserting back-pressure until its time comes.
    ///     If this <see cref="IFlow{TOut,TMat}" /> gets upstream error - no elements from the given
    ///     <see cref="Source{TOut,TMat}" /> will be pulled.
    ///     <para>
    ///         Emits when element is available from current stream or from the given <see cref="Source{TOut,TMat}" /> when
    ...
    /// <param name="other">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut, TMat, TClosed> Concat<TOut, TMat, TClosed>(this SubFlow<TOut, TMat, TClosed> flow,
        IGraph<SourceShape<TOut>, TMat> other)
    {
    ...
    /// <summary>
    ///     Prepend the given <see cref="Source{TOut,TMat}" /> to this <see cref="IFlow{TOut,TMat}" />, meaning that before
    ///     elements
    ///     are generated from this <see cref="IFlow{TOut,TMat}" />, the Source's elements will be produced until it
    ///     is exhausted, at which point Flow elements will start being produced.
    ///     Note that this <see cref="IFlow{TOut,TMat}" /> will be materialized together with the
    ///     <see cref="Source{TOut,TMat}" /> and just kept
    ///     from producing elements by asserting back-pressure until its time comes.
    ///     If the given <see cref="Source{TOut,TMat}" /> gets upstream error - no elements from this
    ///     <see cref="IFlow{TOut,TMat}" /> will be pulled.
    ///     Emits when element is available from the given <see cref="Source{TOut,TMat}" /> or from current stream when the
    ///     <see cref="Source{TOut,TMat}" /> is completed
    ///     Backpressures when downstream backpressures
    ...
    /// <param name="that">TBD</param>
    /// <returns>TBD</returns>
    public static SubFlow<TOut2, TMat, TClosed> Prepend<TOut1, TOut2, TMat, TClosed>(
        this SubFlow<TOut1, TMat, TClosed> flow,
    ...
    ///     '''Emits when''' element is available from first stream or first stream closed without emitting any elements and an
    ///     element
    ///     is available from the second stream
    ///     '''Backpressures when''' downstream backpressures
    ///     '''Completes when''' the primary stream completes after emitting at least one element, when the primary stream
    ///     completes
    ...
    /// <returns>TBD</returns>
    public static SubFlow<T, TMat, TClosed> OrElse<T, TMat, TClosed>(this SubFlow<T, TMat, TClosed> flow,
        IGraph<SourceShape<T>, TMat> secondary)
    {
        return (SubFlow<T, TMat, TClosed>)InternalFlowOperations.OrElse(flow, secondary);
    }
}