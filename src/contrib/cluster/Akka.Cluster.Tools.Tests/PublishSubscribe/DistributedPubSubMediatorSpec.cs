...
public class DistributedPubSubMediatorSpec : AkkaSpec
{
    ...
    /// <summary>
    ///     Checks to see if
    ///     http://stackoverflow.com/questions/41249465/how-to-test-distributedpubsub-with-the-testkit-in-akka-net is resolved
    /// </summary>
    [Fact]
    public void DistributedPubSubMediator_can_be_activated_in_child_actor()
    {
        EventFilter.Exception<NullReferenceException>().Expect(0, () =>
        {
            var actor = Sys.ActorOf((dsl, _) =>
            {
                ...
                dsl.OnPreStart = actorContext => { mediator = DistributedPubSub.Get(actorContext.System).Mediator; };
