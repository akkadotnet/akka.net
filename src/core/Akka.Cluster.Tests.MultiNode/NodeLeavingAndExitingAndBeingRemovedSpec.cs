...
public class NodeLeavingAndExitingAndBeingRemovedSpec : MultiNodeClusterSpec
{
    ...
    protected NodeLeavingAndExitingAndBeingRemovedSpec(NodeLeavingAndExitingAndBeingRemovedSpecConfig config) : base(
        config, typeof(NodeLeavingAndExitingAndBeingRemovedSpec))
    {
        _config = config;
    }
        ...
        Node_that_is_leaving_non_singleton_cluster_eventually_set_to_removed_and_removed_from_membership_ring_and_seen_table()
    {
        ...
        {
            RunOn(() => { Cluster.Leave(GetAddress(_config.Second)); }, _config.First);
            ...
            RunOn(() =>
            {
                ...
                AwaitAssert(() =>
                {
                    ClusterView.UnreachableMembers.Select(c => c.Address).Should()
                        .NotContain(GetAddress(_config.Second));
                });
