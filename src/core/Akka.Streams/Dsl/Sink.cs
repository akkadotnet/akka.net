...
public sealed class Sink<TIn, TMat> : IGraph<SinkShape<TIn>, TMat>
{
    ...
    /// <summary>
    ///     Connect this <see cref="Sink{TIn,TMat}" /> to a <see cref="Source{T,TMat}" /> and run it. The returned value is the
    ///     materialized value
    ...
    /// <returns>
    ///     A tuple containing the (1) materialized value and (2) a new <see cref="Sink" />
    ///     that can be used to consume elements from the newly materialized <see cref="Sink" />.
    /// </returns>
...
public static class Sink
{
    ...
    {
        return graph is Sink<TIn, TMat> sink
            ? sink
    ...
    {
        return new Sink<TIn, object>(new SubscriberSink<TIn>(subscriber, DefaultAttributes.SubscriberSink,
            Shape<TIn>("SubscriberSink")));
    }

    /// <summary>
    ///     A <see cref="Sink{TIn,TMat}" /> that materializes into a <see cref="Task{TIn}" /> of the first value received.
    ///     If the stream completes before signaling at least a single element, the Task will be failed with a
    ///     <see cref="NoSuchElementException" />.
    ///     If the stream signals an error before signaling at least a single element, the Task will be failed with the streams
    ///     exception.
    /// </summary>
    /// <typeparam name="TIn">TBD</typeparam>
    ...
    {
        return FromGraph(new FirstOrDefault<TIn>(true))
    ...
    ///     If the stream signals an error errors before signaling at least a single element, the Task will be failed with the
    ///     streams exception.
    /// </summary>
    /// <typeparam name="TIn">TBD</typeparam>
    /// <returns>TBD</returns>
    ...
    {
        return FromGraph(new LastOrDefault<TIn>(true)).WithAttributes(DefaultAttributes.LastOrDefaultSink);
    }
    ...
    {
        return new Sink<TIn, IPublisher<TIn>>(new PublisherSink<TIn>(DefaultAttributes.PublisherSink,
            Shape<TIn>("PublisherSink")));
    }
    ...
    {
        return new Sink<TIn, IPublisher<TIn>>(new FanoutPublisherSink<TIn, ResizableMultiReaderRingBuffer<TIn>>(
            DefaultAttributes.FanoutPublisherSink, Shape<TIn>("FanoutPublisherSink")));
    }

    [InternalApi]
    public static Sink<TIn, IPublisher<TIn>> DistinctRetainingFanOutPublisher<TIn>(Action onTerminated = null)
    {
        return new Sink<TIn, IPublisher<TIn>>(new FanoutPublisherSink<TIn, DistinctRetainingMultiReaderBuffer<TIn>>(
            DefaultAttributes.FanoutPublisherSink, Shape<TIn>("DistinctRetainingFanOutPublisherSink"),
            onTerminated));
    }
    ...
    {
        return FromGraph(new IgnoreSink<TIn>());
    }
    ...
    /// <summary>
    ///     A <see cref="Sink{TIn,TMat}" /> that will invoke the given async <paramref name="action" /> for each received
    ///     element.
    ///     The sink is materialized into a <see cref="Task" /> will be completed with success when reaching the
    ///     normal end of the stream, or completed with a failure if there is a failure signaled in
    ///     the stream.
    ...
    /// <summary>
    ///     Combine several sinks with fan-out strategy like <see cref="Broadcast{TIn}" /> or <see cref="Balance{TIn}" /> and
    ///     returns <see cref="Sink{TIn,TMat}" />.
    /// </summary>
    /// <typeparam name="TIn">TBD</typeparam>
    ...
    public static Sink<TIn, NotUsed> Combine<TIn, TOut, TMat>(
        Func<int, IGraph<UniformFanOutShape<TIn, TOut>, TMat>> strategy, Sink<TOut, NotUsed> first,
        Sink<TOut, NotUsed> second, params Sink<TOut, NotUsed>[] rest)
    {
    ...
    /// <returns>TBD</returns>
    [Obsolete(
        "Use `ForEachAsync` instead, it allows you to choose how to run the procedure, by calling some other API returning a Task or using Task.Run. Obsolete since 1.5.2")]
    ...
    /// <summary>
    ///     A <see cref="Sink{TIn, Task}" /> that will invoke the given <paramref name="aggregate" /> function for every
    ///     received element,
    ...
    {
        return Flow.Create<TIn, Task<TOut>>()
    ...
    /// <summary>
    ///     <para>
    ///         A <see cref="Sink{TIn,Task}" /> that will invoke the given <paramref name="reduce" /> for every received
    ///         element, giving it its previous
    ...
    public static Sink<TIn, NotUsed> ActorRef<TIn>(IActorRef actorRef, object onCompleteMessage,
        Func<Exception, object> onFailureMessage)
    {
        return FromGraph(new ActorRefSinkStage<TIn>(actorRef, onCompleteMessage, onFailureMessage));
    }
    ...
    public static Sink<TIn, NotUsed> ActorRef<TIn>(IActorRef actorRef, object onCompleteMessage)
    {
        return FromGraph(new ActorRefSinkStage<TIn>(actorRef, onCompleteMessage, ex => new Status.Failure(ex)));
    }
    ...
    /// <summary>
    ///     Creates a <see cref="Sink{TIn,TMat}" /> that is materialized to an <see cref="IActorRef" /> which points to an
    ///     Actor
    ...
    {
        return new Sink<TIn, IActorRef>(new ActorSubscriberSink<TIn>(props, DefaultAttributes.ActorSubscriberSink,
            Shape<TIn>("ActorSubscriberSink")));
    }

    /// <summary>
    ///     <para>
    ///         Creates a <see cref="Sink{TIn,TMat}" /> that is materialized as an <see cref="ISinkQueue{TIn}" />.
    ///         <see cref="ISinkQueue{TIn}.PullAsync" /> method is pulling element from the stream and returns
    ///         <see cref="Task{Option}" />.
    ...
    /// <summary>
    ///     <para>
    ///         Creates a real <see cref="Sink{TIn,TMat}" /> upon receiving the first element. Internal
    ///         <see cref="Sink{TIn,TMat}" /> will not be created if there are no elements,
    ///         because of completion or error.
    ///     </para>
    ///     <para>
    ///         If upstream completes before an element was received then the <see cref="Task" /> is completed with the value
    ///         created by <paramref name="fallback" />.
    ///     </para>
    ///     <para>
    ///         If upstream fails before an element was received, <paramref name="sinkFactory" /> throws an exception, or
    ///         materialization of the internal
    ...
    public static Sink<TIn, Task<TMat>> LazySink<TIn, TMat>(Func<TIn, Task<Sink<TIn, TMat>>> sinkFactory,
        Func<TMat> fallback)
    {
        return FromGraph(new LazySink<TIn, TMat>(sinkFactory)).MapMaterializedValue(t =>
            t.ContinueWith(t1 => t1.Result.GetOrElse(fallback()), TaskContinuationOptions.ExecuteSynchronously));
    }

    /// <summary>
    ///     Creates a real <see cref="Sink{TIn,TMat}" /> upon receiving the first element. Internal
    ///     <see cref="Sink{TIn,TMat}" />
    ...
    {
        return FromGraph(new LazySink<TIn, TMat>(_ => sinkFactory()));
    }
    ...
    {
        return new Sink<TIn, NotUsed>(new SubscriberSink<TIn>(subscriber, DefaultAttributes.SubscriberSink,
            Shape<TIn>("SubscriberSink")));
    }
    ...
    {
        return new Sink<TIn, NotUsed>(new CancelSink<TIn>(DefaultAttributes.CancelledSink,
            Shape<TIn>("CancelledSink")));
    }

    /// <summary>
    ///     A <see cref="Sink{TIn,TMat}" /> that materializes into a <see cref="IPublisher{TIn}" />.
    ///     If <paramref name="fanout" /> is true, the materialized <see cref="IPublisher{TIn}" /> will support multiple
    ///     <see cref="ISubscriber{TIn}" />`s and
    ///     the size of the <see cref="ActorMaterializerSettings.MaxInputBufferSize" /> configured for this stage becomes the
    ///     maximum number of elements that
    ///     the fastest <see cref="ISubscriber{T}" /> can be ahead of the slowest one before slowing
    ///     the processing down due to back pressure.
    ///     If <paramref name="fanout" /> is false then the materialized <see cref="IPublisher{TIn}" /> will only support a
    ///     single <see cref="ISubscriber{TIn}" /> and
    ...
    public static Sink<TIn, IPublisher<TIn>> AsPublisher<TIn>(bool fanout)
    {
        ...
        if (fanout)
            publisherSink =
                new FanoutPublisherSink<TIn, ResizableMultiReaderRingBuffer<TIn>>(DefaultAttributes.FanoutPublisherSink,
                    Shape<TIn>("FanoutPublisherSink"));
        else
    ...
    /// <summary>
    ///     A <see cref="Sink{TIn,TMat}" /> that materializes into a <see cref="IObservable{T}" />. It supports multiple
    ///     subscribers.
    ///     Since observables have no notion of backpressure, it will push incoming elements as fast as possible, potentially
    ///     risking
    ///     process overrun.
    /// </summary>
    ...
    {
        return FromGraph(new ObservableSinkStage<T>());
    }

    public static Sink<T, ChannelReader<T>> ChannelReader<T>(int bufferSize, bool singleReader,
