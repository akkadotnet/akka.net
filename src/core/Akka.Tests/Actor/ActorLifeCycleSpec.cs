...
public class ActorLifeCycleSpec : AkkaSpec
{
    ...
    public async Task Actor_lifecycle_test1()
    {
        ...
        var supervisor = Sys.ActorOf(Props.Create(() =>
            new Supervisor(new OneForOneStrategy(3, TimeSpan.FromSeconds(1000), x => Directive.Restart))));
    ...
    public async Task Log_failures_in_PostStop()
    {
        ...
        await EventFilter.Exception<Exception>("hurrah").ExpectOneAsync(() =>
        {
    ...
    [Fact(DisplayName =
        "If a parent receives a Terminated event for a child actor, the parent should no longer supervise it")]
    ...
    public async Task Call_PreStart_with_correct_message_and_sender()
    {
        ...
        {
            c.Receive<string>((_, _) => { throw new MyCustomException(); });
    ...
    public class LifeCycleTestActor : UntypedActor
    {
        ...
        public LifeCycleTestActor(IActorRef testActor, string id, AtomicCounter generationProvider)
        {
            ...
            CurrentGeneration = generationProvider.Next();
        }
        ...
        {
            if (message is string s && s == "status") testActor.Tell(("OK", id, CurrentGeneration));
        }
