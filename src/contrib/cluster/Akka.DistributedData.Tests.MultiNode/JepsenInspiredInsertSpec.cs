...
public class JepsenInspiredInsertSpec : MultiNodeClusterSpec
{
    ...
    protected JepsenInspiredInsertSpec(JepsenInspiredInsertSpecConfig config) : base(config,
        typeof(JepsenInspiredInsertSpec))
    {
        ...
        var nodeindex = _nodes.Zip(Enumerable.Range(0, _nodes.Count),
                (name, i) => new KeyValuePair<int, RoleName>(i, name))
    ...
    public void Insert_from_5_nodes_should_replicate_values_when_all_nodes_connected()
    {
        ...
        RunOn(() =>
        {
            ...
            var writeAcks = MyData.Select(i =>
            {
                SleepDelay();
                _replicator.Tell(
                    Dsl.Update(key, ORSet<int>.Empty, WriteLocal.Instance, i,
                        x => x.Add(_cluster.SelfUniqueAddress, i)), writeProbe.Ref);
    ...
    public void Insert_from_5_nodes_should_read_write_to_majority_when_all_nodes_connected()
    {
        ...
        RunOn(() =>
        {
            ...
            var writeAcks = MyData.Select(i =>
            {
                SleepDelay();
                _replicator.Tell(
                    Dsl.Update(key, ORSet<int>.Empty, writeMajority, i, x => x.Add(_cluster.SelfUniqueAddress, i)),
                    writeProbe.Ref);
