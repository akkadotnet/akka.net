...
internal abstract class TopicLike : ActorBase
{
    ...
    ///     TBD
    /// </summary>
    protected Deadline PruneDeadline;
    ...
    /// <param name="sendToDeadLettersWhenNone">
    ///     When set to <c>true</c>, this actor will
    ///     publish a <see cref="DeadLetter" /> for each message if the total number of subscribers == 0.
    /// </param>
    ...
    protected void Remove(IActorRef actorRef)
    {
        ...
        if (Subscribers.Count == 0 && !Context.GetChildren().Any()) PruneDeadline = Deadline.Now + EmptyTimeToLive;
    }
}
...
internal class Topic : TopicLike
{
    ...
    public Topic(TimeSpan emptyTimeToLive, RoutingLogic routingLogic, bool sendToDeadLettersWhenNone) : base(
        emptyTimeToLive, sendToDeadLettersWhenNone)
    {
...
internal class Group : TopicLike
{
    ...
    /// </param>
    /// <param name="routingLogic">The routing logic to use for distributing messages to subscribers.</param>
    public Group(TimeSpan emptyTimeToLive, RoutingLogic routingLogic, bool sendToDeadLettersWhenNone) : base(
        emptyTimeToLive, sendToDeadLettersWhenNone)
    {
        _routingLogic = routingLogic;
    }
...
{
    private static readonly Regex _pathRegex = new("^/remote/.+(/user/.+)");
