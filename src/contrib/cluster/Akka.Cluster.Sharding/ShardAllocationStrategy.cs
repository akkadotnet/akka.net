...
///     <see cref="PersistentShardCoordinator" />.
/// </summary>
public interface IShardAllocationStrategy : INoSerializationVerificationNeeded
{
    ...
    /// </returns>
    Task<IActorRef> AllocateShard(IActorRef requester, ShardId shardId,
    ...
    Task<IImmutableSet<ShardId>> Rebalance(
        IImmutableDictionary<IActorRef, IImmutableList<ShardId>> currentShardAllocations,
        ...
        IImmutableSet<ShardId> rebalanceInProgress)
    {
        if (rebalanceInProgress.Count < _maxSimultaneousRebalance)
        {
            ...
            if (IsAGoodTimeToRebalance(sortedRegionEntries))
            {
                ...
                var mostShards =
                    sortedRegionEntries.Select(r => r.ShardIds.Where(s => !rebalanceInProgress.Contains(s)))
                        .MaxBy(i => i.Count())?.ToArray() ?? Array.Empty<string>();
                ...
                if (difference >= _rebalanceThreshold)
                {
                    ...
                    return Task.FromResult<IImmutableSet<ShardId>>(mostShards.OrderBy(i => i).Take(n)
                        .ToImmutableHashSet());
                }
            }
        }
