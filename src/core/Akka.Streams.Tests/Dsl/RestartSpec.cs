...
public class RestartSpec : AkkaSpec
{
        ...
        A_restart_with_backoff_source_should_reset_exponential_backoff_back_to_minimum_when_source_runs_for_at_least_minimum_backoff_without_completing()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            await Task.Delay(_minBackoff + TimeSpan.FromTicks(_minBackoff.Ticks * 2) + _minBackoff +
    ...
    [Fact]
    public async Task
        A_restart_with_backoff_source_should_stop_on_completion_if_it_should_only_be_restarted_in_failures()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var created = new AtomicCounter(0);
    ...
    [Fact]
    public async Task
        A_restart_with_backoff_source_should_restart_on_failure_when_only_due_to_failures_should_be_restarted()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var created = new AtomicCounter(0);
    ...
    [Fact]
    public async Task
        A_restart_with_backoff_source_should_reset_maxRestarts_when_source_runs_for_at_least_minimum_backoff_without_completing()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var created = new AtomicCounter(0);
        ...
        A_restart_with_backoff_source_should_allow_using_withMaxRestarts_instead_of_minBackoff_to_determine_the_maxRestarts_reset_time()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            await Task.Delay(_shortMinBackoff + TimeSpan.FromTicks(_shortMinBackoff.Ticks * 2) +
                             _shortMinBackoff); // if using shortMinBackoff as deadline cause reset
    ...
    public async Task A_restart_with_backoff_sink_should_restart_on_cancellation()
    {
        ...
        {
            var created = new AtomicCounter(0);
            var (queue, sinkProbe) = this.SourceProbe<string>().ToMaterialized(this.SinkProbe<string>(), Keep.Both)
                .Run(Materializer);
            var probe = this.SourceProbe<string>()
                .ToMaterialized(RestartSink.WithBackoff(() =>
                {
                    created.IncrementAndGet();
                    return Flow.Create<string>().TakeWhile(c => c != "cancel", true)
    ...
    [Fact]
    public async Task
        A_restart_with_backoff_sink_should_reset_maxRestarts_when_sink_runs_for_at_least_minimum_backoff_without_completing()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var created = new AtomicCounter(0);
            ...
            Thread.Sleep(_minBackoff + TimeSpan.FromTicks(_minBackoff.Ticks * 2) + _minBackoff +
                         TimeSpan.FromMilliseconds(500));
    ...
    /// </summary>
    private static Flow<TIn, TOut, NotUsed> RestartFlowFactory<TIn, TOut, TMat>(Func<Flow<TIn, TOut, TMat>> flowFactory,
        bool onlyOnFailures, RestartSettings settings)
    {
    ...
    private (AtomicCounter, TestPublisher.Probe<string>, TestSubscriber.Probe<string>, TestPublisher.Probe<string>,
        TestSubscriber.Probe<string>) SetupFlow(
            TimeSpan minBackoff,
            TimeSpan maxBackoff,
            int maxRestarts = -1,
    ...
    {
        var created = new AtomicCounter(0);
        var (flowInSource, flowInProbe) = this.SourceProbe<string>().ToMaterialized(this.SinkProbe<string>(), Keep.Both)
            .Run(Materializer);
        var (flowOutProbe, flowOutSource) = this.SourceProbe<string>()
            .ToMaterialized(BroadcastHub.Sink<string>(), Keep.Both).Run(Materializer);
        ...
        var (source, sink) = this.SourceProbe<string>().ViaMaterialized(RestartFlowFactory(() =>
                ...
                RestartSettings.Create(minBackoff, maxBackoff, 0).WithMaxRestarts(maxRestarts, minBackoff)), Keep.Left)
            ...
            var flow = RestartFlowFactory(() =>
                {
                    ...
                    return Flow.Create<int>()
                        .Select(i =>
                        {
                            if (i == 6) throw new ArgumentException("BOOM");
    ...
    [Fact]
    public async Task
        A_restart_with_backoff_flow_should_continue_running_flow_out_port_after_in_has_been_sent_completion()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
    ...
    public async Task A_restart_with_backoff_flow_should_not_restart_on_completion_when_maxRestarts_is_reached()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var (created, _, flowInProbe, flowOutProbe, sink) = SetupFlow(_shortMinBackoff, _shortMaxBackoff, 1);
    ...
    public async Task A_restart_with_backoff_flow_should_stop_on_cancellation_when_using_onlyOnFailuresWithBackoff()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var (created, source, flowInProbe, flowOutProbe, sink) =
                SetupFlow(_shortMinBackoff, _shortMaxBackoff, -1, true);
