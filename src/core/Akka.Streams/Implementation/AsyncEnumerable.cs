...
namespace Akka.Streams.Dsl
{
    ...
    public sealed class StreamsAsyncEnumerableRerunnable<T> : IAsyncEnumerable<T>
    {
        private readonly IMaterializer _materializer;
        private readonly IRunnableGraph<(UniqueKillSwitch, ISinkQueue<T>)> _graph;

        public StreamsAsyncEnumerableRerunnable(IRunnableGraph<(UniqueKillSwitch, ISinkQueue<T>)> graph, IMaterializer materializer)
        {
            _graph = graph;
        ...
        public IAsyncEnumerator<T> GetAsyncEnumerator(CancellationToken cancellationToken)
        {
            ...
            return new SinkQueueAsyncEnumerator<T>(_graph.Run(_materializer), cancellationToken);
        }
    }
    /// <summary>
