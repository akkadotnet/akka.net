...
public class AkkaProtocolSpec : AkkaSpec
{
    ...
    {
        return new InboundPayload(_codec.ConstructDisassociate(info));
    }

    private IHandleEvent TestAssociate(int uid)
    {
        return new InboundPayload(_codec.ConstructAssociate(new HandshakeInfo(_remoteAkkaAddress, uid)));
    }
    ...
    {
        public Collaborators(AssociationRegistry registry, TestTransport transport, TestAssociationHandle handle,
            TestFailureDetector failureDetector)
        {
    ...
    private class TestFailureDetector : FailureDetector
    {
        ...
        {
            Volatile.Write(ref _isAvailable, available);
        }
    ...
    public async Task ProtocolStateActor_must_register_itself_as_reader_on_injected_handles()
    {
        ...
        Sys.ActorOf(ProtocolStateActor.InboundProps(
            new HandshakeInfo(_localAddress, 42),
            collaborators.Handle,
            new ActorAssociationEventListener(TestActor),
            new AkkaProtocolSettings(_config),
            _codec,
    ...
    public async Task ProtocolStateActor_must_in_inbound_mode_accept_payload_after_Associate_PDU_received()
    {
        ...
        await AwaitConditionAsync(() => Task.FromResult(LastActivityIsHeartbeat(collaborators.Registry)),
            DefaultTimeout);

        reader.Tell(_testPayload, TestActor);
        await ExpectMsgAsync<InboundPayload>(inbound => { Assert.Equal(_testEnvelope, inbound.Payload); },
            hint: "expected InboundPayload");
    }

    [Fact]
    public async Task
        ProtocolStateActor_must_in_inbound_mode_disassociate_when_an_unexpected_message_arrives_instead_of_Associate()
    {
        var collaborators = GetCollaborators();
    ...
    public async Task ProtocolStateActor_must_in_outbound_mode_delay_readiness_until_handshake_finished()
    {
        ...
        await AwaitConditionAsync(() => Task.FromResult(LastActivityIsAssociate(collaborators.Registry, 42)),
            DefaultTimeout);
    ...
    [Fact]
    public async Task
        ProtocolStateActor_must_handle_correctly_when_the_handler_is_registered_only_after_the_association_is_already_closed()
    {
        var collaborators = GetCollaborators();
        collaborators.Transport.AssociateBehavior.PushConstant(collaborators.Handle);
    ...
    public async Task ProtocolStateActor_must_give_up_outbound_after_connection_timeout()
    {
        ...
        await Awaiting(async () => { await statusPromise.Task.WithTimeout(TimeSpan.FromSeconds(5)); }).Should()
