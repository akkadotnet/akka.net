...
{
    private Sequenced Msg(long seq)
    {
        return new Sequenced(seq, "msg" + seq);
    }
    ...
    public void SendBuffer_must_throw_exception_if_nonbuffered_sequence_number_is_NACKed()
    {
        ...
        XAssert.Throws<ResendUnfulfillableException>(
            () => b1.Acknowledge(new Ack(new SeqNo(2), new[] { new SeqNo(0) })));
    }

    [Fact]
    public void
        ReceiveBuffer_must_enqueue_message_in_buffer_if_needed_return_the_list_of_deliverable_messages_and_acks()
    {
    ...
    private sealed class Sequenced : IHasSequenceNumber
    {
    ...
    public void SendBuffer_and_ReceiveBuffer_must_correctly_cooperate_with_each_other()
    {
        ...
        //Dropping phase
        Debug.WriteLine("Starting unreliable delivery for {0} messages, with delivery probably P = {1}", msgCount,
            deliveryProbability);
