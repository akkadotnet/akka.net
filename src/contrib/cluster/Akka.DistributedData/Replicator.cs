...
///         You can use your own custom <see cref="IReplicatedData{T}" /> or <see cref="IDeltaReplicatedData{T,TDelta}" />
///         types,
...
///             <term>Maps</term>
///             <description>
///                 <see cref="ORDictionary{TKey,TValue}" />, <see cref="ORMultiValueDictionary{TKey,TValue}" />,
///                 <see cref="LWWDictionary{TKey,TValue}" />, <see cref="PNCounterDictionary{TKey}" />
///             </description>
///         </item>
///     </list>
///     <para>
...
///         <a href="http://hal.upmc.fr/docs/00/55/55/88/PDF/techreport.pdf">
...
///                 </term>
///                 <description>
///                     The value will immediately only be written to the local replica, and later disseminated with
///                     gossip.
///                 </description>
///             </item>
///             <item>
...
///                 </term>
///                 <description>
///                     The value will immediately be written to at least <see cref="WriteTo.Count" /> replicas, including
///                     the local replica.
///                 </description>
///             </item>
///             <item>
///                 <term>
///                     <see cref="WriteMajority" />
///                 </term>
///                 <description>
...
///                 </term>
///                 <description>
///                     The value will immediately be written to all nodes in the cluster (or all nodes in the cluster role
///                     group).
///                 </description>
///             </item>
///         </list>
...
///         In the <see cref="Update" /> message you can pass an optional request context, which the
///         <see cref="Replicator" />
...
///                 </term>
///                 <description>
///                     The value will be read and merged from <see cref="ReadFrom.N" /> replicas, including the
///                     local replica.
///                 </description>
///             </item>
///             <item>
///                 <term>
///                     <see cref="ReadMajority" />
///                 </term>
///                 <description>
///                     The value will be read and merged from a majority of replicas, i.e. at least `N/2 + 1` replicas,
///                     where N is the number of nodes in the cluster (or cluster role group).
///                 </description>
///             </item>
///             <item>
...
///                 </term>
///                 <description>
///                     The value will be read and merged from all nodes in the cluster (or all nodes in the
///                     cluster role group).
///                 </description>
///             </item>
///         </list>
...
///         local <see cref="Replicator" />. As reply of the <see cref="Delete" /> a <see cref="DeleteSuccess" /> is sent
///         to
...
///         Subsequent <see cref="Delete" />, <see cref="Update" /> and <see cref="Get" /> requests will be replied with
///         <see cref="DataDeleted" />.
...
///         For example a <see cref="GCounter" /> keeps track of one counter per node. If a <see cref="GCounter" /> has
///         been updated
...
///         cluster. Data types that need pruning have to implement <see cref="IRemovedNodePruning{T}" />. The pruning
///         consists
///         of several steps:
///         <list type="">
///             <item>
...
///                 <see cref="ReplicatorSettings.MaxPruningDissemination" /> duration has elapsed. The time measurement is
///                 stopped when any
...
///                 The leader initiates the pruning by adding a <see cref="PruningInitialized" /> marker in the data
///                 envelope.
///                 This is gossiped to all other nodes and they mark it as seen when they receive it.
///             </item>
///             <item>
///                 When the leader sees that all other nodes have seen the <see cref="PruningInitialized" /> marker
///                 the leader performs the pruning and changes the marker to <see cref="PruningPerformed" /> so that
///                 nobody
...
///                 the removed node to the leader node. For example, a <see cref="GCounter" /> is a `Map` with the node as
///                 key
///                 and the counts done by that node as value. When pruning the value of the removed node is
///                 moved to the entry owned by the leader node. See <see cref="IRemovedNodePruning{T}.Prune" />.
///             </item>
///             <item>
///                 Thereafter the data is always cleared from parts associated with the removed node so that
///                 it does not come back when merging. See <see cref="IRemovedNodePruning{T}.PruningCleanup" />
///             </item>
///             <item>
...
internal sealed class Replicator : UntypedActor, IWithUnboundedStash
{
    ...
    private static readonly Digest LazyDigest = ByteString.CopyFrom(0);
    private static readonly Digest NotFoundDigest = ByteString.CopyFrom(255);
    ...
    private long _allReachableClockTime;
    ...
    ///     The actual data.
    /// </summary>
    private ImmutableDictionary<string, (DataEnvelope envelope, Digest digest)> _dataEntries =
        ImmutableDictionary<string, (DataEnvelope, Digest)>.Empty;
    ...
    /// </summary>
    private ImmutableSortedSet<Member> _leader =
        ImmutableSortedSet<Member>.Empty.WithComparer(Member.LeaderStatusOrdering);
    ...
    private ImmutableSortedSet<Member>
        _membersByAge = ImmutableSortedSet<Member>.Empty.WithComparer(Member.AgeOrdering);
    ...
    public Replicator(ReplicatorSettings settings)
    {
        ...
        _gossipTask = Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(_settings.GossipInterval,
            _settings.GossipInterval, Self, GossipTick.Instance, Self);
        _notifyTask = Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(_settings.NotifySubscribersInterval,
            _settings.NotifySubscribersInterval, Self, FlushChanges.Instance, Self);
        _pruningTask = _settings.PruningInterval != TimeSpan.Zero
            ? Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(_settings.PruningInterval,
                _settings.PruningInterval, Self, RemovedNodePruningTick.Instance, Self)
            : null;
        _clockTask = Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(_settings.GossipInterval,
            _settings.GossipInterval, Self, ClockTick.Instance, Self);
        ...
        _deltaPropagationTask = Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(deltaPropagationInterval,
            deltaPropagationInterval, Self, DeltaPropagationTick.Instance, Self);
    ...
    {
        return Actor.Props.Create(() => new Replicator(settings)).WithDeploy(Deploy.Local)
            .WithDispatcher(settings.Dispatcher);
    }

    private bool IsKnownNode(Address node)
    {
        return _nodes.Contains(node) || _weaklyUpNodes.Contains(node) ||
    ...
    private IImmutableList<Address> NodesForReadWrite(bool excludeExiting)
    {
        if (excludeExiting && !_exitingNodes.IsEmpty)
        {
            if (_settings.PreferOldest)
                return _membersByAge.Where(i => !_exitingNodes.Contains(i.Address)).Select(i => i.Address)
                    .ToImmutableList();
    ...
    protected override SupervisorStrategy SupervisorStrategy()
    {
        return new OneForOneStrategy(e =>
        {
            ...
            return Actor.SupervisorStrategy.DefaultDecider.Decide(e);
        });
    }
        ...
        switch (message)
        {
            case IDestinationSystemUid msg:
                if (msg.ToSystemUid.HasValue && msg.ToSystemUid != _selfUniqueAddress.Uid)
                    ...
                    {
                        case Status s:
                            ReceiveStatus(s.Digests, s.Chunk, s.TotalChunks);
                            return true;
                        case Gossip g:
            ...
            case ISendingSystemUid msg:
                if (msg.FromNode != null && !IsKnownNode(msg.FromNode.Address))
                    ...
                    _log.Info("Ignoring message [{0}] from [{1}] unknown node [{2}]", Logging.SimpleName(msg), Sender,
                        msg.FromNode);
                else
                    switch (msg)
                    {
                        case Read r:
                            ...
                            return true;
                        case Write w:
                            ReceiveWrite(w.Key, w.Envelope);
                            return true;
                        case DeltaPropagation d:
                            ReceiveDeltaPropagation(msg.FromNode, d.ShouldReply, d.Deltas);
                            return true;
                    }
            ...
            case Get g:
                ReceiveGet(g.Key, g.Consistency, g.Request);
                return true;
            case Update msg:
                ReceiveUpdate(msg.Key, msg.Modify, msg.Consistency, msg.Request);
                return true;
            case ReadRepair rr:
                ReceiveReadRepair(rr.Key, rr.Envelope);
                return true;
            case FlushChanges _:
                ReceiveFlushChanges();
                return true;
            case DeltaPropagationTick _:
            ...
            case ClockTick c:
                ...
                return true;
            case Subscribe s:
                ReceiveSubscribe(s.Key, s.Subscriber);
                return true;
            case Unsubscribe u:
                ReceiveUnsubscribe(u.Key, u.Subscriber);
                return true;
            case Terminated t:
                ReceiveTerminated(t.ActorRef);
                return true;

            case ClusterEvent.MemberJoined m:
                ReceiveMemberJoining(m.Member);
                return true;
            case ClusterEvent.MemberWeaklyUp m:
                ReceiveMemberWeaklyUp(m.Member);
                return true;
            case ClusterEvent.MemberUp m:
                ReceiveMemberUp(m.Member);
                return true;
            case ClusterEvent.MemberExited m:
                ReceiveMemberExiting(m.Member);
                return true;
            case ClusterEvent.MemberRemoved m:
                ReceiveMemberRemoved(m.Member);
                return true;

            case ClusterEvent.IMemberEvent m:
                ReceiveOtherMemberEvent(m.Member);
                return true;
            case ClusterEvent.UnreachableMember u:
                ReceiveUnreachable(u.Member);
                return true;
            case ClusterEvent.ReachableMember r:
                ReceiveReachable(r.Member);
                return true;
            ...
            case GetKeyIds _:
                ...
                return true;
            case Delete d:
                ReceiveDelete(d.Key, d.Consistency, d.Request);
                return true;
            case RemovedNodePruningTick r:
    ...
    private void ReceiveGet(IKey key, IReadConsistency consistency, object req)
    {
        ...
        else
        {
            ...
            Context.ActorOf(ReadAggregator
                .Props(key, consistency, req, NodesForReadWrite(excludeExiting), _unreachable, !_settings.PreferOldest,
                    localValue, Sender)
    ...
    {
        return string.IsNullOrEmpty(_settings.Role) || m.HasRole(_settings.Role);
    }

    private void ReceiveUpdate(IKey key, Func<IReplicatedData, IReplicatedData> modify, IWriteConsistency consistency,
        object request)
    {
        var localValue = GetData(key.Id);
        ...
        try
        {
            ...
            // handle the delta
            if (delta != null) _deltaPropagationSelector.Update(key.Id, delta);
            ...
            if (IsLocalUpdate(consistency))
            {
                if (durable)
                {
                    var reply = new StoreReply(
                        new UpdateSuccess(key, request),
                        new StoreFailure(key, request),
                        Sender);
            ...
            else
            {
                ...
                var writeAggregator = Context.ActorOf(WriteAggregator
                    .Props(key, writeEnvelope, writeDelta, consistency, request, NodesForReadWrite(excludeExiting),
                        _unreachable, shuffle, Sender, durable)
                    .WithDispatcher(Context.Props.Dispatcher));
    ...
    {
        return _durableKeys.Contains(key) || (_durableWildcards.Count > 0 && _durableWildcards.Any(key.StartsWith));
    }
    ...
    {
        WriteAndStore(key, envelope, true);
    }
    ...
    private void WriteAndStore(string key, DataEnvelope writeEnvelope, bool reply)
    {
        ...
        {
            Sender.Tell(WriteNack.Instance);
        }
    }
    ...
    {
        WriteAndStore(key, writeEnvelope, false);
    ...
    private void ReceiveDelete(IKey key, IWriteConsistency consistency, object request)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                var writeAggregator = Context.ActorOf(WriteAggregator
                    .Props(key, DeletedEnvelope, null, consistency, request, NodesForReadWrite(excludeExiting),
                        _unreachable, !_settings.PreferOldest, Sender, durable)
                    .WithDispatcher(Context.Props.Dispatcher));
    ...
    private DataEnvelope SetData(string key, DataEnvelope envelope)
    {
        ...
        else if (newEnvelope.Data is DeletedData)
        {
        ...
        if (newEnvelope.Data is DeletedData) _deltaPropagationSelector.Delete(key);
    ...
    private bool IsNodeRemoved(UniqueAddress node, IEnumerable<string> keys)
    {
        ...
        return keys.Any(key =>
            _dataEntries.TryGetValue(key, out var tuple) && tuple.envelope.Pruning.ContainsKey(node));
    }
            ...
            if (IsNodeRemoved(from, deltas.Keys))
                ...
                foreach (var entry in deltas)
                {
                    ...
                    if (envelope.Data is IRequireCausualDeliveryOfDeltas)
                    {
                        ...
                        {
                            _log.Debug(
                                "Skipping DeltaPropagation from [{0}] for [{1}] because toSeqNr [{2}] already handled [{3}]",
                                from.Address, key, delta.FromSeqNr, currentSeqNr);
                            if (reply)
                                Sender.Tell(WriteAck.Instance);
                        }
                        ...
                        {
                            _log.Debug(
                                "Skipping DeltaPropagation from [{0}] for [{1}] because missing deltas between [{2}-{3}]",
                                from.Address, key, currentSeqNr + 1, delta.FromSeqNr - 1);
                            if (reply)
                        ...
                        {
                            _log.Debug(
                                "Applying DeltaPropagation from [{0}] for [{1}] with sequence numbers [{2}-{3}], current was [{4}]",
                                from.Address, key, delta.FromSeqNr, delta.ToSeqNr, currentSeqNr);
    ...
    private void GossipTo(Address address)
    {
        ...
        {
            var status = new Status(_dataEntries
                .ToImmutableDictionary(x => x.Key, y => GetDigest(y.Key)), 0, 1);
            to.Tell(status);
        }
        else
        {
            ...
            for (var i = 1; i <= Math.Min(totChunks, 10); i++)
            {
                ...
                var status = new Status(entries, chunk, totChunks);
                to.Tell(status);
            }
        }
    }
    ...
    {
        if (addresses.Count > 0) return addresses[ThreadLocalRandom.Current.Next(addresses.Count)];
        return null;
    }
    ...
    private void ReceiveStatus(IImmutableDictionary<string, ByteString> otherDigests, int chunk, int totChunks)
    {
        if (_log.IsDebugEnabled && _settings.VerboseDebugLogging)
            _log.Debug("Received gossip status from [{0}], chunk {1}/{2} containing [{3}]", Sender.Path.Address,
                chunk + 1, totChunks, string.Join(", ", otherDigests.Keys));
        ...
        if (!myMissingKeys.IsEmpty)
        {
            if (Context.System.Log.IsDebugEnabled)
                Context.System.Log.Debug("Sending gossip status to {0}, requesting missing {1}", Sender.Path.Address,
                    string.Join(", ", myMissingKeys));

            var status = new Status(myMissingKeys.ToImmutableDictionary(x => x, _ => NotFoundDigest), chunk, totChunks);
            Sender.Tell(status);
        }
    }
    ...
    {
        if (_log.IsDebugEnabled && _settings.VerboseDebugLogging)
            _log.Debug("Received gossip from [{0}], containing [{1}]", Sender.Path.Address,
                string.Join(", ", updatedData.Keys));
        ...
        foreach (var d in updatedData)
        {
            ...
            WriteAndStore(key, envelope, false);
            if (sendBack)
            {
        ...
        if (sendBack && replyData.Count != 0) Sender.Tell(new Gossip(replyData.ToImmutable(), false));
    }

    private void ReceiveSubscribe(IKey key, IActorRef subscriber)
    {
        if (!_newSubscribers.TryGetValue(key.Id, out var set)) _newSubscribers[key.Id] = set = new HashSet<IActorRef>();
        set.Add(subscriber);
    ...
    private void CollectRemovedNodes()
    {
        ...
        foreach (var pair in _dataEntries)
            if (pair.Value.envelope.Data is IRemovedNodePruning removedNodePruning)
                newRemovedNodes.UnionWith(removedNodePruning.ModifiedByNodes.Where(n =>
                    !(n == _selfUniqueAddress || knownNodes.Contains(n.Address))));
        ...
        foreach (var entry in _dataEntries)
        {
            ...
            if (envelope.Data is IRemovedNodePruning data)
                foreach (var entry2 in envelope.Pruning)
                    if (entry2.Value is PruningInitialized init && init.Owner == _selfUniqueAddress &&
                        (AllNodes.IsEmpty || AllNodes.IsSubsetOf(init.Seen)))
                    {
                        var removed = entry2.Key;
                        var isDurable = IsDurable(key);
                        var newEnvelope = envelope.Prune(removed,
                            isDurable ? durablePrunningPerformed : prunningPerformed);
    ...
    private sealed class ReplicatorDeltaPropagationSelector : DeltaPropagationSelector
    {
        ...
        protected override DeltaPropagation CreateDeltaPropagation(
            ImmutableDictionary<string, (IReplicatedData data, long from, long to)> deltas)
        {
            ...
            return new DeltaPropagation(_replicator._selfUniqueAddress, false, newDeltas);
        }
    }
