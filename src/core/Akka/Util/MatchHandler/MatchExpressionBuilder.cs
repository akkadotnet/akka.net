...
internal class MatchExpressionBuilder<T> : IMatchExpressionBuilder
{
    ...
    private static readonly ParameterExpression _extraArgsArrayParameter =
        Expression.Parameter(typeof(object[]), "extraArgsArray");
    ...
    public MatchExpressionBuilderResult BuildLambdaExpression(IReadOnlyList<TypeHandler> typeHandlers)
    {
        ...
        var castedInputVariables = new Dictionary<Type, ParameterExpression> { { _inputItemType, inputParameter } };

        //Loop over all handlers
        var numberOfTypeHandlers = typeHandlers.Count;
        for (var i = 0; i < numberOfTypeHandlers; i++)
        {
            ...
            if (handlesType == _inputItemType)
            {
                ...
                AddCodeForHandlers(typeHandler.Handlers, mainBodyExpressions, inputParameter, returnTarget,
                    inputParameter);
            }
            else
            {
                if (handlesType.IsValueType)
                {
                    ...
                    //Next we'll create the rest of the if-body, i.e. the handlers code
                    AddCodeForHandlers(typeHandler.Handlers, ifBodyExpressions, castedVariable, returnTarget,
                        inputParameter);
                ...
                else
                {
                    ...
                    if (!castedInputVariables.TryGetValue(handlesType, out var castedVariable))
                    {
                        ...
                        var assignment = Expression.Assign(castedVariable,
                            Expression.TypeAs(inputParameter, handlesType));
    ...
    private static (ParameterExpression[], object[]) DecorateHandlerAndPredicateExpressions(
        IReadOnlyList<Argument> arguments, ParameterExpression inputParameter)
    {
        ...
        var numberOfInlineParameters = arguments.Count;
        ...
        if (numberOfExtraArgs > 0)
        {
            ...
            for (var i = 0; i < numberOfExtraArgs; argsIndex++, i++)
            {
                ...
                var expression =
                    Expression.Convert(Expression.ArrayIndex(_extraArgsArrayParameter, Expression.Constant(i)),
    ...
    private static void AddCodeForHandlers(List<PredicateAndHandler> predicateAndHandlers,
        List<Expression> bodyExpressions, Expression castedValueExpression, LabelTarget returnTarget,
        Expression inputValueExpression)
    {
        foreach (var predicateAndHandler in predicateAndHandlers)
        {
            var valueExpression = predicateAndHandler.HandlerFirstArgumentShouldBeBaseType
            ...
            switch (predicateAndHandler.HandlerKind)
            {
                ...
                case HandlerKind.ActionWithPredicate:
                    AddActionHandlerWithPredicateExpressions(predicateAndHandler, bodyExpressions, valueExpression,
                        returnTarget);
                    break;
    ...
    private static void AddActionHandlerExpressions(PredicateAndHandler handler, List<Expression> body,
        Expression argumentExpression, LabelTarget returnTarget)
    {
    ...
    private static void AddActionHandlerWithPredicateExpressions(PredicateAndHandler handler, List<Expression> body,
        Expression argumentExpression, LabelTarget returnTarget)
    {
    ...
    private static void AddFuncHandlerExpressions(PredicateAndHandler handler, List<Expression> body,
        Expression argumentExpression, LabelTarget returnTarget)
    {
        //Add this code to the body:
        //    if(func(castedValue))
