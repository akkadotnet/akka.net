...
///     with the main purpose of crossing a network boundary. Usually obtaining a SinkRef would be done via Actor
///     messaging,
...
///     to share some data with it, and the remote one decides to do so in a back-pressured streaming fashion -- using a
///     stream ref.
...
public sealed class RemoteStreamRefActorTerminatedException : Exception
{
    internal static readonly RemoteStreamRefActorTerminatedException Default =
        new(
            "Remote target receiver of data terminated. Local stream terminating, message loss (on remote side) may have happened.");
