...
internal class ResizablePoolCell : RoutedActorCell
{
    ...
    private readonly AtomicCounterLong _resizeCounter;

    /// <summary>
    ///     must always use ResizeInProgressState static class to compare or assign values
    /// </summary>
    private readonly AtomicBoolean _resizeInProgress;
        ...
        : base(system, self, routerProps, dispatcher, routeeProps, supervisor)
    {
        if (pool.Resizer == null)
            throw new ArgumentException("RouterConfig must be a Pool with defined resizer", nameof(pool));
    ...
    protected override void PreSuperStart()
    {
        ...
        if (resizer.IsTimeForResize(_resizeCounter.GetAndIncrement())) Resize(true);
    }
