...
internal sealed class ClusterHeartbeatReceiver : UntypedActor
{
    ...
    protected override void OnReceive(object message)
    {
        switch (message)
        {
            case ClusterHeartbeatSender.Heartbeat hb:
                if (VerboseHeartbeat)
                    _cluster.CurrentInfoLogger.LogDebug(
                        $"Heartbeat from [{hb.From}] - Sequence number [{hb.SequenceNr.ToString(CultureInfo.InvariantCulture)}]");
    ...
    {
        return Actor.Props.Create(() => new ClusterHeartbeatReceiver(getCluster));
    }
}
...
internal class ClusterHeartbeatSender : ReceiveActor
{
    ...
    /// <summary>
    ///     Create a new instance of the <see cref="ClusterHeartbeatSender" /> and pass in a reference to the
    ///     <see cref="Cluster" />
    ///     to which it belongs.
    /// </summary>
    ...
    {
        foreach (var receiver in _state.ActiveReceivers) _failureDetector.Remove(receiver.Address);
    ...
    private void DoHeartbeatRsp(HeartbeatRsp rsp)
    {
        if (_cluster.Settings.VerboseHeartbeatLogging)
            _log.Debug(
                "Cluster Node [{0}] - Heartbeat response from [{1}] - Sequence number [{2}] - Creation time [{3}]",
                _cluster.SelfAddress, rsp.From.Address,
                rsp.SequenceNr.ToString(CultureInfo.InvariantCulture),
                new TimeSpan(rsp.CreationTimeNanos.ToTicks()).ToString());
    ...
    private void TriggerFirstHeart(UniqueAddress from)
    {
        ...
        {
            if (_cluster.Settings.VerboseHeartbeatLogging)
                _log.Debug("Cluster Node [{0}] - Trigger extra expected heartbeat from [{1}]", _cluster.SelfAddress,
                    from.Address);
...
internal sealed class ClusterHeartbeatSenderState
{
    ...
    public ClusterHeartbeatSenderState(HeartbeatNodeRing ring,
        ImmutableHashSet<UniqueAddress> oldReceiversNowUnreachable, IFailureDetectorRegistry<Address> failureDetector)
    {
        Ring = ring;
    ...
    ///     TBD
    /// </summary>
    public UniqueAddress SelfAddress => Ring.SelfAddress;
    ...
    {
        return Copy(Ring.Copy(nodes: nodes.Add(SelfAddress), unreachable: unreachable));
    }
    ...
    public ClusterHeartbeatSenderState HeartbeatRsp(UniqueAddress from)
    {
        if (ActiveReceivers.Contains(from))
        {
            ...
            if (OldReceiversNowUnreachable.Contains(from))
            {
                ...
                if (!Ring.MyReceivers.Value.Contains(from)) FailureDetector.Remove(from.Address);
    ...
    /// <param name="failureDetector">TBD</param>
    /// <returns>TBD</returns>
    public ClusterHeartbeatSenderState Copy(HeartbeatNodeRing? ring = null,
        ImmutableHashSet<UniqueAddress> oldReceiversNowUnreachable = null,
        IFailureDetectorRegistry<Address> failureDetector = null)
    {
        return new ClusterHeartbeatSenderState(ring ?? Ring, oldReceiversNowUnreachable ?? OldReceiversNowUnreachable,
            failureDetector ?? FailureDetector);
    }
}
...
internal struct HeartbeatNodeRing
{
    ...
    ///     This exception is thrown when the specified <paramref name="nodes" /> doesn't contain the specified
    ///     <paramref name="selfAddress" />.
    /// </exception>
        ...
        int monitoredByNumberOfNodes)
    {
        ...
        if (!nodes.Contains(selfAddress))
            throw new ArgumentException(
                $"Nodes [${string.Join(", ", nodes)}] must contain selfAddress [{selfAddress}]");
    ...
    {
        if (_useAllAsReceivers) return NodeRing.Remove(sender);
        ...
        (int, ImmutableSortedSet<UniqueAddress>) Take(int n, IEnumerator<UniqueAddress> iter,
            ImmutableSortedSet<UniqueAddress> acc, ImmutableHashSet<UniqueAddress> unreachable,
            int monitoredByNumberOfNodes)
        {
            while (true)
        ...
        var (remaining, slice1) = Take(MonitoredByNumberOfNodes, NodeRing.From(sender).Skip(1).GetEnumerator(),
            ImmutableSortedSet<UniqueAddress>.Empty, Unreachable, MonitoredByNumberOfNodes);

        IImmutableSet<UniqueAddress> slice = remaining == 0
            ? slice1 // or, wrap-around
            : Take(remaining, NodeRing.TakeWhile(x => x != sender).GetEnumerator(), slice1, Unreachable,
                MonitoredByNumberOfNodes).Item2;
    ...
    /// <param name="monitoredByNumberOfNodes">TBD</param>
    /// <returns>TBD</returns>
    public HeartbeatNodeRing Copy(UniqueAddress selfAddress = null, ImmutableHashSet<UniqueAddress> nodes = null,
        ImmutableHashSet<UniqueAddress> unreachable = null, int? monitoredByNumberOfNodes = null)
    {
