...
internal abstract class ReadWriteAggregator : ActorBase
{
    ...
    protected ReadWriteAggregator(IImmutableList<Address> nodes, IImmutableSet<Address> unreachable, TimeSpan timeout,
        bool shuffle)
    {
        Timeout = timeout;
        Nodes = nodes;
        ...
        _sendToSecondarySchedule =
            Context.System.Scheduler.ScheduleTellOnceCancelable((int)Timeout.TotalMilliseconds / 5, Self,
                SendToSecondary.Instance, Self);
        _timeoutSchedule =
            Context.System.Scheduler.ScheduleTellOnceCancelable(Timeout, Self, ReceiveTimeout.Instance, Self);
