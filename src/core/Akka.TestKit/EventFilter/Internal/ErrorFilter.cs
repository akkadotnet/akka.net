...
public class ErrorFilter : EventFilterBase
{
    ...
    ///     This exception is thrown when the specified <paramref name="exceptionType" /> does not implement
    ///     <see cref="Exception" />.
    /// </exception>
        ...
        : base(messageMatcher, sourceMatcher)
    {
        if (exceptionType != null && !exceptionType.Implements<Exception>())
            throw new ArgumentException($"The type must be an exception. It was: {exceptionType}",
    ...
    private bool IsMatch(string logSource, object errorMessage, Exception cause)
    {
        ...
        {
            if (!hasCause) return InternalDoMatch(logSource, errorMessage);
        }
        else
        {
