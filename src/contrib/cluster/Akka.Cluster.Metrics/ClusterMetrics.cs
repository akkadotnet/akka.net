...
public class ClusterMetrics : IExtension
{
    ...
    internal ClusterMetrics(ExtendedActorSystem system)
    {
        ...
        Supervisor = _system.SystemActorOf(
            Props.Create<ClusterMetricsSupervisor>().WithDispatcher(Settings.MetricsDispatcher)
    ...
    public ClusterMetricsStrategy Strategy =>
        DynamicAccess.CreateInstanceFor<ClusterMetricsStrategy>(Settings.SupervisorStrategyProvider,
                Settings.SupervisorStrategyConfiguration)
            .GetOrElse(() =>
            {
                _system.Log.Error(
    ...
    /// <summary>
    ///     Supervisor actor.
    ///     Accepts subtypes of <see cref="ClusterMetricsSupervisorMetadata.ICollectionControlMessage" /> to manage metrics
