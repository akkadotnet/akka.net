...
///     <ul>
///         <li>
///             taking exactly one argument of type <see cref="ExtendedActorSystem" />;
...
public abstract class Serializer
{
    ...
    {
        this.system = system;
        _value = new FastLazy<int>(
            () => SerializerIdentifierHelper.GetSerializerIdentifierFromConfig(GetType(), system));
    }
    ...
    {
        return SerializerErrorCode.GetErrorForSerializerId(id);
    }
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    {
        return (T)FromBinary(bytes, typeof(T));
    }
}
...
public static class SerializerIdentifierHelper
{
    ...
    ///     This exception is thrown if the system couldn't find the given serializer <paramref name="type" /> id in the
    ///     configuration.
    /// </exception>
    /// <returns>TBD</returns>
        ...
        */
        ...
        if (!identifiers.TryGetValue(type, out var value))
            throw new ArgumentException(
                $"Couldn't find serializer id for [{type}] under [{SerializationIdentifiers}] HOCON path",
                nameof(type));
