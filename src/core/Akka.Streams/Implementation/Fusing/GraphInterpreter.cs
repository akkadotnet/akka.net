...
///     <para />
///     - Boundary logics are attached via <see cref="AttachDownstreamBoundary(Connection,DownstreamBoundaryStageLogic)" />
///     and <see cref="AttachUpstreamBoundary(Connection,UpstreamBoundaryStageLogic)" />
///     <para />
///     - <see cref="Init" /> is called
///     <para />
///     - <see cref="Execute" /> is called whenever there is need for execution, providing an upper limit on the processed
///     events
///     <para />
///     - <see cref="Finish" /> is called before the interpreter is disposed, preferably after <see cref="IsCompleted" />
///     returned true, although
///     in abort cases this is not strictly necessary
///     The <see cref="Execute" /> method of the interpreter accepts an upper bound on the events it will process. After
///     this limit
///     is reached or there are no more pending events to be processed, the call returns. It is possible to inspect
///     if there are unprocessed events left via the <see cref="IsSuspended" /> method. <see cref="IsCompleted" /> returns
///     true once all stages
...
///     on the hot paths.
///     One of the basic abstractions inside the interpreter is the <see cref="Connection" />. A connection represents an
///     output-input port pair
///     (an analogue for a connected RS Publisher-Subscriber pair). The Connection object contains all the necessary data
///     for the interpreter
...
///     <para />
///     - inHandler contains the <see cref="InHandler" /> instance that handles the events corresponding
///     to the input port of the connection
///     <para />
///     - outHandler contains the <see cref="OutHandler" /> instance that handles the events corresponding
...
///     after a bounded number of other events. This property, together with suspendability means that even infinite cycles
///     can
...
#pragma warning disable CS0162 // Disabled since the flag can be set while debugging
    /// <summary>
    ///     Executes pending events until the given limit is met. If there were remaining events, <see cref="IsSuspended" />
    ///     will return true.
    /// </summary>
...
#pragma warning restore CS0162
    ...
    /// <summary>
    ///     Register that a connection in which the given stage participated has been completed and therefore the stage itself
    ///     might stop, too.
    /// </summary>
    ...
    private string NameIn(int port)
    {
        return Assembly.InletOwners[port] == Boundary ? "Out" + port : "N" + port;
    }

    private string NameOut(int port)
    {
        return Assembly.OutletOwners[port] == Boundary ? "Out" + port : "N" + port;
    }
    ...
    public abstract class UpstreamBoundaryStageLogic : GraphStageLogic
    {
        ...
        ///     TBD
        /// </summary>
        protected UpstreamBoundaryStageLogic() : base(0, 1)
        {
        }
    ...
    public abstract class DownstreamBoundaryStageLogic : GraphStageLogic
    {
        ...
        ///     TBD
        /// </summary>
        protected DownstreamBoundaryStageLogic() : base(1, 0)
        {
        }
    ...
    public sealed class Connection
    {
        ...
        /// </summary>
        /// <param name="id">
        ///     Identifier of the connection. Corresponds to the array slot in the <see cref="GraphAssembly" />
        /// </param>
        /// <param name="inOwnerId">
        ///     Identifier of the owner of the input side of the connection. Corresponds to the array slot in
        ///     the <see cref="GraphAssembly" />
        /// </param>
        /// <param name="inOwner">The stage logic that corresponds to the input side of the connection.</param>
        /// <param name="outOwnerId">
        ///     Identifier of the owner of the output side of the connection. Corresponds to the array slot in
        ///     the <see cref="GraphAssembly" />
        /// </param>
        ...
        public override string ToString()
        {
            return $"Connection({Id}, {PortState}, {Slot}, {InHandler}, {OutHandler})";
        }
    }
