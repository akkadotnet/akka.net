...
using Akka.Cluster.Metrics.Serialization.Proto;
using Akka.Dispatch;
...
public class ClusterMetricsMessageSerializer : SerializerWithStringManifest
{
        ...
        switch (obj)
        {
            ...
            default:
                throw new ArgumentException(
                    $"Can't serialize object of type ${obj.GetType().Name} in [${GetType().Name}]");
        }
    }
    ...
    public override object FromBinary(byte[] bytes, string manifest)
    {
        switch (manifest)
        {
            ...
            default:
                throw new ArgumentException(
                    $"Unimplemented deserialization of message with manifest [{manifest}] in [${GetType().Name}");
        }
    }
    ...
    private byte[] AdaptiveLoadBalancingPoolToBinary(AdaptiveLoadBalancingPool pool)
    {
        ...
        {
            NrOfInstances = (uint)pool.NrOfInstances, UsePoolDispatcher = pool.UsePoolDispatcher
        };
    ...
    {
        var proto = new Proto.MixMetricsSelector { Selectors = { selector.Selectors.Select(MetricsSelectorToProto) } };
        return proto.ToByteArray();
    }
    ...
    private Proto.MetricsGossipEnvelope MetricsGossipEnvelopeToProto(MetricsGossipEnvelope envelope)
    {
        ...
        var addressMapping = allAddresses.Select((a, i) => (Index: i, Value: a))
            .ToImmutableDictionary(p => p.Value, p => p.Index);
        var allMetricNames = allNodeMetrics.Aggregate(
            ImmutableHashSet<string>.Empty,
            (set, metrics) => set.Union(metrics.Metrics.Select(m => m.Name))).ToImmutableArray();
        var metricNamesMapping = allMetricNames.Select((a, i) => (Index: i, Value: a))
            .ToImmutableDictionary(p => p.Value, p => p.Index);

        int MapAddress(Address address)
        {
        ...
        {
            return MapWithErrorMessage(metricNamesMapping, name, "metric name");
        }

        Option<Proto.NodeMetrics.Types.EWMA> EwmaToProto(Option<NodeMetrics.Types.EWMA> ewma)
        {
            return ewma.Select(e => new Proto.NodeMetrics.Types.EWMA { Value = e.Value, Alpha = e.Alpha });
        }
        ...
        Proto.NodeMetrics.Types.Metric MetricToProto(NodeMetrics.Types.Metric m)
        {
            ...
            {
                NameIndex = MapName(m.Name), Number = NumberToProto(m.Value)
            };
    ...
    private MetricsGossipEnvelope MetricsGossipEnvelopeFromProto(Proto.MetricsGossipEnvelope envelope)
    {
        ...
        {
            return new NodeMetrics.Types.EWMA(ewma.Value, ewma.Alpha);
        }
            ...
            switch (number.Type)
            {
                ...
                case Proto.NodeMetrics.Types.NumberType.Serialized:
                    ...
                    throw new NotImplementedException(
                        $"{Proto.NodeMetrics.Types.NumberType.Serialized} number type is not supported");
                default:
        ...
        return new MetricsGossipEnvelope(AddressFromProto(envelope.From), new MetricsGossip(nodeMetrics),
            envelope.Reply);
    }
    ...
    private AdaptiveLoadBalancingPool AdaptiveLoadBalancingPoolFromBinary(byte[] bytes)
    {
        ...
        if (proto.MetricsSelector != null)
        {
            ...
            selector =
                _serialization.Value.Deserialize(s.Data.ToByteArray(), (int)s.SerializerId, s.Manifest) as
                    IMetricsSelector;
        }
        else
        {
        ...
        return new AdaptiveLoadBalancingPool(
            selector,
            (int)proto.NrOfInstances,
            null,
            !string.IsNullOrEmpty(proto.RouterDispatcher) ? proto.RouterDispatcher : Dispatchers.DefaultDispatcherId,
            proto.UsePoolDispatcher);
    }
    ...
    {
        return _serialization.Value.Deserialize(selector.Data.ToByteArray(), (int)selector.SerializerId,
            selector.Manifest) as IMetricsSelector;
    }
