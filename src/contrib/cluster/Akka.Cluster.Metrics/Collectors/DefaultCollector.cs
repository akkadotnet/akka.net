...
public class DefaultCollector : IMetricsCollector
{
    ...
    public NodeMetrics Sample()
    {
        using (var process = Process.GetCurrentProcess())
        {
            ...
            var availableMemory = NodeMetrics.Types.Metric.Create(StandardMetrics.MemoryAvailable,
                process.WorkingSet64 + process.PagedMemorySize64);
            if (availableMemory.HasValue)
                metrics.Add(availableMemory.Value);

            var processorCount =
                NodeMetrics.Types.Metric.Create(StandardMetrics.Processors, Environment.ProcessorCount);
    ...
    private (double ProcessUsage, double TotalUsage) GetCpuUsages(int currentProcessId)
    {
        ...
        try
        {
            var measureStartTime = TimeSpan.Zero;
            ...
            return (cpuUsagePercentages.GetValueOrDefault(currentProcessId, 0),
                cpuUsagePercentages.Values.DefaultIfEmpty().Sum());
        }
        finally
        {
