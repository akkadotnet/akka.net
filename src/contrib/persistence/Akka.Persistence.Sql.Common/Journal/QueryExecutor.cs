...
namespace Akka.Persistence.Sql.Common.Journal
{
    ...
    public interface IJournalQueryExecutor
    {
        ...
        /// provided <paramref name="tag"/>, within boundaries of <paramref name="fromSequenceNr"/> and 
        /// <paramref name="toSequenceNr"/>, up to <paramref name="max"/> number of elements.
        /// Returns highest sequence number from selected events.
        /// </summary>
        Task<long> SelectByTagAsync(DbConnection connection, CancellationToken cancellationToken, string tag, long fromSequenceNr, long toSequenceNr, long max, Action<ReplayedTaggedMessage> callback);
    ...
    {
        public readonly string TagPersistenceIdPrefix;
            ...
            TimeSpan timeout)
        {
            ...
            TagPersistenceIdPrefix = tagPersistenceIdPrefix;
        }
    ...
    public abstract class AbstractQueryExecutor : IJournalQueryExecutor
    {
        ...
        protected const int TagSequenceNrIndex = 6;
        ...
        protected AbstractQueryExecutor(QueryConfiguration configuration, Akka.Serialization.Serialization serialization, ITimestampProvider timestampProvider)
        {
            ...
            ByTagSql =
                $@"
                SELECT {allEventColumnNames}, tag.{Configuration.SequenceNrColumnName} as TagSequenceNr
                FROM {Configuration.FullJournalTableName} e
                JOIN {Configuration.FullJournalTableName} tag ON e.{Configuration.IdColumnName} = CAST(tag.{Configuration.PayloadColumnName} AS BIGINT)
                WHERE tag.{Configuration.PersistenceIdColumnName} = @Tag
                AND tag.{Configuration.SequenceNrColumnName} BETWEEN @FromSequenceNr AND @ToSequenceNr;";
        ...
        public virtual async Task<long> SelectByTagAsync(DbConnection connection, CancellationToken cancellationToken, string tag, long fromSequenceNr, long toSequenceNr, long max,
            Action<ReplayedTaggedMessage> callback)
        {
            using (var command = GetCommand(connection, ByTagSql))
            {
                AddParameter(command, "@Tag", DbType.String, Configuration.TagPersistenceIdPrefix + tag);
                AddParameter(command, "@FromSequenceNr", DbType.Int64, fromSequenceNr);
                AddParameter(command, "@ToSequenceNr", DbType.Int64, toSequenceNr);
                ...
                using (var reader = await command.ExecuteReaderAsync(cancellationToken))
                {
                    ...
                    {
                        var persistent = ReadEvent(reader);
                        var tagSequenceNr = reader.GetInt64(TagSequenceNrIndex);
                        maxSequenceNr = Math.Max(maxSequenceNr, persistent.SequenceNr);
                        callback(new ReplayedTaggedMessage(persistent, tag, tagSequenceNr));
                    }
        ...
        public virtual async Task InsertBatchAsync(DbConnection connection, CancellationToken cancellationToken, WriteJournalBatch write)
        {
            ...
            using (var tx = connection.BeginTransaction())
            {
                ...
                foreach (var entry in write.EntryTags)
                {
                    ...
                    WriteEvent(command, evt);
                    var result = await command.ExecuteScalarAsync(cancellationToken);
                    var eventId = Convert.ToInt64(result);

                    foreach (var tag in tags)
                    {
                        var tagPersistenceId = Configuration.TagPersistenceIdPrefix + tag.Tag;
                        AddParameter(command, "@PersistenceId", DbType.String, tagPersistenceId);
                        AddParameter(command, "@SequenceNr", DbType.Int64, tag.SequenceNr);
                        AddParameter(command, "@Timestamp", DbType.DateTime, TimestampProvider.GenerateTimestamp(evt));
                        AddParameter(command, "@IsDeleted", DbType.Boolean, false);
                        AddParameter(command, "@Manifest", DbType.String, LongTypeName);
                        AddParameter(command, "@Payload", DbType.Int64, eventId);
        ...
        protected virtual void WriteEvent(DbCommand command, IPersistentRepresentation e)
        {
            ...
            AddParameter(command, "@Timestamp", DbType.DateTime, TimestampProvider.GenerateTimestamp(e));
            AddParameter(command, "@IsDeleted", DbType.Boolean, false);
        ...
        private string HintToSql(IHint hint, DbCommand command)
        {
            if (hint is TimestampRange)
            {
                ...
                if (range.From.HasValue)
                {
                    ...
                    AddParameter(command, "@TimestampFrom", DbType.DateTime, range.From.Value);
                }
                ...
                if (range.To.HasValue)
                {
                    ...
                    AddParameter(command, "@TimestampTo", DbType.DateTime, range.To.Value);
                }
