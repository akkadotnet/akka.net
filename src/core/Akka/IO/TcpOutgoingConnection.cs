...
internal sealed class TcpOutgoingConnection : TcpConnection
{
            ...
            tcp.Settings.WriteCommandsQueueMaxSize >= 0 ? tcp.Settings.WriteCommandsQueueMaxSize : Option<int>.None)
    {
        ...
        foreach (var option in connect.Options) option.BeforeConnect(Socket);
    ...
    private void Stop(Exception cause)
    {
        ...
        StopWith(new CloseInformation(new HashSet<IActorRef>(new[] { _commander }),
            _connect.FailureMessage.WithCause(cause)));
    }
        ...
        ReportConnectFailure(() =>
        {
            if (_connect.RemoteAddress is DnsEndPoint remoteAddress)
            {
                ...
                if (resolved == null)
                    ...
                    Register(new IPEndPoint(resolved.Ipv4.FirstOrDefault(), remoteAddress.Port),
            ...
            if (message is Tcp.SocketConnected)
            {
                if (args.SocketError == SocketError.Success)
                {
                    ...
                    CompleteConnect(_commander, _connect.Options);
                         ...
                         fallbackAddress != null) // used only when we've resolved a DNS endpoint.
                {
                    var self = Self;
