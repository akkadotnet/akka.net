...
public class TestConductorSpec : MultiNodeSpec
{
    ...
    public void Support_Throttling_of_Network_Connections()
    {
        ...
        RunOn(
            () =>
            {
                TestConductor.Throttle(_config.Slave, _config.Master, ThrottleTransportAdapter.Direction.Send, 0.01f)
                    .Wait();
            }, _config.Master);
        ...
        RunOn(() =>
        {
            foreach (var i in Enumerable.Range(0, 10)) GetEchoActorRef().Tell(i);
        }, _config.Slave);
        ...
        RunOn(() =>
        {
            ...
            TestConductor.Throttle(_config.Slave, _config.Master, ThrottleTransportAdapter.Direction.Receive, 0.01F)
                .Wait();
        }, _config.Master);
        ...
        RunOn(() =>
        {
            foreach (var i in Enumerable.Range(10, 10)) GetEchoActorRef().Tell(i);
        }, _config.Slave);
        ...
        RunOn(
            () =>
            {
                TestConductor.Throttle(_config.Slave, _config.Master, ThrottleTransportAdapter.Direction.Receive, -1)
                    .Wait();
            }, _config.Master);
