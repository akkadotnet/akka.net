...
///         (optionally in addition to and isolated from the user stash) can create new stashes via
///         <see cref="StashFactory" />.
///     </para>
/// </summary>
public abstract class AbstractStash : IStash
{
    ...
    protected AbstractStash(IActorContext context)
    {
        ...
        Capacity = actorCell.Props.Deploy.StashCapacity == Deploy.NoStashSize
    ...
    ///     This exception is thrown if we attempt to stash the same message more than
    ///     once.
    /// </exception>
    /// <exception cref="StashOverflowException">
    ///     This exception is thrown in the event that we're using a <see cref="BoundedMessageQueue" /> for the
    ///     <see cref="IStash" /> and we've exceeded capacity.
    /// </exception>
    public void Stash()
    {
        ...
        if (Capacity <= 0 || _theStash.Count < Capacity)
            ...
            throw new StashOverflowException(
                $"Couldn't enqueue message {currMsg} from ${sender} to stash of {_actorCell.Self}");
    }
    ...
    public void Prepend(IEnumerable<Envelope> envelopes)
    {
        ...
        foreach (var envelope in envelopes.Distinct().Reverse()) _theStash.AddFirst(envelope);
    }
