...
public class TestLeaseActor : ActorBase
{
            ...
            case ActionRequest ar:
                ...
                else
                {
                    throw new InvalidOperationException(
                        $"unknown request to action: {ar.Request}. Requests: {string.Join(", ", _requests.Select(i => $"([{i.Item1}],[{i.Item2}])"))}");
                }
    ...
    public sealed class Acquire : ILeaseRequest, IEquatable<Acquire>
    {
        ...
        {
            return obj is Acquire a && Equals(a);
        }
    ...
    public sealed class Release : ILeaseRequest, IEquatable<Release>
    {
        ...
        public override bool Equals(object obj)
        {
            return obj is Release r && Equals(r);
        }
    ...
    public sealed class Create : ILeaseRequest, IEquatable<Create>
    {
        ...
        public override bool Equals(object obj)
        {
            return obj is Create c && Equals(c);
        }
        ...
        public override string ToString()
        {
            return $"Create({LeaseName}, {OwnerName})";
        }
    }
    ...
    public sealed class LeaseRequests
    {
        ...
        public override string ToString()
        {
            return $"LeaseRequests({string.Join(", ", Requests.Select(i => i.ToString()))})";
        }
    }
    ...
    public sealed class ActionRequest // boolean of Failure
    {
        ...
        public override string ToString()
        {
            return $"ActionRequest({Request}, {Result})";
        }
    }
}
...
public class TestLeaseActorClientExt : IExtension
{
    ...
    private readonly AtomicReference<IActorRef> leaseActor = new();
...
public class TestLeaseActorClient : Lease
{
    ...
    private readonly IActorRef leaseActor;
