...
public sealed class ByteString : IEquatable<ByteString>, IEnumerable<byte>
{
    ...
    public byte this[int index]
    {
        ...
        {
            if (index >= Count)
                throw new IndexOutOfRangeException("Requested index is outside of the bounds of the ByteString");
            int j;
    ...
    public IEnumerator<byte> GetEnumerator()
    {
        foreach (var buffer in _buffers)
            for (var i = buffer.Offset; i < buffer.Offset + buffer.Count; i++)
    ...
    public ByteString Slice(int index, int count)
    {
        ...
        if (index + count > Count)
            throw new ArgumentOutOfRangeException(nameof(count),
                "Index + count is outside of the bounds of the ByteString");
    ...
    private int GetBufferFittingIndex(int index, out int indexWithinBuffer)
    {
        ...
        throw new IndexOutOfRangeException(
            $"Requested index [{index}] is outside of the bounds of current ByteString.");
    }
    ...
    public int IndexOf(byte b, int from)
    {
        ...
        for (; i < _buffers.Length; i++)
        {
            ...
            for (; j < buffer.Count; j++, idx++)
                if (buffer.Array[buffer.Offset + j] == b)
                    return idx;
            j = 0;
        }
    ...
    public ByteString Concat(ByteString other)
    {
        ...
        Array.Copy(_buffers, 0, array, 0, len1);
    ...
    {
        return CopyTo(ref buffer, 0, buffer.Length);
    }
    ...
    public void WriteTo(Stream stream)
    {
        ...
        foreach (var buffer in _buffers) stream.Write(buffer.Array, buffer.Offset, buffer.Count);
    }
    ...
    public async Task WriteToAsync(Stream stream)
    {
        ...
        foreach (var buffer in _buffers) await stream.WriteAsync(buffer.Array, buffer.Offset, buffer.Count);
    }
    ...
    {
        var hashCode = 0;
        foreach (var b in this) hashCode = (hashCode * 397) ^ b.GetHashCode();
        return hashCode;
    }
    ...
    {
        return CopyFrom(buffer.Array, buffer.Offset, buffer.Count);
    }
    ...
    {
        return CopyFrom(memory, 0, memory.Length);
    }
    ...
    {
        return CopyFrom(span, 0, span.Length);
    }
    ...
    {
        return FromBytes(buffer.Array, buffer.Offset, buffer.Count);
    }
