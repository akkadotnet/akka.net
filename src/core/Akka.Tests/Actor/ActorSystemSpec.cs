...
public class ActorSystemSpec : AkkaSpec
{
    ...
    public void Reject_invalid_names()
    {
        new List<string>
        {
            ...
            "hallo%welt",
            "hallo/welt"
        }.ForEach(n => { XAssert.Throws<ArgumentException>(() => ActorSystem.Create(n)); });
    }
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
    ...
    public void Support_using_a_custom_scheduler()
    {
        var actorSystem = ActorSystem.Create(Guid.NewGuid().ToString(),
            DefaultConfig.WithFallback(
                "akka.scheduler.implementation = \"Akka.Tests.Actor.TestScheduler, Akka.Tests\""));
    ...
    public async Task Allow_configuration_of_guardian_supervisor_strategy()
    {
        ...
        var a = system.ActorOf(actor => { actor.Receive<string>((_, _) => { throw new Exception("Boom"); }); });
    ...
    {
        var config = ConfigurationFactory
            .ParseString("akka.actor.guardian-supervisor-strategy=\"Akka.Tests.Actor.TestStrategy, Akka.Tests\"")
            .WithFallback(DefaultConfig);
