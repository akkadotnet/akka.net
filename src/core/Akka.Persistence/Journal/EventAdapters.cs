...
/// <summary>
///     <para>
///         An <see cref="IEventAdapter" /> is both a <see cref="IWriteEventAdapter" /> and a
///         <see cref="IReadEventAdapter" />.
///         Facility to convert from and to specialised data models, as may be required by specialized persistence
///         Journals.
///     </para>
...
public interface IWriteEventAdapter
{
    ...
    ///         may require adapters to return "EventWithPrimaryKey(event, key)".
    ///     </para>
    ///     <para>
    ///         The <see cref="ToJournal" /> adaptation must be an 1-to-1 transformation.
...
public interface IReadEventAdapter
{
    ...
    ///         <see cref="PersistentActor" />.
    ///         Use the specialised <see cref="EventSequence.Single" /> method to emit exactly one event,
    ///         or <see cref="EventSequence.Empty" /> in case the adapter is not handling this event. Multiple
    ///         <see cref="IEventAdapter" /> instances are
    ///         applied in order as defined in configuration and their emitted event seqs are concatenated and delivered in
    ///         order
    ...
    /// <param name="manifest">
    ///     optionally provided manifest(type hint) in case the Adapter has stored one for this event. Use
...
public class EventAdapters
{
    ...
    protected EventAdapters(ConcurrentDictionary<Type, IEventAdapter> map,
        IEnumerable<KeyValuePair<Type, IEventAdapter>> bindings, ILoggingAdapter log)
    {
        _map = map;
        _bindings = bindings;
        _log = log;
    ...
    private static EventAdapters Create(ExtendedActorSystem system, IDictionary<string, string> adapters,
        IDictionary<string, string[]> adapterBindings)
    {
        ...
        foreach (var boundAdapter in kv.Value)
            if (!adapterNames.Contains(boundAdapter))
                throw new ArgumentException(string.Format(
                    "{0} was bound to undefined event-adapter: {1} (bindings: [{2}], known adapters: [{3}])",
        ...
        foreach (var pair in bindings) backing.AddOrUpdate(pair.Key, pair.Value, (_, _) => pair.Value);
    ...
    private static IEventAdapter CombineAdapters(IEnumerable<IEventAdapter> adapters)
    {
        ...
        if (writeAdapters.Count() == 1)
            return new ReadWriteEventAdapter(
                new CombinedReadEventAdapter(adapters.Where(a => a is NoopWriteEventAdapter)), writeAdapters.First());
    ...
    public virtual IEventAdapter Get(Type type)
    {
        ...
        var value = !pair.Equals(default(KeyValuePair<Type, IEventAdapter>))
            ? pair.Value
            : IdentityEventAdapter.Instance;
    ...
    private static T Instantiate<T>(string qualifiedName, ExtendedActorSystem system)
    {
        ...
        if (!typeof(T).IsAssignableFrom(type))
            throw new ArgumentException(string.Format(
                "Couldn't create instance of [{0}] from provided qualified type name [{1}], because it's not assignable from it",
    ...
    private static IDictionary<string, string[]> ConfigToListMap(Config config, string path)
    {
        if (config.HasPath(path))
        {
            ...
            return hoconObject.Unwrapped.ToDictionary(kv => kv.Key, kv =>
            {
                ...
                return new[] { kv.Value.ToString().Trim('"') };
            });
        }
