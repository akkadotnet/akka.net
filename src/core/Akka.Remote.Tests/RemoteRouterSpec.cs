...
".Replace("${masterSysName}", "Master" + _sysName).Replace("${sysName}", _sysName).Replace("${port}", _port.ToString()))
    ...
    [Fact]
    public async Task
        RemoteRouter_must_deploy_dynamic_resizable_number_of_children_on_remote_host_driven_by_configuration()
    {
        var probe = CreateTestProbe(_masterSystem);
    ...
    public async Task RemoteRouter_must_load_settings_from_config_for_local_child_router_of_system_actor()
    {
        ...
        var probe = CreateTestProbe(_masterSystem);
        var parent =
            ((ExtendedActorSystem)_masterSystem).SystemActorOf(FromConfig.Instance.Props(Props.Create<Parent>()),
                "sys-parent");
    ...
    private class Parent : UntypedActor
    {
        protected override void OnReceive(object message)
        {
            if (message is ValueTuple<Props, string> tuple) Sender.Tell(Context.ActorOf(tuple.Item1, tuple.Item2));
        }
    }
