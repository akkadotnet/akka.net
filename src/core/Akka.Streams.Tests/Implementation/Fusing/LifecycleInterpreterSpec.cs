...
public class LifecycleInterpreterSpec : GraphInterpreterSpec
{
    ...
    public void Interpreter_must_call_PreStart_in_order_on_stages()
    {
        var ops = new[]
        {
            new PreStartAndPostStopIdentity<string>(() => TestActor.Tell("start-a")),
            new PreStartAndPostStopIdentity<string>(() => TestActor.Tell("start-b")),
            new PreStartAndPostStopIdentity<string>(() => TestActor.Tell("start-c"))
        };
    ...
    public void Interpreter_must_call_PostStop_in_order_on_stages_when_upstream_completes()
    {
        var ops = new[]
        {
            new PreStartAndPostStopIdentity<string>(onUpstreamCompleted: () => TestActor.Tell("complete-a"),
                onStop: () => TestActor.Tell("stop-a")),
            new PreStartAndPostStopIdentity<string>(onUpstreamCompleted: () => TestActor.Tell("complete-b"),
                onStop: () => TestActor.Tell("stop-b")),
            new PreStartAndPostStopIdentity<string>(onUpstreamCompleted: () => TestActor.Tell("complete-c"),
    ...
    {
        var op = new PreStartAndPostStopIdentity<string>(() => TestActor.Tell("start-a"),
            () => TestActor.Tell("stop-a"));
    ...
    {
        var op = new PreStartFailer<string>(() => { throw new TestException("Boom!"); });
    ...
    {
        var op = new PostStopFailer<string>(() => { throw new TestException("Boom!"); });
    ...
    public void Interpreter_must_call_OnError_when_PreStart_fails_with_stages_after()
    {
        var ops = new IGraphStageWithMaterializedValue<FlowShape<string, string>, object>[]
        {
            ...
            new PreStartFailer<string>(() => { throw new TestException("Boom!"); }),
            new Select<string, string>(x => x)
        };
    ...
    [Fact]
    public void
        Interpreter_must_call_PostStop_when_PushAndFinish_called_with_PushAndFinish_if_indirect_upsteam_completes_with_PushAndFinish()
    {
        var ops = new IGraphStageWithMaterializedValue<FlowShape<string, string>, object>[]
        {
            new Select<string, string>(x => x), new PushFinishStage<string>(() => TestActor.Tell("stop")),
            new Select<string, string>(x => x)
        };
    ...
    [Fact]
    public void
        Interpreter_must_call_PostStop_when_PushAndFinish_called_with_PushAndFinish_if_upsteam_completes_with_PushAndFinish_and_downstream_immediately_pulls()
    {
        var ops = new IGraphStageWithMaterializedValue<FlowShape<string, string>, object>[]
        {
    ...
    private sealed class PreStartAndPostStopIdentity<T> : SimpleLinearGraphStage<T>
    {
        ...
        {
            return "PreStartAndPostStopIdentity";
        }
