...
public class HubSpec : AkkaSpec
{
    ...
    public async Task MergeHub_must_work_in_the_happy_case()
    {
        ...
        {
            var (sink, task) = MergeHub.Source<int>(16).Take(20).ToMaterialized(Sink.Seq<int>(), Keep.Both)
                .Run(Materializer);
    ...
    public async Task MergeHub_must_notify_existing_producers_if_consumer_cancels_after_a_few_elements()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var (sink, task) = MergeHub.Source<int>(16).Take(5).ToMaterialized(Sink.Seq<int>(), Keep.Both)
                .Run(Materializer);
            var upstream = this.CreatePublisherProbe<int>();
    ...
    [Fact(Skip =
        "Very racy")] // @Aaronontheweb - I believe the issue here is a genuine race condition with the hub itself. Messages start getting read before any subscribers are attached. Very tricky N+1 error.
    ...
    public async Task BroadcastHub_must_properly_signal_error_to_consumers_arriving_after_producer_finished()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            await Awaiting(async () => { await source.RunWith(Sink.Seq<int>(), Materializer); }).Should()
                .ThrowAsync<TestException>().ShouldCompleteWithin(3.Seconds());
        }, Materializer);
    }
    ...
    public async Task PartitionHub_must_ensure_that_from_two_different_speed_consumers_the_slower_controls_the_rate()
    {
        ...
        {
            var (firstElement, source) = Source.Maybe<int>()
                .ConcatMaterialized(Source.From(Enumerable.Range(1, 19)), Keep.Left)
            ...
            (await f2.ShouldCompleteWithin(3.Seconds())).Should()
                .BeEquivalentTo(Enumerable.Range(1, 19).Where(v => v % 2 != 0));
        }, Materializer);
    }
    ...
    public async Task PartitionHub_must_remember_completion_for_materialization_after_completion()
    {
        ...
        {
            var (sourceProbe, source) = this.SourceProbe<NotUsed>()
                .ToMaterialized(PartitionHub.Sink<NotUsed>((_, _) => 0, 0), Keep.Both)
                .Run(Materializer);
            var sinkProbe = source.RunWith(this.SinkProbe<NotUsed>(), Materializer);
    ...
    public async Task PartitionHub_must_properly_signal_error_to_consumer_arriving_after_producer_finished()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            await Awaiting(async () => { await source.RunWith(Sink.Seq<int>(), Materializer); }).Should()
                .ThrowAsync<TestException>().WithMessage("Fail!").ShouldCompleteWithin(3.Seconds());
        }, Materializer);
    }
}