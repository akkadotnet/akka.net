...
public sealed class CoordinatedShutdown : IExtension
{
    ...
    private readonly AtomicBoolean _clrHooksStarted = new();
    ...
    private volatile bool _runningClrHook;
    ...
    /// <summary>
    ///     The <see cref="Reason" /> for the shutdown as passed to the <see cref="Run(Reason, string)" /> method.
    ///     <see langword="null" /> if the shutdown
    ...
    public void AddTask(string phase, string taskName, Func<Task<Done>> task)
    {
        if (!_knownPhases.Contains(phase))
            throw new ConfigurationException(
                $"Unknown phase [{phase}], known phases [{string.Join(",", _knownPhases)}]. " +
    ...
    {
        if (!_clrHooksStarted.Value) _clrShutdownTasks.TryAdd(hook);
    }
    ...
    /// <returns>
    ///     A task that is completed when all such tasks have been completed, or
    ///     there is failure when <see cref="Phase.Recover" /> is disabled.
    /// </returns>
    /// <remarks>
    ...
    public Task<Done> Run(Reason reason, string fromPhase = null)
    {
        if (_runStarted.CompareAndSet(null, reason))
        {
            ...
            Task<Done> Loop(List<string> remainingPhases)
            {
                ...
                {
                    if (debugEnabled)
                        Log.Debug("Performing phase [{0}] with [{1}] tasks: [{2}]", phase, phaseTasks.Count,
                            string.Join(",", phaseTasks.Select(x => x.Item1)));
                    ...
                    try
                    {
                        ...
                        {
                            if (phase == PhaseActorSystemTerminate && MonotonicClock.ElapsedHighRes < deadLine)
                            ...
                            return FromException<Done>(
                                new TimeoutException($"Coordinated shutdown phase[{phase}] timed out after {timeout}"));
                        });
                    }
    ...
    internal static List<string> TopologicalSort(Dictionary<string, Phase> phases)
    {
        ...
        while (unmarked.Any()) DepthFirstSearch(unmarked.Head());
        ...
        if (terminateActorSystem || exitClr)
            coord.AddTask(PhaseActorSystemTerminate, "terminate-system", () =>
            {
                if (exitClr && terminateActorSystem)
                {
                    ...
                    Task.Run(() =>
                    {
                        if (!system.WhenTerminated.Wait(timeout) && !coord._runningClrHook) Environment.Exit(0);
                    });
                }
                ...
                if (terminateActorSystem)
                    ...
                    {
                        if (exitClr && !coord._runningClrHook) Environment.Exit(0);
    ...
    internal static void InitClrHook(ActorSystem system, Config conf, CoordinatedShutdown coord)
    {
        ...
        if (runByClrShutdownHook)
        {
            ...
            system.WhenTerminated.ContinueWith(_ => { AppDomain.CurrentDomain.ProcessExit -= exitTask; });
