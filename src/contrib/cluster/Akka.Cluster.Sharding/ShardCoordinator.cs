...
internal class ShardCoordinator
{
    ...
    private bool _allRegionsRegistered;
    ...
    private ImmutableDictionary<string, ImmutableHashSet<IActorRef>> _rebalanceInProgress =
        ImmutableDictionary<string, ImmutableHashSet<IActorRef>>.Empty;
    ...
    private ImmutableDictionary<string, ICancelable>
        _unAckedHostShards = ImmutableDictionary<string, ICancelable>.Empty;

    private bool _waitingForLocalRegionToTerminate;
    ...
    )
    {
        ...
        _rebalanceTask = context.System.Scheduler.ScheduleTellRepeatedlyCancelable(
            Settings.TuningParameters.RebalanceInterval, Settings.TuningParameters.RebalanceInterval, context.Self,
            RebalanceTick.Instance, ActorRefs.NoSender);

        _cluster.Subscribe(context.Self, ClusterEvent.SubscriptionInitialStateMode.InitialStateAsEvents,
    ...
    internal bool Active(object message)
    {
        switch (message)
        {
            case Register m:
                ...
                {
                    Log.Debug(
                        "{0}: ShardRegion [{1}] was not registered since the coordinator currently does not know about a node of that region",
                        TypeName,
                        m.ShardRegion);
                }
            ...
            case GetShardHome m:
                if (!HandleGetShardHome(m.Shard))
                {
                    ...
                    var activeRegions = State.Regions.RemoveRange(_gracefulShutdownInProgress)
                        .RemoveRange(_regionTerminationInProgress);
            ...
            case RebalanceTick _:
                ...
                {
                    var shardsTask =
                        AllocationStrategy.Rebalance(State.Regions, _rebalanceInProgress.Keys.ToImmutableHashSet());
                    ...
                    if (State.Regions.TryGetValue(m.ShardRegion, out var shards))
                    {
                        if (Log.IsDebugEnabled)
                        {
                            if (VerboseDebug)
                                ...
                                Log.Debug("{0}: Graceful shutdown of region [{1}] with [{2}] shards", TypeName,
                                    m.ShardRegion, shards.Count);
                        }
            ...
            case GetClusterShardingStats m:
                ...
                var tasks = _aliveRegions.Select(regionActor => (Region: regionActor,
                        Task: regionActor.Ask<ShardRegionStats>(GetShardRegionStats.Instance, m.Timeout)))
                    .ToImmutableList();
                Task.WhenAll(tasks.Select(i => i.Task)).ContinueWith(ps =>
                {
                    ...
                    {
                        var regionAddress = i.Region.Path.Address;
                        var address = regionAddress.HasLocalScope && regionAddress.System == _cluster.SelfAddress.System
            ...
            case GetCurrentRegions _:
                var reply = new CurrentRegions(State.Regions.Keys.Select(r =>
                        string.IsNullOrEmpty(r.Path.Address.Host) ? _cluster.SelfAddress : r.Path.Address)
                    .ToImmutableHashSet());
    ...
    {
        if (_aliveRegions.Any(i => i.Path.Address.HasLocalScope) ||
            _gracefulShutdownInProgress.Any(i => i.Path.Address.HasLocalScope))
        {
    ...
    private void ClearRebalanceInProgress(string shard)
    {
        if (_rebalanceInProgress.TryGetValue(shard, out var pendingGetShardHome))
        {
            ...
            foreach (var getShardHomeSender in pendingGetShardHome) _context.Self.Tell(msg, getShardHomeSender);
    ...
    private void DeferGetShardHomeRequest(ShardId shard, IActorRef from)
    {
        Log.Debug(
            ...
            shard,
            from);
        _rebalanceInProgress = _rebalanceInProgress.SetItem(shard,
            _rebalanceInProgress.GetValueOrDefault(shard, ImmutableHashSet<IActorRef>.Empty).Add(from));
    }
    ...
    internal bool HandleGetShardHome(ShardId shard)
    {
        ...
        {
            Log.Debug(
                "{0}: GetShardHome [{1}] request from [{2}] ignored, because not all regions have registered yet.",
                TypeName,
                shard,
                _context.Sender);
            return true;
        ...
        switch (message)
        {
            case Terminated t:
                if (State.Regions.ContainsKey(t.ActorRef))
                {
                    ...
                    {
                        _context.System.Scheduler.ScheduleTellOnce(_removalMargin, _context.Self,
                            new DelayedShardRegionTerminated(t.ActorRef), ActorRefs.NoSender);
    ...
    internal void WatchStateActors()
    {
        ...
        // amount of lost messages during startup.
        _context.System.Scheduler.ScheduleTellOnce(TimeSpan.FromMilliseconds(500), _context.Self,
    ...
    private bool HasAllRegionsRegistered()
    {
        ...
        if (_allRegionsRegistered) return true;
    ...
    private void RegionTerminated(IActorRef @ref)
    {
        ...
        if (State.Regions.TryGetValue(@ref, out var shards))
        {
            ...
            foreach (var s in shards) _context.Self.Tell(new GetShardHome(s), _ignoreRef);
    ...
    private void RegionProxyTerminated(IActorRef @ref)
    {
        ...
        if (State.RegionProxies.Contains(@ref))
        {
            ...
            Update(new ShardRegionProxyTerminated(@ref), evt => { State = State.Updated(evt); });
        }
    }
    ...
    private void SendHostShardMsg(ShardId shard, IActorRef region)
    {
        ...
        var cancel = _context.System.Scheduler.ScheduleTellOnceCancelable(Settings.TuningParameters.ShardStartTimeout,
            _context.Self, new ResendShardHost(shard, region), ActorRefs.NoSender);
    ...
    private void ContinueGetShardHome(ShardId shard, IActorRef region, IActorRef getShardHomeSender)
    {
        ...
        else
        {
            ...
            {
                if (State.Regions.ContainsKey(region) && !_gracefulShutdownInProgress.Contains(region) &&
                    !_regionTerminationInProgress.Contains(region))
                {
                ...
                else
                {
                    if (VerboseDebug)
                        Log.Debug(
                            "{0}: Allocated region [{1}] for shard [{2}] is not (any longer) one of the registered regions: {3}",
                            TypeName,
                            region,
                            shard,
                            State);
                        ...
                        Log.Debug(
                            "{0}: Allocated region [{1}] for shard [{2}] is not (any longer) one of the registered regions.",
                            TypeName,
                            region,
                            shard);
                }
        ...
        foreach (var shard in shards)
            ...
            if (!_rebalanceInProgress.ContainsKey(shard))
            {
                if (State.Shards.TryGetValue(shard, out var region))
                {
                    ...
                    StartShardRebalanceIfNeeded(shard, region, Settings.TuningParameters.HandOffTimeout, true);
                }
                else
                {
    ...
    private void ShutdownShards(IActorRef shuttingDownRegion, IImmutableSet<ShardId> shards)
    {
        ...
        foreach (var shard in shards)
            StartShardRebalanceIfNeeded(shard, shuttingDownRegion, Settings.TuningParameters.HandOffTimeout, false);
    }
    ...
    internal class RebalanceWorker : ActorBase, IWithTimers
    {
            ...
            bool isRebalance)
        {
            ...
            foreach (var region in _remaining) region.Tell(new BeginHandOff(shard));
            ...
            bool isRebalance)
        {
            return Actor.Props.Create(() =>
                new RebalanceWorker(typeName, shard, shardRegionFrom, handOffTimeout, regions, isRebalance));
        }
            ...
            switch (message)
            {
                ...
                case ShardRegionTerminated t:
                    if (_remaining.Contains(t.Region))
                        Log.Debug(
                            "{0}: ShardRegion [{1}] terminated while waiting for BeginHandOffAck for shard [{2}].",
                            _typeName, t.Region, _shard);
                    Acked(t.Region);
                    return true;
                case ReceiveTimeout _:
                    if (_isRebalance)
                        ...
                        Log.Debug("{0}: Shutting down [{1}] shard from [{2}] timed out", _typeName, _shard,
                            _shardRegionFrom);
                    Done(false);
                    return true;
            }
        ...
        private bool StoppingShard(object message)
        {
            switch (message)
            {
                ...
                case ShardRegionTerminated t when t.Region.Equals(_shardRegionFrom):
                    Log.Debug("{0}: ShardRegion [{1}] terminated while waiting for ShardStopped for shard [{2}].",
                        _typeName, t.Region, _shard);
                    Done(true);
                    return true;
            }
    ...
    /// <summary>
    ///     <see cref="Sharding.ShardRegion" /> registers to <see cref="ShardCoordinator" />, until it receives
    ///     <see cref="RegisterAck" />.
    /// </summary>
    [Serializable]
    ...
    /// <summary>
    ///     <see cref="ShardRegion" /> in proxy only mode registers to <see cref="ShardCoordinator" />, until it receives
    ///     <see cref="RegisterAck" />.
    /// </summary>
    [Serializable]
    internal sealed class RegisterProxy : ICoordinatorCommand, IDeadLetterSuppression, IEquatable<RegisterProxy>
    {
        ...
        public override string ToString()
        {
            return $"RegisterProxy({ShardRegionProxy})";
        }
    ...
    /// <summary>
    ///     Acknowledgement from <see cref="ShardCoordinator" /> that <see cref="Register" /> or <see cref="RegisterProxy" />
    ///     was successful.
    /// </summary>
    internal sealed class RegisterAck : ICoordinatorMessage, IEquatable<RegisterAck>
    {
        ...
        public override string ToString()
        {
            return $"RegisterAck({Coordinator})";
        }
    ...
    internal sealed class ShardHome : ICoordinatorMessage, IEquatable<ShardHome>
    {
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                var hashCode = Shard.GetHashCode();
        ...
        public override string ToString()
        {
            return $"ShardHome(shardId:{Shard}, ref:{Ref})";
        }
    ...
    /// <summary>
    ///     <see cref="ShardRegion" /> replies with this message for <see cref="HostShard" /> requests which lead to it hosting
    ///     the shard
    /// </summary>
    [Serializable]
    ...
    internal sealed class RegionStopped : ICoordinatorCommand, IEquatable<RegionStopped>
    {
        ...
        public override string ToString()
        {
            return $"RegionStopped({ShardRegion})";
        }
    ...
    /// </summary>
    [Serializable]
    internal sealed class GracefulShutdownRequest : ICoordinatorCommand, IDeadLetterSuppression,
        IEquatable<GracefulShutdownRequest>
    {
        /// <summary>
        ///     TBD
        /// </summary>
        ...
        public override string ToString()
        {
            return $"GracefulShutdownRequest({ShardRegion})";
        }
    ...
    public sealed class ShardRegionRegistered : IDomainEvent, IEquatable<ShardRegionRegistered>
    {
        ...
        public override string ToString()
        {
            return $"ShardRegionRegistered({Region})";
        }
    ...
    public sealed class ShardRegionProxyRegistered : IDomainEvent, IEquatable<ShardRegionProxyRegistered>
    {
        ...
        public override string ToString()
        {
            return $"ShardRegionProxyRegistered({RegionProxy})";
        }
    ...
    public sealed class ShardRegionTerminated : IDomainEvent, IEquatable<ShardRegionTerminated>
    {
        ...
        public override string ToString()
        {
            return $"ShardRegionTerminated({Region})";
        }
    ...
    public sealed class ShardRegionProxyTerminated : IDomainEvent, IEquatable<ShardRegionProxyTerminated>
    {
        ...
        public override string ToString()
        {
            return $"ShardRegionProxyTerminated({RegionProxy})";
        }
    ...
    public sealed class ShardHomeAllocated : IDomainEvent, IEquatable<ShardHomeAllocated>
    {
        ...
        public override string ToString()
        {
            return $"ShardHomeAllocated(shard:{Shard}, region:{Region})";
        }
    ...
    public sealed class ShardHomeDeallocated : IDomainEvent, IEquatable<ShardHomeDeallocated>
    {
        ...
        public override string ToString()
        {
            return $"ShardHomeDeallocated({Shard})";
        }
    ...
    public sealed class ShardCoordinatorInitialized : IDomainEvent, IEquatable<ShardCoordinatorInitialized>
    {
        ...
        public override string ToString()
        {
            return "ShardCoordinatorInitialized";
        }
    ...
    internal sealed class CoordinatorState : IClusterShardingSerializable, IEquatable<CoordinatorState>
    {
        ...
        private CoordinatorState() : this(
            ImmutableDictionary<ShardId, IActorRef>.Empty,
            ImmutableDictionary<IActorRef, IImmutableList<ShardId>>.Empty,
            ImmutableHashSet<IActorRef>.Empty,
            ImmutableHashSet<ShardId>.Empty)
        {
        }
        ...
        public bool IsEmpty => Shards.Count == 0 && Regions.Count == 0 && RegionProxies.Count == 0;

        public IImmutableSet<ShardId> AllShards => Shards.Keys.Union(UnallocatedShards).ToImmutableHashSet();
            ...
            switch (e)
            {
                ...
                case ShardRegionProxyRegistered message:
                    if (RegionProxies.Contains(message.RegionProxy))
                        throw new ArgumentException($"Region proxy {message.RegionProxy} already registered: {this}",
                            nameof(e));
                ...
                case ShardRegionTerminated message:
                {
                    if (!Regions.TryGetValue(message.Region, out var shardRegions))
                        throw new ArgumentException($"Terminated region {message.Region} not registered: {this}",
                            nameof(e));

                    var newUnallocatedShards =
                        RememberEntities ? UnallocatedShards.Union(shardRegions) : UnallocatedShards;
                    return Copy(
                ...
                case ShardRegionProxyTerminated message:
                    if (!RegionProxies.Contains(message.RegionProxy))
                        throw new ArgumentException(
                            $"Terminated region proxy {message.RegionProxy} not registered: {this}", nameof(e));
                ...
                case ShardHomeAllocated message:
                {
                    ...
                    return Copy(
                        Shards.SetItem(message.Shard, message.Region),
                        Regions.SetItem(message.Region, shardRegions.Add(message.Shard)),
                        unallocatedShards: newUnallocatedShards);
                }
                case ShardHomeDeallocated message:
                {
                    ...
                    if (!Regions.TryGetValue(region, out var shardRegions))
                        throw new ArgumentException($"Region {region} for shard {message.Shard} not registered: {this}",
                            nameof(e));

                    var newUnallocatedShards =
                        RememberEntities ? UnallocatedShards.Add(message.Shard) : UnallocatedShards;
                    return Copy(
                        Shards.Remove(message.Shard),
                        Regions.SetItem(region, shardRegions.Where(s => s != message.Shard).ToImmutableList()),
                        unallocatedShards: newUnallocatedShards);
                }
    ...
    private sealed class AllocateShardResult
    {
        ...
        public override string ToString()
        {
            return $"AllocateShardResult(shard:{Shard}, shardRegion:{ShardRegion}, sender:{GetShardHomeSender})";
        }
    }
    ...
    private sealed class RebalanceResult
    {
        ...
        public override string ToString()
        {
            return $"RebalanceResult({string.Join(", ", Shards)})";
        }
    }
