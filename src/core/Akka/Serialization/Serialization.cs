...
public class Serialization
{
    ...
    /// </summary>
    [ThreadStatic] internal static Information CurrentTransportInformation;
    ...
    public Serialization(ExtendedActorSystem system)
    {
        ...
        var serializerBindingConfig = system.Settings.Config.GetConfig("akka.actor.serialization-bindings")
            .AsEnumerable().ToList();
        ...
        foreach (var kvp in serializersConfig)
        {
            ...
            {
                system.Log.Warning("The type name for serializer '{0}' did not resolve to an actual Type: '{1}'",
        ...
        foreach (var details in _serializerDetails) AddSerializer(details.Alias, details.Serializer);
    ...
    /// <remarks>
    ///     WARNING: if you change this method it's likely that the DaemonMsgCreateSerializer and other calls will need changes
    ///     too.
    /// </remarks>
    ...
    public void AddSerializer(Serializer serializer)
    {
        ...
        if (_logSerializerOverrideOnStart && _serializersById.ContainsKey(id) &&
            _serializersById[id].GetType() != serializer.GetType())
            LogWarning(
    ...
    public void AddSerializer(string name, Serializer serializer)
    {
        ...
        if (_logSerializerOverrideOnStart && _serializersByName.ContainsKey(name) &&
            _serializersByName[name].GetType() != serializer.GetType())
            LogWarning(
    ...
    {
        if (_logSerializerOverrideOnStart && _serializerMap.ContainsKey(type) &&
            _serializerMap[type].GetType() != serializer.GetType())
            LogWarning(
    ...
    ///     >Thrown when <paramref name="type" /> is of type <see cref="Object" />
    /// </exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal void RemoveSerializationMap(Type type)
    {
        ...
        if (!_serializerMap.TryRemove(type, out _)) LogWarning($"Failed to remove serializer for type [{type}]");
        ...
        foreach (var serializerType in _serializerMap.Keys)
            if (serializerType.IsAssignableFrom(type) && serializerType != _objectType)
                if (!_serializerMap.TryRemove(serializerType, out _))
                    LogWarning(
                        $"Failed to remove serializer for type [{serializerType}] which derives from type [{type}]");
    }
    ...
    /// <summary>
    ///     Deserializes the given array of bytes using the specified serializer id, using the optional type hint to the
    ///     Serializer.
    /// </summary>
        ...
        else
        {
            ...
            var res =
                path.ToSerializationFormatWithAddress(
                    provider.GetExternalAddressFor(address) ?? provider.DefaultAddress);
            return res;
        }
    }
