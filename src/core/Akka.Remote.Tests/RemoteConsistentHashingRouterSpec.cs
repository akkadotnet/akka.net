...
public class RemoteConsistentHashingRouterSpec : AkkaSpec
{
    ...
    public void ConsistentHashingGroup_must_use_same_hash_ring_independent_of_self_address()
    {
        ...
        var nodes1 = new List<ConsistentRoutee>(new[]
        {
            new ConsistentRoutee(s1, a1), new ConsistentRoutee(s2, a1), new ConsistentRoutee(s3, a1)
        });
        var nodes2 =
    ...
    public void ConsistentHashingGroup_must_use_same_hash_ring_independent_of_local_and_remote_nodes()
    {
        ...
        var s2 = new ActorSelectionRoutee(
            Sys.ActorSelection("akka.tcp://RemoteConsistentHashingRouterSpec-1@client2:2552/user/a"));
        var nodes1 = new List<ConsistentRoutee>(new[] { new ConsistentRoutee(s1, a1), new ConsistentRoutee(s2, a1) });

        var s4 = new ActorSelectionRoutee(
            Sys.ActorSelection("akka.tcp://RemoteConsistentHashingRouterSpec-1@client1:2552/user/a"));
