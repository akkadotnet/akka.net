...
public class ShardWithLeaseSpec : AkkaSpec
{
    ...
    private static readonly AtomicCounter typeIdx = new(0);
    ...
    private string NextTypeIdx()
    {
        return $"{typeIdx.IncrementAndGet()}";
    }
    ...
    public void Lease_handling_in_sharding_must_shutdown_if_lease_is_lost()
    {
        ...
        EventFilter.Error(
                start:
                $"{setup.TypeName}: Shard id [1] lease lost, stopping shard and killing [1] entities. Reason for losing lease: {typeof(BadLease).FullName}: bye bye lease")
            .ExpectOne(() =>
            {
    ...
    private class MessageExtractor : IMessageExtractor
    {
        ...
        {
            return (int.Parse(entityId) % NumberOfShards).ToString();
        }
    }
    ...
    private class Setup
    {
        ...
        {
            settings = ClusterShardingSettings.Create(spec.Sys)
                .WithLeaseSettings(new LeaseUsageSettings("test-lease", TimeSpan.FromSeconds(2)));
