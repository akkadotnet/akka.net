...
public sealed class ClusterClientReceptionist : IExtension
{
    ...
    /// </summary>
    public bool IsTerminated => Cluster.Get(_system).IsTerminated ||
                                !(string.IsNullOrEmpty(_role) || Cluster.Get(_system).SelfRoles.Contains(_role));

    /// <summary>
    ///     Register the actors that should be reachable for the clients in this <see cref="DistributedPubSubMediator" />.
    /// </summary>
    internal IActorRef PubSubMediator => DistributedPubSub.Get(_system).Mediator;
