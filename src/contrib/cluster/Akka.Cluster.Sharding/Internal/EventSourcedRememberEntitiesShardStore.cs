...
internal class EventSourcedRememberEntitiesShardStore : PersistentActor
{
    ...
    /// <summary>
    ///     Persistent actor keeping the state for Akka Persistence backed remember entities (enabled through
    ...
    private bool IsSnapshotNeeded =>
        LastSequenceNr % Settings.TuningParameters.SnapshotAfter == 0 && LastSequenceNr != 0;
    ...
    protected override bool ReceiveCommand(object message)
    {
        switch (message)
        {
            ...
            case SaveSnapshotSuccess e:
                ...
                InternalDeleteMessagesBeforeSnapshot(e, Settings.TuningParameters.KeepNrOfBatches,
            ...
            case DeleteMessagesSuccess e:
                ...
                var keepNrOfBatchesWithSafetyBatch = Settings.TuningParameters.KeepNrOfBatches == 0
                    ? 0
                    : Settings.TuningParameters.KeepNrOfBatches + 1;
                var deleteFrom = Math.Max(0,
                    deleteTo - keepNrOfBatchesWithSafetyBatch * Settings.TuningParameters.SnapshotAfter);
                ...
                Log.Debug(
                    ...
                    deleteFrom,
                    deleteTo);
                DeleteSnapshots(new SnapshotSelectionCriteria(minSequenceNr: deleteFrom,
                    maxTimeStamp: DateTime.MaxValue, maxSequenceNr: deleteTo));
                return true;
    ...
    {
        if (IsSnapshotNeeded) SaveSnapshot();
    }
    ...
    public sealed class State : IClusterShardingSerializable, IEquatable<State>
    {
        ...
        {
            return $"State({string.Join(", ", Entities)})";
        }
    ...
    public sealed class EntitiesStarted : IStateChange, IEquatable<EntitiesStarted>
    {
        ...
        public override string ToString()
        {
            return $"EntitiesStarted({string.Join(", ", Entities)})";
        }
    ...
    public sealed class EntitiesStopped : IStateChange, IEquatable<EntitiesStopped>
    {
        ...
        public override string ToString()
        {
            return $"EntitiesStopped({string.Join(", ", Entities)})";
        }
