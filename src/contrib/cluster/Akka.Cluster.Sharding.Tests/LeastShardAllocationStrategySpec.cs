...
public class LeastShardAllocationStrategySpec : AkkaSpec
{
    ...
    private readonly IImmutableList<string> shards = Enumerable.Range(1, 999).Select(n => n.ToString("000"))
        .ToImmutableList();

    private readonly IShardAllocationStrategy strategyWithoutLimits;

    public LeastShardAllocationStrategySpec(ITestOutputHelper helper) : base(helper)
    {
        ...
        strategyWithoutLimits = StrategyWithFakeCluster(1000, 1.0);
    }
    ...
    {
        var allocationsAfterRemoval = allocations.SetItems(allocations.Select(i =>
            new KeyValuePair<IActorRef, IImmutableList<string>>(i.Key,
                i.Value.ToImmutableHashSet().Except(rebalance).OrderBy(j => j).ToImmutableList())));

        var acc = allocationsAfterRemoval;
    ...
    internal static ImmutableList<int> CountShardsPerRegion(
        IImmutableDictionary<IActorRef, IImmutableList<string>> newAllocations)
    {
    ...
    private static ImmutableList<int> AllocationCountsAfterRebalance(
        IShardAllocationStrategy allocationStrategy,
        IImmutableDictionary<IActorRef, IImmutableList<string>> allocations,
        IImmutableSet<string> rebalance)
    {
    ...
    private IShardAllocationStrategy StrategyWithFakeCluster(int absoluteLimit, double relativeLimit)
    {
        ...
        return new TestLeastShardAllocationStrategy(absoluteLimit, relativeLimit,
    ...
    public void LeastShardAllocationStrategy_must_rebalance_shards_4_4_2()
    {
        ...
        var result = allocationStrategy.Rebalance(allocations, ImmutableHashSet<string>.Empty).Result;
        result.Should().BeEquivalentTo("001");
        AllocationCountsAfterRebalance(allocationStrategy, allocations, result).OrderBy(i => i).Should()
            .Equal(new[] { 3, 4, 3 }.OrderBy(i => i));
    }
    ...
    public void LeastShardAllocationStrategy_must_rebalance_shards_5_5_0()
    {
        ...
        AllocationCountsAfterRebalance(allocationStrategy, allocations2, result2).OrderBy(i => i).Should()
            .Equal(new[] { 3, 4, 3 }.OrderBy(i => i));
    }
    ...
    {
        var allocationStrategy = strategyWithoutLimits;
        var allocations = CreateAllocations(50, cCount: 50);
        var result1 = allocationStrategy.Rebalance(allocations, ImmutableHashSet<string>.Empty).Result;
        result1.Should().BeEquivalentTo(shards.Take(50 - 34).Union(shards.Skip(50).Take(50 - 34)));

        // so far [34, 34, 32]
        AllocationCountsAfterRebalance(allocationStrategy, allocations, result1).OrderBy(i => i).Should()
            .Equal(new[] { 34, 34, 32 }.OrderBy(i => i));
        var allocations2 = AfterRebalance(allocationStrategy, allocations, result1);
        // second phase will find the diff of 2, resulting in [33, 34, 33]
        var result2 = allocationStrategy.Rebalance(allocations2, ImmutableHashSet<string>.Empty).Result;
        result2.Should().BeEquivalentTo("017");
        AllocationCountsAfterRebalance(allocationStrategy, allocations2, result2).OrderBy(i => i).Should()
            .Equal(new[] { 33, 34, 33 }.OrderBy(i => i));
    }

    [Fact]
    public void LeastShardAllocationStrategy_must_respect_absolute_limit_of_number_shards()
    {
        var allocationStrategy = StrategyWithFakeCluster(3, 1.0);
        var allocations = CreateAllocations(1, 9);
        var result = allocationStrategy.Rebalance(allocations, ImmutableHashSet<string>.Empty).Result;
    ...
    {
        var allocationStrategy = StrategyWithFakeCluster(5, 0.3);
        var allocations = CreateAllocations(1, 9);
        var result = allocationStrategy.Rebalance(allocations, ImmutableHashSet<string>.Empty).Result;
        result.Should().BeEquivalentTo("002", "003", "004");
        AllocationCountsAfterRebalance(allocationStrategy, allocations, result).Should().Equal(2, 6, 2);
    ...
    public void LeastShardAllocationStrategy_must_not_rebalance_when_rolling_update_in_progress()
    {
        ...
        var allocationStrategy =
            new TestLeastShardAllocationStrategy(1000, 1.0,
    ...
    {
        public static readonly ImmutableDictionaryKeepOrder<TKey, TValue> Empty =
        ...
        private ImmutableDictionaryKeepOrder(ImmutableDictionary<TKey, TValue> dictionary,
            ImmutableList<KeyValuePair<TKey, TValue>> items)
        {
    ...
    internal class TestLeastShardAllocationStrategy : Sharding.Internal.LeastShardAllocationStrategy
    {
        private readonly Func<CurrentClusterState> clusterState;
        private readonly Func<Member> selfMember;

        public TestLeastShardAllocationStrategy(int absoluteLimit, double relativeLimit,
