...
internal sealed class AkkaPduProtobuffCodec : AkkaPduCodec
{
    ...
    {
        var handshakeInfo = new AkkaHandshakeInfo { Origin = SerializeAddress(info.Origin), Uid = (ulong)info.Uid };
    ...
    private ByteString DISASSOCIATE => ConstructControlMessagePdu(CommandType.Disassociate);

    private ByteString DISASSOCIATE_SHUTTING_DOWN => ConstructControlMessagePdu(CommandType.DisassociateShuttingDown);

    private ByteString DISASSOCIATE_QUARANTINED => ConstructControlMessagePdu(CommandType.DisassociateQuarantined);
    ...
    private static ByteString ConstructControlMessagePdu(CommandType code, AkkaHandshakeInfo handshakeInfo = null)
    {
        ...
        if (handshakeInfo != null) controlMessage.HandshakeInfo = handshakeInfo;
