...
public class HubsDocTests : TestKit
{
    ...
    public void Hubs_must_demonstrate_creating_a_dynamic_merge()
    {
        ...
        var consumer = Sink.ForEach<string>(WriteLine);

        // Attach a MergeHub Source to the consumer. This will materialize to a
        // corresponding Sink.
        var runnableGraph =
            MergeHub.Source<string>(16).To(consumer);

        // By running/materializing the consumer we get back a Sink, and hence
        // now have access to feed elements into it. This Sink can be materialized
        // any number of times, and every element that enters the Sink will
        // be consumed by our consumer.
        var toConsumer = runnableGraph.Run(Materializer);
    ...
    public void Hubs_must_demonstrate_creating_a_dynamic_broadcast()
    {
        ...
        var producer = Source.Tick(TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(1), "New message");
        ...
        var runnableGraph =
            producer.ToMaterialized(BroadcastHub.Sink<string>(256), Keep.Right);
    ...
    public void Hubs_must_demonstrate_combination()
    {
        ...
        var (sink, source) = MergeHub
            .Source<string>(16)
            .ToMaterialized(BroadcastHub.Sink<string>(256), Keep.Both)
            .Run(Materializer);
        ...
        var busFlow = Flow.FromSinkAndSource(sink, source)
    ...
    public void Hubs_must_demonstrate_creating_a_dynamic_partition_hub()
    {
        ...
        var producer = Source.Tick(TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(1), "message")
    ...
    public void Hubs_must_demonstrate_creating_a_dynamic_partition_hub_routing_to_fastest_consumer()
    {
        ...
        // A simple producer that publishes a new "message-" every second
        var producer = Source.From(Enumerable.Range(0, 100));
