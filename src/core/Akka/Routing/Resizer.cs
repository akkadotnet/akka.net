...
/// <summary>
///     <see cref="Akka.Routing.Pool" /> routers with dynamically resizable number of routees are implemented by providing
///     a Resizer
...
/// <summary>
///     Implementation of <see cref="Akka.Routing.Resizer" /> that adjust the <see cref="Akka.Routing.Pool" /> based on
...
public class DefaultResizer : Resizer, IEquatable<DefaultResizer>
{
        ...
        int messagesPerResize = 10)
    {
        ...
        if (backoffThreshold > 1.0)
            throw new ArgumentException($"backoffThreshold must be <= 1.0, was {backoffThreshold}",
                nameof(backoffThreshold));
        if (backoffRate < 0.0)
            throw new ArgumentException($"backoffRate must be >= 0.0, was {backoffRate}", nameof(backoffRate));
        if (messagesPerResize <= 0)
            throw new ArgumentException($"messagesPerResize must be > 0, was {messagesPerResize}",
                nameof(messagesPerResize));
    ...
    public bool Equals(DefaultResizer other)
    {
        ...
        return MessagesPerResize == other.MessagesPerResize && BackoffRate.Equals(other.BackoffRate) &&
               RampupRate.Equals(other.RampupRate) && BackoffThreshold.Equals(other.BackoffThreshold) &&
               UpperBound == other.UpperBound && PressureThreshold == other.PressureThreshold &&
    ...
    {
        return resizerConfig.GetBoolean("resizer.enabled") ? Apply(resizerConfig.GetConfig("resizer")) : null;
    }
                ...
                if (routee is ActorRefRoutee actorRefRoutee && actorRefRoutee.Actor is ActorRefWithCell actorRef)
                {
                    ...
                    if (underlying is ActorCell cell)
                    {
                        if (PressureThreshold == 1)
                            return (cell.Mailbox.IsScheduled() || cell.Mailbox.IsSuspended()) &&
    ...
    public override int GetHashCode()
    {
        unchecked
        {
            var hashCode = MessagesPerResize;
