...
public class TransitionSpec : MultiNodeClusterSpec
{
    ...
    private int _gossipBarrierCounter;
    ...
    private void A_Cluster_must_perform_correct_transitions_when_second_joining_first()
    {
        ...
        RunOn(() =>
        {
            ...
            AwaitAssert(() =>
                SeenLatestGossip().Should().BeEquivalentTo(ImmutableHashSet.Create(_config.First, _config.Second)));
    ...
    {
        RunOn(() => { Cluster.Join(GetAddress(_config.Second)); }, _config.Third);

        RunOn(() =>
        {
            // gossip chat from the join will synchronize the views
            AwaitAssert(() =>
                SeenLatestGossip().Should().BeEquivalentTo(ImmutableHashSet.Create(_config.Second, _config.Third)));
        ...
        RunOn(() =>
        {
            ...
            AwaitMemberStatus(GetAddress(_config.Second), Akka.Cluster.MemberStatus.Up);
            AwaitAssert(() =>
                SeenLatestGossip().Should()
                    .BeEquivalentTo(ImmutableHashSet.Create(_config.First, _config.Second, _config.Third)));
        }, _config.First, _config.Second);
    ...
    private void A_Cluster_must_perform_correct_transitions_when_second_becomes_unavailable()
    {
        RunOn(() =>
        {
            ...
            ReapUnreachable();
            AwaitAssert(() =>
                ClusterView.UnreachableMembers.Select(c => c.Address).Should().Contain(GetAddress(_config.Second)));
        ...
        RunOn(() => { Cluster.Down(GetAddress(_config.Second)); }, _config.First);
