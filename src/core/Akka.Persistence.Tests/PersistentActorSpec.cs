...
public partial class PersistentActorSpec : PersistenceSpec
{
    ...
    {
        EventFilter.Exception<ActorInitializationException>().And
            .Error(contains: "PersistenceId is [null] for PersistentActor").ExpectOne(() =>
            {
    ...
    [Fact]
    public void
        PersistentActor_should_support_multiple_PersistAsync_calls_for_one_command_and_execute_them_when_possible_not_hindering_command_processing()
    {
    ...
    public void PersistentActor_should_reply_to_the_original_sender_of_a_command_even_on_PersistAsync()
    {
        ...
        for (var i = 0; i < 10; i++) pref.Tell(commands[i], probes[i].Ref);

        Within(TimeSpan.FromSeconds(3), () =>
        {
            foreach (var probe in probes) probe.ExpectMsgAllOf(new string[] { });
        });
    }
    ...
    [Fact]
    public void
        PersistentActor_should_support_a_mix_of_persist_calls_sync_async_sync_and_persist_calls_in_expected_order()
    {
    ...
    public void PersistentActor_should_support_a_mix_of_persist_calls_sync_async_and_persist_calls()
    {
        ...
        var found = ExpectMsgAnyOf(expected.Cast<object>()
            .ToArray()); // ea2 is PersistAsync, b can be processed before it
    ...
    public void PersistentActor_should_correlate_PersistAsync_handlers_after_restart()
    {
        ...
        for (var i = 1; i < 100; i++) pref.Tell(new Cmd(i));
        pref.Tell("boom");
        for (var i = 1; i < 20; i++) pref.Tell(new Cmd(i));
    ...
    [Fact]
    public void
        PersistentActor_should_allow_deferring_handlers_in_order_to_provide_ordered_processing_in_respect_to_Persist_handlers()
    {
    ...
    [Fact]
    public void
        PersistentActor_should_allow_deferring_handlers_in_order_to_provide_ordered_processing_in_respect_to_PersistAsync_handlers()
    {
    ...
    [Fact]
    public void
        PersistentActor_should_invoke_deferred_handlers_in_presence_of_mixed_a_long_series_Persist_and_PersistAsync_calls()
    {
    ...
    public void PersistentActor_should_allow_deeply_nested_Persist_calls()
    {
        ...
        ExpectMsg("a");
        ReceiveN(nestedPersists).Select(m => m.ToString())
            .ShouldOnlyContainInOrder(Enumerable.Range(1, nestedPersists).Select(i => "a-" + i).ToArray());
        // and only then process "b"
        ExpectMsg("b");
        ReceiveN(nestedPersists).Select(m => m.ToString())
            .ShouldOnlyContainInOrder(Enumerable.Range(1, nestedPersists).Select(i => "b-" + i).ToArray());
    }
    ...
    public void PersistentActor_should_allow_mixed_nesting_of_PersistAsync_in_Persist_calls()
    {
        ...
        ExpectMsg("a");
        ReceiveN(4).Select(m => m.ToString()).ToArray()
            .ShouldOnlyContainInOrder("a-outer-1", "a-outer-2", "a-inner-async-1", "a-inner-async-2");
    }
    ...
    public void PersistentActor_should_allow_mixed_nesting_of_Persist_in_PersistAsync_calls()
    {
        ...
        ExpectMsg("a");
        ReceiveN(4).Select(m => m.ToString()).ToArray()
            .ShouldOnlyContainInOrder("a-outer-async-1", "a-outer-async-2", "a-inner-1", "a-inner-2");
    }

    [Fact]
    public void
        PersistentActor_should_make_sure_Persist_retains_promised_semantics_when_nested_in_PersistAsync_callback()
    {
