...
public abstract class AllTestForEventFilterBase<TLogEvent> : EventFilterTestBase where TLogEvent : LogEvent
{
    ...
    {
        await _testingEventFilter.ForLogLevel(LogLevel, "whatever")
            .ExpectOneAsync(() =>
            {
                LogMessage("whatever");
                return Task.CompletedTask;
    ...
    {
        await EventFilter.ForLogLevel(LogLevel, "whatever").ExpectOneAsync(() =>
        {
            LogMessage("let-me-thru");
            LogMessage("whatever");
            return Task.CompletedTask;
    ...
    public async Task Do_not_intercept_messages_when_source_does_not_match()
    {
        ...
        {
            PublishMessage("message", "expected-source");
            PublishMessage("message", "let-me-thru");
            return Task.CompletedTask;
        });
...
#pragma warning disable CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
    ...
    public async Task Chain_many_filters()
    {
        await _testingEventFilter
            .ForLogLevel(LogLevel, "Message 1").And
            .ForLogLevel(LogLevel, "Message 3")
            .ExpectAsync(2, () =>
            {
