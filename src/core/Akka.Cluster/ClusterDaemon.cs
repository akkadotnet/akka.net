...
internal sealed class InternalClusterAction
{
    ...
    internal sealed class Join : IClusterMessage
    {
        ...
        {
            return Node.Equals(other.Node) && !Roles.Except(other.Roles).Any() && AppVersion.Equals(other.AppVersion);
        }
...
internal class ClusterCoreSupervisor : ReceiveActor, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
{
    ...
    private void CreateChildren(Cluster cluster)
    {
        ...
        _publisher =
            Context.ActorOf(
                Props.Create(() => new ClusterDomainEventPublisher(_cluster.SelfUniqueAddress))
                    .WithDispatcher(Context.Props.Dispatcher), "publisher");
        _coreDaemon =
            Context.ActorOf(
                Props.Create(() => new ClusterCoreDaemon(_cluster, _publisher))
                    .WithDispatcher(Context.Props.Dispatcher), "daemon");
...
internal class ClusterCoreDaemon : UntypedActor, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
{
    ...
    private int _seedNodeProcessCounter; //for unique names
    private ImmutableList<Address> _seedNodes;
    private int _selfDownCounter;
    ...
    /// </summary>
    public bool IsSingletonCluster => LatestGossip.IsSingletonCluster;

    public UniqueAddress SelfUniqueAddress1 => SelfUniqueAddress;
    ...
    private void CleanupExitingConfirmed()
    {
        ...
        if (_exitingConfirmed.Any())
            _exitingConfirmed = _exitingConfirmed.Where(n => LatestGossip.Members.Any(m => m.UniqueAddress.Equals(n)))
                .ToImmutableHashSet();
    }
    ...
    private void BecomeInitialized()
    {
        ...
        Context.ActorOf(
            Props.Create(() => new ClusterHeartbeatSender(_cluster)).WithDispatcher(_cluster.Settings.UseDispatcher),
            "heartbeatSender");
        // make sure that join process is stopped
        StopSeedNodeProcess();
    ...
    private void Initialized(object message)
    {
        if (message is GossipEnvelope ge)
        {
            ...
            if (_cluster.Settings.VerboseGossipReceivedLogging)
                _log.Debug("Cluster Node [{0}] - Received gossip from [{1}] which was {2}.", _cluster.SelfAddress,
                    ge.From, receivedType);
        }
    ...
    {
        if (message is InternalClusterAction.ITick or GossipEnvelope or GossipStatus
            or InternalClusterAction.ExitingConfirmed)
        {
            //do nothing
        }
        else
    ...
    public void JoinSeedNodes(ImmutableList<Address> newSeedNodes)
    {
        if (!newSeedNodes.IsEmpty)
        {
            ...
            if (newSeedNodes.SequenceEqual(
                    ImmutableList.Create(_cluster.SelfAddress))) // self-join for a singleton cluster
            {
            ...
            else
            {
                ...
                if (newSeedNodes.Head().Equals(_cluster.SelfAddress))
                    _seedNodeProcess = Context.ActorOf(Props
                            .Create(() => new FirstSeedNodeProcess(_cluster, newSeedNodes))
                            .WithDispatcher(_cluster.Settings.UseDispatcher),
                        "firstSeedNodeProcess-" + _seedNodeProcessCounter);
                else
                    _seedNodeProcess = Context.ActorOf(Props
                            .Create(() => new JoinSeedNodeProcess(_cluster, newSeedNodes))
                            .WithDispatcher(_cluster.Settings.UseDispatcher),
    ...
    public void Joining(UniqueAddress node, ImmutableHashSet<string> roles, AppVersion appVersion)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                var newGossip = LatestGossip.Copy(newMembers);
                ...
                {
                    _cluster.LogInfo(
                        "Node [{0}] is JOINING itself (with roles [{1}], version [{2}]) and forming a new cluster",
                        node.Address, string.Join(",", roles), appVersion);
                ...
                {
                    _cluster.LogInfo("Node [{0}] is JOINING, roles [{1}], version [{2}]", node.Address,
                        string.Join(",", roles), appVersion);
                    Sender.Tell(new InternalClusterAction.Welcome(SelfUniqueAddress, LatestGossip));
                }
    ...
    public void Leaving(Address address)
    {
        ...
        if (LatestGossip.Members.Any(m =>
                ...
                m.Status is MemberStatus.Joining or MemberStatus.WeaklyUp or MemberStatus.Up))
        {
            // mark node as LEAVING
            var newMembers = LatestGossip.Members.Select(m =>
            {
                if (m.Address == address) return m.Copy(MemberStatus.Leaving);
                return m;
    ...
    public void Quarantined(UniqueAddress node)
    {
        ...
        if (localGossip.HasMember(node))
        {
            ...
            UpdateLatestGossip(newGossip);
            _log.Warning(
                "Cluster Node [{0}] - Marking node as TERMINATED [{1}], due to quarantine. Node roles [{2}]. It must still be marked as down before it's removed.",
    ...
    public ReceiveGossipType ReceiveGossip(GossipEnvelope envelope)
    {
        ...
        {
            _cluster.LogInfo(
                "Ignoring received gossip intended for someone else, from [{0}] to [{1}]. Our full address is [{2}]",
        ...
        switch (comparison)
        {
            ...
            default:
                ...
                var prunedLocalGossip = localGossip.Members.Aggregate(localGossip, (g, m) =>
                {
                    ...
                    {
                        _log.Debug("Cluster Node [{0}] - Pruned conflicting local gossip: {1}", _cluster.SelfAddress,
                            m);
                ...
                var prunedRemoteGossip = remoteGossip.Members.Aggregate(remoteGossip, (g, m) =>
                {
                    ...
                    {
                        _log.Debug("Cluster Node [{0}] - Pruned conflicting remote gossip: {1}", _cluster.SelfAddress,
                            m);
                        return g.Prune(VectorClock.Node.Create(VclockName(m.UniqueAddress)));
                    }
        ...
        if (_membershipState.IsLeader(SelfUniqueAddress))
        {
            ...
            else
            {
                ...
                if (_cluster.Settings.AllowWeaklyUpMembers &&
                    _leaderActionCounter * _cluster.Settings.LeaderActionsInterval.TotalMilliseconds >=
                    _cluster.Settings.WeaklyUpAfter.TotalMilliseconds)
    ...
    private void MoveJoiningToWeaklyUp()
    {
        ...
        bool IsJoiningToWeaklyUp(Member m)
        {
            return m.Status == MemberStatus.Joining
                   && enoughMembers
        ...
        if (!changedMembers.IsEmpty)
        {
            ...
            var newGossip = localGossip.Copy(newMembers);
            UpdateLatestGossip(newGossip);
    ...
    private void ShutdownSelfWhenDown()
    {
        if (LatestGossip.GetMember(SelfUniqueAddress).Status == MemberStatus.Down)
        {
            ...
            if (_selfDownCounter >= MaxTicksBeforeShuttingDownMyself ||
                downed.All(node => unreachable.Contains(node) || LatestGossip.SeenByNode(node)))
            {
    ...
    ///     4. Move unreachable EXITING => REMOVED      -- When all nodes have seen the EXITING node as unreachable
    ///     (convergence) -
    ...
    public void LeaderActionsOnConvergence()
    {
        ...
        if (!removedUnreachable.IsEmpty || !removedExitingConfirmed.IsEmpty || !changedMembers.IsEmpty)
        {
            ...
            var newOverview = localOverview.Copy(newSeen, newReachability);
            ...
            var newGossip = localGossip.Copy(newMembers, newOverview, newVersion);
        ...
        bool IsJoiningUp(Member m)
        {
            return m.Status is MemberStatus.Joining or MemberStatus.WeaklyUp && enoughMembers;
        }
    }
                ...
                if (!ReferenceEquals(newReachability2, localOverview.Reachability))
                {
                    ...
                    if (!newlyDetectedReachableMembers.IsEmpty)
                        _cluster.LogInfo("Marking node(s) as REACHABLE [{0}]. Node roles [{1}]",
                            newlyDetectedReachableMembers.Select(m => m.ToString()).Aggregate((a, b) => a + ", " + b),
                            string.Join(",", _cluster.SelfRoles));
    ...
    /// <param name="exitingMembers"></param>
    /// <returns></returns>
    public static IReadOnlyCollection<Member> GossipTargetsForExitingMembers(Gossip latestGossip,
        IReadOnlyCollection<Member> exitingMembers)
    {
    ...
    {
        if (_cluster.Settings.VerboseGossipReceivedLogging)
            _log.Debug("Cluster Node [{0}] - New gossip published [{1}]", SelfUniqueAddress,
                _membershipState.LatestGossip);
...
internal sealed class JoinSeedNodeProcess : UntypedActor
{
    ...
    ///     or the first listed seed is a reference to the <see cref="IActorContext.System">IUntypedActorContext.System</see>'s
    ///     address.
    /// </exception>
...
internal sealed class FirstSeedNodeProcess : UntypedActor
{
    ...
    public FirstSeedNodeProcess(Cluster cluster, ImmutableList<Address> seeds)
    {
        ...
        _retryTaskToken = Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(TimeSpan.FromSeconds(1),
            TimeSpan.FromSeconds(1), Self, new InternalClusterAction.JoinSeenNode(), Self);
        Self.Tell(new InternalClusterAction.JoinSeenNode());
    }
...
internal sealed class GossipStats
{
    ...
    /// <summary>
    ///     Decrements the first set of statistics, <paramref name="a" />, using the second set of statistics,
    ///     <paramref name="b" />.
    /// </summary>
    /// <param name="a">The set of statistics to decrement.</param>
    /// <param name="b">The set of statistics used to decrement.</param>
    /// <returns>
    ///     A new <see cref="GossipStats" /> that is calculated by decrementing <paramref name="a" /> by
    ///     <paramref name="b" />.
    /// </returns>
    public static GossipStats operator -(GossipStats a, GossipStats b)
    {
...
internal class OnMemberStatusChangedListener : ReceiveActor
{
    ...
    /// <inheritdoc cref="ActorBase.PreStart" />
    /// <exception cref="ArgumentException">
    ///     This exception is thrown when the current status neither <see cref="MemberStatus.Up" /> or
    ///     <see cref="MemberStatus.Down" />.
    /// </exception>
    protected override void PreStart()
    {
