...
public class ActorRefSpec : AkkaSpec, INoImplicitSender
{
    ...
    public void An_ActorRef_should_not_allow_actors_to_be_created_outside_an_ActorOf()
    {
        ...
        Assert.Throws<ActorInitializationException>(() => { new BlackHoleActor(); });
    }
    ...
    {
        var actor = ActorOfAsTestActorRef<NonPublicActor>(
            Props.Create<NonPublicActor>(SupervisorStrategy.StoppingStrategy));
        // actors with a null sender should always write to deadletters
        await EventFilter.DeadLetter<object>().ExpectOneAsync(() =>
        {
    ...
    private class SenderActor : ActorBase
    {
        ...
        protected override bool Receive(object message)
        {
            ...
            {
                case "complex":
                    _replyTo.Tell("complexRequest");
                    break;
                case "complex2":
                    _replyTo.Tell("complexRequest2");
                    break;
                case "simple":
                    _replyTo.Tell("simpleRequest");
                    break;
                case "complexReply":
                    _latch.CountDown();
                    break;
