...
public class ClusterClientDiscovery : UntypedActor, IWithUnboundedStash, IWithTimers
{
            ...
            using var response = await _http.GetAsync(uri, cts.Token);
            {
                if (!response.IsSuccessStatusCode)
                {
                    if (_verboseLogging && _log.IsInfoEnabled)
                        _log.Info(
                            $"Failed to probe {uri}, Akka.Management HTTP endpoint returns {response.StatusCode}");
                    return null;
                }
    ...
    private bool Discovering(object message)
    {
        switch (message)
        {
            ...
            case ServiceDiscovery.Resolved resolved:
            {
                ...
                if (!resolved.ServiceName.Equals(_lookup.ServiceName))
                {
                    if (_log.IsWarningEnabled)
                        _log.Warning(
                            $"Received Akka.Discovery result for wrong service name. Expected: {_lookup.ServiceName}, got: {resolved.ServiceName}");
                ...
                if (_verboseLogging && _log.IsDebugEnabled)
                    _log.Debug("Initial contacts are discovered at [{0}], verifying existence.",
                ...
                {
                    var tasks = resolved.Addresses.Select(c =>
                        ResolveContact(c, Context, _probeTimeout, _shutdownCts.Token));
            ...
            case DiscoveryFailure fail:
                if (_verboseLogging && _log.IsInfoEnabled)
                    _log.Info(fail.Cause,
                        "Cluster client contact point service discovery phase failed, will try again.");
            ...
            case ContactVerificationFailure fail:
                if (_verboseLogging && _log.IsInfoEnabled)
                    _log.Info(fail.Cause,
                        "Cluster client receptionist path verification phase failed, will try again.");
    ...
    private Receive Active(ActorPath[] contacts)
    {
        ...
        var clusterClient =
            Context.System.ActorOf(Props.Create(() => new ClusterClient(currentSettings)).WithDeploy(Deploy.Local));
    ...
    internal sealed class DiscoverTick : IEquatable<DiscoverTick>
    {
        ...
        public bool Equals(DiscoverTick? other)
        {
        ...
        {
            return ReferenceEquals(this, obj) || obj is DiscoverTick;
        }
