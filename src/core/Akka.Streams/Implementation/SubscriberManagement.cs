...
internal interface ISubscriptionWithCursor<in T> : ISubscription, ICursor
{
    ...
    /// <summary>
    ///     Do not increment directly, use <see cref="SubscriberManagement{T, TStreamBuffer}.MoreRequested" /> instead (it
...
internal abstract class SubscriberManagement<T, TStreamBuffer> : ICursors where TStreamBuffer : IStreamBuffer<T>
{
    ...
    protected SubscriberManagement()
    {
        _buffer = new Lazy<IStreamBuffer<T>>(()
            => (IStreamBuffer<T>)Activator.CreateInstance(typeof(TStreamBuffer), InitialBufferSize, MaxBufferSize,
                this));
    }
    ...
    protected void MoreRequested(ISubscriptionWithCursor<T> subscription, long elements)
    {
        ...
        if (elements < 1)
        {
            try
            {
                ReactiveStreamsCompliance.TryOnError(subscription.Subscriber,
        ...
        else
        {
            if (_endOfStream is SubscriberManagement.NotReached or SubscriberManagement.Completed)
            {
                ...
                var remainingRequested =
                    DispatchFromBufferAndReturnRemainingRequested(demand, subscription, _endOfStream);
    ...
    private long DispatchFromBufferAndReturnRemainingRequested(long requested, ISubscriptionWithCursor<T> subscription,
        SubscriberManagement.IEndOfStream endOfStream)
    {
        while (requested != 0)
        ...
        return !(endOfStream is SubscriberManagement.NotReached) && _buffer.Value.Count(subscription) == 0
            ? long.MinValue
            : 0;
    }
    ...
    private ICollection<ISubscriptionWithCursor<T>> CompleteDoneSubscriptions(
        ICollection<ISubscriptionWithCursor<T>> subscriptions)
    {
