...
public class ChildRestartStats : IChildStats
{
    ...
    public ChildRestartStats(IInternalActorRef child, uint maxNrOfRetriesCount = 0,
        long restartTimeWindowStartTicks = 0)
    {
        Child = child;
    ...
    public bool RequestRestartPermission(int maxNrOfRetries, int withinTimeMilliseconds)
    {
        ...
        if (windowIsDefined) return RetriesInWindowOkay(retriesIsDefined ? maxNrOfRetries : 1, withinTimeMilliseconds);
        return true;
        //retriesWindow match {
