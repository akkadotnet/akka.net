...
public static class Retry
{
    ...
    ///         If the flow emits a failed element (i.e. <see cref="Result{T}.IsSuccess" /> is false), the
    ///         <paramref name="retryWith" />
    ///         function is fed with the `state` of the failed element, and may produce a new input-state tuple to pass through
    ///         the original flow. The function may also yield `Option.None` instead of `(input, state)`, which means not to
    ///         retry a failed element.
    ///     </para>
    ///     <para>
    ///         IMPORTANT CAVEAT:
    ...
    /// <param name="retryWith">
    ///     if output was failure, we can optionaly recover from it,
    ///     and retry with a new pair of input and new state we get from this function.
    /// </param>
    ...
    /// </summary>
    /// <param name="limit">
    ///     since every retry can generate more elements, the inner queue can get too big.
    ///     if the limit is reached, the stage will fail.
    /// </param>
    /// <param name="flow">the flow to retry</param>
    /// <param name="retryWith">
    ///     if output was failure, we can optionaly recover from it, and retry with
    ///     a sequence of input and new state pairs we get from this function.
    /// </param>
    /// <typeparam name="TIn">input elements type</typeparam>
    /// <typeparam name="TState">state to create a new `(I,S)` to retry with</typeparam>
    /// <typeparam name="TOut">output elements type</typeparam>
    /// <typeparam name="TMat">materialized value type</typeparam>
    [ApiMayChange]
    public static IGraph<FlowShape<(TIn, TState), (Result<TOut>, TState)>, TMat> Concat<TIn, TState, TOut, TMat>(
        long limit,
        IGraph<FlowShape<(TIn, TState), (Result<TOut>, TState)>, TMat> flow,
    ...
    private class RetryCoordinator<TIn, TState, TOut> : GraphStage<
        BidiShape<(TIn, TState), (Result<TOut>, TState), (Result<TOut>, TState), (TIn, TState)>>
    {
        ...
        public RetryCoordinator(Func<TState, Option<(TIn, TState)>> retryWith)
        {
            ...
            Shape = new BidiShape<(TIn, TState), (Result<TOut>, TState), (Result<TOut>, TState), (TIn, TState)>(In1,
                Out1, In2, Out2);
        }

        public override BidiShape<(TIn, TState), (Result<TOut>, TState), (Result<TOut>, TState), (TIn, TState)> Shape
        {
            get;
        }
    ...
    private class RetryConcatCoordinator<TIn, TState, TOut> : GraphStage<
        BidiShape<(TIn, TState), (Result<TOut>, TState), (Result<TOut>, TState), (TIn, TState)>>
    {
                        ...
                        else
                        {
                            ...
                            if (items.Count + _queue.Count > retry._limit)
                            {
                                FailStage(new IllegalStateException(
                                    $"Queue limit of {retry._limit} has been exceeded. Trying to append {items.Count} elements to a queue that has {_queue.Count} elements."));
                            }
                            else
                            {
