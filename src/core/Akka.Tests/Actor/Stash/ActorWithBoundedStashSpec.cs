...
#pragma warning restore CS0618 // Type or member is obsolete
{
    ...
    private void AfterWorldBehavior(object message)
    {
        Stash.Stash();
    }
}
...
public class ActorWithBoundedStashSpec : AkkaSpec
{
    ...
    {
        base.AtStartup();
        Sys.EventStream.Publish(EventFilter.Warning(new Regex(".*Received dead letter from.*hello.*")).Mute());
    }
    ...
    [Fact(Skip =
        "DequeWrapperMessageQueue implementations are not actually double-ended queues, so this cannot currently work.")]
    public void
        An_actor_with_stash_must_end_up_in_DeadLetters_in_case_of_a_capacity_violation_when_configure_via_dispatcher()
    {
    ...
    [Fact(Skip =
        "DequeWrapperMessageQueue implementations are not actually double-ended queues, so this cannot currently work.")]
    public void
        An_actor_with_stash_must_end_up_in_DeadLetters_in_case_of_a_capacity_violation_when_configure_via_mailbox()
    {
    ...
    [Fact]
    public void
        An_actor_with_stash_must_throw_a_StashOverflowException_in_case_of_a_stash_capacity_violation_when_configured_via_dispatcher()
    {
    ...
    [Fact]
    public void
        An_actor_with_stash_must_throw_a_StashOverflowException_in_case_of_a_stash_capacity_violation_when_configured_via_mailbox()
    {
    ...
    public void An_actor_with_stash_must_get_stash_capacity_from_deployment_overridden_by_config()
    {
        ...
        var stasher =
            Sys.ActorOf(
                Props.Create<StashingActorWithOverflow>().WithStashCapacity(10)
                    .WithDispatcher("my-aliased-dispatcher-2"), "configStashingActor");
    ...
    {
        // deployment configuration settings should override dispatcher settings
        var stasher = Sys.ActorOf(Props.Create<StashingActorWithOverflow>().WithStashCapacity(10)
            .WithDispatcher("my-aliased-dispatcher-2"));
