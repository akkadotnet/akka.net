...
public abstract class SupervisorStrategy : ISurrogated
{
    ...
    public static IDecider DefaultDecider = Actor.Decider.From(Directive.Restart,
    ...
    /// <summary>
    ///     Determines which <see cref="Directive" /> this strategy uses to handle
    ///     <paramref name="exception">exceptions</paramref>
    ...
    public bool HandleFailure(ActorCell actorCell, IActorRef child, Exception cause, ChildRestartStats stats,
    ...
    /// <param name="stats">
    ///     The stats for the child that failed. The ActorRef to the child can be obtained via the
    ///     <see cref="ChildRestartStats.Child" /> property
    /// </param>
    /// <param name="children">The stats for all children</param>
    public abstract void ProcessFailure(IActorContext context, bool restart, IActorRef child, Exception cause,
    ...
    protected virtual void LogFailure(IActorContext context, IActorRef child, Exception cause, Directive directive)
    {
        ...
        {
            string message;
            if (cause is ActorInitializationException actorInitializationException &&
                actorInitializationException.InnerException != null)
    ...
    /// <param name="children">TBD</param>
    public abstract void HandleChildTerminated(IActorContext actorContext, IActorRef child,
...
public class OneForOneStrategy : SupervisorStrategy, IEquatable<OneForOneStrategy>
{
    ...
    /// </param>
    /// <param name="withinTimeRange">
    ///     duration of the time window for <paramref name="maxNrOfRetries" />,
    ///     <see cref="Timeout.InfiniteTimeSpan" /> means no window.
    /// </param>
    /// <param name="localOnlyDecider">The mapping used to translate an <see cref="Exception" /> to a <see cref="Directive" />.</param>
    public OneForOneStrategy(int? maxNrOfRetries, TimeSpan? withinTimeRange,
        Func<Exception, Directive> localOnlyDecider)
        : this(maxNrOfRetries.GetValueOrDefault(-1),
            (int)withinTimeRange.GetValueOrDefault(Timeout.InfiniteTimeSpan).TotalMilliseconds, localOnlyDecider)
    {
    }
    ...
    /// </param>
    /// <param name="withinTimeRange">
    ///     duration of the time window for maxNrOfRetries, System.Threading.Timeout.InfiniteTimeSpan
    ///     means no window.
    /// </param>
    ...
    public OneForOneStrategy(int? maxNrOfRetries, TimeSpan? withinTimeRange, IDecider decider)
        : this(maxNrOfRetries.GetValueOrDefault(-1),
            (int)withinTimeRange.GetValueOrDefault(Timeout.InfiniteTimeSpan).TotalMilliseconds, decider)
    {
    }
    ...
    ///     if the limit is exceeded the child actor is stopped.
    /// </param>
    /// <param name="withinTimeMilliseconds">
    ///     duration in milliseconds of the time window for <paramref name="maxNrOfRetries" />
    ///     , negative values means no window.
    /// </param>
    /// <param name="localOnlyDecider">The mapping used to translate an <see cref="Exception" /> to a <see cref="Directive" />.</param>
    /// <param name="loggingEnabled">If <c>true</c> failures will be logged</param>
    public OneForOneStrategy(int maxNrOfRetries, int withinTimeMilliseconds,
        Func<Exception, Directive> localOnlyDecider, bool loggingEnabled = true)
    ...
    /// <param name="decider">The mapping used to translate an <see cref="Exception" /> to a <see cref="Directive" />.</param>
    /// <param name="loggingEnabled">If <c>true</c> failures will be logged</param>
    public OneForOneStrategy(int maxNrOfRetries, int withinTimeMilliseconds, IDecider decider,
    ...
    /// <summary>
    ///     The duration in milliseconds of the time window for <see cref="MaxNumberOfRetries" />, negative values means no
    ///     window.
    /// </summary>
    ...
    /// </summary>
    public override IDecider Decider => _decider;
    ...
    ///     This exception is thrown if the <see cref="Decider" /> is of type
    ///     <see cref="LocalOnlyDecider" />.
    /// </exception>
...
public class AllForOneStrategy : SupervisorStrategy, IEquatable<AllForOneStrategy>
{
    ...
    /// </param>
    /// <param name="withinTimeRange">
    ///     duration of the time window for maxNrOfRetries, <see cref="Timeout.InfiniteTimeSpan" />
    ///     means no window.
    /// </param>
    /// <param name="localOnlyDecider">The mapping used to translate an <see cref="Exception" /> to a <see cref="Directive" />.</param>
    public AllForOneStrategy(int? maxNrOfRetries, TimeSpan? withinTimeRange,
        Func<Exception, Directive> localOnlyDecider)
        : this(maxNrOfRetries.GetValueOrDefault(-1),
            (int)withinTimeRange.GetValueOrDefault(Timeout.InfiniteTimeSpan).TotalMilliseconds, localOnlyDecider)
    {
    ...
    /// <param name="localOnlyDecider">The mapping used to translate an <see cref="Exception" /> to a <see cref="Directive" />.</param>
    /// <param name="loggingEnabled">If <c>true</c> failures will be logged</param>
    public AllForOneStrategy(int maxNrOfRetries, int withinTimeMilliseconds,
        Func<Exception, Directive> localOnlyDecider, bool loggingEnabled = true)
        : this(maxNrOfRetries, withinTimeMilliseconds, new LocalOnlyDecider(localOnlyDecider), loggingEnabled)
    {
    }
    ...
    /// <param name="decider">The mapping used to translate an <see cref="Exception" /> to a <see cref="Directive" />.</param>
    /// <param name="loggingEnabled">If <c>true</c> failures will be logged</param>
    public AllForOneStrategy(int maxNrOfRetries, int withinTimeMilliseconds, IDecider decider,
        bool loggingEnabled = true)
    {
        MaxNumberOfRetries = maxNrOfRetries;
        WithinTimeRangeMilliseconds = withinTimeMilliseconds;
        ...
        ChildRestartStats stats, IReadOnlyCollection<ChildRestartStats> children)
    {
        if (children.Count > 0)
        {
            if (restart && children.All(
                    c => c.RequestRestartPermission(MaxNumberOfRetries, WithinTimeRangeMilliseconds)))
                foreach (var crs in children)
                    RestartChild(crs.Child, cause, !child.Equals(crs.Child));
            else
                foreach (var crs in children)
...
public static class Decider
{
    ...
    /// <param name="localOnlyDecider">The mapping used to translate an <see cref="Exception" /> to a <see cref="Directive" />.</param>
    /// <returns>
    ///     A <see cref="LocalOnlyDecider" /> that uses the specified <paramref name="localOnlyDecider" /> to map
            ...
            default:
                ...
                if (configuratorType == null)
                    throw new ConfigurationException(
                        $"Could not resolve SupervisorStrategyConfigurator type {typeName}");
