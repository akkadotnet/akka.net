...
public class FlowLogSpec : AkkaSpec
{
    ...
    {
        return LogProbe.ReceiveN(n).Cast<Debug>().Select(x => x.Message.ToString()).ToArray();
    }
    ...
    {
        var log = new BusLogging(Sys.EventStream, "com.example.ImportantLogger", LogType,
    ...
    public void A_Log_on_source_must_allow_configuring_log_levels_via_Attributes()
    {
        ...
        error.Message.ToString().Should()
            .Be("[flow-6e] Upstream failed, cause: Akka.Streams.TestKit.TestException test");
    }
