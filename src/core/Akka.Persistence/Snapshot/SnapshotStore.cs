...
public abstract class SnapshotStore : ActorBase
{
    ...
    ///     This exception is thrown when the associated Persistence extension has not been used in current actor system
    ///     context.
    /// </exception>
    ...
    private bool ReceiveSnapshotStore(object message)
    {
        ...
        if (message is LoadSnapshot loadSnapshot)
        {
            if (loadSnapshot.Criteria == SnapshotSelectionCriteria.None)
                ...
                _breaker.WithCircuitBreaker(() =>
                        LoadAsync(loadSnapshot.PersistenceId, loadSnapshot.Criteria.Limit(loadSnapshot.ToSequenceNr)))
        ...
        {
            var metadata = new SnapshotMetadata(saveSnapshot.Metadata.PersistenceId, saveSnapshot.Metadata.SequenceNr,
                DateTime.UtcNow);
