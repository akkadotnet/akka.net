...
public class SubscriberFluentBuilder<T>
{
    ...
    /// <summary>
    ///     Execute the async chain and then receive messages for a given duration or until one does not match a given filter
    ///     function.
    ///     NOTE: This method will execute the async chain
    /// </summary>
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        await ExecuteAsync(cancellationToken: cancellationToken)
            .ConfigureAwait(false);
        await foreach (var item in ManualProbe<T>.ReceiveWithinTask<TOther>(Probe.TestProbe, max, messages,
    ...
    /// <summary>
    ///     Execute the async chain and then execute code block while bounding its execution time with a
    ///     <paramref name="max" /> timeout.
    ///     <![CDATA[
    ...
    /// <summary>
    ///     Execute the async chain and then attempt to drain the stream into an IAsyncEnumerable (by requesting long.MaxValue
    ///     elements).
    ///     NOTE: This method will execute the async chain
    /// </summary>
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        await ExecuteAsync(cancellationToken: cancellationToken)
            .ConfigureAwait(false);
        await foreach (var item in ManualProbe<T>.ToStrictTask(Probe, atMost, cancellationToken)) yield return item;
    }
    ...
    public SubscriberFluentBuilder<T> EnsureSubscription()
    {
        if (!(Probe is Probe<T> probe))
            throw new InvalidOperationException(
                "EnsureSubscription() can only be used on a TestSubscriber.Probe<T> instance");
    ...
    public SubscriberFluentBuilder<T> RequestNextN(params T[] elements)
    {
        if (!(Probe is Probe<T> probe))
            throw new InvalidOperationException(
                "RequestNextN() can only be used on a TestSubscriber.Probe<T> instance");
        _tasks.Add(async ct =>
        {
            await Probe<T>.EnsureSubscriptionTask(probe, ct);
