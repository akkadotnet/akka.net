...
public class SplitBrainResolverDowningSpec : MultiNodeClusterSpec
{
    ...
    protected SplitBrainResolverDowningSpec(SplitBrainDowningSpecConfig config) : base(config,
        typeof(SplitBrainResolverDowningSpec))
    {
        _config = config;
    }
    ...
    private void A_Cluster_must_detect_network_partition_and_down_minor_part_of_the_cluster()
    {
        ...
        RunOn(() => { Cluster.RegisterOnMemberRemoved(() => downed = true); }, minority);
        ...
        RunOn(() =>
        {
            ...
            AwaitMembersUp(majority.Length, minority.Select(GetAddress).ToImmutableHashSet());
