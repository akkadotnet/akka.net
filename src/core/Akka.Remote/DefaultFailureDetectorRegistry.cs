...
public class DefaultFailureDetectorRegistry<T> : IFailureDetectorRegistry<T>
{
    ...
    private readonly AtomicReference<ImmutableDictionary<T, FailureDetector>> _resourceToFailureDetector =
        new(ImmutableDictionary<T, FailureDetector>.Empty);
    ...
    {
        get => _resourceToFailureDetector.Value;
    ...
    public void Reset()
    {
        while (true)
        {
            ...
            if (!_resourceToFailureDetector.CompareAndSet(oldTable, ImmutableDictionary<T, FailureDetector>.Empty))
                continue;
            break;
        }
    }
