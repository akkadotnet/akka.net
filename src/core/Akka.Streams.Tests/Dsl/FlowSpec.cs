...
public class FlowSpec : AkkaSpec
{
    ...
    public FlowSpec(ITestOutputHelper helper) : base(Config.WithFallback(StreamTestDefaultMailbox.DefaultConfig),
        helper)
    {
    ...
    [Fact]
    public async Task
        A_Flow_with_multiple_subscribers_FanOutBox_must_support_incoming_subscriber_while_elements_were_requested_before()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var setup = await new ChainSetup<string, string, NotUsed>(Identity, Settings.WithInputBuffer(1, 1),
            ...
            await downstream2.ExpectNoMsgAsync(
                TimeSpan.FromMilliseconds(
                    100)); // as nothing was requested yet, fanOutBox needs to cache element in this case
    ...
    [Fact]
    public async Task
        A_Flow_with_multiple_subscribers_FanOutBox_must_be_unblocked_when_blocking_subscriber_cancels_subscription()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var setup = await new ChainSetup<string, string, NotUsed>(Identity, Settings.WithInputBuffer(1, 1),
    ...
    [Fact]
    public async Task
        A_Flow_with_multiple_subscribers_FanOutBox_must_call_future_subscribers_OnError_after_OnSubscribe_if_initial_upstream_was_completed()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var setup = await new ChainSetup<string, string, NotUsed>(Identity, Settings.WithInputBuffer(1, 1),
    ...
    [Fact]
    public async Task
        A_Flow_with_multiple_subscribers_FanOutBox_must_call_future_subscribers_OnError_should_be_called_instead_of_OnSubscribed_after_initial_upstream_reported_an_error()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
    ...
    [Fact]
    public async Task
        A_Flow_with_multiple_subscribers_FanOutBox_must_call_future_subscribers_OnError_when_all_subscriptions_were_cancelled()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var setup = await new ChainSetup<string, string, NotUsed>(Identity, Settings.WithInputBuffer(1, 1),
    ...
    [Fact]
    public async Task
        A_broken_Flow_must_cancel_upstream_and_call_onError_on_current_and_future_downstream_subscribers_if_an_internal_error_occurs()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var setup = await new ChainSetup<string, string, NotUsed>(FaultyFlow<string, string, string>,
                    Settings.WithInputBuffer(1, 1),
                    (settings, factory) => ActorMaterializer.Create(factory, settings),
                    (source, materializer) => ToFanoutPublisher(source, materializer, 16), this)
                .InitializeAsync();
            ...
            {
                new ErrorFilter(typeof(NullReferenceException)), new ErrorFilter(typeof(IllegalStateException)),
                new ErrorFilter(
                    typeof(PostRestartException)) // This is thrown because we attach the dummy failing actor to toplevel
            };
    ...
    private static Flow<TIn, TOut, TMat> Identity<TIn, TOut, TMat>(Flow<TIn, TOut, TMat> flow)
    {
        return flow.Select(e => e);
    }

    private static Flow<TIn, TOut, TMat> Identity2<TIn, TOut, TMat>(Flow<TIn, TOut, TMat> flow)
    {
    ...
    private Flow<TIn, TOut2, NotUsed> FaultyFlow<TIn, TOut, TOut2>(Flow<TIn, TOut, NotUsed> flow) where TOut : TOut2
    {
        Flow<TOut, TOut2, NotUsed> CreateGraph()
        {
            ...
            var assembly = new GraphAssembly(
                new IGraphStageWithMaterializedValue<Shape, object>[] { stage },
                new[] { Attributes.None },
                new Inlet[] { stage.Shape.Inlet, null },
                new[] { 0, -1 },
                new Outlet[] { null, stage.Shape.Outlet },
                new[] { -1, 0 });

            var (connections, logics) = assembly.Materialize(
                Attributes.None,
                assembly.Stages.Select(s => s.Module).ToArray(),
    ...
    {
        return flow.RunWith(Source.AsSubscriber<TIn>(), Sink.AsPublisher<TOut>(false), materializer);
    }
