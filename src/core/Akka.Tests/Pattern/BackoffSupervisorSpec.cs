...
public class BackoffSupervisorSpec : AkkaSpec
{
    ...
    public async Task BackoffSupervisor_must_support_custom_supervision_strategy()
    {
        ...
        await EventFilter.Exception<TestException>().ExpectAsync(2, async () =>
        {
            ...
            {
                if (ex is TestException) return Directive.Stop;
            ...
            {
                if (ex is TestException) return Directive.Restart;
    ...
    public async Task BackoffSupervisor_must_reply_to_sender_if_replyWhileStopped_is_specified()
    {
        await EventFilter.Exception<TestException>().ExpectAsync(1, async () =>
        {
            var supervisor = Create(Backoff
                .OnFailure(Child.Props(TestActor), "c1", TimeSpan.FromSeconds(100), TimeSpan.FromSeconds(300), 0.2, -1)
    ...
    public async Task BackoffSupervisor_must_not_reply_to_sender_if_replyWhileStopped_is_not_specified()
    {
        await EventFilter.Exception<TestException>().ExpectAsync(1, async () =>
        {
            var supervisor = Create(Backoff.OnFailure(Child.Props(TestActor), "c1", TimeSpan.FromSeconds(100),
                TimeSpan.FromSeconds(300), 0.2, -1));
            supervisor.Tell(BackoffSupervisor.GetCurrentChild.Instance);
    ...
    [Theory]
    [ClassData(typeof(DelayTable))]
    public void BackoffSupervisor_must_correctly_calculate_the_delay(int restartCount, TimeSpan minBackoff,
        TimeSpan maxBackoff, double randomFactor, TimeSpan expectedResult)
    {
        Assert.Equal(expectedResult,
            BackoffSupervisor.CalculateDelay(restartCount, minBackoff, maxBackoff, randomFactor));
    }

    [LocalFact(SkipLocal = "Racy on Azure DevOps")]
    public async Task
        BackoffSupervisor_must_stop_restarting_the_child_after_reaching_maxNrOfRetries_limit_using_BackOff_OnStop()
    {
        var supervisor = Create(OnStopOptions(2));
    ...
    [LocalFact(SkipLocal = "Racy on Azure DevOps")]
    public async Task
        BackoffSupervisor_must_stop_restarting_the_child_after_reaching_maxNrOfRetries_limit_using_BackOff_OnFailure()
    {
        await EventFilter.Exception<TestException>().ExpectAsync(3, async () =>
        {
            var supervisor = Create(OnFailureOptions(2));
    ...
    [Fact]
    public async Task
        BackoffSupervisor_must_stop_restarting_the_child_if_final_stop_message_received_using_BackOff_OnStop()
    {
        const string stopMessage = "stop";
        var supervisor =
            Create(OnStopOptions(100).WithFinalStopMessage(message => ReferenceEquals(message, stopMessage)));
        supervisor.Tell(BackoffSupervisor.GetCurrentChild.Instance);
        var c1 = (await ExpectMsgAsync<BackoffSupervisor.CurrentChild>()).Ref;
    ...
    internal class DelayTable : IEnumerable<object[]>
    {
        private readonly List<object[]> delayTable = new()
        {
            ...
            new object[]
            {
                int.MaxValue, TimeSpan.FromDays(10000), TimeSpan.FromDays(10000), 0d, TimeSpan.FromDays(10000)
            }
        };

        public IEnumerator<object[]> GetEnumerator()
        {
            return delayTable.GetEnumerator();
        }
...
#pragma warning disable CS0162 // Disabled because without the return, the compiler complains about ambigious reference between Receive<T>(Action<T>,Predicate<T>) and Receive<T>(Predicate<T>,Action<T>)
        public Child(IActorRef probe)
        {
            ...
            ReceiveAny(msg => { _probe.Tell(msg); });
        }
...
#pragma warning restore CS0162
    ...
    private BackoffOptions OnStopOptions(int maxNrOfRetries = -1)
    {
        return OnStopOptions(Child.Props(TestActor), maxNrOfRetries);
    }

    private BackoffOptions OnStopOptions(Props props, int maxNrOfRetries = -1)
    {
        return Backoff.OnStop(props, "c1", 100.Milliseconds(), 3.Seconds(), 0.2, maxNrOfRetries);
    }

    private BackoffOptions OnFailureOptions(int maxNrOfRetries = -1)
    {
        return OnFailureOptions(Child.Props(TestActor), maxNrOfRetries);
    }

    private BackoffOptions OnFailureOptions(Props props, int maxNrOfRetries = -1)
    {
        return Backoff.OnFailure(props, "c1", 100.Milliseconds(), 3.Seconds(), 0.2, maxNrOfRetries);
    }

    private IActorRef Create(BackoffOptions options)
    {
        return Sys.ActorOf(BackoffSupervisor.Props(options));
    }
