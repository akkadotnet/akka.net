...
public class CachedMatchCompilerTests
{
    ...
    {
        //Arrange
        var argumentValues = Array.Empty<object>();
        ...
        var expressionCompiler = new DummyLambdaExpressionCompiler { CompileResult = deleg };

        PartialAction<object> partialAction = _ => true;
        var partialActionBuilder = new DummyPartialActionBuilder { BuildResult = partialAction };

        var compiler =
            new CachedMatchCompiler<object>(matchExpressionBuilder, partialActionBuilder, expressionCompiler);
        ...
        //Assert
        AssertOneCall(matchExpressionBuilder.BuildLambdaExpressionCalls, typeHandlers, "BuildLambdaExpression");
        AssertOneCall(expressionCompiler.CompileCalls, lambdaExpression, "Compile");
        AssertOneCall(partialActionBuilder.BuildCalls, "Build",
            i => ReferenceEquals(i.CompiledDelegate, deleg) && ReferenceEquals(i.DelegateArguments, argumentValues));
        Assert.Same(partialAction, resultPartialAction);

        AssertNoCall(matchExpressionBuilder.CreateArgumentValuesArrayCalls, "CreateArgumentValuesArray");
    }
    ...
    public void When_compiling_second_time_with_same_signature_the_cached_version_should_be_used()
    {
        ...
        AssertOneCall(matchExpressionBuilder.CreateArgumentValuesArrayCalls, arguments, "CreateArgumentValuesArray");
        AssertOneCall(partialActionBuilder.BuildCalls, "Build",
            i => ReferenceEquals(i.CompiledDelegate, deleg) &&
                 ReferenceEquals(i.DelegateArguments, Array.Empty<object>()));
        Assert.Same(partialAction, resultPartialAction);

        AssertNoCall(matchExpressionBuilder.BuildLambdaExpressionCalls, "BuildLambdaExpression");
        AssertNoCall(expressionCompiler.CompileCalls, "Compile");
    }
    ...
    {
        public readonly List<CompiledMatchHandlerWithArguments> BuildCalls = new();
    ...
    {
        public readonly List<LambdaExpression> CompileCalls = new();
