...
{
    private static readonly ImmutableHashSet<string> aRoles = ImmutableHashSet.Create("AA", "AB");

    private static readonly Member aJoining =
        TestMember.Create(new Address("akka.tcp", "sys", "a", 2552), MemberStatus.Joining, aRoles);

    private static readonly Member aUp =
        TestMember.Create(new Address("akka.tcp", "sys", "a", 2552), MemberStatus.Up, aRoles);

    private static readonly Member aRemoved =
        TestMember.Create(new Address("akka.tcp", "sys", "a", 2552), MemberStatus.Removed, aRoles);

    private static readonly ImmutableHashSet<string> bRoles = ImmutableHashSet.Create("AB", "BB");

    private static readonly Member bUp =
        TestMember.Create(new Address("akka.tcp", "sys", "b", 2552), MemberStatus.Up, bRoles);

    private static readonly Member bDown =
        TestMember.Create(new Address("akka.tcp", "sys", "b", 2552), MemberStatus.Down, bRoles);

    private static readonly ImmutableHashSet<string> cRoles = ImmutableHashSet.Create<string>();

    private static readonly Member cUp =
        TestMember.Create(new Address("akka.tcp", "sys", "c", 2552), MemberStatus.Up, cRoles);

    private static readonly Member cLeaving =
        TestMember.Create(new Address("akka.tcp", "sys", "c", 2552), MemberStatus.Leaving, cRoles);

    private static readonly ImmutableHashSet<string> dRoles = ImmutableHashSet.Create("DD", "DE");

    private static readonly Member dLeaving =
        TestMember.Create(new Address("akka.tcp", "sys", "d", 2552), MemberStatus.Leaving, dRoles);

    private static readonly Member dExiting =
        TestMember.Create(new Address("akka.tcp", "sys", "d", 2552), MemberStatus.Exiting, dRoles);

    private static readonly Member dRemoved =
        TestMember.Create(new Address("akka.tcp", "sys", "d", 2552), MemberStatus.Removed, dRoles);

    private static readonly ImmutableHashSet<string> eRoles = ImmutableHashSet.Create("EE", "DE");

    private static readonly Member eJoining =
        TestMember.Create(new Address("akka.tcp", "sys", "e", 2552), MemberStatus.Joining, eRoles);

    private static readonly Member eUp =
        TestMember.Create(new Address("akka.tcp", "sys", "e", 2552), MemberStatus.Up, eRoles);

    private static readonly Member eDown =
        TestMember.Create(new Address("akka.tcp", "sys", "e", 2552), MemberStatus.Down, eRoles);

    private static readonly UniqueAddress selfDummyAddress = new(new Address("akka.tcp", "sys", "selfDummy", 2552), 17);
    ...
    public void DomainEvents_must_be_empty_for_the_same_gossip()
    {
        ...
        ClusterEvent.DiffUnreachable(State(g1), State(g1))
            .Should()
    ...
    public void DomainEvents_must_be_produced_for_new_members()
    {
        ...
        ClusterEvent.DiffMemberEvents(State(g1), State(g2))
            .Should()
            .BeEquivalentTo(ImmutableList.Create<ClusterEvent.IMemberEvent>(new ClusterEvent.MemberUp(bUp),
                new ClusterEvent.MemberJoined(eJoining)));
        ...
        ClusterEvent.DiffSeen(State(g1), State(g2))
            .Should()
            .BeEquivalentTo(
                ImmutableList.Create(new ClusterEvent.SeenChanged(true,
                    s2.Select(s => s.Address).ToImmutableHashSet())));
    }
    ...
    public void DomainEvents_must_be_produced_for_changed_status_of_members()
    {
        ...
        ClusterEvent.DiffMemberEvents(State(g1), State(g2))
            .Should()
            .BeEquivalentTo(ImmutableList.Create<ClusterEvent.IMemberEvent>(new ClusterEvent.MemberUp(aUp),
                new ClusterEvent.MemberLeft(cLeaving), new ClusterEvent.MemberJoined(eJoining)));
    ...
    {
        var reachability1 = Reachability.Empty.Unreachable(aUp.UniqueAddress, cUp.UniqueAddress)
        ...
        var reachability2 = reachability1.Unreachable(aUp.UniqueAddress, bDown.UniqueAddress);
        ...
        ClusterEvent.DiffUnreachable(new MembershipState(g1, bDown.UniqueAddress),
                new MembershipState(g2, bDown.UniqueAddress))
            .Should()
            .BeEquivalentTo(ImmutableList.Create<ClusterEvent.UnreachableMember>());
    ...
    {
        var reachability1 = Reachability.Empty.Unreachable(aUp.UniqueAddress, cUp.UniqueAddress)
            .Reachable(aUp.UniqueAddress, cUp.UniqueAddress).Unreachable(aUp.UniqueAddress, eUp.UniqueAddress)
            .Unreachable(aUp.UniqueAddress, bUp.UniqueAddress);
        var g1 = new Gossip(ImmutableSortedSet.Create(aUp, bUp, cUp, eUp), new GossipOverview(reachability1));
        var reachability2 = reachability1.Unreachable(aUp.UniqueAddress, cUp.UniqueAddress)
        ...
        // never include self member in unreachable
        ClusterEvent.DiffUnreachable(new MembershipState(g1, cUp.UniqueAddress),
                new MembershipState(g2, cUp.UniqueAddress))
            .Should()
            .BeEquivalentTo(ImmutableList.Create<ClusterEvent.UnreachableMember>());
        ...
        ClusterEvent.DiffReachable(new MembershipState(g1, bUp.UniqueAddress),
                new MembershipState(g2, bUp.UniqueAddress))
            .Should()
    ...
    public void DomainEvents_must_be_produced_for_removed_members()
    {
        ...
        ClusterEvent.DiffMemberEvents(State(g1), State(g2))
            .Should()
            .BeEquivalentTo(
                ImmutableList.Create<ClusterEvent.IMemberEvent>(
                    new ClusterEvent.MemberRemoved(dRemoved, MemberStatus.Exiting)));
    ...
    public void DomainEvents_must_be_produced_for_convergence_changes()
    {
        ...
        ClusterEvent.DiffSeen(State(g1), State(g2))
            .Should()
            .BeEquivalentTo(ImmutableList.Create(new ClusterEvent.SeenChanged(true,
                ImmutableHashSet.Create(aUp.Address, bUp.Address))));
        ...
        ClusterEvent.DiffSeen(State(g2), State(g1))
            .Should()
            .BeEquivalentTo(ImmutableList.Create(new ClusterEvent.SeenChanged(true,
                ImmutableHashSet.Create(aUp.Address, bUp.Address, eJoining.Address))));
    }
    ...
    public void DomainEvents_must_be_produced_for_leader_changes()
    {
        ...
        ClusterEvent.DiffSeen(State(g1), State(g2))
            ...
            .BeEquivalentTo(
                ImmutableList.Create(new ClusterEvent.SeenChanged(true,
                    s2.Select(a => a.Address).ToImmutableHashSet())));
    ...
    public void DomainEvents_must_be_produced_for_role_leader_changes()
    {
        ...
        ClusterEvent.DiffRolesLeader(State(g0), State(g1)).Should().BeEquivalentTo(expected);
        ...
        ClusterEvent.DiffRolesLeader(State(g1), State(g2)).Should().BeEquivalentTo(expected2);
    }
}