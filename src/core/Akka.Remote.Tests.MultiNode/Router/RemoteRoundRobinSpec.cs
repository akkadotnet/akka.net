...
public class RemoteRoundRobinSpec : MultiNodeSpec
{
    ...
    public void
        A_remote_round_robin_pool_must_be_locally_instantiated_on_a_remote_node_and_be_able_to_communicate_through_its_remote_actor_ref()
    {
        RunOn(() => { EnterBarrier("start", "broadcast-end", "end"); }, _config.First, _config.Second, _config.Third);

        RunOn(() =>
        {
            EnterBarrier("start");
            var actor = Sys.ActorOf(new RoundRobinPool(0)
        ...
        A_remote_round_robin_pool_with_resizer_must_be_locally_instantiated_on_a_remote_node_after_several_resize_rounds()
    {
        ...
        {
            RunOn(() => { EnterBarrier("start", "broadcast-end", "end"); }, _config.First, _config.Second,
                _config.Third);
            ...
            RunOn(() =>
            {
                ...
                var repliesFrom = ImmutableHashSet.Create<IActorRef>();
                ...
                {
                    Node(_config.First).Address, Node(_config.Second).Address, Node(_config.Third).Address
                });
    ...
    public void A_remote_round_robin_group_must_send_messages_with_actor_selection_to_remote_paths()
    {
        RunOn(() =>
        {
            Sys.ActorOf<SomeActor>("target-" + Myself.Name);
    ...
    private class SomeActor : UntypedActor
    {
        ...
        {
            if (message.Equals("hit")) Sender.Tell(Self);
        }
    }
