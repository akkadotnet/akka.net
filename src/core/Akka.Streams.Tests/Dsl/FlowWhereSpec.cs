...
namespace Akka.Streams.Tests.Dsl
{
    ...
    public class FlowWhereSpec : ScriptedTest
    {
        ...
        public void A_Where_must_filter()
        {
            ...
            RandomTestRange(Sys).ToList().Select(async _ => await RunScriptAsync(script, Settings, flow => flow.Where(x => x%2 == 0)));
        }
        ...
        public void A_WhereNot_must_filter_based_on_inverted_predicate()
        {
            ...
            RandomTestRange(Sys).ToList().Select(async _ => await RunScriptAsync(script, Settings, flow => flow.WhereNot(x => x % 2 == 0)));
        }
    }
}
