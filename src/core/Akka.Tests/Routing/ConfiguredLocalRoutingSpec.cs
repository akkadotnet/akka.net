...
public class ConfiguredLocalRoutingSpec : AkkaSpec
{
    ...
    public async Task RouterConfig_must_be_overridable_in_explicit_deployment()
    {
        var actor = Sys.ActorOf(
            FromConfig.Instance.Props(Props.Create<EchoProps>()).WithDeploy(new Deploy(new RoundRobinPool(12))),
            "someOther");
        var routerConfig = RouterConfig(actor);
    ...
    {
        Assert.Throws<ConfigurationException>(() => { Sys.ActorOf(FromConfig.Instance.Props()); });
    }
    ...
    public async Task RouterConfig_must_not_get_confused_when_trying_to_wildcard_configure_children()
    {
        ...
        var expected = new List<string> { "a", "b", "c" }
            .Select(i => Sys.ActorSelection("/user/weird/$" + i).ResolveOne(RemainingOrDefault).Result).ToList();
    ...
    private class Parent : ReceiveActor
    {
        public Parent()
        {
            Receive<PropsName>(c => { Sender.Tell(Context.ActorOf(c.Props, c.Name)); });
        }
    }
