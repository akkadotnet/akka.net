...
public class HoconValue : IMightBeAHoconObject
{
    ...
    /// <param name="adoptedFromFallback">
    ///     Indicates whether this instance was constructed during association with fallback
    ///     <see cref="Config" />.
    /// </param>
    ...
    private static ByteSize[] ByteSizes { get; } =
    {
        new()
        {
            Factor = 1024L * 1024L * 1024L * 1024L * 1024 * 1024L,
            Suffixes = new[] { "E", "e", "Ei", "EiB", "exbibyte", "exbibytes" }
        },
        new()
        {
            Factor = 1000L * 1000L * 1000L * 1000L * 1000L * 1000L,
            Suffixes = new[] { "EB", "exabyte", "exabytes" }
        },
        new()
        {
        ...
        new() { Factor = 1000L * 1000L * 1000L * 1000L, Suffixes = new[] { "TB", "terabyte", "terabytes" } },
        new()
        {
            Factor = 1024L * 1024L * 1024L, Suffixes = new[] { "G", "g", "Gi", "GiB", "gibibyte", "gibibytes" }
        },
        new() { Factor = 1000L * 1000L * 1000L, Suffixes = new[] { "GB", "gigabyte", "gigabytes" } },
        new() { Factor = 1024L * 1024L, Suffixes = new[] { "M", "m", "Mi", "MiB", "mebibyte", "mebibytes" } },
        new() { Factor = 1000L * 1000L, Suffixes = new[] { "MB", "megabyte", "megabytes" } },
        new() { Factor = 1024L, Suffixes = new[] { "K", "k", "Ki", "KiB", "kibibyte", "kibibytes" } },
        new() { Factor = 1000L, Suffixes = new[] { "kB", "kilobyte", "kilobytes" } },
        new() { Factor = 1, Suffixes = new[] { "b", "B", "byte", "bytes" } }
    };
    ...
    {
        if (IsString()) return ConcatString();
    ...
    ///     <see href="https://github.com/lightbend/config/blob/master/HOCON.md#size-in-bytes-format">byte size suffix</see>,
    ///     from
    ...
    public virtual string ToString(int indent)
    {
        if (IsString())
        {
            var text = QuoteIfNeeded(GetString());
            return text;
        }

        if (IsObject())
        {
            if (indent == 0) return GetObject().ToString(indent + 1);

            var i = new string(' ', indent * 2);
            return string.Format("{{\r\n{1}{0}}}", i, GetObject().ToString(indent + 1));
        }

        if (IsArray()) return string.Format("[{0}]", string.Join(",", GetArray().Select(e => e.ToString(indent + 1))));
    ...
    private string QuoteIfNeeded(string text)
    {
        ...
        if (EscapeRegex.IsMatch(text)) return "\"" + text + "\"";
