    ...
    public class MultiplesOfTwo : Akka.TestKit.Xunit2.TestKit
    {
        ...
        private class MultiplesOfTwoPage
        {
            ...
            public Task<PagedSource.Page<int, int>> Page(int key)
            {
                ...
                return Task.FromResult(new PagedSource.Page<int, int>(filteredIndices.Select(x => x * 2),
                    Option<int>.Create(key + 1)));
            }
        }
    }
    ...
    public class LinkedIntPages : Akka.TestKit.Xunit2.TestKit
    {
        ...
        (
            ...
            key =>
            {
                ...
                return Task.FromResult(new PagedSource.Page<int, string>(items,
                    next == "" ? Option<string>.None : Option<string>.Create(next)));
            }
        );
