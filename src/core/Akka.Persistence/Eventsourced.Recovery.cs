...
public abstract partial class Eventsourced
{
    ...
    private EventsourcedState RecoveryStarted(long maxReplays)
    {
        ...
        var timeoutCancelable =
            Context.System.Scheduler.ScheduleTellOnceCancelable(timeout, Self, new RecoveryTick(true), Self);
            ...
            try
            {
                switch (message)
                {
                    case LoadSnapshotResult res:
                    {
                        ...
                        Journal.Tell(new ReplayMessages(LastSequenceNr + 1L, res.ToSequenceNr, maxReplays,
                            PersistenceId, Self));
                        break;
                    }
    ...
    private EventsourcedState Recovering(Receive recoveryBehavior, TimeSpan timeout)
    {
        ...
        var timeoutCancelable =
            Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(timeout, timeout, Self, new RecoveryTick(false),
                Self);
    ...
    private void FlushBatch()
    {
        ...
        {
            foreach (var p in _eventBatch) _journalBatch.Add(p);
            _eventBatch.Clear();
        }
    ...
    /// <summary>
    ///     Event persisting state. Remains until pending events are persisted and then changes state to
    ///     <see cref="ProcessingCommands" />.
