...
public class PersistentFSMSpec : PersistenceSpec
{
    ...
    public void PersistentFSM_must_recover_successfully_with_correct_state_timeout()
    {
        ...
        ExpectNoMsg(TimeSpan
            .FromSeconds(0.6)); // arbitrarily chosen delay, less than the timeout, before stopping the FSM
        fsmRef.Tell(PoisonPill.Instance);
        ExpectTerminated(fsmRef);
    ...
    public async Task PersistentFSM_must_pass_latest_statedata_to_AndThen()
    {
        ...
        var response1 = await actor.Ask<AndThenTestActor.Data>(new AndThenTestActor.Command("Message 1"))
            .ConfigureAwait(true);
        var response2 = await actor.Ask<AndThenTestActor.Data>(new AndThenTestActor.Command("Message 2"))
            .ConfigureAwait(true);
    ...
    internal class WebStoreCustomerFSM : PersistentFSM<IUserState, IShoppingCart, IDomainEvent>
    {
        public WebStoreCustomerFSM(string persistenceId, IActorRef reportActor)
        {
            ...
            When(Shopping.Instance, (evt, state) =>
            {
                ...
                if (evt.FsmEvent is StateTimeout) return GoTo(Inactive.Instance).ForMax(TimeSpan.FromSeconds(2));
            ...
            {
                if (evt.FsmEvent is Leave) return Stop();
        ...
        {
            if (evt is ItemAdded itemAdded) return cartBeforeEvent.AddItem(itemAdded.Item);

            if (evt is OrderExecuted) return cartBeforeEvent;

            if (evt is OrderDiscarded) return cartBeforeEvent.Empty();
...
public class TimeoutFsm : PersistentFSM<TimeoutFsm.ITimeoutState, string, string>
{
    public TimeoutFsm(IActorRef probe)
    {
        ...
        When(Init.Instance, (evt, _) =>
        {
            ...
            if (evt.FsmEvent is OverrideTimeoutToInf)
            {
    ...
    public class Init : ITimeoutState
    {
        ...
        {
            return !ReferenceEquals(obj, null) && obj is Init;
        }

        public override int GetHashCode()
        {
            return nameof(Init).GetHashCode();
        }
    }
...
internal class SimpleTransitionFSM : PersistentFSM<IUserState, IShoppingCart, IDomainEvent>
{
    public SimpleTransitionFSM(string persistenceId, IActorRef reportActor)
    {
        ...
        {
            if (evt.FsmEvent.Equals("stay")) return Stay();
            return GoTo(LookingAround.Instance);
        });

        OnTransition((state, nextState) => { reportActor.Tell($"{state} -> {nextState}"); });
    }
...
internal class PersistentEventsStreamer : PersistentActor
{
    ...
    {
        if (!(message is RecoveryCompleted)) _client.Tell(message);
...
public class Shopping : IUserState
{
    ...
    public override bool Equals(object obj)
    {
        return !ReferenceEquals(obj, null) && obj is Shopping;
    }

    public override int GetHashCode()
    {
        return nameof(Shopping).GetHashCode();
    }

    public override string ToString()
    {
        return nameof(Shopping);
    }
}
...
public class Inactive : IUserState
{
    ...
    public override bool Equals(object obj)
    {
        return !ReferenceEquals(obj, null) && obj is Inactive;
    }

    public override int GetHashCode()
    {
        return nameof(Inactive).GetHashCode();
    }
...
public class Paid : IUserState
{
    ...
    public override bool Equals(object obj)
    {
        return !ReferenceEquals(obj, null) && obj is Paid;
    }

    public override int GetHashCode()
    {
        return nameof(Paid).GetHashCode();
    }
...
public class LookingAround : IUserState
{
    ...
    public override bool Equals(object obj)
    {
        return !ReferenceEquals(obj, null) && obj is LookingAround;
    }

    public override int GetHashCode()
    {
        return nameof(LookingAround).GetHashCode();
    }

    public override string ToString()
    {
        return nameof(LookingAround);
    }
}
...
internal class SnapshotFSM : PersistentFSM<ISnapshotFSMState, List<int>, ISnapshotFSMEvent>
{
    public SnapshotFSM(IActorRef probe)
    {
        ...
        {
            if (evt.FsmEvent is int i) return Stay().Applying(new IntAdded(i));

            if (evt.FsmEvent.Equals("4x"))
            {
                return GoTo(Persist4xAtOnce.Instance);
            }

            if (evt.FsmEvent is SaveSnapshotSuccess snap)
            {
