...
public class TimerSpec : AbstractTimerSpec
{
    ...
    {
        return Props.Create(() => new Target(monitor, interval, repeat, initial ?? (() => 1)));
    }
}
...
public class FsmTimerSpec : AbstractTimerSpec
{
    protected override Props TargetProps(IActorRef monitor, TimeSpan interval, bool repeat, Func<int> initial)
    {
        return Props.Create(() => new FsmTarget(monitor, interval, repeat, initial ?? (() => 1)));
    }
}

public abstract class AbstractTimerSpec : AkkaSpec
{
    private readonly TimeSpan dilatedInterval;
    ...
    internal class Tock : IEvent
    {
        ...
        {
            if (obj is Tock other) return N.Equals(other.N);
            return false;
        }
    }
    ...
    internal class GotPostStop : IEvent
    {
        ...
        public override bool Equals(object obj)
        {
            if (obj is GotPostStop other) return TimerActive.Equals(other.TimerActive);
            return false;
        }
    }
    ...
    internal class GotPreRestart : IEvent
    {
        ...
        public override bool Equals(object obj)
        {
            if (obj is GotPreRestart other) return TimerActive.Equals(other.TimerActive);
            return false;
        }
    }
    ...
    internal class FsmTarget : FSM<TheState, int>
    {
        ...
        private State<TheState, int> Bump(int bumpCount)
        {
            SetTimer("T", new Tick(bumpCount + 1), interval, repeat);
            return Stay().Using(bumpCount + 1);
        }

        private State<TheState, int> AutoReceive()
        {
