...
public class MatchHandlerBuilderTests : AkkaSpec
{
    ...
    public void Test_that_signatures_are_equal()
    {
        ...
        AssertSameSignatures<object>(bldr => bldr.MatchAny(_ => { }),
            bldr => bldr.MatchAny(o => { str = o.ToString(); }));

        AssertSameSignatures<object>(bldr => bldr.MatchAny(_ => { }),
            bldr => bldr.Match<object>(o => { str = o.ToString(); }));

        AssertSameSignatures<object>(bldr => bldr.Match<string>(_ => { }),
            bldr => bldr.Match<string>(s => { str = s; }));
        ...
        AssertSameSignatures<object>(bldr => bldr.Match<string>(_ => { }, _ => true),
            bldr => bldr.Match<string>(_ => { }, _ => false));
    ...
    public void Test_that_signatures_differs()
    {
        ...
        AssertDifferentSignatures<object>(bldr => bldr.Match<string>(_ => { }),
            bldr => bldr.Match<string>(_ => { }, s => s == null));

        AssertDifferentSignatures<object>(bldr => bldr.MatchAny(_ => { }),
            bldr => bldr.Match<object>(_ => { }, _ => true));
        ...
        AssertDifferentSignatures<object>(
            bldr => { bldr.Match(typeof(string), _ => true); },
            bldr => { bldr.Match(typeof(int), _ => true); });
    }
    ...
    private void TestSignature<T>(Action<MatchBuilder<T>> build1, Action<MatchBuilder<T>> build2,
        bool signaturesShouldMatch)
    {
    ...
    private class DummyCompiler<T> : IMatchCompiler<T>
    {
        ...
        public void CompileToMethod(IReadOnlyList<TypeHandler> handlers, IReadOnlyList<Argument> capturedArguments,
            MatchBuilderSignature signature, TypeBuilder typeBuilder, string methodName,
