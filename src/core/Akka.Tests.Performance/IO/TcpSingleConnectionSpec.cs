...
public class TcpSingleConnectionSpec
{
    ...
    public void Setup(BenchmarkContext context)
    {
        ...
        client = system.ActorOf(Props.Create(() =>
            new TestClient(TestEndpoint, outboundThroughputCounter, resetEvent, completion)));
        completion.Task.Wait(Timeout);
    }

    [PerfBenchmark(
        Description =
            "Measures how quickly and with how much GC overhead a client/server actor connection can send 50B messages in both directions",
    ...
    {
        public TestClient(IPEndPoint remoteEndPoint, Counter outboundCounter, ManualResetEventSlim reset,
        ...
        private Action Connected(IActorRef connection, Counter counter, ManualResetEventSlim reset)
        {
            return () =>
            {
                ...
                Receive<byte[]>(data => { connection.Tell(Tcp.Write.Create(ByteString.FromBytes(data))); });
