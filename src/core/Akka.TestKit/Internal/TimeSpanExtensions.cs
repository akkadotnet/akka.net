...
public static class TimeSpanExtensions
{
    ...
    /// <returns>
    ///     <c>true</c> if the given timespan is equal to <see cref="Timeout.InfiniteTimeSpan" />; otherwise, <c>false</c>
    ///     .
    /// </returns>
    ...
    public static void EnsureIsPositiveFinite(this TimeSpan timeSpan, string parameterName)
    {
        if (!IsPositiveFinite(timeSpan))
            throw new ArgumentException($"The timespan must be greater than zero. Actual value: {timeSpan}",
                parameterName);
    }
