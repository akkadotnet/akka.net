...
public static class ActorRefFactoryShared
{
    ...
    {
        return new ActorSelection(((ActorSystemImpl)system).Provider.RootGuardianAt(actorPath.Address),
            actorPath.Elements);
    }
    ...
    public static ActorSelection ActorSelection(string path, ActorSystem system, IActorRef lookupRoot)
    {
        ...
        //no path given
        if (string.IsNullOrEmpty(path)) return new ActorSelection(system.DeadLetters, "");

        //absolute path
        var elements = path.Split('/');
        if (elements[0] == "") return new ActorSelection(provider.RootGuardian, elements.Skip(1));
