...
public class AkkaProtocolStressTest : AkkaSpec
{
    ...
    [Fact]
    public async Task
        AkkaProtocolTransport_must_guarantee_at_most_once_delivery_and_message_ordering_despite_packet_loss()
    {
    ...
    private class SequenceVerifier : UntypedActor
    {
        ...
        {
            if (message is ResendFinal) _controller.Tell((_maxSeq, _losses));
        }
    }
    ...
    private class Echo : UntypedActor
    {
        protected override void OnReceive(object message)
        {
            ...
            if (message is int or long) Sender.Tell(message);
        }
    }
    ...
    private Address AddressB => _systemB.AsInstanceOf<ExtendedActorSystem>().Provider.DefaultAddress;

    private RootActorPath RootB => new(AddressB);
    ...
    protected override void BeforeTermination()
    {
        ...
        _systemB.EventStream.Publish(new Mute(
            new WarningFilter(new RegexMatcher(new Regex("received dead letter.*(InboundPayload|Disassociate)"))),
