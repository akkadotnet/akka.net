...
public sealed class DeadLetter : AllDeadLetters
{
    ...
    public DeadLetter(object message, IActorRef sender, IActorRef recipient) : base(message, sender, recipient)
    {
        ...
        if (recipient == null)
            throw new ArgumentNullException(nameof(recipient), "DeadLetter recipient may not be null");
    }
}

/// <summary>
///     Similar to <see cref="DeadLetter" /> with the slight twist of NOT being logged by the default dead letters
///     listener.
///     Messages which end up being suppressed dead letters are internal messages for which ending up as dead-letter is
...
public sealed class SuppressedDeadLetter : AllDeadLetters
{
    ...
    ///     This exception is thrown when either the sender or the recipient is undefined.
    /// </exception>
    public SuppressedDeadLetter(IDeadLetterSuppression message, IActorRef sender, IActorRef recipient) : base(message,
        sender, recipient)
    {
        if (sender == null)
...
///     When this message was sent without a sender <see cref="IActorRef" />, `sender` will be
///     <see cref="ActorRefs.NoSender" /> , i.e. `null`.
/// </summary>
