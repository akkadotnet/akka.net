...
- [x] 6.4 Send generated payload over classic remoting (ClassicRemotingSpec: two independent `ActorSystem`s over `akka.remote.dot-netty.tcp`, dynamic port, HOCON-registered generated serializer mirroring `Akka.Remote.Tests.Artery.ArteryControlMessageSerializerSpec`'s `Remote.conf`-style registration; covers a scalar+`IActorRef` payload and a nested-value-object payload, both round-tripped in both directions — `src/core/Akka.Serialization.V2.Tests/ClassicRemotingSpec.cs`)
- [x] 6.5 Persist and recover generated event payload (PersistenceIntegrationSpec.Generated_event_should_persist_and_recover_through_journal: test-only `WireFormatJournal`, which genuinely round-trips each event through `Serialization.FindSerializerFor` + `ToBinary`/`FromBinary` and stores only the resulting bytes — the built-in `akka.persistence.journal.inmem` (`MemoryJournal`) intentionally keeps the live object reference and would prove nothing about the wire format, see the spec's doc comment — `src/core/Akka.Serialization.V2.Tests/PersistenceIntegrationSpec.cs`)
- [x] 6.6 Save and load generated snapshot payload (PersistenceIntegrationSpec.Generated_snapshot_should_save_and_load_through_local_snapshot_store: real, shipped `akka.persistence.snapshot-store.local` plugin (`LocalSnapshotStore`), which already round-trips through `Akka.Persistence.Serialization.PersistenceSnapshotSerializer` -> `Serialization.FindSerializerFor` -> real bytes on disk — `src/core/Akka.Serialization.V2.Tests/PersistenceIntegrationSpec.cs`)
...
- [x] 6.9 Validate generated payloads inside Akka.Delivery wrappers (ReliableDeliveryIntegrationSpec: generated V2 payload round-tripped through the real, shipped (internal) `Akka.Cluster.Serialization.ReliableDeliverySerializer` via `ConsumerController.SequencedMessage<T>`, found/dispatched through `Serialization.FindSerializerFor`/`Serialize`/`Deserialize` since the serializer type itself isn't public; plus a live `ProducerController`/`ConsumerController` exchange under `akka.actor.serialize-messages = on` proving the same wrapper fires transparently on every local hand-off — `src/core/Akka.Cluster.Tests/ReliableDeliveryIntegrationSpec.cs`)
- [x] 6.10 Validate generated payloads inside DistributedData wrappers (GeneratedV2WrapperSpec: generated V2 payload preserved through the real `Akka.DistributedData.Serialization.ReplicatedDataSerializer`'s `OtherMessage` wrapper, both as an `LWWRegister<T>` value and as `ORSet<T>` elements — `src/contrib/cluster/Akka.DistributedData.Tests/Serialization/GeneratedV2WrapperSpec.cs`)
...
- [x] 8.5 Run focused Akka.Remote tests using generated serializers (`dotnet test src/core/Akka.Serialization.V2.Tests -c Release --filter FullyQualifiedName~ClassicRemotingSpec`: 2/2 passing — `src/core/Akka.Serialization.V2.Tests/ClassicRemotingSpec.cs`; classic remoting is covered here, Artery's generated-serializer coverage already existed at `src/core/Akka.Remote.Tests/Artery/ArteryControlMessageSerializerSpec.cs`)
- [x] 8.6 Run focused Akka.Persistence tests using generated serializers (`dotnet test src/core/Akka.Serialization.V2.Tests -c Release --filter FullyQualifiedName~PersistenceIntegrationSpec`: 2/2 passing — `src/core/Akka.Serialization.V2.Tests/PersistenceIntegrationSpec.cs`)
