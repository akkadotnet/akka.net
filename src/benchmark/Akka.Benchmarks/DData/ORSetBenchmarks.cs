...
public class ORSetBenchmarks
{
    ...
    public void Setup()
    {
        ...
        foreach (var i in Enumerable.Range(0, NumElements)) newElements.Add(i.ToString());
        _elements = newElements.ToArray();

        _c1 = ORSet<string>.Empty;
        foreach (var node in _nodes) _c1 = _c1.Add(node, _elements[0]);

        // add some data that _c2 doesn't have
        _c2 = _c1;
        foreach (var node in _nodes.Skip(NumNodes / 2)) _c2 = _c2.Add(node, _elements[1]);

        _c3 = _c1;
        foreach (var node in _nodes.Take(NumNodes / 2)) _c3 = _c3.Remove(node, _elements[0]);
    }
    ...
    public void Should_add_node_to_ORSet()
    {
        for (var i = 0; i < Iterations; i++)
        {
            ...
            foreach (var node in _nodes) init = init.Add(node, _elements[0]);
        }
    }
    ...
    public void Should_add_elements_for_Same_node()
    {
        ...
        {
            var init = ORSet<string>.Empty;
            foreach (var element in _elements) init = init.Add(_nodes[0], element);
        }
    }
