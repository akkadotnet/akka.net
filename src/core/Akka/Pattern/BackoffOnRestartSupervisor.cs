...
///     This back-off supervisor is created by using
///     <see cref="BackoffSupervisor.Props(Props, string, TimeSpan, TimeSpan, double)">BackoffSupervisor.Props</see>
...
internal sealed class BackoffOnRestartSupervisor : BackoffSupervisorBase
{
    ...
    public BackoffOnRestartSupervisor(
        ...
        Func<object, bool> finalStopMessage = null) : base(childProps, childName, reset, replyWhileStopped,
        finalStopMessage)
    {
    ...
    protected override SupervisorStrategy SupervisorStrategy()
    {
        return new OneForOneStrategy(_strategy.MaxNumberOfRetries, _strategy.WithinTimeRangeMilliseconds,
            ex =>
            {
                var directive = _strategy.Decider?.Decide(ex) ??
                                Actor.SupervisorStrategy.DefaultStrategy.Decider.Decide(ex);
                ...
                if (directive == Directive.Restart)
                {
                    if (_strategy.WithinTimeRangeMilliseconds > 0 && RestartCountN == 0)
                        ...
                        Context.System.Scheduler.ScheduleTellOnce(_strategy.WithinTimeRangeMilliseconds, Self,
                    ...
                    if (_strategy.MaxNumberOfRetries >= 0 && nextRestartCount > _strategy.MaxNumberOfRetries)
                    {
                        ...
                        _log.Debug($"Terminating on restart #{0} which exceeds max allowed restarts ({1})",
                            nextRestartCount, _strategy.MaxNumberOfRetries);
                        Become(Receive);
                        Context.Stop(Self);
                    }
                    else
    ...
    private bool WaitChildTerminatedBeforeBackoff(object message, IActorRef childRef)
    {
        switch (message)
        {
            case Terminated terminated when terminated.ActorRef.Equals(childRef):
            {
                ...
                Child = null;
                var restartDelay =
                    BackoffSupervisor.CalculateDelay(RestartCountN, _minBackoff, _maxBackoff, _randomFactor);
                Context.System.Scheduler.ScheduleTellOnce(restartDelay, Self, BackoffSupervisor.StartChild.Instance,
                    Self);
                RestartCountN++;
                break;
            }
