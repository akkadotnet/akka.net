...
public class SnapshotDecodeFailureSpec : PersistenceSpec
{
    ...
    [Fact]
    public void
        PersistentActor_with_a_failing_snapshot_loading_must_fail_recovery_and_stop_actor_when_no_snapshot_could_be_loaded()
    {
        ...
        try
        {
            var lPersistentActor =
                Sys.ActorOf(Props.Create(() => new LoadSnapshotTestPersistentActor(Name, TestActor)));
            ExpectMsg<Error>(msg =>
                msg.Message.ToString().StartsWith("Persistence failure when replaying events for persistenceId"));
    ...
    internal class SaveSnapshotTestPersistentActor : NamedPersistentActor
    {
        ...
        protected override bool ReceiveCommand(object message)
        {
            ...
            {
                case Cmd c:
                    Persist(c.Data, _ => { SaveSnapshot(c.Data); });
                    return true;
