...
public class InterpreterSpec : GraphInterpreterSpecKit
{
    ...
    public void Interpreter_should_implement_chain_of_maps_correctly()
    {
        WithOneBoundedSetup(
            new[]
            {
                new Select<int, int>(x => x + 1), new Select<int, int>(x => x * 2), new Select<int, int>(x => x + 1)
            },
            (lastEvents, upstream, downstream) =>
            {
                lastEvents().Should().BeEmpty();
    ...
    {
        WithOneBoundedSetup(
            new IGraphStageWithMaterializedValue<FlowShape<int, int>, object>[]
            {
                new Doubler<int>(), new Where<int>(x => x != 0)
            },
            (lastEvents, upstream, downstream) =>
            {
                lastEvents().Should().BeEmpty();
    ...
    public void Interpreter_should_implement_many_to_one_one_to_many_chain_correctly()
    {
        WithOneBoundedSetup(
            new IGraphStageWithMaterializedValue<FlowShape<int, int>, object>[]
            {
                new Where<int>(x => x != 0), new Doubler<int>()
            },
            (lastEvents, upstream, downstream) =>
            {
                lastEvents().Should().BeEmpty();
                ...
                downstream.Cancel();
                lastEvents().Should()
                    .BeEquivalentTo(new Cancel(SubscriptionWithCancelException.NoMoreElementsNeeded.Instance));
            });
    }
    ...
    public void Interpreter_should_implement_take()
    {
        WithOneBoundedSetup(TakeTwo,
            (lastEvents, upstream, downstream) =>
            {
                ...
                upstream.OnNext(1);
                lastEvents().Should().BeEquivalentTo(new OnNext(1),
                    new Cancel(SubscriptionWithCancelException.StageWasCompleted.Instance), new OnComplete());
            });
    }
    ...
    {
        WithOneBoundedSetup<int, int>(
            new IGraphStageWithMaterializedValue<Shape, object>[]
            {
                new Where<int>(x => x != 0), TakeTwo, new Select<int, int>(x => x + 1)
            },
            (lastEvents, upstream, downstream) =>
            {
                lastEvents().Should().BeEmpty();
    ...
    public void Interpreter_should_work_with_expand_expand()
    {
        WithOneBoundedSetup<int>(
            ...
            (lastEvents, upstream, downstream) =>
            {
                ...
                downstream.RequestOne();
                lastEvents().Should()
                    .BeEquivalentTo(new OnNext(2), new RequestOne()); // one element is still in the pipeline
    ...
    public void Interpreter_should_implement_doubler_batch_doubler_conflate()
    {
        WithOneBoundedSetup<int>(
            new IGraphStageWithMaterializedValue<Shape, object>[]
            {
                new Doubler<int>(), new Batch<int, int>(1L, _ => 0L, e => e, (agg, x) => agg + x)
            },
            (lastEvents, upstream, downstream) =>
            {
                lastEvents().Should().BeEquivalentTo(new RequestOne());
    ...
    public void Interpreter_should_work_with_jumpback_table_and_completed_elements()
    {
        WithOneBoundedSetup(
            new IGraphStageWithMaterializedValue<FlowShape<int, int>, object>[]
            {
                new Select<int, int>(x => x), new Select<int, int>(x => x), new KeepGoing<int>(),
                new Select<int, int>(x => x), new Select<int, int>(x => x)
            },
            (lastEvents, upstream, downstream) =>
            {
                lastEvents().Should().BeEmpty();
    ...
    public void Interpreter_should_work_with_PushAndFinish_if_indirect_upstream_completes_with_PushAndFinish()
    {
        WithOneBoundedSetup(
            new IGraphStageWithMaterializedValue<FlowShape<int, int>, object>[]
            {
                new Select<int, int>(x => x), new PushFinishStage<int>(), new Select<int, int>(x => x)
            },
            (lastEvents, upstream, downstream) =>
            {
                lastEvents().Should().BeEmpty();
    ...
    [Fact]
    public void
        Interpreter_should_work_with_PushAndFinish_if_upstream_completes_with_PushAndFinish_and_downstream_immediately_pulls()
    {
        WithOneBoundedSetup(
            new IGraphStageWithMaterializedValue<FlowShape<int, int>, object>[]
            {
                new PushFinishStage<int>(), new Aggregate<int, int>(0, (x, y) => x + y)
            },
            (lastEvents, upstream, downstream) =>
            {
                lastEvents().Should().BeEmpty();
    ...
    {
        WithOneBoundedSetup(new[] { TakeOne, TakeOne },
            (lastEvents, upstream, downstream) =>
            {
                lastEvents().Should().BeEmpty();
                ...
                upstream.OnNext(1);
                lastEvents().Should()
                    .BeEquivalentTo(new Cancel(SubscriptionWithCancelException.StageWasCompleted.Instance),
                        new OnNext(1), new OnComplete());
            });
    }
    ...
    public class PullWhileOpIsTerminating<T> : SimpleLinearGraphStage<T>
    {
        ...
        private sealed class Logic : GraphStageLogic
        {
            ...
            {
                SetHandler(stage.Outlet, () => Pull(stage.Inlet));
                SetHandler(stage.Inlet, () => Pull(stage.Inlet), () =>
                {
