...
public static class Framing
{
    private static readonly Func<IEnumerator<byte>, int, int> BigEndianDecoder = (enumerator, length) =>
    {
        ...
        while (count > 0)
        {
            ...
            if (!enumerator.MoveNext())
                throw new IndexOutOfRangeException("LittleEndianDecoder reached end of byte string");
    ...
    ///     If there are buffered bytes (an incomplete frame) when the input stream finishes and
    ///     <paramref name="allowTruncation" /> is set to
    ...
    ///     If false, then when the last frame being decoded contains no valid delimiter this Flow
    ...
    /// <param name="maximumFramelength">
    ///     The maximum length of allowed frames while decoding. If the maximum length is exceeded
    ///     this Flow will fail the stream. This length *includes* the header (i.e the offset and the length of the size field)
    /// </param>
    ...
    ///     incoming frames have a field that encodes their length.
    ///     <para>
    ///         If the input stream finishes before the last frame has been fully decoded, this Flow will fail the stream
    ///         reporting
    ///         a truncated frame.
    ///     </para>
    /// </summary>
    ...
    ///     Then computeFrameSize can be used to compute the frame size: `(offset bytes, computed size) => (actual frame
    ///     size)`.
    ///     "Actual frame size" must be equal or bigger than sum of `fieldOffset` and `fieldLength`, the operator fails
    ///     otherwise.
    /// </param>
    /// <exception cref="ArgumentException">
    ///     This exception is thrown when the specified <paramref name="fieldLength" /> is not equal to either 1, 2, 3 or 4.
    ...
    /// <summary>
    ///     Returns a BidiFlow that implements a simple framing protocol. This is a convenience wrapper over
    ///     <see cref="LengthField(int, int, int, ByteOrder)" />
    ///     and simply attaches a length field header of four bytes (using big endian encoding) to outgoing messages, and
    ///     decodes
    ...
    /// </summary>
    /// <param name="maximumMessageLength">
    ///     Maximum length of allowed messages. If sent or received messages exceed the
    ///     configured limit this BidiFlow will fail the stream. The header attached by this BidiFlow are not included in this
    ///     limit.
    /// </param>
    /// <returns>TBD</returns>
    ...
    private sealed class DelimiterFramingStage : SimpleLinearGraphStage<ByteString>
    {
        ...
        private sealed class Logic : InAndOutGraphStageLogic
        {
            ...
            private void DoParse()
            {
                while (true)
                {
                    var possibleMatchPosition = _buffer.IndexOf(_firstSeparatorByte, _nextPossibleMatch);
