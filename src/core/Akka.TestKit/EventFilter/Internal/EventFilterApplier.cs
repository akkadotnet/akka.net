...
public class InternalEventFilterApplier : IEventFilterApplier
{
    ...
    public void Expect(
        ...
        CancellationToken cancellationToken = default)
    {
        ExpectAsync(expectedCount, timeout, () =>
            {
                action();
                return Task.CompletedTask;
            }, cancellationToken)
    ...
    ///     TBD
    /// </summary>
    public EventFilterFactory And => new(_testkit, _actorSystem, _filters);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            foreach (var filter in _filters) filter.EventMatched += matchedEventHandler.HandleEvent;
        ...
        CancellationToken cancellationToken = default)
    {
        if (expectedOccurrences.HasValue)
        {
            ...
            if (expected > 0)
            {
                await _testkit.AwaitConditionNoThrowAsync(
                    () => Task.FromResult(matchedEventHandler.ReceivedCount >= expected), timeout,
                    cancellationToken: cancellationToken);
            ...
            var foundEvent = await _testkit.AwaitConditionNoThrowAsync(
                () => Task.FromResult(matchedEventHandler.ReceivedCount > 0), timeout,
                cancellationToken: cancellationToken);
    ...
    protected class InternalUnmutableFilter : IUnmutableFilter
    {
        ...
        public void Unmute()
        {
            ...
            _filters = null;
            if (!_isDisposed && filters != null) _system.EventStream.Publish(new Unmute(filters));
        }
