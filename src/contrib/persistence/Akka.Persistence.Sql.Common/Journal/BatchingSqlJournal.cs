...
namespace Akka.Persistence.Sql.Common.Journal
{
    ...
    public sealed class ReplayFilterSettings
    {
        ...
        ///     This exception is thrown when an invalid <c>replay-filter.mode</c> is read from the specified
        ...
        public ReplayFilterSettings(Config config)
        {
            ...
            {
                case "off":
                    mode = ReplayFilterMode.Disabled;
                    break;
                case "repair-by-discard-old":
                    mode = ReplayFilterMode.RepairByDiscardOld;
                    break;
                case "fail":
                    mode = ReplayFilterMode.Fail;
                    break;
                case "warn":
                    mode = ReplayFilterMode.Warn;
                    break;
                default:
        ...
        where TCommand : DbCommand
    {
        ...
        protected BatchingSqlJournal(BatchingSqlJournalSetup setup)
        {
            ...
            _circuitBreaker = CircuitBreaker.Create(
                ...
                Setup.CircuitBreakerSettings.MaxFailures,
                Setup.CircuitBreakerSettings.CallTimeout,
                Setup.CircuitBreakerSettings.ResetTimeout);
        ...
        protected virtual async Task HandleReplayMessages(ReplayMessages req, TCommand command, IActorContext context)
        {
            ...
            var replyTo = replaySettings.IsEnabled
                ? context.ActorOf(ReplayFilter.Props(
                    req.PersistentActor,
                    replaySettings.Mode,
                    replaySettings.WindowSize,
                    replaySettings.MaxOldWriters,
                    replaySettings.IsDebug))
                : req.PersistentActor;
            var persistenceId = req.PersistenceId;
        ...
        protected virtual void WriteEvent(TCommand command, IPersistentRepresentation persistent, string tags = "")
        {
            ...
            Akka.Serialization.Serialization.WithTransport(_serialization.System, () =>
            {
                ...
                {
                    if (serializer.IncludeManifest) manifest = persistent.Payload.GetType().TypeQualifiedName();
                }
