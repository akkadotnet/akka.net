...
public partial class ActorCell
{
    ...
    ///     TBD
    /// </summary>
    public int CurrentEnvelopeId { get; private set; }
    ...
    ///     This exception is thrown if a <see cref="Akka.Actor.Kill" /> message is included in the given
    ...
    public void Invoke(Envelope envelope)
    {
        ...
        try
        {
            ...
            CurrentEnvelopeId++;
            if (CurrentEnvelopeId == int.MaxValue) CurrentEnvelopeId = 0;
    ...
    protected virtual void ReceiveMessage(object message)
    {
        ...
        if (System.Settings.AddLoggingReceive && Actor is ILogReceive)
        {
            ...
            var msg = "received " + (wasHandled ? "handled" : "unhandled") + " message " + message + " from " +
                      Sender.Path;
    ...
    ///     This exception is thrown if a <see cref="Akka.Dispatch.SysMsg.Create" /> system message is included in the given
    ///     <paramref name="envelope" />.
    /// </exception>
...
#pragma warning restore 420
    ...
    private void Supervise(IActorRef child, bool async)
    {
        if (!IsTerminating)
        {
            ...
            {
                Publish(new Debug(Self.Path.ToString(), ActorType,
                    "received Supervise from unregistered child " + child.Path + ", this will not end well"));
            }
        }
    }
    ...
    {
        if (async && child is RepointableActorRef @ref) @ref.Point();
    }
