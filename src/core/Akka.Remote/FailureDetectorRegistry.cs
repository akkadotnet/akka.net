...
/// <summary>
///     Interface for a registry of Akka <see cref="FailureDetector" />s. New resources are implicitly registered when
///     heartbeat is first
...
{
    /// <summary>
    ///     Loads an instantiates a given <see cref="FailureDetector" /> implementation. The class to be loaded must have a
    ///     constructor
    ///     that accepts a <see cref="Config" /> and an <see cref="EventStream" /> parameter. Will throw
    ...
    public static FailureDetector Load(string fqcn, Config config, ActorSystem system)
    {
        ...
        var failureDetector =
            (FailureDetector)Activator.CreateInstance(failureDetectorClass, config, system.EventStream);
    ...
    /// </summary>
    /// <param name="context">
    ///     The ActorContext used to resolve an <see cref="ActorSystem" /> for this
    ///     <see cref="FailureDetector" /> instance.
    /// </param>
    /// <param name="fqcn">The fully-qualified .NET assembly name of the FailureDetector implementation class to be loaded.</param>
    /// <param name="config">Configuration that will be passed to the implementation.</param>
    /// <exception cref="ConfigurationException">
