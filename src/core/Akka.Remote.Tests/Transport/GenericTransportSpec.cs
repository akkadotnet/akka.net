...
public abstract class GenericTransportSpec : AkkaSpec
{
    ...
    private readonly Address _addressA;
    private readonly Address _addressB;
    private readonly Address _nonExistingAddress;
    ...
    protected abstract Remote.Transport.Transport FreshTransport(TestTransport testTransport);

    private Remote.Transport.Transport WrapTransport(Remote.Transport.Transport transport)
    {
        if (_withAkkaProtocol)
        {
            var provider = (RemoteActorRefProvider)((ExtendedActorSystem)Sys).Provider;

            return new AkkaProtocolTransport(transport, Sys, new AkkaProtocolSettings(provider.RemoteSettings.Config),
                new AkkaPduProtobuffCodec(Sys));
        }

        return transport;
    }

    private Remote.Transport.Transport NewTransportA(AssociationRegistry registry)
    {
        return WrapTransport(FreshTransport(new TestTransport(_addressATest, registry)));
    }

    private Remote.Transport.Transport NewTransportB(AssociationRegistry registry)
    {
    ...
    public async Task Transport_must_associate_successfully_with_another_transport_of_its_kind()
    {
        ...
        (await transportA.Listen().WithTimeout(DefaultTimeout)).Item2.SetResult(
            new ActorAssociationEventListener(TestActor));
        (await transportB.Listen().WithTimeout(DefaultTimeout)).Item2.SetResult(
            new ActorAssociationEventListener(TestActor));
        ...
        Assert.Contains(registry.LogSnapshot().OfType<AssociateAttempt>(),
            x => x.LocalAddress == _addressATest && x.RemoteAddress == _addressBTest);
    ...
    public async Task Transport_must_successfully_send_PDUs()
    {
        ...
        Assert.Contains(registry.LogSnapshot().OfType<WriteAttempt>(),
            x => x.Sender == _addressATest && x.Recipient == _addressBTest && x.Payload.Equals(pdu));
    }
    ...
    public async Task Transport_must_successfully_disassociate()
    {
        ...
        await AwaitConditionAsync(() =>
            Task.FromResult(registry.LogSnapshot().OfType<DisassociateAttempt>()
                .Any(x => x.Requestor == _addressATest && x.Remote == _addressBTest))
        );
    }
}