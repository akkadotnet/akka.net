...
public class ActorBecomeTests : AkkaSpec
{
    ...
    [Fact]
    public async Task
        Given_actor_that_has_called_default_Become_twice_When_calling_unbecome_Then_the_default_handler_is_used_and_not_the_last_handler()
    {
    ...
    [Fact]
    public async Task
        Given_actor_that_has_called_default_Become_without_overwriting_previous_handler_When_calling_unbecome_Then_the_previous_handler_is_used()
    {
        //Calling Become() does not persist the current handler, it just overwrites it, so when we call Unbecome(),
        //no matter how many times, there is no persisted handler to revert to, so we'll end up with the default one
    ...
    private class Become3Actor : ReceiveActor
    {
        ...
        public void A()
        {
            Receive<string>(s => { Sender.Tell("A says: " + s); });
            Receive<bool>(s => { Sender.Tell("A says: " + s); });
        ...
        public void B()
        {
            Receive<string>(s => { Sender.Tell("B says: " + s); });
            Receive<bool>(s => { Sender.Tell("B says: " + s); });
            Receive<int>(s => { Sender.Tell("B says: " + s); });
        }
    }
}