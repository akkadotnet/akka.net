...
namespace Akka.Util
{
    ...
    internal static class FastHash
    {
        ...
        /// <param name="s">The input string.</param>
        /// <returns>A 32-bit pseudo-random hash value.</returns>
        public static int OfStringFast(ReadOnlySpan<byte> s)
        {
    ...
    internal abstract class LruSegmentCache<TValue> where TValue : class
    {
        protected LruSegmentCache(int capacity, int evictAgeThreshold)
        {
            if (capacity <= 0)
        ...
        public TValue GetOrCompute(ReadOnlySpan<byte> k)
        {
            var h = Hash(k);
            unchecked { _epoch += 1; }
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    {
                        var value = Compute(k);
                        if (IsCacheable(value)) Move(position, k.ToArray(), h, value, _epoch, probeDistance);
                        return value;
                    }
                    else if (_hashes[position] == h &&  k.SequenceEqual(_keys[position]))
                    {
                        // Update usage
                        _epochs[position] = _epoch;
                        return _values[position];
        ...
        private void Move(int position, byte[] k, int h, TValue value, int elemEpoch, int probeDistance)
        {
            if (_values[position] == null)
            {
        ...
        protected int Hash(ReadOnlySpan<byte> k)
        {
            return FastHash.OfStringFast(k);
        }

        protected abstract TValue Compute(ReadOnlySpan<byte> k);

        protected abstract bool IsCacheable(TValue v);

        public override string ToString()
        {
            return $"LruSegmentCache(values = [{string.Join<TValue>(",", _values)}], hashes = [{string.Join(",", _hashes)}], " +
