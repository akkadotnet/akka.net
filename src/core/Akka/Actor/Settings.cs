...
public class Settings
{
    ...
    ///     This exception is thrown if the 'akka.actor.provider' configuration item is not a valid type name or a valid actor
    ///     ref provider.
    /// </exception>
    ...
    public Settings(ActorSystem system, Config config, ActorSystemSetup setup)
    {
        ...
        var providerSelectionSetup = Setup.Get<BootstrapSetup>()
            ...
            .Select(_ => _.Fqn)
            .GetOrElse(Config.GetString("akka.actor.provider"));
        ...
        if (!typeof(IActorRefProvider).IsAssignableFrom(providerType))
            throw new ConfigurationException(
                $"'akka.actor.provider' is not a valid actor ref provider: '{ProviderClass}'");

        SupervisorStrategyClass = Config.GetString("akka.actor.guardian-supervisor-strategy");

        AskTimeout = Config.GetTimeSpan("akka.actor.ask-timeout");
        ...
        EmitActorTelemetry = Config.GetBoolean("akka.actor.telemetry.enabled");
        ...
        var stdoutClassName = Config.GetString("akka.stdout-logger-class");
        ...
        else
        {
            ...
            if (!typeof(MinimalLogger).IsAssignableFrom(stdoutLoggerType))
                throw new ArgumentException(
                    "Standard out logger type must inherit from the MinimalLogger abstract class.");
        ...
        LoggerAsyncStart = Config.GetBoolean("akka.logger-async-start");

        var loggerFormatterName = Config.GetString("akka.logger-formatter");
        ...
        LogDeadLettersSuspendDuration =
            Config.GetString(key) == "infinite" ? Timeout.InfiniteTimeSpan : Config.GetTimeSpan(key);
        ...
        CoordinatedShutdownRunByActorSystemTerminate =
            Config.GetBoolean("akka.coordinated-shutdown.run-by-actor-system-terminate");
    ...
    /// <summary>
    ///     When set to <c>true</c>, all actors will emit <see cref="IActorTelemetryEvent" />s when they are created, stopped,
    ///     or restarted.
    /// </summary>
    /// <remarks>
    ...
    private void RebuildConfig()
    {
        ...
        System?.Dispatchers?.ReloadPrerequisites(
            new DefaultDispatcherPrerequisites(System.EventStream, System.Scheduler, this, System.Mailboxes));
        if (System is ISupportSerializationConfigReload rs)
