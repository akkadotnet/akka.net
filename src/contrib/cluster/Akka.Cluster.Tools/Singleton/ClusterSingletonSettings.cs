...
public class ClusterSingletonSettings : INoSerializationVerificationNeeded
{
        ...
        bool considerAppVersion)
    {
        if (singletonIdentificationInterval == TimeSpan.Zero)
            throw new ArgumentException("singletonIdentificationInterval must be positive",
                nameof(singletonIdentificationInterval));

        if (removalMargin < TimeSpan.Zero)
            throw new ArgumentException("ClusterSingletonManagerSettings.RemovalMargin must be positive",
                nameof(removalMargin));

        if (handOverRetryInterval <= TimeSpan.Zero)
            throw new ArgumentException("ClusterSingletonManagerSettings.HandOverRetryInterval must be positive",
                nameof(handOverRetryInterval));
    ...
    /// <summary>
    ///     Singleton among the nodes tagged with specified role. If the role is not specified it's a singleton among all nodes
    ///     in the cluster.
    /// </summary>
    ...
    /// <summary>
    ///     If the location of the singleton is unknown the proxy will buffer this number of messages and deliver them when the
    ///     singleton
    ///     is identified. When the buffer is full old messages will be dropped when new messages are sent viea the proxy. Use
    ///     `0` to
    ...
    /// <summary>
    ///     Should <see cref="Member.AppVersion" /> be considered when the cluster singleton instance is being moved to another
    ///     node.
    ///     When set to false, singleton instance will always be created on oldest member.
    ///     When set to true, singleton instance will be created on the oldest member with the highest
    ///     <see cref="Member.AppVersion" /> number.
    /// </summary>
    ...
    /// <summary>
    ///     Create settings from a configuration with the same layout as the default configuration `akka.cluster.singleton` and
    ...
    {
        return new ClusterSingletonManagerSettings(singletonName, Role, RemovalMargin, HandOverRetryInterval,
            LeaseSettings, ConsiderAppVersion);
    }

    [InternalApi]
    internal ClusterSingletonProxySettings ToProxySettings(string singletonName)
    {
        return new ClusterSingletonProxySettings(singletonName, Role, SingletonIdentificationInterval, BufferSize,
            ConsiderAppVersion);
    }

    [InternalApi]
    internal bool ShouldRunManager(Cluster cluster)
    {
