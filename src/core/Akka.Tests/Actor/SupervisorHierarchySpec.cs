...
public class SupervisorHierarchySpec : AkkaSpec
{
    ...
    public async Task A_supervisor_hierarchy_must_Restart_Manager_And_Workers_In_AllForOne()
    {
        ...
        var managerProps =
            new PropsWithName(Props.Create(() => new CountDownActor(countDown, new AllForOneStrategy(decider))),
                "manager");
        ...
        var worker1 =
            await manager.Ask<IActorRef>(new PropsWithName(workerProps, "worker1"), TestKitSettings.DefaultTimeout);
        var worker2 =
            await manager.Ask<IActorRef>(new PropsWithName(workerProps, "worker2"), TestKitSettings.DefaultTimeout);
        var worker3 =
            await manager.Ask<IActorRef>(new PropsWithName(workerProps, "worker3"), TestKitSettings.DefaultTimeout);
    ...
    public async Task A_supervisor_must_send_notifications_to_supervisor_when_permanent_failure()
    {
        ...
        var boss = ActorOf((cfg, ctx) =>
        {
            var crasher =
                ctx.ActorOf(
                    Props.Create(() => new CountDownActor(countDownMessages, SupervisorStrategy.DefaultStrategy)),
                    "crasher");
        ...
        where T : ActorBase, new()
    {
        ...
        await EventFilter.Warning("expected").ExpectOneAsync(
    ...
    public async Task A_supervisor_hierarchy_must_suspend_children_while_failing()
    {
        ...
        var slowResumer = ActorOf(c =>
        {
            ...
            c.Receive<string>(s => s.StartsWith("spawn:"),
                (s, ctx) => ctx.Sender.Tell(ctx.ActorOf<Resumer>(s.Substring(6))));
        ...
        await ExpectMsgAsync("pong");
        await EventFilter.Warning("expected").ExpectOneAsync(
            async () => //expected exception is thrown by the boss when it crashes
            {
    ...
    [Fact]
    public void
        A_supervisor_hierarchy_must_handle_failure_in_creation_when_supervision_strategy_returns_Resume_and_Restart()
    {
        ...
        EventFilter.Exception<Failure>()
            ...
            .Mute(() =>
            {
                ...
                {
                    resumerDsl.Strategy = new OneForOneStrategy(ex =>
                        ex is ActorInitializationException
                            ? createAttempt.Current % 2 == 0 ? Directive.Resume : Directive.Restart
                            : Directive.Escalate);
...
#pragma warning disable AK1003
            ReceiveAsync<string>(s => s.StartsWith("spawn:"),
                async s => Sender.Tell(Context.ActorOf<ResumerAsync>(s.Substring(6))));
...
#pragma warning restore CS1998
    ...
    {
        public Failure(Directive directive, bool stop, int depth, int failPre, int failPost, int failConstr,
            int stopKids)
            : base("failure")
        {
            Directive = directive;
