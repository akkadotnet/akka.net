        ...
        switch (selectorTypeName)
        {
            ...
            default:
                return DynamicAccess.CreateInstanceFor<IMetricsSelector>(selectorTypeName, config)
                    .Recover(ex => throw new ArgumentException(
                        $"Cannot instantiate metrics-selector [{selectorTypeName}]," +
...
public abstract class CapacityMetricsSelector : IMetricsSelector
{
    ...
    public IImmutableDictionary<Address, int> Weights(IImmutableDictionary<Address, double> capacity)
    {
        ...
        return capacity.ToImmutableDictionary(pair => pair.Key,
            pair => (int)Math.Round(pair.Value / divisor, MidpointRounding.AwayFromZero));
    }
}
        ...
        return nodeMetrics
            ...
            .ToImmutableDictionary(m => m.Value.Address, m =>
            {
                if (m.Value.MaxRecommendedSmoothValue.HasValue &&
                    ...
                    return (m.Value.MaxRecommendedSmoothValue.Value - m.Value.UsedSmoothValue) /
                           m.Value.MaxRecommendedSmoothValue.Value;
...
public abstract class MixMetricsSelectorBase : CapacityMetricsSelector
{
    ...
    public override IImmutableDictionary<Address, double> Capacity(IImmutableSet<NodeMetrics> nodeMetrics)
    {
        ...
        var init = ImmutableDictionary<Address, (double Sum, int Count)>.Empty;
