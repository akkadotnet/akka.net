...
namespace Akka.Cluster.SBR
{
    ...
    internal class AcquireLeaseAndDownIndirectlyConnected : IAcquireLeaseDecision, IEquatable<AcquireLeaseAndDownIndirectlyConnected>
    {
    ...
    internal abstract class DowningStrategy
    {
        ...
        public ImmutableHashSet<UniqueAddress> Unreachable { get; private set; } = ImmutableHashSet<UniqueAddress>.Empty;

        public bool IsUnreachable(Member m) => Unreachable.Contains(m.UniqueAddress);

        public string Role { get; protected set; }

        // all Joining and WeaklyUp members
        public ImmutableSortedSet<Member> Joining =>
            AllMembers.Where(m => m.Status == MemberStatus.Joining || m.Status == MemberStatus.WeaklyUp).ToImmutableSortedSet(Ordering);
        ...
        public ImmutableSortedSet<Member> Members =>
            GetMembers(includingPossiblyUp: false, excludingPossiblyExiting: false);
        ...
        public ImmutableSortedSet<Member> GetMembers(bool includingPossiblyUp, bool excludingPossiblyExiting) =>
            AllMembers.Where(m => !((!includingPossiblyUp && m.Status == MemberStatus.Joining) ||
        ...
        public ImmutableSortedSet<Member> MembersWithRole =>
            GetMembersWithRole(includingPossiblyUp: false, excludingPossiblyExiting: false);
        ...
        public ImmutableSortedSet<Member> GetMembersWithRole(bool includingPossiblyUp, bool excludingPossiblyExiting)
        {
            ...
            return GetMembers(includingPossiblyUp, excludingPossiblyExiting).Where(m => m.HasRole(Role)).ToImmutableSortedSet(Ordering);
        }

        public ImmutableSortedSet<Member> ReachableMembers =>
            GetReachableMembers(includingPossiblyUp: false, excludingPossiblyExiting: false);
        ...
        public ImmutableSortedSet<Member> ReachableMembersWithRole =>
            GetReachableMembersWithRole(includingPossiblyUp: false, excludingPossiblyExiting: false);

        public ImmutableSortedSet<Member> GetReachableMembersWithRole(bool includingPossiblyUp, bool excludingPossiblyExiting)
        {
            if (string.IsNullOrEmpty(Role))
                return GetReachableMembers(includingPossiblyUp, excludingPossiblyExiting);
            return GetReachableMembers(includingPossiblyUp, excludingPossiblyExiting).Where(m => m.HasRole(Role)).ToImmutableSortedSet(Ordering);
        }

        public ImmutableSortedSet<Member> UnreachableMembers =>
            GetUnreachableMembers(includingPossiblyUp: false, excludingPossiblyExiting: false);
        ...
        public ImmutableSortedSet<Member> GetUnreachableMembers(bool includingPossiblyUp, bool excludingPossiblyExiting)
        {
            ...
            return GetMembers(includingPossiblyUp, excludingPossiblyExiting).Where(IsUnreachable).ToImmutableSortedSet(Ordering);
        }

        public ImmutableSortedSet<Member> UnreachableMembersWithRole =>
            GetUnreachableMembersWithRole(includingPossiblyUp: false, excludingPossiblyExiting: false);

        public ImmutableSortedSet<Member> GetUnreachableMembersWithRole(bool includingPossiblyUp, bool excludingPossiblyExiting)
        {
            if (string.IsNullOrEmpty(Role))
                return GetUnreachableMembers(includingPossiblyUp, excludingPossiblyExiting);
            return GetUnreachableMembers(includingPossiblyUp, excludingPossiblyExiting).Where(m => m.HasRole(Role)).ToImmutableSortedSet(Ordering);
        }
        ...
        private void RemoveFromAllMembers(Member m)
        {
            ...
            else
            {
                ...
                AllMembers = AllMembers.Where(i => !i.UniqueAddress.Equals(m.UniqueAddress)).ToImmutableSortedSet(Ordering);
            }
        }
        ...
        public void SetReachability(Reachability r)
        {
            ...
            Reachability = r.FilterRecords(record =>
                (record.Status == Reachability.ReachabilityStatus.Unreachable || record.Status == Reachability.ReachabilityStatus.Terminated)
                );
        }
        ...
        private ImmutableHashSet<UniqueAddress> IndirectlyConnectedFromSeenCurrentGossip
        {
            get
            {
                return Reachability.Records.SelectMany(r =>
                {
        ...
        public ImmutableHashSet<UniqueAddress> UnreachableButNotIndirectlyConnected => Unreachable.Except(IndirectlyConnected);
        ...
        private ImmutableHashSet<UniqueAddress> AdditionalNodesToDownWhenIndirectlyConnected
        {
            get
            {
                if (UnreachableButNotIndirectlyConnected.IsEmpty)
                    return ImmutableHashSet<UniqueAddress>.Empty;
                else
                {
                    ...
                    try
                    {
                        ...
                        Reachability = Reachability.FilterRecords(
                            r =>
                              !((intersectionOfObserversAndSubjects.Contains(r.Observer) && intersectionOfObserversAndSubjects.Contains(r.Subject)) ||
                        ...
                        if (additionalDecision.IsIndirectlyConnected)
                            throw new InvalidOperationException($"SBR double {additionalDecision} decision, downing all instead. " +
    ...
    internal class StaticQuorum : DowningStrategy
    {
        ...
        public override IDecision Decide()
        {
            if (IsTooManyMembers)
                ...
                return DownIndirectlyConnected.Instance;
            else if (MembersWithRole.Count - UnreachableMembersWithRole.Count >= QuorumSize)
                return DownUnreachable.Instance;
            else
                return DownReachable.Instance;
        }
    ...
    internal class KeepMajority : DowningStrategy
    {
        ...
        public override IDecision Decide()
        {
            ...
            else
            {
                ...
                if (ms.IsEmpty)
                    return DownAll.Instance; // no node with matching role
                else
                {
        ...
        private IDecision MajorityDecisionWhenIncludingMembershipChangesEdgeCase()
        {
            ...
            var ms = GetMembersWithRole(includingPossiblyUp: false, excludingPossiblyExiting: true);
            if (ms.IsEmpty)
            {
                return DownAll.Instance;
            }
            ...
            {
                var thisSideReachableSize = GetReachableMembersWithRole(includingPossiblyUp: false, excludingPossiblyExiting: true).Count;
                // for other side we count as many as could be possible (including joining, including leaving)
                var otherSideUnreachableSize = GetUnreachableMembersWithRole(includingPossiblyUp: true, excludingPossiblyExiting: false).Count;
    ...
    internal class KeepOldest : DowningStrategy
    {
        ...
        private IDecision OldestDecisionWhenIncludingMembershipChangesEdgeCase()
        {
            ...
            else
            {
                ...
                var reachableCount = GetReachableMembersWithRole(includingPossiblyUp, excludingPossiblyExiting: true).Count;
                var unreachableCount = GetUnreachableMembersWithRole(includingPossiblyUp, excludingPossiblyExiting: true).Count;
    ...
    internal class LeaseMajority : DowningStrategy
    {
        ...
        private bool IsInMinority
        {
            get
            {
                var ms = MembersWithRole;
                if (ms.IsEmpty)
                    return false; // no node with matching role
                else
                {
                    var unreachableSize = UnreachableMembersWithRole.Count;
