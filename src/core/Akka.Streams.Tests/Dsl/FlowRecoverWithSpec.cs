...
public class FlowRecoverWithSpec : AkkaSpec
{
            ...
            var probe = Source.From(Enumerable.Range(1, 4))
                .Select(x =>
                {
                    if (x == 3)
                        throw Ex;
                    return x;
                }).RecoverWithRetries(_ => Source.From(new[] { 0, -1 }), -1)
    ...
    [Fact]
    public async Task
        A_RecoverWith_must_terminate_with_exception_if_partial_function_fails_to_match_after_an_alternative_source_failure()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var probe = Source.From(Enumerable.Range(1, 3))
