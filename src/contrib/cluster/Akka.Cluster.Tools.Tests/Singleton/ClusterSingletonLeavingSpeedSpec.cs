...
public class ClusterSingletonLeavingSpeedSpec : AkkaSpec
{
              ...
              }")
    {
        _systems = Enumerable.Range(1, 3).Select(n =>
            ActorSystem.Create(Sys.Name,
                ConfigurationFactory.ParseString($"akka.cluster.roles=[role-{n % 3}]")
                    .WithFallback(Sys.Settings.Config))).ToArray();
    ...
    private async Task ClusterSingleton_that_is_leaving_must_quickly_hand_over_to_next_oldest()
    {
        ...
        for (var i = 0; i < _systems.Length; i++)
        {
            ...
            Log.Info(
                $"Singleton {i} stopped in {(int)stoppedDuration.TotalMilliseconds} ms, started in {startedDuration.Milliseconds} ms, diff ${(int)(startedDuration - stoppedDuration).TotalMilliseconds} ms");
        ...
        for (var i = 0; i < durations.Count; i++)
            Log.Info(
                $"Singleton {i} stopped in {(int)durations[i].Item1.TotalMilliseconds} ms, started in {durations[i].Item2.Milliseconds} ms, diff ${(int)(durations[i].Item2 - durations[i].Item1).TotalMilliseconds} ms");
    }
    ...
    internal class TheSingleton : UntypedActor
    {
        ...
        {
            return Props.Create(() => new TheSingleton(probe));
        }
