...
public class FSMTimingSpec : AkkaSpec
{
    ...
    public async Task FSM_must_correctly_cancel_a_named_timer()
    {
        ...
        await WithinAsync(300.Milliseconds(), 1.Seconds(), async () => { await ExpectMsgAsync<Tock>(); });
        FSM.Tell(Cancel.Instance);
    ...
    public async Task FSM_must_not_get_confused_between_named_and_state_timers()
    {
        ...
        FSM.Tell(Tick.Instance);
        await ExpectMsgAsync(new Transition<FsmState>(FSM, FsmState.Initial,
            FsmState.TestCancelStateTimerInNamedTimerMessage));
        await ExpectMsgAsync<Tick>(500.Milliseconds());
        await Task.Delay(200.Milliseconds());
        Resume(FSM);
        await ExpectMsgAsync(
            new Transition<FsmState>(FSM, FsmState.TestCancelStateTimerInNamedTimerMessage,
                FsmState.TestCancelStateTimerInNamedTimerMessage2), 500.Milliseconds());
        FSM.Tell(Cancel.Instance);
        await WithinAsync(500.Milliseconds(), async () =>
        {
            await ExpectMsgAsync<Cancel>();
            await ExpectMsgAsync(new Transition<FsmState>(FSM, FsmState.TestCancelStateTimerInNamedTimerMessage2,
                FsmState.Initial));
        });
    }
    ...
    public async Task FSM_must_receive_and_cancel_a_repeated_timer()
    {
        ...
        await WithinAsync(500.Milliseconds(),
            async () =>
            {
    ...
    private static void Suspend(IActorRef actorRef)
    {
        var l = actorRef as ActorRefWithCell;
        l?.Suspend();
    }

    private static void Resume(IActorRef actorRef)
    {
        var l = actorRef as ActorRefWithCell;
        l?.Resume();
    }
    ...
    public class StateMachine : FSM<FsmState, int>, ILoggingFSM
    {
        public StateMachine(IActorRef tester)
        {
            ...
            When(FsmState.TestStateTimeout, @event =>
            {
                ...
                if (@event.FsmEvent is Cancel) return GoTo(FsmState.Initial).Replying(Cancel.Instance);
                return null;
            }, 800.Milliseconds());
            ...
            When(FsmState.TestCancelStateTimerInNamedTimerMessage, @event =>
            {
                ...
                if (@event.FsmEvent is Tock) return GoTo(FsmState.TestCancelStateTimerInNamedTimerMessage2);
                return null;
            });
