...
public abstract class Transport
{
    ...
    /// <summary>
    ///     Asynchronously opens a logical duplex link between two <see cref="Transport" /> entities over a network. It could
    ///     be backed
    ///     with a real transport layer connection (TCP), socketless connections provided over datagram protocols (UDP), and
    ///     more.
    ///     This call returns a Task of an <see cref="AssociationHandle" />. A faulted Task indicates that the association
    ///     attempt was
    ///     unsuccessful. If the exception is <see cref="InvalidAssociationException" /> then the association request was
    ///     invalid and it's
    ...
    /// <summary>
    ///     Shuts down the transport layer and releases all of the corresponding resources. Shutdown is asynchronous and is
    ///     signaled
...
/// <summary>
///     This exception is thrown when an association setup request is invalid and it is impossible to recover (malformed IP
///     address, unknown hostname, etc...).
/// </summary>
...
///     <see cref="AssociationHandle.ReadHandlerSource" />)
/// </summary>
...
/// <summary>
///     Converts an <see cref="IActorRef" /> into an <see cref="IHandleEventListener" />, so <see cref="IHandleEvent" />
///     messages
...
/// <summary>
///     Message sent to <see cref="IAssociationEventListener" /> registered to a transport (via the TaskCompletionSource
///     returned by <see cref="Transport.Listen" />)
...
/// <summary>
///     Converts an <see cref="IActorRef" /> into an <see cref="IAssociationEventListener" />, so
///     <see cref="IAssociationEvent" /> messages
///     can be passed directly to the Actor.
/// </summary>
...
/// <summary>
///     A Service Provider Interface (SPI) layer for abstracting over logical links (associations) created by a
///     <see cref="Transport" />.
///     Handles are responsible for providing an API for sending and receiving from the underlying channel.
///     To register a listener for processing incoming payload data, the listener must be registered by completing the Task
///     returned by
///     <see cref="AssociationHandle.ReadHandlerSource" />. Incoming data is not processed until this registration takes
///     place.
/// </summary>
public abstract class AssociationHandle
{
    ...
    /// <summary>
    ///     Asynchronously sends the specified <paramref name="payload" /> to the remote endpoint. This method's implementation
    ///     MUST be thread-safe
    ///     as it might be called from different threads. This method MUST NOT block.
    ///     Writes guarantee ordering of messages, but not their reception. The call to write returns with a boolean indicating
    ///     if the
    ///     channel was ready for writes or not. A return value of false indicates that the channel is not yet ready for
    ///     deliver
    ///     (e.g.: the write buffer is full)and the sender  needs to wait until the channel becomes ready again.
    ///     Returning false also means that the current write was dropped (this MUST be guaranteed to ensure duplication-free
    ///     delivery).
    /// </summary>
    ...
    ///     guaranteed.
    ///     The transport that provides the handle MUST guarantee that <see cref="Disassociate()" /> could be called
    ...
    public void Disassociate(string reason, ILoggingAdapter log)
    {
        if (log.IsDebugEnabled)
            log.Debug("Association between local [{0}] and remote [{1}] was disassociated because {2}", LocalAddress,
...
#pragma warning restore 618
    ...
    public override int GetHashCode()
    {
        unchecked
        {
            return ((LocalAddress != null ? LocalAddress.GetHashCode() : 0) * 397) ^
                   (RemoteAddress != null ? RemoteAddress.GetHashCode() : 0);
        }
    }
}