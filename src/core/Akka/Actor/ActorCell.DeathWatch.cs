...
partial class ActorCell
{
    ...
    public IActorRef Unwatch(IActorRef subject)
    {
        ...
        if (!a.Equals(Self) && WatchingContains(a))
        {
            ...
            MaintainAddressTerminatedSubscription(() => { _state = _state.RemoveWatching(a); }, a);
        }
    ...
    protected void ReceivedTerminated(Terminated t)
    {
        ...
        (_state, var customTerminatedMessage) =
            _state.RemoveTerminated(t.ActorRef); // here we know that it is the SAME ref which was put in
    ...
    protected void WatchedActorTerminated(IActorRef actor, bool existenceConfirmed, bool addressTerminated)
    {
        ...
        {
            MaintainAddressTerminatedSubscription(() => { _state = _state.RemoveWatching(actor); }, actor);
            if (!IsTerminating)
            {
        ...
        if (ChildrenContainer.Contains(actor)) HandleChildTerminated(actor);
    }
    ...
    protected void TellWatchersWeDied()
    {
        ...
        {
            MaintainAddressTerminatedSubscription(() => { _state = _state.ClearWatchedBy(); });
        }
    }
    ...
    protected void AddressTerminated(Address address)
    {
        ...
        foreach (var a in _state.GetWatching().Where(a => a.Path.Address == address))
            ((IInternalActorRef)Self).SendSystemMessage(new DeathWatchNotification(a,
                true /*TODO: childrenRefs.getByRef(a).isDefined*/, true));
    }
