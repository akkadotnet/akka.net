    ...
    public void GraphInterpreter_should_implement_chained_identity()
    {
        WithTestSetup((setup, lastEvents) =>
        {
            ...
            var assembly = new GraphAssembly(
                new IGraphStageWithMaterializedValue<Shape, object>[] { _identity, _identity },
                new[] { Attributes.None, Attributes.None },
                new Inlet[] { _identity.Inlet, _identity.Inlet, null },
                new[] { 0, 1, -1 },
                new Outlet[] { null, _identity.Outlet, _identity.Outlet },
                new[] { -1, 0, 1 }
            );
