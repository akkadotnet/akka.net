...
public class ResizerSpec : AkkaSpec
{
    ...
    [Fact(Skip = "DefaultOptimalSizeExploringResizer has not implemented yet")]
    public void
        Resizer_fromConfig_must_load_MetricsBasedResizer_fromConfig_when_optimalsizeexploringresizer_is_enabled()
    {
        var config = ConfigurationFactory.ParseString(@"
    ...
    [Fact(Skip = "DefaultOptimalSizeExploringResizer has not implemented yet")]
    public void
        Resizer_fromConfig_must_load_MetricsBasedResizer_fromConfig_when_both_resizer_and_optimalsizeexploringresizer_is_enabled()
    {
        var config = ConfigurationFactory.ParseString(@"
                resizer {
                  enabled = on
    ...
    public void DefaultResizer_must_use_settings_to_evaluate_capacity()
    {
        ...
        {
            new ActorRefRoutee(Sys.ActorOf<ResizerTestActor>()), new ActorRefRoutee(Sys.ActorOf<ResizerTestActor>())
        };
    ...
    {
        var resizer = new DefaultResizer(2, 10, rampupRate: 0.2);

        resizer.Rampup(9, 10).Should().Be(0);
        resizer.Rampup(5, 5).Should().Be(1);
        resizer.Rampup(6, 6).Should().Be(2);
    }
    ...
    public void DefaultResizer_must_use_settings_to_evaluate_backoff()
    {
        ...
        resizer.Backoff(10, 10).Should().Be(0);
        resizer.Backoff(4, 10).Should().Be(0);
        resizer.Backoff(3, 10).Should().Be(0);
        resizer.Backoff(2, 10).Should().Be(-1);
        resizer.Backoff(0, 10).Should().Be(-1);
        resizer.Backoff(1, 9).Should().Be(-1);
        resizer.Backoff(0, 9).Should().Be(-1);
    }
    ...
    public async Task DefaultResizer_must_grow_as_needed_under_pressure()
    {
        ...
        Func<int, TimeSpan, Task> loop = async (loops, d) =>
        {
            ...
            {
                for (var i = 0; i < loops; i++) await ExpectMsgAsync("done");
            });
        };
    ...
    public async Task DefaultResizer_must_backoff()
    {
        await WithinAsync(10.Seconds(), async () =>
        {
            ...
            var router = Sys.ActorOf(new RoundRobinPool(0, resizer)
    ...
    private class PressureActor : ReceiveActor
    {
        public PressureActor()
        {
            ...
            Receive<string>(s => s == "echo", _ => { Sender.Tell("reply"); });
        }
    }
