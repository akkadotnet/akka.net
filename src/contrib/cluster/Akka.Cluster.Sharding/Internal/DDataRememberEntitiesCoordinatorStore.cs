...
internal sealed class DDataRememberEntitiesCoordinatorStore : ActorBase
{
    ...
    private IImmutableSet<ShardId> _allShards;
    private IActorRef _coordinatorWaitingForShards;
        ...
        int majorityMinCap)
    {
        return Actor.Props.Create(() =>
            new DDataRememberEntitiesCoordinatorStore(typeName, settings, replicator, majorityMinCap));
    }
    ...
    protected override bool Receive(object message)
    {
        switch (message)
        {
            ...
            case RememberEntitiesCoordinatorStore.AddShard m:
                _replicator.Tell(Dsl.Update(_allShardsKey, GSet<string>.Empty, _writeMajority, (Sender, m.ShardId),
                    reg => reg.Add(m.ShardId)));
                return true;
            ...
            case UpdateTimeout m when m.Key.Equals(_allShardsKey) && m.Request is ValueTuple<IActorRef, ShardId> r:
                _log.Error(
                    "The ShardCoordinator was unable to update shards distributed state within 'updating-state-timeout': {0} millis (retrying), adding shard={1}",
