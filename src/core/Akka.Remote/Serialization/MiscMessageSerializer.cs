...
using ActorIdentity = Akka.Actor.ActorIdentity;
using DefaultResizer = Akka.Routing.DefaultResizer;
using FromConfig = Akka.Routing.FromConfig;
using Identify = Akka.Actor.Identify;
using RemoteRouterConfig = Akka.Remote.Routing.RemoteRouterConfig;
using RemoteScope = Akka.Actor.RemoteScope;
using TailChoppingPool = Akka.Routing.TailChoppingPool;
...
public sealed class MiscMessageSerializer : SerializerWithStringManifest
{
        ...
        switch (manifest)
        {
            ...
            default:
                throw new SerializationException(
                    $"Unimplemented deserialization of message with manifest [{manifest}] in [{nameof(MiscMessageSerializer)}]");
        }
    }
    ...
    private ActorIdentity ActorIdentityFromProto(byte[] bytes)
    {
        ...
        return new ActorIdentity(_payloadSupport.PayloadFrom(protoMessage.CorrelationId),
            ResolveActorRef(protoMessage.Path));
    }
    ...
    {
        var protoActor = new ActorRefData();
    ...
    {
        var protoMessage = ActorRefData.Parser.ParseFrom(bytes);
    ...
    {
        var message = new RemoteWatcherHeartbeatResponse();
    ...
    {
        var message = RemoteWatcherHeartbeatResponse.Parser.ParseFrom(bytes);
    ...
    {
        var message = StatusSuccess.Parser.ParseFrom(bytes);
    ...
    {
        var message = StatusFailure.Parser.ParseFrom(bytes);
    ...
    private FromConfig FromConfigFromProto(byte[] bytes)
    {
        ...
        var resizer = fromConfig.Resizer != null
    ...
    {
        var message = new GenericRoutingPool();
    ...
    {
        var broadcastPool = GenericRoutingPool.Parser.ParseFrom(bytes);

        var resizer = broadcastPool.Resizer != null
    ...
    {
        var randomPool = GenericRoutingPool.Parser.ParseFrom(bytes);

        var resizer = randomPool.Resizer != null
    ...
    //
    private byte[] ScatterGatherFirstCompletedPoolToProto(
        ScatterGatherFirstCompletedPool scatterGatherFirstCompletedPool)
    {
        var message = new ScatterGatherPool();
        message.Generic = GenericRoutingPoolBuilder(scatterGatherFirstCompletedPool);
        message.Within = Duration.FromTimeSpan(scatterGatherFirstCompletedPool.Within);
        return message.ToByteArray();
    }

    private ScatterGatherFirstCompletedPool ScatterGatherFirstCompletedPoolFromProto(byte[] bytes)
    {
        var scatterGatherFirstCompletedPool = ScatterGatherPool.Parser.ParseFrom(bytes);

        var resizer = scatterGatherFirstCompletedPool.Generic.Resizer != null
    ...
    private byte[] TailChoppingPoolToProto(TailChoppingPool tailChoppingPool)
    {
        ...
        message.Within = Duration.FromTimeSpan(tailChoppingPool.Within);
        message.Interval = Duration.FromTimeSpan(tailChoppingPool.Interval);
        return message.ToByteArray();
    }
    ...
    private TailChoppingPool TailChoppingPoolFromProto(byte[] bytes)
    {
        ...
        var resizer = tailChoppingPool.Generic.Resizer != null
    ...
    {
        var consistentHashingPool = GenericRoutingPool.Parser.ParseFrom(bytes);

        var resizer = consistentHashingPool.Resizer != null
    ...
    private RemoteRouterConfig RemoteRouterConfigFromProto(byte[] bytes)
    {
        ...
        return new RemoteRouterConfig(_payloadSupport.PayloadFrom(protoMessage.Local).AsInstanceOf<Pool>(),
