...
akka.actor.deployment {
    ...
    public class BoundedQueueReportingActor : MailboxReportingActor,
        IRequiresMessageQueue<IBoundedMessageQueueSemantics>
    {
    }
    ...
    private async Task<IMessageQueue> CheckMailBoxType(Props props, string name, IEnumerable<Type> expectedMailboxTypes)
    {
        ...
        var mailbox = await ExpectMsgAsync<IMessageQueue>(msg =>
        {
            expectedMailboxTypes.ForEach(type => Assert.True(type.IsAssignableFrom(msg.GetType()),
                string.Format(CultureInfo.InvariantCulture, "Type [{0}] is not assignable to [{1}]", msg.GetType(),
                    type)));
        });
    ...
    {
        await CheckMailBoxType(Props.Create<MailboxReportingActor>(), "default-default",
            new[] { typeof(UnboundedMessageQueue) });
    }
