...
/// <summary>
///     This is a special <see cref="Serializer" /> that serializes and deserializes <see cref="ActorSelectionMessage" />
///     only.
/// </summary>
public class MessageContainerSerializer : Serializer
{
        ...
        if (obj is ActorSelectionMessage sel)
        {
            ...
            foreach (var element in sel.Elements)
            {
                ...
                if (element is SelectChildName m1)
                    selection = BuildPattern(m1.Name, Selection.Types.PatternType.ChildName);
                else if (element is SelectChildPattern m)
                    selection = BuildPattern(m.PatternStr, Selection.Types.PatternType.ChildPattern);
                else if (element is SelectParent) selection = BuildPattern(null, Selection.Types.PatternType.Parent);
    ...
    {
        var selectionEnvelope = SelectionEnvelope.Parser.ParseFrom(bytes);
        ...
        for (var i = 0; i < selectionEnvelope.Pattern.Count; i++)
        {
            ...
            if (p.Type == Selection.Types.PatternType.ChildName)
                elements[i] = new SelectChildName(p.Matcher);
            if (p.Type == Selection.Types.PatternType.ChildPattern)
                elements[i] = new SelectChildPattern(p.Matcher);
            if (p.Type == Selection.Types.PatternType.Parent)
        ...
        catch (Exception ex)
        {
            ...
            throw new SerializationException(
                ...
                $"{string.Join(",", elements.Select(e => e.ToString()))}]. {GetErrorForSerializerId(payload.SerializerId)}",
                ex);
        }
    ...
    private static Selection BuildPattern(string matcher, Selection.Types.PatternType tpe)
    {
