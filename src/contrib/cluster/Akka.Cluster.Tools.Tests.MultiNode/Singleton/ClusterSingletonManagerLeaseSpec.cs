...
public class ClusterSingletonManagerLeaseSpecConfig : MultiNodeConfig
{
    public ClusterSingletonManagerLeaseSpecConfig()
    {
        ...
        NodeConfig(new[] { First, Second, Third, Fourth }, new[]
        {
    ...
    internal class ImportantSingleton : ActorBase
    {
        ...
        private readonly Address selfAddress;
        ...
        internal sealed class Response : IEquatable<Response>
        {
            ...
            {
                return obj is Response r && Equals(r);
            }
...
public class ClusterSingletonManagerLeaseSpec : MultiNodeClusterSpec
{
    ...
    public void Cluster_singleton_manager_with_lease_should_form_a_cluster()
    {
        ...
        RunOn(() =>
        {
            ...
            AwaitAssert(
                () =>
                {
                    Cluster.State.Members.Select(i => i.Status).Should()
                        .BeEquivalentTo(MemberStatus.Up, MemberStatus.Up, MemberStatus.Up);
        ...
        RunOn(() =>
        {
            ...
            AwaitAssert(
                () =>
                {
                    Cluster.State.Members.Select(i => i.Status).Should().BeEquivalentTo(MemberStatus.Up,
                        MemberStatus.Up, MemberStatus.Up, MemberStatus.Up);
                }, TimeSpan.FromSeconds(10));
        }, _config.Third);
        ...
        RunOn(() =>
        {
            ...
            AwaitAssert(
                ...
                {
                    Cluster.State.Members.Select(i => i.Status).Should().BeEquivalentTo(MemberStatus.Up,
                        MemberStatus.Up, MemberStatus.Up, MemberStatus.Up, MemberStatus.Up);
                }, TimeSpan.FromSeconds(10));
        }, _config.Fourth);
    ...
    {
        RunOn(() => { Sys.ActorOf(TestLeaseActor.Props, $"lease-{Sys.Name}"); }, _config.Controller);
    ...
    public void Cluster_singleton_manager_with_lease_should_find_the_lease_on_every_node()
    {
        ...
        var leaseRef = ExpectMsg<ActorIdentity>().Subject;
    ...
    public void Cluster_singleton_manager_with_lease_should_Start_singleton_and_ping_from_all_nodes()
    {
        ...
        _proxy = Sys.ActorOf(
            ClusterSingletonProxy.Props(
                "/user/important",
                ClusterSingletonProxySettings.Create(Sys).WithRole("worker")));
        ...
        RunOn(() =>
        {
            ...
            ExpectMsg<TestLeaseActor.LeaseRequests>(msg =>
                msg.Requests.Should().BeEquivalentTo(new TestLeaseActor.Acquire(GetAddress(_config.First).HostPort())));
            TestLeaseActorClientExt.Get(Sys).GetLeaseActor().Tell(
                new TestLeaseActor.ActionRequest(new TestLeaseActor.Acquire(GetAddress(_config.First).HostPort()),
                    true));
        }, _config.Controller);
        ...
        RunOn(
            () =>
            {
                ExpectMsg(new ClusterSingletonManagerLeaseSpecConfig.ImportantSingleton.Response("Ping",
                    GetAddress(_config.First)));
            }, _config.First, _config.Second, _config.Third, _config.Fourth);
        EnterBarrier("pinged");
    }
    ...
    public void Cluster_singleton_manager_with_lease_should_Move_singleton_when_oldest_node_downed()
    {
        ...
        RunOn(() =>
        {
            ...
            leaseActor.Tell(
                new TestLeaseActor.ActionRequest(new TestLeaseActor.Release(GetAddress(_config.First).HostPort()),
                    true));
            leaseActor.Tell(
                new TestLeaseActor.ActionRequest(new TestLeaseActor.Acquire(GetAddress(_config.Second).HostPort()),
                    true));
        }, _config.Controller);
        ...
        RunOn(
            ...
            {
                ExpectMsg(
                    new ClusterSingletonManagerLeaseSpecConfig.ImportantSingleton.Response("Ping",
                        GetAddress(_config.Second)), TimeSpan.FromSeconds(20));
            }, _config.Second, _config.Third, _config.Fourth);
    ...
    public void Cluster_singleton_manager_with_lease_proxy_should_reacquire_singleton_actor_when_lease_lost()
    {
        RunOn(() =>
        {
            ...
            actorRef.Tell(new LeaseLost(new Exception("Lease not found")),
                TestLeaseActorClientExt.Get(Sys).GetLeaseActor());
        }, _config.Second);
        ...
        RunOn(() =>
        {
            ...
            TestLeaseActorClientExt.Get(Sys).GetLeaseActor().Tell(
                new TestLeaseActor.ActionRequest(new TestLeaseActor.Release(GetAddress(_config.Second).HostPort()),
                    false));
        }, _config.Controller);
