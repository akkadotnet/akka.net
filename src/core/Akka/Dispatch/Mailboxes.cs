...
/// <summary>
///     Contains the directory of all <see cref="MailboxType" />s registered and configured with a given
...
public class Mailboxes
{
    ...
    ///     This exception is thrown if a mapping is not configured for the given
    ///     <paramref name="queueType" />.
    /// </exception>
    ...
    /// <exception cref="ConfigurationException">
    ///     This exception is thrown if the mailbox type is not configured or the system could not load or find the type
    ///     specified.
    /// </exception>
            ...
            else
            {
                ...
                var mailboxType = Type.GetType(mailboxTypeName)
                                  ?? throw new ConfigurationException(
                                      $"Found mailbox-type [{mailboxTypeName}] in configuration for [{id}], but could not find that type in any loaded assemblies.");
                ...
                try
                {
                    ...
                    if (!_mailboxNonZeroPushTimeoutWarningIssued)
                        ...
                        {
                            Warn(
                                $"Configured potentially-blocking mailbox [{id}] configured with non-zero PushTimeOut ({m.PushTimeout}), " +
                ...
                catch (Exception ex)
                {
                    throw new ArgumentException(
                        $"Cannot instantiate MailboxType {mailboxType}, defined in [{id}]. Make sure it has a public " +
    ...
    private Type GetProducedMessageQueueType(MailboxType mailboxType)
    {
        ...
        throw new ArgumentException(nameof(mailboxType),
            $"No IProducesMessageQueue<TQueue> supplied for {mailboxType}; illegal mailbox type definition.");
    }

    private Type GetMailboxRequirement(Config config)
    {
        var mailboxRequirement = config.GetString("mailbox-requirement");
        return mailboxRequirement == null || mailboxRequirement.Equals(NoMailboxRequirement)
            ? typeof(IMessageQueue)
            : Type.GetType(mailboxRequirement, true);
    }
    ...
    public MailboxType GetMailboxType(Props props, Config dispatcherConfig)
    {
        ...
        MailboxType VerifyRequirements(MailboxType mailboxType)
        {
            ...
            if (hasMailboxRequirement && !mailboxRequirement.IsAssignableFrom(mqType.Value))
                throw new ArgumentException(
                    $"produced message queue type [{mqType.Value}] does not fulfill requirement for dispatcher [{id}]." +
            ...
            if (HasRequiredType(actorType) && !actorRequirement.Value.IsAssignableFrom(mqType.Value))
                throw new ArgumentException(
                    $"produced message queue type of [{mqType.Value}] does not fulfill requirement for actor class [{actorType}]." +
