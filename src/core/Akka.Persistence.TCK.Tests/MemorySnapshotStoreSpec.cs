...
public class MemorySnapshotStoreSpec : SnapshotStoreSpec
{
    ...
    public void MemorySnapshotStore_is_threadsafe()
    {
        EventFilter.Error().Expect(0, () =>
        {
            ...
            var sas = new List<IActorRef> { sa1, sa2, sa3 };

            // hammer with write requests
            Parallel.ForEach(Enumerable.Range(0, writeCount),
                i => { sas[ThreadLocalRandom.Current.Next(0, 3)].Tell(i); });
            ...
            ReceiveN(writeCount).All(x => x is SaveSnapshotSuccess).Should()
                .BeTrue("Expected all snapshot store saves to be successful, but some were not");
            ...
            var sas2 = new List<IActorRef> { sa4, sa5, sa6 };

            // hammer with write requests
            Parallel.ForEach(Enumerable.Range(0, writeCount),
                i => { sas2[ThreadLocalRandom.Current.Next(0, 3)].Tell(i); });
            ...
            var sas12 = new List<IActorRef> { sa12, sa22, sa32 };

            // hammer other entities
            Parallel.ForEach(Enumerable.Range(0, writeCount),
                i => { sas12[ThreadLocalRandom.Current.Next(0, 3)].Tell(i); });

            ReceiveN(writeCount * 2).All(x => x is SaveSnapshotSuccess).Should()
                .BeTrue("Expected all snapshot store saves to be successful, but some were not");
        });
    }
    ...
    public class SnapshotActor : ReceivePersistentActor
    {
        ...
        public SnapshotActor(string persistenceId, IActorRef reporter)
        {
            ...
            {
                if (offer.Snapshot is int i) _count = i;
            });
            ...
            Command<string>(str => str.Equals("get"), _ => { Sender.Tell(_count); });
        }
