...
public abstract partial class TestKitBase
{
    ...
    ///     If null then will be ignored. If not null then will be initially cleared, then filled with
    ///     all the messages until <paramref name="isMessage" /> returns <c>true</c>
    /// </param>
    /// <returns>Returns the message that <paramref name="isMessage" /> matched</returns>
    public T FishForMessage<T>(
        Predicate<T> isMessage,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            _assertions.AssertTrue(
                msg != null,
                "Timeout ({0}) during fishForMessage{1}", maxValue,
                string.IsNullOrEmpty(hint) ? "" : ", hint: " + hint);
            if (msg is T msg1 && isMessage(msg1)) return msg1;
            allMessages?.Add(msg);
        }
    }
    ...
    /// <summary>
    ///     Waits for a <paramref name="max" /> period of 'radio-silence' limited to a number of
    ///     <paramref name="maxMessages" />.
    ...
    /// <param name="cancellationToken"></param>
    /// If set to null then this method will loop for an infinite number of
    /// <paramref name="max" />
    /// periods. 
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (uint i = 0;; i++)
        {
            _assertions.AssertFalse(maxMessages.HasValue && i > maxMessages.Value,
                $"{nameof(maxMessages)} violated (current iteration: {i}).");
    ...
    ///     <remarks>
    ///         <b>Note!</b> that the returned <paramref name="envelope" />
    ///         is a <see cref="MessageEnvelope" /> containing the sender and the message.
    ///     </remarks>
    ///     <remarks>This method does NOT automatically scale its Duration parameter using <see cref="Dilated(TimeSpan)" />!</remarks>
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            Log.Warning(
                "Trying to receive message from TestActor queue with infinite timeout! Will wait indefinitely!");
        ...
        ConditionalLog(shouldLog, "Received no message after {0}.{1}", Now - start,
            cancellationToken.IsCancellationRequested ? " Was canceled" : "");
        return take;
    }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (count < msgs)
        {
            ...
            if (result != null)
            {
                ...
                var (receiveSuccess, receiveEnvelope) =
                    await InternalTryReceiveOneAsync(TimeSpan.Zero, true, cancellationToken);
                if (!receiveSuccess)
    ...
    ///     It will continue to receive messages until the <paramref name="shouldContinue" /> predicate returns <c>false</c> or
    ///     the idle
    ///     timeout is met (disabled by default) or the overall
    ///     maximum duration is elapsed or expected messages count is reached.
    ///     If a message that isn't of type <typeparamref name="T" /> the parameter
    ///     <paramref name="shouldIgnoreOtherMessageTypes" />
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        ConditionalLog(
            ...
            msgs == int.MaxValue ? "" : msgs + " ", typeof(T), maxValue,
            shouldIgnoreOtherMessageTypes ? "be ignored" : "cause this to stop");
    ...
    ///     <para>
    ///         <c>True</c> is returned if a message existed, and the message
    ...
    public ValueTask<(bool success, MessageEnvelope envelope)> TryPeekOneAsync(TimeSpan? max,
        CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        ConditionalLog(shouldLog, "Peeked no message after {0}.{1}", Now - start,
            cancellationToken.IsCancellationRequested ? " Was canceled" : "");
        return peek;
    }
