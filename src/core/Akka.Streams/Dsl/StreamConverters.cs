...
public static class StreamConverters
{
    ...
    ///     </para>
    ///     <para>
    ///         It materializes a <see cref="Task{TResult}" /> of <see cref="IOResult" /> containing the number of bytes read
    ///         from the source file upon completion,
    ...
    /// <returns>TBD</returns>
    public static Source<ByteString, Task<IOResult>> FromInputStream(Func<Stream> createInputStream,
        int chunkSize = 8192)
    {
    ...
    /// <summary>
    ///     Creates a <see cref="Source{TOut,TMat}" /> which when materialized will return an <see cref="Stream" /> which it is
    ///     possible
    ///     to write the ByteStrings to the stream this Source is attached to.
    ///     This Source is intended for inter-operation with legacy APIs since it is inherently blocking.
    ///     You can configure the default dispatcher for this Source by changing the "akka.stream.blocking-io-dispatcher" or
    ///     set it for a given Source by using <see cref="ActorAttributes.CreateDispatcher" />.
    ///     The created <see cref="Stream" /> will be closed when the <see cref="Source{TOut,TMat}" /> is cancelled, and
    ///     closing the <see cref="Stream" />
    ///     will complete this <see cref="Source{TOut,TMat}" />.
    /// </summary>
    /// <param name="writeTimeout">
    ///     The max time the write operation on the materialized OutputStream should block, defaults to
    ///     5 seconds
    /// </param>
    /// <returns>TBD</returns>
    public static Source<ByteString, Stream> AsOutputStream(TimeSpan? writeTimeout = null)
    {
        return Source.FromGraph(new OutputStreamSourceStage(writeTimeout ?? TimeSpan.FromSeconds(5)));
    }

    /// <summary>
    ///     Creates a Sink which writes incoming <see cref="ByteString" />s to an <see cref="Stream" /> created by the given
    ///     function.
    ///     Materializes a <see cref="Task{TResult}" /> of <see cref="IOResult" /> that will be completed with the size of the
    ///     file (in bytes) at the streams completion,
    ///     and a possible exception if IO operation was not completed successfully.
    ///     You can configure the default dispatcher for this Source by changing the "akka.stream.blocking-io-dispatcher" or
    ///     set it for a given Source by using <see cref="ActorAttributes.CreateDispatcher" />.
    ///     If <paramref name="autoFlush" /> is true the OutputStream will be flushed whenever a byte array is written,
    ///     defaults to false.
    ///     The <see cref="Stream" /> will be closed when the stream flowing into this <see cref="Sink{TIn,TMat}" /> is
    ///     completed. The <see cref="Sink{TIn,TMat}" />
    ...
    ///     If set to true the <see cref="Stream" /> will be flushed whenever a byte array is written,
    ///     default is false
    /// </param>
    /// <returns>TBD</returns>
    public static Sink<ByteString, Task<IOResult>> FromOutputStream(Func<Stream> createOutputStream,
    ...
    ///     You can configure the default dispatcher for this Source by changing the "akka.stream.blocking-io-dispatcher" or
    ///     set it for a given Source by using <see cref="ActorAttributes.CreateDispatcher" />.
    ///     The <see cref="Stream" /> will be closed when the stream flowing into this <see cref="Sink{TIn,TMat}" /> completes,
    ///     and
