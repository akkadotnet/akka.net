...
akka.persistence.snapshot-store.plugin = ""akka.persistence.no-snapshot-store""");
    ...
    [Fact]
    public void
        PersistentActor_journal_protocol_should_not_send_WriteMessages_while_a_write_is_still_outstanding_when_using_simple_Persist()
    {
    ...
    [Fact]
    public void
        PersistentActor_journal_protocol_should_not_send_WriteMessages_while_a_write_is_still_outstanding_when_using_nested_Persist()
    {
    ...
    [Fact]
    public void
        PersistentActor_journal_protocol_should_not_send_WriteMessages_while_a_write_is_still_outstanding_when_using_nested_multiple_Persist()
    {
    ...
    [Fact]
    public void
        PersistentActor_journal_protocol_should_not_send_WriteMessages_while_a_write_is_still_outstanding_when_using_large_number_of_Persist_calls()
    {
        var subject = StartActor("test-4");
        subject.Tell(
            new Multi(Enumerable.Range(0, 30).Select(i => new Persist(i, "a-" + i)).Cast<ICommand>().ToArray()));
        var w1 = ExpectWrite(subject, Enumerable.Range(0, 30).Select(i => new Msgs("a-" + i)).ToArray());
        Confirm(w1);
        for (var i = 0; i < 30; i++) ExpectMsg<Done>(m => m.Id == i && m.Sub == 1);
        subject.Tell(PoisonPill.Instance);
    ...
    [Fact]
    public void
        PersistentActor_journal_protocol_should_not_send_WriteMessages_while_a_write_is_still_outstanding_when_using_large_number_of_PersistAsync_calls()
    {
        ...
        Confirm(w0);
        for (var i = 1; i <= 11; i++) ExpectMsg<Done>(m => m.Id == -1 && m.Sub == i);
    ...
    [Fact]
    public void
        PersistentActor_journal_protocol_should_not_send_WriteMessages_while_a_write_is_still_outstanding_when_the_actor_fails_with_queued_events()
    {
