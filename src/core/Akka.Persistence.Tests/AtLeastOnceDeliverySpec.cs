...
public class AtLeastOnceDeliverySpec : PersistenceSpec
{
    ...
    public void AtLeastOnceDelivery_must_deliver_messages_in_order_when_nothing_is_lost()
    {
        ...
        var sender =
            Sys.ActorOf(
                Props.Create(() =>
                    new Sender(TestActor, Name, TimeSpan.FromMilliseconds(500), 5, 1000, false, destinations)), Name);
    ...
    public void AtLeastOnceDelivery_must_redeliver_lost_messages()
    {
        ...
        var destinations =
            new Dictionary<string, ActorPath>
            {
    ...
    public void
        AtLeastOnceDelivery_must_resend_replayed_deliveries_with_an_initially_in_order_strategy_before_delivering_fresh_messages()
    {
        var probe = CreateTestProbe();
        var dest = Sys.ActorOf(Props.Create(() => new Destination(probe.Ref)));
        var destinations =
        ...
        probe.ExpectMsg<Action>(a => a.Id == 4 && a.Payload == "a-4",
            TimeSpan.FromSeconds(20)); // redelivered, 3th time
    ...
    public void AtLeastOnceDelivery_must_warn_about_unconfirmed_messages()
    {
        ...
        var sender =
            Sys.ActorOf(
                Props.Create(() =>
                    new Sender(TestActor, Name, TimeSpan.FromMilliseconds(500), 3, 1000, false, destinations)), Name);
    ...
    public void AtLeastOnceDelivery_must_redeliver_many_lost_messages()
    {
        ...
        var sender =
            Sys.ActorOf(
                Props.Create(() =>
                    new Sender(TestActor, Name, TimeSpan.FromMilliseconds(1000), 5, 1000, true, destinations)), Name);
    ...
    public void AtLeastOnceDelivery_must_limit_the_number_of_messages_redelivered_at_once()
    {
        ...
        const int N = 10;
        for (var i = 1; i <= N; i++) sender.Tell(new Req("a-" + i), probe.Ref);

        // initially all odd messages should go through
        for (var i = 1; i <= N; i = i + 2) probeA.ExpectMsg<Action>(a => a.Id == i && a.Payload == "a-" + i);
    ...
    private class Sender : AtLeastOnceDeliveryActor
    {
        ...
        public Sender(IActorRef testActor, string name, TimeSpan redeliverInterval, int warn, int redeliveryBurstLimit,
            bool isAsync, IDictionary<string, ActorPath> destinations)
            : base(new PersistenceSettings.AtLeastOnceDeliverySettings(redeliverInterval, redeliveryBurstLimit, warn,
                100000))
        {
        ...
        protected override bool ReceiveCommand(object message)
        {
            switch (message)
            {
                case Req req:
                {
                    ...
                    {
                        Sender.Tell(InvalidReq.Instance);
                    }
                    else
                    {
                ...
                case UnconfirmedWarning warn:
                    _log.Debug("Sender got unconfirmed warning: unconfirmed deliveries count {0}",
                        warn.UnconfirmedDeliveries.Count());
        ...
        private void UpdateState(IEvt evt)
        {
            switch (evt)
            {
                ...
                case AcceptedSelectionReq a:
                    _log.Debug("Deliver(destination, deliveryId => Action(deliveryId, {0})), recovering: {1}",
                        a.Payload, IsRecovering);
                    Deliver(Context.System.ActorSelection(a.DestinationPath),
                        deliveryId => new Action(deliveryId, a.Payload));
                    break;
                case ReqDone r:
    ...
    private class Unreliable : ReceiveActor
    {
        ...
        public Unreliable(int dropMod, IActorRef target)
        {
            ...
            {
                _count++;
                if (_count % dropMod != 0) target.Forward(message);
                return true;
            });
        }
    }
    ...
    [Serializable]
    private sealed class AcceptedSelectionReq : IEvt
    {
