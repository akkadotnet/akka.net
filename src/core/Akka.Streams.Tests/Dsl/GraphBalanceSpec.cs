        ...
        await this.AssertAllStagesStoppedAsync(() =>
        {
            ...
            var t = RunnableGraph.FromGraph(GraphDsl.Create(sink, sink, sink, sink, sink, ValueTuple.Create,
                (b, s1, s2, s3, s4, s5) =>
                {
                    ...
                    var source = Source.From(Enumerable.Range(0, 15)).MapMaterializedValue(_ =>
                        default((Task<IEnumerable<int>>, Task<IEnumerable<int>>, Task<IEnumerable<int>>,
                            Task<IEnumerable<int>>, Task<IEnumerable<int>>)));
                    b.From(source).To(balance.In);
                    ...
                    var source =
                        Source.From(Enumerable.Range(1, 7))
                            .MapMaterializedValue(_ =>
                                default((TestSubscriber.Probe<int>, TestSubscriber.Probe<int>, TestSubscriber.Probe<int>
                                    )));
                    b.From(source).To(balance.In);
                    b.From(balance.Out(0)).To(o1);
                    b.From(balance.Out(1)).To(o2);
