...
public class DilatedTests : AkkaSpec
{
    ...
    public async Task AwaitConditionAsync_should_dilate_timeout()
    {
        ...
        stopwatch.Stop();
        AssertDilated(stopwatch.ElapsedMilliseconds,
            $"Expected the timeout to be {ExpectedTimeout} but in fact it was {stopwatch.ElapsedMilliseconds}.");
    }
    ...
    public async Task ExpectMsgAllOfAsync_should_dilate_timeout()
    {
        ...
        await Awaiting(async () =>
                await ExpectMsgAllOfAsync(TimeSpan.FromMilliseconds(Timeout), new[] { "1", "2" }).ToListAsync())
            .Should().ThrowAsync<TrueException>();
        stopwatch.Stop();
        AssertDilated(stopwatch.ElapsedMilliseconds,
            $"Expected the timeout to be {ExpectedTimeout} but in fact it was {stopwatch.ElapsedMilliseconds}.");
