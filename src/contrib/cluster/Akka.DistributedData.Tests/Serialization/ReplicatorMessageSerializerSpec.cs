...
public class ReplicatorMessageSerializerSpec : TestKit.Xunit2.TestKit
{
    ...
    public ReplicatorMessageSerializerSpec(ITestOutputHelper output) : base(BaseConfig,
        "ReplicatorMessageSerializerSpec", output)
    {
    ...
    public void ReplicatorMessageSerializer_should_serialize_Replicator_message()
    {
        ...
        _serializer.Invoking(s =>
            {
                s.ToBinary(new Get(_keyA, new ReadMajority(TimeSpan.FromMilliseconds((double)int.MaxValue * 3)),
                    "x"));
            })
            .Should().Throw<ArgumentOutOfRangeException>(
                "Our protobuf protocol does not support timeouts larger than unsigned ints")
        ...
        CheckSerialization(new DataEnvelope(data1,
            ImmutableDictionary.CreateRange(new Dictionary<UniqueAddress, IPruningState>
            {
