...
public class ExternalShardAllocationStrategy : IStartableAllocationStrategy
{
    ...
    public async Task<IActorRef> AllocateShard(IActorRef requester, string shardId,
        IImmutableDictionary<IActorRef, IImmutableList<string>> currentShardAllocations)
    {
        _log.Debug("allocateShard [{0}] [{1}] [{2}]", shardId, requester,
    ...
    public async Task<IImmutableSet<string>> Rebalance(
        IImmutableDictionary<IActorRef, IImmutableList<string>> currentShardAllocations,
        IImmutableSet<string> rebalanceInProgress)
    {
        ...
        _log.Debug("Current allocations by address: [{0}]",
            string.Join(", ", currentAllocationByAddress.Select(i => $"{i.Key}: {string.Join(", ", i.Value)}")));

        try
        {
            var desiredMappings = await _shardState.Ask<GetShardLocationsResponse>(GetShardLocations.Instance, Timeout);

            _log.Debug("desired allocations: [{0}]",
                string.Join(", ", desiredMappings.DesiredAllocations.Select(i => $"{i.Key}: {i.Value}")));
            ...
            if (!shardsThatNeedRebalanced.IsEmpty)
                _log.Debug("Shards not currently in their desired location [{0}]",
                    string.Join(", ", shardsThatNeedRebalanced));
    ...
    public sealed class GetShardLocationsResponse : INoSerializationVerificationNeeded,
        IEquatable<GetShardLocationsResponse>
    {
        ...
        {
            return
                $"GetShardLocationsResponse({string.Join(", ", DesiredAllocations.Select(i => $"{i.Key}: {i.Value}"))})";
        }

        #endregion
    }

    public sealed class GetShardLocationResponse : INoSerializationVerificationNeeded,
        IEquatable<GetShardLocationResponse>
    {
        ...
        public override string ToString()
        {
            return $"GetShardLocationResponse({Address})";
        }
    ...
    private class DDataStateActor : ActorBase, IWithUnboundedStash
    {
        ...
        {
            return Actor.Props.Create(() => new DDataStateActor(typeName));
        }
        ...
        protected override bool Receive(object message)
        {
            switch (message)
            {
                ...
                case GetShardLocation m:
                    ...
                    var shardLoc =
                        _currentLocations.GetValueOrDefault(m.Shard); //.map(asStr => AddressFromURIString(asStr));
