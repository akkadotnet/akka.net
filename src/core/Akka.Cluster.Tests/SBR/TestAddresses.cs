...
public static class TestAddresses
{
    static TestAddresses()
    {
        ...
        MemberAWeaklyUp = new Member(MemberA.UniqueAddress, int.MaxValue, MemberStatus.WeaklyUp, MemberA.Roles,
            AppVersion.Zero);
        MemberBWeaklyUp = new Member(MemberB.UniqueAddress, int.MaxValue, MemberStatus.WeaklyUp, MemberB.Roles,
            AppVersion.Zero);
    }
    ...
    public static Member Joining(Member m)
    {
        return Member.Create(m.UniqueAddress, m.Roles, AppVersion.Zero);
    }

    public static Member Leaving(Member m)
    {
        return m.Copy(MemberStatus.Leaving);
    }

    public static Member Exiting(Member m)
    {
        return Leaving(m).Copy(MemberStatus.Exiting);
    }

    public static Member Downed(Member m)
    {
        return m.Copy(MemberStatus.Down);
    }
}