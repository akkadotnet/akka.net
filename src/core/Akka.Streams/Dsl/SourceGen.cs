...
public static class SourceGen
{
    ...
    [ApiMayChange]
    public static Source<TOut, TMat> UnfoldFlow<TState, TOut, TMat>(TState seed,
        IGraph<FlowShape<TState, (TState, TOut)>, TMat> flow, TimeSpan timeout)
    {
        return UnfoldFlowGraph(
            new FanOut2UnfoldingStage<(TState, TOut), TState, TOut>(shape =>
                new UnfoldFlowGraphStageLogic<TState, TOut>(shape, seed, timeout)), flow);
    }
    ...
    ///         generates a pair of the next state <typeparamref name="TState" /> and output elements of type
    ...
    /// <param name="timeout">timeout</param>
    [ApiMayChange]
    public static Source<TOut, TMat> UnfoldFlowWith<TOut, TState, TFlowOut, TMat>(TState seed,
        IGraph<FlowShape<TState, TFlowOut>, TMat> flow, Func<TFlowOut, Option<(TState, TOut)>> unfoldWith,
        TimeSpan timeout)
    {
        return UnfoldFlowGraph(
            new FanOut2UnfoldingStage<TFlowOut, TState, TOut>(shape =>
                new UnfoldFlowWithGraphStageLogic<TFlowOut, TState, TOut>(shape, seed, unfoldWith, timeout)), flow);
    }

    private static Source<TOut, TMat> UnfoldFlowGraph<TOut, TState, TFlowOut, TMat>(
        GraphStage<FanOutShape<TFlowOut, TState, TOut>> fanOut2Stage, IGraph<FlowShape<TState, TFlowOut>, TMat> flow)
    {
    ...
    private class UnfoldFlowGraphStageLogic<TState, TOut> : UnfoldFlowGraphStageLogic<(TState, TOut), TState, TOut>,
        IInHandler
    {
        public UnfoldFlowGraphStageLogic(FanOutShape<(TState, TOut), TState, TOut> shape, TState seed, TimeSpan timeout)
            : base(shape, seed, timeout)
        {
    ...
    private class
        UnfoldFlowWithGraphStageLogic<TFlowOut, TState, TOut> : UnfoldFlowGraphStageLogic<TFlowOut, TState, TOut>,
        IInHandler
    {
        private readonly Func<TFlowOut, Option<(TState, TOut)>> _unfoldWith;

        public UnfoldFlowWithGraphStageLogic(FanOutShape<TFlowOut, TState, TOut> shape, TState seed,
            Func<TFlowOut, Option<(TState, TOut)>> unfoldWith, TimeSpan timeout) : base(shape, seed, timeout)
        {
