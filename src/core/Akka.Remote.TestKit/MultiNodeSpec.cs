...
{
    private readonly Lazy<RoleName> _myself;

    private ImmutableList<string> _allDeploy = ImmutableList.Create<string>();
    ...
    private Config _commonConf = ConfigurationFactory.Empty;

    private ImmutableDictionary<RoleName, ImmutableList<string>> _deployments =
        ImmutableDictionary.Create<RoleName, ImmutableList<string>>();

    private ImmutableDictionary<RoleName, Config> _nodeConf = ImmutableDictionary.Create<RoleName, Config>();
    private bool _testTransport;
    ...
    protected MultiNodeConfig()
    {
        ...
        if (string.IsNullOrEmpty(roleName))
            ...
            {
                if (MultiNodeSpec.SelfIndex > Roles.Count)
                    throw new ArgumentException("not enough roles declared for this test");
            ...
            _myself = new Lazy<RoleName>(() =>
            {
                ...
                if (myself == default) throw new ArgumentException($"cannot find {roleName} among configured roles");
                return myself;
            });
    }
    ...
    public RoleName Myself => _myself.Value;
...
public abstract class MultiNodeSpec : TestKitBase, IMultiNodeSpecCallbacks, IDisposable
{
    ...
    public static int MaxNodes
    {
        get
        {
            if (_maxNodes == MaxNodesUnset) _maxNodes = CommandLine.GetInt32("multinode.max-nodes");
    ...
    public static string SelfName
    {
        get
        {
            if (string.IsNullOrEmpty(_multiNodeHost)) _multiNodeHost = CommandLine.GetProperty("multinode.host");

            //Run this assertion every time. Consistency is more important than performance.
            if (string.IsNullOrEmpty(_multiNodeHost))
                throw new InvalidOperationException("multinode.host must not be empty");
    ...
    public static int SelfPort
    {
        get
        {
            if (_selfPort == SelfPortUnsetValue) //unset
            {
                ...
                _selfPort = string.IsNullOrEmpty(selfPortStr) ? 0 : int.Parse(selfPortStr);
            }

            if (!(_selfPort >= 0 && _selfPort < 65535))
                throw new InvalidOperationException("multinode.port is out of bounds: " + _selfPort);
            return _selfPort;
        }
    }
    ...
    public static string ServerName
    {
        get
        {
            if (string.IsNullOrEmpty(_serverName)) _serverName = CommandLine.GetProperty("multinode.server-host");
            if (string.IsNullOrEmpty(_serverName))
                throw new InvalidOperationException("multinode.server-host must not be empty");
            return _serverName;
        }
    }
    ...
    public static int ServerPort
    {
        get
        {
            if (_serverPort == ServerPortUnsetValue)
            {
                ...
                _serverPort = string.IsNullOrEmpty(serverPortStr) ? ServerPortDefault : int.Parse(serverPortStr);
            }

            if (!(_serverPort > 0 && _serverPort < 65535))
                throw new InvalidOperationException("multinode.server-port is out of bounds: " + _serverPort);
            return _serverPort;
        }
    }
    ...
    public static int SelfIndex
    {
        get
        {
            if (_selfIndex == SelfIndexUnset) _selfIndex = CommandLine.GetInt32("multinode.index");

            if (!(_selfIndex >= 0 && _selfIndex < MaxNodes))
                throw new InvalidOperationException("multinode.index is out of bounds: " + _selfIndex);
            return _selfIndex;
        }
    }
    ...
    protected virtual TimeSpan ShutdownTimeout => TimeSpan.FromSeconds(5);

    /// <summary>
    ///     Override this and return `true` to assert that the
    ///     shutdown of the `ActorSystem` was done properly.
    /// </summary>
    protected virtual bool VerifySystemShutdown => false;
        ...
        get
        {
            ...
            if (initialParticipants <= 0)
                throw new InvalidOperationException(
                    "InitialParticipantsValueFactory must be populated early on, and it must be greater zero");
            if (initialParticipants > MaxNodes)
                throw new InvalidOperationException("not enough nodes to run this test");
            ...
            foreach (var pair in ConfigurationFactory.ParseString(deployString).AsEnumerable())
                ...
                else
                {
                    throw new ArgumentException(string.Format(
                        "key {0} must map to deployment section, not simple value {1}",
                        pair.Key, pair.Value));
                }
        }
    }
    ...
    private sealed class Replacement
    {
        ...
        public string Addr => _addr.Value;
    }
}
