...
internal class Controller : UntypedActor, ILogReceive
{
    ...
    private readonly TestConductorSettings _settings = TestConductor.Get(Context.System).Settings;

    // map keeping unanswered queries for node addresses (enqueued upon GetAddress, serviced upon NodeInfo)
    private ImmutableDictionary<RoleName, ImmutableHashSet<IActorRef>> _addrInterest =
    ...
    private int _initialParticipants;

    private ImmutableDictionary<RoleName, NodeInfo> _nodes =
    ...
    public Controller(int initialParticipants, IPEndPoint controllerPort)
    {
        ...
        _connection = RemoteConnection.CreateConnection(Role.Server, controllerPort,
            _settings.ServerSocketWorkerPoolSize,
        ...
        return new OneForOneStrategy(e =>
        {
            switch (e)
            {
                ...
                case BarrierCoordinator.WrongBarrierException wrongBarrier:
                    wrongBarrier.Client.Tell(
                        new ToClient<BarrierResult>(new BarrierResult(wrongBarrier.Barrier, false)));
    ...
    protected override void OnReceive(object message)
    {
        ...
        if (message is ICommandOp)
            switch (message)
            {
                ...
                {
                    if (!_nodes.TryGetValue(throttle.Target, out var target))
                        throw new IllegalActorStateException(
                            $"Throttle target {throttle.Target} was not found among nodes registered in {nameof(Controller)}: {string.Join(", ", _nodes.Keys)}");
                    if (!_nodes.TryGetValue(throttle.Node, out var source))
                        throw new IllegalActorStateException(
                            $"Throttle source {throttle.Node} was not found among nodes registered in {nameof(Controller)}: {string.Join(", ", _nodes.Keys)}");

                    source.FSM.Forward(new ToClient<ThrottleMsg>(new ThrottleMsg(target.Addr, throttle.Direction,
                        throttle.RateMBit)));
                    return;
                }
                case Disconnect disconnect:
                {
                    if (!_nodes.TryGetValue(disconnect.Target, out var target))
                        throw new IllegalActorStateException(
                            $"Disconnect target {disconnect.Target} was not found among nodes registered in {nameof(Controller)}: {string.Join(", ", _nodes.Keys)}");
                    if (!_nodes.TryGetValue(disconnect.Node, out var source))
                        throw new IllegalActorStateException(
                            $"Disconnect source {disconnect.Node} was not found among nodes registered in {nameof(Controller)}: {string.Join(", ", _nodes.Keys)}");
                ...
                case Terminate terminate:
                {
                    ...
                    if (!_nodes.TryGetValue(terminate.Node, out var node))
                        throw new IllegalActorStateException(
                            $"Terminate target {terminate.Node} was not found among nodes registered in {nameof(Controller)}: {string.Join(", ", _nodes.Keys)}");
    ...
    internal sealed class NodeInfo : IEquatable<NodeInfo>
    {
        ...
        public override bool Equals(object obj)
        {
            return obj is NodeInfo node && Equals(node);
        }
