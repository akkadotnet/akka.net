...
public class TestActorRefSpec : AkkaSpec
{
    ...
    public void TestActorRef_must_stop_when_sent_a_PoisonPill()
    {
        ...
        var a = new TestActorRef<WorkerActor>(Sys, Props.Create(() => new WorkerActor(_thread, _otherThread)), null,
            "will-be-killed");
    ...
    public void TestActorRef_must_support_futures()
    {
        ...
        if (!task.Wait(DefaultTimeout))
            XAssert.Fail("Timed out"); //Using a timeout to stop the test if there is something wrong with the code
    ...
    {
        var actorRef =
            new TestActorRef<SaveStringActor>(Sys, Props.Create(() => new SaveStringActor(_thread, _otherThread)));
    ...
    {
        var a = new TestActorRef<WorkerActor>(Sys,
            Props.Create(() => new WorkerActor(_thread, _otherThread)).WithDispatcher("test-dispatcher1"));
        var actorRef = (InternalTestActorRef)a.Ref;
    ...
    {
        public SaveStringActor(Thread parentThread, AtomicReference<Thread> otherThread) : base(parentThread,
            otherThread)
        {
        }
