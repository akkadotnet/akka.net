...
{
    private readonly string persistenceId = "p1";
        ...
        protected override bool ReceiveRecover(object message)
        {
            switch (message)
            {
                ...
                case RecoveryCompleted _:
                    if (IsRecovering)
                        throw new InvalidOperationException("Expected !IsRecovering in RecoveryCompleted");
                    _probe.Tell(RecoveryCompleted.Instance);
                    // Verify that persist can be used here
                    Persist(PersistFromRecoveryCompleted.Instance,
                        _ => _probe.Tell(PersistFromRecoveryCompleted.Instance));
                    return true;
            }
