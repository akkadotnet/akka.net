...
internal sealed class OldestChangedBuffer : UntypedActor
{
    ...
    /// <summary>
    ///     It's a delicate difference between <see cref="CoordinatedShutdown.PhaseClusterExiting" /> and
    ///     <see cref="ClusterEvent.MemberExited" />.
    ...
    private void HandleInitial(ClusterEvent.CurrentClusterState state)
    {
        ...
        var safeToBeOldest =
            !oldest.Any(m => m.Status is MemberStatus.Down or MemberStatus.Exiting or MemberStatus.Leaving);
    ...
    private void OnDeliverNext(object message)
    {
        ...
        else if (message is ClusterEvent.MemberExited exited &&
