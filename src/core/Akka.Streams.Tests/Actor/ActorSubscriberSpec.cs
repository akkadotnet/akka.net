...
public class ActorSubscriberSpec : AkkaSpec
{
    ...
    public async Task ActorSubscriberSpec_should_remember_requested_after_restart()
    {
        ...
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            actorRef.Tell("ready");
            actorRef.Tell("boom");
            foreach (var n in Enumerable.Range(3, 4)) (await ExpectMsgAsync<OnNext>()).Element.Should().Be(n);
            await ExpectNoMsgAsync(200);
            actorRef.Tell("ready");
    ...
    public async Task ActorSubscriberSpec_should_work_with_OneByOneRequestStrategy()
    {
        ...
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            Source.From(Enumerable.Range(1, 17))
                .RunWith(
                    Sink.ActorSubscriber<int>(RequestStrategySubscriber.Props(TestActor,
                        OneByOneRequestStrategy.Instance)), materializer);
            foreach (var n in Enumerable.Range(1, 17)) (await ExpectMsgAsync<OnNext>()).Element.Should().Be(n);
            await ExpectMsgAsync<OnComplete>();
        }, materializer);
    }
    ...
    public async Task ActorSubscriberSpec_should_should_work_with_WatermarkRequestStrategy()
    {
        ...
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            Source.From(Enumerable.Range(1, 17))
                .RunWith(
                    Sink.ActorSubscriber<int>(RequestStrategySubscriber.Props(TestActor,
                        new WatermarkRequestStrategy(10))), materializer);
            foreach (var n in Enumerable.Range(1, 17)) (await ExpectMsgAsync<OnNext>()).Element.Should().Be(n);
            await ExpectMsgAsync<OnComplete>();
        }, materializer);
    }
...
internal class ManualSubscriber : ActorSubscriber
{
    ...
    {
        return Akka.Actor.Props.Create(() => new ManualSubscriber(probe))
...
internal class ImmediatelyCancelledSubscriber : ManualSubscriber
{
    ...
    {
        return Akka.Actor.Props.Create(() => new ImmediatelyCancelledSubscriber(probe))
            .WithDispatcher("akka.test.stream-dispatcher");
    }
...
internal class RequestStrategySubscriber : ActorSubscriber
{
    ...
    {
        return Akka.Actor.Props.Create(() => new RequestStrategySubscriber(probe, strat))
            .WithDispatcher("akka.test.stream-dispatcher");
    }
