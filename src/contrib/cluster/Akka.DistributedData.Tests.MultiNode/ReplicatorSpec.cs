...
public class ReplicatorSpec : MultiNodeClusterSpec
{
    ...
    public void Cluster_CRDT_should_merge_the_update_with_existing_value()
    {
        RunOn(() =>
        {
            var update = new Update(KeyJ, new GSet<string>(), WriteLocal.Instance,
                x => ((GSet<string>)x).Add("a").Add("b"));
            _replicator.Tell(update);
    ...
    public void Cluster_CRDT_should_reply_with_ModifyFailure_if_exception_is_thrown_by_modify_function()
    {
        RunOn(() =>
        {
            ...
            Func<IReplicatedData, IReplicatedData> update = _ => { throw exception; };
    ...
    {
        var changedProbe = CreateTestProbe();
        RunOn(() => { _replicator.Tell(Dsl.Subscribe(KeyC, changedProbe.Ref)); }, _first, _second);
        ...
        RunOn(() =>
        {
            ...
            _replicator.Tell(Dsl.Update(KeyC, GCounter.Empty, WriteLocal.Instance, x => x.Increment(_cluster)));
            ExpectMsg(new UpdateSuccess(KeyC, null));
    ...
    public void Cluster_CRDT_should_converge_after_partition()
    {
        ...
        RunOn(() =>
        {
            ...
            _replicator.Tell(
                Dsl.Update(KeyD, GCounter.Empty.Increment(_cluster), _writeTwo, x => x.Increment(_cluster)));
            ExpectMsg(new UpdateTimeout(KeyD, null), _timeOut.Add(TimeSpan.FromSeconds(1)));
            _replicator.Tell(Dsl.Update(KeyD, GCounter.Empty, _writeTwo, x => x.Increment(_cluster)));
            ExpectMsg(new UpdateTimeout(KeyD, null), _timeOut.Add(TimeSpan.FromSeconds(1)));
        }, _first, _second);
            ...
            Within(TimeSpan.FromSeconds(10), () =>
                AwaitAssert(() =>
                {
                    for (ulong i = 1; i <= 30UL; i++)
                    {
                        ...
                        ExpectMsg<GetSuccess>(g => Equals(g.Key, keydn), TimeSpan.FromMilliseconds(50)).Get(keydn).Value
                            .ShouldBe(i);
                    }
                }));
        }, _first, _second);
    ...
    public void Cluster_CRDT_should_support_majority_quorum_write_and_read_with_3_nodes_with_1_unreachable()
    {
        ...
        RunOn(() =>
        {
            TestConductor.Blackhole(_first, _third, ThrottleTransportAdapter.Direction.Both)
                .Wait(TimeSpan.FromSeconds(5));
            TestConductor.Blackhole(_second, _third, ThrottleTransportAdapter.Direction.Both)
                .Wait(TimeSpan.FromSeconds(5));
        }, _first);
        ...
        RunOn(() =>
        {
            _replicator.Tell(Dsl.Update(KeyE, GCounter.Empty, WriteLocal.Instance, x => x.Increment(_cluster)));
            ExpectMsg(new UpdateSuccess(KeyE, null));
        }, _second);
        ...
        RunOn(() =>
        {
            ...
            _replicator.Tell(Dsl.Update(KeyE, GCounter.Empty, _writeMajority, data =>
            {
                ...
                return data.Increment(_cluster);
            }), probe2.Ref);
        ...
        RunOn(() =>
        {
            ...
            _replicator.Tell(Dsl.Update(KeyE, GCounter.Empty, _writeMajority, 153, x => x.Increment(_cluster)),
                probe1.Ref);

            // verify read your own writes, without waiting for the UpdateSuccess reply
            // note that the order of the replies are not defined, and therefore we use separate probes
            var probe2 = CreateTestProbe();
            _replicator.Tell(Dsl.Update(KeyE, GCounter.Empty, _writeMajority, 154, x => x.Increment(_cluster)),
                probe2.Ref);
            var probe3 = CreateTestProbe();
            _replicator.Tell(Dsl.Update(KeyE, GCounter.Empty, _writeMajority, 155, x => x.Increment(_cluster)),
                probe3.Ref);
        ...
        RunOn(() =>
        {
            ...
            TestConductor.PassThrough(_first, _third, ThrottleTransportAdapter.Direction.Both)
                .Wait(TimeSpan.FromSeconds(5));
            TestConductor.PassThrough(_second, _third, ThrottleTransportAdapter.Direction.Both)
                .Wait(TimeSpan.FromSeconds(5));
        ...
        Within(TimeSpan.FromSeconds(10), () =>
        {
            RunOn(() =>
            {
                ...
                for (ulong i = 0; i < 100UL; i++)
                {
                    ...
                    _replicator.Tell(Dsl.Update(KeyF, GCounter.Empty, _writeTwo, x => x.Increment(_cluster)));
                }
        ...
        Cluster_CRDT_should_check_that_remote_update_and_local_update_both_cause_a_change_event_to_emit_with_the_merged_data()
    {
        ...
        RunOn(() =>
        {
            ...
            _replicator.Tell(Dsl.Update(KeyH, ORDictionary<string, Flag>.Empty, _writeTwo,
                x => x.SetItem(_cluster, "a", Flag.False)));
            changedProbe.ExpectMsg<Changed>(g => Equals(g.Key, KeyH)).Get(KeyH).Entries
                .SequenceEqual(
                    ImmutableDictionary.CreateRange(new[] { new KeyValuePair<string, Flag>("a", Flag.False) }))
                .ShouldBeTrue();
        }, _second);
        ...
        RunOn(
            ...
            {
                _replicator.Tell(Dsl.Update(KeyH, ORDictionary<string, Flag>.Empty, _writeTwo,
                    x => x.SetItem(_cluster, "a", Flag.True)));
            }, _first);
        ...
        RunOn(() =>
        {
            changedProbe.ExpectMsg<Changed>(g => Equals(g.Key, KeyH)).Get(KeyH).Entries
                .SequenceEqual(
                    ImmutableDictionary.CreateRange(new[] { new KeyValuePair<string, Flag>("a", Flag.True) }))
                .ShouldBeTrue();

            _replicator.Tell(Dsl.Update(KeyH, ORDictionary<string, Flag>.Empty, _writeTwo,
                x => x.SetItem(_cluster, "b", Flag.True)));
            changedProbe.ExpectMsg<Changed>(g => Equals(g.Key, KeyH)).Get(KeyH).Entries.SequenceEqual(
                ImmutableDictionary.CreateRange(new[]
                {
                    new KeyValuePair<string, Flag>("a", Flag.True), new KeyValuePair<string, Flag>("b", Flag.True)
                })).ShouldBeTrue();
        }, _second);
    ...
    public void Cluster_CRDT_should_support_prefer_oldest_members()
    {
        ...
        var oldestReplicator = Sys.ActorOf(
            Replicator.Props(
                ReplicatorSettings.Create(Sys).WithPreferOldest(true)
                    .WithGossipInterval(TimeSpan.FromMinutes(1))), //.withDeltaCrdtEnabled(false)),
            "oldestReplicator");
        Within(TimeSpan.FromSeconds(5), () =>
        {
        ...
        RunOn(() =>
        {
            oldestReplicator.Tell(
                Dsl.Update(KeyK, new LWWRegister<string>(Cluster.SelfUniqueAddress, "0"), _writeTwo,
                    a => a.WithValue(Cluster.SelfUniqueAddress, "1")),
                probe.Ref);
