...
public abstract class RouterConfig : ISurrogated, IEquatable<RouterConfig>
{
    ...
    public bool Equals(RouterConfig other)
    {
        ...
        return GetType() == other.GetType() &&
               (GetType() == typeof(NoRouter) || string.Equals(RouterDispatcher, other.RouterDispatcher));
    }
...
public abstract class Group : RouterConfig, IEquatable<Group>
{
    ...
    {
        return InternalPaths?.GetHashCode() ?? 0;
    }
}
...
public abstract class Pool : RouterConfig, IEquatable<Pool>
{
    ...
    ///     Specify that this router should stop itself when all routees have terminated (been removed).
    /// </summary>
    public override bool StopRouterWhenAllRouteesRemoved => Resizer == null;
    ...
    public override int GetHashCode()
    {
        unchecked
        {
            var hashCode = Resizer?.GetHashCode() ?? 0;
...
public class FromConfig : Pool
{
    ...
    ///     This exception is automatically thrown since 'akka.actor.dispatch' is missing router configuration for
    ///     <paramref name="path" />.
    /// </exception>
    /// <returns>N/A</returns>
    public override void VerifyConfig(ActorPath path)
    {
        throw new ConfigurationException(
            $"Configuration missing for router [{path}] in 'akka.actor.deployment' section.");
    }
...
public class NoRouter : RouterConfig
{
    ...
    /// </exception>
    public override string RouterDispatcher => throw new NotSupportedException("NoRouter has no dispatcher");
