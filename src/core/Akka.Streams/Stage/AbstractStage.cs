...
#pragma warning restore CS0618 // Type or member is obsolete
        : base(shape)
    {
        ...
        SetHandler(_shape.Outlet,
            ...
            cause => _currentStage.OnDownstreamFinish(Context, cause));
    }
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public ITerminationDirective AbsorbTermination()
    {
        ...
        {
            var exception =
                new NotSupportedException("It is not allowed to call AbsorbTermination() from OnDownstreamFinish.");
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    {
        return $"PushPullGraphLogic({_currentStage})";
    }
}
...
public class
    PushPullGraphStageWithMaterializedValue<TIn, TOut, TMat> : GraphStageWithMaterializedValue<FlowShape<TIn, TOut>,
    TMat>
{
    /// <summary>
    ///     TBD
...
#pragma warning disable CS0618 // Type or member is obsolete
    public PushPullGraphStageWithMaterializedValue(Func<Attributes, (IStage<TIn, TOut>, TMat)> factory,
...
public class PushPullGraphStage<TIn, TOut> : PushPullGraphStageWithMaterializedValue<TIn, TOut, NotUsed>
{
    ...
    /// <returns>TBD</returns>
#pragma warning disable CS0618 // Type or member is obsolete
    public PushPullGraphStage(Func<Attributes, IStage<TIn, TOut>> factory, Attributes stageAttributes) : base(
        attributes => (factory(attributes), NotUsed.Instance), stageAttributes)
#pragma warning restore CS0618 // Type or member is obsolete
    {
    }
}
...
public abstract class AbstractStage<TIn, TOut> : IStage<TIn, TOut>
{
    ...
    ///         To emit additional elements before terminating you can use <see cref="IContext.AbsorbTermination" /> and push
    ///         final elements
...
[Obsolete("Please use GraphStage instead. [1.1.2]")]
public abstract class AbstractStage<TIn, TOut, TPushDirective, TPullDirective, TContext> : AbstractStage<TIn, TOut>
    where TPushDirective : IDirective where TPullDirective : IDirective where TContext : IContext
{
    /// <summary>
    ///     TBD
    /// </summary>
    ...
    ///     <para>
    ///         This method is called when an element from upstream is available and there is demand from downstream, i.e.
    ///         in <see cref="OnPush(TIn,TContext)" /> you are allowed to call <see cref="IContext.Push" /> to emit one element
    ///         downstreams,
    ///         or you can absorb the element by calling <see cref="IContext.Pull" />. Note that you can only
    ///         emit zero or one element downstream from <see cref="OnPull(TContext)" />.
    ///     </para>
    ///     <para>
    ///         To emit more than one element you have to push the remaining elements from <see cref="OnPull(TContext)" />,
    ///         one-by-one.
    ///         <see cref="OnPush(TIn,TContext)" /> is not called again until <see cref="OnPull(TContext)" /> has requested
    ///         more elements with
    ///         <see cref="IContext.Pull" />.
    ///     </para>
    /// </summary>
    ...
    {
        return OnDownstreamFinish((TContext)context, cause);
    }
    ...
    {
        return context.Finish(cause);
    }

    /// <summary>
    ///     <para>
    ///         <see cref="OnUpstreamFailure(System.Exception,Akka.Streams.Stage.IContext)" /> is called when upstream has
    ///         signaled that the stream is completed
    ///         with failure. It is not called if <see cref="OnPull(TContext)" /> or <see cref="OnPush(TIn,TContext)" /> of the
    ///         stage itself
    ///         throws an exception.
    ///     </para>
    ///     <para>
    ///         Note that elements that were emitted by upstream before the failure happened might
    ///         not have been received by this stage when
    ///         <see cref="OnUpstreamFailure(System.Exception,Akka.Streams.Stage.IContext)" /> is called, i.e.
    ///         failures are not backpressured and might be propagated as soon as possible.
    ///     </para>
    ///     <para>
    ///         Here you cannot call <see cref="IContext.Push" />, because there might not
