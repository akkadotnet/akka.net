...
public class Dispatcher : MessageDispatcher
{
    ...
    public Dispatcher(MessageDispatcherConfigurator configurator, string id, int throughput,
    ...
    protected override void ExecuteTask(IRunnable run)
    {
        ...
        catch (RejectedExecutionException e)
        {
            ...
            {
                EventStream.Publish(new Error(e, GetType().FullName, GetType(),
                    "Schedule(IRunnable) was rejected twice!"));
                throw;
            }
        }
    }
