...
public class ReceiveTimeoutSpec : AkkaSpec
{
    ...
    public void An_actor_with_receive_timeout_must_get_timeout()
    {
        ...
        var timeoutActor =
            Sys.ActorOf(Props.Create(() => new TimeoutActor(timeoutLatch, TimeSpan.FromMilliseconds(500))));
    ...
    [Fact]
    public void
        An_async_actor_with_receive_timeout_must_get_timeout_while_receiving_NotInfluenceReceiveTimeout_messages()
    {
        var timeoutLatch = new TestLatch();
        var timeoutActor =
            Sys.ActorOf(Props.Create(() => new AsyncTimeoutActor(timeoutLatch, TimeSpan.FromSeconds(1))));
    ...
    [Fact]
    public void
        An_actor_with_receive_timeout_must_get_timeout_while_receiving_only_NotInfluenceReceiveTimeout_messages()
    {
    ...
    [Fact]
    public void
        An_actor_with_receive_timeout_must_be_able_to_turn_on_timeout_in_NotInfluenceReceiveTimeout_message_handler()
    {
        var timeoutLatch = new TestLatch();
    ...
    [Fact]
    public void
        An_actor_with_receive_timeout_must_be_able_to_turn_off_timeout_in_NotInfluenceReceiveTimeout_message_handler()
    {
        var timeoutLatch = new TestLatch();
    ...
    public class TimeoutActor : ActorBase
    {
        ...
        protected override bool Receive(object message)
        {
            ...
            if (message is Tick) return true;

            if (message is TransparentTick) return true;
...
#pragma warning disable AK1003
            ...
            ReceiveAsync<TransparentTick>(async _ => { log.Info($"Received {nameof(TransparentTick)}"); });

            ReceiveAsync<Tick>(async _ => { log.Info($"Received {nameof(Tick)}"); });
