...
public readonly struct IOResult
{
    ...
    ///     Is thrown if the property is accessed for a successful <see cref="IOResult" />
    /// </exception>
    ...
    {
        return new IOResult(count, Result.Success(NotUsed.Instance));
    }

    /// <summary>
    ///     Creates failed IOResult, <paramref name="count" /> should be the number of bytes (or other unit, please document in
    ///     your APIs) processed before failing
    /// </summary>
    /// <param name="count">Numeric value depending on context, for example IO operations performed or bytes processed.</param>
    ...
    {
        return new IOResult(count, Result.Failure<NotUsed>(reason));
    }
}

/// <summary>
///     This exception signals that a stream has been completed by an onError signal while there was still IO operations in
///     progress.
/// </summary>
public sealed class AbruptIOTerminationException : Exception
{
    /// <summary>
    ///     Initializes a new instance of the <see cref="AbruptIOTerminationException" /> class with the result of the IO
    ///     operation
