...
public class NodeChurnSpec : MultiNodeClusterSpec
{
    ...
    private ImmutableList<Address> SeedNodes => ImmutableList.Create(GetAddress(_config.First),
        GetAddress(_config.Second), GetAddress(_config.Third));
    ...
    public void Cluster_with_short_lived_members_must_join_and_remove_transient_nodes_without_growing_gossip_payload()
    {
        ...
        {
            Log.Info("round-" + n);
            var systems = Enumerable.Repeat(0, 2).Select(_ => ActorSystem.Create(Sys.Name, Sys.Settings.Config))
                .ToImmutableList();
            ...
            foreach (var node in systems) Shutdown(node, verifySystemShutdown: true);
        ...
        Within(20.Seconds(), () =>
        {
            AwaitAssert(() =>
            {
                additionalSystems.ForEach(s =>
                {
                    ...
                    cluster.State.Members.All(c => c.Status == MemberStatus.Up).Should()
                        .BeTrue("All members should be up.");
                });
            });
        });
    }
