...
///     The subscribers are IActorRef instances and events can be any object. Subscriptions are hierarchical meaning that
///     if you listen to
    ...
    public void StartUnsubscriber(ActorSystemImpl system)
    {
        if (_unsubscriber.Value.IsNobody())
            lock (this)
            {
                ...
                var unsubscriber = system.SystemActorOf(
                    Props.Create<EventStreamUnsubscriber>(this, system, _debug)
                ...
                if (_unsubscriber.CompareAndSet(currentValue, unsubscriber))
                {
                    ...
                    foreach (var s in _pendingUnsubscribers) unsubscriber.Tell(new EventStreamUnsubscriber.Register(s));
