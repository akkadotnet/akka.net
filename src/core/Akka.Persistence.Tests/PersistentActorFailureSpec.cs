...
public class PersistentActorFailureSpec : PersistenceSpec
{
    ...
    internal class FailingMemoryJournal : MemoryJournal
    {
        protected override Task<IImmutableList<Exception>> WriteMessagesAsync(IEnumerable<AtomicWrite> messages)
        {
            ...
            if (checkSerializable.Any(cs => cs != null)) return Task.FromResult(checkSerializable);
            ...
            Action<IPersistentRepresentation> recoveryCallback)
        {
            ...
            if (IsCorrupt(readFromStore))
            {
                ...
                promise.SetException(
                    new SimulatedException(string.Format("blahonga {0} {1}", fromSequenceNr, toSequenceNr)));
                return promise.Task;
            }
    ...
    internal class FailingRecovery : PersistentActorSpec.ExamplePersistentActor
    {
        ...
        protected override bool ReceiveCommand(object message)
        {
            ...
            {
                var cmd = message as Cmd;
                if (cmd != null) Persist(new Evt(cmd.Data), evt => UpdateState(evt));
            }
            else
            {
                return false;
        ...
        {
            if (!FailingRecover(message)) return base.ReceiveRecover(message);
            return true;
        }
    }
