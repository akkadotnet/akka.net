...
public class DistributedPubSubRestartSpecConfig : MultiNodeConfig
{
    ...
    internal class Shutdown : ReceiveActor
    {
        public Shutdown()
        {
            Receive<string>(str => str.Equals("shutdown"), _ => { Context.System.Terminate(); });
        }
    }
}
...
public class DistributedPubSubRestartSpec : MultiNodeClusterSpec
{
    ...
    protected DistributedPubSubRestartSpec(DistributedPubSubRestartSpecConfig config) : base(config,
        typeof(DistributedPubSubRestartSpec))
    {
        _config = config;
    }
    ...
    private IActorRef Mediator => DistributedPubSub.Get(Sys).Mediator;
    ...
    public void A_Cluster_with_DistributedPubSub_must_handle_restart_of_nodes_with_same_address()
    {
        Within(30.Seconds(), () =>
        {
            ...
            RunOn(() => { Mediator.Tell(new Publish("topic1", "msg1")); }, _config.First);
            ...
            RunOn(() =>
            {
                ...
                Within(20.Seconds(), () =>
                {
                    AwaitAssert(() =>
                    {
                        Sys.ActorSelection(new RootActorPath(thirdAddress) / "user" / "shutdown")
