...
///     producers. It consists of two parts, a <see cref="Source{TOut,TMat}" /> and a <see cref="Sink{TIn,TMat}" />. The
///     <see cref="Source{TOut,TMat}" /> streams the element to a consumer from
///     its merged inputs. Once the consumer has been materialized, the <see cref="Source{TOut,TMat}" /> returns a
///     materialized value which is
///     the corresponding <see cref="Sink{TIn,TMat}" />. This <see cref="Sink{TIn,TMat}" /> can then be materialized
///     arbitrary many times, where each of the new
...
public static class MergeHub
{
    ...
    /// <summary>
    ///     Creates a <see cref="Source{TOut,TMat}" /> that emits elements merged from a dynamic set of producers. After the
    ///     <see cref="Source{TOut,TMat}" /> returned
    ///     by this method is materialized, it returns a <see cref="Sink{TIn,TMat}" /> as a materialized value. This
    ///     <see cref="Sink{TIn,TMat}" /> can be materialized
    ///     arbitrary many times and each of the materializations will feed the elements into the original
    ///     <see cref="Source{TOut,TMat}" />.
    ///     Every new materialization of the <see cref="Source{TOut,TMat}" /> results in a new, independent hub, which
    ///     materializes to its own
    ///     <see cref="Sink{TIn,TMat}" /> for feeding that materialization.
    ///     If one of the inputs fails the <see cref="Sink{TIn,TMat}" />, the <see cref="Source{TOut,TMat}" /> is failed in
    ///     turn (possibly jumping over already buffered
    ///     elements). Completed <see cref="Sink{TIn,TMat}" />s are simply removed. Once the <see cref="Source{TOut,TMat}" />
    ...
    {
        return Dsl.Source.FromGraph(new MergeHub<T>(perProducerBufferSize));
    }
...
internal sealed class MergeHub<T> : GraphStageWithMaterializedValue<SourceShape<T>, Sink<T, NotUsed>>
{
    ...
    public MergeHub(int perProducerBufferSize)
    {
        if (perProducerBufferSize <= 0)
            throw new ArgumentOutOfRangeException(nameof(perProducerBufferSize), perProducerBufferSize,
...
/// <summary>
///     A BroadcastHub is a special streaming hub that is able to broadcast streamed elements to a dynamic set of
///     consumers.
///     It consists of two parts, a <see cref="Sink{TIn,TMat}" /> and a <see cref="Source{TOut,TMat}" />. The
///     <see cref="Sink{TIn,TMat}" /> broadcasts elements from a producer to the
///     actually live consumers it has. Once the producer has been materialized, the <see cref="Sink{TIn,TMat}" /> it feeds
///     into returns a
///     materialized value which is the corresponding <see cref="Source{TOut,TMat}" />. This
///     <see cref="Source{TOut,TMat}" /> can be materialized arbitrary many times,
///     where each of the new materializations will receive their elements from the original <see cref="Sink{TIn,TMat}" />.
/// </summary>
public class BroadcastHub
{
    /// <summary>
    ///     Creates a <see cref="Sink{TIn,TMat}" /> that receives elements from its upstream producer and broadcasts them to a
    ///     dynamic set
    ///     of consumers. After the <see cref="Sink{TIn,TMat}" /> returned by this method is materialized, it returns a
    ///     <see cref="Source{TOut,TMat}" /> as materialized
    ///     value. This <see cref="Source{TOut,TMat}" /> can be materialized arbitrary many times and each materialization will
    ///     receive the
    ///     broadcast elements from the original <see cref="Sink{TIn,TMat}" />.
    ///     Every new materialization of the <see cref="Sink{TIn,TMat}" /> results in a new, independent hub, which
    ///     materializes to its own
    ///     <see cref="Source{TOut,TMat}" /> for consuming the <see cref="Sink{TIn,TMat}" /> of that materialization.
    ///     If the original <see cref="Sink{TIn,TMat}" /> is failed, then the failure is immediately propagated to all of its
    ///     materialized
    ///     <see cref="Source{TOut,TMat}" />s (possibly jumping over already buffered elements). If the original
    ///     <see cref="Sink{TIn,TMat}" /> is completed, then
    ///     all corresponding <see cref="Source{TOut,TMat}" />s are completed. Both failure and normal completion is
    ///     "remembered" and later
    ///     materializations of the <see cref="Source{TOut,TMat}" /> will see the same (failure or completion) state.
    ///     <see cref="Source{TOut,TMat}" />s that are
    ...
    {
        return Dsl.Sink.FromGraph(new BroadcastHub<T>(bufferSize));
    }
...
internal sealed class BroadcastHub<T> : GraphStageWithMaterializedValue<SinkShape<T>, Source<T, NotUsed>>
{
    ...
    private sealed class HubLogic : InGraphStageLogic
    {
        ...
        ///     Consumer API
        /// </summary>
        public object Poll(int offset)
        {
            return offset == _tail ? null : _queue[offset & _stage._mask];
        }
    }
    ...
    private sealed class HubSourceLogic : GraphStage<SourceShape<T>>
    {
        ...
        protected override GraphStageLogic CreateLogic(Attributes inheritedAttributes)
        {
            return new Logic(this, _counter.IncrementAndGet());
        }
...
/// <summary>
///     A <see cref="PartitionHub" /> is a special streaming hub that is able to route streamed elements to a dynamic set
///     of consumers.
///     It consists of two parts, a <see cref="Sink{TIn,TMat}" /> and a <see cref="Source{TOut,TMat}" />. The
///     <see cref="Sink{TIn,TMat}" /> e elements from a producer to the
///     actually live consumers it has.The selection of consumer is done with a function. Each element can be routed to
///     only one consumer.Once the producer has been materialized, the <see cref="Sink{TIn,TMat}" /> it feeds into returns
///     a
///     materialized value which is the corresponding <see cref="Source{TOut,TMat}" />. This
///     <see cref="Source{TOut,TMat}" /> can be materialized an arbitrary number
///     of times, where each of the new materializations will receive their elements from the original
...
public static class PartitionHub
{
    ...
    /// <summary>
    ///     Creates a <see cref="Sink{TIn,TMat}" /> that receives elements from its upstream producer and routes them to a
    ///     dynamic set
    ///     of consumers.After the <see cref="Sink{TIn,TMat}" /> returned by this method is materialized, it returns a
    ///     <see cref="Source{TOut,TMat}" /> as materialized  value.
    ///     This <see cref="Source{TOut,TMat}" /> can be materialized an arbitrary number of times and each materialization
    ///     will receive the
    ...
    ///     cancelled are simply removed from the dynamic set of consumers.
    ///     <para />
    ///     This <see cref="StatefulSink{T}" /> should be used when there is a need to keep mutable state in the partition
    ///     function,
    ...
    ///     This is only used initially when the stage is starting up, i.e.it is not honored when consumers have been removed
    ///     (canceled).
    /// </param>
    /// <param name="bufferSize">
    ///     Total number of elements that can be buffered. If this buffer is full, the producer is
    ///     backpressured.
    /// </param>
    [ApiMayChange]
    ...
    ///     cancelled are simply removed from the dynamic set of consumers.
    ///     <para />
    ///     This <see cref="Sink{T}" /> should be used when the routing function is stateless, e.g. based on a hashed value of
    ///     the
    ...
    public interface IConsumerInfo
    {
        ...
        ///     When selecting a specific consumerId by its index, prefer using the dedicated <see cref="ConsumerByIndex" /> method
        ///     instead,
...
internal sealed class PartitionHub<T> : GraphStageWithMaterializedValue<SinkShape<T>, Source<T, NotUsed>>
{
    ...
    private sealed class PartitionSinkLogic : InGraphStageLogic
    {
        ...
        private sealed class ConsumerInfo : PartitionHub.IConsumerInfo
        {
            ...
            public long ConsumerByIndex(int index)
            {
                return Consumers[index].Id;
            }

            public int QueueSize(long consumerId)
            {
                return _partitionSinkLogic._queue.Size(consumerId);
            }
    ...
    private sealed class PartitionSource : GraphStage<SourceShape<T>>
    {
        ...
        private sealed class Logic : OutGraphStageLogic
        {
            ...
            public override void PostStop()
            {
                _hubCallback?.Invoke(new UnRegister(_id));
            }
    ...
    private sealed class ConsumerQueue
    {
        ...
        public ConsumerQueue Enqueue(object element)
        {
            return new ConsumerQueue(_queue.Enqueue(element), Size + 1);
        }
    }
    ...
    private sealed class PartitionQueue : IPartitionQueue
    {
        ...
        {
            _queues.TryAdd(id, ConsumerQueue.Empty);
        }
