...
public class ClusterEvent
{
    ...
    {
        if (ReferenceEquals(newState, oldState)) return ImmutableList<UnreachableMember>.Empty;
    ...
    {
        if (ReferenceEquals(newState, oldState)) return ImmutableList<ReachableMember>.Empty;
    ...
    {
        if (ReferenceEquals(newState, oldState)) return ImmutableList<IMemberEvent>.Empty;
        ...
        var removedEvents = removedMembers.Select(m => new MemberRemoved(m.Copy(MemberStatus.Removed), m.Status));
    ...
    ///     INTERNAL API
    /// </summary>
    internal static ImmutableHashSet<RoleLeaderChanged> DiffRolesLeader(MembershipState oldState,
        MembershipState newState)
    {
        return InternalDiffRolesLeader(oldState, newState).ToImmutableHashSet();
    }

    private static IEnumerable<RoleLeaderChanged> InternalDiffRolesLeader(MembershipState oldState,
        MembershipState newState)
    {
    ...
    {
        if (ReferenceEquals(newState, oldState)) return ImmutableList<SeenChanged>.Empty;

        var newConvergence = newState.Convergence(ImmutableHashSet<UniqueAddress>.Empty);
        var newSeenBy = newState.LatestGossip.SeenBy;
        if (!newConvergence.Equals(oldState.Convergence(ImmutableHashSet<UniqueAddress>.Empty)) ||
            !newSeenBy.SequenceEqual(oldState.LatestGossip.SeenBy))
    ...
    ///     INTERNAL API
    /// </summary>
    internal static ImmutableList<ReachabilityChanged> DiffReachability(MembershipState oldState,
        MembershipState newState)
    {
    ...
    public sealed class CurrentClusterState : INoSerializationVerificationNeeded
    {
        ...
        {
            return Members.Any(m => m.Address.Equals(address) && m.Status == MemberStatus.Up);
        }
    ...
    public abstract class MemberStatusChange : IMemberEvent
    {
        ...
        ///     This exception is thrown if the node's current status doesn't match the given status,
        ///     <paramref name="validStatus" />.
        /// </exception>
    ...
    ///     both <see cref="Akka.Cluster.ClusterEvent.UnreachableMember" /> and
    ///     <see cref="Akka.Cluster.ClusterEvent.ReachableMember" />.
    /// </summary>
    ...
    internal sealed class SeenChanged : IClusterDomainEvent
    {
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                ...
                foreach (var t in SeenBy) hash = hash * 23 + t.GetHashCode();
...
internal sealed class ClusterDomainEventPublisher : ReceiveActor, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
{
    ...
    private void SendCurrentClusterState(IActorRef receiver)
    {
        ...
        var state = new ClusterEvent.CurrentClusterState(
            _membershipState.Members,
            unreachable,
            _membershipState.LatestGossip.SeenBy.Select(s => s.Address).ToImmutableHashSet(),
            _membershipState.Leader?.Address,
            _membershipState.LatestGossip.AllRoles
