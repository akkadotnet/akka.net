...
{
    private static readonly ImmutableHashSet<MemberStatus> JoiningCluster =
        ImmutableHashSet.Create(MemberStatus.Joining, MemberStatus.WeaklyUp);

    private static readonly ImmutableHashSet<MemberStatus> LeavingClusterStatuses =
        ImmutableHashSet.Create(MemberStatus.Leaving, MemberStatus.Exiting, MemberStatus.Down);
    ...
    public abstract Task<IImmutableSet<string>> Rebalance(
        IImmutableDictionary<IActorRef, IImmutableList<string>> currentShardAllocations,
    ...
    protected ImmutableList<RegionEntry> RegionEntriesFor(
        IImmutableDictionary<IActorRef, IImmutableList<string>> currentShardAllocations)
    {
    ...
    internal sealed class ShardSuitabilityOrdering : IComparer<RegionEntry>
    {
        ...
        public int Compare(RegionEntry x, RegionEntry y)
        {
            ...
            if (x.Member.AppVersion != y.Member.AppVersion)
