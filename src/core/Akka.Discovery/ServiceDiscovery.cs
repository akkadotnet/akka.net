...
public abstract class ServiceDiscovery
{
    ...
    /// <param name="resolveTimeout">Timeout. Up to the discovery-method to adhere to this</param>
    public Task<Resolved> Lookup(string serviceName, TimeSpan resolveTimeout)
    {
        return Lookup(new Lookup(serviceName), resolveTimeout);
    }
    ...
    public class Resolved : IDeadLetterSuppression, INoSerializationVerificationNeeded, IEquatable<Resolved>
    {
        ...
        {
            return $"Resolved({ServiceName}, {string.Join(", ", Addresses)})";
        }
    ...
    public class ResolvedTarget : INoSerializationVerificationNeeded, IEquatable<ResolvedTarget>
    {
        ...
        public override string ToString()
        {
            return $"ResolvedTarget({Host}, {Port}, {Address})";
        }
...
public class Lookup : INoSerializationVerificationNeeded, IEquatable<Lookup>
{
    ...
    private static readonly Regex domainNameRegex = new(
        @"^((?![0-9-])[A-Za-z0-9-]{1,63}(?<!-))((\.(?![0-9-])[A-Za-z0-9-]{1,63}(?<!-)))*$",
        RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.IgnoreCase);

    /// <summary>
    ///     Create a service Lookup with <paramref name="serviceName" />, optional <paramref name="portName" /> and optional
    ///     <paramref name="protocol" />.
    ///     Use <see cref="WithPortName" /> and <see cref="WithProtocol" /> to provide optional <paramref name="portName" />
    ///     and <paramref name="protocol" />.
    /// </summary>
    ...
    public Lookup(string serviceName, string portName = null, string protocol = null)
    {
        if (string.IsNullOrEmpty(serviceName))
            throw new ArgumentNullException(nameof(serviceName), "cannot be null or empty");
    ...
    public static Lookup ParseSrv(string srv)
    {
        if (srv == null)
            throw new ArgumentNullException(nameof(srv),
                "Unable to create Lookup from passed SRV string. Passed value is 'null'");
    ...
    public override string ToString()
    {
        return $"Lookup({ServiceName}, {PortName}, {Protocol})";
    }

    public Lookup Copy(string serviceName = null, string portName = null, string protocol = null)
    {
        return new Lookup(serviceName ?? ServiceName, portName ?? PortName, protocol ?? Protocol);
    }
