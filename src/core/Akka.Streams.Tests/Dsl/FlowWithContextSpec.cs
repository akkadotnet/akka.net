...
public class FlowWithContextSpec : AkkaSpec
{
    ...
    {
        var flow = Flow.Create<Message>().Select(m => m.Copy(m.Data + "z"));
        var flowWithContext =
            flow.AsFlowWithContext<Message, long, Message, long, NotUsed, Message>((m, o) => new Message(m.Data, o),
                m => m.Offset);
    ...
    public void A_FlowWithContext_must_be_able_to_map_materialized_value_via_FlowWithContext_MapMaterializedValue()
    {
        ...
        var mapMaterializedValueFlow =
            FlowWithContext.Create<Message, long>().MapMaterializedValue(_ => materializedValue);
...
internal sealed class Message : IEquatable<Message>
{
    ...
    public Message Copy(string data = null, long? offset = null)
    {
        return new Message(data ?? Data, offset ?? Offset);
    }
