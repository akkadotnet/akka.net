...
public sealed class ClusterClientSettings : INoSerializationVerificationNeeded
{
    ...
    {
        if (bufferSize is < 0 or > 10000) throw new ArgumentException("BufferSize must be >= 0 and <= 10000");
    ...
    /// <summary>
    ///     Actor paths of the <see cref="ClusterReceptionist" /> actors on the servers (cluster nodes) that the client will
    ///     try to contact initially.
    /// </summary>
    public IImmutableSet<ActorPath> InitialContacts { get; }

    /// <summary>
    ///     Interval at which the client retries to establish contact with one of ClusterReceptionist on the servers (cluster
    ///     nodes)
    /// </summary>
    public TimeSpan EstablishingGetContactsInterval { get; }

    /// <summary>
    ///     Interval at which the client will ask the <see cref="ClusterReceptionist" /> for new contact points to be used for
    ///     next reconnect.
    /// </summary>
    ...
    ///     them the connection is established. When the buffer is full old messages will be dropped when new messages are sent
    ///     via the client.
    ///     Use 0 to disable buffering, i.e. messages will be dropped immediately if the location of the receptionist is
    ///     unavailable.
    /// </summary>
    ...
    ///     serializer.
    ///     If set to false, will cause all ClusterClient message to be serialized using
    ///     <see cref="ClusterClientMessageSerializer" />
    /// </summary>
            ...
            throw
                ConfigurationException
                    .NullOrEmptyConfig<
                        ...
                        "akka.cluster.client"); //($"Failed to create {nameof(ClusterClientSettings)}: Actor system [{system.Name}] doesn't have `akka.cluster.client` config set up");
    ...
    public static ClusterClientSettings Create(Config config)
    {
        ...
        var initialContacts = config.GetStringList("initial-contacts", new string[] { }).Select(ActorPath.Parse)
    ...
    {
        if (initialContacts.Count == 0) throw new ArgumentException("InitialContacts must be defined");

        return Copy(initialContacts);
    }
    ...
    {
        return Copy(useLegacySerialization: useLegacySerialization);
    }
    ...
    public ClusterClientSettings WithInitialContactsDiscovery(
        ...
        ClusterClientDiscoverySettings? discoverySettings = null)
    {
        return Copy(useInitialContactsDiscovery: useInitialContactsDiscovery, discoverySettings: discoverySettings);
    }
