    ...
    class ClassReferenceCounter : Rewriter
    {
        public override async Task<Document> Rewrite(Document document)
        {
            ...
            foreach (var @class in classes)
            {
                ...
                if (semanticClass.Name.Contains("Tests") || semanticClass.BaseType != null && semanticClass.BaseType.Name.Contains("Tests"))
                    continue;

                if (semanticClass.ContainingNamespace.Name.Contains("Tests") || semanticClass.ContainingNamespace.Name.Contains("Spec"))
                    continue;
