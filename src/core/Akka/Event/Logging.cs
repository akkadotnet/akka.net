...
///     * providing a <see cref="Type" /> will extract its simple name,  append "(ActorSystem address)", and use the
///     result.
/// </summary>
...
/// <summary>
///     This class provides the functionality for creating logger instances and helpers for converting to/from
...
public static class Logging
{
    ...
    ///     is being booted, and therefore can cause problems similar to https://github.com/akkadotnet/akka.net/issues/4677 at
    ///     startup.
    /// </summary>
    ...
    internal static ILoggingAdapter GetLoggerStartup(this IActorContext context,
        ILogMessageFormatter logMessageFormatter = null)
    {
        ...
        catch // had a failure, don't want to propagate it. Just start the logger without remote context
        {
            ...
            return new BusLogging(context.System.EventStream, logSource.Source, logSource.Type,
                logMessageFormatter ?? context.System.Settings.LogFormatter);
        }
    }
    ...
    /// <param name="logMessageFormatter">The formatter used to format log messages.</param>
    /// <returns>The newly created logging adapter.</returns>
    public static ILoggingAdapter GetLogger(ActorSystem system, object logSourceObj,
        ILogMessageFormatter logMessageFormatter = null)
    {
        var logSource = LogSource.Create(logSourceObj, system);
        return new BusLogging(system.EventStream, logSource.Source, logSource.Type,
            logMessageFormatter ?? system.Settings.LogFormatter);
    }
    ...
    /// <param name="logMessageFormatter">The formatter used to format log messages.</param>
    /// <returns>The newly created logging adapter.</returns>
    public static ILoggingAdapter GetLogger(LoggingBus loggingBus, object logSourceObj,
        ILogMessageFormatter logMessageFormatter = null)
    {
        var logSource = LogSource.Create(logSourceObj);
        return new BusLogging(loggingBus, logSource.Source, logSource.Type,
            logMessageFormatter ?? DefaultLogMessageFormatter.Instance);
    }
    ...
    {
        if (!string.IsNullOrEmpty(logLevel)) logLevel = logLevel.ToUpperInvariant();
        ...
        switch (logLevel)
        {
            ...
            default:
                throw new ArgumentException(
                    $@"Unknown LogLevel: ""{logLevel}"". Valid values are: ""{Debug}"", ""{Info}"", ""{Warning}"", ""{Error}""",
                    nameof(logLevel));
        }
    }
    ...
    ///     The exception is thrown if the given
    ///     <typeparamref name="T">log event</typeparamref> is unknown.
    /// </exception>
    ...
    public static LogLevel LogLevelFor<T>() where T : LogEvent
    {
        ...
        throw new ArgumentException(
            $@"Unknown LogEvent type: ""{type.FullName}"". Valid types are: ""{typeof(Debug).FullName}"", ""{typeof(Info).FullName}"", ""{typeof(Warning).FullName}"", ""{typeof(Error).FullName}""");
    }
}