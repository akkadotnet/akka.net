...
public class FSMTransitionSpec : AkkaSpec
{
    ...
    public class SendAnyTransitionFSM : FSM<int, int>
    {
        public SendAnyTransitionFSM(IActorRef target)
        {
            ...
            OnTransition((state1, state2) => { Target.Tell((state1, state2)); });
    ...
    public class OtherFSM : FSM<int, int>
    {
        public OtherFSM(IActorRef target)
        {
            ...
            When(0, @event =>
            {
                ...
                if (@event.FsmEvent.Equals("stay")) return Stay();
                return null;
            });
    ...
    public class LeakyFSM : FSM<int, IActorRef>
    {
        public LeakyFSM()
        {
            ...
            When(0, @event =>
            {
                if (@event.FsmEvent.Equals("switch")) return GoTo(1).Using(Sender);

                return null;
            });

            OnTransition((state1, state2) => { NextStateData.Tell((state1, state2)); });
