...
///         <see cref="DistributedPubSubMediator" />. Messages from the client are wrapped in <see cref="Send" />,
///         <see cref="SendToAll" />
...
public sealed class ClusterReceptionist : ActorBase
{
    ...
    public ClusterReceptionist(IActorRef pubSubMediator, ClusterReceptionistSettings settings)
    {
        ...
        if (!(_settings.Role == null || _cluster.SelfRoles.Contains(_settings.Role)))
            throw new ArgumentException(
                $"This cluster member [{_cluster.SelfAddress}] does not have a role [{_settings.Role}]");
    ...
    protected override void PostStop()
    {
        ...
        foreach (var c in _clientInteractions.Keys) c.Tell(ReceptionistShutdown.Instance);
    }
    ...
    private IActorRef ResponseTunnel(IActorRef client)
    {
        ...
        return !child.Equals(ActorRefs.Nobody)
            ? child
            : Context.ActorOf(
                Actor.Props.Create(() => new ClientResponseTunnel(client, _settings.ResponseTunnelReceiveTimeout)),
                encName);
    }
    ...
    protected override bool Receive(object message)
    {
        switch (message)
        {
            ...
            case Heartbeat:
            {
                if (_cluster.Settings.VerboseHeartbeatLogging) _log.Debug("Heartbeat from client [{0}]", Sender.Path);
            ...
            case GetContacts when _settings.NumberOfContacts >= _nodes.Count:
            {
                ...
                if (_log.IsDebugEnabled)
                    _log.Debug("Client [{0}] gets contactPoints [{1}] (all nodes)", Sender.Path,
            ...
            case GetContacts:
            {
                ...
                if (_log.IsDebugEnabled)
                    _log.Debug("Client [{0}] gets ContactPoints [{1}]", Sender.Path,
                        string.Join(", ", contacts.ContactPoints));
    ...
    private void UpdateClientInteractions(IActorRef client)
    {
        ...
        {
            failureDetector =
                new DeadlineFailureDetector(_settings.AcceptableHeartbeatPause, _settings.HeartbeatInterval);
            failureDetector.HeartBeat();
    ...
    internal sealed class Contacts : IClusterClientMessage
    {
        ...
        public override int GetHashCode()
        {
            ...
            {
                var hash = 19;
                foreach (var foo in ContactPoints) hash = hash * 31 + foo.GetHashCode();
                return hash;
            }
        }
    }
...
/// <summary>
///     Replies are tunneled via this actor, child of the receptionist, to avoid inbound connections from other cluster
///     nodes to the client.
/// </summary>
