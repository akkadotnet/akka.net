...
internal sealed class Write : IReplicatorMessage, IEquatable<Write>, ISendingSystemUid
{
    ...
    {
        return obj is Write write && Equals(write);
    }
    ...
    public override string ToString()
    {
        return $"Write(key={Key}, envelope={Envelope})";
    }
}
...
internal sealed class WriteNack : IReplicatorMessage, IEquatable<WriteNack>
{
    ...
    /// <returns>TBD</returns>
    public bool Equals(WriteNack other)
    {
        return true;
    }
...
internal sealed class Read : IReplicatorMessage, IEquatable<Read>, ISendingSystemUid
{
    ...
    public override bool Equals(object obj)
    {
        return obj is Read read && Equals(read);
    }
...
internal sealed class ReadResult : IReplicatorMessage, IEquatable<ReadResult>, IDeadLetterSuppression
{
    ...
    public override bool Equals(object obj)
    {
        return obj is ReadResult result && Equals(result);
    }

    /// <inheritdoc />
    public override int GetHashCode()
    {
        return Envelope?.GetHashCode() ?? 0;
    }

    /// <inheritdoc />
    public override string ToString()
    {
        return $"ReadResult(envelope={Envelope})";
    }
}
...
internal sealed class ReadRepair : IEquatable<ReadRepair>
{
    ...
    public override bool Equals(object obj)
    {
        return obj is ReadRepair repair && Equals(repair);
    }
    ...
    public override string ToString()
    {
        return $"ReadRepair(key={Key}, envelope={Envelope})";
    }
}
...
public sealed class DataEnvelope : IEquatable<DataEnvelope>, IReplicatorMessage
{
    ...
    internal DataEnvelope(IReplicatedData data, ImmutableDictionary<UniqueAddress, IPruningState> pruning = null,
        VersionVector deltaVersions = null)
    {
        Data = data;
    ...
    {
        return new DataEnvelope(data, Pruning, DeltaVersions);
    }
    ...
    /// <returns></returns>
    private VersionVector CleanedDeltaVersions(UniqueAddress from)
    {
        return DeltaVersions.PruningCleanup(from);
    }
    ...
    internal DataEnvelope Prune(UniqueAddress from, PruningPerformed pruningPerformed)
    {
        if (Data is IRemovedNodePruning dataWithRemovedNodePruning)
        {
            ...
            if (state is PruningInitialized initialized)
            {
                ...
                return new DataEnvelope(prunedData, Pruning.SetItem(from, pruningPerformed),
                    CleanedDeltaVersions(from));
            }
        }
    ...
    internal DataEnvelope Merge(DataEnvelope other)
    {
        ...
        var cleanedOtherDeltaVersions =
            removedNodes.Aggregate(other.DeltaVersions, (acc, node) => acc.PruningCleanup(node));
        ...
        return new DataEnvelope(
                Cleaned(Data, filteredMergedPruning),
                filteredMergedPruning,
                mergedDeltaVersions)
            .Merge(other.Data);
    }
        ...
        if (cleanedData is IReplicatedDelta d)
        {
            var delta = Data as IDeltaReplicatedData ??
                        throw new ArgumentException(
                            $"Expected {nameof(IDeltaReplicatedData)} but got '{Data}' instead.");
    ...
    private IReplicatedData Cleaned(IReplicatedData c, IImmutableDictionary<UniqueAddress, IPruningState> p)
    {
        return p.Aggregate(c, (_, kvp) =>
        {
    ...
    public override bool Equals(object obj)
    {
        return obj is DataEnvelope envelope && Equals(envelope);
    }
    ...
    public override int GetHashCode()
    {
        unchecked
        {
            ...
            foreach (var p in Pruning) seed *= p.Key.GetHashCode() ^ p.Value.GetHashCode();
...
internal sealed class DeletedData : IReplicatedData<DeletedData>, IEquatable<DeletedData>, IReplicatedDataSerialization
{
    ...
    public DeletedData Merge(DeletedData other)
    {
        return this;
    }
    ...
    {
        return Merge((DeletedData)other);
    }
...
internal sealed class Status : IReplicatorMessage, IEquatable<Status>, IDestinationSystemUid
{
    ...
    public Status(IImmutableDictionary<string, ByteString> digests, int chunk, int totalChunks,
        long? toSystemUid = null, long? fromSystemUid = null)
    {
        Digests = digests;
        Chunk = chunk;
...
internal sealed class Gossip : IReplicatorMessage, IEquatable<Gossip>, IDestinationSystemUid
{
    ...
    /// <param name="toSystemUid">TBD</param>
    /// <param name="fromSystemUid">TBD</param>
    public Gossip(IImmutableDictionary<string, DataEnvelope> updatedData, bool sendBack, long? toSystemUid = null,
        long? fromSystemUid = null)
    {
    ...
    public override bool Equals(object obj)
    {
        return obj is Gossip gossip && Equals(gossip);
    }
...
public sealed class DeltaPropagation : IReplicatorMessage, IEquatable<DeltaPropagation>, ISendingSystemUid
{
    ...
    public bool Equals(DeltaPropagation other)
    {
        if (ReferenceEquals(null, other)) return false;
        if (ReferenceEquals(this, other)) return true;
        if (!Equals(FromNode, other.FromNode) || !ShouldReply == other.ShouldReply ||
            Deltas.Count != other.Deltas.Count)
            return false;

        foreach (var entry in Deltas)
            if (!Equals(other.Deltas.GetValueOrDefault(entry.Key), entry.Value))
                return false;
    ...
    private sealed class NoDelta : IDeltaReplicatedData<IReplicatedData, IReplicatedDelta>,
        IRequireCausualDeliveryOfDeltas
    {
        ...
        IReplicatedData IReplicatedData<IReplicatedData>.Merge(IReplicatedData other)
        {
            return Merge(other);
        }
        ...
        IReplicatedData IDeltaReplicatedData<IReplicatedData, IReplicatedDelta>.MergeDelta(IReplicatedDelta delta)
        {
            return MergeDelta(delta);
        }

        public IReplicatedData ResetDelta()
        {
            return this;
        }

        public IReplicatedData MergeDelta(IReplicatedDelta delta)
        {
            return this;
        }
...
public sealed class DeltaNack : IReplicatorMessage, IDeadLetterSuppression, IEquatable<DeltaNack>
{
    ...
    public bool Equals(DeltaNack other)
    {
        return true;
    }
    ...
    public override bool Equals(object obj)
    {
        return obj is DeltaNack;
    }

    public override int GetHashCode()
    {
        return nameof(DeltaNack).GetHashCode();
    }
}