...
public partial class ActorCell
{
    ...
    private ImmutableDictionary<string, FunctionRef> _functionRefsDoNotCallMeDirectly =
        ImmutableDictionary<string, FunctionRef>.Empty;

    private long
        _nextRandomNameDoNotCallMeDirectly =
            -1; // Interlocked.Increment automatically adds 1 to this value. Allows us to start from 0.

    private IReadOnlyCollection<IActorRef> Children => ChildrenContainer.Children;

    private ImmutableDictionary<string, FunctionRef> FunctionRefs =>
        Volatile.Read(ref _functionRefsDoNotCallMeDirectly);

    /// <summary>
    ///     TBD
    /// </summary>
    protected bool IsNormal => ChildrenContainer.IsNormal;

    /// <summary>
    ///     TBD
    /// </summary>
    protected bool IsTerminating => ChildrenContainer.IsTerminating;

    private bool IsWaitingForChildren =>
        ChildrenContainer is TerminatingChildrenContainer terminating &&
        terminating.Reason is SuspendReason
            .IWaitingForChildren; // This is called isWaitingForChildrenOrNull in AkkaJVM but is used like if returned a bool
    ...
    public IInternalActorRef GetSingleChild(string name)
    {
        if (name.IndexOf('#') < 0)
        {
            ...
            if (ChildrenContainer.TryGetByName(name, out var stats) && stats is ChildRestartStats r) return r.Child;

            if (TryGetFunctionRef(name, out var functionRef)) return functionRef;
        }
        else
        {
            ...
            if (TryGetFunctionRef(s, uid, out var functionRef)) return functionRef;
        }
    ...
    {
        return FunctionRefs.TryGetValue(name, out functionRef);
    }

    internal bool TryGetFunctionRef(string name, int uid, out FunctionRef functionRef)
    {
        return FunctionRefs.TryGetValue(name, out functionRef) &&
               (uid == UndefinedUid || uid == functionRef.Path.Uid);
    }
    ...
    internal bool RemoveFunctionRef(FunctionRef functionRef)
    {
        if (functionRef.Path.Parent != Self.Path)
            throw new InvalidOperationException(
                $"Trying to remove FunctionRef {functionRef.Path} from wrong ActorCell");
    ...
    {
        var refs = Interlocked.Exchange(ref _functionRefsDoNotCallMeDirectly,
            ImmutableDictionary<string, FunctionRef>.Empty);
        foreach (var pair in refs) pair.Value.Stop();
    }
    ...
    ///     If <c>true</c>, then this actor is a system actor and activates a special initialization
    ///     path.
    /// </param>
    /// <param name="name">
    ///     The name of the actor being started. Can be <c>null</c>, and if it is we will automatically
    ///     generate a random name for this actor.
    /// </param>
    /// <exception cref="InvalidActorNameException">
    ///     This exception is thrown if the given <paramref name="name" /> is an invalid actor name.
    /// </exception>
    ...
    private bool SwapChildrenRefs(IChildrenContainer oldChildren, IChildrenContainer newChildren)
    {
        return ReferenceEquals(
            Interlocked.CompareExchange(ref _childrenContainerDoNotCallMeDirectly, newChildren, oldChildren),
            oldChildren);
    }
    ...
    /// <summary>
    ///     Tries to get the stats for the child with the specified name. This ignores children for whom only names have been
    ///     reserved.
    /// </summary>
    ...
    /// <returns>TBD</returns>
    protected bool
        TryGetChildStatsByRef(IActorRef actor, out ChildRestartStats child) //This is called getChildByRef in Akka JVM
    {
