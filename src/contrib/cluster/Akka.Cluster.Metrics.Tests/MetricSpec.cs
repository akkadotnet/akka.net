...
public class MetricSpec
{
    ...
    {
        var metric = NodeMetrics.Types.Metric.Create(StandardMetrics.MemoryUsed, 256L, 0.18).Value;
    ...
    public void New_metric_with_undefined_value_Should_be_None()
    {
        ...
        NodeMetrics.Types.Metric.Create("x", new Try<AnyNumber>(new Exception()), Option<double>.None).HasValue.Should()
            .BeFalse();
    }
...
{
    private readonly Address _node1 = new("akka", "sys", "a", 2554);
    private readonly Address _node2 = new("akka", "sys", "a", 2555);
            ...
            (nodes, _) =>
            {
                return nodes.Select(n =>
                {
                    return new NodeMetrics(n.Address, n.Timestamp,
                        Collector.Sample().Metrics.SelectMany(latest =>
                        {
    ...
    public void NodeMetrics_MetricValues_should_extract_expected_MetricValue_types_for_load_balancing()
    {
        foreach (var node in _nodes)
        {
            ...
            if (heapMemory.HasValue)
            {
                ...
                heapMemory.Value.AvailableSmoothValue.Should()
                    .BeGreaterOrEqualTo(0, "Available memory should be collected");
            }
            ...
            if (cpu.HasValue)
            {
                ...
                cpu.Value.CpuTotalUsage.Should()
                    .BeInRange(0, 1, "Total tracked CPU usage should be between 0% and 100%");
            }
        }
    }
}