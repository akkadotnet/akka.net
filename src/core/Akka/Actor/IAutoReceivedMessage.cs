...
public sealed class Terminated : IAutoReceivedMessage, IPossiblyHarmful, IDeadLetterSuppression,
    INoSerializationVerificationNeeded, IEquatable<Terminated>
{
    /// <summary>
    ///     Initializes a new instance of the <see cref="Terminated" /> class.
    /// </summary>
    /// <param name="actorRef">the watched actor that terminated</param>
    /// <param name="existenceConfirmed">
    ///     is false when the Terminated message was not sent directly from the watched actor, but
    ///     derived from another source, such as when watching a non-local ActorRef, which might not have been resolved
    /// </param>
    /// <param name="addressTerminated">
    ///     the Terminated message was derived from that the remote node hosting the watched actor
    ///     was detected as unreachable
    /// </param>
    ...
    public bool Equals(Terminated other)
    {
        ...
        return Equals(ActorRef, other.ActorRef) && AddressTerminated == other.AddressTerminated &&
    ...
    {
        return obj is Terminated terminated && Equals(terminated);
    }
...
public sealed class Identify : IAutoReceivedMessage, INotInfluenceReceiveTimeout
{
    ...
    ///     A correlating ID used to distinguish multiple <see cref="Identify" /> requests to the same
    ///     receiver.
    /// </param>
...
public sealed class ActorIdentity
{
    ...
    public override int GetHashCode()
    {
        unchecked
        {
            return ((MessageId != null ? MessageId.GetHashCode() : 0) * 397) ^
                   (Subject != null ? Subject.GetHashCode() : 0);
        }
    }
...
///     <para>
///         See also <see cref="Kill" /> which causes the actor to throw an  <see cref="ActorKilledException" /> when
///         it processes the message, which gets handled using the normal supervisor mechanism, and
///         <see cref="IActorContext.Stop" /> which causes the actor to stop without processing any more messages.
///     </para>
/// </summary>
...
///     <see cref="ActorKilledException" /> when it processes the message, which gets handled using the normal supervisor
///     mechanism.
///     <para>
///         See also <see cref="PoisonPill" /> which causes the actor to stop when the <see cref="PoisonPill" />
