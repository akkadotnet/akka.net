...
public class PersistentClusterShardingCustomShardAllocationSpec : ClusterShardingCustomShardAllocationSpec
{
    public PersistentClusterShardingCustomShardAllocationSpec()
        : base(new PersistentClusterShardingCustomShardAllocationSpecConfig(),
            typeof(PersistentClusterShardingCustomShardAllocationSpec))
    {
    }
}
...
public class DDataClusterShardingCustomShardAllocationSpec : ClusterShardingCustomShardAllocationSpec
{
    public DDataClusterShardingCustomShardAllocationSpec()
        : base(new DDataClusterShardingCustomShardAllocationSpecConfig(),
            typeof(DDataClusterShardingCustomShardAllocationSpec))
    {
    }
}

public abstract class
    ClusterShardingCustomShardAllocationSpec : MultiNodeClusterShardingSpec<
    ClusterShardingCustomShardAllocationSpecConfig>
{
    [MultiNodeFact]
    ...
    private void Cluster_sharding_with_custom_allocation_strategy_must_use_specified_region()
    {
        Within(TimeSpan.FromSeconds(30), () =>
        {
            ...
            ExpectMsg(2);
            RunOn(() => { LastSender.Path.Should().Be(_region.Value.Path / "2" / "2"); }, Config.First);
            RunOn(
                () =>
                {
            ...
            ExpectMsg(3);
            RunOn(() => { LastSender.Path.Should().Be(_region.Value.Path / "3" / "3"); }, Config.Second);
    ...
    {
        private IImmutableSet<string> Rebalance = ImmutableHashSet<string>.Empty;
        private IActorRef UseRegion;
    ...
    internal class TestAllocationStrategy : IShardAllocationStrategy
    {
        ...
        public Task<IActorRef> AllocateShard(IActorRef requester, string shardId,
