...
///     using round-robin.
///     This process has the router select from a list of routees in sequential order. When the list has been exhausted,
///     the router iterates
...
public sealed class RoundRobinRoutingLogic : RoutingLogic
{
    ...
    public override Routee Select(object message, Routee[] routees)
    {
        if (routees.Length > 0)
        {
            ...
            var index = (Interlocked.Increment(ref _next) & int.MaxValue) % size;
...
public sealed class RoundRobinPool : Pool
{
    ...
    ///     Initializes a new instance of the <see cref="RoundRobinPool" /> class.
    ///     <note>
    ///         A <see cref="RoundRobinPool" /> configured in this way uses the <see cref="Pool.DefaultSupervisorStrategy" />
    ///         supervisor strategy.
    ///     </note>
    /// </summary>
    ...
    public RoundRobinPool(int nrOfInstances, Resizer resizer) : this(nrOfInstances, resizer, DefaultSupervisorStrategy,
        Dispatchers.DefaultDispatcherId)
    {
    }
...
public sealed class RoundRobinGroup : Group
{
    ...
    public override ISurrogate ToSurrogate(ActorSystem system)
    {
        return new RoundRobinGroupSurrogate { Paths = InternalPaths, RouterDispatcher = RouterDispatcher };
    }
