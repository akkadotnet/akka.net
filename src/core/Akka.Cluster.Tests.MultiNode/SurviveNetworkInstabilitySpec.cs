...
public class SurviveNetworkInstabilitySpecConfig : MultiNodeConfig
{
    ...
    public class Watcher : ReceiveActor
    {
        ...
        public Watcher()
        {
            ...
            Receive<string>(s => s.Equals("boom"), _ => { _targets.ForEach(x => Context.Watch(x)); });
...
public class SurviveNetworkInstabilitySpec : MultiNodeClusterSpec
{
    ...
    protected SurviveNetworkInstabilitySpec(SurviveNetworkInstabilitySpecConfig config) : base(config,
        typeof(SurviveNetworkInstabilitySpec))
    {
        _config = config;
    ...
    public void A_Network_partition_tolerant_cluster_must_heal_after_a_broken_pair()
    {
        Within(45.Seconds(), () =>
        {
            ...
            RunOn(() => { AssertUnreachable(_config.Second); }, _config.First);

            RunOn(() => { AssertUnreachable(_config.First); }, _config.Second);

            RunOn(() => { AssertUnreachable(_config.First, _config.Second); }, _config.Third, _config.Fourth,
                _config.Fifth);
    ...
    public void A_Network_partition_tolerant_cluster_must_heal_after_one_isolated_node()
    {
        Within(45.Seconds(), () =>
        {
            ...
            RunOn(() => { AssertUnreachable(others.ToArray()); }, _config.First);

            RunOn(() => { AssertUnreachable(_config.First); }, others.ToArray());
    ...
    public void A_Network_partition_tolerant_cluster_must_heal_two_isolated_islands()
    {
        Within(45.Seconds(), () =>
        {
            ...
            RunOn(() => { AssertUnreachable(island2.ToArray()); }, island1.ToArray());

            RunOn(() => { AssertUnreachable(island1.ToArray()); }, island2.ToArray());
    ...
    public void A_Network_partition_tolerant_cluster_must_heal_after_unreachable_when_ring_is_changed()
    {
        Within(60.Seconds(), () =>
        {
            ...
            RunOn(() => { AssertUnreachable(others.ToArray()); }, joining.Add(_config.First).ToArray());
    ...
    public void A_Network_partition_tolerant_cluster_must_mark_quarantined_node_with_reachability_status_Terminated()
    {
        Within(60.Seconds(), () =>
        {
            var others = ImmutableArray.Create(_config.First, _config.Third, _config.Fourth, _config.Fifth,
                _config.Sixth, _config.Seventh);
            ...
            RunOn(() =>
            {
                ...
                Sys.ActorSelection(Node(_config.Third) / "user" / "watcher")
                    .Tell(new SurviveNetworkInstabilitySpecConfig.Targets(refs));
            ...
            RunOn(() =>
            {
                ...
                var secondUniqueAddress =
                    Cluster.State.Members.SingleOrDefault(m => m.Address == GetAddress(_config.Second));
                secondUniqueAddress.Should().NotBeNull("2nd node should stay visible");
                secondUniqueAddress?.Status.Should().Be(MemberStatus.Up, "2nd node should be Up");

                // second should be marked with reachability status Terminated
                AwaitAssert(() =>
                    ClusterView.Reachability.Status(secondUniqueAddress?.UniqueAddress).Should()
                        .Be(Reachability.ReachabilityStatus.Terminated));
            }, others.ToArray());
            EnterBarrier("reachability-terminated");

            RunOn(() => { Cluster.Down(GetAddress(_config.Second)); }, _config.Fourth);
    ...
    public void A_Network_partition_tolerant_cluster_must_continue_and_move_Joining_to_Up_after_downing_of_one_half()
    {
        Within(60.Seconds(), () =>
        {
            ...
            RunOn(() => { AssertUnreachable(side2.ToArray()); }, side1.ToArray());

            RunOn(() => { AssertUnreachable(side1.ToArray()); }, side2.ToArray());

            EnterBarrier("unreachable-7");

            RunOn(() => { Cluster.Join(GetAddress(_config.Third)); }, _config.Eighth);

            RunOn(() =>
            {
                foreach (var role2 in side2) Cluster.Down(GetAddress(role2));
            }, _config.Fourth);
            ...
            RunOn(() =>
            {
                ...
                AwaitAssert(() =>
                {
                    ...
                    ClusterView.Members.Where(m => m.Address.Equals(GetAddress(_config.Eighth))).Select(m => m.Status)
                        .FirstOrDefault().Should().Be(MemberStatus.Up);
                });
