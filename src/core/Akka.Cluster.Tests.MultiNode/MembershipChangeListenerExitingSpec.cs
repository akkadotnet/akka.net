...
public class MembershipChangeListenerExitingSpec : MultiNodeClusterSpec
{
    ...
    protected MembershipChangeListenerExitingSpec(MembershipChangeListenerExitingConfig config) : base(config,
        typeof(MembershipChangeListenerExitingSpec))
    {
        _config = config;
    }
    ...
    public void Registered_MembershipChangeListener_must_be_notified_when_new_node_is_exiting()
    {
        ...
        RunOn(() =>
        {
            ...
            Cluster.Subscribe(watcher, typeof(ClusterEvent.IMemberEvent));
            EnterBarrier("registered-listener");
            exitingLatch.Ready();
            removedLatch.Ready();
        }, _config.Second);
    ...
    private class Watcher : ReceiveActor
    {
        ...
        public Watcher(TestLatch exitingLatch, TestLatch removedLatch, Address secondAddress)
        {
            ...
            {
                if (m.Member.Address == secondAddress) exitingLatch.CountDown();
            });
            Receive<ClusterEvent.MemberRemoved>(m =>
            {
                if (m.Member.Address == secondAddress) _removedLatch.CountDown();
            });
        }
    }
}