...
public abstract partial class TestKitBase
{
    ...
    {
        return ExpectMsgAsync<T>(duration, hint, cancellationToken).AsTask().GetAwaiter().GetResult();
    }
        ...
        return InternalExpectMsgAsync<T>(
            ...
            assert: (m, sender) =>
            {
                _assertions.AssertTrue(
                    ...
                    m, typeof(T).FullName, m?.GetType().FullName ?? "null", sender);
            },
            hint: hint,
            cancellationToken: cancellationToken);
    }
    ...
    private void AssertPredicateIsTrueForMessage<T>(Predicate<T> isMessage, T m, string? hint)
    {
        _assertions.AssertTrue(isMessage(m), 
            ...
            "did not match", hint ?? "the predicate to return true", m, typeof(T).FullName, m?.GetType().FullName ?? "null");
    }
    ...
    private T InternalExpectMsg<T>(
        ...
        CancellationToken cancellationToken)
    {
        return InternalExpectMsgAsync(timeout, msgAssert, senderAssert, hint, cancellationToken).AsTask()
            .GetAwaiter().GetResult();
    }
    ...
    {
        ExpectNoMsgAsync(cancellationToken).AsTask().GetAwaiter().GetResult();
    }
    ...
    {
        ExpectNoMsgAsync(duration, cancellationToken).AsTask().GetAwaiter().GetResult();
    }
    ...
    {
        ExpectNoMsgAsync(milliseconds, cancellationToken).AsTask().GetAwaiter().GetResult();
    }
    ...
    public IReadOnlyCollection<T> ExpectMsgAllOf<T>(
        ...
        params T[] messages)
    {
        return ExpectMsgAllOf(max, messages, CancellationToken.None);
    }
    ...
    {
        if (TestKitSettings.LogTestKitCalls == true)
        {
