...
public static class StreamLayout
{
        ...
        IDictionary<object, int> idMap = null)
    {
        ...
        {
            if (!idMap.TryGetValue(obj, out var x))
            {
                x = ids();
                idMap.Add(obj, x);
            }
        ...
        if (print)
        {
            ...
            Console.Out.WriteLine("{0}{1}({2}): {3} {4}", indent, nameof(StreamLayout), shape, ins(inPorts),
                outs(outPorts));
    ...
    public sealed class Atomic : IMaterializedValueNode
    {
        ...
        public override string ToString()
        {
            return $"Atomic({Module.Attributes.GetNameOrDefault(Module.GetType().Name)}[{Module.GetHashCode()}])";
        }
    }
...
public static class IgnorableMaterializedValueComposites
{
    ...
    public static bool Apply(StreamLayout.IMaterializedValueNode composition)
    {
        switch (composition)
        {
            case StreamLayout.Combine or StreamLayout.Transform:
                return false;
            case StreamLayout.Ignore:
                return true;
            default:
            {
    ...
    public static bool Apply(IModule module)
    {
        switch (module)
        {
            case AtomicModule or EmptyModule:
                return true;
...
public interface IModule : IComparable<IModule>
{
    ...
    /// <summary>
    ///     Fuses this Module to <paramref name="that" /> Module by wiring together <paramref name="from" /> and
    ///     <paramref name="to" />,
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        return new CompositeModule(
            ...
            Downstreams.SetItem(from, to),
            Upstreams.SetItem(to, from),
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        return new CompositeModule(
            IsSealed ? ImmutableArray.Create(this as IModule) : SubModules,
            Shape,
            Downstreams,
            Upstreams,
            new StreamLayout.Transform(x => mapFunc((TMat)x), IsSealed
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        return new CompositeModule(
            modulesLeft.Union(modulesRight).ToImmutableArray(),
            new AmorphousShape(
                Shape.Inlets.Concat(other.Shape.Inlets).ToImmutableArray(),
                Shape.Outlets.Concat(other.Shape.Outlets).ToImmutableArray()),
            Downstreams.AddRange(other.Downstreams),
            Upstreams.AddRange(other.Upstreams),
            comp,
            Attributes.None);
    }
...
#pragma warning restore CS0162 // Unreachable code detected
    ...
    {
        return new CompositeModule(
            ImmutableArray.Create(this as IModule),
            Shape,
            /*
    ...
    private bool IsIgnorable(StreamLayout.IMaterializedValueNode computation)
    {
        ...
        if (computation is StreamLayout.Combine or StreamLayout.Transform)
            return false;
    ...
    private bool IsIgnorable(IModule module)
    {
        ...
        {
            AtomicModule or EmptyModule => true,
            CopiedModule copiedModule => IsIgnorable(copiedModule.CopyOf),
            CompositeModule compositeModule => IsIgnorable(compositeModule.MaterializedValueComputation),
            FusedModule fusedModule => IsIgnorable(fusedModule.MaterializedValueComputation),
            _ => throw new NotSupportedException(
                $"Module of type {module.GetType()} is not supported by this method")
        };
    }
}
...
public sealed class CopiedModule : Module
{
    ...
    public override IModule CarbonCopy()
    {
        return new CopiedModule(Shape.DeepCopy(), Attributes, CopyOf);
    }
    ...
    public override string ToString()
    {
        return $"{GetHashCode()} copy of {CopyOf}";
    }
}
...
public sealed class StructuralInfoModule : Module
{
    ...
    public override IModule ReplaceShape(Shape shape)
    {
        ...
        {
            if (!Shape.HasSamePortsAs(shape))
                throw new ArgumentException("StructuralInfoModule requires shape with same ports to replace",
                    nameof(shape));
...
public sealed class FusedModule : Module
{
    ...
    public override IModule WithAttributes(Attributes attributes)
    {
        return new FusedModule(SubModules, Shape, Downstreams, Upstreams, MaterializedValueComputation, attributes,
            Info);
    }
...
public sealed class VirtualProcessor<T> : AtomicReference<object>, IProcessor<T, T>
{
        ...
        void Rec(ISubscriber<T> sub)
        {
            switch (Value)
            {
                ...
                case ISubscription subscription:
                    PrintDebug(
                        $"VirtualProcessor#{_hashCode}({subscription}).Subscribe.Rec({subscriber}) -> Establishing(sub)");
                ...
                case var other:
                    PrintDebug(
                        $"VirtualProcessor#{_hashCode}({other}).Subscribe.Rec({subscriber}): RejectAdditionalSubscriber");
                    ...
                    case ISubscriber<T> subscriber:
                        switch (obj)
                        {
                            case ISubscription sub:
                                PrintDebug(
                                    $"VirtualProcessor#{_hashCode}({subscriber}).OnSubscribe.Rec({obj}) -> Establishing");
                            ...
                            case IPublisher<T> publisher:
                                PrintDebug(
                                    $"VirtualProcessor#{_hashCode}({publisher}).OnSubscribe.Rec({obj}) -> Inert");
                    ...
                    case var state:
                        ...
                        ReactiveStreamsCompliance.TryCancel(subscription,
                            new IllegalStateException(
                                $"Spec violation: VirtualProcessor in wrong state [{state.GetType()}]."));
                        return;
                }
        }
        ...
        void Rec(Exception ex)
        {
            while (true)
                switch (Value)
                {
                    ...
                    case Both both:
                        PrintDebug(
                            $"VirtualProcessor#{_hashCode}(Both({both.Subscriber})).OnError({ex.Message}) -> ErrorPublisher");
                    ...
                    case ISubscriber<T> s:
                        // spec violation
                        PrintDebug(
                            $"VirtualProcessor#{_hashCode}({s}).OnError({ex.Message}) -> Inert: Spec violation.");
                        var inert = GetAndSet(Inert.Instance);
                    ...
                    case var other:
                        ...
                        PrintDebug(
                            $"VirtualProcessor#{_hashCode}({other}).OnError({ex.Message}): Spec violation or cancellation race");
                        return;
                }
        }
                ...
                while (true)
                    switch (Value)
                    {
                        case IHasActualSubscriber h:
                            ...
                            {
                                PrintDebug(
                                    $"VirtualProcessor#{_hashCode}({h.GetType()}({s})).OnNext({element}) threw {e.Message} -> Inert. Spec violation");
                                Value = Inert.Instance;
                                throw new IllegalStateException(
                        ...
                        case ISubscriber<T> subscriber:
                            // spec violation
                            PrintDebug(
                                $"VirtualProcessor#{_hashCode}({subscriber}).OnNext({element}).Rec() -> Inert: Spec violation");
                        ...
                        case var other:
                            PrintDebug(
                                $"VirtualProcessor#{_hashCode}({other}).OnNext({element}).Rec() -> ErrorPublisher");
                            var publisher = new ErrorPublisher<T>(new IllegalStateException(NoDemand),
    ...
    public override string ToString()
    {
        return $"VirtualProcessor({_hashCode})";
    }
    ...
    private void EstablishSubscription(Establishing establishing, ISubscription subscription)
    {
        ...
        try
        {
            ...
            if (CompareAndSet(establishing, new Both(establishing.Subscriber)))
                ...
                switch (Value)
                {
                    case Establishing { OnErrorBuffered: not null } est:
                        // there was an onError while establishing
                        PrintDebug(
                            $"VirtualProcessor#{_hashCode}.EstablishSubscription.Rec(Establishing(OnErrorBuffered)) -> Inert");
                        ReactiveStreamsCompliance.TryOnError(est.Subscriber, est.OnErrorBuffered);
                        Value = Inert.Instance;
                        break;

                    case Establishing { OnCompleteBuffered: true } est:
                        // there was on onComplete while we were establishing
                        PrintDebug(
                            $"VirtualProcessor#{_hashCode}.EstablishSubscription.Rec(Establishing(OnCompleteBuffered)) -> Inert");
                    ...
                    case Inert _:
                        ReactiveStreamsCompliance.TryCancel(subscription,
                            new IllegalStateException("VirtualProcessor was already subscribed to."));
                        break;
    ...
    private sealed class WrappedSubscription : AtomicReference<ISubscriptionState>, ISubscription
    {
        ...
        public void Request(long n)
        {
            if (n < 1)
            {
                ...
                ReactiveStreamsCompliance.TryCancel(_real,
                    new IllegalStateException($"Demand must not be < 1. Was: {n}"));
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    {
                        PrintDebug(
                            $"VirtualProcessor#{_processor._hashCode}.WrappedSubscription({_real}.BufferDemand({n}) PassThrough");
                        _real.Request(n);
                        return;
                    }
                    ...
                    {
                        PrintDebug(
                            $"VirtualProcessor#{_processor._hashCode}.WrappedSubscription({_real}.BufferDemand({n}) buffering");
                        continue;
                    }
        ...
        public void UngateDemandAndRequestBuffered()
        {
            // Ungate demand
            PrintDebug(
                $"VirtualProcessor#{_processor._hashCode}.WrappedSubscription({_real}.UngateDemandAndRequestBuffered()");
    ...
    {
        public Establishing(ISubscriber<T> subscriber, bool onCompleteBuffered = false,
...
internal sealed class VirtualPublisher<T> : AtomicReference<object>, IPublisher<T>, IUntypedVirtualPublisher
{
    ...
    public void Subscribe(ISubscriber<T> subscriber)
    {
        ...
        while (true)
            switch (Value)
            {
                case null when !CompareAndSet(null, subscriber):
                    continue;
                case null:
                    return;
                case IPublisher<T> publisher when CompareAndSet(publisher, Inert.Subscriber):
    ...
    public void RegisterPublisher(IPublisher<T> publisher)
    {
        ...
        while (true)
            switch (Value)
            {
                case null when !CompareAndSet(null, publisher):
                    continue;
                case null:
                    return;
                case ISubscriber<T> subscriber:
...
public abstract class MaterializerSession
{
    ...
    private readonly LinkedList<IDictionary<OutPort, IUntypedPublisher>> _publishersStack = new();

    private readonly LinkedList<IDictionary<InPort, object>> _subscribersStack = new();
    ...
    private IDictionary<StreamLayout.IMaterializedValueNode, LinkedList<IMaterializedValueSource>>
        MaterializedValueSource => _materializedValueSources.First.Value;
    ...
    protected object MaterializeModule(IModule module, Attributes effectiveAttributes)
    {
        ...
        foreach (var submodule in module.SubModules)
        {
            ...
            switch (submodule)
            {
                ...
                case CopiedModule copied:
                    ...
                    ExitScope(copied);
                    break;
                case CompositeModule or FusedModule:
        ...
        {
            Console.WriteLine(
                $"resolving module [{module.GetHashCode()}%08x] computation {module.MaterializedValueComputation}");
        ...
        int spaces)
    {
        ...
        if (!MaterializedValueSource.TryGetValue(node, out var sources)) return result;
    ...
    private static void DoSubscribe(IUntypedPublisher publisher, object subscriberOrVirtual)
    {
...
public sealed class ProcessorModule<TIn, TOut, TMat> : AtomicModule, IProcessorModule
{
    ...
    public override IModule WithAttributes(Attributes attributes)
    {
        return new ProcessorModule<TIn, TOut, TMat>(_createProcessor, attributes);
    }
}