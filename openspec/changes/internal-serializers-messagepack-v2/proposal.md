...
- Durable/persisted state migrates going forward, not retroactively: durable stores become self-describing so new writes may be MessagePack while old records read as protobuf forever, with no bulk rewrite of history (persistence rule from `messagepack-sourcegen-validation` design.md Decision 8.1). Akka.Persistence already stamps the format per record (covers Sharding remember-entities and Akka.Delivery's durable queue); DData's `LmdbDurableStore` is headerless today and gains a `(serializerId, manifest)` header as a prerequisite, treating headerless records as legacy protobuf (design.md Decision 11).
...
- Per-subsystem reference.conf changes only: dual serializer registration (both ids, unconditional) and the binding flip when each subsystem is ready. **No core `Akka` code or config-surface changes.** One prerequisite code change: `LmdbDurableStore` gains a self-describing per-record header so DData durable data recovers by stored format (headerless = legacy protobuf) rather than by the current binding.
