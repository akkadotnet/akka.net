        ...
        SubReceive = new SubReceive(msg =>
        {
            switch (msg)
            {
                case FanIn.OnSubscribe subscribe:
                    _inputs[subscribe.Id].SubReceive.CurrentReceive(new OnSubscribe(subscribe.Subscription));
                    return true;
                ...
                case FanIn.OnNext next:
                {
                    ...
                    Pending(id, true);
                    _receivedInput = true;
                    _inputs[id].SubReceive.CurrentReceive(new OnNext(next.Element));
                    return true;
                }
                ...
                case FanIn.OnComplete complete:
                {
                    ...
                    _inputs[id].SubReceive.CurrentReceive(OnComplete.Instance);
