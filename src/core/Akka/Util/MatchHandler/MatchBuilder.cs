...
internal class MatchBuilder<TItem>
{
    ...
    private readonly List<TypeHandler>
        _typeHandlers = new(); //Contains all handlers, with the handled types and predicates
    ...
    ///     <remarks>
    ///         Note that if a previous added handler handled the item, this <paramref name="handler" /> will not be
    ///         invoked.
    ///     </remarks>
    /// </summary>
    ...
    /// <param name="shouldHandle">
    ///     An optional predicate to test if the item matches. If it returns <c>true</c> the
    ///     <paramref name="handler" /> is invoked.
    /// </param>
    /// <exception cref="InvalidOperationException">
    ///     This exception is thrown if a handler that catches all messages has been added or a partial action has already been
    ///     built.
    /// </exception>
    ...
    ///     This exception is thrown if the given <paramref name="handler" /> cannot handle the given
    ///     <paramref name="handlesType" />.
    /// </exception>
    /// <exception cref="ArgumentOutOfRangeException">
    ///     This exception is thrown if the current state is unknown.
    /// </exception>
    ...
    /// <typeparam name="T">The type that it must match in order for <paramref name="handler" /> to be called.</typeparam>
    /// <param name="handler">
    ///     The handler that is invoked. It should return <c>true</c> if the item sent in matched and was
    ///     handled.
    /// </param>
    /// <exception cref="ArgumentOutOfRangeException">
    ///     This exception is thrown if the current state is unknown.
    ...
    private static void EnsureCanHandleType(Type handlesType)
    {
        if (!_itemType.IsAssignableFrom(handlesType))
            throw new ArgumentException($"The specified type ({handlesType}) must implement {_itemType}",
                nameof(handlesType));
    }
    ...
    private void EnsureCanAdd()
    {
        switch (_state)
        {
            ...
            case State.Built:
                throw new InvalidOperationException(
                    "The partial action has been built. No handler can be added after that.");
            default:
