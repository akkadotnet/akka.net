...
public class ReliableDeliverySpecs : TestKit.Xunit2.TestKit
{
    ...
    public async Task ReliableDelivery_must_illustrate_point_to_point_usage()
    {
        ...
        var testConsumer = Sys.ActorOf(PropsFor(DefaultConsumerDelay, 42, consumerEndProbe.Ref, consumerController),
            $"destination-{_idCount}");
        ...
        var producer = Sys.ActorOf(Props.Create(() => new TestProducer(DefaultProducerDelay, producerController)),
            $"producer-{_idCount}");
    ...
    public async Task ReliableDelivery_must_illustrate_point_to_point_usage_with_ask()
    {
        ...
        var producer =
            Sys.ActorOf(
                Props.Create(() => new TestProducerWithAsk(DefaultProducerDelay, replyProbe.Ref, producerController)),
                $"producer-{_idCount}");
    ...
    private async Task TestWithDelays(TimeSpan producerDelay, TimeSpan consumerDelay)
    {
        ...
        var consumerController = Sys.ActorOf(ConsumerController.Create<Job>(Sys, Option<IActorRef>.None),
            $"consumerController-{_idCount}");
        var testConsumer = Sys.ActorOf(PropsFor(consumerDelay, 42, consumerEndProbe.Ref, consumerController),
            $"destination-{_idCount}");
    ...
    {
        await TestWithDelays(TimeSpan.FromMilliseconds(30), TimeSpan.Zero);
    }

    [Fact]
    public async Task ReliableDelivery_must_work_with_fast_producer_and_slow_consumer()
    {
        await TestWithDelays(TimeSpan.Zero, TimeSpan.FromMilliseconds(30));
    }

    [Fact]
    public async Task ReliableDelivery_must_work_with_fast_producer_and_fast_consumer()
    {
        await TestWithDelays(TimeSpan.Zero, TimeSpan.Zero);
    }
    ...
    public async Task ReliableDelivery_must_allow_replacement_of_destination()
    {
        ...
        var consumerController2 = Sys.ActorOf(ConsumerController.Create<Job>(Sys, Option<IActorRef>.None),
            $"consumerController2-{_idCount}");
        var testConsumer2 =
            Sys.ActorOf(PropsFor(DefaultConsumerDelay, 42, consumerEndProbe2.Ref, consumerController2, true),
                $"destination2-{_idCount}");
    ...
    public async Task ReliableDelivery_must_allow_replacement_of_producer()
    {
        ...
        var producerController1 = Sys.ActorOf(ProducerController.Create<Job>(Sys, ProducerId, Option<Props>.None),
            $"producerController-{_idCount}");
        ...
        var producerController2 = Sys.ActorOf(ProducerController.Create<Job>(Sys, ProducerId, Option<Props>.None),
            $"producerController2-{_idCount}");
