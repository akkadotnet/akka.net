...
public class FutureFlattenSourceSpec : AkkaSpec
{
    ...
    [Fact]
    public async Task
        TaskSource_must_fail_as_the_underlying_task_fails_after_outer_source_materialization_with_no_demand()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var failure = new TestException("foo");
    ...
    public async Task TaskSource_must_fail_when_the_task_source_materialization_fails()
    {
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            ...
            var (innerSourceMat, outerSinkMat) = Source.FromTaskSource(inner).ToMaterialized(Sink.Seq<int>(), Keep.Both)
                .Run(_materializer);
    ...
    private class FailingMatGraphStage : GraphStageWithMaterializedValue<SourceShape<int>, string>
    {
        ...
        public override ILogicAndMaterializedValue<string> CreateLogicAndMaterializedValue(
