...
public class RootGuardianSupervisor : MinimalActorRef
{
    ...
    private readonly Switch _stopped = new();
    ...
    public RootGuardianSupervisor(RootActorPath root, IActorRefProvider provider,
        TaskCompletionSource<Status> terminationPromise, ILoggingAdapter log)
    {
        _log = log;
    ...
    ///     This exception is thrown if the given <paramref name="message" /> is
    ///     undefined.
    /// </exception>
    ...
    public override void Stop()
    {
        ...
        var status = causeOfTermination == null
            ? (Status)new Status.Success(null)
            : new Status.Failure(causeOfTermination);
