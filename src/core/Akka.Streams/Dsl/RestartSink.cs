...
///     They are useful for graphs that need to run for longer than the <see cref="Sink" /> can necessarily guarantee it
///     will, for
...
{
    /// <summary>
    ///     Wrap the given <see cref="Sink" /> with a <see cref="Sink" /> that will restart it when it fails or complete using
    ///     an exponential
    ///     backoff.
    ///     <para>
    ///         This <see cref="Sink" /> will never cancel, since cancellation by the wrapped <see cref="Sink" /> is always
    ///         handled by restarting it.
    ///         The wrapped <see cref="Sink" /> can however be completed by feeding a completion or error into this
    ///         <see cref="Sink" />. When that
    ///         happens, the <see cref="Sink" />, if currently running, will terminate and will not be restarted. This can be
    ///         triggered
    ///         simply by the upstream completing, or externally by introducing a <see cref="IKillSwitch" /> right before this
    ///         <see cref="Sink" /> in the
    ///         graph.
    ///         The restart process is inherently lossy, since there is no coordination between cancelling and the sending of
    ///         messages. When the wrapped <see cref="Sink" /> does cancel, this <see cref="Sink" /> will backpressure, however
    ///         any elements already
    ...
    public static Sink<T, NotUsed> WithBackoff<T, TMat>(Func<Sink<T, TMat>> sinkFactory, TimeSpan minBackoff,
    ...
    {
        return Sink.FromGraph(new RestartWithBackoffSink<T, TMat>(sinkFactory, settings));
    }
}
...
internal sealed class RestartWithBackoffSink<T, TMat> : GraphStage<SinkShape<T>>
{
    ...
    {
        return new Logic(this, inheritedAttributes, "Sink");
    }
    ...
    private sealed class Logic : RestartWithBackoffLogic<SinkShape<T>, T, T>
    {
        ...
        public Logic(RestartWithBackoffSink<T, TMat> stage, Attributes inheritedAttributes, string name)
            : base(name, stage.Shape, stage.In, null, stage.Settings, false)
        {
            _stage = stage;
        ...
        protected override void StartGraph()
        {
            ...
            SubFusingMaterializer.Materialize(Source.FromGraph(sourceOut.Source).To(_stage.SinkFactory()),
