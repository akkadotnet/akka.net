...
public class TestScheduler : IScheduler, IAdvancedScheduler
{
    ...
    ///     TBD
    /// </summary>
    protected DateTimeOffset TimeNow => _now;
    ...
    public void Advance(TimeSpan offset)
    {
        ...
        foreach (var t in tickItems)
        {
            ...
            foreach (var i in removed.Where(r =>
                         r.Repeating && (r.Cancelable == null || !r.Cancelable.IsCancellationRequested)))
                InternalSchedule(null, i.Delay, i.Receiver, i.Message, i.Action, i.Sender, i.Cancelable,
                    i.DeliveryCount);
        }
    }
    ...
    ///     This exception is thrown when the specified <paramref name="when" /> offset is less than the currently tracked
    ///     time.
    /// </exception>
    ...
    private void InternalSchedule(TimeSpan? initialDelay, TimeSpan delay, ICanTell receiver, object message,
        Action action,
        IActorRef sender, ICancelable cancelable, int deliveryCount = 0)
    {
        ...
        while (!_scheduledWork.TryGetValue(scheduledTime, out tickItems))
        {
            ...
            if (_scheduledWork.TryAdd(scheduledTime, tickItems)) break;
        }
    ...
    internal class ScheduledItem
    {
        ...
        /// <param name="sender">TBD</param>
        /// <param name="cancelable">TBD</param>
        public ScheduledItem(TimeSpan initialDelay, TimeSpan delay, ScheduledItemType type, object message,
            Action action, bool repeating, ICanTell receiver,
            IActorRef sender, ICancelable cancelable)
        {
