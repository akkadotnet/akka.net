...
public class VersionVectorBenchmarks
{
    ...
    [Params(100)] public int ClockSize;
    ...
    internal (VersionVector clock, ImmutableSortedSet<UniqueAddress> nodes) CreateVectorClockOfSize(int size)
    {
        ...
        {
            return new UniqueAddress(new Address("akka.tcp", "ClusterSys", "localhost", nodeCount), nodeCount);
        }

        return Enumerable.Range(1, size)
            .Aggregate((Empty, ImmutableSortedSet<UniqueAddress>.Empty),
                (tuple, i) =>
                {
                    var (vc, nodes) = tuple;
    ...
    private void CheckThunkFor(VersionVector vc1, VersionVector vc2, Action<VersionVector, VersionVector> thunk,
        int times)
    {
        var vcc1 = CopyVectorClock(vc1);
        var vcc2 = CopyVectorClock(vc2);
        for (var i = 0; i < times; i++) thunk(vcc1, vcc2);
    }
    ...
    {
        CheckThunkFor(_vcBaseLast, _vcBaseLast, (clock, vectorClock) => CompareTo(clock, vectorClock, Ordering.Same),
            Iterations);
    }
    ...
    {
        CheckThunkFor(_vcBefore, _vcBaseLast, (clock, vectorClock) => CompareTo(clock, vectorClock, Ordering.Before),
            Iterations);
    }
    ...
    {
        CheckThunkFor(_vcAfterLast, _vcBaseLast, (clock, vectorClock) => CompareTo(clock, vectorClock, Ordering.After),
            Iterations);
    }
    ...
    {
        CheckThunkFor(_vcAfterLast, _vcConcurrentLast,
            (clock, vectorClock) => CompareTo(clock, vectorClock, Ordering.Concurrent), Iterations);
    }

    [Benchmark]
    public void VectorClock_comparisons_should_compare_Before_middle()
    {
        CheckThunkFor(_vcBefore, _vcBaseMiddle, (clock, vectorClock) => CompareTo(clock, vectorClock, Ordering.Before),
            Iterations);
    }
    ...
    public void VectorClock_comparisons_should_compare_After_middle()
    {
        CheckThunkFor(_vcAfterMiddle, _vcBaseMiddle,
            (clock, vectorClock) => CompareTo(clock, vectorClock, Ordering.After), Iterations);
    }

    [Benchmark]
    public void VectorClock_comparisons_should_compare_Concurrent_middle()
    {
        CheckThunkFor(_vcAfterMiddle, _vcConcurrentMiddle,
            (clock, vectorClock) => CompareTo(clock, vectorClock, Ordering.Concurrent), Iterations);
    }
    ...
    {
        CheckThunkFor(_vcAfterMiddle, _vcConcurrentMiddle, (clock, vectorClock) => NotEqual(clock, vectorClock),
            Iterations);
    }
