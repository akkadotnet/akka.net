...
internal sealed class ExternalShardAllocationClientImpl : IExternalShardAllocationClient
{
    ...
    public async Task<Done> UpdateShardLocation(string shard, Address location)
    {
        ...
        switch (await _replicator.Ask(
                    Dsl.Update(_key, LWWDictionary<ShardId, string>.Empty, WriteLocal.Instance, null,
                        existing => { return existing.SetItem(_self, shard, location.ToString()); }), _askTimeout))
        {
    ...
    public async Task<ShardLocations> ShardLocations()
    {
        switch (await _replicator.Ask(Dsl.Get(_key, new ReadMajority(_timeout)), _askTimeout))
        {
            ...
            case NotFound _:
                return new ShardLocations(ImmutableDictionary<ShardId, ExternalShardAllocationStrategy.ShardLocation>
                    .Empty);
            case GetFailure _:
            default:
    ...
    {
        _log.Debug("updateShardLocations {0} for {1}", string.Join(", ", locations.Select(i => $"{i.Key}: {i.Value}")),
            _key);
        switch (await _replicator.Ask(Dsl.Update(_key, LWWDictionary<ShardId, string>.Empty, WriteLocal.Instance, null,
                    existing =>
                    {
                        var acc = existing;
