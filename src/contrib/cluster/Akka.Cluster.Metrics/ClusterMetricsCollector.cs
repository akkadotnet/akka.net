...
public class ClusterMetricsCollector : ActorBase
{
    ...
    protected override bool Receive(object message)
    {
        switch (message)
        {
            ...
            case MetricsTick _:
                Sample();
                return true;
            case MetricsGossipEnvelope msg:
                ReceiveGossip(msg);
                return true;
    ...
    {
        _nodes = _nodes.Add(member.Address);
    }
    ...
    private void Gossip()
    {
        SelectRandomNode(_nodes.Remove(_cluster.SelfAddress).ToImmutableArray()).OnSuccess(GossipTo);
    }

    private void GossipTo(Address address)
    {
        SendGossip(address, new MetricsGossipEnvelope(_cluster.SelfAddress, _latestGossip, false));
    }

    private void ReplyGossipTo(Address address)
    {
        SendGossip(address, new MetricsGossipEnvelope(_cluster.SelfAddress, _latestGossip, true));
    }
    ...
    /// </summary>
    private sealed class MetricsTick
    {
    ...
    /// </summary>
    private sealed class GossipTick
    {
