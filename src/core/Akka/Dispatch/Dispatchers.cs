...
{
    [ThreadStatic] private static bool _threadRunning;
    ...
    public void ReadChannel()
    {
        ...
        try
        {
            while (_tasks.TryDequeue(out var runnable)) TryExecuteTask(runnable);
        }
        catch
        {
            // suppress exceptions
...
public sealed class Dispatchers
{
    ...
    ///     of the actual dispatcher config to use. If several ids leading to the same actual dispatcher config is used only
    ///     one
...
public sealed class DispatcherConfigurator : MessageDispatcherConfigurator
{
        ...
        : base(config, prerequisites)
    {
        ...
        var deadlineTime = Config.GetTimeSpan("throughput-deadline-time");
