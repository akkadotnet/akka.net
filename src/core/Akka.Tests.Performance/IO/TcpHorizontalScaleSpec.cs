...
public class TcpHorizontalScaleSpec
{
    ...
    private static readonly IPEndPoint TestEndPoint =
        new(IPAddress.Loopback, ThreadLocalRandom.Current.Next(5000, 11000));

    private static readonly TimeSpan SleepInterval = TimeSpan.FromMilliseconds(100);
    private readonly byte[] _bytes = new byte[50];
    ...
    [PerfBenchmark(
        Description =
            "Measures how quickly and with how much GC overhead a Tcp server <-> Tcp client connection can passe 50B messages",
    ...
    public void Tcp_horizontal_scale_test()
    {
        ...
        {
            var client = _system.ActorOf(Props.Create(() =>
                new TestClient(TestEndPoint, _outboundThroughputCounter, _errorCounter, _bytes)));
            _clients.Add(client);
    ...
    public void Setup(BenchmarkContext context)
    {
        ...
        _listener = _system.ActorOf(
            Props.Create(() =>
                new TestListener(TestEndPoint, _clientConnectedCounter, _inboundThroughputCounter, _errorCounter)),
            "listener");
        var client = _system.ActorOf(Props.Create(() =>
            new TestClient(TestEndPoint, _outboundThroughputCounter, _errorCounter, _bytes)));
        _clients.Add(client);
    ...
    public void Cleanup()
    {
        foreach (var client in _clients) _system.Stop(client);
    ...
    {
        public TestListener(EndPoint endPoint, Counter clientConnectCounter, Counter inboundCounter,
            Counter errorCounter)
        {
            ...
            Receive<Tcp.Connected>(_ =>
            {
                ...
                var handler =
                    Context.ActorOf(Props.Create(() => new TestHandler(connection, inboundCounter, errorCounter)));
