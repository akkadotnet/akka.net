...
public class Index<TKey, TValue> where TValue : IComparable<TValue>
{
    ...
    /// </summary>
    public HashSet<TValue> Values => new(_container.SelectMany(x => x.Value));
    ...
    /// <returns>
    ///     The first <typeparamref name="TValue" /> matching <paramref name="predicate" />. <c>default(TValue)</c>
    ///     otherwise.
    /// </returns>
