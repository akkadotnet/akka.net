...
module Serialization = 
    ...
    let internal exprSerializationSupport (system: ActorSystem) =
        ...
        system.Serialization.AddSerializer("expr", serializer)
...
module Actors =
    ...
    type FunActor<'Message, 'Returned>(actor : Actor<'Message> -> Cont<'Message, 'Returned>) as this = 
        ...
        new(actor : Expr<Actor<'Message> -> Cont<'Message, 'Returned>>) =
            FunActor(QuotationEvaluator.Compile actor)
...
and ExprDecider (expr: Expr<(exn->Directive)>) =
    member __.Expr = expr
    member private this.Compiled = lazy (QuotationEvaluator.Compile this.Expr)
