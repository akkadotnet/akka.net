...
public sealed class TimeoutSettings
{
    ...
    ///     If the node that acquired the leases crashes, how long should the lease be held before
    ///     another owner can get it
    /// </param>
    ...
    /// <summary>
    ///     Lease implementations are expected to time out acquire and release calls or document that they do not implement an
    ///     operation timeout
    /// </summary>
    ...
    private TimeoutSettings Copy(TimeSpan? heartbeatInterval = null, TimeSpan? heartbeatTimeout = null,
        TimeSpan? operationTimeout = null)
    {
        return new TimeoutSettings(heartbeatInterval ?? HeartbeatInterval, heartbeatTimeout ?? HeartbeatTimeout,
            operationTimeout ?? OperationTimeout);
    }
