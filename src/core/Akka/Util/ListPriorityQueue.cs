...
public sealed class ListPriorityQueue
{
    ...
    private readonly Func<object, int> _priorityCalculator;
    ...
    public void Enqueue(Envelope item)
    {
        ...
        while (ci > 0)
        {
            ...
            if (_priorityCalculator(_data[ci].Message).CompareTo(_priorityCalculator(_data[pi].Message)) >= 0)
    ...
    public Envelope Dequeue()
    {
        ...
        while (true)
        {
            ...
            if (rc <= li && _priorityCalculator(_data[rc].Message).CompareTo(_priorityCalculator(_data[ci].Message)) <
                0) // if there is a rc (ci + 1), and it is smaller than left child, use the rc instead
                ci = rc;
            if (_priorityCalculator(_data[pi].Message).CompareTo(_priorityCalculator(_data[ci].Message)) <= 0)
    ...
    public bool IsConsistent()
    {
        ...
        for (var pi = 0; pi < _data.Count; ++pi) // each parent index
        {
            ...
            if (lci <= li && _priorityCalculator(_data[pi].Message).CompareTo(_priorityCalculator(_data[lci].Message)) >
                0) return false; // if lc exists and it's greater than parent then bad.
            if (rci <= li && _priorityCalculator(_data[pi].Message).CompareTo(_priorityCalculator(_data[rci].Message)) >
                0) return false; // check the right child too.
        }
