...
{
    protected CurrentEventsByTagSpec(Config config = null, string actorSystemName = null,
    ...
    public virtual void ReadJournal_query_CurrentEventsByTag_should_find_existing_events()
    {
        if (ReadJournal is not ICurrentEventsByTagQuery queries)
            throw IsTypeException.ForMismatchedType(nameof(ICurrentEventsByTagQuery),
                ReadJournal?.GetType().Name ?? "null");
        ...
        var greenSrc = queries.CurrentEventsByTag("green", NoOffset());
    ...
    public virtual void ReadJournal_query_CurrentEventsByTag_should_complete_when_no_events()
    {
        ...
        var greenSrc = queries.CurrentEventsByTag("pink", NoOffset());
        var probe = greenSrc.RunWith(this.SinkProbe<EventEnvelope>(), Materializer);
    ...
    public virtual void ReadJournal_query_CurrentEventsByTag_should_find_events_from_offset_exclusive()
    {
        ...
        var greenSrc1 = queries.CurrentEventsByTag("green", NoOffset());
        ...
        var greenSrc = queries.CurrentEventsByTag("green", offs);
    ...
    public virtual void ReadJournal_query_CurrentEventsByTag_should_see_all_150_events()
    {
        ...
        var probe = greenSrc.RunWith(this.SinkProbe<EventEnvelope>(), Materializer);
        probe.Request(150);
        foreach (var i in Enumerable.Range(1, 150)) ExpectEnvelope(probe, "a", i, "a green apple", "green");
