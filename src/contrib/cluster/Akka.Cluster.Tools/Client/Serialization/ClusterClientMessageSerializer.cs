...
using Akka.Actor;
using Akka.Cluster.Tools.Client.Serialization.Proto.Msg;
...
public class ClusterClientMessageSerializer : SerializerWithStringManifest
{
    ...
    ///     <see cref="ClusterReceptionist.Contacts" /> | <see cref="ClusterReceptionist.GetContacts" /> |
    ///     <see cref="ClusterReceptionist.Heartbeat" /> | <see cref="ClusterReceptionist.HeartbeatRsp" />
    /// </exception>
    ...
    public override object FromBinary(byte[] bytes, string manifest)
    {
        return manifest switch
        {
            ...
            _ => throw new SerializationException(
                $"Unimplemented deserialization of message with manifest [{manifest}] in serializer {nameof(ClusterClientMessageSerializer)}")
        };
    }
    ...
    {
        var protoMessage = new Contacts();
        foreach (var contactPoint in message.ContactPoints) protoMessage.ContactPoints.Add(contactPoint);
        return protoMessage.ToByteArray();
    }

    private static ClusterReceptionist.Contacts ContactsFromBinary(byte[] binary)
    {
        var proto = Contacts.Parser.ParseFrom(binary);
    ...
    {
        var sendProto = Send.Parser.ParseFrom(bytes);
        return new ClusterClient.Send(sendProto.Path, _payloadSupport.PayloadFrom(sendProto.Payload),
            sendProto.LocalAffinity);
    }

    private byte[] SendToAllToProto(ClusterClient.SendToAll sendToAll)
    {
        var protoMessage = new SendToAll();
    ...
    {
        var sendToAllProto = SendToAll.Parser.ParseFrom(bytes);
    ...
    {
        var publishProto = Publish.Parser.ParseFrom(bytes);
