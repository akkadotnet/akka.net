...
public class DeltaPropagationSelectorSpec
{
    ...
    {
        selfUniqueAddress = new UniqueAddress(new Address("akka", "Sys", "localhost", 4999), 1);
        nodes = Enumerable.Range(2500, 100)
    ...
    public void DeltaPropagationSelector_must_collect_1_when_one_node()
    {
        ...
        selector.CollectPropagations().Should()
            .Equal(ImmutableDictionary.CreateRange(new[]
            {
    ...
    public void DeltaPropagationSelector_must_collect_2_plus_1_when_three_node()
    {
        ...
        selector.CollectPropagations().Should()
            .Equal(ImmutableDictionary.CreateRange(new[]
            {
                new KeyValuePair<Address, DeltaPropagation>(nodes[2], expected)
            }));
        selector.CollectPropagations().Should().BeEmpty();
        selector.CleanupDeltaEntries();
        selector.HasDeltaEntries("A").Should().BeFalse();
    ...
    public void DeltaPropagationSelector_must_keep_track_of_deltas_per_node()
    {
        ...
        selector.CollectPropagations().Should()
            .Equal(ImmutableDictionary.CreateRange(new[]
            {
                new KeyValuePair<Address, DeltaPropagation>(nodes[1], expected3)
            }));
        selector.CollectPropagations().Should().BeEmpty();
        selector.CleanupDeltaEntries();
    ...
    public void DeltaPropagationSelector_must_bump_version_for_each_update()
    {
        ...
        selector.CollectPropagations().Should()
            .Equal(ImmutableDictionary.CreateRange(new[]
            {
                new KeyValuePair<Address, DeltaPropagation>(nodes[0], expected1)
            }));
        ...
        selector.CollectPropagations().Should()
            .Equal(ImmutableDictionary.CreateRange(new[]
            {
                new KeyValuePair<Address, DeltaPropagation>(nodes[0], expected2)
            }));
        selector.CollectPropagations().Should().BeEmpty();
    }
    ...
    public void DeltaPropagationSelector_must_merge_deltas()
    {
        ...
        selector.CollectPropagations().Should()
            .Equal(ImmutableDictionary.CreateRange(new[]
            {
                new KeyValuePair<Address, DeltaPropagation>(nodes[1], expected2)
            }));
        ...
        selector.CollectPropagations().Should()
            .Equal(ImmutableDictionary.CreateRange(new[]
            {
                new KeyValuePair<Address, DeltaPropagation>(nodes[2], expected3)
            }));
        ...
        selector.CollectPropagations().Should()
            .Equal(ImmutableDictionary.CreateRange(new[]
            {
                new KeyValuePair<Address, DeltaPropagation>(nodes[0], expected4)
            }));
        ...
        selector.CollectPropagations().Should()
            .Equal(ImmutableDictionary.CreateRange(new[]
            {
                new KeyValuePair<Address, DeltaPropagation>(nodes[1], expected5)
            }));
    ...
    public void DeltaPropagationSelector_must_calculate_right_slice_size()
    {
        ...
        for (var i = 2; i < 10; i++) selector.NodeSliceSize(i).Should().Be(2);
        for (var i = 10; i < 15; i++) selector.NodeSliceSize(i).Should().Be(3);
        for (var i = 15; i < 20; i++) selector.NodeSliceSize(i).Should().Be(4);
        for (var i = 20; i < 25; i++) selector.NodeSliceSize(i).Should().Be(5);
        for (var i = 25; i < 30; i++) selector.NodeSliceSize(i).Should().Be(6);
        for (var i = 30; i < 35; i++) selector.NodeSliceSize(i).Should().Be(7);
        for (var i = 35; i < 40; i++) selector.NodeSliceSize(i).Should().Be(8);
        for (var i = 40; i < 45; i++) selector.NodeSliceSize(i).Should().Be(9);
        for (var i = 45; i < 200; i++) selector.NodeSliceSize(i).Should().Be(10);
    }
    ...
    public void DeltaPropagationSelector_must_discard_too_large_deltas()
    {
        ...
        var expected = new DeltaPropagation(selfUniqueAddress, false,
            new Dictionary<string, Delta>
            {
                { "A", new Delta(new DataEnvelope(DeltaPropagation.NoDeltaPlaceholder), 1L, 1000L) }
            }.ToImmutableDictionary());
        selector.CollectPropagations().Should()
            .Equal(new Dictionary<Address, DeltaPropagation> { { nodes[0], expected } });
    }
    ...
    private class TestSelector : DeltaPropagationSelector
    {
        ...
        protected override DeltaPropagation CreateDeltaPropagation(
            ImmutableDictionary<string, (IReplicatedData, long, long)> deltas)
        {
            return new DeltaPropagation(selfUniqueAddress, false, deltas
                .Select(kv => new KeyValuePair<string, Delta>(kv.Key,
                    new Delta(new DataEnvelope(kv.Value.Item1), kv.Value.Item2, kv.Value.Item3)))
                .ToImmutableDictionary());
        }
    }
    ...
    {
        public TestSelector2(UniqueAddress selfUniqueAddress, ImmutableArray<Address> allNodes) : base(
            selfUniqueAddress, allNodes)
        {
        }

        public override int NodeSliceSize(int allNodesSize)
        {
            return 1;
        }
    }
