...
public class RemoteWatcher : UntypedActor, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
{
    ...
    /// <summary>
    ///     Nodes that this node is watching, i.e. expecting heartbeats from these nodes. Map of address --> Set(watchee) on
    ///     this address.
    /// </summary>
    ...
    ///     This exception is thrown when the actor system does not have a <see cref="RemoteActorRefProvider" /> enabled in the
    ///     configuration.
    /// </exception>
    ...
    )
    {
        ...
        if (Context.System.AsInstanceOf<ExtendedActorSystem>().Provider is IRemoteActorRefProvider systemProvider)
        ...
        _heartbeatCancelable = Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(heartbeatInterval,
            heartbeatInterval, Self, HeartbeatTick.Instance, Self);
        _failureDetectorReaperCancelable = Context.System.Scheduler.ScheduleTellRepeatedlyCancelable(
            unreachableReaperInterval, unreachableReaperInterval, Self, ReapUnreachableTick.Instance, Self);
    }
    ...
    protected override void OnReceive(object message)
    {
        switch (message)
        {
            ...
            case Terminated t:
            {
                ProcessTerminated(t.ActorRef.AsInstanceOf<IInternalActorRef>(), t.ExistenceConfirmed,
                    t.AddressTerminated);
                break;
            }
            case Stats _:
    ...
    private void ReceiveHeartbeatRsp(int uid)
    {
        ...
        {
            if (!_addressUids.TryGetValue(from, out var addressUid) || addressUid != uid)
                ReWatch(from);
        ...
        foreach (var a in WatchingNodes)
            if (!Unreachable.Contains(a) && !_failureDetector.IsAvailable(a))
            {
                ...
                var nullableAddressUid =
                    _addressUids.TryGetValue(a, out var addressUid) ? new int?(addressUid) : null;
            ...
            if (Watching.TryGetValue(watchee, out var watchers))
                foreach (var watcher in watchers)
                    ...
                    watcher.SendSystemMessage(
                        new DeathWatchNotification(watchee, existenceConfirmed, addressTerminated));
        ...
        foreach (var a in WatchingNodes)
            if (!Unreachable.Contains(a))
            {
                ...
                else
                {
                    ...
                    _scheduler.ScheduleTellOnce(_heartbeatExpectedResponseAfter, Self, new ExpectedFirstHeartbeat(a),
                        Self);
                }
    ...
    private void ReWatch(Address address)
    {
        ...
        foreach (var watchee in WatcheeByNodes[address])
        {
            ...
            watchee.SendSystemMessage(new Watch(watchee,
                watcher)); // ➡➡➡ NEVER SEND THE SAME SYSTEM MESSAGE OBJECT TO TWO ACTORS ⬅⬅⬅
        }
    }
    ...
    public sealed class Stats
    {
        ...
        public Stats(int watching, int watchingNodes, ImmutableHashSet<(IActorRef, IActorRef)> watchingRefs,
        ...
        public override string ToString()
        {
            ...
            return
                $"Stats(watching={Watching}, watchingNodes={WatchingNodes}, watchingRefs=[{FormatWatchingRefs()}], watchingAddresses=[{FormatWatchingAddresses()}])";
        }
        ...
        /// <param name="watchingAddresses">TBD</param>
        /// <returns>TBD</returns>
        public Stats Copy(int watching, int watchingNodes, ImmutableHashSet<(IActorRef, IActorRef)> watchingRefs = null,
            ImmutableHashSet<Address> watchingAddresses = null)
        {
            return new Stats(watching, watchingNodes, watchingRefs ?? WatchingRefs,
                watchingAddresses ?? WatchingAddresses);
        }
    }
}