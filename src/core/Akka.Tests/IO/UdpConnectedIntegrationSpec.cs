...
public class UdpConnectedIntegrationSpec : AkkaSpec
{
    ...
    private async Task<(IActorRef, IPEndPoint)> ConnectUdpAsync(IPEndPoint localAddress, IPEndPoint remoteAddress,
        IActorRef handler)
    {
        var commander = CreateTestProbe();
        ...
        commander.Send(
            ...
            new UdpConnected.Connect(handler, remoteAddress, localAddress,
                new[] { new TestSocketOption(socket => realLocalAddress = (IPEndPoint)socket.LocalEndPoint) }));
    ...
    public async Task The_UDP_connection_oriented_implementation_must_to_send_batch_writes_and_reads()
    {
        ...
        var raw = await serverProbe.ReceiveNAsync(3).ToListAsync();
        ...
        raw = await clientProbe.ReceiveNAsync(3).ToListAsync();
