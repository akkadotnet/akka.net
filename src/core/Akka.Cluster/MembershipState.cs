...
internal sealed class MembershipState : IEquatable<MembershipState>
{
    ...
    /// <summary>
    ///     If there are unreachable members in the cluster with any of these statuses, they will be skipped during convergence
    ///     checks.
    /// </summary>
    ...
    /// <summary>
    ///     If there are unreachable members in the cluster with any of these statuses, they will be pruned from the local
    ///     gossip
    /// </summary>
    ...
    public Member SelfMember
    {
        get
        {
            if (_selfMember == null) _selfMember = LatestGossip.GetMember(SelfUniqueAddress);
    ...
    public UniqueAddress LeaderOf(IImmutableSet<Member> mbrs)
    {
        ...
        var reachableMembers = (reachability.IsAllReachable
                ...
                : mbrs
                    .Where(m => (m.Status != MemberStatus.Down && reachability.IsReachable(m.UniqueAddress)) ||
    ...
    {
        return Copy(LatestGossip.Seen(SelfUniqueAddress));
    }
