...
public abstract class SnapshotStoreProxy : SnapshotStore, IWithUnboundedStash
{
    ...
    protected override async Task DeleteAsync(SnapshotMetadata metadata)
    {
        ...
        try
        {
            ...
            if (response is DeleteSnapshotFailure f) ExceptionDispatchInfo.Capture(f.Cause).Throw();
        }
    ...
    protected override async Task DeleteAsync(string persistenceId, SnapshotSelectionCriteria criteria)
    {
        ...
        try
        {
            ...
            if (response is DeleteSnapshotsFailure f) ExceptionDispatchInfo.Capture(f.Cause).Throw();
        }
        catch (AskTimeoutException)
        {
            throw new TimeoutException();
    ...
    protected override async Task SaveAsync(SnapshotMetadata metadata, object snapshot)
    {
        ...
        try
        {
            ...
            if (response is SaveSnapshotFailure f) ExceptionDispatchInfo.Capture(f.Cause).Throw();
        }
        catch (AskTimeoutException)
        {
            throw new TimeoutException();
