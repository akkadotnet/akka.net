...
akka.remote.dot-netty.tcp.port = {port}";
    ...
    [Fact(DisplayName =
        "Ask operation using selector to a remote actor that expects Status.Failure should return Status.Failure")]
    ...
    [Fact(DisplayName =
        "Ask operation using selector to a remote actor that does not expects Status.Failure should throw the exception cause")]
    public async Task RemoteSelectorFailureExceptionTest()
    {
        _sys2.ActorOf(Props.Create(() => new FailActor()), "fail");
        var selector = _sys1.ActorSelection($"akka.tcp://{_sys2.Name}@localhost:19999/user/fail");

        (await Awaiting(async () => { await selector.Ask<string>("doesn't matter"); }).Should()
            .ThrowAsync<TestException>()).And.Message.Should().Be("BOOM");
    }

    [Fact(DisplayName =
        "Ask operation using deployment to a remote actor that expects Status.Failure should return Status.Failure")]
    ...
    [Fact(DisplayName =
        "Ask operation using deployment to a remote actor that does not expects Status.Failure should throw the exception cause")]
    public async Task RemoteDeploymentFailureExceptionTest()
    {
        ...
        (await Awaiting(async () => { await remote.Ask<string>("doesn't matter"); }).Should()
            .ThrowAsync<TestException>()).And.Message.Should().Be("BOOM");
    }
