...
public class ProtobufSerializer : Serializer
{
    ...
    public override byte[] ToBinary(object obj)
    {
        ...
        if (message != null) return message.ToByteArray();

        throw new ArgumentException(
            $"Can't serialize a non-protobuf message using protobuf [{obj.GetType().TypeQualifiedName()}]");
    }

    /// <inheritdoc />
    public override object FromBinary(byte[] bytes, Type type)
    {
        if (TypeLookup.TryGetValue(type.FullName, out var parser)) return parser.ParseFrom(bytes);
        // MethodParser is not in the cache, look it up with reflection
        var msg = Activator.CreateInstance(type) as IMessage;
        if (msg == null)
            throw new ArgumentException(
                $"Can't deserialize a non-protobuf message using protobuf [{type.TypeQualifiedName()}]");
