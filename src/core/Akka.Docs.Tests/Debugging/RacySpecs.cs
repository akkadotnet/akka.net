...
public class RacySpecs : TestKit
{
    ...
    public void PoorOrderingSpec()
    {
        ...
        {
            return Sys.ActorOf(act => { act.ReceiveAny((o, _) => { actorRef.Forward(o); }); });
        }

        // arrange
        var a1 = Sys.ActorOf(act =>
            act.Receive<string>((str, context) => { context.Sender.Tell(str + "a1"); }), "a1");

        var a2 = CreateForwarder(a1);
        var a3 = CreateForwarder(a1);
