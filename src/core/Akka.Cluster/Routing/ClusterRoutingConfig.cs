...
public sealed class ClusterRouterGroupSettings : ClusterRouterSettingsBase
{
    ...
    /// <param name="allowLocalRoutees">
    ///     When <c>true</c>, allows routees to be deployed locally
    ///     on the node doing the deploying so long as that node also
    ///     satisfies the useRole setting when used.
    /// </param>
    ...
    /// </exception>
    public ClusterRouterGroupSettings(int totalInstances, IEnumerable<string> routeesPaths, bool allowLocalRoutees,
        string useRole = null)
        : base(totalInstances, allowLocalRoutees, useRole)
    {
        ...
        foreach (var path in routeesPaths)
            if (RelativeActorPath.Unapply(path) == null)
                throw new ArgumentException($"routeesPaths [{path}] is not a valid relative actor path.",
                    nameof(routeesPaths));
    }
...
public sealed class ClusterRouterPoolSettings : ClusterRouterSettingsBase
{
    ...
    /// </exception>
    public ClusterRouterPoolSettings(int totalInstances, int maxInstancesPerNode, bool allowLocalRoutees,
        string useRole = null)
        : base(totalInstances, allowLocalRoutees, useRole)
    {
        ...
        if (MaxInstancesPerNode <= 0)
            throw new ArgumentOutOfRangeException(nameof(maxInstancesPerNode),
...
public abstract class ClusterRouterSettingsBase
{
    ...
    protected ClusterRouterSettingsBase(int totalInstances, bool allowLocalRoutees, string useRole)
    {
        ...
        if (useRole == string.Empty)
            throw new ArgumentOutOfRangeException(nameof(useRole), "useRole must be either null or non-empty");
        if (totalInstances <= 0)
            throw new ArgumentOutOfRangeException(nameof(totalInstances),
    ...
    {
        return !string.IsNullOrEmpty(role) ? role : null;
    }
    ...
    {
        var number = config.GetInt("nr-of-instances");
        if (number is 0 or 1)
...
public sealed class ClusterRouterPool : Pool
{
    ...
    /// <summary>
    ///     Dispatcher ID to use for running the "head" actor, which handles supervision, death watch and router management
    ///     messages.
    /// </summary>
    ...
    internal override Routee NewRoutee(Props routeeProps, IActorContext context)
    {
        ...
        var actorRef =
            ((ActorCell)context).AttachChild(Local.EnrichWithPoolDispatcher(routeeProps, context), false, name);
...
public sealed class ClusterRouterGroup : Group
{
    ...
    public ClusterRouterGroup(Group local, ClusterRouterGroupSettings settings)
        : base(settings.AllowLocalRoutees ? settings.RouteesPaths.ToArray() : Enumerable.Empty<string>(),
            local.RouterDispatcher)
    {
        Settings = settings;
        Local = local;
...
internal abstract class ClusterRouterActor : RouterActor
{
    ...
    ///     This exception is thrown when this actor is configured as something other than a <see cref="Pool" /> router or
    ///     <see cref="Group" /> router.
    /// </exception>
    ...
    protected override void OnReceive(object message)
    {
        switch (message)
        {
            case ClusterEvent.CurrentClusterState state:
                Nodes = ImmutableSortedSet.CreateRange(Member.AddressOrdering,
                    state.Members.Where(IsAvailable).Select(x => x.Address));
                AddRoutees();
                break;
...
internal class ClusterRouterGroupActor : ClusterRouterActor
{
    ...
    public (Address, string)? SelectDeploymentTarget()
    {
        ...
        return minPath == null ? null : (minNode.Address, minPath);
    }
...
internal class ClusterRouterPoolActor : ClusterRouterActor
{
    ...
    /// <param name="settings">The settings used to configure the router.</param>
    /// <exception cref="ActorInitializationException">
    ///     This exception is thrown when this actor is configured as something other than a <see cref="Akka.Routing.Pool" />
    ///     router.
    /// </exception>
    ...
    public override void AddRoutees()
    {
        ...
        while (deploymentTarget != null)
        {
            ...
            var deploy = new Deploy(
                ...
                new RemoteScope(deploymentTarget),
