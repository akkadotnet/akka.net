...
public abstract class JournalSerializationSpec : PluginSpec
{
    ...
    public virtual void Journal_should_serialize_Persistent()
    {
        ...
        var messages = new List<AtomicWrite> { new(persistentEvent) };

        Journal.Tell(new WriteMessages(messages, probe.Ref, ActorInstanceId));
        probe.ExpectMsg<WriteMessagesSuccessful>();
        probe.ExpectMsg<WriteMessageSuccess>(m =>
            m.ActorInstanceId == ActorInstanceId && m.Persistent.PersistenceId == Pid);
        ...
        probe.ExpectMsg<ReplayedMessage>(s => s.Persistent.PersistenceId == Pid
                                              ...
                                              && s.Persistent.Payload.AsInstanceOf<TestJournal.MyPayload>().Data
                                                  .Equals(".a."));
    ...
    public virtual void Journal_should_serialize_Persistent_with_string_manifest()
    {
        var probe = CreateTestProbe();
        var persistentEvent =
            new Persistent(new TestJournal.MyPayload2("b", 5), 1L, Pid, null, false, null, WriterGuid);
        ...
        probe.ExpectMsg<ReplayedMessage>(s => s.Persistent.PersistenceId == persistentEvent.PersistenceId
                                              && s.Persistent.SequenceNr == persistentEvent.SequenceNr
                                              && s.Persistent.Payload.AsInstanceOf<TestJournal.MyPayload2>().Data
                                                  .Equals(".b."));
        probe.ExpectMsg<RecoverySuccess>();
    }
    ...
    public virtual void Journal_should_serialize_Persistent_with_EventAdapter_manifest()
    {
        var probe = CreateTestProbe();
        var persistentEvent =
            new Persistent(new TestJournal.MyPayload3("item1"), 1L, Pid, null, false, null, WriterGuid);
    ...
    public virtual void Journal_should_serialize_StateChangeEvent()
    {
        ...
        var messages = new List<AtomicWrite> { new(new Persistent(stateChangeEvent, 1, Pid)) };
    ...
    public class MyPayloadSerializer : Serializer
    {
        ...
        public override byte[] ToBinary(object obj)
        {
            ...
            throw new ArgumentException(
                $"Can't serialize object of type [{obj.GetType()}] in [{nameof(MyPayloadSerializer)}]");
        }
        ...
        public override object FromBinary(byte[] bytes, Type type)
        {
            ...
            throw new ArgumentException(
                $"Unimplemented deserialization of message with manifest [{type}] in serializer {nameof(MyPayloadSerializer)}");
        }
    }
