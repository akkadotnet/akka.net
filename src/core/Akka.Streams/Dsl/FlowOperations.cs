...
namespace Akka.Streams.Dsl
{
    ...
    public interface IFlow<T, TMat>
    {
        ...
        /// <see cref="Flow{TIn,TOut,TMat}.ViaMat{T,TMat2,TMat3}"/> if a different strategy is needed.
        /// </summary>
        IFlow<T2, TMat> Via<T2, TMat2>(IGraph<FlowShape<T, T2>, TMat2> flow);

        IFlow<T, TMat> WithAttributes(Attributes attributes);
    }

    public interface IFlowsOpsMaterialized<TOut, TMat> : IFlow<TOut, TMat>
    {
        /// <summary>
        /// Transform this <see cref="Flow{TIn,TOut,TMat}"/> by appending the given processing steps.
        ...
        /// </summary>
        IFlow<T, TMat3> ViaMat<T, TMat2, TMat3>(IGraph<FlowShape<TOut, T>, TMat2> flow, Func<TMat, TMat2, TMat3> combine);
    }

    public abstract class FlowOps<TOut, TMat> : IFlow<TOut, TMat>
    {
        public IFlow<T2, TMat> Via<T2, TMat2>(IGraph<FlowShape<TOut, T2>, TMat2> flow)
        {
            throw new NotImplementedException();
        }

        public IFlow<TOut, TMat> WithAttributes(Attributes attributes)
        {
            throw new NotImplementedException();
        }
    }
