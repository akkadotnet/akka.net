...
public static class Timers
{
    ...
    {
        return new TimeSpan(
            Math.Min(Math.Max(timeout.Ticks / 8, 100 * TimeSpan.TicksPerMillisecond), timeout.Ticks / 2));
    }
}
...
public sealed class Initial<T> : SimpleLinearGraphStage<T>
{
    ...
    private sealed class Logic : TimerGraphStageLogic, IInHandler, IOutHandler
    {
        ...
        {
            ScheduleOnce(Timers.GraphStageLogicTimer, _stage.Timeout);
        }
    }
    ...
    private sealed class Logic : TimerGraphStageLogic, IInHandler, IOutHandler
    {
        ...
        public Logic(IdleTimeoutBidi<TIn, TOut> stage) : base(stage.Shape)
        {
            ...
            SetHandler(_stage.In2, () =>
                ...
                () => Complete(_stage.Out2));

            SetHandler(_stage.Out2,
                () => Pull(_stage.In2),
                cause => Cancel(_stage.In2, cause));
        }
