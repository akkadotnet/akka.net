...
internal class WriteFailedException : TestException
{
    public WriteFailedException(IEnumerable<AtomicWrite> ps)
        : base(string.Format("write failed for payloads = [{0}]",
            string.Join(", ", ps.SelectMany(x => (IEnumerable<IPersistentRepresentation>)x.Payload))))
    {
    }
}
...
internal class ReplayFailedException : TestException
{
    public ReplayFailedException(IEnumerable<IPersistentRepresentation> ps)
        : base(string.Format("recovery failed after replaying payloads = [{0}]",
            string.Join(", ", ps.Select(x => x.Payload))))
    {
    }
}
...
public class ChaosJournal : AsyncWriteJournal, IMemoryMessages
{
        ...
        long toSequenceNr, long max, Action<IPersistentRepresentation> recoveryCallback)
    {
        ...
        if (ChaosSupportExtensions.ShouldFail(_replayFailureRate))
        {
            ...
            foreach (var persistentRepresentation in top) recoveryCallback(persistentRepresentation);
            promise.SetException(new ReplayFailedException(top));
        }
        else
        {
            foreach (var p in Read(persistenceId, fromSequenceNr, toSequenceNr, max)) recoveryCallback(p);
    ...
    protected override Task DeleteMessagesToAsync(string persistenceId, long toSequenceNr)
    {
        ...
        try
        {
            foreach (var sequenceNr in Enumerable.Range(1, (int)toSequenceNr)) Delete(persistenceId, sequenceNr);
            promise.SetResult(new object());
        }
        catch (Exception e)
        {
