        ...
        public class Watcher : ReceiveActor
        {
            ...
            public Watcher()
            {
                ...
                Receive<Targets>(targets =>
                {
                    ...
                    Sender.Tell(new TargetsRegistered());
                });
        ...
        public void A_Network_partition_tolerant_cluster_must_continue_and_move_Joining_to_Up_after_downing_of_one_half()
        {
            Within(60.Seconds(), () =>
            {
                ...
                RunOn(() =>
                {
                    ...
                    AwaitAssert(() => ClusterView.Members.Select(c => c.Address).Should().BeEquivalentTo(expected));
                    AwaitAssert(() => ClusterView.Members.Where(m => m.Address.Equals(GetAddress(_config.Eighth))).Select(m => m.Status).FirstOrDefault().Should().Be(MemberStatus.Up));
