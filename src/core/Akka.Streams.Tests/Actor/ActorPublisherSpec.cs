...
my-dispatcher1 {
  ...
  mailbox-requirement = ""Akka.Dispatch.IUnboundedMessageQueueSemantics""
    ...
    [Fact]
    public async Task
        ActorPublisher_should_be_able_to_define_a_subscription_timeout_which_is_cancelled_by_the_first_incoming_Subscriber()
    {
    ...
    public async Task ActorPublisher_should_use_dispatcher_from_materializer_settings()
    {
        ...
        {
            var s = this.CreateManualSubscriberProbe<string>();
            var actorRef = Source.ActorPublisher<string>(TestPublisher.Props(TestActor, false))
    ...
    public async Task ActorPublisher_should_use_dispatcher_from_props()
    {
        ...
        await this.AssertAllStagesStoppedAsync(async () =>
        {
            var s = this.CreateManualSubscriberProbe<string>();
            var actorRef = Source
                .ActorPublisher<string>(TestPublisher.Props(TestActor, false).WithDispatcher("my-dispatcher1"))
    ...
    protected override bool Receive(object message)
    {
        switch (message)
        {
            ...
            case ThreadName _:
                _probe.Tell(
                    Context.Props
                        .Dispatcher /*Thread.CurrentThread.Name*/); // TODO fix me when thread name is set by dispatcher
                return true;
            default:
                return false;
        }
...
internal class TimeoutingPublisher : Actors.ActorPublisher<int>
{
    ...
    {
        return Akka.Actor.Props.Create(() => new TimeoutingPublisher(probe, timeout))
    ...
    protected override bool Receive(object message)
    {
        switch (message)
        {
            ...
            case SubscriptionTimeoutExceeded _:
                ...
                Context.System.Scheduler.ScheduleTellOnce(SubscriptionTimeout, Self, PoisonPill.Instance,
                    Nobody.Instance);
                return true;
            default:
                return false;
...
internal class Receiver : ActorSubscriber
{
    ...
    {
        return Akka.Actor.Props.Create(() => new Receiver(probe)).WithDispatcher("akka.test.stream-dispatcher");
    }
...
internal class TotalDemand
{
    ...
    protected bool Equals(TotalDemand other)
    {
