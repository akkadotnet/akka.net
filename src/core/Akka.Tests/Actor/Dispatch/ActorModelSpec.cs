...
public abstract class ActorModelSpec : AkkaSpec
{
    ...
    protected void AssertDispatcher(MessageDispatcherInterceptor dispatcher, long stops)
    {
        ...
        {
            Sys.EventStream.Publish(new Error(ex, dispatcher.ToString(), dispatcher.GetType(),
                $"actual: stops={dispatcher.Stops.Current}, required: stops={stops}"));
            throw;
        }
    }
    ...
    protected void AssertRefDefaultZero(IActorRef actorRef, MessageDispatcher dispatcher = null, long suspensions = 0,
        long resumes = 0, long registers = 0,
    ...
    private void AwaitStarted(IActorRef actorRef)
    {
        AwaitCondition(() =>
        {
    ...
    public void A_dispatcher_must_process_messages_one_at_a_time()
    {
        ...
        AssertCountdown(start, (int)Dilated(TimeSpan.FromSeconds(3.0)).TotalMilliseconds,
        ...
        AssertCountdown(oneAtTime, (int)Dilated(TimeSpan.FromSeconds(1.5)).TotalMilliseconds,
            "Should process message when allowed");
        AssertRefDefaultZero(a, registers: 1, msgsReceived: 3, msgsProcessed: 3, dispatcher: dispatcher);

        Sys.Stop(a);
        AssertRefDefaultZero(a, registers: 1, msgsReceived: 3, msgsProcessed: 3, unregisters: 1,
            dispatcher: dispatcher);
    }
...
#pragma warning disable CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
            Task.Run(() =>
            {
                foreach (var c in Enumerable.Range(1, 20)) a.Tell(new CountDown(counter));
            });
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
    ...
    public void A_dispatcher_should_not_process_messages_for_a_suspended_actor()
    {
        ...
        a.Resume();
        AssertCountdown(done, (int)Dilated(TimeSpan.FromSeconds(3.0)).TotalMilliseconds,
            "Should resume processing of messages when resumed");
        AssertRefDefaultZero(a, dispatcher, registers: 1, msgsReceived: 1, msgsProcessed: 1, suspensions: 1,
            resumes: 1);
    ...
    public void A_dispatcher_must_handle_waves_of_actors()
    {
        ...
        Action<int> flood = num =>
        {
            ...
            Action<IActorDsl> bossActor = c =>
            {
                ...
                {
                    for (var i = 1; i <= num; i++) context.Watch(context.ActorOf(props)).Tell(cachedMessage);
                });

                c.Receive<Terminated>((_, _) => { stopLatch.Signal(); });
            };
            ...
            try
            {
                ...
                dispatcher.Schedule(() => { keepAliveLatch.Wait(waitTime); });
                boss.Tell("run");
    ...
    {
        var dispatcher = InterceptedDispatcher();
        for (var i = 1; i <= 1000; i++) Sys.ActorOf(Props.Empty);
        var a = NewTestActor(dispatcher.Id);
    ...
    private interface IActorModelMessage : INoSerializationVerificationNeeded
    {
    }

    private sealed class TryReply : IActorModelMessage
    {
    ...
    private sealed class Reply : IActorModelMessage
    {
    ...
    private sealed class Forward : IActorModelMessage
    {
    ...
    private sealed class Increment : IActorModelMessage
    {
    ...
    private sealed class AwaitLatch : IActorModelMessage
    {
    ...
    private sealed class CountDownNStop : IActorModelMessage
    {
    ...
    private sealed class Wait : IActorModelMessage
    {
    ...
    private sealed class WaitAck : IActorModelMessage
    {
    ...
    private sealed class Interrupt : IActorModelMessage
    {
    ...
    private sealed class InterruptNicely : IActorModelMessage
    {
    ...
    private sealed class Restart : IActorModelMessage
    {
    ...
    private sealed class DoubleStop : IActorModelMessage
    {
    ...
    private sealed class ThrowException : IActorModelMessage
    {
    ...
    private class DispatcherActor : ReceiveActor
    {
        ...
        private readonly Switch _busy = new();

        private readonly MessageDispatcherInterceptor _interceptor =
            Context.Dispatcher.AsInstanceOf<MessageDispatcherInterceptor>();

        public DispatcherActor()
        {
            Receive<AwaitLatch>(latch =>
            {
                Ack();
                latch.Latch.Wait();
                _busy.SwitchOff();
            ...
            {
                Ack();
                Thread.Sleep((int)wait.Time);
                _busy.SwitchOff();
            });
            Receive<WaitAck>(waitAck =>
            {
                Ack();
                Thread.Sleep((int)waitAck.Time);
                waitAck.Latch.Signal();
                _busy.SwitchOff();
            });
            Receive<Reply>(reply =>
            {
                Ack();
                Sender.Tell(reply.Expect);
                _busy.SwitchOff();
            });
            Receive<TryReply>(tryReply =>
            {
                Ack();
                Sender.Tell(tryReply.Expect, null);
                _busy.SwitchOff();
            });
            Receive<Forward>(forward =>
            {
                Ack();
                forward.To.Forward(forward.Msg);
                _busy.SwitchOff();
            });
            Receive<CountDown>(countDown =>
            {
                Ack();
                countDown.Latch.Signal();
                _busy.SwitchOff();
            });
            Receive<Increment>(increment =>
            {
                Ack();
                increment.Counter.IncrementAndGet();
                _busy.SwitchOff();
            });
            Receive<CountDownNStop>(countDown =>
            {
                Ack();
                countDown.Latch.Signal();
                Context.Stop(Self);
            ...
            }, _ => true); // had to add predicate for compiler magic
            Receive<Interrupt>(_ =>
            {
                Ack();
                Sender.Tell(new Status.Failure(new ActorInterruptedException(cause: new Exception(Ping))));
                _busy.SwitchOff();
            ...
            Receive<ThrowException>(throwEx =>
            {
                Ack();
                _busy.SwitchOff();
                throw throwEx.E;
            }, _ => true);
            Receive<DoubleStop>(_ =>
            {
                Ack();
                Context.Stop(Self);
                Context.Stop(Self);
    ...
    public class MessageDispatcherInterceptor : Dispatcher
    {
        ...
        public MessageDispatcherInterceptor(MessageDispatcherConfigurator configurator, string id, int throughput,
            long? throughputDeadlineTime, ExecutorServiceFactory executorServiceFactory,
            TimeSpan shutdownTimeout) : base(configurator, id, throughput, throughputDeadlineTime,
            executorServiceFactory, shutdownTimeout)
        {
        }
    ...
    protected class MessageDispatcherInterceptorConfigurator : MessageDispatcherConfigurator
    {
        ...
        public MessageDispatcherInterceptorConfigurator(Config config, IDispatcherPrerequisites prerequisites) : base(
...
public class DispatcherModelSpec : ActorModelSpec
{
    ...
    public void A_dispatcher_must_process_messages_in_parallel()
    {
        ...
        AssertCountdown(aStart, (int)Dilated(TimeSpan.FromSeconds(3)).TotalMilliseconds,
            "Should process first message within 3 seconds");

        b.Tell(new CountDown(bParallel));
        AssertCountdown(bParallel, (int)Dilated(TimeSpan.FromSeconds(3)).TotalMilliseconds,
        ...
        SpinWait.SpinUntil(() =>
            a.AsInstanceOf<IInternalActorRef>().IsTerminated && b.AsInstanceOf<IInternalActorRef>().IsTerminated);
