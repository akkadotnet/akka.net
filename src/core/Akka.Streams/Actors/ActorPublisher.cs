...
public abstract class ActorPublisher<T> : ActorBase
{
    ...
    ///     Use this feature in order to avoid leaking actors when you suspect that this Publisher may never get subscribed to
    ///     by some Subscriber.
    /// </summary>
    /// <summary>
    ///     <para>
    ...
    ///         When <see cref="OnComplete" /> is called before any <see cref="ISubscriber{T}" /> has had the chance to
    ///         subscribe
    ...
    public void OnErrorThenStop(Exception cause)
    {
        switch (_lifecycleState)
        {
            ...
            case LifecycleState.PreSubscriber:
                _lifecycleState = LifecycleState.ErrorEmitted;
                _onError = new OnErrorBlock(cause, true);
                if (_subscriber != null)
                    // otherwise onError will be called when the subscription arrives
                    try
                    {
    ...
    public override void AroundPreStart()
    {
        ...
        if (SubscriptionTimeout != Timeout.InfiniteTimeSpan)
        {
            ...
            Context.System.Scheduler.ScheduleTellOnce(SubscriptionTimeout, Self, SubscriptionTimeoutExceeded.Instance,
                Self, _scheduledSubscriptionTimeout);
        }
    }
    ...
    public override void AroundPreRestart(Exception cause, object message)
    {
        ...
        _state.Set(Self,
            new ActorPublisherState.State(UntypedSubscriber.FromTyped(_subscriber), TotalDemand, _lifecycleState));
...
public sealed class ActorPublisherImpl<T> : IPublisher<T>
{
    ...
    {
        if (@ref == null)
            throw new ArgumentNullException(nameof(@ref), "ActorPublisherImpl requires IActorRef to be defined");
        _ref = @ref;
    }
    ...
    public ActorPublisherSubscription(IActorRef @ref)
    {
        if (@ref == null)
            throw new ArgumentNullException(nameof(@ref),
                "ActorPublisherSubscription requires IActorRef to be defined");
        _ref = @ref;
    }
