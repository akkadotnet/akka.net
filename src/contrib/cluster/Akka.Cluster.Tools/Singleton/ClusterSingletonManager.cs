...
///         <see cref="ClusterSingletonManager.Props(Actor.Props, ClusterSingletonManagerSettings)">ClusterSingletonManager.Props</see>
///         to create the <see cref="Actor.Props" /> for the actor.
///     </para>
/// </summary>
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        var minRetries =
            Context.System.Settings.Config.GetInt("akka.cluster.singleton.min-number-of-hand-over-retries");
        if (minRetries < 1)
    ...
    protected override void PreStart()
    {
        ...
        _cluster.Subscribe(Self, InitialStateAsEvents, typeof(MemberRemoved),
            typeof(MemberDowned));

        SetTimer(CleanupTimer, Cleanup.Instance, TimeSpan.FromMinutes(1.0), true);
        ...
        UniqueAddress? oldest)
    {
        ...
        switch (oldest)
        {
            ...
            case not null:
                ...
                SetTimer(TakeOverRetryTimer, new TakeOverRetry(1), _settings.HandOverRetryInterval, false);
            ...
            case null:
                ...
                SetTimer(TakeOverRetryTimer, new TakeOverRetry(1), _settings.HandOverRetryInterval, false);
                return GoTo(ClusterSingletonState.WasOldest)
                    .Using(new WasOldestData(singleton, null));
        }
    }
    ...
    {
        if (singleton == null) return HandleHandOverDone(handOverTo);
            ...
            switch (e.FsmEvent)
            {
                case OldestChangedBuffer.OldestChanged oldestChanged when e.StateData is YoungerData youngerData:
                {
                    ...
                    if (oldestChanged.NewOldest != null && oldestChanged.NewOldest.Equals(_selfUniqueAddress))
                    {
                        ...
                        if (youngerData.Oldest.All(m => _removed.ContainsKey(m))) return TryGotoOldest();
        ...
        When(ClusterSingletonState.BecomingOldest, e =>
        {
            switch (e.FsmEvent)
            {
                ...
                case HandOverDone when e.StateData is BecomingOldestData b:
                {
                    ...
                    if (oldest != null)
                    {
                        if (Sender.Path.Address.Equals(oldest.Address)) return TryGotoOldest();
                ...
                case HandOverRetry handOverRetry when e.StateData is BecomingOldestData becomingOldest:
                {
                    ...
                    if (_cluster.IsTerminated) return Stop();
        ...
        When(ClusterSingletonState.AcquiringLease, e =>
        {
            switch (e.FsmEvent)
            {
                case AcquireLeaseResult alr:
                {
                    ...
                    if (alr.HoldingLease) return GoToOldest();

                    SetTimer(LeaseRetryTimer, LeaseRetry.Instance, _leaseRetryInterval, false);
    ...
    private void ScheduleDelayedMemberRemoved(Member member)
    {
        if (_removalMargin > TimeSpan.Zero)
        {
            ...
            SetTimer("delayed-member-removed-" + member.UniqueAddress, new DelayedMemberRemoved(member),
                _removalMargin, false);
        }
        else
        {
            Self.Tell(new DelayedMemberRemoved(member));
        }
    }
}