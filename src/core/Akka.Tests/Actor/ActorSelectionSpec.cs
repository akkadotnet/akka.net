...
public class ActorSelectionSpec : AkkaSpec
{
    ...
    private async Task<IActorRef> Identify(string path)
    {
        return await Identify(Sys.ActorSelection(path));
    }

    private async Task<IActorRef> Identify(ActorPath path)
    {
        return await Identify(Sys.ActorSelection(path));
    }
    ...
    [Fact]
    public async Task
        An_ActorSystem_must_return_ActorIdentity_None_respectively_for_non_existing_paths_and_DeadLetters()
    {
    ...
    public async Task An_ActorContext_must_find_system_generated_actors()
    {
        ...
        foreach (var actorRef in new[] { Root, System, User }) await Check(actorRef);
    }

    [Fact]
    public async Task
        An_ActorContext_must_return_deadLetters_or_ActorIdentity_None_respectively_for_non_existing_paths()
    {
        ...
        async Task Check(IActorRef looker)
        {
            ...
            {
                new SelectString("a/b/c"), new SelectString("akka://all-systems/Nobody"),
                new SelectPath(User.Path / "hallo"), new SelectPath(looker.Path / "hallo"),
                new SelectPath(looker.Path / new[] { "a", "b" })
            };

            foreach (var query in queries) await CheckOne(looker, query);
        }

        foreach (var actorRef in _all) await Check(actorRef);
    }
    ...
    public void An_ActorSelection_must_compare_equally()
    {
        ...
        new ActorSelection(_c21, "../*/hello").GetHashCode()
            .ShouldBe(new ActorSelection(_c21, "../*/hello").GetHashCode());
        new ActorSelection(_c2, "../*/hello").ShouldNotBe(new ActorSelection(_c21, "../*/hello"));
        new ActorSelection(_c2, "../*/hello").GetHashCode()
            .ShouldNotBe(new ActorSelection(_c21, "../*/hello").GetHashCode());
        new ActorSelection(_c21, "../*/hell").ShouldNotBe(new ActorSelection(_c21, "../*/hello"));
        new ActorSelection(_c21, "../*/hell").GetHashCode()
            .ShouldNotBe(new ActorSelection(_c21, "../*/hello").GetHashCode());
    }
    ...
    [InlineData("/user/foo/bar/*")]
    public async Task Bugfix3420_A_wilcard_ActorSelection_that_selects_no_actors_must_go_to_DeadLetters(
        string actorPathStr)
    {
        var actorA =
            Sys.ActorOf(act => { act.ReceiveAny((o, context) => { context.ActorSelection(actorPathStr).Tell(o); }); },
                "a");
    ...
    public async Task An_ActorSelection_must_identify_actors_with_wildcard_selection_correctly()
    {
        ...
        var received = await probe.ReceiveNAsync(2)
            ...
            .Select(i => i.Subject)
            .ToListAsync();
        received.Should().BeEquivalentTo(b1, b2);
    ...
    public void Selections_are_implicitly_anchored(string data, string pattern, bool isMatch)
    {
        ...
        data.Like(pattern).ShouldBe(isMatch, $"'{pattern}' {predicate} '{data}'");
    }
