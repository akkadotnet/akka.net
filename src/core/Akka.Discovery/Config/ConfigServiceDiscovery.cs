    ...
    public static Dictionary<string, ServiceDiscovery.Resolved> Parse(Configuration.Config config)
    {
        return config.AsEnumerable()
            ...
            .ToDictionary(pair => pair.Key, pair =>
            {
                ...
                {
                    var values = e.Split(':');
                    return new ServiceDiscovery.ResolvedTarget(values[0],
                        int.TryParse(values.Skip(1).FirstOrDefault(), out var i) ? i : default(int?));
                }).ToArray();
...
public class ConfigServiceDiscovery : ServiceDiscovery
{
    ...
    public bool TryRemoveEndpoint(string serviceName, ResolvedTarget target)
    {
        ...
        {
            _log.Info(
                $"Could not find service {serviceName}, adding a new service. Available services: {string.Join(", ", _resolvedServices.Keys)}");
