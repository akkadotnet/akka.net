...
///     <see cref="IShardAllocationStrategy" /> that  allocates new shards to the <see cref="ShardRegion" /> (node) with
///     least
...
///     The <see cref="LeastShardAllocationStrategy" /> picks shards for rebalancing from the <see cref="ShardRegion" />s
///     with most number
...
{
    private static readonly Task<IImmutableSet<ShardId>> _emptyRebalanceResult =
        Task.FromResult<IImmutableSet<ShardId>>(ImmutableHashSet<ShardId>.Empty);
    ...
    /// <param name="relativeLimit">
    ///     Fraction (&lt; 1.0) of total number of (known) shards that will be rebalanced in one
    ///     rebalance round
    /// </param>
    ...
    /// <returns>TBD</returns>
    public override Task<IImmutableSet<ShardId>> Rebalance(
        IImmutableDictionary<IActorRef, IImmutableList<ShardId>> currentShardAllocations,
        IImmutableSet<ShardId> rebalanceInProgress)
    {
            ...
            IImmutableList<RegionEntry> sortedEntries)
        {
            ...
            var countBelowOptimal =
                sortedEntries.Select(entry => Math.Max(0, optimalPerRegion - 1 - entry.ShardIds.Count)).Sum();

            if (countBelowOptimal == 0) return _emptyRebalanceResult;
            ...
            var result = selected.ToImmutable().Take(Math.Min(countBelowOptimal, Limit(numberOfShards)))
                .ToImmutableHashSet();
        ...
        var sortedRegionEntries = RegionEntriesFor(currentShardAllocations)
            .OrderBy(i => i, ShardSuitabilityOrdering.Instance).ToImmutableList();
        if (!IsAGoodTimeToRebalance(sortedRegionEntries)) return _emptyRebalanceResult;

        {
            var numberOfShards = sortedRegionEntries.Select(i => i.ShardIds.Count).Sum();
            var numberOfRegions = sortedRegionEntries.Count;
            if (numberOfRegions == 0 || numberOfShards == 0) return _emptyRebalanceResult;
