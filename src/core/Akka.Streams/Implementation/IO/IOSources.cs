...
namespace Akka.Streams.Implementation.IO
{
    ...
    /// </summary>
    internal sealed class FileSource : SourceModule<ReadOnlyMemory<byte>, Task<IOResult>>
    {
        ...
        /// <returns>TBD</returns>
        public FileSource(FileInfo f, int chunkSize, long startPosition, Attributes attributes, SourceShape<ReadOnlyMemory<byte>> shape) : base(shape)
        {
            if(chunkSize <= 0)
        ...
        /// <param name="shape">TBD</param>
        /// <returns>TBD</returns>
        protected override SourceModule<ReadOnlyMemory<byte>, Task<IOResult>> NewInstance(SourceShape<ReadOnlyMemory<byte>> shape)
        ...
        /// <returns>TBD</returns>
        public override IPublisher<ReadOnlyMemory<byte>> Create(MaterializationContext context, out Task<IOResult> task)
        {
            ...
            return new ActorPublisherImpl<ReadOnlyMemory<byte>>(actorRef);
    ...
    /// </summary>
    internal sealed class InputStreamSource : SourceModule<ReadOnlyMemory<byte>, Task<IOResult>>
    {
        ...
        /// <param name="attributes">TBD</param>
        /// <param name="shape">TBD</param>
        public InputStreamSource(Func<Stream> createInputStream, int chunkSize, Attributes attributes, SourceShape<ReadOnlyMemory<byte>> shape) : base(shape)
        {
        ...
        public override IPublisher<ReadOnlyMemory<byte>> Create(MaterializationContext context, out Task<IOResult> task)
        {
            var materializer = ActorMaterializerHelper.Downcast(context.Materializer);
            var ioResultPromise = TaskEx.NonBlockingTaskCompletionSource<IOResult>();
            IPublisher<ReadOnlyMemory<byte>> pub;
            ...
            try
            {
                ...
                pub = new ActorPublisherImpl<ReadOnlyMemory<byte>>(actorRef);
            }
            catch (Exception ex)
            {
                ioResultPromise.TrySetException(ex);
                pub = new ErrorPublisher<ReadOnlyMemory<byte>>(ex, Attributes.GetNameOrDefault("inputStreamSource"));
            }
